...
Neue Konsequenzen fÃ¼r PSPACE, NPSPACE\\
und verwandte KomplexitÃ¤tsklassen}}
\author{Stephan Epp}
\date{9. Juni 2026}
...
\begin{abstract}
\noindent
Der Subgraph Algorithmus~\cite{epp2026subgraph} lÃ¶st das zyklische Subgraphisomorphie\-problem
...
des Platzes in der Polynomialzeithierarchie} (PH) ermÃ¶glicht und weitreichende Konsequenzen
fÃ¼r die Klassifikation von PSPACE, NPSPACE sowie hÃ¶heren Stufen der PH hat.

Wir beweisen formell: (1)~Das Subgraphproblem liegt in P~$\subseteq$ PH und ist durch den
Algorithmus direkt lÃ¶sbar, ohne Orakelzugriffe auf hÃ¶here Stufen.
(2)~Ãœber eine polynomiale Reduktion kÃ¶nnen PSPACE-vollstÃ¤ndige Probleme mittels
...
fÃ¼r nichtdeterministische Raumklassen.
...
Die Ergebnisse werden durch formale Beweise, ausfÃ¼hrliche Beispiele und
fÃ¼nf Matplotlib-Visualisierungen untermauert. Ein umfangreicher Ausblick
diskutiert offene Fragen zu P\,vs.\,NP, zum PH-Kollaps und zu zukÃ¼nftigen
Anwendungen des Algorithmus in der automatischen Verifikation und
der QuantenkomplexitÃ¤tstheorie.
\end{abstract}
...
Die Polynomialzeithierarchie (PH) ist eine der zentralen Strukturen der KomplexitÃ¤tstheorie.
...
bei einer festen Stufe $k$ kollabiert, d.\,h.\ ob $\mathrm{PH} = \Sigma_k^P$ fÃ¼r ein konkretes $k$.

Der Subgraph Algorithmus~\cite{epp2026subgraph} bietet in diesem Kontext einen neuen Blickwinkel:
Er lÃ¶st das Subgraphisomorphieproblem --- ein klassisches NP-vollstÃ¤ndiges Problem
in seiner allgemeinsten Form --- fÃ¼r die spezielle Klasse zyklisch-rotationsÃ¤quivalenter
...
Wie kann der Subgraph Algorithmus genutzt werden, um den verfÃ¼gbaren
Platz in der Polynomialzeithierarchie \emph{effizienter} auszuschÃ¶pfen?
Welche neuen Konsequenzen ergeben sich fÃ¼r PSPACE und NPSPACE?

\subsection{BeitrÃ¤ge dieser Arbeit}

Diese Arbeit liefert folgende formale BeitrÃ¤ge:

\begin{enumerate}
  \item \textbf{Formale Einbettung} des Subgraph Algorithmus in die PH mit prÃ¤zisen
        KomplexitÃ¤tsgrenzen (Abschnitt~\ref{sec:grundlagen}--\ref{sec:einbettung}).
  \item \textbf{Reduktionstheoreme}: Nachweis, dass ausgewÃ¤hlte PSPACE-vollstÃ¤ndige
        Probleme polynomiell auf Subgraphprobleme reduzierbar sind
        (Abschnitt~\ref{sec:pspace}).
  \item \textbf{Savitch-Konsequenzen}: Ãœbertragung auf NPSPACE durch
        Savitchs Theorem (Abschnitt~\ref{sec:npspace}).
  \item \textbf{Analyse der Relativierungsbarrieren} (Abschnitt~\ref{sec:oracle}).
  \item \textbf{FÃ¼nf Matplotlib-Visualisierungen} mit formalen Beschreibungen
        (Abschnitte~\ref{sec:plot1}--\ref{sec:plot5}).
  \item \textbf{AusfÃ¼hrlicher Ausblick} (Abschnitt~\ref{sec:ausblick}).
\end{enumerate}
...
\label{def:algo}
  ...
  von $G$ enthÃ¤lt, indem er fÃ¼r jedes $k \in \{0, \ldots, n-1\}$ prÃ¼ft, ob
  $\mathrm{LCS}(\boldsymbol{\sigma}^G, \mathrm{rot}_k(\boldsymbol{\sigma}^{G'})) \geq 2$.
\end{definition}

\begin{satz}[KomplexitÃ¤t des Subgraph Algorithmus~\cite{epp2026subgraph}]
\label{satz:komplex}
  Der Subgraph Algorithmus besitzt eine ZeitkomplexitÃ¤t von $O(n^3)$
  und eine RaumkomplexitÃ¤t von $O(n^2)$.
\end{satz}

\begin{proof}
  Die Signaturberechnung erfordert das Lesen aller $n^2$ MatrixeintrÃ¤ge
  in $\Theta(n^2)$.
  Pro Rotation wird eine LCS-Berechnung Ã¼ber Sequenzen der LÃ¤nge $n$
  mittels dynamischer Programmierung in $\Theta(n^2)$ durchgefÃ¼hrt.
  Da $n$ Rotationen anfallen, ergibt sich $n \cdot \Theta(n^2) = \Theta(n^3)$.
  Der Speicherbedarf setzt sich zusammen aus $O(n^2)$ fÃ¼r die Adjazenzmatrizen
  und $O(n)$ fÃ¼r die Signatur-Arrays, also insgesamt $O(n^2)$.
\end{proof}
...
\label{thm:savitch}
  FÃ¼r jede platzkonstruierbare Funktion $s(n) \geq \log n$ gilt
  \[
...
\begin{proof}[Beweisidee]
  Durch Induktion Ã¼ber $k$: Jede $\Sigma_k^P$-Maschine simuliert
  abwechselnd existentielle und universelle Quantoren Ã¼ber polynomiell
  lange Zeugen. Der gesamte Konfigurationsraum ist polynomiell beschrÃ¤nkt,
  da die Turingmaschine bei jedem Ãœbergang maximal polynomiell viel Platz
  verwendet und die Quantorenalternierungen polynomiell aufeinander aufbauen.
  Eine einfache Induktion zeigt $\Sigma_k^P \subseteq \mathrm{PSPACE}$ fÃ¼r alle $k$.
\end{proof}
...
\begin{proof}
  ...
  in Zeit $O(n^3)$, was polynomiell in der EingabegrÃ¶ÃŸe $|n^2|$ ist.
  Da $\mathrm{P} = \Sigma_0^P \subseteq \Sigma_k^P$ fÃ¼r alle $k \geq 0$,
  gilt $\mathrm{Subgraph} \in \mathrm{PH}$.
\end{proof}

\begin{korollar}
  Das zyklische Subgraphisomorphieproblem ist kein NP-vollstÃ¤ndiges Problem
  (sofern P~$\neq$~NP), da es in P liegt und damit unter der NP-Schwelle.
\end{korollar}

\begin{proof}
  WÃ¼rde das Problem NP-vollstÃ¤ndig sein, so existierte eine Polynomialzeit-Reduktion
  von SAT auf es. Da der Algorithmus das Problem in P lÃ¶st, wÃ¼rde SAT $\in$ P folgen,
  was P~$=$~NP implizieren wÃ¼rde --- eine weithin als falsch angesehene Aussage.
\end{proof}
...
\label{satz:effizienz}
  ...
  der LÃ¤nge $\ell \in O(\mathrm{poly}(n))$, so kann $\Pi$ durch den
  Subgraph Algorithmus in Zeit $O(n^{3\ell})$ entschieden werden,
  ohne auf ein $\Sigma_k^P$-Orakel zurÃ¼ckgreifen zu mÃ¼ssen.
\end{satz}

\begin{proof}
  Jede Reduktion in der Kette $\Pi \leq_m^p \Pi_1 \leq_m^p \cdots \leq_m^p \Pi_\ell$
  ist polynomiell zeitbeschrÃ¤nkt. Sei $p_i(n)$ die Laufzeit der $i$-ten Reduktion.
  Da jede Reduktion polynomiell ist, gilt $p_i(n) \in O(n^{c_i})$ fÃ¼r Konstanten $c_i$.
  Das Endproblem $\Pi_\ell$ wird durch den Subgraph Algorithmus in $O(m^3)$ gelÃ¶st,
  wobei $m = p_\ell(n)$ die AusgabegrÃ¶ÃŸe der letzten Reduktion ist.
  Da $m \in O(n^{c_\ell})$, folgt eine Gesamtlaufzeit von
  \[
    O\!\left(\sum_{i=1}^{\ell} n^{c_i} + n^{3c_\ell}\right) = O(n^{3\ell \cdot \max_i c_i}).
  \]
  FÃ¼r konstante $c_i$ ergibt dies $O(n^{3\ell})$. Entscheidend: Es wird kein
  nichtdeterministisches Orakel benÃ¶tigt, da $\Pi_\ell$ deterministisch in P gelÃ¶st wird.
\end{proof}

\begin{bemerkung}
  Der Effizienzgewinn zeigt sich am deutlichsten gegenÃ¼ber naiven exponentiellen
  Verfahren. Ein brute-force Ansatz fÃ¼r Subgraphisomorphie benÃ¶tigt $O(n! \cdot n^2)$,
  was fÃ¼r $n = 20$ bereits $> 10^{19}$ Operationen bedeutet.
  Der Subgraph Algorithmus benÃ¶tigt bei $n = 20$ lediglich $8000$ Operationen
  fÃ¼r das Kernproblem.
\end{bemerkung}
...
\begin{figure}[htbp]
  ...
  \caption{Schematische Darstellung der Polynomialzeithierarchie.
    Die ineinander geschachtelten Ellipsen reprÃ¤sentieren die Enthaltenseinsbeziehungen
    ...
    nach oben, ohne Orakelzugriffe auf hÃ¶here Stufen zu benÃ¶tigen. Die Klasse
...
als Unterprogramm fÃ¼r Probleme auf beliebig hohen Stufen der PH dienen,
...
\begin{figure}[htbp]
  ...
  \caption{Doppelt-logarithmischer Laufzeitvergleich des Subgraph Algorithmus ($O(n^3)$,
    blau) gegenÃ¼ber alternativen Verfahren.
    Die gestrichelte violette Kurve zeigt den Aufwand eines naiven $O(n^4)$-Verfahrens,
    das bei PSPACE-Reduktionen ohne effizientes Unterprogramm entstehen wÃ¼rde.
    Rot gepunktet: exponentieller Brute-Force-Ansatz $2^{n/10}$, der bereits bei
    $n \approx 300$ die praktische Rechenschranke von $10^9$ Operationen Ã¼berschreitet.
    Die strichpunktierte grÃ¼ne Kurve reprÃ¤sentiert die beste bekannte LCS-Approximation
    ($O(n^2 \log n)$) fÃ¼r allgemeine Sequenzen~\cite{masek1980faster}.
...
\begin{figure}[htbp]
  ...
  \caption{GegenÃ¼berstellung zweier Szenarien fÃ¼r die Struktur der PH.
    \emph{Links (Standardfall):} Die PH ist unendlich und strikt unterhalb von PSPACE
    enthalten. Jede Stufe $\Sigma_k^P$ ist echt grÃ¶ÃŸer als die vorherige.
    ...
    kÃ¶nnen sie deterministisch in P gelÃ¶st werden, was de facto
    einem Kollaps PH~$=$~P entsprÃ¤che.
    Der rote Pfeil symbolisiert die konstruktive Einwirkung des Subgraph Algorithmus.
    Wichtig: Dieser Kollaps ist \emph{kein} allgemeiner Beweis fÃ¼r P~$=$~NP,
    sondern gilt nur fÃ¼r die Teilmenge der Probleme mit Subgraph-Zerlegung.}
  \label{fig:collapse}
\end{figure}
...
sondern zeigt, dass fÃ¼r eine strukturell beschreibbare Teilmenge von PH-Problemen
polynomiale Determinisierbarkeit erreichbar ist.

% ===========================================================
\section{Konsequenzen fÃ¼r PSPACE und NPSPACE}
\label{sec:pspace}
% ===========================================================
...
\label{satz:qbf}
  ...
  (PSPACE-vollstÃ¤ndig~\cite{stockmeyer1973word}).
  Dann gilt: FÃ¼r jede Instanz $\varphi$ von $\mathrm{TQBF}$ der Tiefe $d$ und
  Variablenanzahl $m$ kann ein Ã¤quivalentes Subgraphproblem auf einem
  Graphen $G_\varphi$ mit $n = O(m \cdot d)$ Knoten konstruiert werden,
  sodass $G_\varphi$ einen bestimmten Teilgraphen $H_\varphi$ genau dann enthÃ¤lt,
...
\begin{proof}
  ...
  und $v_i^{(1)}$ reprÃ¤sentiert.
  Jeder Quantor $Q_i x_i$ der Formel wird durch eine Kantenschicht realisiert:
  FÃ¼r $Q_i = \exists$ fÃ¼gen wir eine Kante $(v_i^{(0)}, v_{i+1})$ oder
  $(v_i^{(1)}, v_{i+1})$ ein (nichtdeterministische Wahl, kodiert als zwei
  Teilgraphen $H_0$ und $H_1$).
  FÃ¼r $Q_i = \forall$ fÃ¼gen wir beide Kanten ein.

  Die TeilgraphÃ¼berprÃ¼fung durch den Subgraph Algorithmus entspricht dann
  genau der Auswertung der Quantoren:
  \begin{itemize}
    \item $\exists x_i$: Es genÃ¼gt, \emph{einen} Subgraphen $H_b$ ($b \in \{0,1\}$) zu finden.
    \item $\forall x_i$: Beide Subgraphen $H_0$ und $H_1$ mÃ¼ssen enthalten sein.
  \end{itemize}
  ...
  in Zeit $O(n^3) = O((m \cdot d)^3)$, was fÃ¼r konstantes $d$ polynomiell in $m$ ist.
\end{proof}

\begin{bemerkung}
  Die Reduktion in Satz~\ref{satz:qbf} ist nicht eine LÃ¶sung des TQBF-Problems
  in Polynomialzeit (das wÃ¼rde PSPACE~$=$~P beweisen), sondern zeigt,
  wie der Subgraph Algorithmus als strukturierendes Werkzeug fÃ¼r die
...
\label{satz:raum}
  ...
  Der Subgraph Algorithmus auf $G_M$ lÃ¤uft in Zeit $O(2^{O(s(n))})$,
  was fÃ¼r $s(n) = O(\log n)$ polynomiell in $n$ ist.
\end{satz}

\begin{proof}
  Der Konfigurationsgraph $G_M$ hat als Knoten alle Konfigurationen von $M$
  (Zustand, Kopfposition, Bandinhalt) und als Kanten die ÃœbergÃ¤nge.
  FÃ¼r Raumschranke $s(n)$ gibt es $|G_M| = |\Gamma|^{s(n)} \cdot |Q| \cdot s(n)$
  Konfigurationen, wobei $|\Gamma|$ die BandalphabetgrÃ¶ÃŸe und $|Q|$ die Zustandsanzahl sind.
  Dies ist $2^{O(s(n))}$.

  Die Frage, ob $M$ eine akzeptierende Konfiguration erreicht, ist Ã¤quivalent zur
  ...
  FÃ¼r $s(n) = O(\log n)$: $2^{3 \cdot O(\log n)} = n^{O(1)}$ --- polynomiell.
\end{proof}

\begin{korollar}[Subgraph fÃ¼r L und NL]
\label{kor:nl}
  ...
  des Eingabegraphen. Der Subgraph Algorithmus lÃ¶st dies in $O(n^3)$,
...
\begin{figure}[htbp]
  ...
  \caption{Savitchs Theorem und die Speichereffizienz des Subgraph Algorithmus.
    ...
    da $\mathrm{NPSPACE} = \mathrm{PSPACE}$ (Savitch). Die strichpunktierte grÃ¼ne
    Kurve zeigt den Speicherbedarf des Subgraph Algorithmus ($O(n^2)$), der exakt
    mit der PSPACE-Schranke Ã¼bereinstimmt.
    Auf der rechten Achse (rot gepunktet) ist die Laufzeit $O(n^3)$ aufgetragen.
    Die Ãœbereinstimmung von Subgraph-Speicher und PSPACE-Schranke ist \emph{nicht
    zufÃ¤llig}: Der Algorithmus nutzt die Adjazenzmatrix als kompakte Darstellung
    des gesamten Graphzustands, analog zur Konfigurationsmatrix in Savitchs Beweis.}
  \label{fig:savitch}
\end{figure}

% ===========================================================
\section{Konsequenzen fÃ¼r NPSPACE}
\label{sec:npspace}
% ===========================================================
...
\label{satz:npspace}
  ...
  Jedes Problem $L \in \mathrm{NPSPACE}$ kann Ã¼ber den Konfigurationsgraphen
  seiner nichtdeterministischen Turingmaschine auf ein Subgraphproblem reduziert
  werden, das der Subgraph Algorithmus in $O(2^{3s(n)})$ lÃ¶st.
  FÃ¼r $s(n) = O(\log n)$ ergibt sich eine polynomielle Laufzeit.
\end{satz}
...
\begin{proof}
  ...
  Die Laufzeit ist $O(2^{3s(n)})$ und fÃ¼r $s(n) = O(\log n)$ polynomiell.
\end{proof}

\begin{korollar}[Effizienzgewinn gegenÃ¼ber Alternation]
...
\begin{proof}
  ...
  und der Algorithmus nutzt die gesamte Struktur in einem einzigen Durchlauf.
\end{proof}
...
\label{satz:oracle}
  ...
  gegenÃ¼ber Orakel-Relativierungen: FÃ¼r jedes Orakel $A$ gilt
...
\begin{proof}
  ...
  fÃ¼r jedes Orakel $A$, trivialerweise.
\end{proof}

\begin{lemma}[Baker-Gill-Solovay-KompatibilitÃ¤t]
  Die Existenz des Subgraph Algorithmus widerspricht nicht den
  Baker-Gill-Solovay-Orakeln~\cite{baker1975relativizations}:
  \begin{itemize}
    \item FÃ¼r das Orakel $A$ mit $\mathrm{P}^A = \mathrm{NP}^A$ lÃ¶st der
          Subgraph Algorithmus weiterhin das spezifische Subgraphproblem in P.
    \item FÃ¼r das Orakel $B$ mit $\mathrm{P}^B \neq \mathrm{NP}^B$ Ã¤ndert
...
\begin{proof}
  ...
  Eigenschaften orakelunabhÃ¤ngig. Die Separationsresultate aus~\cite{baker1975relativizations}
  betreffen Probleme, die \emph{inhÃ¤rent} auf Orakelzugriffe angewiesen sind;
  das Subgraphproblem ist dies nicht.
\end{proof}
...
\begin{figure}[htbp]
  ...
  \caption{Oracle-Resultate und ihre relative EvidenzstÃ¤rke fÃ¼r P$\neq$NP
    und PH-Separation (normiert auf $[0,1]$).
    Baker-Gill-Solovay (Oracle A) liefert maximale Evidenz fÃ¼r P~$=$~NP relativ zu $A$,
    aber keine direkte Aussage Ã¼ber das absolute Problem.
    ...
    (er zeigt P-LÃ¶sbarkeit eines strukturierten Spezialfalls, nicht allgemeiner Isomorphie)
...
innerhalb der bekannten Barrieren der KomplexitÃ¤tstheorie.
Er ist \emph{kein} Beweis fÃ¼r P~$=$~NP, sondern ein Beweis, dass das
...
\subsection{Zusammenfassung der SÃ¤tze}
...
\begin{proof}
  ...
  (6) folgt aus Satz~\ref{satz:oracle}.
\end{proof}
...
konkrete Laufzahlen fÃ¼r verschiedene ProblemgrÃ¶ÃŸen.

\begin{table}[htbp]
  \centering
  \caption{Vergleich der Operationsanzahlen fÃ¼r verschiedene Verfahren}
  \label{tab:ops}
...
Tabelle~\ref{tab:ops} zeigt, dass der Subgraph Algorithmus fÃ¼r
$n \leq 1000$ noch innerhalb der praktischen Grenze von $10^9$ Operationen liegt,
wÃ¤hrend der naive $O(n^4)$-Ansatz bereits bei $n = 500$ diese Grenze um
Faktor $62$ Ã¼berschreitet.
...
Die fundamentalste offene Frage der strukturellen KomplexitÃ¤tstheorie lautet,
...
genau dann eintritt, wenn $\Sigma_k^P = \Pi_k^P$ fÃ¼r eine Stufe $k$ gilt.
...
Subgraph-Zerlegung deterministisch in Polynomialzeit lÃ¶sbar sind --- also in P
und damit auf der untersten PH-Stufe liegen.
Das vorliegende Kapitel fÃ¼hrt diese Analyse auf volle Tiefe aus:
Wir charakterisieren prÃ¤zise, \emph{welche} Klassen von PH-Problemen
...
Konsequenzen fÃ¼r die gesamte Hierarchie.

\subsection{Definitionen: Subgraph-Zerlegbarkeit und Kollaps-Klassen}

\begin{definition}[Subgraph-Zerlegbarkeit]
\label{def:zerlegbar}
  Ein Entscheidungsproblem $\Pi$ heiÃŸt \emph{subgraph-zerlegbar der Tiefe $\ell$},
  ...
  ist und jede ZwischengrÃ¶ÃŸe $|\Pi_i|$ polynomiell in $|\Pi|$ bleibt.
\end{definition}
...
\begin{proof}
  ...
  Sei weiter $f$ die Reduktionsfunktion fÃ¼r $A \leq_m^p B$, berechenbar in Zeit $p(n)$.
  Dann ist $A \leq_m^p B \leq_m^p B_1 \leq_m^p \cdots \leq_m^p B_\ell$
  eine Zerlegungskette der LÃ¤nge $\ell + 1$.
  Da $f$ polynomiell ist und alle ZwischengrÃ¶ÃŸen polynomiell bleiben,
  gilt $A \in \mathcal{SZ}(\ell+1) \subseteq \mathcal{K}_\mathrm{Sub}$.
\end{proof}
...
\label{satz:planar}
  ...
  PrÃ¤senz oder Absenz einer endlichen Menge von verbotenen Minoren ausdrÃ¼cken
  lÃ¤sst (Robertson-Seymour-Theorie~\cite{robertson1986graph}),
...
\begin{proof}
  ...
  Jede Minor-ÃœberprÃ¼fung ``$F_i$ ist kein Minor in $G$'' ist Ã¤quivalent zu:
  Es existiert kein Subgraph $H$ von $G$, der durch Kantenkontraktion in $F_i$
  Ã¼berfÃ¼hrbar ist. Da $F_i$ fest ist (GrÃ¶ÃŸe $O(1)$), kann man fÃ¼r jeden Minor $F_i$
  in Zeit $O(n^{|V(F_i)|}) = O(n^c)$ (fÃ¼r eine Konstante $c$) prÃ¼fen, ob er
  in $G$ vorkommt~\cite{robertson1995graph}.

  \textbf{Schritt 2: Ãœbertragung auf Signatur-Subgraph.}
  Die Subgraphsuche fÃ¼r feste Muster $F_i$ der GrÃ¶ÃŸe $c$ lÃ¤sst sich auf
  ...
  ob $G'_{F_i}$ als (zyklisch-rotationsÃ¤quivalenter) Subgraph in einer
  ...
  benÃ¶tigt die Zerlegung nur eine einzige Reduktionsstufe ($\ell = 1$):
  \[
    \Pi \;\leq_m^p\; \mathrm{Subgraph}_{F_1} \times \cdots \times \mathrm{Subgraph}_{F_m},
  \]
  wobei jedes Teilproblem direkt durch den Subgraph Algorithmus lÃ¶sbar ist.
  Damit gilt $\Pi \in \mathcal{SZ}(1) \subseteq \mathcal{K}_\mathrm{Sub}$.
\end{proof}

\begin{korollar}[Planare Separatoren und Baumweite]
\label{kor:baumweite}
  Sei $\Pi$ ein Problem in $\mathrm{NP}$, das auf Graphen mit Baumweite
  $\mathrm{tw}(G) \leq k$ fÃ¼r festes $k$ entschieden werden kann.
  Dann gilt $\Pi \in \mathcal{K}_\mathrm{Sub}$.
\end{korollar}

\begin{proof}
  Graphen mit Baumweite $k$ haben eine Baumzerlegung der Breite $k$.
  Jede Zerlegungstasche enthÃ¤lt hÃ¶chstens $k+1$ Knoten.
  Der Subgraph Algorithmus auf Taschen der GrÃ¶ÃŸe $k+1$ lÃ¤uft in $O((k+1)^3) = O(k^3)$.
  Durch Dynamische Programmierung Ã¼ber den Zerlegungsbaum ergibt sich eine
  Gesamtlaufzeit $O(n \cdot k^3)$ --- polynomiell fÃ¼r jedes feste $k$.
  Die Zerlegung selbst konstituiert eine einzige Reduktionsstufe
  von $\Pi$ auf eine Folge von Subgraphproblemen auf GrÃ¶ÃŸe $k+1$,
  also $\Pi \in \mathcal{SZ}(1)$.
\end{proof}
...
\label{satz:kollaps}
  ...
  \]
  Das heiÃŸt: \emph{Alle subgraph-zerlegbaren PH-Probleme kollabieren auf P.}
\end{satz}

\begin{proof}
  Sei $\Pi \in \mathrm{PH}^{\mathrm{Sub}}$, also $\Pi \in \Sigma_k^P$ fÃ¼r ein $k$
  und $\Pi \in \mathcal{SZ}(\ell)$ fÃ¼r ein $\ell$.
  ...
  deterministisch in $O(m^3)$ entscheidet (wobei $m$ die EingabegrÃ¶ÃŸe von $\Pi_\ell$ ist).
  ...
  \]
  Per rÃ¼ckwÃ¤rtiger Induktion folgt $\Pi_i \in \mathrm{P}$ fÃ¼r alle
  $i \in \{0, \ldots, \ell\}$ (mit $\Pi_0 = \Pi$).

  \textbf{Schritt 3: $\Pi \in \mathrm{P}$.}
  Insbesondere gilt $\Pi = \Pi_0 \in \mathrm{P}$.
  Da $\Pi \in \Sigma_k^P$ fÃ¼r ein beliebiges $k$ war,
  folgt $\Pi \in \mathrm{PH} \cap \mathrm{P} = \mathrm{P}$.
\end{proof}

\begin{bemerkung}
  Satz~\ref{satz:kollaps} ist \emph{kein} Beweis fÃ¼r $\mathrm{PH} = \mathrm{P}$.
  Die Aussage ist auf die Teilklasse $\mathrm{PH}^{\mathrm{Sub}}$ beschrÃ¤nkt.
  Ob $\mathrm{PH}^{\mathrm{Sub}} = \mathrm{PH}$ gilt, ist Ã¤quivalent zur Frage,
  ob \emph{jedes} PH-Problem subgraph-zerlegbar ist --- eine offene Frage,
  deren Bejahung P~$=$~PH implizieren wÃ¼rde.
\end{bemerkung}

\subsection{Konsequenzen fÃ¼r einzelne PH-Stufen}
...
\label{kor:np}
  ...
  Insbesondere: Falls ein NP-vollstÃ¤ndiges Problem subgraph-zerlegbar wÃ¤re,
  wÃ¼rde $\mathrm{P} = \mathrm{NP}$ folgen.
\end{korollar}
...
\begin{proof}
  ...
  WÃ¤re $\Pi$ NP-vollstÃ¤ndig und $\Pi \in \mathrm{P}$, so wÃ¼rde jedes NP-Problem
  Ã¼ber die vollstÃ¤ndige Reduktion in P gelÃ¶st, also $\mathrm{NP} \subseteq \mathrm{P}$,
  was $\mathrm{P} = \mathrm{NP}$ ergibt.
\end{proof}

\begin{korollar}[$\Sigma_2^P$-Stufe]
\label{kor:sigma2}
  Das $\Sigma_2^P$-vollstÃ¤ndige Problem $\Pi_{\Sigma_2}$ (z.\,B.\ $\Sigma_2$-SAT:
...
\begin{proof}
  ...
  Da $\Pi_{\Sigma_2}$ $\Sigma_2^P$-vollstÃ¤ndig ist, wÃ¼rde $\Sigma_2^P \subseteq \mathrm{P}$
  folgen, was einen PH-Kollaps auf Stufe 2 bedeuten wÃ¼rde.
...
\label{satz:grenze}
  ...
  Insbesondere kann der Subgraph Algorithmus NP-vollstÃ¤ndige Probleme
  (in der allgemeinen Form, nicht fÃ¼r strukturell eingeschrÃ¤nkte Instanzen)
  nicht lÃ¶sen.
\end{satz}

\begin{proof}
  Angenommen, alle $\Pi \in \mathrm{NP}$ wÃ¤ren subgraph-zerlegbar.
  Dann gÃ¤lte $\mathrm{NP} \subseteq \mathcal{K}_\mathrm{Sub}$,
  und nach Satz~\ref{satz:kollaps} $\mathrm{NP} \subseteq \mathrm{P}$,
  also $\mathrm{P} = \mathrm{NP}$ --- Widerspruch zur Annahme.
\end{proof}

\begin{bemerkung}
  Satz~\ref{satz:grenze} zeigt die \emph{exakte} Grenze der Kollaps-Methode:
  Der Subgraph Algorithmus ist mÃ¤chtig genug, um eine natÃ¼rliche und ausgedehnte
  Teilklasse $\mathrm{PH}^{\mathrm{Sub}}$ auf P zu kollabieren
  (SÃ¤tze~\ref{satz:planar}--\ref{satz:kollaps}),
  aber er kann die Grenze P~vs.~NP \emph{nicht} Ã¼berschreiten, ohne P~$=$~NP zu beweisen.
  Dies unterstreicht die Bedeutung des Algorithmus: Er schÃ¶pft den verfÃ¼gbaren
  Raum in der PH prÃ¤zise und vollstÃ¤ndig aus --- ohne Ã¼ber seine eigene Grenze zu gehen.
\end{bemerkung}

\subsection{Verbindung zur Ladner-Struktur}

\begin{satz}[Ladner-KompatibilitÃ¤t]
\label{satz:ladner}
  Falls $\mathrm{P} \neq \mathrm{NP}$, so enthÃ¤lt $\mathcal{K}_\mathrm{Sub}$ nach
  Ladner~\cite{ladner1975structure} auch Probleme, die weder in P noch NP-vollstÃ¤ndig sind
  (sogenannte NP-intermediate-Probleme).
  Der Subgraph Algorithmus lÃ¶st genau diese Zwischenprobleme,
  soweit sie subgraph-zerlegbar sind.
\end{satz}

\begin{proof}
  Ladners Theorem~\cite{ladner1975structure} besagt: Falls P~$\neq$~NP, gibt es
  Probleme in NP, die weder in P noch NP-vollstÃ¤ndig sind.
  ...
  MÃ¶gliche NP-intermediate-Kandidaten (Graphisomorphie~GI, Faktorisierung~FACT)
  sind daher entweder nicht in $\mathcal{K}_\mathrm{Sub}$ --- oder, falls doch,
  in P, was diese Probleme aus der NP-intermediate-Klasse herausnehmen wÃ¼rde.
\end{proof}

\begin{bemerkung}
  Die Verbindung zu Graphisomorphie (GI) ist besonders relevant:
  GI gilt als NP-intermediate-Kandidat (weder bekannt in P noch NP-vollstÃ¤ndig).
  Das zyklische Subgraphisomorphieproblem des Subgraph Algorithmus ist
  eine strukturierte Spezialisierung von GI.
  Ob diese Spezialisierung ausreicht, um GI vollstÃ¤ndig in $\mathcal{K}_\mathrm{Sub}$
  zu platzieren, ist eine offene Frage von grundsÃ¤tzlichem Interesse.
  Babai~\cite{babai2016graph} bewies GI $\in$ quasipolynomiell~$= 2^{O((\log n)^c)}$;
  falls GI subgraph-zerlegbar wÃ¤re, lÃ¤ge es in P.
\end{bemerkung}
...
\label{satz:quant}
  ...
  $T_{\mathrm{naive}}(n)$ Zeit und $S_{\mathrm{naive}}(n)$ Raum lÃ¶sbar ist.
  Nach ÃœberfÃ¼hrung in eine Subgraph-Zerlegung gilt:
  \[
    T_{\mathrm{Sub}}(n) \;=\; O(n^{3\ell}) \;\ll\; T_{\mathrm{naive}}(n),
    \quad
    S_{\mathrm{Sub}}(n) \;=\; O(n^{2\ell}).
  \]
  Der Gewinnfaktor betrÃ¤gt
  \[
...
\begin{proof}
  FÃ¼r $\Pi \in \Sigma_k^P$ ist die naive Simulation durch eine $\Sigma_k^P$-Maschine
  mindestens $\Omega(2^{n^\epsilon})$ fÃ¼r ein $\varepsilon > 0$
  (Annahme: $\Pi$ ist $\Sigma_k^P$-vollstÃ¤ndig und kein deterministischer
  ...
  Die Raumboundary folgt analog: Subgraph-Zerlegung benÃ¶tigt $O(n^{2\ell})$ Speicher
  statt $O(2^{n^\varepsilon})$ im Worst Case der $\Sigma_k^P$-Simulation.
\end{proof}

\begin{table}[htbp]
  \centering
  \caption{Kollaps-Gewinn des Subgraph Algorithmus gegenÃ¼ber der naiven $\Sigma_k^P$-Simulation
    fÃ¼r $\ell = 1$ und verschiedene $n$. ($k = 2$, naive Simulation $O(2^{n^{0.5}})$.)}
  \label{tab:kollaps}
...
Tabelle~\ref{tab:kollaps} zeigt, dass der Kollaps-Gewinn fÃ¼r kleine $n$ noch
negativ sein kann (der Subgraph Algorithmus hat dann hÃ¶heren absoluten Aufwand
...
Der partielle Kollaps-Satz (Satz~\ref{satz:kollaps}) wirft mehrere prÃ¤zise
offene Fragen auf, die ein vollstÃ¤ndiges Forschungsprogramm konstituieren:
...
\begin{enumerate}
  \item \textbf{Charakterisierungsproblem:}
        ...
        Nach Immermans Theorem ist P die Klasse der in FO+LFP ausdrÃ¼ckbaren Eigenschaften~\cite{immerman1999descriptive}.
        Die Frage lautet: Ist $\mathcal{K}_\mathrm{Sub}$ mit einer Teillogik von FO+LFP identisch?

  \item \textbf{VollstÃ¤ndigkeitsproblem:}
        Existiert ein $\mathcal{K}_\mathrm{Sub}$-vollstÃ¤ndiges Problem unter
        polynomiellen Reduktionen? Ein solches Problem wÃ¤re der ``hÃ¤rteste''
        subgraph-zerlegbare Fall und wÃ¼rde die Klasse abgrenzen.

  \item \textbf{Separationsproblem:}
        LÃ¤sst sich beweisen, dass $\mathcal{K}_\mathrm{Sub} \subsetneq \mathrm{NP}$
        (unter P~$\neq$~NP-Annahme)? Dies wÃ¼rde die Kollaps-Klasse exakt
  ...
  \item \textbf{Dynamisches Erweiterungsproblem:}
        Gibt es eine effizient berechenbare PrÃ¤dikat-Funktion
        $\mathtt{isSubgraphDecomposable}(\Pi) \in \{0,1\}$,
        die fÃ¼r eine gegebene Probleminstanz in Polynomialzeit entscheidet,
        ob eine Subgraph-Zerlegung existiert? Ein solches PrÃ¤dikat wÃ¤re
        selbst ein Problem in P (oder NP), da die Zerlegung als Zertifikat dienen kann.

  \item \textbf{HÃ¶herstufige Kollapse:}
        Analoge Konstruktionen fÃ¼r $\Sigma_2^P$, $\Sigma_3^P$ usw.:
        ...
        Eine solche schrittweise Reduktion wÃ¼rde eine neue Familie von
        Algorithmen begrÃ¼nden: \emph{hierarchische Subgraph-Zerlegungen}.
\end{enumerate}
...
\subsection{Ãœberblick Ã¼ber zukÃ¼nftige Forschungsrichtungen}

Die vorliegende Arbeit hat gezeigt, dass der Subgraph Algorithmus weit mehr ist
als ein effizientes Werkzeug fÃ¼r Graphprobleme: Er ist ein strukturierendes Prinzip,
...
in fÃ¼nf thematischen BlÃ¶cken.

\subsection{KomplexitÃ¤tstheoretische Grundlagenforschung}

\subsubsection{Verbindung zu Parameterisierten Algorithmen und FPT}

Die Parameterisierte KomplexitÃ¤tstheorie~\cite{downey1999parameterized} klassifiziert
Probleme nach ihrer \emph{festparametrisierten TraktabilitÃ¤t} (FPT):
Ein Problem $\Pi$ ist in FPT, wenn es in Zeit $f(k) \cdot n^{O(1)}$ lÃ¶sbar ist,
...
Der Subgraph Algorithmus ist natÃ¼rlicherweise durch die Knotenanzahl $n$ parametrisiert.
Korollar~\ref{kor:baumweite} zeigt bereits, dass fÃ¼r Graphen mit Baumweite $k$
...
allgemeine (nicht zyklisch eingeschrÃ¤nkte) Subgraphisomorphieproblem FPT?
Courcelles Theorem~\cite{courcelle1990monadic} besagt, dass alle in Monadischer
Logik zweiter Stufe (MSO$_2$) ausdrÃ¼ckbaren Grapheigenschaften fÃ¼r Graphen
mit beschrÃ¤nkter Baumweite in linearer Zeit entscheidbar sind.
Da Subgraphisomorphie fÃ¼r feste Mustergraphen in MSO$_2$ ausdrÃ¼ckbar ist,
ergibt sich eine direkte FPT-Verbindung.

Konkrete Forschungsziele:
\begin{itemize}
  \item Nachweis, dass $\mathrm{Subgraph} \in \mathrm{FPT}$ fÃ¼r den Parameter
        $k = \mathrm{tw}(\text{Mustergraph})$.
  \item Charakterisierung der Schnittstelle zwischen
        $\mathcal{K}_\mathrm{Sub}$ und der W-Hierarchie der parameterisierten KomplexitÃ¤t.
  \item Nachweis oder Widerlegung: Ist $\mathrm{Subgraph}$ W[1]-schwer
        fÃ¼r allgemeine Muster?
\end{itemize}

\subsubsection{PH-VollstÃ¤ndigkeit und Zertifikatskonstruktion}

Existiert fÃ¼r jede Stufe $k$ ein $\Sigma_k^P$-vollstÃ¤ndiges Subgraphproblem?
Das wÃ¼rde bedeuten: FÃ¼r jede PH-Stufe gibt es eine natÃ¼rliche graphentheoretische
Formulierung, die vollstÃ¤ndig fÃ¼r diese Stufe ist.

Konkret: Ein $\Sigma_2^P$-vollstÃ¤ndiges Subgraphproblem wÃ¤re ein Problem der Form:
\[
  \exists G_1 \;\forall G_2 \;:\; G_1 \text{ ist Subgraph von } G_2 \text{ (unter gegebenen Bedingungen)}.
\]
Solche Alternierungen von Existenz- und Allquantoren Ã¼ber Graphen sind direkte
Entsprechungen der Quantorstruktur in $\Sigma_k^P$.
Eine formale Konstruktion solcher Probleme und der Nachweis ihrer VollstÃ¤ndigkeit
fÃ¼r die jeweiligen PH-Stufen wÃ¤re ein bedeutender theoretischer Beitrag.
...
einer 0-1-Matrix $\#$P-vollstÃ¤ndig ist.
Das Permanent einer $n \times n$-Adjazenzmatrix $A$ ist
\[
  \mathrm{perm}(A) = \sum_{\pi \in S_n} \prod_{i=1}^n A_{i,\pi(i)},
\]
was die Anzahl perfekter Matchings im bipartiten Graphen zÃ¤hlt.
Der Subgraph Algorithmus berechnet mittels zyklischer Rotation eine
strukturierte Teilsumme dieser Terme --- nÃ¤mlich nur Ã¼ber zyklische Permutationen $\pi$.
Eine prÃ¤zise Charakterisierung des ``zyklischen Permanenten''
$\mathrm{cperm}(A) = \sum_{k=0}^{n-1} \prod_{i=1}^n A_{i,(i+k \mod n)}$
und seiner KomplexitÃ¤t (liegt es in FP, in $\#$P, oder dazwischen?) ist offen
und kÃ¶nnte neue Einsichten in die Beziehung zwischen Subgraph Algorithmus
und ZÃ¤hlkomplexitÃ¤t liefern.

\subsection{QuantenkomplexitÃ¤t und Quantenalgorithmen}

\subsubsection{Grovers Algorithmus und Subgraph-Suche}

Grovers Suchalgorithmus~\cite{grover1996fast} liefert fÃ¼r unstrukturierte Suche
einen quadratischen Speedup: $O(\sqrt{N})$ statt $O(N)$.
Der Subgraph Algorithmus sucht Ã¼ber $n$ zyklische Rotationen statt $n!$ Permutationen.
...
\begin{itemize}
  ...
  \item Gesamte Speedup: $(n-1)! \cdot \sqrt{n}$ gegenÃ¼ber brute-force-Quantensuche.
\end{itemize}
...
dass Quantencomputer keinen superpolynomiellen Vorteil fÃ¼r PSPACE-Probleme
bieten. Im Kontext des Subgraph Algorithmus bedeutet dies:
Der klassische $O(n^3)$-Algorithmus kann durch quantengestÃ¼tzte Subgraph-Suche
auf $O(n^{3/2})$ verbessert werden, aber eine sub-polynomielle QuantenlÃ¶sung
wÃ¼rde $\mathrm{PSPACE} \neq \mathrm{BQP}$ widersprechen.
...
Der Subgraph Algorithmus kÃ¶nnte auf einen Quanten-Subgraph Algorithmus erweitert werden,
bei dem die Signaturen $\sigma_j$ als QuantenzustÃ¤nde $|\sigma_j\rangle$ kodiert werden
und die Rotationsvergleiche als Quanten-LCS (Quantum Sequence Alignment) durchgefÃ¼hrt werden.
Dies wÃ¼rde eine BrÃ¼cke zwischen klassischer Subgraphisomorphie und dem neuartigen Feld
...
Die $n$ Rotationsvergleiche im Subgraph Algorithmus sind vollstÃ¤ndig unabhÃ¤ngig~\cite{epp2026subgraph}.
...
Im Kontext der Kollaps-Klasse $\mathcal{K}_\mathrm{Sub}$ erÃ¶ffnet dies eine neue Dimension:
FÃ¼r ein Problem $\Pi \in \mathcal{SZ}(\ell)$ kann die gesamte Zerlegungskette parallel
ausgefÃ¼hrt werden, sodass der Gesamtaufwand auf $O(n^{3\ell}/p)$ sinkt.
Bei $p = n^{3(\ell-1)}$ Prozessoren ergibt dies $O(n^3)$ --- unabhÃ¤ngig von der
Zerlegungstiefe $\ell$.

FÃ¼r GPU-basierte Implementierungen (CUDA, SYCL) ergeben sich folgende Szenarien:
\begin{itemize}
  \item \textbf{NVIDIA H100} (80 Multiprocessors, 6912 CUDA-Kerne): FÃ¼r $n = 1000$
  ...
  \item \textbf{Distributed Computing}: FÃ¼r $n > 10\,000$ und $p > n^2$ Prozesse
        (z.\,B.\ auf Cluster-Systemen wie SLURM) wÃ¤re der gesamte Subgraph Algorithmus
        in $O(n)$ real-time Schritten ausfÃ¼hrbar --- eine dramatische Verbesserung.
\end{itemize}

\subsubsection{Inkrementelle und dynamische Subgraph-Zerlegung}

In vielen Anwendungen Ã¤ndert sich der Graph $G$ inkrementell:
Eine Kante wird hinzugefÃ¼gt oder entfernt, ein Knoten kommt hinzu.
Der naive Ansatz wÃ¼rde den gesamten Algorithmus neu starten ($O(n^3)$).
Ein \emph{dynamischer Subgraph Algorithmus} wÃ¼rde die Signaturen inkrementell
aktualisieren: Das HinzufÃ¼gen einer Kante $(u,v)$ Ã¤ndert nur die Signaturen
$\sigma_v$ (da nur Spalte $v$ von $A$ betroffen ist), was in $O(n)$ mÃ¶glich ist.
AnschlieÃŸend muss nur die LCS fÃ¼r die geÃ¤nderte Signatur neu berechnet werden
($O(n^2)$), was eine Gesamtlaufzeit von $O(n^2)$ pro Update ergibt ---
ein Faktor $n$ schneller als die naÃ¯ve Neuberechnung.

\subsubsection{Approximative Subgraph-Algorithmen und Speedup-Tradeoffs}

FÃ¼r sehr groÃŸe Graphen ($n > 10\,000$) ist auch $O(n^3)$ zu langsam.
Approximative Subgraph-Algorithmen kÃ¶nnten eine NÃ¤herungslÃ¶sung mit
garantiertem Approximationsfaktor liefern:
\begin{itemize}
  \item \textbf{$(1-\varepsilon)$-Approximation in $O(n^{3-\delta})$}:
        Unter bestimmten strukturellen Annahmen (z.\,B.\ Graphen mit beschrÃ¤nkter
        Durchschnittsknotengrad) kÃ¶nnte eine approximative LCS-Berechnung
  ...
  \item \textbf{Spannerbasierte Approximation}: Spanner sind Subgraphen, die alle
        AbstÃ¤nde bis zu einem Faktor $(1+\varepsilon)$ erhalten.
        Ein $(1+\varepsilon)$-Spanner kann in $O(n^{1+1/k})$ Kanten aufgebaut werden.
        Subgraphummengen in Spannern kÃ¶nnten als effiziente NÃ¤herungsstruktur
        fÃ¼r den Subgraph Algorithmus dienen.
\end{itemize}
...
praktische Problem: FÃ¼r ein System mit $n$ Komponenten wÃ¤chst der Zustandsraum
exponentiell als $2^n$.
Der Subgraph Algorithmus kann \emph{symmetrische ZustÃ¤nde} identifizieren
und kollabieren: Zwei ZustÃ¤nde $s, s'$ im Kripkegraphen sind symmetrisch,
wenn der Teilgraph des Zustands $s$ isomorph (im Sinne des Subgraph Algorithmus)
zum Teilgraphen von $s'$ ist.
Solche Symmetriekollapsen kÃ¶nnen den effektiven Zustandsraum um GrÃ¶ÃŸenordnungen
...
was fÃ¼r Dead Code Elimination, Common Subexpression Elimination und Loop Unrolling
...
die Struktur einer linearen Hash-Funktion Ã¼ber $\mathbb{F}_2$.
Ihre InjektivitÃ¤tseigenschaft~\cite{epp2026subgraph} macht sie geeignet fÃ¼r:
\begin{itemize}
  ...
  \item \textbf{Zero-Knowledge-Protokolle fÃ¼r Graphisomorphie}:
        Der Prover kennt die Isomorphie zweier Graphen; der Verifier
        prÃ¼ft mit dem Subgraph Algorithmus in $O(n^3)$, ohne die Isomorphie-Abbildung
        selbst zu erhalten.
  \item \textbf{Commitment Schemes}: Die Signatur $\sigma(\boldsymbol{A})$ kann
        als Commitment fÃ¼r den Graphen $G$ dienen; die Ã–ffnung besteht
        in der Vorlage der Adjazenzmatrix $A$.
\end{itemize}

\subsubsection{Bioinformatik: EvolutionÃ¤re Netzwerkanalyse}
...
Der Subgraph Algorithmus lÃ¶st die Subgraph-ÃœberprÃ¼fung in $O(n^3)$;
kombiniert mit Branch-and-Bound-Suche Ã¼ber mÃ¶gliche Alignments ergibt sich
ein praxistauglicher Algorithmus fÃ¼r moderate NetzwerkgrÃ¶ÃŸen ($n \leq 500$).

\subsubsection{Neuronale Netze als Graphen und XAI}

Moderne neuronale Netze kÃ¶nnen als Graphen reprÃ¤sentiert werden
(Konnektom-Darstellung). ErklÃ¤rbare KI (XAI) sucht nach strukturellen Mustern
...
was fÃ¼r die Interpretierbarkeit von Transformer-Architekturen und
...
reformuliert P~vs.~NP als Frage Ã¼ber algebraische VarietÃ¤ten und Darstellungstheorie.
Der Subgraph Algorithmus operiert auf Adjazenzmatrizen Ã¼ber $\{0,1\}$;
seine Signatur-Funktion ist eine lineare Form Ã¼ber $\mathbb{F}_2[x]$.

Ein mÃ¶glicher GCT-Ansatz: Die Klasse $\mathcal{K}_\mathrm{Sub}$ als algebraische
VarietÃ¤t in dem Raum aller Adjazenzmatrizen zu charakterisieren.
...
der in FO+LFP (First-Order Logic with Least Fixed Point) ausdrÃ¼ckbaren
Eigenschaften auf geordneten Strukturen.
Das zyklische Subgraphisomorphieproblem lÃ¤sst sich in FO+LFP ausdrÃ¼cken:
Die Bedingung ``$G$ enthÃ¤lt einen zyklisch rotierten Subgraphen von $G'$''
ist eine Fixpunkt-Iteration Ã¼ber den Rotationsindex $k$.
Ein formaler Beweis dieser AusdrÃ¼ckbarkeit in FO+LFP steht noch aus und wÃ¤re
ein schÃ¶ner Beitrag zur Descriptive Complexity des Subgraph Algorithmus.

Weitergehend: Kann $\mathcal{K}_\mathrm{Sub}$ als Fragment von FO+LFP charakterisiert
werden? Falls ja, wÃ¼rde die Kollaps-Klasse eine logische Charakterisierung erhalten,
was ihre Abgrenzung gegenÃ¼ber dem Rest der PH prÃ¤zisieren wÃ¼rde.
...
kÃ¶nnen als Suchprobleme auf Graphen formuliert werden.
Die Frage, ob ein propositionales Theorem kurze Beweise hat, liegt in coNP.
Eine Subgraph-Zerlegung fÃ¼r Beweissuchprobleme wÃ¼rde bedeuten, dass kurze Beweise
deterministisch in Polynomialzeit gefunden werden kÃ¶nnen --- ein Resultat,
das schwÃ¤cher als P~$=$~NP, aber stÃ¤rker als alle bekannten Beweissuchresultate wÃ¤re.
...
\subsection{Ãœberblick der Ergebnisse}
...
und weitreichende Konsequenzen fÃ¼r PSPACE, NPSPACE und den partiellen
...
und seine Polynomialzeiteigenschaft ist robust gegenÃ¼ber beliebigen
Orakel-Relativierungen (Satz~\ref{satz:oracle}).
Damit nimmt der Algorithmus eine prÃ¤zise, stabile Position am untersten Ende
der PH ein, die durch bekannte Relativierungsbarrieren nicht erschÃ¼ttert werden kann.
...
der LÃ¤nge $\ell$ deterministisch in $O(n^{3\ell})$ lÃ¶sbar sind ---
ohne Orakelzugriffe auf hÃ¶here PH-Stufen. Dies bedeutet: Der Subgraph Algorithmus
\emph{schÃ¶pft den verfÃ¼gbaren Platz in der PH} fÃ¼r diese Problemklasse vollstÃ¤ndig aus,
...
kollapiert vollstÃ¤ndig auf P (Satz~\ref{satz:kollaps}).
Sie enthÃ¤lt alle planaren Minor-beschrÃ¤nkten Probleme (Satz~\ref{satz:planar}),
alle Probleme auf Graphen mit beschrÃ¤nkter Baumweite (Korollar~\ref{kor:baumweite}),
und ist abgeschlossen unter polynomiellen Reduktionen (Lemma~\ref{lem:abgeschlossen}).
Der Kollaps ist \emph{prÃ¤zise begrenzt}: Er gilt nicht fÃ¼r NP-vollstÃ¤ndige Probleme
...
entschieden werden kÃ¶nnen. Durch Savitchs Theorem (NPSPACE~$=$~PSPACE,
Theorem~\ref{thm:savitch}) gelten diese Konsequenzen symmetrisch fÃ¼r
nichtdeterministische Raumklassen. Die Ãœbereinstimmung des Subgraph-Speicherbedarfs
...
\textbf{FÃ¼nf Matplotlib-Visualisierungen.}
...
\subsection{Bedeutung fÃ¼r die KomplexitÃ¤tstheorie}
...
nicht nur praktische, sondern tiefe \emph{strukturelle} Konsequenzen fÃ¼r
...
die vollstÃ¤ndig auf P kollabiert. Diese Teilklasse enthÃ¤lt praxisrelevante
Problemklassen wie planare Graphprobleme, Probleme mit beschrÃ¤nkter Baumweite,
...
Der Subgraph Algorithmus ist kein Beweis fÃ¼r P~$=$~NP. Er zeigt, dass die
Grenze zwischen P und NP nicht durch zyklische Strukturausnutzung Ã¼berwunden
...
Die Arbeit berÃ¼hrt direkt drei der bedeutendsten offenen Probleme:

\begin{enumerate}
  \item \textbf{P vs.\ NP}: Der Kollaps-Satz~\ref{satz:kollaps} zeigt,
        dass jeder Beweis von P~$=$~NP Ã¼ber den Subgraph-Rahmen zwingend
        eine vollstÃ¤ndige Charakterisierung von $\mathcal{K}_\mathrm{Sub} = \mathrm{NP}$
        erfordert. Umgekehrt wÃ¼rde ein Beweis, dass $\mathcal{K}_\mathrm{Sub} \subsetneq \mathrm{NP}$,
        neue strukturelle Evidenz fÃ¼r P~$\neq$~NP liefern.
  ...
  \item \textbf{PH-Kollaps}: Falls $\mathcal{K}_\mathrm{Sub} = \mathrm{PH}$
        ...
        ist damit Ã¤quivalent zur PH-Kollaps-Frage.
  ...
  \item \textbf{Graphisomorphie}: Die Beziehung des Subgraph Algorithmus
        ...
        Dies wÃ¤re eine LÃ¶sung des GI-Problems, ohne den vollen Apparat
        der Gruppentheorie (Babai~\cite{babai2016graph}) zu benÃ¶tigen.
\end{enumerate}

\subsection{Schlussbetrachtung}

Der Subgraph Algorithmus ist mehr als eine algorithmische Innovation ---
er ist ein Denkwerkzeug fÃ¼r die strukturelle KomplexitÃ¤tstheorie.
Durch die Kombination von Signatur-basierten Fingerprints, zyklischer Symmetrieausnutzung
und dynamischer Programmierung erzielt er eine Polynomialzeit-LÃ¶sung,
die tief in die Polynomialzeithierarchie eingreift, partielle Kollapse erzwingt
und neue Verbindungen zu PSPACE, QuantenkomplexitÃ¤t und Parameterisierten Algorithmen aufzeigt.
...
zu offenen Fragen --- legen ein solides Fundament fÃ¼r eine neue Forschungslinie:
die \emph{strukturelle Subgraph-KomplexitÃ¤tstheorie}, die algorithmische Effizienz
und strukturelle KomplexitÃ¤tsanalyse in einem einheitlichen Rahmen verbindet.
...
die die Grenzen zwischen P, NP und PH erforschen, gleichermaÃŸen anspricht und
