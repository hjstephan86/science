...
\begin{proof}
  ...
  und $O(n)$ für die Signatur-Arrays, also insgesamt $O(n^2)$.
\end{proof}
...
\begin{proof}[Beweisidee]
  ...
  Eine einfache Induktion zeigt $\Sigma_k^P \subseteq \mathrm{PSPACE}$ für alle $k$.
\end{proof}
...
\begin{proof}
  ...
  was P~$=$~NP implizieren würde --- eine weithin als falsch angesehene Aussage.
\end{proof}
...
\begin{proof}
  ...
  nichtdeterministisches Orakel benötigt, da $\Pi_\ell$ deterministisch in P gelöst wird.
\end{proof}
...
\begin{proof}
  ...
  in Zeit $O(n^3) = O((m \cdot d)^3)$, was für konstantes $d$ polynomiell in $m$ ist.
\end{proof}
...
\begin{proof}
  ...
  Die Laufzeit ist $O(2^{3s(n)})$ und für $s(n) = O(\log n)$ polynomiell.
\end{proof}
...
\begin{proof}
  ...
  für jedes Orakel $A$, trivialerweise.
\end{proof}
...
\begin{proof}
  ...
  in P, was diese Probleme aus der NP-intermediate-Klasse herausnehmen würde.
\end{proof}
