...
\textit{sekundÃ¤res Mikroplastik}), sowie direkt aus industriellen Prozessen wie
...
\subsection{PartikelgrÃ¶\ss{}enverteilung und Transportdynamik}
...
(da $c = c(r)$ rotationssymmetrisch). Die stationÃ¤re Gleichung reduziert sich auf:
\begin{equation}
...
groÃŸe Abst\"ande asymptotisch durch eine Gau\ss{}kurve nach Gleichung~\eqref{eq:gauss_acc}
approximiert wird.
\end{proof}
...
\begin{definition}[ZirkulÃ¤re Materialflu\ss{}effizienz]
...
Das marine Ã–kosystem erreicht eine Netto-Null-Belastung ($dM/dt = 0$)
genau dann, wenn:
\begin{equation}
  I_0 (1 - \varepsilon_{\rm circ}) + I_{\rm extern} = R_{\rm clean} + D_{\rm bio} + S_{\rm sed},
  \label{eq:netzero}
\end{equation}
wobei $I_{\rm extern}$ unvermeidliche AuÃŸeneintr\"age (atmosph\"arische
...
genetisch modifizierter Organismen (GVOs) in das MeeresÃ¶kosystem
...
abbaubare Materialien erfordert L\"osungen f\"ur bislang ungelÃ¶ste
...
Cellulose-Diacetate, PolyhydroxyfettsÃ¤uren und funktionalisierte
...
(abh\"angig von Enzymkonzentration $[E]$ und N\"ahrstoffverfÃ¼gbarkeit $[N]$)
...
\begin{itemize}
  ...
  \item \textbf{Design-Vorgaben}: Verpflichtendes \textit{Design for Recyclability}
        f\"ur alle KonsumgÃ¼terverpackungen
  \item \textbf{Technologietransfer}: Verbindliche Finanzierungsmechanismen
        f\"ur EntwicklungslÃ¤nder ($\geq 5$\,Mrd.\,USD/a)
...
$80\,\%$ stammen aus 20 Schwellen- und EntwicklungslÃ¤ndern \cite{jambeck2015plastic},
...
Reinigungsfonds -- vergleichbar mit dem GrÃ¼nen Klimafonds (GCF) der UNFCCC --
...
\subsubsection{Meeresschutzgebiete als ReinigungsprioritÃ¤ten}
...
intensiv zu reinigende PrioritÃ¤tszonen ausgewiesen werden. Das
...
und die Gewichte $\alpha, \beta, \gamma$ die politischen PrioritÃ¤ten
...
\begin{enumerate}[leftmargin=*, label=\textbf{F\arabic*.}]
  ...
  \item \textbf{Material-Passport-Systeme}: Digitale MaterialpÃ¤sse (\"ahnlich
...
\subsection{VisionÃ¤res Schlusswort}

Die Verschmutzung der Weltmeere mit Mikroplastik ist eine der dr\"angendsten
Umweltkrisen des 21.~Jahrhunderts -- und gleichzeitig eine lÃ¶sbare.
