...
\begin{tikzpicture}
	\begin{axis}[
		...
		% Spule: u_L fÃ¼hrt, i_L folgt
		\addplot[color=red!75!black, very thick, domain=0:6.2832, samples=300]
			{cos(deg(x))};
		\addlegendentry{$u_L(t) = U_0\cos(\omega t)$ â€” Spannung Spule}
		\addplot[color=blue!75!black, thick, domain=0:6.2832, samples=300]
			{sin(deg(x))};
		\addlegendentry{$i_L(t) = I_0\sin(\omega t)$ â€” Strom Spule \textbf{(eilt nach)}}
		% Kondensator: i_C fÃ¼hrt, u_C folgt
		\addplot[color=orange!85!black, dashed, very thick, domain=0:6.2832, samples=300]
			{sin(deg(x))};
		\addlegendentry{$u_C(t) = U_0\sin(\omega t)$ â€” Spannung Kondensator}
		\addplot[color=teal!75!black, dashed, thick, domain=0:6.2832, samples=300]
			{cos(deg(x))};
		\addlegendentry{$i_C(t) = I_0\cos(\omega t)$ â€” Strom Kondensator \textbf{(eilt nach â€” kausale Wirkung)}}
