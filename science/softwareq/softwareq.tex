...
\definecolor{darkblue}{RGB}{25, 70, 150}
...
\begin{definition}[Zustandsmodell des Bestellservice] Das Zustandsmodell des Bestellservice wird durch die Menge $Z = \{z_1, z_2, z_3, z_4, z_5\}$ beschrieben:
...
%     % Knoten
%     \node[state] (z1) at (0, 0) {$z_1$ \scriptsize Normal};
%     \node[state] (z2) at (-3, 2) {$z_2$ \scriptsize Latenz};
%     \node[state] (z3) at (0, 3) {$z_3$ \scriptsize Fehler};
%     \node[state] (z4) at (3, 2) {$z_4$ \scriptsize Degrdd};
%     \node[fault] (z5) at (0, -3) {$z_5$ \scriptsize Offline};
...
%     \draw[arrow_slow] (z5) to[bend right=60] (z1) node[midway, below] {}; % {2.0};
    
%     % Background
%     \begin{scope}[on background layer]
%         \node[rectangle, rounded corners, draw=darkgreen, dashed, fit=(z1) (z2) (z3) (z4), inner sep=0.3cm] {};
...
%     \node[server] (s2) at (4.5, 0.0) {$S_2$};
%     \node (dots) at (4.5, -0.55) {$\vdots$};
%     \node[server] (s143) at (4.5, -1.35) {$S_{c}$};
...
%     \node at (0, -0.15) [anchor=north, font=\small] {Poisson-Ankunft};
...
\begin{definition}[UML-Komponentenmodell] Das UML-Komponentenmodell beschreibt die Architektur des Bestellservice-Systems.
...
%     % Load Balancer
%     \node[component] (lb) at (0, 5) {Balancer};
...
%     \node at (3, 5.5) [anchor=south, font=\small] {WebServer-Cluster (143 Instanzen)};
...
%     \node at (5, 2.8) [anchor=south, font=\small] {Cluster (PostgreSQL)};
...
Die zentrale Erkenntnis ist: \emph{Es existieren Optimal-Punkte für Systemgröße, Zuverlässigkeit und Verfügbarkeit, 
