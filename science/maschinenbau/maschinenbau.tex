...
\begin{satz}[PartikulÃ¤re LÃ¶sung -- Ansatz vom Typ der rechten Seite]
...
und eine gleichm\"a\ss{}ig verzÃ¶gerte (Erdbeschleunigung $g = 9{,}81\,\mathrm{m/s^2}$)
...
\begin{itemize}
    ...
    \item[-] \textbf{0{,}2\%-Dehngrenze} $R_{p0,2}$: Spannung, bei der nach Entlastung
          eine bleibende Dehnung von 0{,}2\% verbleibt (f\"ur nicht ausgeprÃ¤gte Streckgrenze)
...
\]
mit der \textbf{KnicklÃ¤nge} $l_k$ (abh\"angig von den Einspannbedingungen):
\begin{itemize}
...
$q = \frac{\rho}{2}v^2$ und geodÃ¤tischer Druck $\rho g z$.
\end{satz}
\begin{proof}
...
thermischen und mechanischen Formen. Ihre HauptsÃ¤tze definieren
...
% ====== Erweiterung WÃ¤rmeÃ¼bertragung ======

\section{WÃ¤rmeÃ¼bertrager und KÃ¤ltemaschinen}

\begin{definition}[WÃ¤rmeÃ¼bertrager -- Typen und Berechnung]
\textbf{WÃ¤rmeÃ¼bertrager} Ã¼bertragen WÃ¤rme zwischen zwei FluidstrÃ¶men.
Wichtige Bauformen:
\begin{itemize}
    \item[-] \textbf{Gleichstrom}: beide Fluide strÃ¶men in gleicher Richtung;
          Austrittstemperaturdifferenz limitiert
    \item[-] \textbf{Gegenstrom}: maximaler WÃ¤rmeÃ¼bertrag; Austrittstemperatur
          kann Eintrittstemperatur des Gegenstroms Ã¼berschreiten
    \item[-] \textbf{Kreuzstrom}: senkrechte StrÃ¶mungen; PlattenwÃ¤rmeÃ¼bertrager
    \item[-] \textbf{ManteldurchstrÃ¶mter RohrbÃ¼ndelwÃ¤rmetauscher}:
          Rohre in Mantel; chemische Industrie
\end{itemize}
Die Ã¼bertragene WÃ¤rmeleistung:
\[
  \dot{Q} = k\,A\,\Delta T_{lm}, \qquad
  \Delta T_{lm} = \frac{\Delta T_1 - \Delta T_2}{\ln(\Delta T_1/\Delta T_2)}
\]
(\textbf{logarithmische mittlere Temperaturdifferenz} $\Delta T_{lm}$
fÃ¼r Gleich- und Gegenstrom).
\end{definition}

\begin{satz}[KÃ¤lteleistungszahl (COP) einer KÃ¤ltemaschine]
Eine KÃ¤ltemaschine entzieht der KÃ¤ltequelle (Temperatur $T_C$) die
WÃ¤rme $|Q_C|$ durch Zufuhr von Arbeit $W$.
Die \textbf{Leistungszahl}:
\[
  \varepsilon = \frac{|Q_C|}{W} = \frac{T_C}{T_H - T_C} \quad (\text{Carnot-KÃ¤ltemaschine}).
\]
Reale KÃ¤ltemaschinen (KompressionskÃ¤ltemaschinen) erreichen
$\varepsilon = 2\ldots5$ (typische Klimaanlage).
\end{satz}
\begin{proof}
FÃ¼r die Carnot-KÃ¤ltemaschine gilt energetisch: $W = Q_H - |Q_C|$
und thermisch: $|Q_C|/T_C = Q_H/T_H$ (reversibel).
Daher $\varepsilon = |Q_C|/(Q_H - |Q_C|) = T_C/(T_H - T_C)$.
\end{proof}

\begin{beispiel}[Haushalts-KÃ¼hlschrank]
Ein KÃ¼hlschrank ($T_C = 5^\circ\mathrm{C} = 278\,\mathrm{K}$,
$T_H = 35^\circ\mathrm{C} = 308\,\mathrm{K}$, Carnot-COP):
\[
  \varepsilon_C = \frac{278}{308 - 278} = \frac{278}{30} \approx 9{,}3.
\]
Mit $\varepsilon_{\text{real}} \approx 3$ und $\dot{Q}_C = 100\,\mathrm{W}$ (KÃ¼hlleistung):
Elektrische Leistungsaufnahme $P = \dot{Q}_C/\varepsilon_{\text{real}} \approx 33\,\mathrm{W}$.
\end{beispiel}

% ====== Erweiterung StrÃ¶mungslehre: Turbulenz ======

\section{Grenzschicht und Turbulenz}

\begin{definition}[Grenzschichttheorie nach Prandtl]
In der \textbf{Prandtl'schen Grenzschicht} nahe einer Ã¼berstrÃ¶mten
Wand vernachlÃ¤ssigt man den Druck quer zur Wand und vereinfacht
die Navier-Stokes-Gleichungen. FÃ¼r die laminare Grenzschichtdicke
$\delta$ auf einer Ã¼berstrÃ¶mten Platte (LÃ¤nge $x$, AnstrÃ¶mgeschwindigkeit $v_\infty$):
\[
...
\end{definition}
\begin{proof}
Blasius (1908) lÃ¶ste die Grenzschichtgleichungen durch Ã„hnlichkeitslÃ¶sung
(Strom\-funktion $\psi = \sqrt{v_\infty\nu x}\,f(\eta)$,
Ã„hnlichkeitsvariable $\eta = y\sqrt{v_\infty/(\nu x)}$).
Die numerische LÃ¶sung der resultierenden Blasius-DGL
...
\]
mit von-KÃ¡rmÃ¡n-Konstante $\kappa \approx 0{,}41$ und $B \approx 5{,}0$.
Dieses Gesetz gilt fÃ¼r $30 \lesssim y^+ \lesssim 300$ (logarithmische Schicht).
WandnÃ¤chster Bereich ($y^+ < 5$): viskose Unterschicht $u^+ = y^+$.
\end{beobachtung}

% ====== Fertigungstechnik Erweiterung ======

\section{SchweiÃŸtechnik und FÃ¼getechnologie}

\begin{definition}[SchweiÃŸverfahren nach DIN 1910]
SchweiÃŸen ist das Verbinden von Werkstoffen durch \emph{WÃ¤rme}
und/oder \emph{Druck}, mit oder ohne Zusatzwerkstoff.
Wichtige thermische SchweiÃŸverfahren:
\begin{itemize}
    \item[-] \textbf{MSG-SchweiÃŸen} (MIG/MAG, DIN EN ISO 14341):
          Metall-Schutzgas-SchweiÃŸen; hÃ¤ufigste Verfahren fÃ¼r Stahl;
          abschmelzende Elektrode; Schutzgas CO$_2$/Ar-Gemisch (MAG) oder Ar (MIG)
    \item[-] \textbf{WIG-SchweiÃŸen} (TIG, DIN EN ISO 636):
          Wolfram-Inertgas; nicht abschmelzende W-Elektrode; Ar-Schutzgas;
          hÃ¶chste NahtqualitÃ¤t; Edelstahl, Al, Ti
    \item[-] \textbf{LaserstrahlschweiÃŸen} (DIN EN ISO 11553):
          Hochenergiedichtestrahl; TiefschweiÃŸen (Keyhole); Automotive-Karosserie
    \item[-] \textbf{ElektronenstrahlschweiÃŸen}: im Vakuum; tiefste Einbrandtiefe ($> 100\,\mathrm{mm}$)
    \item[-] \textbf{ReibrÃ¼hrschweiÃŸen (FSW)}: Festphasen-Verbindung;
          Aluminium; keine HeiÃŸrisse; Luft- und Raumfahrt
\end{itemize}
\end{definition}

\begin{satz}[WÃ¤rmeeintrag und AbkÃ¼hlgeschwindigkeit beim SchweiÃŸen]
Der \textbf{Streckenenergie} (WÃ¤rmeeintrag pro LÃ¤nge) beim SchweiÃŸen:
\[
  Q_s = \eta\,\frac{U\,I}{v_s} \quad [\mathrm{J/mm}],
\]
mit thermischem Wirkungsgrad $\eta$ (MIG: $\eta \approx 0{,}8$;
Laser: $\eta \approx 0{,}4$), Lichtbogenspannung $U$, SchweiÃŸstrom $I$
und SchweiÃŸgeschwindigkeit $v_s$.
Die AbkÃ¼hlgeschwindigkeit $t_{8/5}$ (Zeit von $800$ auf $500^\circ\mathrm{C}$)
bestimmt das NahtgefÃ¼ge:
\[
  t_{8/5} \approx \frac{4300 - 4{,}3\,T_0}{(T_0 - 100)^2}\cdot Q_s^2
  \quad (\text{3D-WÃ¤rmeleitung, dicke Bleche}),
\]
mit VorwÃ¤rmtemperatur $T_0$. Langsame AbkÃ¼hlung (groÃŸes $t_{8/5}$) ergibt
Ferrit/Bainit (zÃ¤h); schnelle AbkÃ¼hlung Martensit (hart, aber sprÃ¶de).
\end{satz}


% ====== Letzte Erweiterung: dichte InhaltsblÃ¶cke in bestehende Kapitel ======

\section{Thermodynamik technischer Gase und Verbrennungsprozesse}

\begin{definition}[Reales Gas -- van-der-Waals-Gleichung]
Das ideale Gasgesetz versagt bei hohen DrÃ¼cken und tiefen Temperaturen.
Die \textbf{van-der-Waals-Gleichung} korrigiert fÃ¼r MolekÃ¼lvolumen $b$
und intermolekulare AnziehungskrÃ¤fte $a$:
\[
  \left(p + \frac{a}{v^2}\right)(v - b) = R_s T.
\]
Bei hohem Druck ($v \to b$): Druck steigt stark an (AbstoÃŸung).
Bei tiefem $T$: VerflÃ¼ssigung durch AnziehungskrÃ¤fte ($a/v^2$-Term dominant).
Der kritische Punkt: $p_c = a/(27b^2)$, $v_c = 3b$, $T_c = 8a/(27R_s b)$.
\end{definition}

\begin{definition}[Verbrennung -- StÃ¶chiometrie und Luftzahl]
VollstÃ¤ndige Verbrennung von Methan (Erdgas):
\[
...
\begin{itemize}
    \item[-] $\lambda < 1$: fettes Gemisch (Luftmangel); unvollstÃ¤ndige Verbrennung; CO-Emission
    \item[-] $\lambda = 1$: stÃ¶chiometrisches Gemisch; hÃ¶chste Flammentemperatur
    \item[-] $\lambda > 1$: mageres Gemisch (LuftÃ¼berschuss); niedrigere Temperatur; niedrigere NO$_x$
\end{itemize}
\end{definition}

\begin{beispiel}[Flammentemperatur bei $\lambda = 1$]
Vereinfachte adiabate Flammentemperatur fÃ¼r Methan/Luft ($\lambda = 1$):
\[
...
\section{Akustik und LÃ¤rmschutz im Maschinenbau}
...
Die Schallausbreitung in einem idealen Medium (homogen, ruhend, ohne ViskositÃ¤t)
gehorcht der \textbf{Wellengleichung}:
\[
  \frac{\partial^2 p}{\partial t^2} = c^2\,\nabla^2 p,
  \qquad c = \sqrt{\kappa R_s T} \quad (\text{Schallgeschwindigkeit im Gas}).
\]
FÃ¼r Luft bei $T = 293\,\mathrm{K}$: $c = \sqrt{1{,}4\cdot287\cdot293} \approx 343\,\mathrm{m/s}$.
\end{satz}
\begin{proof}
Aus der Linearisierung der Euler-Gleichungen (KontinuitÃ¤t + Impuls)
fÃ¼r kleine StÃ¶rungen $p = p_0 + p'$, $\rho = \rho_0 + \rho'$, $\vec{v} = \vec{v}'$
folgt die Wellengleichung. Die Schallgeschwindigkeit ergibt sich aus der
isentropen ZustandsÃ¤nderung: $(\partial p/\partial \rho)_s = \kappa p_0/\rho_0 = c^2$.
\end{proof}

\begin{beobachtung}[KÃ¶rperschall und KÃ¶rperschalldÃ¤mmung]
\textbf{KÃ¶rperschall} entsteht durch SchwingungsÃ¼bertragung von Maschinen
auf umgebende Strukturen und wird dort in Luftschall (Abstrahlung) umgewandelt.
MaÃŸnahmen zur KÃ¶rperschalldÃ¤mmung:
\begin{itemize}
    \item[-] \textbf{Elastische Lagerung}: Maschine auf Federn/Elastomeren;
          EinfÃ¼gungsdÃ¤mmmaÃŸ $D_E = 20\log_{10}(\eta/\sqrt{(\eta^2-1)})$ (fÃ¼r $\eta > \sqrt{2}$)
    \item[-] \textbf{Tilger und DÃ¤mpfer}: aktive oder passive Schwingungsminderung
          an der Quelle (tuned vibration absorber)
    \item[-] \textbf{Schwere Massen (Fundamente)}: Masse erhÃ¶ht Impedanz;
          $f_0 = (1/2\pi)\sqrt{c_{Unterbau}/m_{Fundament}}$ tief halten
    \item[-] \textbf{KÃ¶rperschallisolation}: Unterbrechung von Schallpfaden
          durch weiche Zwischenlagen (Gleitlager, Manschetten)
\end{itemize}
\end{beobachtung}

\section{Additive Fertigung -- Prozessphysik und QualitÃ¤tssicherung}
...
\begin{itemize}
    \item[-] \textbf{Absorptionsgrad}: Laserenergie wird an der PulveroberflÃ¤che
          absorbiert; TiefschweiÃŸen (Keyhole-Modus) bei hoher IntensitÃ¤t ($> 10^6\,\mathrm{W/cm^2}$)
    \item[-] \textbf{SchmelzbadstrÃ¶mung}: Marangoni-Konvektion (thermische OberflÃ¤chenspannung),
          Dampfreaktionskraft, Kapillarkraft bestimmen die Schmelzbadform
    \item[-] \textbf{Eigenspannungen}: Schnelle AbkÃ¼hlung erzeugt hohe Zugeigenspannungen
          ($\approx 0{,}5\,R_e$) -- WÃ¤rmenachbehandlung oder StÃ¼tzstrukturen nÃ¶tig
    \item[-] \textbf{PorositÃ¤t}: Keyhole-Poren (Gasblasen), Lack-of-Fusion-Poren
          (unaufgeschmolzenes Pulver) -- durch Prozessparameteroptimierung ($E_v = P/(v\cdot h\cdot d)$,
          volumetrische Energiedichte) minimierbar
    \item[-] \textbf{In-situ QualitÃ¤tsÃ¼berwachung}: Melt-Pool-Monitoring,
...
dauerhafter Last zeitabhÃ¤ngig: \textbf{Kriechen}.
Das Norton-Kriechgesetz:
\[
  \dot{\varepsilon}_{cr} = A\,\sigma^n\,e^{-Q/(RT)},
\]
mit materialabhÃ¤ngigem Kriechexponent $n$ (Metalle: $n \approx 3\ldots8$),
...
Gasturbinenschaufeln moderner Triebwerke (HeiÃŸgastemperatur $> 1700^\circ\mathrm{C}$,
Schaufelmaterial-Schmelzpunkt $\approx 1350^\circ\mathrm{C}$) werden durch
\textbf{Thermal Barrier Coatings} (TBC) geschÃ¼tzt:
\begin{itemize}
    \item[-] \textbf{Topschicht}: 7YSZ (zirconia stabilisiert mit 7\,\% Yttria);
          WÃ¤rmeleitfÃ¤higkeit $\lambda \approx 2\,\mathrm{W/(mK)}$; Schichtdicke $\approx 100\ldots300\,\mu\mathrm{m}$
    \item[-] \textbf{Haftvermittler}: MCrAlY (M = Ni, Co); Oxidationsschutz;
          bildet schÃ¼tzende $\alpha$-Al$_2$O$_3$-Schicht (TGO, Thermally Grown Oxide)
    \item[-] \textbf{InnenkÃ¼hlung}: SerpentinenkanÃ¤le + FilmkÃ¼hlung durch
          Bohrungen; KÃ¼hlluft $\approx 600^\circ\mathrm{C}$;
          WÃ¤rmeÃ¼bergangskoeffizient innen $\alpha \approx 1000\ldots5000\,\mathrm{W/(m^2 K)}$
\end{itemize}
Das TBC-System ermÃ¶glicht $\Delta T \approx 100\ldots200\,\mathrm{K}$ Temperaturgradient --
dies steigert den Turbinenwirkungsgrad erheblich (jeder K TemperaturerhÃ¶hung
...
(Ferrit-Zementit-Lamellengemisch). StÃ¤hle (links der 2{,}06\,\%-Grenze)
...
\begin{itemize}
    ...
    \item[-] \textbf{Ausscheidungsh\"artung}: durch Ausl\"agern (GlÃ¼hen, Abschrecken,
...
\section{GefÃ¼ge, WÃ¤rmebehandlungsdiagramme und Eigenspannungen}

\begin{definition}[Zeit-Temperatur-Umwandlungsdiagramm (ZTU)]
Das \textbf{ZTU-Diagramm} (isothermes ZTU: isotherm abgekÃ¼hlt; kontinuierliches ZTU: KU)
beschreibt die Umwandlung des Austenits beim AbkÃ¼hlen von Stahl.
Im \textbf{kontinuierlichen ZTU-Diagramm} sind AbkÃ¼hlkurven mit
den entstehenden GefÃ¼gephasen eingetragen:
\begin{itemize}
    \item[-] \textbf{Schnelle AbkÃ¼hlung} (Abschrecken in Wasser/Ã–l): Martensit ($M_s$-Linie)
    \item[-] \textbf{Mittlere AbkÃ¼hlung}: Bainit (oberer und unterer Bainit)
    \item[-] \textbf{Langsame AbkÃ¼hlung} (LuftkÃ¼hlung, OfenkÃ¼hlung): Ferrit + Perlit
\end{itemize}
Die kritische AbkÃ¼hlgeschwindigkeit $v_{krit}$ trennt martensitische
von bainitischer/perlitischer Umwandlung. FÃ¼r einen unlegierten C45-Stahl:
$v_{krit} \approx 200\,\mathrm{K/s}$; fÃ¼r legierten 42CrMo4:
$v_{krit} \approx 2\,\mathrm{K/s}$ (bessere DurchhÃ¤rtbarkeit).
\end{definition}

\begin{beobachtung}[Eigenspannungen und Verzug beim HÃ¤rten]
Beim Abschrecken entstehen \textbf{Eigenspannungen} aus zwei Quellen:
\begin{itemize}
    \item[-] \textbf{Thermische Eigenspannungen}: AuÃŸen kÃ¼hlt schnell ab
          und zieht sich zusammen, wÃ¤hrend innen noch warm -- Druckeigenspannungen innen,
          Zugeigenspannungen auÃŸen (dann Umkehr beim weiteren AbkÃ¼hlen)
...
\end{itemize}
Ergebnis: GehÃ¤rteter Stahl zeigt oft \emph{Druckeigenspannungen an der OberflÃ¤che}
($-400$ bis $-800\,\mathrm{MPa}$) -- vorteilhaft fÃ¼r ErmÃ¼dungsfestigkeit.
Schleifbrand kann diese in Zugeigenspannungen umkehren -- gefÃ¤hrlich!
\end{beobachtung}

\section{Nichtlineare Mechanik und groÃŸe Verformungen}

\begin{definition}[Geometrische NichtlinearitÃ¤t]
Bei groÃŸen Verformungen versagt die lineare Theorie ($\varepsilon = \Delta l/l_0$).
Korrekte kinematische MaÃŸe:
\begin{itemize}
    ...
    \item[-] \textbf{Cauchy-Spannung} $\boldsymbol{\sigma}$: Kraft pro FlÃ¤che in aktueller Konfiguration
    \item[-] \textbf{2. Piola-Kirchhoff-Spannung} $\mathbf{S}$: in Referenzkonfiguration;
          ...
          ($J = \det\mathbf{F}$: VolumenverhÃ¤ltnis)
\end{itemize}
\end{definition}
...
\]
mit Volumenlast $\vec{b}_0$ und OberflÃ¤chenlast $\vec{t}_0$ in der Referenzkonfiguration.
Die Linearisierung des Gesamtresiduals fÃ¼hrt auf den inkrementellen Schritt
der Newton-Raphson-Iteration in der nichtlinearen FEM.
\end{satz}

\section{Schichtdickenoptimierung in der Bauteilauslegung}

\begin{beispiel}[Leichtbauquerschnitt: TrÃ¤ger unter Biegung und Knicken]
...
\]
\textbf{Schritt 2 -- Knicken}: FÃ¼r einen TrÃ¤ger (Euler-Fall 2, $l_k = L = 2\,\mathrm{m}$):
\[
  I_{erf} = \frac{F\cdot S\cdot l_k^2}{\pi^2 E} = \frac{50000\cdot2\cdot4}{\pi^2\cdot70\cdot10^9}
  \approx 5{,}8\,\mathrm{cm^4}.
\]
\textbf{Schritt 3 -- Profil wÃ¤hlen}: Ein IPE~80 Profil
($I_y = 80\,\mathrm{cm^4}$, $W_y = 20\,\mathrm{cm^3}$, Masse $6\,\mathrm{kg/m}$)
erfÃ¼llt Knicken, aber nicht ganz die Biegung -- stattdessen IPE~100
($I_y = 171\,\mathrm{cm^4}$, $W_y = 34{,}2\,\mathrm{cm^3}$) gewÃ¤hlt.
ÃœberprÃ¼fung: $W_y = 34{,}2 > 29{,}4\,\mathrm{cm^3}$ $\checkmark$; $I_y = 171 > 5{,}8\,\mathrm{cm^4}$ $\checkmark$.
\end{beispiel}

\section{Hydraulische Ãœbertragungsglieder und Servosysteme}
...
VolumenstromÃ¶ffnungsquerschnitt um. Die Ãœbertragungsfunktion (vereinfacht):
\[
  G_{Ventil}(s) = \frac{K_V}{(1 + T_V s)^2},
\]
mit VentilverstÃ¤rkung $K_V$ und Ventilzeitkonstante $T_V \approx 2\ldots10\,\mathrm{ms}$.
Der gesamte hydraulische Lageregelkreis (Position $x$ eines Zylinders):
\[
  G_{Kreis}(s) = \frac{K_A}{s}\cdot\frac{K_V}{(1+T_V s)^2},
\]
mit hydraulisch-mechanischem VerstÃ¤rkungsfaktor $K_A$ [m/(As)].
StabilitÃ¤tsbedingung nach Hurwitz fÃ¼r dieses System 3. Ordnung:
$K_A K_V T_V^2 < 1$ (kritische VerstÃ¤rkung).
\end{definition}
...
\]
Passfedern sind einfach montierbar, aber nicht drehmoment-stoÃŸ-fest.
...
\begin{definition}[Evolventenverzahnung und EingriffsverhÃ¤ltnisse]
...
\section{Getriebetechnik und AntriebsstrÃ¤nge}
...
\begin{itemize}
    \item[-] \textbf{Stirnradgetriebe}: parallele Achsen; hoher Wirkungsgrad
          $\eta \approx 0{,}98\ldots0{,}99$ pro Stufe; groÃŸe Ãœbersetzungsbandbreite
    \item[-] \textbf{Kegelradgetriebe}: schneidende Achsen (meist $90^\circ$);
          $\eta \approx 0{,}97\ldots0{,}99$; Fahrzeugachsen, Differentiale
    \item[-] \textbf{Schneckengetriebe}: windschiefe Achsen; groÃŸe Ãœbersetzungen
          ($i = 5\ldots80$ pro Stufe); Selbsthemmung mÃ¶glich;
    ...
    \item[-] \textbf{Harmonic-Drive}: tordierte Stahlflexspline;
          Ãœbersetzung bis $i = 320$; spielfrei; Robotergelenke
\end{itemize}
Mehrstufige Getriebe: $\eta_{ges} = \prod_k \eta_k$;
GesamtÃ¼bersetzung: $i_{ges} = \prod_k i_k$.
\end{definition}

\begin{satz}[Planetengetriebe -- Willis-Gleichung]
FÃ¼r ein Planetengetriebe mit Sonnenrad ($z_s$), Planetenrad ($z_p$),
Hohlrad ($z_h$) und Steg gilt die \textbf{Willis-Gleichung}:
\[
  \frac{\omega_s - \omega_0}{\omega_h - \omega_0} = -\frac{z_h}{z_s},
\]
wobei $\omega_0$ die Stegdrehzahl ist. FÃ¼r festgehaltenes Hohlrad
($\omega_h = 0$): $\omega_s/\omega_0 = 1 + z_h/z_s$ (StandÃ¼bersetzung $i_0 = -z_h/z_s$).
\end{satz}
\begin{proof}
...
Da Sonnenrad und Hohlrad Ã¼ber die Planeten im Ã¤uÃŸeren Eingriff stehen
(Hohlrad: Innenverzahnung): $i_0 = -z_h/z_s$ (Vorzeichenwechsel durch Inneneingriff).
\end{proof}

\section{Technische Akustik -- SchalldÃ¤mmung und Absorption}

\begin{definition}[SchalldÃ¤mm-MaÃŸ und Absorptionsgrad]
Das \textbf{SchalldÃ¤mm-MaÃŸ} $R$ einer Trennwand (Masse $m''$ [kg/m$^2$]):
\[
  R \approx 20\log_{10}(f\cdot m'') - 47\,\mathrm{dB} \quad (\text{Massegesetz}),
\]
d.\,h. Verdoppelung der flÃ¤chenbezogenen Masse erhÃ¶ht $R$ um $6\,\mathrm{dB}$;
...
\]
PorÃ¶se Materialien (Mineralwolle, Schaumstoff): $\alpha \to 1$ bei hohen Frequenzen.
Glatte Betonwand: $\alpha \approx 0{,}01$ (fast vollstÃ¤ndige Reflexion).
\end{definition}

\begin{beobachtung}[Aktive GerÃ¤uschunterdrÃ¼ckung (ANC)]
\textbf{Active Noise Control} (ANC) erzeugt ein Gegenschallfeld,
das durch destruktive Interferenz den LÃ¤rm auslÃ¶scht.
Prinzip: Mikrofon misst StÃ¶rgerÃ¤usch $x(n)$; adaptiver Filter
...
Anwendungen: KopfhÃ¶rer mit ANC, Kraftfahrzeug-Innenraum-Akustik (Road Noise ANC).
\end{beobachtung}
...
\end{itemize}
SchlÃ¼sselwerkzeuge: MATLAB/Simulink, dSPACE Rapid Prototyping,
AUTOSAR-Softwarearchitektur, SPICE (Elektronik-Simulation).
\end{definition}

\begin{beobachtung}[Hardware-in-the-Loop (HiL) Simulation]
In der \textbf{HiL-Simulation} wird ein reales SteuergerÃ¤t (ECU)
...
\begin{itemize}
    ...
    \item[-] Fehlersimulation (KurzschlÃ¼sse, KabelbrÃ¼che) fÃ¼r Robustheitstests
    \item[-] Gesetzlich vorgeschrieben fÃ¼r sicherheitskritische ECUs (Bremse, ESP, Airbag)
\end{itemize}
HiL ermÃ¶glicht vollstÃ¤ndige Erprobung vor dem ersten Fahrzeugprototypen --
typische Einsparung: $30\ldots50\,\%$ der Entwicklungskosten fÃ¼r Software-Tests.
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{Selektives Lasersintern (SLS)}: Kunststoffpulver wird schichtweise
          gesintert; komplexe Geometrien ohne StÃ¼tzen; $\pm 0{,}3\,\mathrm{mm}$ Genauigkeit
...
% ===== GroÃŸe Erweiterung: vor der Zusammenfassung =====

% ============================================================
\chapter{Numerische Methoden im Maschinenbau}
% ============================================================

Numerische Methoden lÃ¶sen technische Probleme, die analytisch nicht
oder nur mit unzumutbarem Aufwand lÃ¶sbar sind. Sie sind das verbindende
Element zwischen mathematischer Theorie und praktischer Ingenieursanwendung.

\section{Lineare Gleichungssysteme}

\begin{definition}[Direkte und iterative LÃ¶ser]
Ein lineares Gleichungssystem $\mathbf{A}\vec{x} = \vec{b}$ mit
$\mathbf{A} \in \mathbb{R}^{n\times n}$ kann gelÃ¶st werden durch:
\begin{itemize}
    \item[-] \textbf{Direkte Methoden}: exakte LÃ¶sung in endlich vielen Schritten
          \begin{itemize}
              \item[-] \textbf{GauÃŸ-Elimination}: $O(n^3)$ Operationen; pivotisiert (LU-Zerlegung)
              \item[-] \textbf{Cholesky-Zerlegung}: $\mathbf{A} = \mathbf{L}\mathbf{L}^\top$
                    fÃ¼r symmetrisch positiv definite $\mathbf{A}$; $n^3/6$ Operationen;
                    FEM-Steifigkeitsmatrizen
              \item[-] \textbf{QR-Zerlegung}: fÃ¼r Ausgleichsprobleme (Least Squares)
          \end{itemize}
    \item[-] \textbf{Iterative Methoden}: Konvergenz gegen LÃ¶sung
          \begin{itemize}
              \item[-] \textbf{Jacobi, Gauss-Seidel}: einfach, aber langsam; $O(n^2)$ pro Iteration
              \item[-] \textbf{Konjugiertes Gradienten-Verfahren (CG)}: fÃ¼r symm. pos. def. $\mathbf{A}$;
                    exakte LÃ¶sung in $\le n$ Schritten; $O(n)$ Speicher fÃ¼r dÃ¼nnbesetzte Matrizen
              \item[-] \textbf{GMRES}: fÃ¼r unsymmetrische Systeme (CFD, instationÃ¤re FEM)
          \end{itemize}
\end{itemize}
\end{definition}

\begin{satz}[LU-Zerlegung und GauÃŸ-Elimination]
Jede regulÃ¤re Matrix $\mathbf{A}$ (mit geeigneter Zeilenpermutation $\mathbf{P}$)
lÃ¤sst sich in $\mathbf{PA} = \mathbf{LU}$ zerlegen,
mit unterer Dreiecksmatrix $\mathbf{L}$ ($L_{ii} = 1$) und oberer
Dreiecksmatrix $\mathbf{U}$. Das System $\mathbf{Ax} = \mathbf{b}$ wird dann
durch VorwÃ¤rts- ($\mathbf{Ly} = \mathbf{Pb}$) und RÃ¼ckwÃ¤rtssubstitution
($\mathbf{Ux} = \mathbf{y}$) gelÃ¶st.
\end{satz}
\begin{proof}
GauÃŸ-Elimination eliminiert sukzessive Subdiagonalelemente durch
...
Partielle Pivotisierung (Spaltenpivot) sichert numerische StabilitÃ¤t.
\end{proof}

\section{Numerische Integration von DGL}

\begin{definition}[Runge-Kutta-Verfahren]
Das \textbf{klassische Runge-Kutta-Verfahren 4. Ordnung} (RK4) lÃ¶st
...
Der RK4-Schritt stimmt durch die gewÃ¤hlten Koeffizienten ($1/6, 2/6, 2/6, 1/6$
nach dem Butcher-Tableau) mit der Taylorreihe bis $O(h^4)$ Ã¼berein.
\end{proof}
...
FÃ¼r steife Systeme versagen explizite Methoden (RK4) bei akzeptabler Schrittweite --
die StabilitÃ¤tsbedingung $h < 2/|\lambda_{\max}|$ erzwingt winzige Schritte.
\textbf{Implizite Methoden} (RÃ¼ckwÃ¤rts-Euler, Trapezregel, BDF-Methoden)
sind $A$-stabil und erlauben groÃŸe Schrittweiten:
\[
  \vec{y}_{n+1} = \vec{y}_n + h\,\vec{f}(t_{n+1}, \vec{y}_{n+1}) \quad (\text{RÃ¼ckwÃ¤rts-Euler}).
\]
Dies erfordert in jedem Schritt das LÃ¶sen eines (nichtlinearen) Gleichungssystems
...
Gegeben ein differenzierbares GÃ¼tefunktional $J(\vec{x})$.
...
Das \textbf{Newton-Verfahren} nutzt zusÃ¤tzlich die Hesse-Matrix:
\[
  \vec{x}_{k+1} = \vec{x}_k - [\mathbf{H}J(\vec{x}_k)]^{-1}\nabla J(\vec{x}_k).
\]
Newton konvergiert quadratisch ($\|\vec{x}_{k+1}-\vec{x}^*\| = O(\|\vec{x}_k-\vec{x}^*\|^2)$),
benÃ¶tigt aber $O(n^3)$ pro Schritt (Hesse-Inversion).
\end{definition}

\begin{beobachtung}[Metaheuristiken fÃ¼r nichtlineare, nichtkonvexe Probleme]
Wenn das GÃ¼tefunktional nicht-konvex, nicht-differenzierbar oder
hochdimensional ist, versagen gradientenbasierte Methoden.
Alternative: \textbf{Metaheuristiken}:
\begin{itemize}
    \item[-] \textbf{Genetische Algorithmen (GA)}: evolutionÃ¤re Optimierung;
          Selektion, Kreuzung, Mutation; parallelisierbar; fÃ¼r diskrete und kombinatorische Probleme
    \item[-] \textbf{Particle Swarm Optimization (PSO)}: Schwarmintelligenz;
          Partikel bewegen sich im Suchraum; gut fÃ¼r kontinuierliche Probleme
    \item[-] \textbf{Simulated Annealing (SA)}: physikalische Analogie (AbkÃ¼hlung);
          zufÃ¤llige Verschlechterungen akzeptiert (Entkomme lokalen Minima)
    \item[-] \textbf{Bayes'sche Optimierung}: surrogate-Modell (Gaussian Process)
          schÃ¤tzt GÃ¼tefunktional; effizient fÃ¼r teure Auswertungen (Simulationen)
\end{itemize}
\end{beobachtung}
...
\begin{align*}
  f'(x) &= \frac{f(x+h) - f(x)}{h} + O(h) \quad (\text{VorwÃ¤rtsdifferenz}),\\
...
\begin{beispiel}[FDM fÃ¼r 1D-WÃ¤rmeleitung]
StationÃ¤re WÃ¤rmeleitung $-\lambda\,T'' = q$ auf $[0,L]$,
$T(0) = T_0$, $T(L) = T_L$, mit WÃ¤rmequelle $q = \mathrm{const}$.
...
das effizient durch den Thomas-Algorithmus in $O(N)$ gelÃ¶st wird.
Analytische LÃ¶sung: $T(x) = T_0 + (T_L-T_0)x/L + (q/2\lambda)(L-x)x$;
FDM-LÃ¶sung ist exakt (da $T''=\mathrm{const}$, kein Diskretisierungsfehler).
\end{beispiel}
...
und Integration Ã¼ber das Gebiet $\Omega$:
\[
  \int_\Omega v\,\mathcal{L}u\,\mathrm{d}\Omega = \int_\Omega v\,f\,\mathrm{d}\Omega.
\]
Beim \textbf{Galerkin-Verfahren} wird die LÃ¶sung $u_h$ im endlichdimensionalen
Ansatzraum $V_h = \mathrm{span}\{\varphi_1,\ldots,\varphi_n\}$ gesucht:
$u_h = \sum_{j=1}^n u_j\,\varphi_j$, wobei $v = \varphi_i$, $i = 1,\ldots,n$.
\end{definition}

\begin{satz}[FEM-Steifigkeitsmatrix fÃ¼r Elastostatik]
FÃ¼r das linearelastische Problem $-\nabla\cdot\boldsymbol{\sigma} = \vec{f}$
fÃ¼hrt die schwache Formulierung auf das lineare System
...
\]
dem ElastizitÃ¤tstensor $\mathbf{C}$ (4-stufig, symmetrisch: $C_{ijkl} = C_{jikl} = C_{ijlk} = C_{klij}$)
...
Integration Ã¼ber $\Omega$ und partielle Integration (Green'sche Formel):
...
\begin{itemize}
    ...
    \item[-] \textbf{$p$-FEM}: fester Gitter, steigender Polynomgrad $p$;
          exponentielle Konvergenz fÃ¼r glatte Probleme
    \item[-] \textbf{$hp$-FEM}: adaptiv grob/fein und $p$ erhÃ¶hen;
          beste Kombination; Basis fÃ¼r kommerzielle Solver (ANSYS, ABAQUS)
\end{itemize}
Locking-PhÃ¤nomene (Volumetric Locking bei fast-inkompressiblen Materialien,
...
\section{Numerische Methoden fÃ¼r nichtlineare Probleme}

\begin{satz}[Newton-Raphson-Verfahren fÃ¼r nichtlineare FEM]
Bei nichtlinearen Problemen (groÃŸe Verformungen, PlastizitÃ¤t, Kontakt)
...
Konvergenz ist quadratisch in der NÃ¤he der LÃ¶sung.
\end{satz}
\begin{proof}
Taylorentwicklung: $\vec{r}(\vec{u}+\Delta\vec{u}) \approx \vec{r}(\vec{u})
+ (\partial\vec{r}/\partial\vec{u})\Delta\vec{u} = \vec{0}$.
AuflÃ¶sen nach $\Delta\vec{u}$ liefert den Newton-Schritt.
Quadratische Konvergenz: $\|\vec{u}_{k+1}-\vec{u}^*\| = O(\|\vec{u}_k-\vec{u}^*\|^2)$
folgt aus der RestgliedabschÃ¤tzung der Taylorentwicklung zweiter Ordnung.
\end{proof}

\begin{beobachtung}[Kontaktmechanik und numerische Behandlung]
Kontaktprobleme (zwei KÃ¶rper berÃ¼hren sich) sind geometrisch und materiell
...
\begin{itemize}
    \item[-] Schritt 1: Impulsgleichungen mit gerattenem Druck $p^*$ lÃ¶sen $\to \vec{v}^*$
    \item[-] Schritt 2: Druckkorrekturgleichung lÃ¶sen $\to p' = p - p^*$
...
Da direkte numerische Simulation (DNS) fÃ¼r technische Anwendungen unpraktikabel
($Re \approx 10^6$: $N \sim Re^{9/4}$ Gitterpunkte nÃ¶tig), werden Turbulenzmodelle eingesetzt:
\begin{itemize}
    \item[-] \textbf{$k$-$\varepsilon$-Modell}: zwei Transportgleichungen fÃ¼r
          turbulente kinetische Energie $k$ und Dissipationsrate $\varepsilon$;
          turbulente ViskositÃ¤t $\mu_t = C_\mu\rho k^2/\varepsilon$;
          robust, weit verbreitet; ungenau in Grenzschichten
    \item[-] \textbf{$k$-$\omega$-SST-Modell} (Menter 1994): $k$-$\omega$ nahe Wand,
          $k$-$\varepsilon$ im Freistrom; Standardmodell fÃ¼r externe Aerodynamik
    \item[-] \textbf{Reynolds-Stress-Modell (RSM)}: sechs Transportgleichungen
          fÃ¼r alle Komponenten des Reynoldsspannungstensors; aufwÃ¤ndig,
          aber genauer fÃ¼r stark anisotrope StrÃ¶mungen (DrallstrÃ¶mungen)
    \item[-] \textbf{LES} (Large Eddy Simulation): groÃŸe Wirbel aufgelÃ¶st,
          kleine modelliert (Sub-Grid-Scale-Modell, Smagorinsky);
          transient, teuer, aber genauer als RANS
\end{itemize}
\end{beobachtung}

\begin{beispiel}[NetzunabhÃ¤ngigkeitsstudie in CFD]
Ein CFD-Modell eines WÃ¤rmetauschers wird auf drei Netzen berechnet
($N_1 = 10^5$, $N_2 = 4\cdot10^5$, $N_3 = 1{,}6\cdot10^6$ Zellen,
Verfeinerungsfaktor $r = 2$). AusgangsgrÃ¶ÃŸe: WÃ¤rmeÃ¼bertragung $\dot{Q}$:
\begin{center}
\begin{tabular}{lccc}
\toprule
...
Gegeben $n+1$ StÃ¼tzpunkte $(x_0,y_0),\ldots,(x_n,y_n)$ mit paarweise
verschiedenen $x_i$. Das \textbf{Lagrange-Interpolationspolynom}:
\[
  P_n(x) = \sum_{i=0}^n y_i\,L_i(x), \quad
  L_i(x) = \prod_{\substack{j=0\\j\neq i}}^n \frac{x-x_j}{x_i-x_j}.
\]
FehlerabschÃ¤tzung: $|f(x) - P_n(x)| \le \frac{M_{n+1}}{(n+1)!}\,|\omega_{n+1}(x)|$,
$\omega_{n+1}(x) = \prod_{i=0}^n(x-x_i)$, $M_{n+1} = \max|f^{(n+1)}|$.
\end{definition}

\begin{beobachtung}[GauÃŸ'sche Prozessregression als Metamodell]
Ein \textbf{Metamodell} (Surrogate Model) approximiert eine teure
Simulation (FEM, CFD) durch ein schnell auszuwertendes Modell.
Die \textbf{GauÃŸ'sche Prozessregression} (GPR, Kriging) modelliert
$f(x)$ als Realisierung eines GauÃŸ'schen Prozesses:
\[
...
posterior-Verteilung fÃ¼r $f(\vec{x}^*)$:
\[
...
und liefert UnsicherheitsschÃ¤tzungen (Konfidenzintervalle) -- ideal fÃ¼r
...
sind die organisatorischen SÃ¤ulen der Fertigungseffizienz.

\section{Lean-Prinzipien und Toyota-Produktionssystem}

\begin{beobachtung}[Die fÃ¼nf Lean-Prinzipien nach Womack/Jones]
...
\begin{itemize}
    \item[-] \textbf{Wert (Value)}: Was der Kunde bereit ist zu zahlen -- definiert WertschÃ¶pfung
    \item[-] \textbf{Wertstrom (Value Stream)}: alle AktivitÃ¤ten, um Wert zu liefern;
          ``Verschwendung'' (Muda) eliminieren: Ãœberproduktion, Warten, Transport,
          Ãœberbearbeitung, BestÃ¤nde, Bewegung, Fehler (7 Arten von Muda)
    \item[-] \textbf{Fluss (Flow)}: Produktion ohne Unterbrechung und Puffer
    \item[-] \textbf{Pull}: Produktion wird durch Kundennachfrage ausgelÃ¶st (Kanban)
...
Die \textbf{GesamtanlageneffektivitÃ¤t} OEE misst die Effizienz von Produktionsanlagen:
\[
  \text{OEE} = \text{VerfÃ¼gbarkeit} \times \text{Leistungsgrad} \times \text{QualitÃ¤tsrate},
\]
\[
...
\]
World-Class OEE: $\ge 85\,\%$. Typische Verluste: RÃ¼sten, StÃ¶rungen,
KleinststillstÃ¤nde, Anlaufverluste, Ausschuss.
\end{definition}

\section{QualitÃ¤tsmanagement}

\begin{definition}[Statistische Prozesskontrolle (SPC)]
Die \textbf{Regelkarte} (Shewhart-Karte) Ã¼berwacht kontinuierlich
...
\]
Die Wahrscheinlichkeit, einen Punkt auÃŸerhalb der Grenzen zu finden,
wenn der Prozess unter Kontrolle ist: $p = 0{,}27\,\%$ (Normalverteilung).
Der \textbf{ProzessfÃ¤higkeitsindex}:
\[
...
\begin{satz}[GauÃŸ'sche Normalverteilung und Wahrscheinlichkeiten]
...
\begin{itemize}
    \item[-] \textbf{Kontraktiles Element (CE)}: erzeugt aktive Kraft;
          Kraft-LÃ¤nge-Relation: $F_{CE}(l) = F_{max}\,f_l(l)\,f_v(\dot{l})$
    \item[-] \textbf{Serielles elastisches Element (SEE)}: Sehne und Aktomyosin-BrÃ¼cken;
...
Die WÃ¤rmerate eines kontrahierenden Muskels ist proportional zur VerkÃ¼rzungsgeschwindigkeit.
Aus Energieerhaltung und empirischer Beobachtung der WÃ¤rmeproduktion
ergibt sich die Hyperbel $(F+a)(v+b)=\mathrm{const}$.
FÃ¼r $v = 0$: $F = F_0$ (isometrische Kraft).
FÃ¼r $F = 0$: $v = v_{max} = b(F_0/a)$ (Maximalgeschwindigkeit bei Nulllast).
\end{proof}

\begin{definition}[GelenkkrÃ¤fte -- Kniescheibe als Beispiel]
Die \textbf{Patella (Kniescheibe)} wirkt als Umlenkrolle des Quadrizeps.
Bei einem Knieflexionswinkel $\phi$ und Quadrizepskraft $F_Q$ betrÃ¤gt
...
$5\ldots10\,\mathrm{MPa}$ erreichen -- eine der hÃ¶chsten im menschlichen KÃ¶rper.
\end{definition}
...
\begin{itemize}
    ...
    \item[-] \textbf{Doppelstandphase}: beide FÃ¼ÃŸe am Boden ($\approx 20\,\%$ des Zyklus)
    \item[-] \textbf{Normalgang-Kennwerte}: Kadenz $\approx 100\,\mathrm{Schritte/min}$,
          SchrittlÃ¤nge $\approx 0{,}75\,\mathrm{m}$, Gehgeschwindigkeit $\approx 1{,}25\,\mathrm{m/s}$
    \item[-] \textbf{Bodenreaktionskraft}: doppelgipflige Kurve ($1{,}0\ldots1{,}3\,\times$ KÃ¶rpergewicht);
          gemessen mit Kraftmessplatten
    \item[-] \textbf{Energieverbrauch}: $\approx 3{,}5\,\mathrm{ml\,O_2/(kg\,min)}$;
          Optimum bei $1{,}25\,\mathrm{m/s}$ -- spontan gewÃ¤hlte Geschwindigkeit!
\end{itemize}
\end{beobachtung}
...
und BodenreaktionskrÃ¤ften ($\vec{F}_{GRF}$, $\vec{M}_{GRF}$) berechnet die
\textbf{inverse Dynamik} die Gelenkmomente $M_j$ durch rÃ¼ckwÃ¤rtige Iteration
...
Ergonomie-Bewertung von ArbeitsplÃ¤tzen.
\end{satz}

\section{Bionik -- Biologische Prinzipien in der Technik}

\begin{beobachtung}[Strukturbionik: Knochen, BÃ¤ume, Schilfrohr]
Biologische Strukturen haben sich Ã¼ber Millionen Jahre zu LeichtbaulÃ¶sungen
...
\begin{itemize}
    \item[-] \textbf{GleichmÃ¤ÃŸige Spannungsverteilung}: Biologische Strukturen
          zeigen konstante OberflÃ¤chenspannung -- lokale Ãœberlastungen werden
    ...
    \item[-] \textbf{Faserverlauf in Holz}: Zellulosefasern verlaufen entlang
          ...
          minimalem Gewicht; Vorbild fÃ¼r faserverstÃ¤rkte Kunststoffe
    \item[-] \textbf{Schilfrohr}: hohler Querschnitt mit gradierter WandstÃ¤rke;
          hohe Knickfestigkeit bei extremer Schlankheit ($L/d > 100$);
          Vorbild fÃ¼r Windenergieanlagen-TÃ¼rme
...
\begin{beobachtung}[StrÃ¶mungsbionik: Walflosse und VogelflÃ¼gel]
\begin{itemize}
    \item[-] \textbf{Buckelwal-Flipper (Tubercle Technology)}: sinusfÃ¶rmige
          Vorderkante des Flippers verzÃ¶gert StrÃ¶mungsabriss um $\approx 40\,\%$
          hÃ¶heren Anstellwinkel; $+8\,\%$ Auftrieb, $-32\,\%$ Widerstand;
          Umsetzung: Windturbinenschaufeln, Schiffspropeller, LÃ¼fterflÃ¼gel
    \item[-] \textbf{VogelflÃ¼gel-Winglet}: Federn am FlÃ¼gelende biegen sich
    ...
    \item[-] \textbf{Pinguinflosse}: elliptischer Querschnitt; optimales
          VerhÃ¤ltnis Auftrieb/Widerstand fÃ¼r Unterwasserflug;
          Vorbild fÃ¼r U-Boot-Ruder und AUV-Antrieb
    \item[-] \textbf{MÃ¼ckenrÃ¼ssel}: Mehrgliedrige Nadel injiziert schmerzlos;
          jedes Segment vibriert leicht versetzt -- reduzierte Geweberisskraft;
          Vorbild fÃ¼r minimalinvasive Nadeln in der Medizin
\end{itemize}
\end{beobachtung}

\begin{definition}[Selbstheilende Materialien -- biologische Inspiration]
Biologische Gewebe heilen sich selbst nach SchÃ¤digung. Technische
\textbf{selbstheilende Werkstoffe} ahmen dieses Prinzip nach:
\begin{itemize}
    \item[-] \textbf{Mikrokapseln}: im Polymer eingebettete Kapseln
          enthalten Monomer; Riss platzt Kapsel, Monomer flieÃŸt in Riss,
          polymerisiert in Kontakt mit eingebettetem Katalysator
    \item[-] \textbf{VaskulÃ¤re Netzwerke}: dreidimensionale Kanalnetzwerke
          im Material (analog BlutgefÃ¤ÃŸe) transportieren Heilmittel gezielt
    \item[-] \textbf{Intrinsische Heilung}: reversible supramolekulare Bindungen
          (H-BrÃ¼cken, Metallkoordinationen) gehen nach Belastung neu ein;
          keine externe Heilungssubstanz nÃ¶tig
    \item[-] \textbf{Anwendungen}: selbstheilende Lacke (Autoindustrie),
          Schutzschichten fÃ¼r Windturbinen, selbstreparierbare Elektrolyte
...
(zufÃ¤llig verteilte Keimzellen, Zellen als Polygone um jeden Keim)
approximiert das Schaumstrukturprinzip natÃ¼rlicher Werkstoffe (Schwamm, Knochenspongiosa):
Relative Dichte $\bar{\rho} = \rho^*/\rho_s$ (Zell- zu Vollmaterialdichte).
Skalierungsgesetze fÃ¼r offenzelligen Schaum (Gibson-Ashby):
\[
  E^*/E_s \approx C_1 \bar{\rho}^2, \qquad \sigma_{pl}^*/\sigma_{ys} \approx C_2 \bar{\rho}^{3/2},
\]
mit materialabhÃ¤ngigen Konstanten $C_1 \approx 1$, $C_2 \approx 0{,}3$.
...
Ein \textbf{PPS-System} plant, steuert und Ã¼berwacht alle Produktionsprozesse
...
\begin{itemize}
    \item[-] \textbf{Produktionsprogrammplanung}: Welche Produkte in welchen Mengen
          und Terminen? Basierend auf KundenauftrÃ¤gen und Lagerbestand
    \item[-] \textbf{Mengenplanung} (MRP -- Material Requirements Planning):
          Bruttobedarfsermittlung $\to$ Nettobedarfsermittlung $\to$ LosgrÃ¶ÃŸenrechnung
    \item[-] \textbf{Terminplanung}: RÃ¼ckwÃ¤rtsterminierung oder VorwÃ¤rtsterminierung;
          kritischer Pfad (CPM -- Critical Path Method)
    \item[-] \textbf{KapazitÃ¤tsplanung}: Abgleich von KapazitÃ¤tsbedarf und
          -angebot; Engpassanalyse (Theory of Constraints, TOC)
    \item[-] \textbf{Auftragsfreigabe und -Ã¼berwachung}: FertigungsauftrÃ¤ge,
          Betriebsdatenerfassung (BDE), RÃ¼ckmeldung
\end{itemize}
\end{definition}

\begin{satz}[LosgrÃ¶ÃŸenformel nach Andler (Economic Order Quantity)]
Die \textbf{optimale LosgrÃ¶ÃŸe} $Q^*$ minimiert die Summe aus RÃ¼st-/Bestellkosten
und Lagerkosten:
\[
  K_{ges}(Q) = \frac{D}{Q}\,K_R + \frac{Q}{2}\,h,
\]
mit Jahresbedarf $D$, RÃ¼stkosten $K_R$ und Lagerkostensatz $h$ [â‚¬/(StÃ¼ckÂ·Jahr)].
Das Minimum bei:
\[
...
\begin{beispiel}[LosgrÃ¶ÃŸenoptimierung in der Fertigung]
Jahresbedarf $D = 12000\,\mathrm{Stk.}$, RÃ¼stkosten $K_R = 300\,\mathrm{EUR}$,
Lagerkostensatz $h = 2\,\mathrm{EUR/(Stk.\,\cdot\,Jahr)}$:
\[
  Q^* = \sqrt{\frac{2\cdot12000\cdot300}{2}} = \sqrt{3600000} \approx 1897\,\mathrm{Stk.}.
\]
Mindestanzahl RÃ¼stvorgÃ¤nge: $D/Q^* \approx 6{,}3$ pro Jahr.
...
Grundprinzip: Ein BehÃ¤lter wird erst dann neu befÃ¼llt, wenn er geleert wurde
...
Vorgehensweise:
\begin{itemize}
    \item[-] \textbf{IST-Zustand aufnehmen}: Alle Prozessschritte, BestÃ¤nde,
          InformationsflÃ¼sse, Taktzeiten, RÃ¼stzeiten, Ausschussraten erfassen
    \item[-] \textbf{Kennzahlen berechnen}: Durchlaufzeit (DLZ), WertschÃ¶pfungsanteil
          (typisch $< 5\,\%$!), Gesamtbestand in Tagen
    \item[-] \textbf{SOLL-Zustand definieren}: Fluss- und Pull-Prinzip anwenden;
          Ãœberproduktion eliminieren; SupermÃ¤rkte dimensionieren; Schrittmacherprozess festlegen
    \item[-] \textbf{Umsetzungsplan}: MaÃŸnahmen mit Verantwortlichen und Terminen
\end{itemize}
Typische Ergebnisse eines VSM-Projekts: DLZ-Reduktion $50\ldots80\,\%$,
Bestandsreduktion $30\ldots60\,\%$, ProduktivitÃ¤tssteigerung $20\ldots40\,\%$.
\end{beobachtung}

\section{Six Sigma und DMAIC}

\begin{definition}[Six-Sigma-Methodik]
\textbf{Six Sigma} ist eine datengestÃ¼tzte QualitÃ¤tsmanagement-Methodik
zur Reduktion von Prozessstreuungen. Der DMAIC-Zyklus:
\begin{itemize}
    \item[-] \textbf{D -- Define}: Problem definieren, Projektcharter, Prozessumfang,
          VOC (Voice of Customer), kritische QualitÃ¤tsmerkmale (CTQ)
    \item[-] \textbf{M -- Measure}: Messsystem analysieren (MSA, Gauge R\&R),
          Baseline-FÃ¤higkeit ermitteln ($C_{pk}$, DPMO)
    \item[-] \textbf{A -- Analyze}: Ursachen identifizieren; Ishikawa-Diagramm
          (FischgrÃ¤te), Pareto-Analyse (80-/20-Regel), FMEA, Regressionsanalyse
    \item[-] \textbf{I -- Improve}: LÃ¶sungen entwickeln und validieren; DoE
          (Design of Experiments), EVOP, Pilotierung
    \item[-] \textbf{C -- Control}: Verbesserungen absichern; Kontrollplan,
          Regelkarten, Standardarbeitsanweisungen (SOP), Ãœbergabe an Prozessinhaber
\end{itemize}
\end{definition}

\begin{satz}[DPMO und Sigma-Niveau]
Die \textbf{Defekte pro Million MÃ¶glichkeiten} (DPMO) werden in ein
...
DoE ermÃ¶glicht die systematische Untersuchung des Einflusses mehrerer
Faktoren $x_1, \ldots, x_k$ auf eine ZielgrÃ¶ÃŸe $y$.
Beim \textbf{$2^k$-faktoriellen Versuchsplan} werden alle Faktoren
auf zwei Stufen ($-1$ und $+1$, kodierte Variablen) eingestellt
und alle $2^k$ Kombinationen durchgefÃ¼hrt.
...
Halbe und fraktionelle PlÃ¤ne ($2^{k-p}$) reduzieren Versuchsanzahl
bei zunehmendem Aliasing hÃ¶herer Wechselwirkungen.
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{Feinplanung und -steuerung}: Sequenzierung, Priorisierung,
          KapazitÃ¤tsabgleich in Echtzeit
    \item[-] \textbf{BDE/MDE}: Betriebsdaten- und Maschinendatenerfassung;
          RÃ¼ckmeldung von Fertigmengen, Ausschuss, StillstandsgrÃ¼nden
    \item[-] \textbf{QualitÃ¤tsmanagement}: Online-SPC, PrÃ¼fplanverwaltung,
          Sperren von Losen; RÃ¼ckverfolgbarkeit (Traceability)
    \item[-] \textbf{OEE-Monitoring}: Echtzeit-Dashboards; automatische
          VerfÃ¼gbarkeitsberechnung; Stillstandsklassifikation
...
Bio-inspiriertes Engineering Ã¼bertrÃ¤gt erfolgreiche biologische LÃ¶sungen
auf technische Probleme.

\section{Grundlagen der Biomechanik}

\begin{definition}[Knochen als Verbundwerkstoff]
Knochen ist ein natÃ¼rlicher \textbf{Faserverbundwerkstoff}:
organische Matrix aus Kollagenfasern (Typ I, Zugfestigkeit $\approx 50\,\mathrm{MPa}$)
verstÃ¤rkt durch anorganische Hydroxylapatit-Kristalle
...
\begin{itemize}
    ...
    \item[-] TrabekulÃ¤rer Knochen (Spongiosa): $E \approx 0{,}1\ldots5\,\mathrm{GPa}$
          (porÃ¶s, 70--95\,\% PorositÃ¤t); hohe spezifische Energie-Absorption
\end{itemize}
\end{definition}
...
mechanische Belastung anpassen: Bereiche hoher Spannung werden verstÃ¤rkt,
unbelastete Bereiche abgebaut.
Mathematisch: lokale Dichte $\rho(\vec{x})$ evoliert gemÃ¤ÃŸ:
\[
  \frac{\mathrm{d}\rho}{\mathrm{d}t} \propto (\tilde{\varepsilon} - \varepsilon_{ref}),
\]
mit BelastungsmaÃŸ $\tilde{\varepsilon}$ und Referenzwert $\varepsilon_{ref}$.
Dieses biologische Prinzip ist direkte Inspiration fÃ¼r die
Topologieoptimierung in der Strukturmechanik.
\end{beobachtung}

\begin{beobachtung}[Bio-inspiriertes Engineering -- Beispiele]
Die Natur hat in Milliarden Jahren Evolutionsjahren technische Probleme gelÃ¶st.
Erfolgreiche Ãœbertragungen auf die Technik:
\begin{itemize}
    \item[-] \textbf{Lotuseffekt}: Superhydrophobe OberflÃ¤chen (Kontaktwinkel $> 150^\circ$)
    ...
    \item[-] \textbf{Haifischhaut}: Riblet-Strukturen reduzieren turbulente
          Wandreibung um $\approx 8\,\%$ -- FlugzeughÃ¼llen, SchwimmanzÃ¼ge
    ...
    \item[-] \textbf{Muschel (Nacre/Perlmutt)}: Ziegelstein-MÃ¶rtel-Mikrostruktur
          aus 95\,\% Aragonit + 5\,\% Protein; ZÃ¤higkeit 3000$\times$ grÃ¶ÃŸer als
          reines Aragonit -- Inspiration fÃ¼r keramische Verbundwerkstoffe
    \item[-] \textbf{Spinnennetze}: optimale Spannungsverteilung, selbstheilend;
          Inspiration fÃ¼r Seilnetzwerke und Membranstrukturen
    \item[-] \textbf{TermitenhÃ¼gel}: passive Klimatisierung ohne KÃ¼hlung --
          Fassadendesign fÃ¼r GebÃ¤ude (Eastgate Centre, Simbabwe)
\end{itemize}
\end{beobachtung}
...
(LÃ¤nge $\approx 2{,}5\,\mu\mathrm{m}$ in Ruhe):
\begin{itemize}
    \item[-] \textbf{Actin-Filamente} (dÃ¼nn, $\varnothing \approx 7\,\mathrm{nm}$):
    ...
    \item[-] \textbf{Myosin-Filamente} (dick, $\varnothing \approx 15\,\mathrm{nm}$):
          QuerbrÃ¼cken ($S_1$-KÃ¶pfe) binden an Actin und erzeugen Kraft durch ATP-Hydrolyse
    \item[-] \textbf{QuerbrÃ¼ckenzyklus}: Bindung $\to$ Kraftschlag (Power Stroke, $5\ldots10\,\mathrm{nm}$)
          $\to$ AblÃ¶sung (ATP bindet) $\to$ erneute Bindung
    \item[-] \textbf{Maximale Kontraktionsgeschwindigkeit}: $v_{max} \approx 5\,l_0/\mathrm{s}$
          (MuskellÃ¤nge $l_0$ pro Sekunde) fÃ¼r schnelle Fasern (Typ IIx)
\end{itemize}
\end{definition}
...
ErmÃ¼dung & langsam & mittel & schnell \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{satz}[Kraft-LÃ¤ngen-Relation des Sarkomers]
Die aktive isometrische Kraft eines Sarkomers ist maximal bei optimaler
Ãœberlappung von Actin und Myosin-Filamenten (optimale SarkomerlÃ¤nge
$l_{opt} \approx 2{,}0\ldots2{,}2\,\mu\mathrm{m}$) und fÃ¤llt bei kÃ¼rzerer
oder lÃ¤ngerer SarkomerlÃ¤nge ab:
\[
  ...
  \end{cases}
\]
(parabolische NÃ¤herung, Breite $w \approx 0{,}5\,l_{opt}$).
\end{satz}
\begin{proof}
Bei kurzer SarkomerlÃ¤nge ($l < l_{min}$): Actin-Filamente Ã¼berlappen
sich gegenseitig -- StÃ¶rung des QuerbrÃ¼ckenzyklus, Kraft sinkt.
Bei optimaler LÃ¤nge: maximale QuerbrÃ¼ckenanzahl aktiv.
Bei langer SarkomerlÃ¤nge ($l > l_{max}$): Ãœberlappung Actin-Myosin zu gering --
weniger QuerbrÃ¼cken bildbar, Kraft sinkt. Die parabolische Approximation
folgt aus der linearen Abnahme der QuerbrÃ¼ckenanzahl mit dem Abstand von $l_{opt}$.
\end{proof}

\section{Gelenkbiomechanik und Knorpelgewebe}

\begin{definition}[Hyaliner Knorpel -- Struktur und mechanische Eigenschaften]
Hyaliner Knorpel bedeckt die GelenkflÃ¤chen und Ã¼bernimmt Druck- und Scherlasten.
Aufbau:
\begin{itemize}
...
\end{itemize}
Biphasisches Modell (Mow 1980): Knorpel als durchstrÃ¶mtes poroelastisches
Medium; effektiver Druckmodul $H_A \approx 0{,}5\ldots1{,}5\,\mathrm{MPa}$;
PermeabilitÃ¤t $k \approx 10^{-15}\,\mathrm{m^4/(N\,s)}$.
\end{definition}

\begin{satz}[Hertz'sche Kontakttheorie fÃ¼r Gelenke]
Zwei elastische Kugeln (Radien $R_1$, $R_2$, E-Module $E_1$, $E_2$,
Querdehnzahlen $\nu_1$, $\nu_2$) unter Normalkraft $F$ erzeugen eine
kreisfÃ¶rmige KontaktflÃ¤che mit Radius:
\[
...
\end{satz}
\begin{proof}
Hertz (1882) lÃ¶ste das Kontaktproblem durch Potentialtheorie:
Der Druckverteilung $p(r) = p_0\sqrt{1-(r/a)^2}$ (halbelliptisch)
entspricht in der Hertz-Theorie eine parabolische Verformung beider OberflÃ¤chen,
aus deren KompatibilitÃ¤tsbedingung die Formeln fÃ¼r $a$ und $p_0$ folgen.
\end{proof}
...
sehr hoch fÃ¼r weiches Gewebe; begrÃ¼ndet die hohe PrÃ¤valenz von Kniearthrose.
\end{beispiel}

\section{Biomechanik der WirbelsÃ¤ule}
...
\begin{itemize}
    \item[-] \textbf{Axiale Druckbelastung}: Nucleus wird komprimiert
          ...
          $\sigma_\theta = p_N R_N / h_A$ (analog DruckbehÃ¤lter)
    ...
    \item[-] \textbf{Bandscheiben-Prolaps}: Riss im Anulus $\to$ Nucleus-Material
          tritt aus $\to$ Kompression der Nervenwurzel; Indikation fÃ¼r Operation
          bei neurologischen AusfÃ¤llen
\end{itemize}
\end{definition}

\begin{beobachtung}[Lumbalbelastung beim Heben -- MuskelkrÃ¤fte]
Das Heben einer Last $F_L = 200\,\mathrm{N}$ in gebeugter Haltung
($\alpha = 30^\circ$ zur Vertikalen) erzeugt ein Biegemoment an L5/S1.
Hebelarm der Last $l_L \approx 0{,}4\,\mathrm{m}$, Hebelarm der RÃ¼ckenstrecker
...
-- ca. $2\times$ KÃ¶rpergewicht allein durch Heben von $20\,\mathrm{kg}$!
Diese hohe Gelenkdruckkraft erklÃ¤rt die hohe Bandscheibendegenerationsrate
bei schwerer kÃ¶rperlicher Arbeit.
\end{beobachtung}
...
Die willkÃ¼rliche Bewegungssteuerung beim Menschen ist hierarchisch organisiert:
\begin{itemize}
    \item[-] \textbf{Kortex (Planung)}: primÃ¤rer motorischer Kortex (M1),
          prÃ¤motorischer Kortex (PMC); Aktivierungssequenzen fÃ¼r Bewegungen
    ...
    \item[-] \textbf{Kleinhirn (Cerebellum)}: Feinabstimmung und internemL Ã¶dell;
          prÃ¤diktive Vorsteuerung; Ausfall $\to$ Ataxie (unkoordinierte Bewegung)
    \item[-] \textbf{RÃ¼ckenmark}: Motorische Einheiten angesteuert durch
          Alpha-Motoneurone; ReflexbÃ¶gen (Muskelspindel, Golgi-Sehnenorgan)
...
Der Muskel-Sehnen-Apparat enthÃ¤lt zwei wichtige Mechanorezeptoren:
\begin{itemize}
    \item[-] \textbf{Muskelspindeln}: intrafusale Muskelfasern mit Ia-Afferenzen
          (Geschwindigkeit) und II-Afferenzen (LÃ¤nge); bei schneller Dehnung:
    ...
    \item[-] \textbf{Golgi-Sehnenorgan} (GTO): in Sehnen eingebettet;
          Ib-Afferenzen; misst Muskelkraft; bei Ãœberlastung:
...
der ohne bewusste Kontrolle GelenkstabilitÃ¤t und Bewegungskoordination sichert.
\end{beobachtung}

\begin{satz}[Biomechanisches StabilitÃ¤tskriterium am Gelenk]
Ein Gelenk ist mechanisch stabil, wenn die Summe der Gelenkdruckkraft $\vec{F}_D$
(resultierend aus MuskelkrÃ¤ften und externen Lasten) innerhalb der
\textbf{stationÃ¤ren StÃ¼tzbasis} (convex hull der KontaktflÃ¤che) liegt:
\[
  \vec{F}_D\cdot\vec{n} > 0 \quad \text{und} \quad
  \frac{F_{D,\parallel}}{F_{D,\perp}} < \mu_s
  \quad (\text{kein Gleiten}).
\]
FÃ¼r das Kniegelenk bedeutet dies: die Resultierende muss durch das
Zentrum des Tibia-Plateaus verlaufen (innerhalb der KontaktflÃ¤che),
sonst droht Subluxation oder BandÃ¼berdehnung.
\end{satz}
...
\begin{itemize}
    \item[-] \textbf{BiokompatibilitÃ¤t}: kein Zellschaden, keine EntzÃ¼ndung,
          keine allergische Reaktion (ISO 10993 -- BiokompatibilitÃ¤tsprÃ¼fung)
    \item[-] \textbf{BiostabilitÃ¤t vs.\ Bioabbaubarkeit}: dauerhaft
          (z.\,B.\ HÃ¼ftprothese) oder resorbierbar (z.\,B.\ Nahtmaterial, Knochenersatz)
    \item[-] \textbf{Mechanische KompatibilitÃ¤t}: Steifigkeit angepasst an
          Umgebungsgewebe (Stress Shielding bei zu steifen Implantaten!)
\end{itemize}
Wichtige Klassen:
\begin{itemize}
    \item[-] \textbf{Metalle}: Ti-6Al-4V (HÃ¼ft- und Knieprothesen, Dentalimplantate);
          Co-Cr-Mo (Gelenkpfannen, WirbelkÃ¶rperersatz); 316L-Stahl (Osteosynthesplatten)
    \item[-] \textbf{Keramiken}: $\mathrm{Al_2O_3}$, ZrO$_2$ (Gleitpaarungen im HÃ¼ftgelenk;
          niedrigstem Abrieb); Hydroxylapatit (KnochenfÃ¼ller, Beschichtung)
    \item[-] \textbf{Polymere}: UHMWPE (Tibiaplateau, Acetabulum);
          PEEK (WirbelkÃ¶rperkÃ¤fig, Knochenplatte); Silikon (Brustimplantate)
    \item[-] \textbf{Verbundwerkstoffe}: CFK-Schaft bei HÃ¼ftprothesen
          (geringer Stress-Shielding-Effekt)
\end{itemize}
\end{definition}

\begin{beobachtung}[Stress Shielding bei HÃ¼ftprothesenschaft]
Ein zu steifer Prothesenschaft ($E_{Ti} \approx 110\,\mathrm{GPa}$
vs.\ $E_{Knochen} \approx 15\,\mathrm{GPa}$) Ã¼bernimmt den grÃ¶ÃŸten Teil
...
LÃ¶sungsansÃ¤tze:
\begin{itemize}
    \item[-] Proximal beschichteter Kurzschaft (Krafteinleitung nahe HÃ¼ftkopf)
    \item[-] CFK-SchÃ¤fte ($E_{CFK} \approx 40\,\mathrm{GPa}$, LÃ¤ngsrichtung)
    \item[-] Gradierte PorositÃ¤t (additiv gefertigt): von porÃ¶s proximal
          zu massiv distal, um Steifigkeitssprung zu reduzieren
\end{itemize}
\end{beobachtung}

\begin{definition}[Osseointegration und ImplantatoberflÃ¤chen]
\textbf{Osseointegration} (BrÃ¥nemark 1969) bezeichnet die direkte,
...
\begin{itemize}
    \item[-] \textbf{Biologischer Ablauf}: Blutkoagulum $\to$ Proteinadsorption
          ...
          Knochenapposition; vollstÃ¤ndige Osseointegration nach $3\ldots6$ Monaten
    \item[-] \textbf{OberflÃ¤chentopographie}: Rauheit $R_a = 1\ldots10\,\mu\mathrm{m}$
          optimal (SLA -- Sand-blasted, Large-grit, Acid-etched);
          zu glatt: schlechte ZelladhÃ¤sion; zu rau: Bakterienansiedlung
    \item[-] \textbf{OberflÃ¤chenchemie}: TiO$_2$-Schicht auf Titan ist bioaktiv;
    ...
    \item[-] \textbf{Anodisierung}: anodische Oxidation erzeugt geordnete
          TiO$_2$-NanorÃ¶hren ($\varnothing \approx 100\,\mathrm{nm}$);
          verbesserte ZelladhÃ¤sion, antibakteriell durch Silber-Nanopartikel
\end{itemize}
\end{definition}

\section{Sport- und Rehabilitationsbiomechanik}

\begin{beobachtung}[Laufbiomechanik und VerletzungsprÃ¤vention]
Laufen unterscheidet sich vom Gehen durch eine \textbf{Flugphase}
(beide FÃ¼ÃŸe in der Luft). Biomechanische Kennwerte:
\begin{itemize}
    \item[-] \textbf{Bodenreaktionskraft}: Einschlagspitze ($> 2\,\times$ KÃ¶rpergewicht bei Fersenaufprall)
          und Vortriebsspitze; BarfuÃŸlaufen/MittelfuÃŸlauf reduziert Einschlagspitze
    \item[-] \textbf{Kniemoment}: maximales Kniebeugegelenksmoment
          $M_K \approx 150\,\mathrm{Nm}$ beim Bergablaufen; VKB-ReiÃŸfestigkeit $\approx 2000\,\mathrm{N}$
    \item[-] \textbf{Kadenz-Optimum}: $\approx 170\ldots180$ Schritte/min reduziert
          StoÃŸbelastung; Achillessehne trÃ¤gt bis $10\times$ KÃ¶rpergewicht bei Sprint
    \item[-] \textbf{Schuhtechnologie}: EVA-Zwischensohle (Energieabsorption $\approx 80\,\%$);
          Carbon-Plate (Sprinter: Steifigkeit $\to$ EnergierÃ¼ckgabe durch gebogene Faser);
          Schaumstoff-Pfeilmuster (Nike Vaporfly: $4\,\%$ schneller)
\end{itemize}
\end{beobachtung}

\begin{beobachtung}[Exoskelett-Technologie und Rehabilitationsrobotik]
\textbf{Exoskelette} sind tragbare Roboter, die sich um GliedmaÃŸen legen
und Bewegungen unterstÃ¼tzen oder fÃ¼hren:
\begin{itemize}
    \item[-] \textbf{Passive Exoskelette}: Federn/DÃ¤mpfer ohne Aktuatoren;
          unterstÃ¼tzen spezifische Bewegungen durch Energiespeicherung;
          Beispiel: Argo ReWalk (FuÃŸheber), HULC (Lastentragung)
    \item[-] \textbf{Aktive Exoskelette}: Elektromotoren oder Hydraulik;
          Regelung durch EMG (OberflÃ¤chen-Elektromyographie) oder Kraft-
          und Lagesensoren; Beispiel: Ekso, Lokomat (gangtherapeutisch)
    \item[-] \textbf{Regelungsherausforderung}: Intentionserkennung
          ($\tau_{react} < 100\,\mathrm{ms}$); StabilitÃ¤tssicherung bei
          unbeschrÃ¤nkten Nutzerfreiheitsgraden; adaptive Impedanzregelung
    \item[-] \textbf{NeuroplastizitÃ¤t und Rehabilitation}: repetitive,
          aufgabenspezifische BewegungsÃ¼bungen mit Exoskelett regen
          NeuroplastizitÃ¤t an; klinische Studien zeigen verbesserte Rekonvaleszenz
          nach Schlaganfall gegenÃ¼ber konventioneller Therapie
\end{itemize}
\end{beobachtung}

\begin{beispiel}[EMG-basierte Intentionserkennung]
Ein OberflÃ¤chen-EMG-Sensor misst das Elektromyogramm des Tibialis anterior.
Das gleichgerichtete und tiefpassgefilterte Signal (HÃ¼llkurve) wird als
...
geschÃ¤tzt und als Sollwert fÃ¼r den Exoskelett-Aktuator verwendet.
...
\begin{definition}[ViskoelastizitÃ¤t biologischer Gewebe]
Biologische Weichgewebe (Sehnen, BÃ¤nder, ArterienwÃ¤nde, Muskel)
...
\begin{itemize}
    ...
    \item[-] \textbf{Hysterese}: Belastungs- und Entlastungskurve nicht deckungsgleich
          (Energiedissipation durch innere Reibung, FlÃ¼ssigkeitsverschiebung)
\end{itemize}
Einfaches Modell: \textbf{Standard-LinearkÃ¶rper} (Zener-Modell):
\[
  \sigma + \frac{\eta}{E_1}\dot{\sigma} = \left(E_2 + \frac{\eta E_2}{E_1}\right)\varepsilon
  + \eta\dot{\varepsilon},
\]
mit Federn $E_1$, $E_2$ und DÃ¤mpfer $\eta$ (Maxwell-Element parallel zu Feder).
\end{definition}
...
\begin{beobachtung}[J-fÃ¶rmige Spannungs-Dehnungs-Kurve von Kollagen]
Kollagenreiche Gewebe (Sehnen, BÃ¤nder, Haut) zeigen charakteristisch
J-fÃ¶rmige Kurven mit drei Regionen:
\begin{itemize}
    ...
    \item[-] \textbf{Versagensbereich} ($\varepsilon > 4\,\%$): FaserbrÃ¼che;
...
der Schwingungslehre mit praktischen Anwendungen der MaschinenzustandsÃ¼berwachung.
Sie ist heute ein unverzichtbares Werkzeug der prÃ¤diktiven Instandhaltung.

\section{Messkette und Sensoren}

\begin{definition}[Messkette in der Schwingungsmesstechnik]
Eine vollstÃ¤ndige \textbf{Messkette} zur Erfassung mechanischer Schwingungen
besteht aus:
\begin{itemize}
    ...
    \item[-] \textbf{Signalkonditionierung}: LadungsverstÃ¤rker, Konstantstromversorgung,
...
exakt rekonstruiert werden, wenn die Abtastfrequenz $f_s > 2f_{\max}$ betrÃ¤gt.
Die Abtastfrequenz $2f_{\max}$ heiÃŸt \textbf{Nyquist-Frequenz}.
\end{satz}
\begin{proof}
...
Wenn $f_s > 2f_{\max}$, Ã¼berlappen sich die periodischen Kopien von $X(f)$
...
$x[n]$, $n = 0, \ldots, N-1$, der LÃ¤nge $N$:
\[
...
weist auf einen AuÃŸenring-Schaden hin.
\end{beispiel}
...
Die EMA validiert FEM-Modelle und ermÃ¶glicht Strukturmodifikations-vorhersagen.
\end{beobachtung}

% ============================================================

\section{Ordnungsanalyse und drehzahlabhÃ¤ngige Signale}

\begin{definition}[Ordnungsanalyse -- Grundprinzip]
Bei rotierenden Maschinen (Motoren, Getriebe, LÃ¼fter) hÃ¤ngen die
...
\section{Akustische Emissionen und KÃ¶rperschalldiagnose}

\begin{definition}[Akustische Emission (AE)]
\textbf{Akustische Emissionen} sind transiente elastische Wellen,
die durch schnelle lokale MaterialverÃ¤nderungen (Rissausbreitung,
ReibvorgÃ¤nge, Phasenumwandlungen) entstehen.
Eigenschaften:
\begin{itemize}
    ...
    \item[-] \textbf{Anwendungen}: RissÃ¼berwachung in DruckbehÃ¤ltern,
          WÃ¤lzlagerschÃ¤den, SchleifÃ¼berwachung, Verbundwerkstoff-PrÃ¼fung (Delaminierung)
\end{itemize}
\end{definition}

\begin{beobachtung}[Quellenlokalisierung mit AE-Sensoren]
Mit \textbf{mehreren AE-Sensoren} (Positionen $\vec{r}_i$) lÃ¤sst sich
...
Mindestens 3 Sensoren fÃ¼r 2D-, 4 Sensoren fÃ¼r 3D-Lokalisierung nÃ¶tig;
AuflÃ¶sung typisch $< 1\,\mathrm{mm}$ in Laborbedingungen.
\end{beobachtung}

\section{MaschinenzustandsÃ¼berwachung und Predictive Maintenance}

\begin{definition}[ISO 13373 -- ZustandsÃ¼berwachung rotierender Maschinen]
Die Norm \textbf{ISO 13373} definiert Grenzwerte fÃ¼r Schwingungsgeschwindigkeit
...
III & GroÃŸ, starr & $< 1{,}8\,\mathrm{mm/s}$ & $> 11{,}2\,\mathrm{mm/s}$ \\
IV & GroÃŸ, flexibel & $< 2{,}8\,\mathrm{mm/s}$ & $> 18\,\mathrm{mm/s}$ \\
\bottomrule
\end{tabular}
\end{center}
\end{definition}

\begin{beobachtung}[TrendÃ¼berwachung und Restlebensdauerprognose]
\textbf{TrendÃ¼berwachung} verfolgt die zeitliche Entwicklung von Kennwerten
...
\begin{itemize}
    ...
    \item[-] \textbf{Bayes'sche Filterung (Partikelfilter)}: aktualisiert
          probabilistische RUL-SchÃ¤tzung bei jedem neuen Messwert;
          liefert Konfidenzintervall der Restlebensdauer
\end{itemize}
Wirtschaftliche Wirkung: Predictive Maintenance reduziert ungeplante
Stillstandszeiten um bis zu $70\,\%$ und verlÃ¤ngert Maschinenlebensdauer
um $20\ldots40\,\%$.
\end{beobachtung}

\begin{beispiel}[WÃ¤lzlagerdiagnose -- vollstÃ¤ndiger Workflow]
...
\textbf{HÃ¼llkurvenanalyse}: Bandpass bei $8\ldots12\,\mathrm{kHz}$ (GehÃ¤use-Resonanz),
Gleichrichtung, Tiefpass, FFT $\to$ Peak bei $f_{BPFO} \approx 48{,}5\,\mathrm{Hz}$
und Oberwellen $2f_{BPFO}$, $3f_{BPFO}$.
\textbf{Diagnose}: AuÃŸenringschaden; Restlebensdauer-Prognose: ca.\ 3~Wochen
(lineare Kurtosis-Extrapolation auf Grenzwert~$= 20$).
\textbf{MaÃŸnahme}: Lagerwechsel in der nÃ¤chsten geplanten Wartung in 2~Wochen.
\end{beispiel}

\section{ModalprÃ¼fung und Strukturdynamik in der Praxis}

\begin{beobachtung}[Betriebsschwingungsanalyse (ODS)]
Die \textbf{Operational Deflection Shape} (ODS) zeigt die tatsÃ¤chliche
Schwingungsform einer Maschine unter Betriebsbedingungen --
im Gegensatz zur Modalanalyse, die Eigenschwingungsformen bei
kÃ¼nstlicher Anregung misst:
\begin{itemize}
    ...
    \item[-] Darstellung als animierte Bewegung bei jeder Frequenz;
          Identifikation von Resonanzen, KÃ¶rperschallpfaden, EntkopplungsmaÃŸnahmen
    \item[-] Anwendung: GerÃ¤uschminderung in Fahrzeugen (NVH -- Noise, Vibration, Harshness),
          MaschinengehÃ¤use, Elektromotor-Akustik
\end{itemize}
\end{beobachtung}
...
Tribologie beschreibt Reibung, VerschleiÃŸ und Schmierung zwischen
sich berÃ¼hrenden und relativ zueinander bewegenden OberflÃ¤chen.
Tribologische Verluste machen nach SchÃ¤tzungen ca. 20\,\% des
globalen PrimÃ¤renergiebedarfs aus.
...
\begin{itemize}
    \item[-] \textbf{Haftreibung} (Statische Reibung): $F_{R,H} \le \mu_H F_N$;
          K\"orper in Ruhe; $\mu_H$ hÃ¤ngt von Werkstoffen, OberflÃ¤chenrauheit
    ...
    \item[-] \textbf{Bohr-/Pivotreibung}: beim Drehen unter Normallast
          (WÃ¤lzlagerkÃ¤fig, Zapfenreibung)
\end{itemize}
\end{definition}

\begin{satz}[Stribeck-Kurve -- SchmierungszustÃ¤nde]
Der Reibungskoeffizient $\mu$ zwischen zwei geschmierten FlÃ¤chen
hÃ¤ngt von der Sommerfeldzahl $So = \eta v / (p \cdot h)$ ab
(dynamische ViskositÃ¤t $\eta$, Gleitgeschwindigkeit $v$, spez. Druck $p$, SpaltmaÃŸ $h$):
\begin{itemize}
    \item[-] \textbf{Grenzschmierung} (kleines $So$): Metallkontakt, hoher VerschleiÃŸ,
          $\mu \approx 0{,}1\ldots0{,}3$
    \item[-] \textbf{Mischschmierung}: Teilvollschmierung, $\mu \approx 0{,}01\ldots0{,}1$
    \item[-] \textbf{Hydrodynamische Schmierung} (groÃŸes $So$): vollstÃ¤ndiger Schmierfilm,
          kein Metallkontakt, $\mu \approx 0{,}001\ldots0{,}01$
    \item[-] \textbf{Elastohydrodynamische Schmierung (EHD)}: bei Hertzscher Punktlast
          (WÃ¤lzlager, ZahnrÃ¤der); elastische Verformung und DruckerhÃ¶hung
          des Ã–ls prÃ¤gen das Reibungsverhalten
\end{itemize}
\end{satz}

\section{VerschleiÃŸmechanismen und Lebensdauer}

\begin{definition}[Grundlegende VerschleiÃŸmechanismen]
Nach Fleischer (1990) unterscheidet man vier grundlegende Mechanismen:
\begin{itemize}
    \item[-] \textbf{AdhÃ¤siver VerschleiÃŸ}: Bildung und AbreiÃŸen von
          GrenzflÃ¤chenkontakten (Kalt-VerschweiÃŸen); typisch bei
          metallic-on-metallic unter Grenzschmierung
    \item[-] \textbf{Abrasiver VerschleiÃŸ}: MikropflÃ¼gen und Mikrospanen
          durch harte Rauheitsspitzen oder Partikel (Sandpapiereffekt)
    \item[-] \textbf{Tribochemischer VerschleiÃŸ}: chemische Reaktion
          durch tribologische Aktivierung (Oxidation, Tribokatalyse)
    \item[-] \textbf{OberflÃ¤chenzerrÃ¼ttung}: Rissbildung und Materialabtrag
          durch zyklische Kontaktbeanspruchung (Pitting, GrÃ¼bchenbildung)
\end{itemize}
\end{definition}

\begin{satz}[Archard-VerschleiÃŸgesetz]
Das lineare VerschleiÃŸvolumen $V_W$ nach dem \textbf{Archard-Modell}:
\[
  V_W = k\,\frac{F_N\,s}{H},
\]
mit dimensionslosem VerschleiÃŸkoeffizient $k$ (materialabhÃ¤ngig,
$10^{-7}\ldots10^{-3}$), Normalkraft $F_N$, Gleitweg $s$ und
HÃ¤rte $H$ des weicheren Partners.
\end{satz}
\begin{proof}
Annahme: Realer Kontakt findet an $n$ kreisfÃ¶rmigen Mikrokontakten
statt mit Radius $a$ und Normalkraft $F_i = H\pi a_i^2$ (plastische Verformung).
Das VerschleiÃŸvolumen pro Mikrokontakt $\sim a_i^3$ (Halbkugel), Ã¼ber alle Kontakte:
$V_W \propto \sum_i a_i^3 = k \sum_i a_i^2 \cdot a_i$.
Mit $\sum_i \pi a_i^2 = F_N/H$ (GesamtkontaktflÃ¤che) folgt $V_W = k F_N s / H$.
\end{proof}

% ============================================================
% ===== ERWEITERUNG KAPITEL 17: Konstruktionsmethodik =====

\section{TRIZ -- Theorie des erfinderischen ProblemlÃ¶sens}

\begin{beobachtung}[TRIZ-Grundprinzipien nach Altshuller]
\textbf{TRIZ} (russisch: Teorija reschenija izobretatjelskich sadatsch)
ist eine systematische KreativitÃ¤tsmethode, entwickelt von Genrich Altshuller
(1946--1998) durch Analyse von $> 400\,000$ Patenten.
Kernaussage: Erfinderische Probleme folgen wiederkehrenden Mustern --
\textbf{WidersprÃ¼che} (technische und physikalische) sind das HerzstÃ¼ck:
\begin{itemize}
    ...
    \item[-] \textbf{Physikalischer Widerspruch}: ein Parameter soll gleichzeitig
          zwei gegensÃ¤tzliche Werte annehmen (z.\,B.\ heiÃŸ UND kalt)
    ...
    \item[-] \textbf{IdealitÃ¤tsprinzip}: das ideale Endresultat (IER) --
          das System erfÃ¼llt seine Funktion, ohne zu existieren (kein Aufwand, kein Schaden)
\end{itemize}
\end{beobachtung}
...
\textbf{Widerspruch}: Schneidgeometrie $\uparrow$ $\Rightarrow$ Reibung $\uparrow$ (DrehmomenthÃ¶he).
\textbf{TRIZ-Prinzip}: Segmentierung -- Schraubengang in Abschnitte unterteilen,
die nur teilweise schneiden. LÃ¶sung: asymmetrische Gewindegangsteigung,
...
Die \textbf{FehlermÃ¶glichkeits- und -einflussanalyse} (FMEA) ist eine
...
\end{itemize}
Kern: die \textbf{RisikoprioritÃ¤tszahl} (RPZ):
\[
...
MaÃŸnahmen bei $\mathrm{RPZ} > 100$ oder $B = 9\ldots10$ (sicherheitskritisch).
\end{definition}

\begin{beobachtung}[Neue FMEA-Methodik nach AIAG \& VDA (2019)]
Die \textbf{harmonisierte FMEA} (AIAG-VDA 2019) ersetzt RPZ durch
\textbf{AktionsprioritÃ¤t} (AP: Hoch, Mittel, Niedrig) basierend auf
einer Lookup-Tabelle fÃ¼r $B$, $A$, $E$:
\begin{itemize}
    \item[-] \textbf{AP Hoch}: sofortiger Handlungsbedarf; $B \ge 9$ immer ``Hoch''
    \item[-] \textbf{AP Mittel}: MaÃŸnahmen bei nÃ¤chster Produktgeneration
...
\textbf{Genichi Taguchi} definierte QualitÃ¤tsverlust als Abweichung vom Zielwert,
nicht als Unter-/Ãœberschreitung von Toleranzgrenzen.
Die \textbf{quadratische Verlustfunktion}:
\[
  L(y) = k\,(y - m)^2,
\]
mit Ist-Wert $y$, Zielwert $m$ und Verlustkoeffizient $k = A_0/\Delta_0^2$
(Verlustkosten $A_0$ bei GrenzÃ¼berschreitung $\Delta_0$).
\end{definition}

\begin{satz}[Signal-Rausch-VerhÃ¤ltnis (SNR)]
Das \textbf{Signal-Rausch-VerhÃ¤ltnis} nach Taguchi bewertet, wie robust
ein Produkt oder Prozess gegenÃ¼ber StÃ¶rgrÃ¶ÃŸen (Rauschen) ist.
FÃ¼r den Typ ``Zielwert ist best'' (Nominal-the-Best):
\[
...
bei gleichzeitiger Minimierung des Einflusses von RauschgrÃ¶ÃŸen (Ã¤uÃŸeres Array).
\end{satz}
...
\begin{itemize}
    ...
    \item[-] \textbf{Montagerichtlinien}: Teile von oben einlegen (Schwerkraft nutzen);
          Selbstzentrierung durch Fasen/Kegel; keine Neuausrichtung nÃ¶tig;
          Handhabungsindex minimieren (Boothroyd-Dewhurst-Methode)
    \item[-] \textbf{DFM}: fertigungsgerechte Toleranzen (nicht enger als nÃ¶tig);
          Hinterschneidungen vermeiden; Standardwerkzeuge bevorzugen;
          OberflÃ¤chenrauheit $R_a$ nur dort vorgeben, wo funktionsnÃ¶tig
    \item[-] \textbf{Design for Recycling (DfR)}: sortenreine Trennbarkeit;
          keine unlÃ¶sbaren Verbindungen; Materialkennung (Kunststoffcodes);
          Schadstoffe vermeiden
\end{itemize}
\end{beobachtung}

\begin{beobachtung}[Design for Reliability (DfR) -- ZuverlÃ¤ssigkeitstechnik]
Die \textbf{ZuverlÃ¤ssigkeit} $R(t) = P(T > t)$ (Ãœberlebenswahrscheinlichkeit
bis Zeit $t$, Ausfallzeitpunkt $T$) wird durch die \textbf{Badewannenkurve}
der Ausfallrate $\lambda(t)$ beschrieben:
\begin{itemize}
    \item[-] \textbf{FrÃ¼hausfÃ¤lle} (sinkende $\lambda$): Fertigungsfehler, Materialfehler;
          Burn-in-Tests eliminieren diese vor Auslieferung
    \item[-] \textbf{ZufallsausfÃ¤lle} (konstante $\lambda$): zufÃ¤llige BeanspruchungsÃ¼berschreitungen;
          Weibull-Verteilung mit Formfaktor $\beta = 1$: Exponentialverteilung
    \item[-] \textbf{VerschleiÃŸausfÃ¤lle} (steigende $\lambda$): ErmÃ¼dung, Korrosion, Abrieb;
          Weibull $\beta > 1$
\end{itemize}
\textbf{Weibull-Verteilung} fÃ¼r Ausfallwahrscheinlichkeit:
\[
  F(t) = 1 - e^{-(t/\eta)^\beta},
...
\begin{beispiel}[SystemzuverlÃ¤ssigkeit aus KomponentenzuverlÃ¤ssigkeiten]
...
technischen Problem zur fertigungsreifen LÃ¶sung. Sie verbindet
...
\begin{itemize}
    \item[-] \textbf{Phase 1: KlÃ¤ren der Aufgabe}: Lastenheft, Anforderungsliste
          (Forderungen, WÃ¼nsche, Festforderungen)
    \item[-] \textbf{Phase 2: Ermitteln von Funktionen und Strukturen}:
          Funktionsstruktur, Schwarz-WeiÃŸ-Box-Modell
    \item[-] \textbf{Phase 3: Suchen nach LÃ¶sungsprinzipien}:
    ...
    \item[-] \textbf{Phase 5: Gestalten der maÃŸgebenden Module}:
          Entwurfszeichnungen, Ãœberschlagsberechnungen
    \item[-] \textbf{Phase 6: Gestalten des gesamten Produkts}:
          Detailkonstruktion, Zeichnungssatz, StÃ¼ckliste
    \item[-] \textbf{Phase 7: Ausarbeiten der AusfÃ¼hrungs- und Nutzungsangaben}:
...
\begin{itemize}
    \item[-] \textbf{Prinzip der Krafteinleitung}: KrÃ¤fte mÃ¶glichst direkt
          (kurze, steife Kraftpfade); Vermeidung von Biegemomenten wo mÃ¶glich
    \item[-] \textbf{Prinzip der Einfachheit}: Wenige Teile, einfache Geometrie,
          Normteile bevorzugen
    \item[-] \textbf{SelbstverstÃ¤rkendes/selbstsicherndes Prinzip}:
          Belastung erhÃ¶ht Haltekraft (Keilwirkung, Reibschluss)
    ...
    \item[-] \textbf{Prinzip der klaren und eindeutigen Funktion}:
          jedes Bauteil soll genau eine Funktion erfÃ¼llen
...
\begin{beobachtung}[Ãœberschlagsrechnung und analytische Auslegung]
Der erfahrene Konstrukteur beherrscht die \textbf{Ãœberschlagsrechnung}:
Durch sinnvolle Vereinfachungen und AbschÃ¤tzungen wird die GrÃ¶ÃŸenordnung
der maÃŸgebenden Beanspruchung in Minuten ermittelt.
Typische Vorgehensweise:
\begin{itemize}
    \item[-] KrÃ¤fte und Momente aus den Betriebsbedingungen berechnen
    \item[-] MaÃŸgebende Beanspruchung identifizieren (Biegung? Torsion? Druck?)
    \item[-] Querschnitt aus Festigkeitsbedingung $\sigma \le R_e/S$ dimensionieren
    \item[-] Normbauteile (Lager, Schrauben, Profile) aus Katalogen wÃ¤hlen
    \item[-] Nachrechnung mit FEM fÃ¼r den Detailentwurf
\end{itemize}
Das \textbf{Fermi-Denken} -- Zerlegen komplexer Probleme in abschÃ¤tzbare
Teilfragen -- ist dabei eine SchlÃ¼sselkompetenz des Ingenieurs.
\end{beobachtung}
...
\]
Die Variante mit dem hÃ¶chsten $NW_i$ ist die empfohlene LÃ¶sung,
...
Technische Normen sichern QualitÃ¤t, Sicherheit und KompatibilitÃ¤t.
Hierarchie der Normung:
\begin{itemize}
    \item[-] \textbf{International}: ISO (Mechanik, Werkstoff, QualitÃ¤t),
          IEC (Elektrotechnik), ASME (DruckbehÃ¤lter, USA)
    \item[-] \textbf{EuropÃ¤isch}: EN-Normen (harmonisiert, CE-Kennzeichnung);
...
und verpflichtet zur Risikobeurteilung fÃ¼r jede in der EU in
...
Bausteine verteilt und wie diese miteinander verknÃ¼pft sind:
\begin{itemize}
    \item[-] \textbf{Integrale Architektur}: viele Funktionen in einem
          Bauteil vereint; kompakt, leicht, aber kaum austauschbar;
          Beispiel: gegossenes MotorgehÃ¤use mit KÃ¼hlkanÃ¤len und Lagersitzen
    \item[-] \textbf{Modulare Architektur}: klare Schnittstellen zwischen
          austauschbaren Modulen; Plattformstrategie mÃ¶glich;
          Beispiel: VW-Modularer Querbaukasten (MQB) -- Motoren, Getriebe,
          Fahrwerk modular kombinierbar Ã¼ber Modellreihen hinweg
    \item[-] \textbf{Hybride Architekturen}: in der Praxis hÃ¤ufig;
          einige Module fest integriert, andere austauschbar
\end{itemize}
Entscheidungskriterien: StÃ¼ckzahl, Variantenvielfalt, Updatefrequenz,
...
(Plattform) fÃ¼r mehrere Produktvarianten:
\begin{itemize}
    \item[-] \textbf{Skaleneffekte}: hÃ¶here StÃ¼ckzahlen auf der Plattform
          $\to$ niedrigere Fertigungskosten (Lernkurveneffekt:
          $-15\ldots20\,\%$ Kosten bei StÃ¼ckzahlverdoppelung)
    \item[-] \textbf{Entwicklungskostenteilung}: Entwicklungskosten werden
          auf mehr Produkte verteilt; Amortisationszeit verkÃ¼rzt sich
    \item[-] \textbf{Lieferantenkonsolidierung}: weniger Teile-Nummern;
          bessere Verhandlungsposition; hÃ¶here QualitÃ¤tssicherung
    \item[-] \textbf{Risiko}: Plattformfehler trifft alle Varianten;
          mangelnde Differenzierung gegenÃ¼ber Wettbewerb
\end{itemize}
\end{beobachtung}
...
\begin{itemize}
    ...
    \item[-] \textbf{Stage 1: Scoping} -- Machbarkeit prÃ¼fen; Wettbewerbsanalyse
...
Ã¼ber WeiterfÃ¼hrung, Abbruch oder Schleife.
\end{definition}
...
\begin{itemize}
    \item[-] \textbf{FrÃ¼hzeitige Fertigungsintegration}: Fertigungsplaner
          arbeiten ab Phase~1 mit; DFM-Anforderungen flieÃŸen sofort ein
    ...
    \item[-] \textbf{FMEA im Entwicklungsteam}: Design-FMEA wird gemeinsam
          von Konstruktion, Fertigung und QualitÃ¤t erstellt -- keine
          sequentielle Ãœbergabe
    \item[-] \textbf{Digital Prototype}: vollstÃ¤ndige CAD-Baugruppe schon
...
\begin{itemize}
    \item[-] \textbf{Patentrecherche}: Espaceenet (EPO), Google Patents,
          DEPATISnet (DPMA); Suche nach relevanten AnsprÃ¼chen im Technologiefeld
    \item[-] \textbf{Claim-Analyse}: ein Patent verletzt, wenn alle Merkmale
          eines unabhÃ¤ngigen Anspruchs im Produkt realisiert sind
          (Feature-by-Feature-Analyse)
    \item[-] \textbf{Umgehung}: Design-around durch alternative technische LÃ¶sung;
    ...
    \item[-] \textbf{Eigene Schutzrechte}: Patentanmeldung (20 Jahre Schutz),
          ...
          Marke (unbegrenzt verlÃ¤ngerbar)
\end{itemize}
\end{beobachtung}

\section{KostenabschÃ¤tzung in frÃ¼hen Entwicklungsphasen}

\begin{definition}[HerstellkostenabschÃ¤tzung -- Parametrische Methoden]
In frÃ¼hen Entwicklungsphasen liegen keine Detailzeichnungen vor.
\textbf{Parametrische KostenschÃ¤tzmodelle} nutzen technische KenngrÃ¶ÃŸen:
\[
  K_{Herst} = a_0 + a_1\,m^{a_2}\,V^{a_3}\,n^{a_4}\,\ldots
\]
($m$: Masse, $V$: Volumen, $n$: StÃ¼ckzahl; $a_i$: empirisch kalibrierte Koeffizienten).
...
\begin{itemize}
    ...
    \item[-] \textbf{Fertigungskosten}: Maschinenstundensatz $\times$ Fertigungszeit;
          abhÃ¤ngig von Toleranzen, OberflÃ¤chenrauheit, LosgrÃ¶ÃŸe
    ...
    \item[-] \textbf{Zielkostenrechnung} (Target Costing): Vom Marktpreis
          ausgehend rÃ¼ckwÃ¤rts erlaubte Herstellkosten ermitteln
\end{itemize}
\end{definition}
...
$\approx 5\,\%$ der tatsÃ¤chlichen Entwicklungskosten ausgegeben werden.
\end{satz}
\begin{proof}[BegrÃ¼ndung]
Einmal festgelegte Konstruktionsentscheidungen (Werkstoff, Fertigungsverfahren,
Anzahl der Teile, Toleranzen) bestimmen die gesamte nachgelagerte WertschÃ¶pfungskette.
NachtrÃ¤gliche Ã„nderungen sind exponentiell teurer (Rule of Ten:
jede Entwicklungsphase, in der ein Fehler entdeckt wird, kostet zehnmal mehr
als die vorhergehende Phase). Daraus folgt: die hÃ¶chste Hebelwirkung
...
ZulÃ¤ssige Gesamtkosten: $85\,\mathrm{EUR}$.
Vertrieb/Verwaltung ($25\,\%$): $21{,}25\,\mathrm{EUR}$.
Erlaubte Herstellkosten: $63{,}75\,\mathrm{EUR}$.
Aktuelle KostenschÃ¤tzung: $78\,\mathrm{EUR}$ -- KostenlÃ¼cke $14{,}25\,\mathrm{EUR}$ ($18\,\%$).
MaÃŸnahmen: Materialsubstitution ($-5\,\mathrm{EUR}$), Teilereduktion DFMA ($-6\,\mathrm{EUR}$),
...
und Integrationsphasen, die sich wie ein V gegenÃ¼berstehen:
\begin{itemize}
...
\begin{itemize}
    ...
    \item[-] \textbf{AktivitÃ¤tsdiagramme} (act): FunktionsflÃ¼sse
    \item[-] \textbf{Parametrische Diagramme} (par): Gleichungen und
          Constraints zwischen Systemparametern
\end{itemize}
Vorteile: KonsistenzprÃ¼fung durch Modell, automatische Dokumentengenerierung,
...
Energietechnik beschÃ¤ftigt sich mit der Wandlung, Ãœbertragung,
Speicherung und effizienten Nutzung von Energie. Sie ist das
technische RÃ¼ckgrat der Energiewende.

\section{Elektrische Antriebe}

\begin{definition}[Permanentmagnet-Synchronmotor (PMSM)]
Der \textbf{PMSM} ist heute der bevorzugte Antrieb fÃ¼r Elektrofahrzeuge
...
\]
mit Polpaarzahl $p$, Flussverkettungen $\psi_{d,q}$ und StrÃ¶men $i_{d,q}$
im rotorfesten $d$-$q$-Koordinatensystem.
Optimale Momentenausnutzung (MTPC -- Maximum Torque Per Current):
$i_d = 0$ (bei OberflÃ¤chen-PMSM ohne Reluktanzanteil).
\end{definition}
...
$d$- und $q$-Achsen und erlaubt unabhÃ¤ngige PI-Regler fÃ¼r $i_d$ und $i_q$.
\end{satz}
\begin{proof}
...
mit Rotorwinkel $\theta_r$; im rotorfesten System sind StatorgrÃ¶ÃŸen
im stationÃ¤ren Betrieb GleichgrÃ¶ÃŸen -- daher eignen sich PI-Regler.
Die Entkopplungsterme kompensieren die Kreuzterme, die ohne Vorsteuerung
die $i_d$- und $i_q$-Regelschleifen koppeln wÃ¼rden.
\end{proof}

\section{Windenergie und Photovoltaik}

\begin{definition}[Betz'sches Gesetz -- Maximaler Windenergie-Ertrag]
Der maximale Leistungsbeiwert einer Windturbine betrÃ¤gt nach Betz (1919):
\[
  c_{P,\max} = \frac{16}{27} \approx 59{,}3\,\%.
\]
Die entnommene Leistung aus einem Windstrom mit Geschwindigkeit $v$,
Luftdichte $\rho$ und RotorflÃ¤che $A = \pi R^2$:
\[
  P = \frac{1}{2}\rho A v^3 \cdot c_P, \quad 0 < c_P \le 16/27.
\]
\end{definition}
\begin{proof}
Sei $v_1$ die AnstrÃ¶mgeschwindigkeit, $v_2$ die AbstrÃ¶mgeschwindigkeit.
...
Eine Solarzelle ist eine groÃŸ\-flÃ¤chige p-n-Diode.
...
SÃ¤ttigungsstrom $I_0$, IdealitÃ¤tsfaktor $n$, Serienwidersand $R_s$,
...
\begin{itemize}
    ...
    \item[-] \textbf{Entladen}: umgekehrter Prozess; elektrische Arbeit wird
          durch den AuÃŸenstromkreis abgegeben
...
(Arrhenius-AbhÃ¤ngigkeit der SEI-Wachstumsrate).
\end{definition}

\begin{satz}[Peukert-Gleichung -- KapazitÃ¤tsabhÃ¤ngigkeit von der Entladerate]
Die nutzbare KapazitÃ¤t $C$ einer Batterie hÃ¤ngt vom Entladestrom $I$ ab:
\[
  I^n\,t = C_0, \quad \text{oder Ã¤quivalent}\quad C = C_0\,\left(\frac{I_0}{I}\right)^{n-1},
\]
mit Peukert-Exponent $n$ ($n \approx 1{,}1\ldots1{,}3$ fÃ¼r Blei, $n \approx 1{,}05$ fÃ¼r Li-Ion)
und NennkapazitÃ¤t $C_0$ bei Nennstrom $I_0$.
\end{satz}
...
Die \textbf{Gasturbine} (Flugtriebwerk, stationÃ¤re Kraftgasturbine) arbeitet
nach dem Joule-Brayton-Kreisprozess:
\begin{enumerate}
    \item Isentrope Verdichtung: $p_1 \to p_2 = p_1\cdot\Pi$
          (DruckverhÃ¤ltnis $\Pi$; axiale Mehrstufenverdichter: $\Pi = 15\ldots50$)
    \item Isobare WÃ¤rmezufuhr: $T_3 = T_{TIT}$ (Turbineneintrittstemperatur;
          moderne Triebwerke: $T_3 > 1800\,\mathrm{K}$, Ã¼ber Schmelzpunkt des Metalls!)
    \item Isentrope Expansion: in Turbine; Wellenarbeit $= W_{Turbine} - W_{Verdichter}$
    \item Isobare WÃ¤rmeabfuhr (offen): AbgaswÃ¤rme
\end{enumerate}
Thermischer Wirkungsgrad:
\[
...
Mit Regenerator (AbgaswÃ¤rme vorwÃ¤rmt Luft nach Verdichter):
...
\begin{beobachtung}[StrÃ¶mungsmaschinen -- Euler-Turbomaschinen-Gleichung]
Die \textbf{Euler-Gleichung der Turbomaschinen} beschreibt die spezifische
Stutzenarbeit (EnergieÃ¼bertragung pro Masseeinheit):
\[
...
FÃ¼r Pumpe/Verdichter: $Y > 0$ (Energie wird zugefÃ¼hrt).
FÃ¼r Turbine: $Y < 0$ (Energie wird abgegeben; Vorzeichen konv. umkehren).
Dreiecksdiagramme (Velocity Triangles): $\vec{c} = \vec{u} + \vec{w}$
(Absolutgeschwindigkeit = Umfangs- + Relativgeschwindigkeit) zeigen
die GeschwindigkeitsverhÃ¤ltnisse grafisch.
\end{beobachtung}

\section{Elektrische EnergieÃ¼bertragung und Netztechnik}

\begin{definition}[Elektrisches Energieversorgungsnetz]
Das Stromnetz gliedert sich in Spannungsebenen:
\begin{itemize}
    \item[-] \textbf{HÃ¶chstspannungsnetz} ($380/220\,\mathrm{kV}$):
          Ãœbertragung Ã¼ber weite Strecken; Maschenstruktur; Betreiber: TransnetBW, TenneT
    \item[-] \textbf{Hochspannungsnetz} ($110\,\mathrm{kV}$):
          regionale Verteilung; Windparks, groÃŸe Industrieabnehmer
    \item[-] \textbf{Mittelspannung} ($10\ldots30\,\mathrm{kV}$):
          Ortsnetzstationen, mittelgroÃŸe Betriebe
    \item[-] \textbf{Niederspannung} ($400/230\,\mathrm{V}$):
          Endverbraucher (Haushalte, KMU)
\end{itemize}
Ãœbertragungsverluste im Drehstromnetz:
$P_{Verlust} = 3 R I^2 = P^2 R/(U^2\cos^2\!\varphi)$ --
hohe Spannung minimiert Ãœbertragungsverluste bei gleicher Leistung.
Deshalb: HÃ¶chstspannungs-DC-Ãœbertragung (HGÃœ) fÃ¼r sehr lange Strecken
($> 600\,\mathrm{km}$, z.\,B.\ SuedLink: $700\,\mathrm{km}$, $2\,\mathrm{GW}$).
\end{definition}

\begin{satz}[Netzsynchronisation -- Leistungsflussgleichungen]
Im Wechselstromnetz zwischen zwei Knoten (Spannung $U_1$, $U_2$, Ãœbertragungswinkel $\delta$,
Reaktanz $X$ der Leitung) flieÃŸt die \textbf{Wirkleistung}:
\[
...
\]
StabilitÃ¤tsgrenze: $\delta < 90^\circ$ (statische StabilitÃ¤tsgrenze;
in Praxis $\delta < 30\ldots40^\circ$ fÃ¼r Reserven).
\end{satz}
\begin{proof}
...
\section{WÃ¤rmepumpen und KÃ¤ltemittel}

\begin{definition}[WÃ¤rmepumpe -- Umkehrcarnot-Prozess]
Eine \textbf{WÃ¤rmepumpe} entzieht der Umgebung (WÃ¤rmequelle $T_C$) WÃ¤rme
und liefert diese auf hÃ¶herem Temperaturniveau (Senke $T_H$):
\[
  \varepsilon_{WP} = \frac{\dot{Q}_H}{P_{el}} = \frac{T_H}{T_H - T_C}
  \quad (\text{Carnot-WÃ¤rmepumpe}).
\]
Reale WÃ¤rmepumpen erreichen $\varepsilon_{WP} = 3\ldots5$ (Heizung)
und $\varepsilon_{WP} = 4\ldots7$ (bei Geothermie, $T_C = 12^\circ\mathrm{C}$).
KÃ¤ltemittel: R-410A (ausgelaufen, GWP~$=2088$), R-32 (GWP~$=675$),
...
\begin{itemize}
    \item[-] \textbf{Fluktuierende Einspeisung}: Wind und Solar sind wetterabhÃ¤ngig;
          Residuallast (Gesamtlast minus Erneuerbarer) fluktuiert stark
    \item[-] \textbf{Sektorenkopplung}: Ãœberschussstrom $\to$ WÃ¤rmepumpen,
          Elektrolyseure (H$_2$), Batteriespeicher, E-Fahrzeuge (Vehicle-to-Grid)
    \item[-] \textbf{Netzregelung}: PrimÃ¤rregelung (Frequenz $\pm\,0{,}2\,\mathrm{Hz}$,
          $30\,\mathrm{s}$), SekundÃ¤rregelung (Minutenreserve), TertiÃ¤rregelung (Stundenreserve)
...
\begin{definition}[PEM-Elektrolyse -- grÃ¼ner Wasserstoff]
...
(Ãœberspannungen durch Aktivierung, Ohmscher Widerstand, Transport);
...
\begin{itemize}
    \item[-] \textbf{Druckspeicherung}: 350 oder 700~bar (CFK-Tank); gravimetrische
          ...
          (ECE R134 fÃ¼r Fahrzeuge)
    \item[-] \textbf{FlÃ¼ssigwasserstoff} ($-253^\circ\mathrm{C}$): hÃ¶chste Volumendichte
          ($70{,}8\,\mathrm{kg/m^3}$); VerflÃ¼ssigungsverlust $\approx 30\,\%$;
    ...
    \item[-] \textbf{Metallhydridspeicher}: reversible chemische Bindung
          ...
          Beladung/Entladung
    \item[-] \textbf{Pipeline}: Anpassung bestehender Erdgasnetze mÃ¶glich
...
Bruchmechanik erklÃ¤rt das Versagen von Bauteilen, die Risse enthalten.
Betriebsfestigkeit beschreibt das ErmÃ¼dungsverhalten unter realen,
zeitlich verÃ¤nderlichen Beanspruchungen.

\section{Linear-elastische Bruchmechanik (LEBM)}

\begin{definition}[SpannungsintensitÃ¤tsfaktor]
Ein Bauteil mit einem Riss der LÃ¤nge $2a$ (durchgehender Innenriss
in einer unendlich breiten Platte) unter Zugspannung $\sigma$ erzeugt
an der Rissspitze eine \textbf{SpannungssingularitÃ¤t}.
Der \textbf{SpannungsintensitÃ¤tsfaktor} $K_I$ (Mode I: Ã–ffnung) ist:
\[
  K_I = \sigma\sqrt{\pi a}\,Y(a/W),
\]
mit der geometrieabhÃ¤ngigen Korrekturfunktion $Y$ (fÃ¼r die unendliche
Platte $Y = 1$) und der Probenbreite $W$.
Einheit: $[K_I] = \mathrm{MPa\sqrt{m}}$.
\end{definition}

\begin{satz}[RisszÃ¤higkeit und Bruchbedingung]
Ein Riss beginnt instabil zu wachsen (Bruch), wenn der
SpannungsintensitÃ¤tsfaktor die \textbf{RisszÃ¤higkeit} (BruchzÃ¤higkeit) $K_{Ic}$
Ã¼berschreitet:
\[
  K_I \ge K_{Ic}.
\]
...
begÃ¼nstigt ist, wenn die durch RissÃ¶ffnung freiwerdende elastische
Energie (Energiefreisetzungsrate $G$) die OberflÃ¤chenenergie der
neuen RissflÃ¤chen ($2\gamma$) Ã¼berschreitet:
$G \ge G_c = 2\gamma$ (Griffith-Kriterium).
Irwin (1957) zeigte: $G = K_I^2/E'$ (mit $E' = E$ fÃ¼r ebenen Spannungszustand,
$E' = E/(1-\nu^2)$ fÃ¼r ebenen Dehnungszustand), woraus das $K_I \ge K_{Ic}$-Kriterium folgt.
\end{proof}
...
\]
Risse bis zu diesem Wert sind vor dem Bruch zulÃ¤ssig --
...
wÃ¤chst ein Riss pro Lastwechsel gemÃ¤ÃŸ dem \textbf{Paris-Gesetz} (1963):
\[
  \frac{\mathrm{d}a}{\mathrm{d}N} = C\,(\Delta K_I)^m,
\]
mit materialabhÃ¤ngigen Konstanten $C$ und $m$ (Stahl: $m \approx 3$, $C \approx 10^{-11}$
...
\section{Betriebsfestigkeit und WÃ¶hlerlinie}

\begin{definition}[WÃ¶hlerlinie und Dauerfestigkeit]
Die \textbf{WÃ¶hlerlinie} (S-N-Kurve) beschreibt den Zusammenhang zwischen
...
\]
mit dem WÃ¶hler-Exponenten $k$ (Stahl: $k \approx 5\ldots8$).
Unterhalb der \textbf{Dauerfestigkeitsamplitude} $\sigma_D$ (fÃ¼r Stahl
bei $N_D \approx 10^6\ldots2\cdot10^6$ Schwingspiele) tritt kein ErmÃ¼dungsbruch auf.
Typische Werte fÃ¼r Stahl (ungekerbt, Zug-Druck): $\sigma_D \approx 0{,}5\,R_m$.
\end{definition}
...
so ist das Bauteil erschÃ¶pft, wenn die \textbf{Schadensakkumulation}
...
Dies ist eine Vereinfachung -- in der Praxis hÃ¤ngt die Reihenfolge der Lasten
...
SchÃ¤digungsakkumulationshypothese im Maschinenbau.
\end{proof}
...
\begin{definition}[VerdrÃ¤ngerpumpen -- KenngrÃ¶ÃŸen]
\textbf{VerdrÃ¤ngerpumpen} fÃ¶rdern Fluid durch periodisches VerdrÃ¤ngerprinzip
(Kolben, ZahnrÃ¤der, FlÃ¼gel). Wichtige Typen:
\begin{itemize}
    \item[-] \textbf{Zahnradpumpe} (AuÃŸen- und Innenzahnrad): einfach, robust;
          Pulsation; $p_{max} \approx 250\,\mathrm{bar}$; $\eta_{vol} \approx 0{,}92$
    \item[-] \textbf{Axialkolbenpumpe} (SchrÃ¤gscheibe, SchrÃ¤gachse):
          $p_{max} \approx 400\ldots700\,\mathrm{bar}$; variables Hubvolumen (Schwenkscheibe);
          $\eta_{vol} \approx 0{,}97$; hohe Leistungsdichte
    \item[-] \textbf{Radialkolbenpumpe}: sehr hohe DrÃ¼cke ($p_{max} > 700\,\mathrm{bar}$);
          Einspritzpumpen (Common Rail); Werkzeugmaschinenhydraulik
    \item[-] \textbf{FlÃ¼gelzellenpumpe}: gleichmÃ¤ÃŸige FÃ¶rderung; $p_{max} \approx 200\,\mathrm{bar}$;
...
TatsÃ¤chlicher Volumenstrom: $\dot{V}_{ist} = \eta_{vol} \cdot \dot{V}_{th}$.
...
(Mikrosekunden) -- Erosion, LÃ¤rm, Effizienzabfall.
GegenmaÃŸnahmen: Ansaugdruck erhÃ¶hen (tiefe Tankinstallation),
Ansaugleitungsquerschnitt groÃŸ dimensionieren, LÃ¼ftungsnormal-filter am Tank,
SpÃ¼lung des Kreislaufs, EntlÃ¼ftung.
\end{satz}
...
\begin{itemize}
    \item[-] \textbf{Offener Kreislauf}: Pumpe saugt aus Tank, Motor/Zylinder
          gibt RÃ¼cklauf in Tank zurÃ¼ck; Tank zur KÃ¼hlung und EntlÃ¼ftung;
          einfach, aber hoher Ã–lvolumenbedarf; typisch fÃ¼r Baumaschinen
    \item[-] \textbf{Geschlossener Kreislauf}: Motor-RÃ¼cklauf direkt zur Pumpe;
          kein Tank (nur SpÃ¼lkreislauf); kompakt; Vierquadrantenbetrieb
...
\begin{definition}[Hydraulikzylinder -- Auslegung und KrÃ¤fte]
FÃ¼r einen \textbf{doppeltwirkenden Hydraulikzylinder} mit Kolbendurchmesser $D$
...
Ein Hydraulikzylinder mit Ã–l-Federkonstante und beweglicher Masse
bildet ein schwingungsfÃ¤higes System. Die \textbf{hydraulische Eigenfrequenz}:
\[
  \omega_{hyd} = \sqrt{\frac{\beta_e\,A_{eff}^2}{V_{eff}\,m}},
\]
mit effektivem Druckmodul $\beta_e$ (Ã–l + Luft: $\beta_e \approx 10^3\ldots10^4\,\mathrm{bar}$),
effektiver KolbenflÃ¤che $A_{eff}$, eingeschlossenem Ã–lvolumen $V_{eff}$
und bewegter Masse $m$. Typisch: $f_{hyd} = 5\ldots50\,\mathrm{Hz}$.
\end{satz}
\begin{proof}
Das eingeschlossene Ã–l der LÃ¤nge $L$ und Querschnitt $A_{eff}$ wirkt
...
\textbf{Kompressoren} erzeugen Druckluft fÃ¼r pneumatische Systeme.
Wichtige Bauarten:
\begin{itemize}
    \item[-] \textbf{Kolbenkompressor}: 1- oder 2-stufig; $p_{max}$ bis $40\,\mathrm{bar}$;
          pulsierend; KÃ¼hlung zwischen Stufen (ZwischenkÃ¼hler); hoher Wirkungsgrad
    \item[-] \textbf{Schraubenkompressor}: kontinuierliche FÃ¶rderung durch
          ineinander greifende Schraubenrotoren; $p_{max}$ bis $13\,\mathrm{bar}$;
          StandardlÃ¶sung fÃ¼r Druckluftnetze; $\eta_{is} \approx 0{,}75\ldots0{,}85$
    \item[-] \textbf{Scrollkompressor}: zwei ineinander liegende Spiralen;
          Ã¶lfrei; leise; KÃ¤ltetechnik, Medizin
    \item[-] \textbf{Turbokompressor}: Kreiselverdichter; hohe VolumenstrÃ¶me,
          niedrige DrÃ¼cke; HochofengeblÃ¤se, Turbolader
\end{itemize}
...
\begin{itemize}
    \item[-] \textbf{5/2-Wegeventil}: fÃ¼nf AnschlÃ¼sse, zwei Schaltstellungen;
          Standardventil fÃ¼r doppeltwirkende Zylinder; monostabil (federrÃ¼ckgestellt)
          oder bistabil (rastend)
    \item[-] \textbf{Drossel-RÃ¼ckschlagventil}: steuert Kolbengeschwindigkeit;
          Abluftdrosselung (ruhigerer Lauf) bevorzugt vor Zulaufdrosselung
    \item[-] \textbf{Endlagenventil (Rollenhebelventil)}: mechanisch betÃ¤tigtes
          Schaltventil fÃ¼r Positions-RÃ¼ckmeldung
    \item[-] \textbf{Proportionalventil (pneumatisch)}: elektrisch proportional
          steuerbarer Volumenstrom; Positionsregelung mÃ¶glich;
          Genauigkeit $\approx \pm 0{,}1\,\mathrm{mm}$ mit Wegmessung
\end{itemize}
Pneumatik-SPS: Ablaufprogramm in Schrittkette (Step Chain); jeder Schritt
wartet auf ein Signal (Endlage, Sensor, Zeit) vor dem nÃ¤chsten Schritt.
\end{beobachtung}
...
\]
GewÃ¤hlt: $D = 32\,\mathrm{mm}$ (nÃ¤chste NormgrÃ¶ÃŸe, DIN ISO 15552).
TatsÃ¤chliche Kraft: $F_{ist} = p\,\pi D^2/4\cdot(1-\mu)/S = 6\cdot10^5\cdot\pi\cdot0{,}032^2/4\cdot0{,}9/1{,}5
...
ermÃ¶glichen die Ãœbertragung und Steuerung von KrÃ¤ften und Momenten
...
\begin{definition}[Hydraulisches Ãœbersetzungsprinzip]
Das grundlegende Prinzip der Hydraulik beruht auf dem
Pascalschen Gesetz: Druck pflanzt sich in einer eingeschlossenen
ruhenden FlÃ¼ssigkeit gleichmÃ¤ÃŸig in alle Richtungen fort.
FÃ¼r zwei verbundene Zylinder mit KolbenflÃ¤chen $A_1$ und $A_2$:
\[
  \frac{F_1}{A_1} = p = \frac{F_2}{A_2}
  \quad\Rightarrow\quad F_2 = F_1\,\frac{A_2}{A_1}.
\]
Das ÃœbersetzungsverhÃ¤ltnis $i_h = A_2/A_1$ kann sehr groÃŸ werden
(Hebekraft bis $10^6\,\mathrm{N}$ aus kleiner Pumpe);
die KontinuitÃ¤t: $A_1\,v_1 = A_2\,v_2$ zeigt, dass der
groÃŸe Kolben entsprechend langsamer fÃ¤hrt.
\end{definition}
...
\begin{itemize}
    \item[-] \textbf{Pumpe}: erzeugt Volumenstrom und Druck
          (Zahnradpumpe, Axialkolbenpumpe, FlÃ¼gelzellenpumpe)
    \item[-] \textbf{Hydrozylinder}: wandelt hydraulische in lineare mechanische Leistung
          (doppeltwirkend: Vorhub und RÃ¼ckhub hydraulisch betÃ¤tigt)
    ...
    \item[-] \textbf{Druckbegrenzungsventil}: schÃ¼tzt vor DruckÃ¼berschreitung
          (Einstellung: Ã–ffnungsdruck $p_{\max}$)
...
\midrule
Fluid & Ã–l ($\rho \approx 870\,\mathrm{kg/m^3}$) & Luft ($\rho \approx 1{,}2\,\mathrm{kg/m^3}$) \\
Druck & 100--400 bar & 4--10 bar \\
Kraft & sehr hoch ($10^6\,\mathrm{N}$) & mittel ($10^4\,\mathrm{N}$) \\
Steifigkeit & hoch (Ã–l inkompressibel) & gering (Luft kompressibel) \\
Positionierung & prÃ¤zise & ungenau ohne FÃ¼hrung \\
Leckage & Ã–lverschmutzung & unkritisch (Luft) \\
Wartung & aufwÃ¤ndig & einfach \\
\bottomrule
\end{tabular}
\end{center}
Pneumatik eignet sich besonders fÃ¼r schnelle, einfache
...
\section{LÃ¤ngenmessung und Koordinatenmesstechnik}

\begin{definition}[LÃ¤ngenmessverfahren im Maschinenbau]
LÃ¤ngenmessverfahren nach Messunsicherheit und Messbereich:
\begin{itemize}
    \item[-] \textbf{Schieblehre (Messschieber)}: mech. Noniusablesung oder digitale Anzeige;
          Messbereich 0--300\,mm; AuflÃ¶sung 0{,}01--0{,}05\,mm; Genauigkeit $\pm 0{,}02\,\mathrm{mm}$
    \item[-] \textbf{Mikrometer (BÃ¼gelmessschraube)}: Steigungsgenauigkeit $0{,}5\,\mathrm{mm}$
          pro Umdrehung; Trommelskala 50 Teilstriche $\to$ AuflÃ¶sung $0{,}01\,\mathrm{mm}$;
          Genauigkeit $\pm 0{,}003\,\mathrm{mm}$
    \item[-] \textbf{Messuhren/Feinzeiger}: Messtaster mit Zahnstangengetriebe;
          AuflÃ¶sung $0{,}001\,\mathrm{mm}$ (Feinzeiger $0{,}0001\,\mathrm{mm}$);
          Relativmessung gegen EndmaÃŸ
    \item[-] \textbf{Laser-Interferometer}: Interferenz von Laserstrahlen
          (He-Ne, $\lambda = 632{,}8\,\mathrm{nm}$); sub-Nanometer-AuflÃ¶sung;
          Kalibrierung von Werkzeugmaschinen, ReferenzmaÃŸstÃ¤be
    \item[-] \textbf{Luftlehre (Pneumatische LÃ¤ngenmessung)}: kontaktlos;
          DÃ¼se $\to$ Spalt $\to$ Gegendruck $\propto$ Abstand;
          AuflÃ¶sung $< 0{,}1\,\mu\mathrm{m}$; schnell; Serienmessung
\end{itemize}
\end{definition}

\begin{beobachtung}[Koordinatenmessmaschine (KMM)]
Eine \textbf{Koordinatenmessmaschine} (CMM -- Coordinate Measuring Machine)
misst rÃ¤umliche Koordinaten durch ein Tastsystem:
\begin{itemize}
    \item[-] \textbf{Schaltender Taster}: mechanisch schaltend bei BerÃ¼hrung
    ...
    \item[-] \textbf{Messender Taster}: kontinuierliche Wegmessung; Auslenkung proportional
          zur Tastkraft; schnelles Scannen von FreiformflÃ¤chen
    \item[-] \textbf{Optischer Sensor}: Laser-Liniensensor, WeiÃŸlichtinterferometer;
          berÃ¼hrungslos; fÃ¼r weiche oder spiegelnde OberflÃ¤chen
    \item[-] \textbf{Computertomographie (CT)}: vollstÃ¤ndige innere und Ã¤uÃŸere
          Geometrie; Messbereich 1\,mm--500\,mm; AuflÃ¶sung $1\ldots50\,\mu\mathrm{m}$;
          SerienprÃ¼fung von Kunststoffteilen
\end{itemize}
Auswertesoftware berechnet geometrische Elemente (Ebene, Zylinder, Kegel, Kugel)
durch Ausgleichsrechnung (Methode der kleinsten Fehlerquadrate -- GauÃŸ-Fit).
\end{beobachtung}

\section{Optische Messtechnik}

\begin{satz}[Interferometrie -- WellenlÃ¤nge als MaÃŸstab]
Bei der \textbf{Interferometrie} interferieren zwei kohÃ¤rente Strahlen
...
Eine Verschiebung um $\lambda/2$ entspricht einem StreifenÃ¼bergang.
FÃ¼r He-Ne-Laser ($\lambda = 632{,}8\,\mathrm{nm}$): 1 Streifen $\hat{=}$ 316{,}4\,nm.
\end{satz}
\begin{proof}
Der WeglÃ¤ngenunterschied $\Delta = 2d$ (Hin- und RÃ¼ckweg).
Maximale IntensitÃ¤t bei Phasengleichheit: $\Delta/\lambda = m$ (ganzzahlig),
minimale IntensitÃ¤t bei Gegenphasigkeit: $\Delta/\lambda = m + 1/2$.
\end{proof}

\begin{beobachtung}[Digitale Bildverarbeitung in der QualitÃ¤tssicherung]
\textbf{Bildverarbeitungssysteme} (Machine Vision) prÃ¼fen Bauteile
...
\begin{itemize}
    ...
    \item[-] \textbf{Kamera}: CCD oder CMOS; AuflÃ¶sung $1\ldots50\,\mathrm{MP}$;
          monochromatisch oder Farbe; Zeilenkamera fÃ¼r schnelle Bandinspektionen
    \item[-] \textbf{Algorithmen}: Kantendetektion (Canny, Sobel),
          Mustererkennung (Blob-Analyse, Template Matching),
          Deep Learning (CNN fÃ¼r Fehlererkennung)
    \item[-] \textbf{Anwendungen}: VollstÃ¤ndigkeitsprÃ¼fung, MaÃŸkontrolle,
          OberflÃ¤cheninspektion (Kratzer, Poren), OCR (Schriften, Barcodes),
          Lageerkennung fÃ¼r Roboter (bin picking)
\end{itemize}
\end{beobachtung}

\section{Durchfluss- und FÃ¼llstandsmessung}

\begin{definition}[Durchflussmessverfahren]
\begin{itemize}
    \item[-] \textbf{Differenzdruckverfahren} (Blende, Venturi, Pitot):
          $\dot{V} = \alpha A_2\sqrt{2\Delta p/\rho}$; kostengÃ¼nstig; Druckverlust
    \item[-] \textbf{Magnetisch-induktives DurchflussmessgerÃ¤t (MID)}:
          Faradaysche Induktion $U_{ind} = B\,v\,D$ (magnetisches Feld $B$,
          StrÃ¶mungsgeschwindigkeit $v$, Rohrdurchmesser $D$);
          nur fÃ¼r leitfÃ¤hige Fluide; $\sigma > 5\,\mu\mathrm{S/cm}$; druckverlustfrei
    \item[-] \textbf{Ultraschalldurchflussmessung}: Laufzeitdifferenzverfahren;
          Sende- und Empfangsrichtung abwechselnd; berÃ¼hrungslos (Clamp-on);
          Gas und FlÃ¼ssigkeit; keine bewegten Teile
    ...
    \item[-] \textbf{WirbelzÃ¤hler (Vortex)}: KÃ¡rmÃ¡nsche WirbelstraÃŸe hinter StÃ¶rkÃ¶rper;
...
\begin{itemize}
    ...
    \item[-] \textbf{Rekonstruktion}: Digital-Analog-Wandler (DAC) + Rekonstruktionsfilter;
          D/A-Ausgang ist Treppenfunktion $\to$ Tiefpass glÃ¤ttet
\end{itemize}
\end{definition}
...
FIR-Filter sind stets stabil (endliche Impulsantwort) und kÃ¶nnen
...
$Y(z) = H(z)\,X(z)$ im $z$-Bereich (Faltungstheorem fÃ¼r diskrete Folgen).
Die Ãœbertragungsfunktion $H(z) = \sum_k h[k] z^{-k}$ hat alle Pole bei $z=0$
...
Ein piezoresistiver Drucksensor wird an fÃ¼nf ReferenzdrÃ¼cken ($p_{ref,i}$)
...
\[
  ...
  a = \bar{U} - b\,\bar{p}.
\]
BestimmtheitsmaÃŸ $R^2 = 1 - SS_{res}/SS_{tot} > 0{,}9999$ gefordert
(LinearitÃ¤tsfehler $< 0{,}01\,\%$ v.E.).
...
GrÃ¶ÃŸen. Im Maschinenbau ist sie unabdingbar fÃ¼r QualitÃ¤tssicherung,
...
\begin{itemize}
    ...
    \item[-] \textbf{ZufÃ¤llige Abweichung}: statistisch verteilt;
          Verringerung durch Mittelung Ã¼ber $n$ Messungen: $\sigma_{\bar{x}} = \sigma/\sqrt{n}$
\end{itemize}
Die \textbf{Messunsicherheit} nach GUM (Guide to the Expression of Uncertainty in Measurement)
kombiniert alle UnsicherheitsbeitrÃ¤ge:
$u_c = \sqrt{\sum_i c_i^2 u_i^2}$ (GauÃŸ'sche Fehlerfortpflanzung).
\end{definition}

\begin{satz}[GauÃŸ'sche Fehlerfortpflanzung]
Ist eine GrÃ¶ÃŸe $y = f(x_1, \ldots, x_n)$ eine Funktion mehrerer unabhÃ¤ngig
gemessener GrÃ¶ÃŸen $x_i$ mit Standardunsicherheiten $u_i$, so ist:
\[
...
Da die $\delta x_i$ unkorreliert sind ($\mathrm{Cov}(\delta x_i, \delta x_j) = 0$ fÃ¼r $i\neq j$):
...
\begin{itemize}
    ...
    \item[-] \textbf{Widerstandsthermometer (PT100/PT1000)}: WiderstandsÃ¤nderung
    ...
    \item[-] \textbf{NTC/PTC-Thermistor}: Halbleiter-Widerstandsthermometer;
          nichtlinear; gÃ¼nstig und schnell; Automotive-Anwendungen
    \item[-] \textbf{Infrarotsensor}: berÃ¼hrungslose Messung; Pyrometer;
          fÃ¼r rotierende oder schwer zugÃ¤ngliche OberflÃ¤chen
\end{itemize}
\end{beobachtung}

\begin{beobachtung}[Kraft- und Drucksensoren]
\begin{itemize}
    \item[-] \textbf{Dehnungsmessstreifen (DMS)}: Ã„nderung des elektrischen
          Widerstands bei Dehnung: $\Delta R / R = k \cdot \varepsilon$
          (Gauge Factor $k \approx 2$ fÃ¼r Metall-DMS, $k \approx 100$ fÃ¼r Halbleiter-DMS);
          Wheatstone-BrÃ¼cke fÃ¼r empfindliche Messung
    ...
    \item[-] \textbf{Piezoresistiver Drucksensor (MEMS)}: Silizium-Membran
          mit diffundierten WiderstÃ¤nden; miniaturisiert; Automotive
...
drei KrÃ¤fte ($F_x, F_y, F_z$) und drei Momente ($M_x, M_y, M_z$) simultan.
Er besteht aus einem metallischen KÃ¶rper mit sechs DMS-BrÃ¼cken.
Die Kalibrier-Matrix $\mathbf{C} \in \mathbb{R}^{6\times6}$
transformiert die sechs Spannungsmesswerte in KrÃ¤fte und Momente:
\[
  \begin{pmatrix}F_x\\ \vdots \\ M_z\end{pmatrix} = \mathbf{C}\,\begin{pmatrix}U_1\\ \vdots \\ U_6\end{pmatrix}.
\]
Einsatz: Montage-ProzessÃ¼berwachung, kollaborative Roboter
(Kontakterkennung), haptisches Feedback in der Telemanipulation.
\end{beispiel}

% ============================================================

\section{Dimensionsanalyse und Ã„hnlichkeitstheorie}

\begin{satz}[Buckingham-Pi-Theorem]
Beschreibt ein physikalisches Problem $n$ GrÃ¶ÃŸen, die durch $k$
unabhÃ¤ngige Grunddimensionen darstellbar sind, so lÃ¤sst es sich auf
...
Jede GrÃ¶ÃŸe $q_i$ lÃ¤sst sich als Potenzprodukt der Grunddimensionen
ausdrÃ¼cken: $[q_i] = \prod_j D_j^{a_{ij}}$ (Dimensionsmatrix $\mathbf{A} \in \mathbb{R}^{k\times n}$).
...
\begin{beispiel}[Dimensionsanalyse der RohrstrÃ¶mung]
Problem: Druckverlust $\Delta p$ in einem glatten Rohr abhÃ¤ngig von
$v$, $d$, $L$, $\rho$, $\eta$ ($n=6$, $k=3$: Masse, LÃ¤nge, Zeit).
...
Die \textbf{LDA} misst StrÃ¶mungsgeschwindigkeiten berÃ¼hrungslos durch den
Doppler-Effekt an Streupartikeln:
Zwei kohÃ¤rente Laserstrahlen kreuzen sich im Messpunkt und bilden
...
\]
AuflÃ¶sung: $< 0{,}01\,\mathrm{m/s}$; kein Kontakt mit Fluid; keine Kalibrierung nÃ¶tig.
Anwendungen: StrÃ¶mungsprofile in Rohren, Turbinenschaufeln, Verbrennungsmotoren.
\end{definition}
...
\begin{itemize}
    \item[-] Tracerpartikel ($d_P \approx 1\ldots10\,\mu\mathrm{m}$,
          z.\,B.\ WeiÃŸÃ¶l-Nebel, Polyamid, Glas-Hohlkugeln) folgen dem Fluid
...
\end{itemize}
AuflÃ¶sung: $\approx 10^4$ Geschwindigkeitsvektoren gleichzeitig; Genauigkeit $\approx 1\,\%$.
\end{definition}

\section{Elektrische Messverfahren und Signalkonditionierung}

\begin{beobachtung}[Wheatstone-MessbrÃ¼cke -- Empfindlichkeit und Temperaturkompensation]
Die \textbf{Wheatstone-BrÃ¼cke} misst WiderstandsÃ¤nderungen mit hoher Empfindlichkeit.
Bei vier aktiven DMS-Elementen in VollbrÃ¼cke ($R_1 = R_3 = R+\Delta R$,
$R_2 = R_4 = R-\Delta R$; Biegeanordnung):
\[
  \frac{U_{aus}}{U_{sp}} = \frac{\Delta R}{R} = k\,\varepsilon,
\]
mit BrÃ¼ckenspeisespannung $U_{sp}$, Gauge-Factor $k$.
Temperaturkompensation durch gegenÃ¼berliegende DMS:
Temperatureffekte ($\Delta R_{T}$) heben sich bei symmetrischer Anordnung auf
(Differenzmessung), weil $+\Delta R_T - \Delta R_T = 0$.
\end{beobachtung}

\begin{definition}[Lock-in-VerstÃ¤rker -- Schwachsignalmessung]
Ein \textbf{Lock-in-VerstÃ¤rker} extrahiert ein Signal bekannter Frequenz $f_0$
...
trÃ¤gt zum Integral bei (alle anderen Frequenzen mitteln sich weg).
...
\begin{beobachtung}[RÃ¼ckfÃ¼hrbarkeit auf SI-Einheiten]
Jede Messung im Maschinenbau ist auf die SI-Grundeinheiten
\textbf{rÃ¼ckfÃ¼hrbar} (messtechnische RÃ¼ckfÃ¼hrbarkeit nach VIM):
\begin{itemize}
    \item[-] \textbf{Kalibrierungshierarchie}: WerksmessgerÃ¤t $\leftarrow$
          Referenzstandard $\leftarrow$ Nationales Metrologieinstitut
          (PTB in Deutschland) $\leftarrow$ SI-Definition
    \item[-] \textbf{LÃ¤nge}: SI-Definition via Lichtgeschwindigkeit
          ($c = 299\,792\,458\,\mathrm{m/s}$, exakt); EndmaÃŸ-Hierarchie
    ...
    \item[-] \textbf{Kalibrierunsicherheit}: sinkt von oben nach unten
          in der Hierarchie; WerksmessgerÃ¤t: $U = k\,u_c$ mit $k=2$
          (95\,\%-Konfidenz)
\end{itemize}
\end{beobachtung}
...
GrÃ¶ÃŸen aus anderen messbaren GrÃ¶ÃŸen mittels Modell:
\[
  \hat{y}(t) = f(\vec{u}(t), \vec{\theta}),
\]
mit Eingangsvektor $\vec{u}$ (messbare GrÃ¶ÃŸen) und Modellparametern $\vec{\theta}$.
Beispiele im Maschinenbau:
\begin{itemize}
    \item[-] \textbf{DrehmomentschÃ¤tzung} aus Motorstrom und Drehzahl
    ...
    \item[-] \textbf{Motortemperatur} aus Betriebsstunden, Last und KÃ¼hlmitteltemperatur
...
\begin{beispiel}[SOC-SchÃ¤tzung mit erweitertem Kalman-Filter]
Batteriemodell (Thevenin-Ersatzschaltbild):
Zustandsvektor $\vec{x} = (SOC, U_{RC})^\top$;
MessgrÃ¶ÃŸe $y = U_{Klemme} = OCV(SOC) - R_0 I - U_{RC}$ (OCV: Open Circuit Voltage).
ZustandsÃ¼bergang: $SOC_{k+1} = SOC_k - \eta I_k \Delta t / C_{nenn}$,
...
Typische SOC-Genauigkeit: $\pm 1\ldots3\,\%$ (Ã¼ber Ladezyklus).
\end{beispiel}

\chapter{Systemdynamik und MehrkÃ¶rpersysteme}
% ============================================================

MehrkÃ¶rpersysteme (MKS) beschreiben das dynamische Verhalten von
Systemen aus mehreren starren oder elastischen KÃ¶rpern, die durch
Gelenke, Federn und DÃ¤mpfer verbunden sind.

\section{Grundlagen der MKS-Simulation}

\begin{definition}[FreiwÃ¤hlige Koordinaten und Zwangsbedingungen]
Ein MKS mit $n_b$ KÃ¶rpern besitzt in 3D maximal $6n_b$ Freiheitsgrade
(3 Translation + 3 Rotation pro KÃ¶rper). Gelenke und Bindungen
...
\begin{satz}[Newton-Euler-Gleichungen fÃ¼r ein MKS]
Die Bewegungsgleichungen eines starren KÃ¶rpers $k$ im MKS-Formalismus:
\[
...
\]
mit KÃ¶rpermasse $m_k$, TrÃ¤gheitstensor $\mathbf{J}_k$,
Ã¤uÃŸeren KrÃ¤ften/Momenten $\vec{F}_k^{ext}$, $\vec{M}_k^{ext}$
und GelenkkrÃ¤ften/-momenten $\vec{F}_k^{joint}$, $\vec{M}_k^{joint}$.
...
FÃ¼r den Jeffcott-Rotor mit Unwucht $m_u\,e$ (Unwuchtmasse $m_u$,
ExzentrizitÃ¤t $e$) ergibt sich die stationÃ¤re Auslenkung:
\[
  r = \frac{m_u e}{m}\,\frac{\eta^2}{\sqrt{(1-\eta^2)^2+(2D\eta)^2}},
\]
mit $\eta = \Omega/\omega_0$. FÃ¼r $\eta \to \infty$: $r \to m_u e / m$
(Rotor dreht sich um seinen Schwerpunkt -- vollstÃ¤ndige Selbstzentrierung).
\end{satz}
...
\begin{itemize}
    ...
    \item[-] \textbf{Zweite Revolution}: ElektrizitÃ¤t und FlieÃŸband (ca. 1870--1914)
...
\begin{itemize}
    ...
    \item[-] \textbf{Cloud Manufacturing}: KapazitÃ¤ten als Service (MaaS);
...
\begin{itemize}
    \item[-] \textbf{Hierarchie}: Produkt -- Feldebene -- Steuerung -- Station -- Werk -- WertschÃ¶pfungsnetzwerk
    \item[-] \textbf{Lebenszyklus}: Typ (Design) und Instanz (Betrieb)
    \item[-] \textbf{Schichten}: Asset -- Integration -- Kommunikation -- Information -- Funktion -- GeschÃ¤ft
\end{itemize}
Das \textbf{Verwaltungsschale} (Asset Administration Shell, AAS) ist
die standardisierte digitale ReprÃ¤sentation jedes physischen Assets --
maschinenlesbar, interoperabel, Ã¼ber Systemgrenzen hinweg.
\end{beobachtung}

\section{Maschinelles Lernen fÃ¼r technische Systeme}
...
Sei $\sigma: \mathbb{R} \to \mathbb{R}$ eine stetige, beschrÃ¤nkte und
nicht-konstante Aktivierungsfunktion. Dann kann jede stetige Funktion
$f: [0,1]^n \to \mathbb{R}$ fÃ¼r jedes $\varepsilon > 0$ durch ein
...
$C([0,1]^n)$ (stetige Funktionen auf dem EinheitswÃ¼rfel) liegt.
Der Beweis nutzt den Stone-WeierstraÃŸ-Satz: die Linearspanne
$\{x\mapsto\sigma(w^\top x + b) : w\in\mathbb{R}^n, b\in\mathbb{R}\}$
ist eine Algebra, trennt Punkte und separiert den Raum -- damit
ist die gleichmÃ¤ÃŸige Approximation mÃ¶glich.
\end{proof}
...
folgende Entwicklungen geprÃ¤gt sein:
\begin{itemize}
    \item[-] \textbf{VollstÃ¤ndige Dekarbonisierung der Stromerzeugung}:
          Wind, Solar und Speicher liefern $> 80\,\%$ der ElektrizitÃ¤t;
          Wasserstoff aus Elektrolyse dient als saisonaler Speicher
    \item[-] \textbf{Direktelektrifizierung}: WÃ¤rmepumpen ersetzen Gasheizungen;
          Elektromotoren ersetzen Verbrennungsmotoren in PKW und LKW
    \item[-] \textbf{Wasserstoff-Infrastruktur}: Pipelinesystem fÃ¼r grÃ¼nen
    ...
    \item[-] \textbf{Kernfusion}: ITER-Experiment (2025--2040) zeigt Energiegewinn;
          kommerzielle Fusionskraftwerke frÃ¼hestens ab 2040--2050 erwartet
          (Deuterium-Tritium-Fusion: $E = 17{,}6\,\mathrm{MeV}$ pro Reaktion)
    \item[-] \textbf{Supraleitende Ãœbertragungsnetze}: bei Raumtemperatur-
          Supraleitung (falls realisierbar) verlustfreie EnergieÃ¼bertragung
\end{itemize}
\end{beobachtung}

\begin{beobachtung}[Fertigungstechnik 2050]
Die Fertigung der Zukunft wird durch folgende Trends geprÃ¤gt:
\begin{itemize}
    \item[-] \textbf{Atomare PrÃ¤zisionsfertigung}: Aufbau von Nanostrukturen
          Atom fÃ¼r Atom; ermÃ¶glicht maÃŸgeschneiderte elektronische, photonische
          und mechanische Eigenschaften (noch Grundlagenforschung)
    \item[-] \textbf{Vollautomatische Fabriken (Lights-out Manufacturing)}:
          24/7-Betrieb ohne menschliche Eingriffe; Roboter + KI Ã¼bernehmen
          alle Handhabungs-, Montage- und QualitÃ¤tssicherungsaufgaben
    ...
    \item[-] \textbf{Selbst-replizierende Systeme}: Maschinen, die einfache
          Versionen ihrer selbst herstellen kÃ¶nnen (von Neumann'sche
...
\begin{itemize}
    \item[-] \textbf{Physikalischem Tiefenwissen}: Die Grundgesetze
          ...
          bleiben unverÃ¤nderlich -- tiefes VerstÃ¤ndnis ihrer Implikationen
    ...
    \item[-] \textbf{Systemdenken}: Maschinen existieren nicht isoliert --
          ...
          und Ã–kosystemen; der gute Ingenieur denkt in Systemen
    \item[-] \textbf{Ethischer Verantwortung}: Ingenieure prÃ¤gen die
          gebaute Welt; Entscheidungen Ã¼ber Design, Material, Energieverbrauch
          und Lebensdauer haben gesellschaftliche und Ã¶kologische Konsequenzen;
          der VDI-Ethikkodex und die UN-Nachhaltigkeitsziele (SDGs) sind Leitbilder
    \item[-] \textbf{InterdisziplinaritÃ¤t}: Die groÃŸen Probleme des 21.\ Jahrhunderts
          (Klimawandel, Gesundheitsversorgung, Ressourcenknappheit) lassen sich
          nicht durch eine Disziplin allein lÃ¶sen -- der Maschinenbauingenieur
          arbeitet im Team mit Biochemikern, Informatikern, Ã–konomen und Sozialwissenschaftlern
\end{itemize}

\medskip
Dieses Werk hat gezeigt: Der Maschinenbau ist lebendig, vielfÃ¤ltig
und von Ã¼berwÃ¤ltigender Relevanz. Von der Differentialgleichung
der Schwingung bis zum digitalen Zwilling, vom Hookeschen Gesetz
bis zur Kernfusion -- alle diese Felder sind durch dieselbe
Sprache der Mathematik und denselben Geist empirischer PrÃ¤zision
miteinander verbunden. Der Weg vom Studium der Grundlagen
zu bahnbrechenden Innovationen ist ein langer, aber ein auÃŸerordentlich
lohnenswerter.
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{SchnittkrÃ¤ften} (Kienzle-Modell kalibriert durch Simulation):
          FEM-Simulation des Spanabtrenns liefert $k_{c1,1}$ und $m_c$ fÃ¼r neue Werkstoffe
    \item[-] \textbf{Werkzeugtemperaturen}: Finite-Differenzen oder FEM-WÃ¤rmeÃ¼bertragung;
          kritisch fÃ¼r WerkzeugverschleiÃŸ und WerkstÃ¼ckgefÃ¼ge in Randzone
    \item[-] \textbf{Eigenspannungen im WerkstÃ¼ck}: Thermomechanische Kopplung;
          Bohrungseigenspannungen nach Drehen und Schleifen
    \item[-] \textbf{BauteiloberflÃ¤che und Ra-Wert}: kinematische Modelle
          fÃ¼r Rauheit aus Vorschub und Eckenradius $r_\varepsilon$:
          $R_a \approx f^2/(32 r_\varepsilon)$
\end{itemize}
Der \textbf{Digitale Prozesszwilling} schlieÃŸt den Regelkreis:
...
(Regelung von Schnittgeschwindigkeit und Vorschub fÃ¼r konstante Werkzeugtemperatur).
\end{beobachtung}

\begin{definition}[GieÃŸsimulation und Erstarrungsmodellierung]
Die \textbf{GieÃŸsimulation} (z.\,B.\ MAGMASOFT, ProCAST) lÃ¶st:
\begin{itemize}
    \item[-] \textbf{FormfÃ¼llung}: freie OberflÃ¤che (Volume-of-Fluid-Methode);
          turbulente SchmelzestrÃ¶mung; LufteinschlÃ¼sse
    \item[-] \textbf{Erstarrung}: Stefan-Problem (bewegliche GrenzflÃ¤che
          flÃ¼ssig/fest); WÃ¤rmeÃ¼bergang Form-Schmelze; lokale Erstarrungszeit
    \item[-] \textbf{Schrumpfung und Lunker}: Volumenkontraktionen beim Erstarren
          (Al: $\approx 6{,}5\,\%$, Stahl: $\approx 3\,\%$); Lunkerpositionen
    \item[-] \textbf{Eigenspannungen}: thermische Spannungen durch ungleichmÃ¤ÃŸige
          AbkÃ¼hlung; Verzug nach Entformung
\end{itemize}
\end{definition}

\section{Sensorik fÃ¼r autonome Systeme}

\begin{definition}[LiDAR, Radar und Kamera im Vergleich]
FÃ¼r autonome Fahrzeuge und Roboter werden drei komplementÃ¤re Sensorsysteme kombiniert:
\begin{center}
\begin{tabular}{llll}
\toprule
Eigenschaft & LiDAR & Radar & Kamera \\
\midrule
AuflÃ¶sung & hoch & niedrig & sehr hoch \\
...
\section{ZustandsÃ¼berwachung und Predictive Maintenance}

\begin{definition}[Condition Monitoring -- Kennwertpyramide]
ZustandsÃ¼berwachung nutzt eine Hierarchie von Kennwerten:
\begin{itemize}
    ...
    \item[-] \textbf{Ebene 2 -- Statistische Kennwerte}: RMS, Peak, Kurtosis, Scheitelfaktor;
          einfach berechenbar, wenig sensitiv fÃ¼r frÃ¼he SchÃ¤den
    \item[-] \textbf{Ebene 3 -- Frequenzbereich}: FFT-Spektrum, HÃ¼llkurvenspektrum (Demodulation),
          Schmalbandamplituden bei Schadensfrequenzen; sensitiv fÃ¼r Lager, Verzahnung
...
\begin{beispiel}[HÃ¼llkurvenanalyse fÃ¼r WÃ¤lzlagerschÃ¤den]
Ein geschÃ¤digtes WÃ¤lzlager erzeugt periodische Impulse bei der
Schadensfrequenz $f_{BPFO}$ (s. Kap.\ 13). Da diese Impulse
einen Hochfrequenzbereich des Spektrums anregen (Resonanzfrequenz
des Sensors/GehÃ¤uses, typisch $5\ldots50\,\mathrm{kHz}$), verfÃ¤hrt
die HÃ¼llkurvenanalyse wie folgt:
\begin{itemize}
    \item[-] \textbf{Bandpass-Filter} um die Resonanzfrequenz
    \item[-] \textbf{Gleichrichtung} (Betragsbildung) und Tiefpassfilterung
          $\to$ \textbf{HÃ¼llkurve} $e(t)$
    \item[-] \textbf{FFT der HÃ¼llkurve}: Peaks bei $f_{BPFO}$ und Oberwellen
          $2f_{BPFO}$, $3f_{BPFO}$ $\to$ eindeutiger Nachweis des AuÃŸenringschadens
\end{itemize}
Diese Methode detektiert SchÃ¤den deutlich frÃ¼her ($\ge 6$ Wochen) als der
...
\]
Aus $\mathbf{T}_0^n$ liest man direkt die \textbf{VorwÃ¤rtskinematik} ab:
Position und Orientierung des Endeffektors als Funktion der Gelenkwinkel $\vec{q}$.
\end{definition}

\begin{satz}[Jacobi-Matrix und singulÃ¤re Konfigurationen]
Die \textbf{Jacobi-Matrix} $\mathbf{J}(\vec{q}) \in \mathbb{R}^{6\times n}$
verknÃ¼pft Gelenkgeschwindigkeiten $\dot{\vec{q}}$ mit der
Endeffektor-Geschwindigkeit $\vec{v}_E = (\dot{\vec{p}}_E^\top, \vec{\omega}_E^\top)^\top$:
\[
  \vec{v}_E = \mathbf{J}(\vec{q})\,\dot{\vec{q}}.
\]
\textbf{SingulÃ¤re Konfigurationen}: $\det(\mathbf{J}\mathbf{J}^\top) = 0$;
der Roboter verliert Beweglichkeit in bestimmten Richtungen.
SingularitÃ¤ten sind zu vermeiden (unendliche Gelenkgeschwindigkeiten nÃ¶tig).
\end{satz}
\begin{proof}
Die Jacobi-Matrix wird durch spaltenweise Differentiation der VorwÃ¤rtskinematik berechnet.
FÃ¼r Drehgelenk $k$ (Achse $\vec{z}_{k-1}$):
...
SingularitÃ¤t: $\mathrm{rank}(\mathbf{J}) < 6$ -- bei gestrecktem Arm,
Ausrichtung von Gelenken, etc.
\end{proof}

\begin{beobachtung}[Inverse Kinematik und Redundanz]
Die \textbf{inverse Kinematik} -- aus gewÃ¼nschter Endeffektor-Pose
die Gelenkwinkel bestimmen -- ist im Allgemeinen nichtlinear.
FÃ¼r $n > 6$ Gelenke ist der Roboter \textbf{redundant}:
unendlich viele Gelenkkonfigurationen realisieren dieselbe Endeffektor-Pose.
LÃ¶sungsmethoden:
\begin{itemize}
    \item[-] \textbf{Analytisch}: fÃ¼r Standard-Architekturen (6-DOF-Industrieroboter)
          existieren bis zu 16 analytische LÃ¶sungen (geschlossene Form)
    \item[-] \textbf{Jacobi-Inverse}: $\dot{\vec{q}} = \mathbf{J}^+\vec{v}_E$
          ...
          iterativ, singularitÃ¤tsnah numerisch instabil
    \item[-] \textbf{Damped Least Squares}: $\dot{\vec{q}} = \mathbf{J}^\top(\mathbf{J}\mathbf{J}^\top + \lambda^2\mathbf{I})^{-1}\vec{v}_E$;
          stabil nahe SingularitÃ¤ten; Robotik-Standard
\end{itemize}
\end{beobachtung}

\section{Thermische Simulation und KÃ¼hlkonzepte in der Elektronik}

\begin{beobachtung}[WÃ¤rmemanagement in Leistungselektronik]
...
\]
mit Junction-to-Case-, Case-to-Heatsink- und Heatsink-to-Ambient-WiderstÃ¤nden.
KÃ¼hlkonzepte nach aufsteigender Leistungsdichte:
\begin{itemize}
    \item[-] \textbf{Naturkonvektion}: bis $\approx 0{,}5\,\mathrm{kW}$; keine bewegten Teile
    \item[-] \textbf{Erzwungene LuftkÃ¼hlung}: LÃ¼fter; bis $\approx 5\,\mathrm{kW}$;
          GerÃ¤usch, Filteraufwand
    \item[-] \textbf{FlÃ¼ssigkeitskÃ¼hlung}: Wasser oder Glykol-Wasser;
          bis $\approx 100\,\mathrm{kW}$; Elektrofahrzeug-Wechselrichter
    \item[-] \textbf{Direkte FlÃ¼ssigkeitskÃ¼hlung} (ImmersionskÃ¼hlung):
          Halbleiter direkt in dielektrischer FlÃ¼ssigkeit; Rechenzentren;
          bis $\approx 1000\,\mathrm{kW/m^2}$
    \item[-] \textbf{WÃ¤rmerohr (Heat Pipe)}: zweiphasige Verdampfung/Kondensation;
          sehr hohe effektive WÃ¤rmeleitfÃ¤higkeit ($\lambda_{eff}$ bis $10^5\,\mathrm{W/(mK)}$);
          passiv, gerÃ¤uschlos; Laptops, Satelliten
\end{itemize}
\end{beobachtung}
...
Ein WÃ¤rmepumpen-System erreicht dabei:
\begin{itemize}
    ...
    \item[-] \textbf{BatteriekÃ¼hlung}: Li-Ion optimal bei $25\ldots35^\circ\mathrm{C}$;
          Schnellladung nur bis $45^\circ\mathrm{C}$ zulÃ¤ssig
    \item[-] \textbf{Vorkonditionierung}: Batterie vor dem Laden
          auf Optimaltemperatur bringen (Wallbox-gesteuert, Navigation-prÃ¤diktiv)
\end{itemize}
...
Jenseits der rein mechanischen additiven Fertigung ermÃ¶glicht
\textbf{Multi-Material-Printing} die Integration elektrischer Funktionen
direkt im Bauteil:
\begin{itemize}
    \item[-] \textbf{3D-gedruckte Dehnungsmessstreifen}: leitfÃ¤hige
          Filamente (Carbon-PETG) oder Silbertinte-Direktschreiben
          erzeugen Messgitter direkt auf der BauteiloberflÃ¤che
    \item[-] \textbf{Eingebettete Antennen}: konforme Antennen auf
          Flugzeugverkleidungen oder HelmoberflÃ¤chen
    \item[-] \textbf{Structural Health Monitoring (SHM)}: Piezoelektrische
          ...
          detektiert Risse bereits bei mm-GrÃ¶ÃŸe
    \item[-] \textbf{Funktionale Gradientenwerkstoffe (FGM)}:
          kontinuierlicher Ãœbergang zwischen Metall und Keramik;
          keine GrenzflÃ¤chenspannungen; TBC-Alternative; additive Herstellung
\end{itemize}
\end{beobachtung}

\begin{beobachtung}[AbschlieÃŸende Worte zur Bedeutung des Maschinenbaus]
Das vorliegende Werk hat in 19 Hauptkapiteln (11 klassische GrundlagenfÃ¤cher
plus 8 erweiterte ThemenblÃ¶cke) gezeigt, wie breit und tief das
...
Drei Ã¼bergreifende Einsichten bleiben:
...
Newton, Maxwell, Fourier, Carnot -- sind unverÃ¤nderlich.
Was sich Ã¤ndert, ist unsere FÃ¤higkeit, ihre Konsequenzen zu berechnen
und in technische Systeme zu Ã¼bersetzen.

\medskip
\noindent\textbf{Zweitens}: Die grÃ¶ÃŸten InnovationssprÃ¼nge im Maschinenbau
...
Wer BrÃ¼cken zwischen FÃ¤chern baut, schafft Mehrwert.
...
MÃ¶ge dieses Werk ein hilfreicher Begleiter auf diesem Weg sein.
\end{beobachtung}
...
Osseointegration, vom Carnot-Prozess bis zur KI-gestÃ¼tzten Regelung.
Jedes Kapitel wurde durch formale SÃ¤tze, vollstÃ¤ndige Beweise und
quantitative Beispiele untermauert, um den Zusammenhang zwischen
abstrakter Theorie und ingenieurmÃ¤ÃŸiger Praxis sichtbar zu machen.
...
\begin{itemize}
    \item[-] \textbf{Linearisierung}: Nichtlineare Systeme werden um
          ...
          liefert analytisch lÃ¶sbare Modelle.
    \item[-] \textbf{Energiemethoden}: Bernoulli (StrÃ¶mung), erster Hauptsatz
          (Thermodynamik), Energiesatz der Mechanik, Castigliano (FEM),
          Hill-Muskelmodell -- alle sind AusprÃ¤gungen derselben Energieerhaltung.
    \item[-] \textbf{Dimensionslose Kennzahlen}: Reynolds-Zahl, DÃ¤mpfungsgrad~$D$,
          FrequenzverhÃ¤ltnis~$\eta$, Isentropenexponent~$\kappa$, Mach-Zahl,
          Sigma-Niveau -- sie erlauben allgemeingÃ¼ltige Aussagen unabhÃ¤ngig
          vom physikalischen MaÃŸstab.
\end{itemize}
\end{beobachtung}
...
sondern ein kohÃ¤rentes WissensgebÃ¤ude. Fundamentale Analogien:
\begin{itemize}
    \item[-] \textbf{Mechanik $\leftrightarrow$ Elektrotechnik}: Masse~$m$,
          Feder~$c$, DÃ¤mpfer~$d$ entsprechen InduktivitÃ¤t~$L$, KapazitÃ¤t~$C$,
          Widerstand~$R$. Dieselbe DGL $m\ddot{x}+d\dot{x}+cx = F$
          regiert Schwingungen und RLC-Kreise.
    \item[-] \textbf{Thermische $\leftrightarrow$ elektrische WiderstÃ¤nde}:
          WÃ¤rmedurchgang durch eine Wand (Konvektion + Leitung + Konvektion)
    ...
    \item[-] \textbf{Biologische $\leftrightarrow$ technische Strukturen}:
          ...
          Hill-Muskelmodell $\leftrightarrow$ Feder-DÃ¤mpfer-Element.
    \item[-] \textbf{Lineare Algebra Ã¼berall}: FEM-Steifigkeitsmatrix, Modalanalyse,
          Jacobi-Matrix in der Roboterkinematik, Systemmatrix in der Regelungstechnik,
          Gram-Matrix im Gaussian-Process-Metamodell -- alle fÃ¼hren auf
          $\mathbf{A}\vec{x} = \vec{b}$ oder Eigenwertprobleme.
\end{itemize}
\end{beobachtung}

\begin{beobachtung}[Sicherheitsphilosophie im Maschinenbau]
Jede technische Berechnung mÃ¼ndet in einen Sicherheitsnachweis.
...
\begin{itemize}
    ...
    \item[-] \textbf{Probabilistisch} (Eurocodes, VDA): statistische Verteilungen von
          Lasten und WiderstÃ¤nden; Teilsicherheitsbeiwerte; ZielfÃ¼llung-Ãœberschreitungswahrscheinlichkeit
...
\section{RÃ¼ckblick auf die Kapitel und ihre Verbindungen}
...
und Steiner'sches Theorem Auflagerreaktionen und FlÃ¤chentrÃ¤gheitsmomente
liefern -- Grundlage jeder Bauteilauslegung.
\textbf{Kinematik und Dynamik} (Kap.~3) verknÃ¼pfte Weg, Geschwindigkeit
und Beschleunigung durch Integration und Differentiation;
Newtonsche Axiome, Drallsatz und MassentrÃ¤gheitsmomente
bildeten die BrÃ¼cke zur Roboterkinematik (Kap.~23) und zur
Schwingungslehre (Kap.~9).
\textbf{Festigkeitslehre} (Kap.~4) -- Hookesches Gesetz, GestaltÃ¤nderungsenergiehypothese,
Biegelinie, Euler-Knicklast -- ist direkte Voraussetzung fÃ¼r
Bruchmechanik (Kap.~20) und Biomechanik (Kap.~14).
\textbf{StrÃ¶mungslehre} (Kap.~5) mit Bernoulli, Hagen-Poiseuille
und AffinitÃ¤tsgesetzen ist Grundlage der Hydraulik (Kap.~21)
und der CFD in Kap.~12.
\textbf{Thermodynamik} (Kap.~6) -- Carnot, Otto/Diesel/Rankine,
WÃ¤rmeÃ¼bertragung -- verbindet sich mit Energietechnik (Kap.~19)
und dem WÃ¤rmemanagement in der Elektronik.
\end{beobachtung}
...
Eigenwertproblem -- bildet die theoretische Basis fÃ¼r
Schwingungsmesstechnik (Kap.~16) und Rotordynamik (Kap.~23).
\textbf{Regelungstechnik} (Kap.~10) -- Laplace, Hurwitz, PID,
ZustandsrÃ¼ckfÃ¼hrung -- verbindet sich mit hydraulischen Servoventilen (Kap.~21),
...
SLM-Prozessphysik, ISO-286-Toleranzen -- schafft den Ãœbergang von
...
und Steifigkeitsmatrix, Newton-Raphson fÃ¼r nichtlineare Systeme,
SIMPLE-Algorithmus fÃ¼r CFD, Turbulenzmodelle und Gaussian-Process-Metamodelle --
liefern die rechnergestÃ¼tzte BrÃ¼cke zwischen analytischem Modell und
industrieller Simulation.
\textbf{Produktionsplanung} (Kap.~13) -- Andler-LosgrÃ¶ÃŸenformel, Kanban,
Wertstromanalyse, Six-Sigma-DMAIC, DoE, MES nach ISA-95 -- verankert
den Maschinenbau in der betriebswirtschaftlichen RealitÃ¤t der Fabrik.
\textbf{Biomechanik} (Kap.~14) -- Hill-Muskelmodell, Hertz'scher Kontakt,
WirbelsÃ¤ulenbelastung, Osseointegration, Exoskelette, QLV nach Fung --
Ã¶ffnet den Maschinenbau fÃ¼r die Medizintechnik, Rehabilitation und
das bio-inspirierte Engineering.
\end{beobachtung}

\begin{beobachtung}[Kapitel 16--23: Messtechnik, Energie, Diagnose und Systeme]
\textbf{Schwingungsmesstechnik} (Kap.~16) -- Nyquist, FFT, HÃ¼llkurvenanalyse,
WÃ¤lzlagerschaden-Frequenzen -- verbindet Schwingungslehre (Kap.~9)
mit industrieller ZustandsÃ¼berwachung und Predictive Maintenance.
\textbf{Tribologie} (Kap.~17) -- Stribeck-Kurve, Archard-Gesetz, EHD-Schmierung --
erklÃ¤rt die GrenzflÃ¤chen zwischen den Maschinenelementen des Kap.~8
und ist SchlÃ¼ssel zu Energieeffizienz und Lebensdauer.
\textbf{Konstruktionsmethodik} (Kap.~18) -- TRIZ, FMEA nach AIAG-VDA,
Taguchi-Verlustfunktion, DFMA, Weibull-ZuverlÃ¤ssigkeit -- Ã¼bersetzt
...
Peukert-Gleichung fÃ¼r Li-Ion-Batterien -- ist die technische Grundlage
der Energiewende und des elektrischen Antriebs.
\textbf{Bruchmechanik} (Kap.~20) -- SpannungsintensitÃ¤tsfaktor~$K_I$,
Paris-Gesetz, Miner-Regel -- verbindet Festigkeitslehre (Kap.~4)
mit Lebensdauer-Prognose und schadensstolerantem Design.
\textbf{Hydraulik und Pneumatik} (Kap.~21) -- VerdrÃ¤ngerpumpen, Kavitation,
Load-Sensing, hydraulische Eigenfrequenz, Kompressoren -- sind die
EnergieÃ¼bertragungs-Systeme der Baumaschinen, Pressen und Robotik.
\textbf{Messtechnik} (Kap.~22) -- Interferometrie, KMM, Machine Vision,
Coriolis-Durchflussmesser, FIR-Filter -- sichert die RÃ¼ckkopplung
zwischen physikalischer RealitÃ¤t und digitalem Modell.
\textbf{Systemdynamik und MehrkÃ¶rpersimulation} (Kap.~23) -- Newton-Euler-Gleichungen,
...
\textbf{FEM} lÃ¶st die Biegelinie (Kap.~4), die Eigenfrequenzen (Kap.~9)
und die Kontaktspannungen im Kniegelenk (Kap.~14) mit beliebiger
GeometriekomplexitÃ¤t. \textbf{CFD} Ã¼bertrÃ¤gt Bernoulli und Navier-Stokes (Kap.~5)
auf turbulente StrÃ¶mungen in Turbinen und WÃ¤rmetauschern. Die
\textbf{MehrkÃ¶rpersimulation} (Kap.~23) integriert MassentrÃ¤gheitsmomente
...
datengetriebene Korrekturen ergÃ¤nzt. Die Schwingungsmesstechnik (Kap.~16)
liefert die Online-Messdaten fÃ¼r Predictive Maintenance;
die Tribologie (Kap.~17) modelliert den VerschleiÃŸfortschritt;
...
aber ihre QualitÃ¤t (Exergie) unwiederbringlich abnimmt.
Daraus folgt unmittelbar die Notwendigkeit hoher Wirkungsgrade:
Carnot-Limit fÃ¼r WÃ¤rmekraftmaschinen, Betz-Limit fÃ¼r Windturbinen (Kap.~19),
...
$\mathrm{GWP} = \sum_i m_i \cdot \mathrm{GWP}_i$ Ã¼ber alle Phasen --
von der Rohstoffgewinnung Ã¼ber Fertigung (Kap.~11) bis zur Entsorgung.
Wichtige Hebel:
\begin{itemize}
    ...
    \item[-] \textbf{Kreislaufwirtschaft}: RecyclingfÃ¤higkeit schon im DFMA-Prozess
          (Kap.~18) berÃ¼cksichtigen; sortenreine Trennbarkeit; Downcycling vermeiden
    \item[-] \textbf{Wasserstoff und Power-to-X}: Elektrolyse mit grÃ¼nem Strom
          (Energietechnik Kap.~19) erzeugt grÃ¼nen Wasserstoff als saisonalen Speicher;
          Brennstoffzellen wandeln ihn mit $\eta_{el} \approx 50\ldots60\,\%$ zurÃ¼ck
\end{itemize}
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{Hochfeste Leichtbauwerkstoffe}: Ultra-hochfester Stahl
          ...
          (Advanced High-Strength Steel); fÃ¼r Fahrzeugbau und Windturbinen
    ...
    \item[-] \textbf{Metamaterialien}: negative Poissonzahl (Auxetik),
          akustische BandlÃ¼cken, elektromagnetische Cloaking --
    ...
    \item[-] \textbf{Biologisch abbaubare Implantate}: Mg-Legierungen
          ...
          bei Knochenfrakturen; keine Zweitoperation zum Entfernen nÃ¶tig
\end{itemize}
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{Multi-Material-Additive Fertigung}: gleichzeitiges Drucken
          ...
          Gradientenwerkstoffe direkt hergestellt (kein FÃ¼geprozess)
    \item[-] \textbf{In-situ ProzessÃ¼berwachung (SLM)}: Melt-Pool-Monitoring,
          Computed Tomography im Bauraum -- Null-Fehler-Fertigung ohne
          nachgeschaltete QualitÃ¤tssicherung
    \item[-] \textbf{Autonome Fabriken}: vollstÃ¤ndige Lights-out-Production;
          AMR fÃ¼r Materialtransport; KI-basierte Feinplanung (Kap.~13);
          QualitÃ¤tssicherung durch Machine Vision (Kap.~22)
    \item[-] \textbf{Dezentrale Fertigung}: On-Demand-Produktion von Ersatzteilen
          nah am Bedarfsort (3D-Druck); LagerbestÃ¤nde durch digitale Blaupausen ersetzt;
          Lieferketten resilient gegen globale StÃ¶rungen
\end{itemize}
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{Feststoff-Batterien}: FestkÃ¶rper-Elektrolyt statt flÃ¼ssig;
          keine Dendritenbildung; Energiedichte $> 400\,\mathrm{Wh/kg}$ angestrebt;
          Skalierbare Herstellung bleibt SchlÃ¼sselherausforderung
    ...
    \item[-] \textbf{Supraleitende Elektromotoren}: bei Hochtemperatursupraleitung
          ...
          Leistungsdichte $> 20\,\mathrm{kW/kg}$ fÃ¼r Flugzeugantriebe
    \item[-] \textbf{E-Fuels und grÃ¼nes Ammoniak}: langfristige Energiespeicherung
...
\begin{itemize}
    ...
    \item[-] \textbf{Implantierbare MEMS-Sensoren}: drahtlose Drucksensoren
          ...
          Telemetrie zur Implantat-Ãœberwachung Ã¼ber Jahre
    \item[-] \textbf{Digitaler Zwilling des menschlichen KÃ¶rpers}:
          personalisiertes FEM-Modell des Skeletts aus CT-Daten;
          prÃ¤operative Simulation von Prothesen-Implantatationen;
          Optimierung der Prothesen\-geometrie fÃ¼r jeden Patienten individuell
\end{itemize}
\end{beobachtung}
...
\begin{itemize}
    \item[-] \textbf{KI-gestÃ¼tzte Konstruktion}: Large Language Models
          und multimodale KI unterstÃ¼tzen NormprÃ¼fung, Literaturrecherche
          und Erst-Auslegung; der Ingenieur prÃ¼ft und entscheidet
    \item[-] \textbf{Physik-informierte KI (PINN)}: neuronale Netze,
          die DGL-Residuen im Verlustfunktional berÃ¼cksichtigen;
          lernen physikalisch konsistente LÃ¶sungen aus wenigen Messdaten
    \item[-] \textbf{Quantencomputing}: Variational Quantum Eigensolver (VQE)
          ...
          Quantenannealing fÃ¼r Kombinatorische Optimierungsprobleme (Produktionsplanung)
...
\section{AbschlieÃŸende Betrachtung}

\begin{beobachtung}[Die drei unverÃ¤nderlichen Prinzipien des Maschinenbaus]
Alle 23 Kapitel dieses Werks lassen sich auf drei Ã¼bergreifende
Prinzipien zurÃ¼ckfÃ¼hren, die unverÃ¤nderlich gelten -- unabhÃ¤ngig von
...
Ã¼ber diese erhaltenen GrÃ¶ÃŸen.

\medskip
\noindent\textbf{2.~Modell und Wirklichkeit.}
Jedes Kapitel bot Modelle -- vereinfachte Abbilder der RealitÃ¤t.
Das Hookesche Gesetz gilt nur bis zur Streckgrenze;
Bernoulli nur fÃ¼r reibungsfreie, stationÃ¤re StrÃ¶mung;
das Hill-Muskelmodell ist eine NÃ¤herung.
Die FÃ¤higkeit, die GÃ¼ltigkeitsgrenzen eines Modells zu benennen
und bewusst mit ihnen umzugehen, ist das Kernmerkmal ingenieursmÃ¤ÃŸigen Denkens.

\medskip
\noindent\textbf{3.~InterdisziplinaritÃ¤t als Notwendigkeit.}
Kein Kapitel steht fÃ¼r sich. Werkstoffkunde informiert
...
informiert Robotik, Hydraulik und Exoskelette. Die grÃ¶ÃŸten Innovationen
entstehen genau dort, wo Disziplinen sich berÃ¼hren.
Der Ingenieur der Zukunft muss diese BrÃ¼cken bauen und begehen kÃ¶nnen.

\medskip
Der Maschinenbau ist lebendig. Die Gesetze sind bekannt;
die Aufgaben sind offen. MÃ¶ge dieses Werk ein verlÃ¤sslicher
