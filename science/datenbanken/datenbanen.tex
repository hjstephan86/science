...
der Seite mit Kardinalität $n$ (der â€žvielenâ€œ Seite, die den
...
Entwurfsparadigma: Die Frage, ob ein Datenbankschema â€žin die Tiefeâ€œ oder
â€žin die Breiteâ€œ orientiert ist, wird formal als eine Eigenschaft des
...
\begin{enumerate}
  ...
  \item Ein Beziehungstyp mit Kardinalität $1{:}n$ wird durch Aufnahme
    des Primärschlüssels der â€ž1â€œ-Seite als Fremdschlüssel in das
    Relationenschema der â€ž$n$â€œ-Seite realisiert.
...
Transformationsregel~2: Der Primärschlüssel von $R_j$ (der â€ž1â€œ-Seite)
...
(â€žrelational vollständigâ€œ) sind:
...
â€žvollständig relationalâ€œ zu gelten~\cite{Codd1970,Codd1974}. Für die
...
â€ždas übrige Tupel mitzuführenâ€œ, das zu einer bestimmten
...
und nicht direkt als gruppierende Aggregation mit â€žTupel-Rückgabeâ€œ
...
Gesamtordnung in â€žzuerst nach $G$, dann innerhalb jeder $G$-Gruppe
nach $(B_1,\ldots,B_q)$â€œ nicht mehr gültig, und PostgreSQL kann nicht
mehr garantieren, dass das â€žerste Tupel pro $G$-Gruppeâ€œ auch das
...
durchgängig verwendete Beispiel -- â€ždie jeweils neueste
$\textsc{Auftrag}$-Zeile pro $\textsc{Kunde}$â€œ -- ist die Anfrage
\emph{entlang} der Kante $(\textsc{Auftrag},\textsc{Kunde})$ des
Tiefenpfads aus Abschnitt~\ref{sec:beispiel}, präzisiert auf
â€žden jeweils \emph{aktuellsten Repräsentanten}â€œ pro Knoten der
...
\begin{itemize}
  ...
  \item \textbf{Erstes Vorkommen:} Für jede $\textsc{Region}$ den
    ...
    â€ždienstältestenâ€œ Kunden) ermitteln
...
â€žSpurious Tuplesâ€œ $(t_1[X],t_2[Y])$ und $(t_2[X],t_1[Y])$, die nicht in
...
â€žLossless-Join-Lemmaâ€œ: Bei der Zerlegung entlang einer transitiven
...
$\Sigma$ in einer â€žaufgeräumtenâ€œ Form vorliegt, in der keine
...
â€žGruppenâ€œ gleicher linker Seiten und damit dieselbe \emph{Anzahl}
...
â€žstarte mit $K=\att(R)$ und entferne iterativ Attribute $B$, solange
$(K\setminus\{B\})^+_\Sigma=\att(R)$â€œ. Dieser Schritt hat
...
Teilmengenrelationen keine FD aus $\Sigma_{\min}$ â€ždoppeltâ€œ
...
â€žmaximaleâ€œ Gruppen von FDs mit gleicher oder ineinander enthaltener
...
â€žBerechtigungsgranularitätâ€œ tiefer Pfade (Proposition zur
...
\begin{enumerate}
  ...
  \item \textbf{Typsystemdiskrepanz:} SQL-Datentypen
    ...
    \texttt{NULL} als â€žfehlender Wertâ€œ (vgl.\
...
â€žCursor über $R_i$ gefiltert nach Fremdschlüssel auf $R_{i-1}$â€œ und
â€žKante $(R_i,R_{i-1})$ in $G_{\mathcal{S}}$â€œ ist die Grundlage der in
...
SQL-Anweisung (das â€žWasâ€œ) von ihren \emph{Parametern} (das â€žWomitâ€œ).
...
â€ž$N{+}1$-Problemâ€œ bekannte Performance-Antimuster. Es ist
...
$G_{\mathcal{S}}$ betreffen -- etwa â€žBerechne den Gesamtumsatz aller
Kunden einer Regionâ€œ ($\textsc{Region}\leftarrow\textsc{Kunde}
...
â€žKompressionenâ€œ von Standardmustern.}
...
\begin{itemize}
  ...
  \item \textbf{Slowly Changing Dimensions als getarnte
    ...
    wird aus einer scheinbar â€žflachenâ€œ Dimensionstabelle implizit ein
    ...
    Theorem~\ref{thm:transitivitaet} um einen Fall â€žtransitiver
    Abhängigkeit über die Zeitachseâ€œ erweitern und damit zeigen, dass
    \emph{auch} die zeitliche Dimension dem Prinzip â€žTiefe vor Breiteâ€œ
    unterliegt.
  ...
  \item \textbf{\texttt{DISTINCT ON} als natürliches Werkzeug für
    ...
    Buches könnte einen eigenen Abschnitt â€žBitemporale Anfragen mit
    \texttt{DISTINCT ON} und Bereichstypen (\texttt{tstzrange})â€œ
...
\begin{itemize}
  \item \textbf{Tiefenpfad-erhaltende Schnitte.} Eine naheliegende
    ...
    â€žTiefenpfad-Schnitte minimieren die Anzahl verteilter
    Fremdschlüsselbeziehungen pro Partitionsgrenzeâ€œ, wäre ein
  ...
  \item \textbf{Sagas als verteilte Tiefenpfadtraversierung.} Das
    ...
    $R_1\to R_2\to\cdots\to R_l$ traversiert (z.\,B.\ â€žlege einen
    neuen Auftrag mit Positionen an, reserviere Lagerbestand,
    initiiere Zahlungâ€œ), entspricht jeder Schritt $T_i$ einer
    ...
    propagieren von â€žKindknotenâ€œ zu â€žElternknotenâ€œ des Tiefenpfads,
  ...
  \item \textbf{CAP-Theorem und lokale vs.\ globale Konsistenz
    ...
    -- von â€žstark konsistentâ€œ an der Wurzel zu â€ževentuell konsistentâ€œ
...
folgt einordnen: Im Muster â€žShared Schemaâ€œ wird \texttt{tenant\_id}
...
als \emph{orthogonale} Dimension (eine Art â€žglobale Wurzelâ€œ, die
...
Löschung.} Art.~17 DSGVO (â€žRecht auf Löschungâ€œ) verlangt, dass
...
\emph{â€žLöschbarkeitstiefeâ€œ} entwickeln: Für jeden Knoten $R_i$ eines
...
werden kann, wenn ein â€žLöschenâ€œ tatsächlich als
...
mathematische Fachliteratur üblichen Form â€žinformeller, aber
rigoroserâ€œ Beweise (vgl.\ \cite{Cormen2009}) geführt. Eine
...
\begin{itemize}
  ...
  \item \textbf{Maschinenüberprüfter Beweis von
    ...
    Tupelkonstruktion (insbesondere die Existenz â€žverirrter
    Tupelâ€œ -- spurious tuples -- im Falle der Notwendigkeitsrichtung)
  ...
  \item \textbf{Modellprüfung (Model Checking) von
    ...
    stammendes quantitatives Argument für â€žTiefe vor Breiteâ€œ.
\end{itemize}
...
â€žFeature-Tabelleâ€œ zu erzeugen.
...
\begin{itemize}
  \item \textbf{Empfangsfeld (Receptive Field) und Pfadtiefe.}
    ...
    $k$ sind für dieselbe semantische â€žReichweiteâ€œ identisch. Für ein
  ...
  \item \textbf{Feature-Engineering entlang von Repository-Grenzen.}
    ...
    â€žWahrscheinlichkeit einer Rückgabeâ€œ) ergibt sich durch
    ...
    Abschnitt~\ref{sec:select}, â€žVirtuelle Tabellenâ€œ) oder
...
\begin{itemize}
  ...
  \item \textbf{Property Graphs als Generalisierung des
    ...
    â€žVerbindungsrelation als Knotenâ€œ zu â€žVerbindungsrelation als
    Kanteâ€œ die in Proposition~\ref{prop:er-rel-graph} bewiesene
    ...
    â€želiminiertenâ€œ Verbindungsrelationen reduziert.
...
$\tau$, also â€žmehr Tiefe relativ zu Breiteâ€œ, impliziert â€žbesseres
$P$â€œ). Die in diesem Ausblick skizzierten Erweiterungen --
...
â€žstärkstenâ€œ klassischen Normalform, als Bezugspunkt für
Theorem~\ref{thm:synthese-korrekt}.
...
â€žRechts auf Löschungâ€œ und der Pseudonymisierungsanforderungen.
...
\bibitem{Angles2018}
Angles, R.; Arenas, M.; BarcelÃ³, P.; Hogan, A.; Reutter, J.;
VrgoÄ, D. (2018).
...
\bibitem{Sjoberg2005}
SjÃ¸berg, D.\,I.\,K.; Hannay, J.\,E.; Hansen, O.; Kampenes, V.\,B.;
