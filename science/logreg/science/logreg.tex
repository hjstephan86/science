...
Die logistische Regression zÃ¤hlt zu den fundamentalsten und gleichzeitig vielseitigsten
Methoden des maschinellen Lernens und der mathematischen Statistik. Seit ihrer formalen
EinfÃ¼hrung durch Cox (1958) hat sie eine bemerkenswerte Renaissance erlebt: WÃ¤hrend sie
als konzeptÃ¼lles Fundament tiefer neuronaler Netze gilt, bleibt sie gleichzeitig eine
...
Ausgehend von der wahrscheinlichkeitstheoretischen Fundierung Ã¼ber die Exponentialfamilie
werden sÃ¤mtliche zentralen Konzepte -- Sigmoid-Transformation, Kreuzentropie-Verlustfunktion,
KonvexitÃ¤t, Gradientenverfahren, Newton-Raphson, L-BFGS, L1/L2-Regularisierung und Softmax-
Erweiterung -- mit vollstÃ¤ndigen formalen Beweisen entwickelt. Vierzehn Abbildungen auf Basis
...
Federated Learning, Differenzielle Privatheit, Bayesianische Erweiterungen, erklÃ¤rbares KI,
Large Language Models als Wissensgrundlage sowie Anwendungen in der PrÃ¤zisionsmedizin,
Klimawissenschaft, Robotik und quantengestÃ¼tztem maschinellen Lernen.

\subsection{Historischer Kontext}

Die logistische Funktion geht auf Pierre-Francois Verhulst zurÃ¼ck, der sie 1845 zur Modellierung
des BevÃ¶lkerungswachstums entwickelte~\citep{verhulst1845recherches}. Den entscheidenden Schritt
zur statistischen Klassifikation vollzog David Cox 1958 mit seiner wegweisenden Arbeit Ã¼ber die
Regressionsanalyse binÃ¤rer SeqÃ¼nzen~\citep{cox1958regression}. In den folgenden Jahrzehnten
...
Einerseits wird sie als konzeptÃ¼lles Fundament tiefer neuronaler Netze und Transformer-
...
eigenstÃ¤ndige, interpretierbare Methode gegenÃ¼ber komplexeren Blackbox-AnsÃ¤tzen, gerade
...
\]
modelliert. FÃ¼nf fundamentale Probleme entstehen, wenn dieses Modell auf Klassifikation angewendet wird:

\begin{enumerate}[label=(\roman*)]
  \item \textbf{Wertebereich-Verletzung:} Lineare Modelle liefern Vorhersagen in $\R$, wÃ¤hrend Wahrscheinlichkeiten auf $[0,1]$ beschrÃ¤nkt sind.
  \item \textbf{HeteroskedastizitÃ¤t:} BinÃ¤re Daten weisen eine von $x$ abhÃ¤ngige Varianz $p(x)(1-p(x))$ auf -- die Gauss-Markov-Annahme der homoskedastischen ResidÃ¼n ist verletzt.
  \item \textbf{Nicht-NormalitÃ¤t der ResidÃ¼n:} BinÃ¤re ResidÃ¼n $y - \hat{y}$ sind diskret und nicht normalverteilt.
  \item \textbf{AusreissersensitivitÃ¤t:} Der quadratische Verlust bestraft weit entfernte Punkte Ã¼bermassig stark.
...
Die logistische Regression adressiert alle fÃ¼nf Probleme durch die Wahl der Bernoulli-Likelihood und der kanonischen Sigmoid-Linkfunktion.

% ============================================================
\section{Mathematische Grundlagen}
% ============================================================

\subsection{Der lineare PrÃ¤diktor}

Sei $\vx = (x_1, x_2, \ldots, x_d)^\T \in \R^d$ ein Merkmalsvektor und $\vw = (w_1, w_2, \ldots, w_d)^\T \in \R^d$ ein Gewichtsvektor. Der \emph{lineare PrÃ¤diktor} (auch: \emph{logit} oder \emph{log-Odds}) ist definiert als:
...
\begin{proposition}[Eigenschaften der Sigmoid-Funktion]
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Symmetrie:} $\sigmoid(-z) = 1 - \sigmoid(z)$ fÃ¼r alle $z \in \R$.
    ...
    \item \textbf{InversitÃ¤t:} $\sigmoid^{-1}(p) = \ln\!\left(\frac{p}{1-p}\right)$ fÃ¼r $p \in (0,1)$ (Logit-Funktion).
...
\begin{proof}
  ...
  \textbf{(vi):} Da $|\sigmoid'(z)| \leq 1/4$ fÃ¼r alle $z$, folgt aus dem Mittelwertsatz:
...
und konseqÃ¼nterweise $\Prob(Y=0 \mid \vx;\,\vw,b) = 1 - \hat{p}(\vx) = \sigmoid(-\vw^\T\vx - b)$.

\subsection{Log-Odds und Interpretierbarkeit}

\begin{proposition}[Log-Odds-LinearitÃ¤t]
  Unter dem logistischen Modell gilt:
  \begin{equation}
    \ln\frac{\hat{p}(\vx)}{1-\hat{p}(\vx)} = \vw^\T\vx + b.
    \label{eq:log_odds}
  \end{equation}
  Eine ErhÃ¶hung von $x_j$ um eine Einheit bei konstanten Ã¼brigen Merkmalen ergibt eine
  multiplikative Ã„nderung der Odds um den Faktor $e^{w_j}$.
\end{proposition}
...
\begin{example}[Klinische Interpretation]
  ...
  etwa $5.1\%$. Dieses Odds-Ratio ist die primÃ¤re BerichtsgrÃ¶sse in epidemiologischen Studien.
\end{example}
...
\begin{definition}[Generalisiertes Lineares Modell nach McCullagh und Nelder]
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Systematische Komponente:} Linearer PrÃ¤diktor $z = \vw^\T\vx + b$.
    \item \textbf{Linkfunktion:} Monotone, differenzierbare Funktion $g$ mit $g(\E[Y\mid\vx]) = z$.
  \end{enumerate}
\end{definition}

FÃ¼r die logistische Regression gilt: $Y\mid\vx \sim \mathrm{Bernoulli}(\hat{p})$ und die
...
\begin{proposition}[Kanonische Form der Bernoulli-Verteilung]
  Die Bernoulli-Verteilung $\mathrm{Bern}(p)$ mit $p \in (0,1)$ lÃ¤sst sich in der kanonischen
...
\begin{proposition}[Momente aus der Log-Partition-Funktion]
  FÃ¼r jede Exponentialfamilien-Verteilung gilt:
  \[
...
\begin{proof}
  ...
  $A''(\eta) = \sigmoid(\eta)(1-\sigmoid(\eta))$. Die IdentitÃ¤t $\E[T(Y)] = A'(\eta)$ gilt
  allgemein fÃ¼r Exponentialfamilien und folgt aus Differentiation unter dem Integral. 
\end{proof}

\subsection{Suffiziente Statistik und Informationsgeometrie}

Die Bernoulli-Verteilung gehÃ¶rt zur einparametrigen Exponentialfamilie mit suffizienter
...
Bernoulli-Annahme mit bedingter UnabhÃ¤ngigkeit ist:
...
\subsection{BinÃ¤re Kreuzentropie und Informationstheorie}

\begin{definition}[BinÃ¤re Kreuzentropie-Verlustfunktion]
  \begin{equation}
...
\begin{figure}[H]
  ...
  \caption{Vergleich der Verlustfunktionen (Kreuzentropie, MSE, Hinge-Loss) fÃ¼r beide Klassen.
           Die Kreuzentropie bestraft hochkonfidente Fehlklassifikationen am stÃ¤rksten.}
  \label{fig:loss}
\end{figure}

\begin{proposition}[Informationstheoretische Interpretation]
  Die binÃ¤re Kreuzentropie ist die \emph{Kreuzentropie} zwischen der empirischen Verteilung
  ...
  \]
  wobei $H(q_i)$ die binÃ¤re Entropie der wahren Verteilung und $\KL(q_i\|\hat{p}_i)$ die
  Kullback-Leibler-Divergenz bezeichnen. Da $H(q_i)$ nicht von den Parametern abhÃ¤ngt, ist
  die Minimierung der Kreuzentropie Ã¤quivalent zur Minimierung der KL-Divergenz.
\end{proposition}

\subsection{KonvexitÃ¤t der Verlustfunktion}

\begin{theorem}[KonvexitÃ¤t der binÃ¤ren Kreuzentropie]
  \label{thm:convexity}
...
\begin{proof}
  ...
  und $\tilde{\vx}^{(i)} = (1, {\vx^{(i)}}^\T)^\T$. FÃ¼r den $i$-ten Term der Verlustfunktion gilt:
  \[
  ...
  $\vS = \diag(s_1, \ldots, s_n) \succeq 0$ ist. FÃ¼r jeden Vektor $\bm{v} \in \R^{d+1}$ gilt:
  \[
    \bm{v}^\T \nabla^2 \mathcal{L}\, \bm{v}
    = \frac{1}{n}\sum_{i=1}^n s_i (\tilde{\vx}^{(i)\T}\bm{v})^2 \geq 0,
  \]
  da $s_i \geq 0$. Also ist $\nabla^2\mathcal{L} \succeq 0$, was die KonvexitÃ¤t beweist.
  Strikte KonvexitÃ¤t gilt genau dann, wenn $\tilde{\vX}$ vollen Spaltenrang hat und
...
\begin{proposition}[Gradient der binÃ¤ren Kreuzentropie]
  \begin{align}
...
\begin{algorithm}[H]
  \caption{Batch-Gradientenabstieg fÃ¼r logistische Regression}
  \label{alg:gd}
  \begin{algorithmic}[1]
    \Require Datensatz $\mathcal{D}$, Lernrate $\eta > 0$, Toleranz $\epsilon > 0$, Max. Iterationen $T$
    \Ensure ParameterschÃ¤tzer $\hat{\vw}$, $\hat{b}$
    \State Initialisiere $\vw \leftarrow \mathbf{0} \in \R^d$, $b \leftarrow 0$
    \For{$t = 1, 2, \ldots, T$}
      \State $z_i \leftarrow \vw^\T\vx^{(i)}+b$ fÃ¼r alle $i = 1,\ldots,n$
      \State $\hat{p}_i \leftarrow \sigmoid(z_i)$ fÃ¼r alle $i$
...
\begin{theorem}[Konvergenz des Gradientenabstiegs]
  ...
  \end{equation}
  wobei $\mu > 0$ der kleinste Eigenwert der Hesse-Matrix (Starke-KonvexitÃ¤ts-Parameter) ist.
\end{theorem}

\begin{proof}
  Da $\mathcal{L}$ $L$-glatt ist, gilt fÃ¼r alle $\vw, \tilde{\vw}$:
  \[
...
\begin{figure}[H]
  ...
  \caption{Links: Einfluss der Lernrate auf die Konvergenzgeschwindigkeit. Zu grosse Lernraten
           fÃ¼hren zur Divergenz. Rechts: Vergleich zwischen vollstÃ¤ndigem GD, Mini-Batch SGD
...
Das Newton-Verfahren nutzt die Hesse-Matrix fÃ¼r quadratisch konvergente Updates:
...
\begin{theorem}[Newton-Raphson als IRLS]
  Das Newton-Raphson-Verfahren fÃ¼r die logistische Regression ist Ã¤quivalent zum
  ...
  \begin{equation}
    ...
    \label{eq:irls}
  \end{equation}
  gelÃ¶st, mit Gewichtsmatrix $\vS^{(t)} = \diag(s_i^{(t)})$, $s_i^{(t)} = \hat{p}_i^{(t)}(1-\hat{p}_i^{(t)})$,
...
\subsection{L-BFGS: Quasi-Newton-Methode fÃ¼r grosse Dimensionen}

FÃ¼r hochdimensionale Probleme ($d \gg 1$) ist die explizite Berechnung und Invertierung der
...
\begin{algorithm}[H]
  \caption{Adam-Optimizer fÃ¼r logistische Regression}
  \label{alg:adam}
  \begin{algorithmic}[1]
...
\begin{definition}[Entscheidungsgrenze]
  ...
  \]
  FÃ¼r $\tau=0.5$: $\mathcal{H}_{0.5} = \{\vx : \vw^\T\vx + b = 0\}$.
\end{definition}
...
Je grÃ¶sser $|d(\vx_0,\mathcal{H})|$, desto weiter ist der Punkt von der Entscheidungsgrenze
entfernt und desto sicherer ist die Vorhersage. Bei $|d| \to \infty$ nÃ¤hert sich $\hat{p}$
...
\begin{definition}[Kernel-Logistische Regression]
  Sei $\phi: \R^d \to \mathcal{H}$ eine Feature-Map in einen (mÃ¶glicherweise unendlichdimensionalen)
  Hilbert-Raum $\mathcal{H}$. Die Kernel-Logistische Regression modelliert:
  \[
    \hat{p}(\vx) = \sigmoid(\langle\vw, \phi(\vx)\rangle_{\mathcal{H}} + b).
  \]
  Der \emph{Representer-Satz} garantiert, dass die LÃ¶sung die Form
  $\vw = \sum_{i=1}^n \alpha_i \phi(\vx^{(i)})$ hat, was die Auswertung via Kernel
  $k(\vx, \vx') = \langle\phi(\vx),\phi(\vx')\rangle$ ermÃ¶glicht.
\end{definition}
...
\begin{proposition}[Bayesianische Interpretation der L2-Regularisierung]
  L2-Regularisierung entspricht einer \emph{Maximum a Posteriori} (MAP)-SchÃ¤tzung mit
...
\begin{proposition}[SparsitÃ¤tsinduktion durch L1]
  Die L1-regularisierte Verlustfunktion hat die Eigenschaft, dass fÃ¼r genÃ¼gend grosse $\lambda_1$
  bestimmte KÃ¶ffizienzen exakt zu Null gesetzt werden (SparsitÃ¤t). Die Karush-Kuhn-Tucker-
  Bedingungen fÃ¼r ein Minimum $\hat{\vw}$ lauten:
  \[
...
\begin{figure}[H]
  ...
  \caption{Regularisierungspfade. Links: L1-Regularisierung setzt KÃ¶ffizienzen exakt auf Null
           (SparsitÃ¤t). Rechts: L2-Regularisierung schrumpft alle KÃ¶ffizienzen gleichmÃ¤ssig
           ohne vollstÃ¤ndige Elimination.}
...
\begin{table}[H]
  \centering
  \caption{Vergleich der Regularisierungsverfahren fÃ¼r logistische Regression.}
...
\begin{figure}[H]
  ...
  \caption{Bias-Varianz-Tradeoff: Starke Regularisierung ($C$ klein) fÃ¼hrt zu hohem Bias
...
% ============================================================
\section{Varianzinflationsfaktor (VIF) und MultikollinearitÃ¤t}
\label{sec:vif}
% ============================================================

Eines der zentralen QualitÃ¤tsprobleme in der logistischen Regression -- wie auch in der 
linearen Regression -- ist die \emph{MultikollinearitÃ¤t} zwischen PrÃ¤diktorvariablen. 
Wenn Eingabemerkmale stark miteinander korrelieren, werden die geschÃ¤tzten RegressionskÃ¶ffizienten 
instabil, ihre Varianzen aufgeblasen und ihre Interpretierbarkeit fundamental beeintrÃ¤chtigt. 
...
praktische Bedeutung fÃ¼r die logistische Regression in vollstÃ¤ndiger Tiefe.

% ------------------------------------------------------------
\subsection{Motivation: Das Problem der MultikollinearitÃ¤t}
% ------------------------------------------------------------

\begin{definition}[MultikollinearitÃ¤t]
	Seien $\vx_1, \vx_2, \ldots, \vx_d \in \R^n$ die Spalten der Designmatrix $\vX \in \R^{n \times d}$.
	\emph{Exakte MultikollinearitÃ¤t} liegt vor, wenn es einen Vektor $\bm{c} = (c_1, \ldots, c_d)^\T \neq \bm{0}$
	gibt mit
	\[
	\sum_{j=1}^{d} c_j \vx_j = \bm{0},
	\]
	d.\,h.\ wenn $\vX$ nicht vollen Spaltenrang hat. \emph{Approximative MultikollinearitÃ¤t} liegt vor,
	wenn diese Beziehung nÃ¤herungsweise gilt, also $\|\sum_j c_j \vx_j\|_2 \approx 0$ fÃ¼r ein
	nichttriviales $\bm{c}$.
\end{definition}

\begin{remark}[KonseqÃ¼nzen exakter MultikollinearitÃ¤t]
	Bei exakter MultikollinearitÃ¤t ist die Designmatrix $\vX^\T\vX$ singulÃ¤r. Die 
	Normalengleichungen der linearen Regression besitzen dann unendlich viele LÃ¶sungen, 
	und der Maximum-Likelihood-SchÃ¤tzer der logistischen Regression existiert nicht eindeutig. 
	In der Praxis ist approximative MultikollinearitÃ¤t das weitaus hÃ¤ufigere und 
	gleichzeitig schwerer zu diagnostizierende Problem.
\end{remark}

Um das Problem zu verstehen, betrachten wir den Standardfehler eines RegressionskÃ¶ffizienten.
Im linearen Regressionsmodell $\vy = \vX\vw + \bm{\varepsilon}$ mit $\bm{\varepsilon} \sim \mathcal{N}(\bm{0}, \sigma^2\mathbf{I})$
ist die Kovarianzmatrix des OLS-SchÃ¤tzers $\hat{\vw} = (\vX^\T\vX)^{-1}\vX^\T\vy$:
\begin{equation}
	\mathrm{Cov}(\hat{\vw}) = \sigma^2 (\vX^\T\vX)^{-1}.
	\label{eq:cov_ols}
\end{equation}
Insbesondere ist die Varianz des $j$-ten KÃ¶ffizienten:
\begin{equation}
...
sehr groÃŸ, was zu groÃŸen Standardfehlern, breiten Konfidenzintervallen und damit zur
UnzuverlÃ¤ssigkeit der KÃ¶ffizienten fÃ¼hrt -- selbst wenn das Modell als Ganzes eine gute
VorhersagequalitÃ¤t besitzt.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			xlabel={Korrelation $\rho$ zwischen zwei PrÃ¤diktoren},
			...
			title={Einfluss der Korrelation auf den Standardfehler eines KÃ¶ffizienten},
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Standardfehler eines RegressionskÃ¶ffizienten als Funktion der Korrelation $\rho$ 
		zwischen zwei PrÃ¤diktoren. Ab $\rho \approx 0.7$ steigt der Standardfehler stark an; 
...
\begin{definition}[Varianzinflationsfaktor]
	...
	FÃ¼r den $j$-ten PrÃ¤diktor $\vx_j$ wird eine Hilfsregression durchgefÃ¼hrt:
	\[
	\vx_j = \alpha_0 + \sum_{k \neq j} \alpha_k \vx_k + \bm{\varepsilon}_j,
	\]
	d.\,h.\ $\vx_j$ wird auf alle anderen PrÃ¤diktoren regressiert. Sei $R_j^2 \in [0,1)$ das
	BestimmtheitsmaÃŸ dieser Hilfsregression. Der \emph{Varianzinflationsfaktor} des $j$-ten
	PrÃ¤diktors ist dann definiert als:
	\begin{equation}
...
\begin{proof}
	...
	Da die Designmatrix vollrangig ist, kÃ¶nnen wir die Spalten so permutieren, dass die
	$j$-te Spalte an letzter Stelle steht. Da Permutationen simultan auf Zeilen und Spalten
	wirken und den gesuchten Diagonaleintrag nicht verÃ¤ndern, schreiben wir o.\,B.\,d.\,A.
	\[
	...
	\]
	Nach dem Satz Ã¼ber die Inverse einer $2\times2$-Blockmatrix (vgl.\@ Golub \& Van
	Loan~\cite{golub2013matrix}) gilt fÃ¼r den $(d,d)$-Eintrag -- also den auf die $j$-te
	...
	haben positiv definite Gramsche Matrix, und positive Definitheit ist Ã¤quivalent zu
	$S_j > 0$ fÃ¼r alle $j$).
	...
	Der OLS-SchÃ¤tzer in der Hilfsregression $\vx_j = \vX_{-j}\bm{\alpha} + \bm{\varepsilon}_j$
	...
	\]
	und die Residualquadratsumme betrÃ¤gt
	\[
	...
	$\mathrm{TSS}_j = \|\vx_j\|_2^2$. Das BestimmtheitsmaÃŸ der Hilfsregression ist somit:
	\[
	R_j^2 = 1 - \frac{\mathrm{RSS}_j}{\mathrm{TSS}_j}
	= 1 - \frac{\|\bm{M}_{-j}\vx_j\|_2^2}{\|\vx_j\|_2^2}
	= 1 - \frac{S_j}{\|\vx_j\|_2^2}.
	\]
	AuflÃ¶sen nach $S_j$ liefert:
	\[
	S_j = \|\vx_j\|_2^2\,(1 - R_j^2).
	\]

	\textbf{Schritt 5: ZusammenfÃ¼hrung.}
...
\begin{corollary}[VIF als Varianzinflation]
	...
	Aus Gleichung~\eqref{eq:var_cÃ¶ff} und Theorem~\ref{thm:vif_derivation} folgt unmittelbar:
	\begin{equation}
		\mathrm{Var}(\hat{w}_j) = \sigma^2 \cdot \frac{\mathrm{VIF}_j}{\|\vx_j\|_2^2}.
		\label{eq:var_inflated}
	\end{equation}
	Im Vergleich zur hypothetischen Situation ohne MultikollinearitÃ¤t ($R_j^2 = 0$, also $\mathrm{VIF}_j = 1$)
	ist die Varianz von $\hat{w}_j$ um den Faktor $\mathrm{VIF}_j$ vergrÃ¶ÃŸert. Daher der Name
...
\begin{remark}[Minimaler VIF-Wert]
	...
	wenn $\vx_j$ vollstÃ¤ndig unkorreliert zu allen anderen PrÃ¤diktoren ist ($R_j^2 = 0$).
	Ein VIF von~1 entspricht dem idealen Fall vollstÃ¤ndiger OrthogonalitÃ¤t.
\end{remark}

% ------------------------------------------------------------
\subsection{Berechnung des VIF: Schritt fÃ¼r Schritt}
% ------------------------------------------------------------

Die praktische Berechnung des VIF erfolgt algorithmisch in drei Schritten, die wir hier
vollstÃ¤ndig formalisieren.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{Schritt 2:} SchÃ¤tze OLS-Regression: $\hat{\bm{\alpha}} = (\vX_{-j}^\T\vX_{-j})^{-1}\vX_{-j}^\T\vx_j$
		\State \textbf{Schritt 3:} Berechne ResidÃ¼n: $\hat{\bm{e}}_j = \vx_j - \vX_{-j}\hat{\bm{\alpha}}$
...
\begin{remark}[Numerisch stabile Berechnung]
	FÃ¼r groÃŸe $d$ empfiehlt sich die Berechnung via QR-Zerlegung statt direkter 
	...
	\]
	Dies vermeidet die numerisch aufwendige und fehleranfÃ¤llige Matrixinversion 
	$(\vX_{-j}^\T\vX_{-j})^{-1}$ und ist in Zeit $\mathcal{O}(nd^2)$ durchfÃ¼hrbar.
\end{remark}

\begin{example}[VIF-Berechnung fÃ¼r zwei PrÃ¤diktoren]
	Betrachte $n = 100$ Beobachtungen mit zwei PrÃ¤diktoren $x_1, x_2$ mit KorrelationskÃ¶ffizient
	...
	\end{equation}
	Tabelle~\ref{tab:vif_rho} zeigt die VIF-Werte fÃ¼r ausgewÃ¤hlte Korrelationen.
\end{example}

\begin{table}[H]
	\centering
	\caption{VIF-Werte in AbhÃ¤ngigkeit von der Korrelation $\rho$ bei zwei PrÃ¤diktoren.}
	\label{tab:vif_rho}
	\begin{tabular}{cccc}
		\toprule
...
Die VIF-Skala lÃ¤sst sich in drei diagnostisch relevante Bereiche einteilen, die in der
Literatur und in der Praxis weitgehend konsensfÃ¤hig sind:
...
\begin{tcolorbox}[thmbox, title={VIF-Klassifikationsschema}]
	\begin{tabular}{p{3cm}p{3.5cm}p{7cm}}
		...
		Keine MultikollinearitÃ¤t. KÃ¶ffizienten sind stabil und zuverlÃ¤ssig interpretierbar. \\[6pt]
		$5 \leq \mathrm{VIF} < 10$ & \textcolor{orange!80!black}{\textbf{Moderat}} (beachtenswert) & 
		Moderate MultikollinearitÃ¤t vorhanden. SorgfÃ¤ltige Modelldiagnose empfohlen. \\[6pt]
		$\mathrm{VIF} \geq 10$ & \textcolor{accentred}{\textbf{Hoch}} (problematisch) & 
		Schwere MultikollinearitÃ¤t. MaÃŸnahmen zur Reduktion dringend erforderlich. \\
	\end{tabular}
\end{tcolorbox}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{VisÃ¼lle Klassifikationsskala des Varianzinflationsfaktors. 
		Werte unterhalb von~5 gelten als unkritisch; ab~10 liegt schwere 
		MultikollinearitÃ¤t vor, die dringende GegenmaÃŸnahmen erfordert.}
	\label{fig:vif_scale}
\end{figure}

\subsubsection{Niedriger VIF: $\mathrm{VIF} < 5$}

Ein VIF unter~5 signalisiert, dass der $j$-te PrÃ¤diktor hÃ¶chstens $80\,\%$ seiner Varianz
durch die Ã¼brigen PrÃ¤diktoren erklÃ¤rt wird ($R_j^2 < 0.8$). Die KÃ¶ffizienten sind in diesem
Bereich:
\begin{itemize}
	\item \textbf{Stabil:} Kleine Ã„nderungen im Datensatz fÃ¼hren zu keinen dramatischen
	KÃ¶ffizientenverschiebungen.
	\item \textbf{Interpretierbar:} Die Log-Odds-Interpretation (vgl.~Gl.~\eqref{eq:log_odds})
	bleibt gÃ¼ltig und inhaltlich bedeutsam.
	\item \textbf{Inferenz zuverlÃ¤ssig:} Wald-Tests, Likelihood-Ratio-Tests und Konfidenzintervalle
	haben ihre nominale Deckungswahrscheinlichkeit.
\end{itemize}

\subsubsection{Moderater VIF: $5 \leq \mathrm{VIF} < 10$}

Im moderaten Bereich besteht eine spÃ¼rbare, aber noch handhabbare MultikollinearitÃ¤t.
Der entsprechende $R_j^2$-Wert liegt zwischen $0.8$ und $0.9$. Die KonseqÃ¼nzen sind:
\begin{itemize}
	\item \textbf{ErhÃ¶hte Standardfehler:} Die Standardfehler der KÃ¶ffizienten sind um den
	Faktor $\sqrt{\mathrm{VIF}_j}$ vergrÃ¶ÃŸert -- also um das $2.2$- bis $3.2$-fache gegenÃ¼ber
	...
	\item \textbf{VorhersagequalitÃ¤t unbeeintrÃ¤chtigt:} Das Modell kann dennoch gute
	Vorhersagen liefern -- das Problem liegt primÃ¤r in der KÃ¶ffizienteninterpretation.
\end{itemize}
In diesem Bereich ist eine sorgfÃ¤ltige EinzelfallabwÃ¤gung erforderlich: Falls Interpretierbarkeit
der KÃ¶ffizienten primÃ¤res Ziel ist, sind GegenmaÃŸnahmen angezeigt; bei rein prÃ¤diktiven
Modellen kann der moderate VIF toleriert werden.

\subsubsection{Hohe MultikollinearitÃ¤t: $\mathrm{VIF} \geq 10$}

Ab einem VIF von~10 liegt $R_j^2 \geq 0.9$ vor: Der $j$-te PrÃ¤diktor ist zu mindestens $90\,\%$
durch die Ã¼brigen PrÃ¤diktoren linear erklÃ¤rbar. Die Folgen sind gravierend:

\begin{theorem}[InstabilitÃ¤t bei hohem VIF]
	\label{thm:instability}
	Sei $\mathrm{VIF}_j \geq 10$ fÃ¼r einen PrÃ¤diktor $x_j$. Dann ist die Konditionszahl
	\[
	...
	\]
	im KÃ¶ffizientenschÃ¤tzer, der durch $\mathrm{VIF}_j$ nach unten beschrÃ¤nkt ist.
\end{theorem}

\begin{proof}
	\textbf{Teil 1: Untere Schranke fÃ¼r die Konditionszahl.}
	...
	\emph{Schranke fÃ¼r $\lambda_{\min}(\vX^\T\vX)$:}
	Nach dem Courant-Fischer-Min-Max-Theorem gilt fÃ¼r jeden Einheitsvektor $\bm{u}$:
	\[
	...
	\emph{Schranke fÃ¼r $\lambda_{\max}(\vX^\T\vX)$:}
	...
	\]
	da $\vX\bm{e}_j$ gerade die $j$-te Spalte von $\vX$ extrahiert, nÃ¤mlich $\vx_j \in \R^n$.
	...
	\textbf{Teil 2: FehlerfortpflanzungsabschÃ¤tzung.}
	
	Sei $\vy$ um $\delta\vy \in \R^n$ gestÃ¶rt. Das normale Gleichungssystem
	$(\vX^\T\vX)\hat{\vw} = \vX^\T\vy$ geht Ã¼ber in
	$(\vX^\T\vX)(\hat{\vw}+\delta\hat{\vw}) = \vX^\T(\vy+\delta\vy)$,
	woraus unmittelbar $(\vX^\T\vX)\,\delta\hat{\vw} = \vX^\T\delta\vy$ folgt. Die LÃ¶sung
	ist $\delta\hat{\vw} = (\vX^\T\vX)^{-1}\vX^\T\delta\vy$, und die Norm des Fehlers wird
	nach oben abgeschÃ¤tzt durch:
	\[
	\|\delta\hat{\vw}\|_2
	\leq \bigl\|(\vX^\T\vX)^{-1}\bigr\|_2 \cdot \|\vX^\T\|_2 \cdot \|\delta\vy\|_2
	= \frac{1}{\lambda_{\min}(\vX^\T\vX)} \cdot \sigma_{\max}(\vX) \cdot \|\delta\vy\|_2,
	\]
	wobei $\sigma_{\max}(\vX) = \sqrt{\lambda_{\max}(\vX^\T\vX)}$ der maximale SingulÃ¤rwert
	von $\vX$ ist. FÃ¼r die Norm der LÃ¶sung gilt analog:
	\[
	...
	KÃ¶ffizientenfehler durch $\mathrm{VIF}_j$ nach unten beschrÃ¤nkt -- auch das geringe
	Messfehler in $\vy$ werden um mindestens den Faktor $\mathrm{VIF}_j$ verstÃ¤rkt.
\end{proof}

Praktische Symptome hoher MultikollinearitÃ¤t ($\mathrm{VIF} \geq 10$) sind:
\begin{enumerate}[label=(\roman*)]
	\item KÃ¶ffizientenschÃ¤tzer mit unerwartetem Vorzeichen (z.\,B.\ negativer KÃ¶ffizient
	fÃ¼r einen inhaltlich positiv wirksamen PrÃ¤diktor).
	\item Hohe SensitivitÃ¤t gegenÃ¼ber kleinen StichprobenverÃ¤nderungen -- das HinzufÃ¼gen
	oder Entfernen weniger Beobachtungen kann KÃ¶ffizienten dramatisch verÃ¤ndern.
	\item Statistische Tests werden insignifikant, obwohl inhaltlich ein starker Zusammenhang
	vermutet wird (``Scheininsignifikanz'' durch aufgeblÃ¤hte Standardfehler).
	\item Iterative Optimierungsverfahren (Gradient Descent, IRLS) konvergieren langsam
	oder benÃ¶tigen sehr kleine Lernraten.
\end{enumerate}
...
Obwohl der VIF ursprÃ¼nglich fÃ¼r die lineare Regression entwickelt wurde, ist er bei der
logistischen Regression ebenso anwendbar und interpretierbar. Dies grÃ¼ndet auf folgender
Parallelstruktur:
...
\begin{proposition}[Hesse-Matrix der logistischen Regression und VIF]
	...
	\end{equation}
	Der \emph{gewichtete VIF} fÃ¼r die logistische Regression ist:
	\begin{equation}
		\mathrm{VIF}_j^{\mathrm{log}} = \frac{1}{1 - R_j^{2,S}},
		\label{eq:vif_logistic}
	\end{equation}
	wobei $R_j^{2,S}$ das gewichtete BestimmtheitsmaÃŸ der Hilfsregression von $\vx_j$ auf
	$\vX_{-j}$ mit Gewichtsmatrix $\vS$ bezeichnet.
\end{proposition}

\begin{remark}[Praktische Approximation]
	In der Praxis wird fÃ¼r die logistische Regression hÃ¤ufig der standard (ungewichtete)
	VIF aus Definition~\ref{def:vif} berechnet, da die Gewichte $s_i = \hat{p}_i(1-\hat{p}_i)$
	vom Modell selbst abhÃ¤ngen. Die ungewichtete Approximation ist in den meisten FÃ¤llen
	ausreichend und bleibt das Standardvorgehen in Statistiksoftware
	(R: \texttt{car::vif()}; Python: \texttt{statsmodels.stats.outliers\_inflÃ¼nce.variance\_inflation\newline \_factor()}).
\end{remark}
...
\begin{sidewaysfigure}
	...
	\begin{tikzpicture}[
		...
		\node[box, right=of aux]   (r2)    {Berechne $R_j^2$\\fÃ¼r alle $j$};
		\node[box, right=of r2]    (vif)   {$\mathrm{VIF}_j = \frac{1}{1-R_j^2}$};
		\node[box, below=of vif]   (diag)  {Diagnose:\\$<5$,\ $5$--$10$,\ $\geq 10$};
		\node[box, below=of diag]  (action){GegenmaÃŸnahmen\\(falls nÃ¶tig)};
	...
	\end{tikzpicture}
	\caption{Workflow zur VIF-basierten MultikollinearitÃ¤tsdiagnose in der logistischen Regression.
		Nach der Diagnose kÃ¶nnen GegenmaÃŸnahmen ergriffen und das Modell neu angepasst werden.}
	\label{fig:vif_workflow}
\end{sidewaysfigure}

% ------------------------------------------------------------
\subsection{Beziehungen zwischen VIF und anderen DiagnosemaÃŸen}
% ------------------------------------------------------------

\begin{proposition}[VIF und Toleranz]
	Das \emph{ToleranzmaÃŸ} ist das Reziproke des VIF:
	\begin{equation}
	...
	PrÃ¤diktoren erklÃ¤rt wird. Ein Toleranzwert nahe~0 entspricht starker MultikollinearitÃ¤t,
	ein Wert nahe~1 vollstÃ¤ndiger Unkorreliertheit.
\end{proposition}
...
\begin{proposition}[Zusammenhang mit dem Konditionsindex]
	...
	\]
	Konditionsindizes $\kappa_k > 30$ deuten auf starke MultikollinearitÃ¤t hin. WÃ¤hrend
	VIF und Konditionsindizes komplementÃ¤re Informationen liefern, sind VIF-Werte prÃ¤diktorspezifisch
	und daher fÃ¼r gezielte Diagnose vorzuziehen.
\end{proposition}

% ------------------------------------------------------------
\subsection{Strategien zur Reduktion der MultikollinearitÃ¤t}
% ------------------------------------------------------------

Wenn der VIF-Test problematische Werte offenbart, stehen folgende GegenmaÃŸnahmen zur VerfÃ¼gung:

\subsubsection{Entfernung korrelierter PrÃ¤diktoren}

Die direkteste MaÃŸnahme ist das Entfernen eines der korrelierten PrÃ¤diktoren. Die Wahl,
welcher PrÃ¤diktor entfernt wird, sollte sich an inhaltlichen Kriterien (theoretische Relevanz)
und statistischen Kriterien (geringerer VIF des verbleibenden PrÃ¤diktors) orientieren.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Ensure Reduzierte Merkmalsmenge ohne problematische MultikollinearitÃ¤t
		...
		\State Aktualisiere alle VIF-Werte fÃ¼r das reduzierte $\vX$
		\EndWhile
		\State \Return aktÃ¼lles $\vX$
	\end{algorithmic}
\end{algorithm}

\subsubsection{Kombination korrelierter PrÃ¤diktoren}

Anstatt PrÃ¤diktoren zu verwerfen, kÃ¶nnen stark korrelierte Variablen zu einem einzigen
PrÃ¤diktor zusammengefasst werden -- etwa durch gewichtete Mittelwertbildung oder durch
einen inhaltlich begrÃ¼ndeten Index:
\[
...
\begin{proposition}[PCA-basierte Dekorrelation]
	Sei $\vX = \bm{U}\bm{\Sigma}\bm{V}^\T$ die SingulÃ¤rwertzerlegung der standardisierten
	...
	Regression von $\vy$ auf die $m \leq d$ fÃ¼hrenden Hauptkomponenten $\bm{Z}_m = \vX\bm{V}_m$:
	\begin{equation}
...
\begin{remark}[AbwÃ¤gung: Interpretierbarkeit vs.\ MultikollinearitÃ¤t]
	PCA lÃ¶st das MultikollinearitÃ¤tsproblem vollstÃ¤ndig, erkauft dies aber mit dem Verlust
	der direkten Interpretierbarkeit der KÃ¶ffizienten: Die $\beta_k$ in~\eqref{eq:pclr}
	beziehen sich auf latente Linearkombinationen der Originalmerkmale, nicht auf diese selbst.
	In Bereichen wie der klinischen Medizin, wo KÃ¶ffizienteninterpretierbarkeit entscheidend ist,
...
\begin{theorem}[Ridge-Regression und VIF]
	...
	effektive VIF erfÃ¼llt fÃ¼r alle $\lambda > 0$:
	\begin{equation}
		\mathrm{VIF}_j^{\mathrm{Ridge}}(\lambda) \leq \mathrm{VIF}_j(\lambda=0) = \mathrm{VIF}_j.
		\label{eq:vif_ridge}
	\end{equation}
	FÃ¼r $\lambda \to \infty$ gilt $\mathrm{VIF}_j^{\mathrm{Ridge}}(\lambda) \to 1$ fÃ¼r alle~$j$.
\end{theorem}
...
\begin{proof}
	...
	\end{equation}
	Diese Darstellung ist wohldefiniert fÃ¼r alle $\lambda > 0$, da $\lambda_k + \lambda > 0$.
	...
	Matrizenspalte der orthogonalen Matrix $\bm{V}$), gilt fÃ¼r alle $\lambda > 0$ und fÃ¼r jeden Summanden:
	\[
	\frac{v_{kj}^2}{\lambda_k + \lambda} < \frac{v_{kj}^2}{\lambda_k}.
	\]
	Summation Ã¼ber $k = 1, \ldots, d$ liefert:
	\[
	\bigl[(\vX^\T\vX + \lambda\bm{I})^{-1}\bigr]_{jj}
	= \sum_{k=1}^d \frac{v_{kj}^2}{\lambda_k+\lambda}
	...
	\textbf{Schritt 4: Ãœbergang zum effektiven VIF.}
	...
	\textbf{Schritt 5: Grenzwert fÃ¼r $\lambda \to \infty$.}
	...
	\]
	da $\lambda_1 > \lambda_d$ (strenges Ungleichen bei $d \geq 2$ und vollrangigem $\vX$). FÃ¼r
	$\lambda \to \infty$ gilt:
	\[
	...
	$\mathrm{VIF}_j^{\mathrm{Ridge}}(\lambda) \to 1$ fÃ¼r alle $j$. Formal:
	\[
	\mathrm{VIF}_j^{\mathrm{Ridge}}(\lambda)
	= \|\vx_j\|_2^2 \sum_{k=1}^d \frac{v_{kj}^2}{\lambda_k+\lambda}
	...
	\]
	d.\,h.\ die Ridge-Regularisierung unterdrÃ¼ckt asymptotisch jeglichen Varianzinflationseffekt.
\end{proof}

Dies zeigt, dass die L2-Regularisierung aus Abschnitt~7 nicht nur Overfitting reduziert,
sondern gleichzeitig MultikollinearitÃ¤t dÃ¤mpft -- ein doppelter Nutzen, der die
Ridge-Regularisierung besonders fÃ¼r DatensÃ¤tze mit korrelierten Merkmalen attraktiv macht.

% ------------------------------------------------------------
\subsection{Formale Bedeutung des VIF fÃ¼r die ModellgÃ¼te}
% ------------------------------------------------------------

\begin{theorem}[VIF und Breite des Konfidenzintervalls]
	\label{thm:vif_ci}
	Sei $\hat{w}_j$ der MLE des $j$-ten KÃ¶ffizienten der logistischen Regression mit
	...
	\begin{equation}
		...
		\label{eq:ci_width}
	\end{equation}
	Ein VIF von~10 vergrÃ¶ÃŸert die Breite des Konfidenzintervalls um den Faktor $\sqrt{10} \approx 3.16$
	gegenÃ¼ber dem unkorrelierten Fall.
\end{theorem}

\begin{proof}
	\textbf{Schritt 1: Asymptotische NormalitÃ¤t des MLE.}
	Unter RegularitÃ¤tsbedingungen ist der MLE $\hat{\vw}$ der logistischen Regression
	...
	Insbesondere konvergiert die marginale Verteilung des $j$-ten KÃ¶ffizienten:
	\[
	\frac{\hat{w}_j - w_j^*}{\hat{\sigma}_j} \xrightarrow{d} \mathcal{N}(0,1),
	\qquad \hat{\sigma}_j^2 = \bigl[\mathcal{I}(\hat{\vw})^{-1}\bigr]_{jj},
	\]
	wobei $\hat{\sigma}_j^2$ die geschÃ¤tzte asymptotische Varianz ist.
	
	\textbf{Schritt 2: ProportionalitÃ¤t $\hat{\sigma}_j^2 \propto \mathrm{VIF}_j$.}
	FÃ¼r die logistische Regression definieren wir den gewichteten Varianzinflationsfaktor durch
	$\mathrm{VIF}_j^{\mathrm{log}} = 1/(1 - R_j^{2,S})$, wobei $R_j^{2,S}$ das gewichtete
	BestimmtheitsmaÃŸ der Hilfsregression von $\vx_j$ auf $\vX_{-j}$ mit Gewichtsmatrix
	...
	Schritt-fÃ¼r-Schritt auf $\tilde{\vX} := \vS^{1/2}\vX$ (wobei $\tilde{\vX}^\T\tilde{\vX} = \vX^\T\vS\vX = \mathcal{I}(\hat{\vw})$):
	\[
	...
	\end{equation}
	Im Gegensatz zum OLS-Fall hÃ¤ngt der Nenner von den SchÃ¤tzwerten $\hat{p}_i$ ab. Die
	ProportionalitÃ¤t $\hat{\sigma}_j \propto \sqrt{\mathrm{VIF}_j^{\mathrm{log}}}$ gilt mithin
	exakt, wenn der Nenner als bekannte Konstante behandelt wird.
	
	\textbf{Schritt 3: Vergleich zweier Modelle (mit/ohne MultikollinearitÃ¤t).}
	Um den Faktor zu quantifizieren, vergleichen wir zwei Szenarien:
	\begin{itemize}
		\item \emph{Baseline (kein KollinearitÃ¤tsproblem):} $\mathrm{VIF}_j^{(0)} = 1$, also
	...
	\]
	mit Breite $\Delta_j = 2z_{0.975}\hat{\sigma}_j$. Der VerhÃ¤ltnis der Breiten betrÃ¤gt:
	\[
	\frac{\Delta_j}{\Delta_j^{(0)}} = \frac{\hat{\sigma}_j}{\hat{\sigma}_j^{(0)}} = \sqrt{\mathrm{VIF}_j}.
	\]
	FÃ¼r $\mathrm{VIF}_j = 10$ ergibt sich der Faktor $\sqrt{10} \approx 3.16$, d.\,h.\ das
	Konfidenzintervall ist mehr als dreimal so breit wie ohne MultikollinearitÃ¤t.
	
	\textbf{Schritt 4: Numerische Verifikation.}
	FÃ¼r $\mathrm{VIF}_j \in \{1,\,2,\,5,\,10,\,25\}$ ergeben sich die Faktoren:
	\[
...
\begin{corollary}[VIF und statistische Power]
	FÃ¼r den Wald-Test $H_0: w_j = 0$ gegen $H_1: w_j \neq 0$ ist die Teststatistik
	$Z_j = \hat{w}_j / \hat{\sigma}_j$. Die Power des Tests nimmt mit steigendem $\mathrm{VIF}_j$
	ab, da $\hat{\sigma}_j \propto \sqrt{\mathrm{VIF}_j}$. Konkret muss die wahre EffektgrÃ¶ÃŸe
	$|w_j|$ um den Faktor $\sqrt{\mathrm{VIF}_j}$ grÃ¶ÃŸer sein, um bei gleicher Power
	nachgewiesen werden zu kÃ¶nnen.
\end{corollary}
...
\begin{example}[Klinische MultikollinearitÃ¤t: Body-Mass-Index und KÃ¶rpergewicht]
	\label{ex:vif_bmi}
	In einer klinischen Studie zur Vorhersage von Typ-2-Diabetes ($Y = 1$: Diabetes)
	werden folgende PrÃ¤diktoren erhoben: KÃ¶rpergrÃ¶ÃŸe $x_1$ (cm), KÃ¶rpergewicht $x_2$ (kg),
	Body-Mass-Index $x_3 = x_2 / x_1^2 \times 10^4$ (kg/m$^2$), Alter $x_4$ (Jahre), 
	Blutdruck $x_5$ (mmHg), NÃ¼chternglukose $x_6$ (mg/dL).
	
	Da $x_3$ eine nichtlineare Funktion von $x_1$ und $x_2$ ist, besteht starke 
	MultikollinearitÃ¤t zwischen diesen drei Variablen.
	Typische VIF-Werte in diesem Szenario:
	
	\begin{center}
		\begin{tabular}{lcc}
			\toprule
			PrÃ¤diktor & VIF & Bewertung \\
			\midrule
			$x_1$: GrÃ¶ÃŸe (cm) & $8.73$ & Moderat \\
	...
	\textbf{Empfehlung:} Da BMI definitorisch von GrÃ¶ÃŸe und Gewicht abhÃ¤ngt, sollte entweder
	der BMI allein (als klinisch etablierte Zusammenfassung) oder GrÃ¶ÃŸe und Gewicht getrennt
	(ohne BMI) in das Modell aufgenommen werden. Die Aufnahme aller drei Variablen simultan
	fÃ¼hrt zu den beobachteten hohen VIF-Werten und macht die KÃ¶ffizienten inhaltlich nicht
	interpretierbar -- ein typisches Beispiel fÃ¼r konstruktionsbedingte MultikollinearitÃ¤t.
\end{example}

\begin{tcolorbox}[notebox, title={Kernaussagen: VIF und MultikollinearitÃ¤t}]
	\begin{enumerate}[label=\textbf{\arabic*.}]
		\item \textbf{Was misst der VIF:} Den Faktor, um den die Varianz eines RegressionskÃ¶ffizienten
		gegenÃ¼ber dem unkorrelierten Fall aufgeblasen ist -- mathematisch prÃ¤zise durch 
		$\mathrm{VIF}_j = 1/(1-R_j^2)$.
		\item \textbf{Berechnungsweg:} FÃ¼r jeden PrÃ¤diktor $x_j$: OLS-Hilfsregression auf alle
		anderen PrÃ¤diktoren, Bestimmung von $R_j^2$, Anwendung der VIF-Formel.
		\item \textbf{Klassifikation:} VIF $< 5$ akzeptabel; $5 \leq$ VIF $< 10$ moderat;
		VIF $\geq 10$ problematisch und handlungsbedÃ¼rftig.
		\item \textbf{Relevanz fÃ¼r logistische Regression:} VIF ist direkt auf die logistische
		Regression Ã¼bertragbar, da die Designmatrixstruktur identisch ist und die
		Fisher-Informationsmatrix dieselbe Invertierungsproblematik bei MultikollinearitÃ¤t aufweist.
		\item \textbf{GegenmaÃŸnahmen:} Entfernung redundanter PrÃ¤diktoren, Kombination,
		...
		\item \textbf{Warum es wichtig ist:} Hohe VIF-Werte untergraben KÃ¶ffizienteninterpretierbarkeit,
		statistische Tests und ModellstabilitÃ¤t -- drei zentrale GÃ¼tekriterien, gerade in
...
\begin{definition}[Softmax-Funktion]
  FÃ¼r Klassifikator-Scores $\vz = (z_1,\ldots,z_K)^\T \in \R^K$ ist die Softmax-Funktion:
  \begin{equation}
...
\begin{proposition}[Eigenschaften der Softmax-Funktion]
  \begin{enumerate}[label=(\roman*)]
    ...
    \item Verbindung zu Sigmoid: FÃ¼r $K=2$ mit $z_2=0$ gilt $\mathrm{softmax}(z_1, 0)_1 = \sigmoid(z_1)$.
  \end{enumerate}
\end{proposition}
...
\begin{figure}[H]
  ...
  \caption{Links: Einfluss der Softmax-Temperatur auf die EntscheidungsschÃ¤rfe.
           Rechts: Softmax-Wahrscheinlichkeit als Funktion zweier linearer PrÃ¤diktoren.}
  \label{fig:softmax}
\end{figure}
...
\begin{theorem}[KonvexitÃ¤t der kategorischen Kreuzentropie]
...
\begin{proof}
  ...
  im binÃ¤ren Fall. Da Summen konvexer Funktionen konvex sind und die Softmax-Funktion
  log-konkav ist, folgt die KonvexitÃ¤t aus denselben Argumenten wie in Theorem~\ref{thm:convexity}. 
\end{proof}
...
\begin{table}[H]
  \centering
  \caption{Konfusionsmatrix und abgeleitete Metriken fÃ¼r binÃ¤re Klassifikation.}
  \begin{tabular}{llcc}
    \toprule
    & & \multicolumn{2}{c}{PrÃ¤dizierte Klasse} \\
    \cmidrule{3-4}
...
\begin{proposition}[Probabilistische Interpretation des AUC]
  Der AUC entspricht der Wahrscheinlichkeit, dass ein zufÃ¤llig gewÃ¤hltes positives Beispiel
  einen hÃ¶heren Vorhersage-Score erhÃ¤lt als ein zufÃ¤llig gewÃ¤hltes negatives:
  \[
    \mathrm{AUC} = \Prob(\hat{p}(X^+) > \hat{p}(X^-)),
  \]
  wobei $X^+ \sim P(\cdot | Y=1)$ und $X^- \sim P(\cdot | Y=0)$ unabhÃ¤ngig sind.
\end{proposition}
...
\begin{figure}[H]
  ...
  \caption{Reliability Diagram: Logistische Regression ist typischerweise besser kalibriert
           als Random Forests (die zur Ãœberkonfidenz neigen).}
...
\subsection{VerhÃ¤ltnis zu Support Vector Machines}
...
\begin{proposition}[Verlustfunktionsvergleich: LR vs. SVM]
  ...
  Ã¼berall und berÃ¼cksichtigt alle Punkte, wÃ¤hrend die SVM nur StÃ¼tzvektoren in der
  NÃ¤che der Grenze verwendet.
\end{proposition}

\subsection{Naive Bayes als generatives GegenstÃ¼ck}

\begin{proposition}[Ã„quivalenz unter Gauss-Annahme]
  Sei $P(\vx|Y=k) = \mathcal{N}(\bm{\mu}_k, \Sigma)$ fÃ¼r $k \in \{0,1\}$ mit gleicher
  Kovarianzmatrix $\Sigma$. Dann stimmt der Naive-Bayes-Klassifikator mit gleichen Vorwahrs-
  wahrscheinlichkeiten asympotisch mit der logistischen Regression Ã¼berein, mit Gewichten:
  \[
...
\begin{figure}[H]
  ...
  \caption{Links: Alle Eigenwerte der Hesse-Matrix sind nichtnegativ, was KonvexitÃ¤t beweist.
           Rechts: 2D-Schnitt der Verlustlandschaft zeigt die typische konvexe SchÃ¼ssel.}
  \label{fig:convexity}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Logistische Regression mit polynomialen Feature Maps. HÃ¶here Grade ermÃ¶glichen
...
\begin{figure}[H]
  ...
  \caption{Anwendungsbeispiel: kardiovaskuales Risiko. Links: KÃ¶ffizienzen der logistischen
           Regression fÃ¼r Alter, BMI, Cholesterin und Raucherstatus. Mitte: Verteilung der
...
\begin{figure}[H]
  ...
  \caption{Links: Posteriori-Dichte des Gewichts $w$ mit MAP-SchÃ¤tzer und 95\%-KredibilitÃ¤ts-
...
markiert einen Wendepunkt: Die VerfÃ¼gbarkeit grosser Sprachmodelle, die Verbreitung von
Federated-Learning-Infrastruktur, der gesellschaftliche Druck nach erklÃ¤rbarer KI und die
Entstehung quantenresistenter und quantengestÃ¼tzter Lernmethoden erÃ¶ffnen vÃ¶llig neÃ¼
Perspektiven auch fÃ¼r ein klassisches Verfahren wie die logistische Regression.

\subsection{Federated Learning und datenschutzerhaltende Klassifikation}

\subsubsection{Konzept und Herausforderungen}

Federated Learning (FL), eingefÃ¼hrt durch Google 2017 \citep{mcmahan2017communication}, ist
ein verteiltes Trainingsparadigma, bei dem Modelle lokal auf den GerÃ¤ten der Nutzer trainiert
werden und nur Gradientenaktualisierungen -- niemals rohe Daten -- an einen zentralen Server
Ã¼bertragen werden. Die logistische Regression ist hierfÃ¼r ein natÃ¼rliches erstes Modell:

\begin{algorithm}[H]
  \caption{FedAvg fÃ¼r logistische Regression}
  \begin{algorithmic}[1]
    \Require $K$ Klienten mit lokalen DatensÃ¤tzen $\mathcal{D}_k$, globales Modell $\vw^{(0)}$
...
Differentielle Privatheit (DP), eingefÃ¼hrt von Dwork et al.~\citep{dwork2006calibrating},
bietet mathematische Garantien fÃ¼r den Datenschutz im maschinellen Lernen. Ein Algorithmus
$\mathcal{A}$ ist $(\varepsilon, \delta)$-differenziell privat, wenn fÃ¼r alle DatensÃ¤tze
...
\begin{equation}
  ...
  \quad \text{fÃ¼r alle messbaren } S.
  \label{eq:dp}
\end{equation}

FÃ¼r die logistische Regression wird Privatheit durch den \emph{Gaussian-Mechanismus} erreicht:
Zum Gradienten wird Rauschen $\bm{n} \sim \mathcal{N}(0, \sigma^2 \mathbf{I})$ addiert, wobei
$\sigma \geq C\sqrt{2\ln(1.25/\delta)}/\varepsilon$ und $C$ die $\ell_2$-SensitivitÃ¤t des
...
\begin{theorem}[Privatheit des DP-SGD]
  ...
  Batch-GrÃ¶sse $m$ und $T$ Iterationen angewandt. Dann ist der Algorithmus
...
In der medizinischen Diagnostik erlaubt Federated Learning, Klassifikatoren Ã¼ber Krankenhausdaten
zu trainieren, ohne Patientendaten die Institutsgrenzen Ã¼berschreiten zu lassen. Das MELLODDY-
Projekt der EuropÃ¤ischen Union trainiert Modelle Ã¼ber Pharmaunternehmen hinweg, ohne propriatÃ¤re
MolekÃ¼ldaten offenzulegen. Im Bereich mobiler EndgerÃ¤te wird Federated Logistic Regression
...
analytisch nicht zugÃ¤nglich ist, werden Approximationsverfahren eingesetzt:

\begin{itemize}
  \item \textbf{Laplace-Approximation:} Approximation der Posteriori durch eine Gauss-Vertei\-lung
    zentriert am MAP-SchÃ¤tzer mit Kovarianzmatrix gleich der negativen inversen Hesse-Matrix.
...
\subsubsection{PrÃ¤diktive Unsicherheit}

Die prÃ¤diktive Unsicherheit wird durch Marginalisierung Ã¼ber die Posteriori ausgedrÃ¼ckt:
...
Dieses Integral ist analytisch nicht lÃ¶sbar, kann aber durch Monte-Carlo-Integration
approximiert werden:
\[
...
\subsubsection{Bedeutung fÃ¼r HochrisikÃ¶ntscheidungen}

PrÃ¤diktive Unsicherheit ist in HochrisikodomÃ¤nen unverzichtbar: Ein medizinisches
Entscheidungssystem muss nicht nur eine Vorhersage liefern, sondern auch die Konfidenz dieser
Vorhersage quantifizieren. Bayesianische Modelle liefern natÃ¼rliche KredibilitÃ¤tsintervalle
fÃ¼r individÃ¼lle Vorhersagen -- ein entscheidender Vorteil gegenÃ¼ber freqÃ¼ntistischen AnsÃ¤tzen.

\subsection{ErklÃ¤rbare KÃ¼nstliche Intelligenz (XAI) und logistische Regression}
...
Die KÃ¶ffizienten $w_j$ haben eine direkte semantische Bedeutung als log-lineare Odds-Ratios.
...
\begin{tcolorbox}[notebox, title={Interpretierbarkeitsgarantie}]
  FÃ¼r die logistische Regression gilt: Der Beitrag des Merkmals $x_j$ zur Vorhersage
  ist additiv und proportional zu $w_j x_j$. Dies ermÃ¶glicht eine vollstÃ¤ndige, modellgetreÃ¼
  ErklÃ¤rung jeder Einzelvorhersage -- ohne Approximation.
\end{tcolorbox}
...
additive BeitrÃ¤ge einzelner Merkmale. FÃ¼r lineare Modelle fallen SHAP-Werte mit den
einfachen Gewichtungs-ErklÃ¤rungen zusammen:
...
was die logistische Regression zu einem perfekten Grundmodell fÃ¼r XAI-Verfahren macht.
...
Pearl~\citep{pearl2009causality}) ist ein aktives Forschungsfeld. Das \emph{do-kalkÃ¼l}
...
doppelt robusten SchÃ¤tzern fÃ¼r kausale Behandlungseffekte.

\subsection{Hochdimensionale Statistik und maschinelles Lernen}

\subsubsection{Hochdimensionales Regime: $d \gg n$}

In Genomik, Proteomik und neuronaler Bildgebung Ã¼berschreitet die Merkmalsdimension $d$
die Stichprobenzahl $n$ hÃ¤ufig um GrÃ¶ssenordnungen. Das klassische MLE-Verfahren versagt
in diesem Regime -- die Hesse-Matrix ist singulaar und der SchÃ¤tzer nicht eindeutig. Dies
motiviert:

\begin{itemize}
  \item \textbf{LASSO-Logit} ($p > n$ Regime): Konsistente Variablenselektion unter
    sogenannten \emph{Restricted EigenvalÃ¼ Conditions}~\citep{bickel2009simultaneous}.
  \item \textbf{Ridge-Logit}: Konsistente SchÃ¤tzung auch fÃ¼r $d \to \infty$ bei angemessener
    RegularisierungsstÃ¤rke $\lambda_2 = \Theta(\sqrt{d/n})$.
  \item \textbf{Group Lasso}: Simultane Selektion von Merkmalgruppen (z.B. One-Hot-kodierte
    kategoriale Variablen).
\end{itemize}

\subsubsection{Doppelt robuste SchÃ¤tzung}

\begin{theorem}[Doppelte Robustheit]
  Sei $\hat{e}(\vx)$ ein SchÃ¤tzer der Behandlungsneigung (propensity score) und
  $\hat{\mu}_k(\vx)$ ein SchÃ¤tzer der bedingten Erwartung des Outcomes. Der \emph{doppelt
  robuste} SchÃ¤tzer fÃ¼r den durchschnittlichen Behandlungseffekt (ATE) ist:
  \[
  ...
  \]
  Dieser SchÃ¤tzer ist konsistent, wenn \emph{entweder} $\hat{e}$ \emph{oder} $\hat{\mu}$
  konsistent ist (aber nicht notwendigerweise beide). Logistische Regression wird regelmÃ¤ssig
  fÃ¼r $\hat{e}$ eingesetzt.
\end{theorem}

\subsection{Logistische Regression in der PrÃ¤zisionsmedizin}
...
gewichteten EffektgrÃ¶ssen aus GWAS-Studien sind. Im Jahr 2025 werden PRS fÃ¼r Ã¼ber 500
...
Maschinelles Lernen auf elektronischen Krankenakten (EHR) ist ein SchlÃ¼ssel-Anwendungs\-feld.
Logistische Regression dient dabei als:
\begin{enumerate}[label=(\roman*)]
  \item Prognosemodell fÃ¼r Krankenhauswiederaufnahmen innerhalb von 30 Tagen,
  \item Vorhersagemodell fÃ¼r arzneimittelinduzierte Nebenwirkungen,
  \item FrÃ¼herkennung intensivmedizinischer Verschlechterungen (FrÃ¼hwarnsysteme).
\end{enumerate}

AktÃ¼lle Forschung (2024--2025) fokussiert auf \emph{time-varying} logistische Regressionsmodelle,
die zeitliche VerlaufsverÃ¤nderungen von Laborparametern modellieren kÃ¶nnen.
...
klimatischer Extremereignisse (Hitzewellen, Ãœberschwemmungen, DÃ¼rre) als Funktion
atmosphÃ¤rischer Zustandsvariablen zu modellieren. Das Verfahren ist hier besonders
attraktiv, weil:
\begin{itemize}
  \item Die KÃ¶ffizienten physikalisch interpretierbar sind (z.B. Einfluss des MeeresoberflÃ¤chen-
    temperaturgradients auf ZyklonintensitÃ¤t),
  \item Bayes'sche Erweiterungen natÃ¼rliche Unsicherheitsquantifizierung bieten,
  \item Regularisierung Overfitting verhindert, wenn historische Extremereignisse selten sind.
\end{itemize}

\subsubsection{SatellitengestÃ¼rzte Fernerkundung}

Logistische Regression und ihre multinomialen Erweiterungen werden zur Klassifikation von
Landbedeckungstypen aus hochauflÃ¶senden Satellitenbildern eingesetzt. In Kombination mit
convolutional feature extractors (CNN-LR-Hybride) erzielen sie Genauigkeiten Ã¼ber $95\%$
bei der Unterscheidung von Wald, Acker, Siedlung und WasserflÃ¤che.

\subsection{Finanzmarkt und Kreditrisikomodellierung}

\subsubsection{Basel III/IV und regulatorische Anforderungen}

Der Basler Ausschuss fÃ¼r Bankenaufsicht (Basel IV, Implementierung 2025) verlangt explizit
interpretierbare und validierbare Kreditrisikomodelle. Logistische Regression ist daher das
gesetzlich bevorzugte Verfahren fÃ¼r Probability-of-Default (PD)-Modelle in der internen
...
wobei LGR = Loan-to-GrossRevenÃ¼, DAR = Debt-Asset-Ratio und ICR = Interest-Coverage-Ratio.

\subsubsection{Echtzeit-Betrugserkennungssysteme}

In HochfreqÃ¼nz-Zahlungsnetzwerken (Visa, Mastercard) werden logistische Regressionsmodelle
eingesetzt, die pro Millisekunde mehrere tausend Transaktionen klassifizieren mÃ¼ssen.
Die LinearitÃ¤t des PrÃ¤diktors macht Inferenz extrem schnell ($O(d)$ Floating-Point-
Operationen), was bei Deep-Learning-Alternativen nicht erreichbar ist. FÃ¼r 2025 wird
hÃ¤ufig ein zweistufiges Ensemble eingesetzt: logistische Regression als Schnell-Filter,
gefolgt von einem komplexeren Modell fÃ¼r GrenzfÃ¤lle.
...
wobei $\bm{h} \in \R^{d_{\text{model}}}$ die KontextreprÃ¤sentation und $V$ das Vokabular
...
Das im Jahr 2022 eingefÃ¼hrte \emph{Classifier-Free Guidance} fÃ¼r Diffusionsmodelle
...
wird in Modellen wie Claude, GPT-4o und Gemini zur SteÃ¼rung der AusgabediversitÃ¤t verwendet.
...
ReprÃ¤sentationen von LLMs trainiert werden, um zu untersuchen, welche linguistischen
...
fÃ¼r diese mechanistische Interpretierbarkeitsforschung.
...
In sicherheitskritischen autonomen Systemen (Fahrzeuge nach ISO 26262 ASIL-D) mÃ¼ssen
...
\begin{itemize}
  ...
  \item Formale Verifikation durch vollstÃ¤ndige Modell-Transparenz,
  \item Integration in Model-Based Design (AUTOSAR, Simulink),
  \item Explizite Wahrscheinlichkeitsausgabe fÃ¼r Risikobewertung.
\end{itemize}

\subsubsection{Probabilistische ZustandssschÃ¤tzung}
...
gegeben einem Systemzustand wird durch logistische Regression geschÃ¤tzt.

\subsection{Quantenmaschinelles Lernen}

\subsubsection{Quantenlogistische Regression}

Quantencomputer bieten theoretische Speedups fÃ¼r bestimmte lineare-algebraische Operationen.
Der HHL-Algorithmus~\citep{harrow2009quantum} lÃ¶st lineare Gleichungssysteme in
$O(\log n)$ Zeit (gegenÃ¼ber $O(n^3)$ klassisch), was -- wenn auf die Newton-Iteration
angewendet -- eine exponentielle Beschleunigung der logistischen Regression ermÃ¶glicht.
...
(QSVM) untersucht. EinschrÃ¤nkungen bestehen durch:
\begin{itemize}
  \item Quantum-DekohÃ¤renz und NISQ-BeschrÃ¤nkungen (2025: $\sim$1000 qubits),
  \item Aufwand fÃ¼r QuantenzustandsprÃ¤paration (Ladezeit der Daten),
  \item Noise und Fehlerkorrektur.
\end{itemize}

\subsubsection{Post-Quantum-Sicherheit fÃ¼r ML-Inferenz}

Sicherheitsaspekte sind ein weiteres Forschungsfeld: Wie kÃ¶nnen logistische
Regressionsmodelle so geschÃ¼tzt werden, dass Angreifer mit Quantencomputern keine
RÃ¼ckschlÃ¼sse auf Trainingsdaten ziehen kÃ¶nnen? Homomorphe VerschlÃ¼sselung (z.B. SEAL,
OpenFHE) erlaubt Inferenz auf verschlÃ¼sselten Eingabedaten, ohne den Klartext zu offenbaren.

\subsection{Mathematische Ausblicke und offene Forschungsfragen}

\subsubsection{Statistische Lerntheorie}

\begin{theorem}[Generalisierungsfehler-Schranke fÃ¼r logistische Regression]
  Sei $\mathcal{F} = \{\vx \mapsto \sigmoid(\vw^\T\vx+b) : \|\vw\|_2 \leq B\}$. Mit
  Wahrscheinlichkeit mindestens $1-\delta$ gilt fÃ¼r den wahren RisikoschÃ¤tzer:
  \begin{equation}
...
\begin{proof}
  Die Schranke folgt aus der Rademacher-KomplexitÃ¤t der logistischen Funktionenklasse
  $\mathcal{F}$ kombiniert mit McDiarmids Ungleichung. Die Rademacher-KomplexitÃ¤t von
  $\mathcal{F}$ ist beschrÃ¤nkt durch $B\|\vx\|_{\max}/\sqrt{n}$ (Massart-Lemma). 
\end{proof}

\subsubsection{Implizite Regularisierung durch Gradientenabstieg}

Ein tiefes Ergebnis der jÃ¼ngsten Forschung (2018--2025) ist, dass Gradientenabstieg fÃ¼r
...
\begin{theorem}[Implizite Regularisierung nach Soudry et al.~\citep{soudry2018implicit}]
  Sei der Datensatz linear separierbar. Dann gilt fÃ¼r den Gradientenabstieg-Pfad:
  \[
...
Dies erklÃ¤rt, warum unregularisierte logistische Regression auf separierbaren Daten oft
...
Testperformance von Sprachmodellen mit ModellgrÃ¶sse, Datenmenge und Rechenaufwand skaliert.
...
Online-Lernalgorithmen aktualisieren logistische Regressionsmodelle mit jedem neÃ¼n
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Finanzwesen & Kreditscoring (PD) & Basel-IV-KonformitÃ¤t, Fairness \\
    Finanzwesen & Betrugserkennung & Echtzeit ($<$1ms), ErklÃ¤rbarkeit \\
    ...
    Genomik & Genotyp-PhÃ¤notyp-Assoz. & GWAS-Meta-Analysen \\
    Klimawissenschaft & Extremwetter-Vorhersage & Physikalische Interpretierbarkeit \\
    Pharmakologie & NebenwirkungsprÃ¤diktion & FDA Drug Safety Reporting \\
    E-Commerce & Click-Through-Rate (CTR) & Online-Learning, FTRL \\
    Industrie 4.0 & QualitÃ¤tskontrolle & Eingebettete Systeme, AUTOSAR \\
    \bottomrule
  \end{tabular}
...
Logistische Regressionsmodelle kÃ¶nnen diskriminierende Entscheidungen aus voreingenommenen
Trainingsdaten lernen. Formale Fairness-Kriterien sind u.a.:

\begin{itemize}
  \item \textbf{Demografische ParitÃ¤t:} $\Prob(\hat{Y}=1|A=0) = \Prob(\hat{Y}=1|A=1)$, wobei $A$ ein schutzwÃ¼rdiges Attribut (z.B. Geschlecht, EthnizitÃ¤t) ist.
  \item \textbf{Equal Opportunity (Hardt et al.):} Gleiche TrÃ¼-Positive-Raten zwischen Gruppen.
  \item \textbf{KalibrierungspaaritÃ¤t:} Gleiche Kalibrierung der Wahrscheinlichkeiten Ã¼ber Gruppen.
\end{itemize}

Fairness-bewusste logistische Regression lÃ¶st ein Optimierungsproblem mit Fairness-Nebenbedingungen:
\begin{equation}
...
und medizinische Klassifikationssysteme als Hochrisiko-KI-Anwendungen (Anhang III). FÃ¼r diese
gilt:
\begin{itemize}
  \item Dokumentationspflicht fÃ¼r Modellarchitektur, Trainingsverfahren und Datengrundlage,
  \item Post-Deployment-Monitoring und regelmÃ¤issige Validierung,
  \item Recht auf ErklÃ¤rung fÃ¼r betroffene IndividÃ¼n,
  \item KonformitÃ¤tsbewertung durch benannte Stellen.
\end{itemize}

Die logistische Regression ist hier als Modellklasse besonders gut positioniert, da ihre
vollstÃ¤ndige ErklÃ¤rbarkeit (Formel~\eqref{eq:log_odds}) eine modellgetreÃ¼ ErklÃ¤rung
jeder Entscheidung ermÃ¶glicht -- ohne NacherklÃ¤rungsverfahren wie LIME oder SHAP.

\subsection{ForschungslÃ¼cken und offene Probleme (Stand 2025)}

\begin{enumerate}[label=\textbf{F\arabic*.}]
  \item \textbf{Skalengesetze fÃ¼r logistische Regression:} Wie verhÃ¤lt sich die
    Generalisierung logistischer Regressionsmodelle, die auf Merkmalen vortrainierter
    Netze (Foundation Models) operieren, in AbhÃ¤ngigkeit von der GrÃ¶sse des Grundmodells?

  \item \textbf{Online-Fairness:} Wie kÃ¶nnen Fairness-Garantien in inkrementellen
  ...
  \item \textbf{Quantenvorteil bei logistischer Regression:} Unter welchen Bedingungen
    ermÃ¶glicht Quantencomputing einen praktischen Speedup fÃ¼r logistische Regression bei
    realen DatensÃ¤tzen? Die AbhiÃ¤ngigkeit von QuantenzustandsprÃ¤paration schrÃ¤nkt
    derzeit den theoretischen Speedup ein.

  \item \textbf{KausalitÃ¤t und Intervenierbarkeit:} Wann sind KÃ¶ffizienten logistischer
    Regressionsmodelle als kausale EffektgrÃ¶ssen interpretierbar? Welche Annahmen sind
    notwendig und Ã¼berprufbar?

  \item \textbf{Robustheit gegenÃ¼ber adversarialen Angriffen:} Logistische Regression ist
    anfÃ¤llig fÃ¼r adversariale Perturbationen. Formale Robustheitszertifikate (certified
    defenses) fÃ¼r logistische Regressionsmodelle in sicherheitskritischen Systemen sind
    ein offenes Problem.

  \item \textbf{Federated Learning unter HeterogenitÃ¤t:} Wie konvergiert FedAvg fÃ¼r
    logistische Regression unter stark heterogenen (non-i.i.d.) Klientenverteilungen?
    NeÃ¼re Ergebnisse (FedProx, Scaffold, MOON) zeigen Verbesserungen, aber vollstÃ¤ndige
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Mathematische Fundierung:} Die logistische Regression ist der kanonische
    GLM fÃ¼r binÃ¤re Daten, fest verankert in der Theorie der Exponentialfamilien.
  \item \textbf{KonvexitÃ¤t:} Die binÃ¤re und kategorische Kreuzentropie sind konvex,
    was globale Konvergenzgarantien fÃ¼r Gradientenverfahren liefert.
  \item \textbf{Reiche Optimierungslandschaft:} Von einfachem Gradientenabstieg Ã¼ber
    Newton-IRLS bis zu L-BFGS und Adam existiert eine Hierarchie effizienter Verfahren.
  \item \textbf{Interpretierbarkeit:} KÃ¶ffizienten sind log-lineare Odds-Ratios mit
  ...
  \item \textbf{Weitreichende Zukunftsperspektiven:} Federated Learning, differentielle
    Privatheit, Bayes'sche Erweiterungen, Quantencomputing, erklÃ¤rbare KI und PrÃ¤zisionsmedizin
    definieren neÃ¼ Anwendungsfelder, in denen logistische Regression als interpretierbare,
    effiziente Kernmethode weiterhin eine SchlÃ¼sselrolle spielen wird.
\end{enumerate}
...
theorie und konvexer Optimierung macht sie zum idealen Ausgangspunkt fÃ¼r das VerstÃ¤ndnis
...
Cox, D.~R. (1958).
\newblock The regression analysis of binary seqÃ¼nces.
...
McMahan, B., Moore, E., Ramage, D., Hampson, S., and AgÃ¼ra y Arcas, B. (2017).
...
Verhulst, P.-F. (1845).
\newblock Recherches mathematiqÃ¼s sur la loi d'accroissement de la population.
