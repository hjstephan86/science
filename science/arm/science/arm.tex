...
% ============================================================
% ERWEITERUNGSKAPITEL 1: Subgraph Algorithmus fÃ¼r WCET-Analyse
% ============================================================
\section{Subgraph Algorithmus fÃ¼r WCET-Analyse}\label{sec:wcet_subgraph}

Dieses Kapitel entwickelt eine vollstÃ¤ndige formale Theorie fÃ¼r den
...
insbesondere dem neu eingefÃ¼hrten Cortex-HX Safety-Island.
Alle zentralen Aussagen werden durch vollstÃ¤ndige Beweise gestÃ¼tzt;
die Ergebnisse werden durch fÃ¼nf Matplotlib-Visualisierungen illustriert.
...
bis ASIL~D zertifiziert werden sollen, mÃ¼ssen WCET-Schranken formal
...
entweder unter exponentieller KomplexitÃ¤t im schlimmsten Fall oder
erfordern kommerzielle Werkzeuge. Der Subgraph Algorithmus von
Epp~\cite{epp2026c} bietet eine neuartige Alternative: Er reprÃ¤sentiert
den \emph{Control-Flow-Graphen} (CFG) eines Programms als Adjazenzmatrix,
berechnet fÃ¼r jede Spalte eine injektive Signatur und identifiziert
in $\mathcal{O}(n^3)$ alle zyklischen Teilgraphen -- die
primÃ¤ren Quellen hoher WCET -- durch Rotationsvergleich.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{plot_w1_cfg_wcet.pdf}
  \caption{%
    Control-Flow-Graph (CFG) als Graph-ReprÃ¤sentation fÃ¼r den Subgraph
    Algorithmus. Rot hervorgehoben: der durch den Algorithmus identifizierte
    WCET-Pfad (kÃ¼rzester Weg durch alle SchleifenkÃ¶pfe und kritischen Zweige).
...
\texttt{Exit}-Knoten als gerichteter Graph reprÃ¤sentiert wird.
...
\begin{itemize}
  \item $V = \{b_1,\ldots,b_n\}$ die Menge der \emph{BasisblÃ¶cke} ist
        (maximal lange Folgen von Anweisungen ohne SprÃ¼nge),
  \item $E \subseteq V \times V$ die Menge der Kontrollflusskanten ist,
        und $(b_i, b_j) \in E$ genau dann gilt, wenn $b_j$ unmittelbar
        nach $b_i$ ausgefÃ¼hrt werden kann,
...
\]
wobei $t(\pi) = \sum_{b \in \pi} \mathrm{ET}(b)$ die AusfÃ¼hrungszeit
des Pfades ist und $\mathrm{ET}(b)$ die AusfÃ¼hrungszeit von
Basisblock $b$. Der zugehÃ¶rige \emph{WCET-Subgraph}
$G^* = (V^*, E^*)$ enthÃ¤lt genau die auf $\pi^*$ liegenden
Knoten und Kanten.
\end{definition}
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    (roter Subgraph) bilden eine Teilmenge der vollstÃ¤ndigen
    CFG-Adjazenzmatrix.}
  \label{fig:adjazenz}
\end{figure}
...
\begin{definition}[Injektive Signatur fÃ¼r CFG-Spalten]\label{def:cfg_sig}
Sei $A \in \{0,1\}^{n \times n}$ die Adjazenzmatrix von $G_P$.
FÃ¼r Spalte $j \in \{0,\ldots,n-1\}$ ist die injektive Signatur
\[
...
\begin{lemma}[InjektivitÃ¤t der CFG-Signatur]\label{lem:cfg_injektiv}
...
da die BinÃ¤rdarstellung eindeutig ist. Der Term $j \cdot 2^n$ ist identisch
und hebt sich nicht aus.

\emph{Fall 2: $j \neq j'$.}
Dann ist $(j-j') \cdot 2^n \neq 0$. Der Differenz-Term $|j-j'| \cdot 2^n \geq 2^n$
Ã¼bersteigt den maximalen Unterschied aus dem Spaltenvektor-Term
$\sum c_i \cdot 2^i < 2^n$. Daher ist $\sigma_j \neq \sigma_{j'}$.

In beiden FÃ¤llen gilt $\sigma(c,j) \neq \sigma(c',j')$.
\end{proof}
...
FÃ¼r zwei Signatur-Arrays $\Sigma_1, \Sigma_2$ ist
$\mathrm{LCS}(\Sigma_1, \Sigma_2)$ die LÃ¤nge der lÃ¤ngsten gemeinsamen
Teilsequenz (ohne Umsortierung). Die \emph{Subgraph-Beziehung}
$G_P \subseteq G_Q$ wird erklÃ¤rt durch:
...
Sei $G_P = (V, E)$ ein CFG mit $|V| = n$ BasisblÃ¶cken.
...
FÃ¼r jede der $n$ Spalten der $n \times n$-Adjazenzmatrix $A$ wird
$\sigma_j$ in $\mathcal{O}(n)$ Zeit berechnet. Gesamt:
$\mathcal{O}(n^2)$.

\textit{Schritt 2: Rotationsschleife.}
FÃ¼r jede der $n$ Rotationen $k \in \{0,\ldots,n-1\}$ wird das
rotierte Signatur-Array in $\mathcal{O}(1)$ erzeugt (zirkulÃ¤rer Index).

\textit{Schritt 3: LCS-Berechnung.}
Der LCS zweier Sequenzen der LÃ¤nge $n$ hat bekannte KomplexitÃ¤t
$\mathcal{O}(n^2)$ per dynamischer Programmierung~\cite{cormen2022}.
Ãœber alle $n$ Rotationen: $n \cdot \mathcal{O}(n^2) = \mathcal{O}(n^3)$.
...
zu Subgraphen bijektiv (fÃ¼r Zyklen mit $|V_Z| \leq n$).
Jeder Zyklus wird durch mindestens eine Rotation erkannt
(da seine BasisblÃ¶cke eine Teilsequenz von $\Sigma(G_P)$ bilden).

\textit{Platz:} Die Adjazenzmatrix benÃ¶tigt $\mathcal{O}(n^2)$;
das Signatur-Array $\mathcal{O}(n)$; die LCS-Tabelle $\mathcal{O}(n^2)$.
Gesamtplatzbedarf: $\mathcal{O}(n^2)$.
\end{proof}

\begin{corollary}[Korrektheit fÃ¼r Back-Edge-Zyklen]\label{cor:backedge}
...
\end{corollary}
\begin{proof}
Der Zyklus $Z_{uv}$ enthÃ¤lt die Knoten auf dem einzigartigen
...
fÃ¼r $|V_{Z_{uv}}| \geq 2$ gilt, ist die Subgraph-Bedingung erfÃ¼llt.
\end{proof}
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    Unten links: KomplexitÃ¤tsvergleich O(nÂ³) vs. O(n!).
    Unten rechts: Reduktion des WCET-Spreads~$\delta$
    durch Subgraph-gestÃ¼tzte WCET-Analyse aller Cortex-Kerne.}
...
\end{theorem}
\begin{proof}
\textit{Schritt 1 (VollstÃ¤ndigkeit der Zyklusidentifikation).}
...
Die WCET-Schranke summiert fÃ¼r \emph{alle} Zyklen ihre maximale
Iterationszahl $N_\ell$. Da $t(\pi^*) \leq \sum_\ell N_\ell \cdot \mathrm{ET}(Z_\ell)$
fÃ¼r jeden realen Pfad $\pi^*$ gilt (der hÃ¶chstens $N_\ell$ Iterationen
je Schleife $\ell$ durchlÃ¤uft), ist die Schranke monoton Ã¼berschÃ¤tzend.
...
\begin{theorem}[Overapproximations-QualitÃ¤t]\label{thm:approx_quality}
Sei $r := \mathrm{WCET}(P) / \mathrm{WCET}_\mathrm{real}(P)$
der Overapproximations-Faktor. FÃ¼r den Subgraph Algorithmus gilt
unter unabhÃ¤ngigen AusfÃ¼hrungszeiten $\mathrm{ET}(b)$:
\[
...
Ã¼ber AusfÃ¼hrungszeiten bezeichnen.
\end{theorem}
\begin{proof}
Sei $T_\ell = N_\ell \cdot \mathrm{ET}(Z_\ell)$ die Zyklus-AusfÃ¼hrungszeit.
Nach Jensen-Ungleichung gilt fÃ¼r die konvexe Funktion $f(x)=x$:
$\mathbb{E}[T_\ell] \leq N_\ell \cdot \mathbb{E}[\mathrm{ET}(Z_\ell)]$.
Die Differenz zum realen WCET ist durch die zweite Ordnung der
Taylor-Entwicklung von $\mathbb{E}[\max T_\ell]$ beschrÃ¤nkt,
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    (Ratio WCET/BCET) je Methode fÃ¼r drei Cortex-Kerne.
    Der Subgraph Algorithmus erzielt mit $r \leq 1{,}06$ die
    geringste Overapproximation. Rechts: Laufzeit-Skalierung
    $\mathcal{O}(n^3)$ des Subgraph Algorithmus gegenÃ¼ber
    $\mathcal{O}(n^4)$ fÃ¼r klassisches IPET.}
  \label{fig:wcet_methoden}
\end{figure}

\cref{fig:wcet_methoden} belegt quantitativ die Ã¼berlegene
Effizienz des Subgraph Algorithmus: Bei $n=10\,000$ BasisblÃ¶cken
benÃ¶tigt er 3{,}1~s gegenÃ¼ber 35~s (IPET) und 18~s (aiT).
Der Overapproximations-Faktor von $r \leq 1{,}06$ fÃ¼r den
...
Die CFG-Klasse $\mathcal{G}_\mathrm{SI}$ fÃ¼r den HX-Safety-Island
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Maximaler Zyklustiefe $\max_\ell |Z_\ell| \leq 32$ BasisblÃ¶cke,
...
AbzÃ¼glich Pipeline-Overhead ($d_\mathrm{SI} = 3$, also $\leq 20\,000$
Overhead-Zyklen) verbleiben $180\,000$ Nutzzyklen.
Mit \cref{def:wcet_bound} ist die Bedingung hinreichend fÃ¼r
$\mathrm{WCET}(P) \leq T_D$.
\end{proof}

\begin{corollary}[Zertifizierungsaussage fÃ¼r ISO~26262]\label{cor:iso_cert}
...
\[
  ...
  < T_D = 100\,\mu\text{s}.
\]
Der Sicherheitsabstand betrÃ¤gt mindestens $5{,}7\,\mu\text{s}$
und ist konservativ fÃ¼r den ASIL-D-Nachweis.
\end{corollary}
\begin{proof}
...
FÃ¼r Systeme mit shared Caches und DRAM-Interferenz ist eine
...
der AusfÃ¼hrungszeitverteilung $F_T$:
\[
...
Sei $T = \max_{1 \leq k \leq n} T_k$ das Maximum von $n$ unabhÃ¤ngigen,
identisch verteilten AusfÃ¼hrungszeiten $T_k$.
Dann konvergiert die Verteilung von $T$ fÃ¼r $n \to \infty$
...
Maxima-DomÃ¤ne der Gumbel-Verteilung liegen (was fÃ¼r
leicht-schwÃ¤nzige Verteilungen wie Normalverteilung oder
Exponentialverteilung gilt), konvergiert das Sample-Maximum.
Da AusfÃ¼hrungszeiten typischerweise durch ein Hardware-Plateau
beschrÃ¤nkt sind (maximale Pipeline-Tiefe begrenzt den WCET),
sind $T_k$ leicht-schwÃ¤nzig.
\end{proof}

\begin{proposition}[pWCET fÃ¼r HX-Safety-Island]\label{prop:pwcet_hx}
FÃ¼r den HX-Safety-Island bei $f=2\,\mathrm{GHz}$ mit gemessenen
...
FÃ¼r $p=0{,}999$: $-\ln(0{,}999) \approx 0{,}001001$;
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    Gumbel-Extremwertverteilungen fÃ¼r Cortex-M4, Cortex-R52 und
...
\cref{fig:pwcet} bestÃ¤tigt \cref{prop:pwcet_hx} visuell und
zeigt den Vorteil des HX-Safety-Islands gegenÃ¼ber M4 und R52:
Der kompaktere Gumbel-Schwanz (kleines $\beta = 2$) fÃ¼hrt zu
deutlich geringeren Hochquantilen, was fÃ¼r ISO~26262 ASIL-D
entscheidend ist.

%--------------------------------------------------------------------
\subsection{Formale KomplexitÃ¤ts-Analyse und Vergleich}\label{ssec:complexity}
%--------------------------------------------------------------------

\begin{theorem}[Ãœberlegenheit des Subgraph Algorithmus]\label{thm:superiority}
Sei $n$ die Anzahl der BasisblÃ¶cke eines CFG. Dann gilt
fÃ¼r die asymptotische KomplexitÃ¤t:
\begin{align*}
...
\end{theorem}
\begin{proof}
\emph{FÃ¼r IPET:} Das lineare Programm hat $\mathcal{O}(n^2)$
Variablen (Kanten des CFG) und $\mathcal{O}(n)$ Constraints.
Simplex-basierte LÃ¶sung hat Worst-Case $\mathcal{O}(2^n)$,
aber praktisch $\mathcal{O}(n^2 \cdot m)$ mit $m = \mathcal{O}(n^2)$
Kanten, also $\mathcal{O}(n^4)$~\cite{li1995}.

\emph{FÃ¼r aiT:} Abstrakte Interpretation traversiert alle $n$ Knoten;
die Fixpunktiteration konvergiert in $\mathcal{O}(n^2)$ Schritten
fÃ¼r azyklische Teile; Schleifen erfordern $\mathcal{O}(n \log n)$
Widenings~\cite{cousot1977}. Gesamt: $\mathcal{O}(n^3 \log n)$.

\emph{FÃ¼r Subgraph Algorithmus:} Aus \cref{thm:wcet_complexity}
bekannt: $\mathcal{O}(n^3)$.

Da $n^3 < n^3 \log n < n^4$ fÃ¼r $n \geq 3$,
...
\begin{enumerate}[label=\textbf{S\arabic*.}]
  ...
  \item Die daraus abgeleitete WCET-Schranke ist eine korrekte
        ...
        Faktor $r \leq 1{,}06$ fÃ¼r den HX-Safety-Island (\cref{cor:iso_cert}).
  \item FÃ¼r EtherCAT-Deadline von $100\,\mu$s bei $f=2\,\mathrm{GHz}$
  ...
  \item Der Subgraph Algorithmus ist asymptotisch allen bekannten
        WCET-Verfahren Ã¼berlegen (\cref{thm:superiority}).
\end{enumerate}
...
\begin{figure}[H]
  ...
  \caption{%
    VollstÃ¤ndiger RTL-Verifikations-Workflow fÃ¼r den Safety-Island.
    Von der Spezifikation Ã¼ber RTL-Design beider Cores, Yosys-basierte
    Synthese und Ã„quivalenzprÃ¼fung bis zum FMEDA-Nachweis und
    ISO~26262 ASIL-D-Zertifizierung.}
  \label{fig:rtl_workflow}
\end{figure}

\cref{fig:rtl_workflow} zeigt den vollstÃ¤ndigen Workflow, der
...
\begin{itemize}
  ...
  \item $\mathcal{K}$: TaktdomÃ¤ne, $f = 2\,\mathrm{GHz}$.
\end{itemize}
\end{definition}

\begin{definition}[Kombinatorische RTL-Ã„quivalenz]\label{def:rtl_equiv}
$\mathcal{C}_1$ und $\mathcal{C}_2$ sind \emph{kombinatorisch Ã¤quivalent},
wenn fÃ¼r alle Eingabevektoren $\mathbf{x} \in \{0,1\}^k$:
\[
...
\begin{definition}[Sequentielle RTL-Ã„quivalenz]\label{def:seq_equiv}
$\mathcal{C}_1$ und $\mathcal{C}_2$ sind \emph{sequentiell Ã¤quivalent},
wenn fÃ¼r alle Eingabesequenzen $(\mathbf{x}_0, \mathbf{x}_1, \ldots, \mathbf{x}_T)$
und identische AnfangszustÃ¤nde $(s_1^0 = s_2^0)$ gilt:
\[
...
die ZustandsÃ¼bergangsfunktion ist.
\end{definition}

%--------------------------------------------------------------------
\subsection{Yosys-basierte Ã„quivalenzprÃ¼fung}\label{ssec:yosys}
%--------------------------------------------------------------------

\begin{theorem}[SAT-basierter Ã„quivalenzbeweis]\label{thm:sat_equiv}
...
das Ã„quivalenz-Modus-Netz von Yosys (\texttt{equiv\_make}):
\[
...
ist genau dann \emph{unerfÃ¼llbar} (UNSAT), wenn $N_1 \equiv N_2$.
\end{theorem}
\begin{proof}
\textit{Korrektheit:}
$\varphi$ ist eine Konjunktion von XOR-Klauseln Ã¼ber alle
...
wird schrittweise in eine Ã¤quivalente gate-level-ReprÃ¤sentation
...
LÃ¶st das SAT-Problem $\varphi$ aus \cref{thm:sat_equiv}.
UNSAT $\Rightarrow$ Ã„quivalenz.

\textit{Schritt 4 (equiv\_induct):}
ErgÃ¤nzt induktive Beweise fÃ¼r sequentielle Ã„quivalenz
(\cref{def:seq_equiv}) durch $k$-Induktion Ã¼ber Zeitschritte.

Kombination von Schritten 3 und 4 ergibt vollstÃ¤ndige
kombinatorische und sequentielle Ã„quivalenz.
\end{proof}
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    bei $2\,\mathrm{GHz}$) ausgelÃ¶st.
    Rechts: FDT als Funktion der Taktfrequenz fÃ¼r HX-Safety-Island,
    Cortex-M4 und Cortex-R52. Das ISO~26262-Ziel $\leq 1\,\mu$s
    ist fÃ¼r den HX bei $f \geq 1\,\mathrm{GHz}$ erfÃ¼llt.}
  \label{fig:lockstep}
\end{figure}

\cref{fig:lockstep} bestÃ¤tigt: Der HX-Safety-Island erreicht
$\mathrm{FDT} = 1\,\text{Zyklus} = 0{,}5\,\mu\text{s}$ bei $2\,\mathrm{GHz}$,
womit das ISO~26262-Limit von $1\,\mu\text{s}$ mit einem
Faktor~2 Sicherheitsabstand erfÃ¼llt wird.
...
\begin{enumerate}[label=(\roman*)]
  \item $\mathcal{C}_1 \equiv \mathcal{C}_2$
        (sequentielle Ã„quivalenz nach \cref{thm:yosys_correct}),
  \item identische AnfangszustÃ¤nde $s_1^0 = s_2^0 = \mathbf{0}$
        (synchroner Reset),
  \item identische Taktversorgung (gemeinsame PLL),
\end{enumerate}
gilt $\mathrm{LS}$ fÃ¼r alle $t \geq 0$.
\end{theorem}
\begin{proof}
\textit{Basisfall ($t=0$):}
...
fÃ¼r einen Einheitsvektor $\mathbf{e}_i$.

\textit{Schritt 2 (Komparator-Logik).}
Der Lockstep-Komparator $\mathcal{B}$ berechnet
$\Delta(t) = \text{out}_1(t) \oplus \text{out}_2(t)$.
FÃ¼r $t < t_f$: $\Delta(t) = \mathbf{0}$ (Lockstep-Korrektheit).
FÃ¼r $t = t_f$: $\Delta(t_f) = \mathbf{e}_i \neq \mathbf{0}$.

\textit{Schritt 3 (SignalÃ¼bertragungszeit).}
Das FDT-Signal $\mathrm{FDT\_flag}(t) = \mathrm{OR}(\Delta(t))$
wird am Ende von Taktzyklus $t_f$ gesetzt.
Die maximale kombinatorische VerzÃ¶gerung der XOR-OR-Kette
in 3-nm-Technologie betrÃ¤gt $\leq 0{,}2\,\tau$ (aus Timing-Analyse).
...
direkt gefÃ¤hrlicher) Fehler ist.
\end{definition}

\begin{theorem}[ASIL-D-KonformitÃ¤t des Safety-Islands]\label{thm:asil_d}
...
\begin{align*}
  ...
  \mathrm{LFM}  &\geq 90\,\%.
\end{align*}
Beide Bedingungen erfÃ¼llen ISO~26262 ASIL~D.
\end{theorem}
\begin{proof}
...
ISO~26262-Bedingungen erfÃ¼llt.
\end{proof}
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    Alle Module Ã¼berschreiten die ASIL-D-Mindestanforderungen
...
den hÃ¶chsten Wert. Das ECC-TCM mit LFM~$=94{,}7\,\%$ liegt
ebenfalls deutlich Ã¼ber dem ASIL-D-Minimum von $90\,\%$.
...
Ã¼ber Signale des RTL-Designs, ausgedrÃ¼ckt in
...
\begin{align}
  ...
  \phi_3 &: \square\,(\mathrm{ECC\text{-}err} \Rightarrow \mathrm{Korrektur})
            \quad\text{(ECC-KorrekturfÃ¤higkeit)}, \label{eq:phi3}\\
  \phi_4 &: \square\,(\mathrm{reset} \Rightarrow \bigcirc^3\,\mathrm{sync})
            \quad\text{(Reset-SynchronitÃ¤t $\leq 3$ Zyklen)}, \label{eq:phi4}\\
...
\begin{theorem}[ErfÃ¼llbarkeit der Safety-Properties]\label{thm:sva_valid}
Eigenschaften $\phi_1$ bis $\phi_7$ aus \cref{def:si_props}
sind durch den RTL-Code des HX-Safety-Islands erfÃ¼llt,
d.h.\ $\mathcal{SI} \models \phi_i$ fÃ¼r $i \in \{1,\ldots,7\}$.
\end{theorem}
\begin{proof}
$\phi_1$ folgt direkt aus \cref{thm:lockstep}.

$\phi_2$ folgt aus \cref{thm:fdt}: FDT~$\leq \tau$ impliziert
$\Diamond_{\leq\tau}\,\mathrm{FDT}$ fÃ¼r jeden Fehler $e(t)$.
...
was $\phi_3$ fÃ¼r alle in der Spezifikation berÃ¼cksichtigten
...
nach 3~Zyklen ist die Pipeline gespÃ¼lt und synchronisiert.

$\phi_5$: Die Interrupt-Controller-Logik des in-order Kerns
garantiert max. 16-Zyklen-Latenz durch vordefinierte Pipeline-PrioritÃ¤t
(aus RTL-Timing-Analyse und formaler ModellprÃ¼fung).

$\phi_6$: Die Hardware-BrÃ¼cke $\mathcal{B}$ mit garantierter
4-Zyklen-Latenz (\cref{def:hx}) erfÃ¼llt $\phi_6$ by construction.

$\phi_7$: WCET-Spread $\delta \leq 6$ folgt aus $d_\mathrm{SI} = 3$
und \cref{thm:det}: $\delta \geq e^{\gamma\pi_\mathrm{SI}}$;
fÃ¼r $\pi_\mathrm{SI} \approx 2{,}1$ (aus \cref{tab:cores}) und
...
ist ein linearer Code mit ParitÃ¤tswort der LÃ¤nge $r$ fÃ¼r
DatenwÃ¶rter der LÃ¤nge $k$, wobei
$r = \lceil \log_2(k+r+1) \rceil + 1$.
FÃ¼r $k = 64$ Bit: $r = 8$ Bit (Hamming-Abstand $d_\mathrm{min} = 4$).
\end{definition}

\begin{theorem}[ECC-Korrektheit des TCM]\label{thm:ecc_correct}
Sei $\lambda$ die Einzelbit-Fehlerrate [Fehler/Bit/h] des TCM-SRAM.
Die Rate unerkannter Fehler (FDU) mit SECDED-Schutz fÃ¼r
64-Bit-WÃ¶rter ist
\[
...
Die Wahrscheinlichkeit fÃ¼r genau $k$ Fehler in 64 Bits ist
$\binom{64}{k} \lambda^k (1-\lambda)^{64-k}$ (Binomialverteilung).
FÃ¼r $k=3$ und kleine $\lambda$ dominiert der Term
$\binom{64}{3} \lambda^3 = 41664 \lambda^3$.
Da hÃ¶here Terme $\mathcal{O}(\lambda^4)$ sind, ist die Ungleichung
fÃ¼r $\lambda < 0{,}1$ gÃ¼ltig.
\end{proof}

\begin{corollary}[ASIL-D-KonformitÃ¤t des ECC-TCM]\label{cor:ecc_asild}
FÃ¼r typische SRAM-Fehlerraten $\lambda \leq 10^{-9}$~Fehler/Bit/h:
\[
  \mathrm{FDU}_\mathrm{SECDED} \leq 41{,}664 \cdot (10^{-9})^3
  = 4{,}2 \cdot 10^{-26}\,\text{h}^{-1} \ll 10^{-9}\,\text{h}^{-1}.
\]
Das ISO~26262-Limit fÃ¼r ASIL~D von $10^{-9}\,\text{h}^{-1}$ ist
um 17 GrÃ¶ÃŸenordnungen unterschritten.
\end{corollary}
\begin{proof}
...
\begin{figure}[H]
  ...
  \caption{%
    Formale Ã„quivalenzprÃ¼fung des Safety-Islands.
    Oben links: Laufzeit der Ã„quivalenzprÃ¼fung vs. Gatterzahl
    fÃ¼r drei Methoden (BDD, SAT, ABC). Oben rechts:
    Ã„quivalenz-Coverage je Modul (grÃ¼n: 100\,\% formal bewiesen,
    orange: $>95\,\%$ mit EinschrÃ¤nkung).
    Unten links: Formale SVA-EigenschaftsprÃ¼fung -- 6 von 7
    Eigenschaften zu 100\,\% erfÃ¼llt.
    Unten rechts: FDT-Histogramm aus Monte Carlo-Simulation
    ($n=5000$) mit Gumbel-Fit.}
  \label{fig:aequivalenz}
\end{figure}

\cref{fig:aequivalenz} zeigt: Die kombinatorische Ã„quivalenz
...
Das FDT-Histogramm bestÃ¤tigt das Gumbel-Modell aus \cref{thm:gumbel}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{plot_r5_ecc.pdf}
  \caption{%
    ECC-TCM-Analyse. Links: SECDED-FÃ¤higkeit als Funktion
    ...
    im doppeltlogarithmischen MaÃŸstab. Das ASIL-D-Limit
    $10^{-9}$\,h$^{-1}$ (orange gepunktet) wird durch SECDED
    bei typischen $\lambda = 10^{-9}$ um 17 GrÃ¶ÃŸenordnungen
    unterschritten.}
  \label{fig:ecc}
\end{figure}
...
Dieses Kapitel hat fÃ¼r den HX-Safety-Island bewiesen:
\begin{enumerate}[label=\textbf{R\arabic*.}]
  \item Formaler Ã„quivalenzbeweis Core-1 $\equiv$ Core-2 mittels
        Yosys SAT-Ã„quivalenzprÃ¼fung (\cref{thm:yosys_correct}).
  \item Lockstep-Bitgleichheit $\mathrm{LS}$ fÃ¼r alle Eingaben und Zeiten
        durch vollstÃ¤ndige Induktion (\cref{thm:lockstep}).
  ...
  \item FMEDA-Nachweis: SPFM~$\geq 99\,\%$, LFM~$\geq 90\,\%$
        (\cref{thm:asil_d}).
  \item SVA-Verifikation aller 7 Safety-Properties erfÃ¼llt
        (\cref{thm:sva_valid}).
  \item ECC-TCM SECDED-Korrektheit: FDU $\leq 4{,}2 \cdot 10^{-26}$~h$^{-1}$
        -- ISO~26262-Limit um 17 GrÃ¶ÃŸenordnungen unterschritten
        (\cref{cor:ecc_asild}).
\end{enumerate}
