...
und erschwert die spÃ¤tere Verifikation am Original-Pseudocode. Der \texttt{py2}-Pr\"aprozessor
...
\texttt{(a,b,c)} & \texttt{[a,b,c]} & Tupel â†’ Liste \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{py2\_preprocessor.py} & Pr\"aprozessor: wandelt \texttt{.py2} â†’ Python~3 \\
...
der Dimension $n$ lautet die Speicherung in \emph{Bandform} fÃ¼r $A_{\text{band}} \in \mathbb{R}^{n \times 3}$:
...
\begin{proof}
	...
	Da in \texttt{py2} keine Tupel-Semantik genutzt wird (unverÃ¤nderlichkeit nicht
...
ver\"anderbare Vektoren ben\"otigt. Die UnverÃ¤nderlichkeit von Tupeln ist ein
...
\begin{itemize}
	\item \textbf{Syntaktische Treue:} \texttt{a(i,j)} ist nÃ¤her an $a(i,j)$ in
...
\url{https://github.com/hjstephan86/science/tree/main/science/python}
