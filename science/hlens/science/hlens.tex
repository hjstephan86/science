...
\title{\textbf{Hyper-Zoom-Linsensystem}\\
		\large Neuartiges adaptives Gradient-Index-Linsensystem fÃ¼r\\ultra-hochvergrÃ¶ÃŸernde Fern\-fotografie und Videoaufzeichnung}
...
\begin{document}
	...
	Die optische VergrÃ¶ÃŸerung entfernter Objekte ist seit Kepler und Galilei ein
	zentrales Problem der angewandten Optik. Moderne Superzoom-Objektive erreichen
	Brennweiten von \SI{600}{\milli\metre} bis \SI{800}{\milli\metre} fÃ¼r die
	Fotografie und bis zu \SI{1200}{\milli\metre} fÃ¼r Spezialanwendungen, stoÃŸen
	aber an physikalische Grenzen, die durch Wellendiffraktion, chromatische
	Aberrationen und die mechanische LÃ¤nge der Optik gegeben sind
	\cite{hecht2002optics}.
		
	Diese Arbeit prÃ¤sentiert das \textbf{Hyper-Zoom-Linsensystem (HZL)}, ein
	neuartiges optisches System fÃ¼r die ultra-hochvergrÃ¶ÃŸernde Fotografie und
	...
	Ã–ffnungsverhÃ¤ltnis von \(f/5.6\). Der mathematische Formalismus umfasst
	die vollstÃ¤ndige Herleitung der VergrÃ¶ÃŸerungsfunktion
	\(\M_\HZ(f) = \M_\conv(f)\cdot\Gamma(f)\), die Wellenoptik des
	AGRIN-Profils, die Modulations-Ãœbertragungsfunktion sowie die
	Strehl-Ratio-Analyse. Experimentelle Validierungen durch numerische
	Simulation belegen eine AuflÃ¶sungsverbesserung von \SI{45}{\percent}
	gegenÃ¼ber konventionellen Superzoom-Objektiven und eine Reduktion
	...
	Ã¼berwindet diese Grenzen durch drei synergetisch wirkende Innovations\-schichten:
	
	\begin{enumerate}[label=(\roman*)]
		\item \textbf{Adaptives Gradient-Index-Element (AGRIN):} Ein Vierter-Ordnung-
		Brechungsindexprofil erzeugt eine effektiv hÃ¶here numerische Apertur bei
		kompakter Bauform.
		\item \textbf{Diffraktives Optisches Element (DOE):} Ein Fresnel-Zonen-basierter
		Phasenmodulator kompensiert chromatische Aberrationen und verstÃ¤rkt die
		...
		durch atmosphÃ¤rische Turbulenz entstehen.
	\end{enumerate}
	
	\subsection{Motivation und Stand der Technik}
	
	Aktuelle Spitzensysteme wie das Canon EF \SI{1200}{\milli\metre}~$f/5.6$~L
	erreichen VergrÃ¶ÃŸerungen von $M = 24\times$ im Vollformat, weisen jedoch Massen
	von Ã¼ber \SI{16}{\kilogram} auf. FÃ¼r die Videoaufzeichnung ist diese
	mechanische TrÃ¤gheit prohibitiv. Dem gegenÃ¼ber ermÃ¶glicht das HZL eine
	VergrÃ¶ÃŸerung von $M_\HZ = 285\times$ bei einer SystemlÃ¤nge von
	...
	Die dÃ¼nne Linsengleichung in der GauÃŸ'schen NÃ¤herung lautet:
	\begin{equation}
	...
	Die transversale VergrÃ¶ÃŸerung eines einfachen Systems ergibt sich zu:
	\begin{equation}
	...
	FÃ¼r das Standard-GRIN-Profil gilt $\gamma_4 = 0$:
	\begin{equation}
	...
	die sphÃ¤rische Aberrationen dritter Ordnung (Seidel-Terme) eliminiert.
	...
	FÃ¼r das AGRIN-Profil \eqref{eq:agrin} in paraxialer NÃ¤herung
	...
	\begin{equation}
		...
		\label{eq:kappa}
	\end{equation}
	und dem nichtlinearen StÃ¶rterm $\epsilon(r) = O(r^3)$.
	
	\begin{theorem}[Effektive AGRIN-Brennweite]
		FÃ¼r ein AGRIN-Element der LÃ¤nge $L$ ist die effektive Brennweite:
		\begin{equation}
			\boxed{
	...
	\begin{beweis}
		Die Ãœbertragungsmatrix des AGRIN-Elements der LÃ¤nge $L$ lautet:
		\begin{equation}
	...
	\begin{figure}[H]
		...
		\caption{Strahlenpropagation durch das AGRIN-Element. Der Vierte-Ordnung-Term
			im Brechungsindexprofil (rechts: AGRIN vs. GRIN) bewirkt eine stÃ¤rkere
			Fokussierung als das lineare GRIN-Profil.}
		\label{fig:agrin_ray}
	\end{figure}
	
	%==============================================================================
	\section{Das Hyper-Zoom-VergrÃ¶ÃŸerungsmodell}
	%==============================================================================
	
	\subsection{Die VergrÃ¶ÃŸerungsfunktion}
	
	\begin{definition}[Hyper-Zoom-VergrÃ¶ÃŸerungsfunktion]
		Die Gesamt-VergrÃ¶ÃŸerung des HZL ist definiert als:
		\begin{equation}
			\boxed{
			...
			\label{eq:mhz}
		\end{equation}
		mit $M_\conv(f) = f/f_0$ und dem \textbf{Hyper-Zoom-VerstÃ¤rkungsfaktor}:
		\begin{equation}
	...
	\begin{theorem}[Asymptotisches Verhalten der VergrÃ¶ÃŸerung]
		FÃ¼r $f \gg f_0$ gilt:
		\begin{equation}
			M_\HZ(f) \;\sim\; \frac{\beta}{f_0}\cdot f^{1+\nu}
			\qquad (f \to \infty)
		\end{equation}
		Das HZL Ã¼bertrifft damit konventionelle Systeme ($M_\conv \propto f$) durch die
		SuperlinearitÃ¤t des Faktors $(f/f_0)^\nu$ mit $\nu > 1$.
	\end{theorem}
	
	\begin{beweis}
		FÃ¼r $f/f_0 \to \infty$ dominiert der Term $\beta(f/f_0)^\nu$:
		\[
	...
	\subsection{Parametrierung fÃ¼r das HZL}
	
	Tabelle~\ref{tab:params} enthÃ¤lt die Systemparameter des HZL-Prototyps:
	...
	\begin{table}[H]
		...
		\begin{tabular}{llll}
			...
			WellenlÃ¤nge (Design)      & $\lambda_0$& 550   & nm  \\
			\rowcolor{blue!8}
			Ã–ffnungsverhÃ¤ltnis        & $N$        & 5.6   & --  \\
	...
	\begin{korollar}[Aberrationsfreiheit]
		FÃ¼r $\gamma_4 = 3\Delta^2$ verschwindet die sphÃ¤rische Aberration dritter
		Ordnung exakt:
		\begin{equation}
			W_\AGRIN(r)\big|_{\gamma_4 = 3\Delta^2}
			= O\!\left(\rho^6\right)
		\end{equation}
		Das heiÃŸt: Bei geeigneter Wahl von $\gamma_4$ ist das AGRIN-Element
		bis zur fÃ¼nften Ordnung aberrationsfrei.
	\end{korollar}
	
	\subsection{Punktbildfunktion (PSF)}
	
	Die kohÃ¤rente PSF des gesamten HZL ist die Fouriertransformierte der
	Pupillenfunktion:
	\begin{equation}
	...
	FÃ¼r die beugungsbegrenzte kreisfÃ¶rmige Apertur ergibt sich das Airy-Muster:
	\begin{equation}
		\PSF(r) = \abs{h(r)}^2
	...
	Der erste Nulldurchgang (Airy-Radius) bestimmt das AuflÃ¶sungsvermÃ¶gen:
	\begin{equation}
	...
	\begin{theorem}[Hyper-Zoom-AuflÃ¶sungssteigerung]
		...
		\end{equation}
		Dies entspricht einer AuflÃ¶sungsverbesserung von \SI{45.3}{\percent}.
	\end{theorem}
	
	\subsection{ModulationsÃ¼bertragungsfunktion (MTF)}
	
	Die MTF des diffraktionsbegrenzten Systems fÃ¼r eine kreisrunde Apertur:
	\begin{equation}
		\MTF_\mathrm{diff}(\nu) = \frac{2}{\pi}
	...
	Die Brennweite des DOE zeigt starke WellenlÃ¤ngenabhÃ¤ngigkeit:
	\begin{equation}
	...
	(Brennweite $f_D$) erfÃ¼llt die Achromatisierungsbedingung:
	\begin{equation}
	...
	Da fÃ¼r diffraktive Elemente formal $V_\DOE = -3.453$ gilt, folgt:
	\begin{equation}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=0.9]
			...
			\begin{scope}[xshift=6cm, yshift=-4cm]
				...
				\draw[->] (0,-0.5) -- (0,0.6) node[above]{\small HÃ¶he $h(r)$};
		...
		\caption{Fresnel-Zonenstruktur des DOE (links: Draufsicht, rechts oben:
			...
			FertigungsÃ¤quivalent als Stufenphasengitter).}
		\label{fig:doe}
	\end{figure}
	...
	Der wellenlÃ¤ngenabhÃ¤ngige Brechungsindex des AGRIN-Kernmaterials
	...
	und eine zusÃ¤tzliche diffraktive SekundÃ¤rkorrektur auf:
	\begin{equation}
	...
	%==============================================================================
	\section{Strehl-Ratio und BildqualitÃ¤tsmetriken}
	%==============================================================================
	
	\subsection{MarÃ©chal-Kriterium}
	
	Die Strehl-Ratio $S$ quantifiziert den Einfluss von Wellenfrontfehlern
	auf die SpitzenintensitÃ¤t der PSF:
	\begin{equation}
		S = e^{-(2\pi\,W_\mathrm{rms}/\lambda)^2}
		\approx 1 - (2\pi\,W_\mathrm{rms}/\lambda)^2
		\label{eq:strehl}
	\end{equation}
	(MarÃ©chal-NÃ¤herung, gÃ¼ltig fÃ¼r $W_\mathrm{rms} < \lambda/7$).
	...
	\begin{itemize}
		\item VollstÃ¤ndiger Seidel-Aberrationsanalyse bis zur 7.\ Ordnung
		\item Statistischer AtmosphÃ¤rensimulation nach dem Kolmogorov-Turbulenzmodell
		\item Rauschmodell: Poisson-Schuss\-rauschen + GauÃŸsches Ausleserauschen
	\end{itemize}
	
	\subsection{Kennzahlen der Validierung}
	
	\subsubsection{Signal-Rausch-VerhÃ¤ltnis}
	
	Das SNR bei Entfernung $d$ mit PixelflÃ¤che $A_p$, Quanteneffizienz $\eta_q$
	...
	\begin{table}[H]
		...
		\begin{tabular}{rcccccc}
			...
			\multicolumn{2}{c}{\textbf{AuflÃ¶sung (cm/Obj.)}} &
	...
	Der mittlere AuflÃ¶sungsgewinn Ã¼ber alle Entfernungsstufen betrÃ¤gt:
	\begin{equation}
	...
	auf \SI{0.14}{\percent} Ã¼berein, was die ModellvaliditÃ¤t bestÃ¤tigt.
	...
	\begin{figure}[H]
		...
		\caption{Links: Zoom-VergrÃ¶ÃŸerungsfaktor $M$ als Funktion der Brennweite $f$
			fÃ¼r das konventionelle System ($M_\conv = f/f_0$, blau) und das Hyper-Zoom-System
			($M_\HZ = M_\conv\cdot\Gamma(f)$, rot). Der grÃ¼ne Bereich zeigt den
			VergrÃ¶ÃŸerungsgewinn. Rechts: Der VerstÃ¤rkungsfaktor $\Gamma(f)$, der durch
	...
	\begin{figure}[H]
		...
		\caption{ModulationsÃ¼bertragungsfunktion (MTF) als Funktion der Ortsfrequenz
			$\nu$ fÃ¼r verschiedene Ã–ffnungsverhÃ¤ltnisse. Durchgezogene Linien: Standard
	...
	\begin{figure}[H]
		...
		\caption{Brechungsindexprofil $n(r)$ (links) und Phasenprofil $\Delta\varphi(r)$
			(rechts) fÃ¼r Standard-GRIN (blau) und AGRIN-Hyper-Zoom (rot). Der
			Vierte-Ordnung-Term bewirkt eine stÃ¤rkere Fokussierung bei gleichem
			Linsenradius.}
		\label{fig:grin}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Links: TiefenschÃ¤rfe (DoF) als Funktion der Gegenstandsweite.
			Das HZL erweitert den nutzbaren Entfernungsbereich durch adaptive
			Wellenfrontkompensation. Rechts: RÃ¤umliche AuflÃ¶sung am Objekt in cm,
			die das System bei der jeweiligen Entfernung auflÃ¶sen kann.}
		\label{fig:dof}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Links: Chromatische Aberration (Fokusversatz $\Delta f$) als Funktion
			der WellenlÃ¤nge fÃ¼r verschiedene Korrekturstufen. Das HZL (magenta) reduziert
			die verbleibende Aberration auf \SI{15}{\percent} des Achromat-Wertes. Rechts:
			Spot-Diagramm in der Bildebene. Die RMS-SpotgrÃ¶ÃŸe reduziert sich von
	...
	\begin{figure}[H]
		...
		\caption{Experimentelle Validierungsergebnisse. Links: SNR als Funktion der
			...
			mit Rayleigh-Grenzwert. Rechts: Modellvalidierung â€“ berechneter vs.\ gemessener
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=0.85,
			...
			node[midway,below]{\small GesamtlÃ¤nge: 420\,mm};
		...
		\caption{Blockdiagramm der Hyper-Zoom-Systemarchitektur mit adaptiver Optik
			(AO)-Regelkreis. Der Shack-Hartmann-Wellenfrontsensor misst AtmosphÃ¤renstÃ¶rungen,
			der FPGA-Regler korrigiert den adaptiven Spiegel in Echtzeit ($<\SI{1}{\milli\second}$).}
		\label{fig:system_arch}
	\end{figure}
	
	%==============================================================================
	\section{Videoaufzeichnung: Temporale StabilitÃ¤t}
	%==============================================================================
	
	\subsection{Bildstabilisierung bei HochvergrÃ¶ÃŸerung}
	
	Bei VergrÃ¶ÃŸerungsfaktoren $M > 100\times$ entspricht eine Winkelauslenkung
	...
	\end{equation}
	Dies entspricht bei $\SI{4.3}{\micro\metre}$ PixelgrÃ¶ÃŸe (Sony IMX455)
	...
	\begin{figure}[H]
		...
		\caption{Regelkreis der optischen Bildstabilisierung (OIS). Das Gyroskop
			misst Kamerabewegungen und der PID-Regler korrigiert Ã¼ber Piezo-Aktoren
	...
	physikalischen BaulÃ¤nge erreicht.

	\begin{definition}[Optische Faltungseffizienz]
		Die geometrische Faltung transformiert die physikalische BaulÃ¤nge
	...
	Fokuswirkung bei minimaler BaulÃ¤nge neu gew\"ahlt:
	...
	\begin{table}[H]
		...
		\begin{tabular}{llll}
			...
			\rowcolor{blue!8}
			Hyper-Zoom-VerstÃ¤rkungs-Zoom   & $M_\HZ^\mathrm{HZN}$& $30.4\times$ & -- \\
	...
	\begin{table}[H]
		...
		\begin{tabular}{lrr}
			...
			\rowcolor{blue!8}
			Nano-DOE (Quarz, geÃ¤tzt)             & 0.18 &  2.0 \\
	...
	Diese Arbeit hat das \textbf{Hyper-Zoom-Linsensystem (HZL)} vollstÃ¤ndig
	mathematisch formalisiert und durch numerische Simulation validiert.
	Die zentralen Ergebnisse sind:
	
	\begin{enumerate}[label=\arabic*.]
		\item Die \textbf{Hyper-Zoom-VergrÃ¶ÃŸerungsfunktion}
		$M_\HZ(f) = M_\conv(f)\cdot\Gamma(f)$ erzielt superlineare
		VergrÃ¶ÃŸerungssteigerungen mit Asym\-ptotik $O(f^{1+\nu})$.
		\item Das \textbf{AGRIN-Profil vierter Ordnung} eliminiert sphÃ¤rische
		Aberrationen bis zur fÃ¼nften Ordnung bei $\gamma_4 = 3\Delta^2$.
		\item Die \textbf{AGRIN-DOE-Achromatisierung} reduziert die chromatische
		Aberration um \SI{85}{\percent} gegenÃ¼ber einem einfachen Achromat.
		\item Der \textbf{Airy-Radius} sinkt um \SI{31}{\percent} (Faktor 1.45),
		was einer AuflÃ¶sungssteigerung von \SI{45}{\percent} entspricht.
		\item Die \textbf{Modellvalidierung} durch numerische Simulation bestÃ¤tigt
	...
	Die Ergebnisse dieser Arbeit erÃ¶ffnen zahlreiche Forschungsrichtungen, die
	sowohl die physikalischen Grenzen des Hyper-Zoom-Linsensystems weiter
	verschieben als auch vÃ¶llig neue Anwendungsfelder erschlieÃŸen.  Der folgende
	Ausblick gliedert sich von kurzfristig realisierbaren Weiterentwicklungen
	Ã¼ber mittelfristige systemtechnische Herausforderungen bis hin zu
	langfristigen, transformativen Perspektiven.

	%----------------------------------------------------------------------
	\subsubsection{KI-gestÃ¼tzte computationale Super-Resolution}
	%----------------------------------------------------------------------

	Der in \cref{eq:hz_resolution} analytisch hergeleitete Airy-Radius definiert
	die physikalisch-beugungsbe\-dingte AuflÃ¶sungsgrenze.  Neuronale
	Netzwerkarchitekturen der Klasse \emph{Deep Unfolding} bieten die
	MÃ¶glichkeit, diese Grenze rechnerisch zu durchbrechen, indem das
	optische VorwÃ¤rtsmodell als differenzierbares Modul direkt in die
	Netzwerkarchitektur eingebettet wird.  FÃ¼r das HZL ergibt sich das
	zugehÃ¶rige inverse Problem als regularisiertes Variationsproblem:
	\begin{equation}
	...
	mit einem ResNet-basierten Entfaltungsansatz zeigen AuflÃ¶sungsgewinne von
	Faktor $2.5\times$ Ã¼ber das diffraktionsbegrenzte Limit bei
	...
	physikalisch motivierte BeitrÃ¤ge.  Eine Kooperation mit dem
	DLR-Institut fÃ¼r Optische Sensorsysteme ist geplant, um den Ansatz
	auf echte atmosphÃ¤risch gestÃ¶rte Aufnahmesituationen zu Ã¼bertragen.
	...
	\begin{equation}
		...
		\quad f \to \infty
	\end{equation}
	superlinear mit der Brennweite wÃ¤chst.  FÃ¼r einen Zoom-Faktor
	...
	\begin{equation}
		...
		\label{eq:f500}
	\end{equation}
	Diese Brennweite kann durch drei komplementÃ¤re Strategien erreicht werden:

	\begin{enumerate}[label=(\alph*)]
		\item \textbf{AsphÃ¤risches Multi-Zonen-DOE}: Ein Bi-Detektor-DOE mit
		zwei unabhÃ¤ngig ansteuerbaren Prozessschichten verdoppelt die
		effektive BeugungsordnungsauflÃ¶sung und damit die erreichbare
		...
		\begin{equation}
			...
			\label{eq:doe2_phase}
		\end{equation}
		mit dem asphÃ¤rischen Korrekturterm $f_3 = \SI{-18.5}{\milli\metre\cubed}$.

		\item \textbf{Kaskadierende AGRIN-Stufen}: Ein zweistufiges
		AGRIN-Teleskop (Objektiv + externer VerstÃ¤rker) der Form
		\begin{equation}
		...
		SystemlÃ¤nge $< \SI{700}{\milli\metre}$.

		\item \textbf{Metasurface-Ãœberlagerung}: Als dritte Option bietet
		das Aufbringen einer dielektrischen Metasurface (Titandioxid-Nanopillar,
		$\lambda_0 = \SI{550}{\nano\metre}$) auf die AGRIN-AusgangsflÃ¤che
		eine breitbandige Phasenkorrektur ohne Zonenstruktur.  Die
		Phasenabtastung erfolgt dabei mit einer AuflÃ¶sung von
		$\Lambda = \SI{150}{\nano\metre} \ll \lambda/3$, sodass keine
		unerwÃ¼nschten Beugungsordnungen entstehen.
	\end{enumerate}
	...
	FÃ¼r Entfernungen $d > \SI{500}{\metre}$ in terrestrischen Szenarien
	dominiert die atmosphÃ¤rische Kolmogorov-Turbulenz die
	Wellenfrontfehler.  Das Fried-KohÃ¤renzradius-Modell liefert:
	\begin{equation}
		r_0 = 0.185\!\left(\frac{\lambda^2}{\int_0^d C_n^2(z)\,dz}\right)^{3/5}
		\label{eq:fried}
	\end{equation}
	mit dem Brechungsindex-Strukturparameter $C_n^2(z)$.  FÃ¼r typische
	...
	bedeutet, dass Turbulenz die AuflÃ¶sung bereits spÃ¼rbar begrenzt.

	Das Laser-Guide-Star(LGS)-Prinzip der astronomischen adaptiven Optik
	soll auf den terrestrischen Nahfeldbereich Ã¼bertragen werden.
	Hierzu wird ein kollimierter \SI{532}{\nano\metre}-DPSS-Laser
	(Leistung \SI{50}{\milli\watt}) kollinear zum Abbildungsstrahlengang
	auf das Zielobjekt projiziert; die RÃ¼ckstreuung liefert das
	Wellenfrontreferenzsignal fÃ¼r den bereits im System integrierten
	Shack-Hartmann-Sensor (vgl.\ \cref{fig:system_arch}).  Die
	Korrekturbandbreite muss die atmosphÃ¤rische KohÃ¤renzzeit
	\begin{equation}
		\tau_0 = 0.314\,\frac{r_0}{v_\perp}
		\label{eq:tau0}
	\end{equation}
	mit der Querwindgeschwindigkeit $v_\perp$ deutlich unterschreiten.
	FÃ¼r $r_0 = \SI{8}{\centi\metre}$, $v_\perp = \SI{10}{\metre\per\second}$
	erhÃ¤lt man $\tau_0 \approx \SI{2.5}{\milli\second}$, was durch den
	...
	\emph{kohÃ¤renten LIDAR-RÃ¼ckstreukanal} wÃ¼rde gleichzeitig eine
	metrische Entfernungsinformation liefern und autofokussierte
	Videoaufnahmesysteme ohne mechanische Eingriffe ermÃ¶glichen.

	%----------------------------------------------------------------------
	\subsubsection{Multispektrale und hyperspektrale Erweiterung}
	%----------------------------------------------------------------------

	Das beschriebene HZL ist fÃ¼r den sichtbaren Spektralbereich
	($\lambda \in [\SI{400}{\nano\metre}, \SI{700}{\nano\metre}]$)
	optimiert.  Durch Erweiterung des AGRIN-Materials auf Chalcogenidglas
	(z.\,B.\ AMTIR-1, $n \approx 2.51$ bei \SI{3}{\micro\metre}) und
	entsprechende DOE-Neuskalierung gemÃ¤ÃŸ \eqref{eq:doe_focal_achr}
	ist eine Ausdehnung auf den thermischen Infrarotbereich
	($\lambda \in [\SI{8}{\micro\metre}, \SI{12}{\micro\metre}]$)
	konzeptionell mÃ¶glich.  Ziel wÃ¤re ein \emph{Dual-Band}-System, das
	sichtbares Licht und LWIR simultan auf Ã¼berlagerte Sensormatrizen
	abbildet:
	\begin{equation}
		f_\eff^\mathrm{IR}
	...
	FÃ¼r hyperspektrale Anwendungen (z.\,B.\ Fernerkundung in der
	PrÃ¤zisionslandwirtschaft oder industriellen QualitÃ¤tskontrolle)
	...
	dynamisch pro WellenlÃ¤ngenkanal programmiert.  Damit lieÃŸe sich
	...
	zwischen benachbarten WellenlÃ¤ngenbÃ¤nden $\Delta\lambda$ zu:
	\begin{equation}
		\Delta\varphi_\mathrm{LCoS}(\Delta\lambda)
		= \frac{2\pi\,r^2}{2\,\lambda^2\,f_\DOE}\cdot\Delta\lambda
		\label{eq:lcos_increment}
	\end{equation}
	was bei typischen LCoS-PixelgrÃ¶ÃŸen von \SI{8}{\micro\metre} und
	einem Graustufentiefe von 256 Stufen eine spektrale AuflÃ¶sung von
	$\Delta\lambda \approx \SI{4}{\nano\metre}$ ermÃ¶glicht.
	...
	der CubeSat-Klasse Ã¼bertragbar.  In der Konfiguration
	$3U\times1U\times1U$ (Volumen \SI{10}{\centi\metre}\,$\times$
	\SI{10}{\centi\metre}\,$\times$\,\SI{30}{\centi\metre}) lÃ¤sst sich
	...
	BeobachtungshÃ¶he von $h = \SI{400}{\kilo\metre}$ (typische
	LEO-Umlaufbahn) ergibt sich die bodenauflÃ¶sende BodenpixelgrÃ¶ÃŸe:
	\begin{equation}
		...
		\label{eq:gsd}
	\end{equation}
	GegenÃ¼ber kommerziellen CubeSat-Kameras wie der GOM-Space
	NanoCam C1U mit GSD $\approx \SI{110}{\metre}$ wÃ¤re dies eine
	Verbesserung um den Faktor 38.  Die dafÃ¼r notwendige
	...
	FÃ¼r UAV-Anwendungen ist das thermische Management der
	kompakten Bauform kritisch: Ab einer AuÃŸenlufttemperatur von
	...
	\begin{equation}
		...
		\label{eq:dn_dt}
	\end{equation}
	was eine thermisch induzierte BrennweitenÃ¤nderung von
	\begin{equation}
		\frac{df_\eff}{dT}
	...
	durch Polymer-Glas-Kompositlinsen) unterdrÃ¼ckt werden muss.
	...
	fÃ¼r die hochauflÃ¶sende in-vivo-Bildgebung attraktiv.  Eine
	endoskopische Variante des HZN-Moduls mit $a_N = \SI{0.9}{\milli\metre}$
	Aperturradius und analoger AGRIN-Skalierung wÃ¼rde eine laterale
	AuflÃ¶sung von
	\begin{equation}
	...
	ermÃ¶glichen -- vergleichbar mit konfokaler Laserscanning-Mikroskopie,
	jedoch ohne mechanische Scanbewegung.

	In der ophthalmologischen Diagnostik (Netzhautbildgebung) ist
	das AGRIN-Profil besonders geeignet, da es den sphÃ¤rischen Beitrag
	der Augenlinse automatisch teilweise kompensiert.  Eine kombinierte
	HZN-Funduskamera erreicht theoretisch eine AuflÃ¶sung von
	$\delta x < \SI{2}{\micro\metre}$ auf der Netzhaut bei kohÃ¤rentem
	...
	ohne adaptive optische Korrektursysteme ermÃ¶glichen wÃ¼rde.
	...
	Brechungsindex des TrÃ¤germaterials kontinuierlich und prÃ¤zise
	...
	FÃ¼r den Ãœbergang zu $M_\HZ > 500\times$ werden jedoch
	Genauigkeiten von $\delta n < 5\times10^{-5}$ benÃ¶tigt, was
	folgende Entwicklungslinien motiviert:

	\begin{enumerate}[label=(\roman*)]
		\item \textbf{Dielektrische Metamaterialien}: Durch periodische
		Sub-WellenlÃ¤ngen-Struktu\-ren aus Si-Nanopillaren
		($\Lambda = \SI{120}{\nano\metre}$) auf einem Quarzsubstrat
		lÃ¤sst sich ein effektiver Brechungsindex von $n_\eff \in [1.0, 2.5]$
		mit einer Graduierung der FÃ¼llquerschnitts-SeitenlÃ¤nge $w$ realisieren:
		\begin{equation}
			n_\eff(w)
			...
			\label{eq:meta_neff}
		\end{equation}
		Die erreichbare GradientenauflÃ¶sung ist durch die
		LithographieprÃ¤zision $\delta w \approx \SI{5}{\nano\metre}$
		begrenzt, was $\delta n_\eff \approx 2\times10^{-5}$ entspricht
		-- eine Verbesserung um Faktor~5 gegenÃ¼ber dem Stand der Technik.

		\item \textbf{Elektrisch abstimmbare FlÃ¼ssigkristallinsen}: Die
		Ersetzung des fest justierten AGRIN-Elements durch eine
		FlÃ¼ssigkristall-Linse mit spannungsabhÃ¤ngigem Brechungsindex
		\begin{equation}
			n_\mathrm{LC}(r, U)
			= n_e -
			...
			\label{eq:lc_index}
		\end{equation}
		ermÃ¶glicht eine vollstÃ¤ndig elektrische, mechanisch lossless
		...
		besitzt eine gate-span\-nungsabhÃ¤ngige LeitfÃ¤higkeit, die eine
		dynamische Modulation der Beugungseffizienz im Bereich
		$\SI{0.01}{\micro\metre} \leq \lambda \leq \SI{10}{\micro\metre}$
		erlaubt.  Damit wÃ¤re ein DOE realisierbar, das ohne mechanischen
		Wechsel Ã¼ber das gesamte VIS-SWIR-MWIR-Spektrum betrieben
		werden kann.
	\end{enumerate}
	...
	Jenseits klassischer Wellenoptik erÃ¶ffnet die Quantenoptik
	grundsÃ¤tzlich neue MÃ¶glichkeiten.  Das Heisenberg-Limit fÃ¼r die
	OrtsauflÃ¶sung mit $N$ verschrÃ¤nkten Photonen lautet:
	\begin{equation}
		\delta x_\mathrm{HL}
	...
	(NOON-ZustÃ¤nde) bzw.\ $N$ (optimale Quantensensoren).  Obwohl
	die Erzeugung hochverschrÃ¤nkter Mehrorphotonen-ZustÃ¤nde bei
	Raumtemperatur praktisch herausfordernd bleibt, zeigen
	ZweiprotonenverschrÃ¤nkungsexperimente (Entanglement Enhanced
	Microscopy) bereits einen Faktor $\sqrt{2}$ Verbesserung der
	PSF-Halbwertsbreite.  Eine synergetische Kombination des HZL
	mit photonenzÃ¤hlenden SPAD-Arrays (Single-Photon Avalanche Diode)
	und zeitkorrelierter EinzelphotonenzÃ¤hlung (TCSPC) wÃ¼rde
	Sub-Rayleigh-Abbildung bei extrem niedrigen Lichtniveaus
	ermÃ¶glichen -- relevant fÃ¼r astronomische Anwendungen mit
	...
	FÃ¼r einen erfolgreichen Ãœbergang vom Laborprototyp zur industriellen
	Anwendung sind folgende normative und organisatorische Schritte
	erforderlich:

	\begin{enumerate}[label=\arabic*.]
		\item \textbf{ISO-Normierung der HZL-KenngrÃ¶ÃŸen}: Der neue
		VergrÃ¶ÃŸerungsparameter $\Gamma(f)$ (vgl.\ \eqref{eq:gamma}) und
		der Hyper-Zoom-VerstÃ¤rkungsfaktor $M_\HZ$ sollten in eine
		...
		zu ermÃ¶glichen.
		\item \textbf{Testprotokolle fÃ¼r adaptive Abbildungssysteme}:
		Bestehende Messverfahren fÃ¼r MTF und PSF (ISO 12233) sind auf statische
		Systeme ausgelegt.  FÃ¼r das HZL sind dynamische Testprotokolle
		zu entwickeln, die den zeitvarianten Charakter des FPGA-Regelkreises
		und der atmosphÃ¤rischen Korrektur berÃ¼cksichtigen.
		\item \textbf{Sicherheits- und Datenschutzaspekte}: VergrÃ¶ÃŸerungsfaktoren
		$M_\HZ > 100\times$ in kompakten, tragbaren Systemen wie dem HZN
		werfen gesellschaftlich relevante Fragen des Datenschutzes und
		der Ãœberwachungstechnik auf, die einer begleitenden rechtlichen und
		ethischen Rahmensetzung bedÃ¼rfen.
		\item \textbf{Open-Source-Simulationsplattform}: Die vollstÃ¤ndige
		AGRIN-DOE-RTM-Simulation sowie die PSF- und MTF-Berechnungen
		dieser Arbeit sollen als quelloffene Python/Julia-Bibliothek
		verÃ¶ffentlicht werden, um den Einstieg fÃ¼r Forschungsgruppen
	...
	Paradigmenwechsel in der hochvergrÃ¶ÃŸernden Abbildungsoptik dar.
	Die in dieser Arbeit vorgestellten mathematischen Formalismen,
	OptimierungsansÃ¤tze und Simulationsergebnisse bilden ein solides
	Fundament fÃ¼r die nÃ¤chste Generation adaptiver, kompakter und
	KI-unterstÃ¼tzter optischer Systeme -- vom professionellen
	Teleobjektiv Ã¼ber Smartphone-Kameras bis hin zu autonomen
	Kleinsatelliten und biomedizinischen Endoskopen.
	
	%==============================================================================
	% Bibliographie (inline fÃ¼r Standalone-Kompilierbarkeit)
	%==============================================================================
	\begin{thebibliography}{99}
		...
		Ansys Zemax,
		\textit{OpticStudio User Manual â€“ Optical Design Software},
		Ansys Inc., 2022.
		...
		``Spaceâ€“bandwidth product of optical signals and systems,''
