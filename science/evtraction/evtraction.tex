...
Drehmomentbereich, der Konstantleistungsbereich (FeldschwÃ¤chung), die
...
beleuchtet zukÃ¼nftige Forschungsrichtungen in Motordesign, Regelungstechnik,
...
grundlegend vom Verbrennungsmotor. WÃ¤hrend konventionelle Motoren ein eng
begrenztes Drehzahlband mit nutzbarem Drehmoment aufweisen und daher mehrstufige
Getriebe benÃ¶tigen, deckt der elektrische Traktionsmotor ein breites Drehzahlspektrum
mit hohem Drehmoment ab. Das HerzstÃ¼ck dieser Eigenschaft ist die
\emph{Drehmoment-Drehzahl-Kennlinie} (engl.\ \emph{torque-speed characteristic}),
welche das Betriebsverhalten des Motors Ã¼ber den gesamten Drehzahlbereich beschreibt.

\subsection{Motivation und Bedeutung}

Das Drehmoment-Drehzahl-Profil bestimmt unmittelbar:
\begin{itemize}
    \item die \textbf{BeschleunigungsfÃ¤higkeit} (Anfahrmoment bei $n = 0$),
    \item die \textbf{SteigfÃ¤higkeit} (Overcoming von Hangwiderstand bei niedrigen
          Geschwindigkeiten),
    \item die \textbf{HÃ¶chstgeschwindigkeit} (Dauerleistung bei hohen Drehzahlen),
    \item die \textbf{Energieeffizienz} im gesamten Fahrzyklus,
    \item die \textbf{Dimensionierung} von Wechselrichter, Batterie und KÃ¼hlsystem.
\end{itemize}

Permanentmagneterregte Synchronmaschinen (PMSM) haben sich wegen ihres hohen
Wirkungsgrades ($\eta > 95\,\%$), ihrer hohen Leistungsdichte und ihrer prÃ¤zisen
Regelbarkeit als bevorzugte Technologie fÃ¼r EV-Traktionsanwendungen
...
die FeldschwÃ¤chung. Die Leistungsbilanz und das Radmoment werden in
...
einen umfassenden Ausblick auf zukÃ¼nftige Entwicklungen.
...
\subsection{Physikalische GrundgrÃ¶ÃŸen}
...
\begin{definition}[Radmoment]
    Das auf die AntriebsrÃ¤der Ã¼bertragene Radmoment $T_W$ berechnet sich zu
    \[
        T_W = T_m \cdot G \cdot \eta_{\mathrm{DT}},
    \]
    wobei $T_m$ das Motordrehmoment, $G$ das GesamtÃ¼bersetzungsverhÃ¤ltnis und
...
\label{lem:nbase}
    FÃ¼r den Referenzmotor mit $P_{\max} = \SI{104700}{\watt}$ und
...
\begin{enumerate}
    ...
    \item \textbf{Konstantleistungsbereich} ($n_{\mathrm{base}} < n \leq n_{\max}$):
          Das Drehmoment sinkt hyperbolisch; die Leistung bleibt annÃ¤hernd konstant.
          ErmÃ¶glicht wird dieser Bereich durch FeldschwÃ¤chung.
\end{enumerate}
...
\begin{proof}
    ...
    Im Konstantmomentbereich ist der Statorstrom $I_s$ gleich dem maximal zulÃ¤ssigen
    Strom $I_{\max}$. Da der Flusslinkung $\psi$ noch nicht geschwÃ¤cht wird, gilt fÃ¼r den
    ...
    $U_{\max}$ des Wechselrichters. Eine weitere DrehzahlerhÃ¶hung erfordert die
    Reduktion der Flusslinkung durch negativen $d$-Achsenstrom $I_d < 0$
    (FeldschwÃ¤chung). Die maximale Leistung ist auf
    \[
        P_{\max} = T_{\max} \cdot \omega_{\mathrm{base}} = T_{\max} \cdot \frac{2\pi n_{\mathrm{base}}}{60}
    \]
    beschrÃ¤nkt. Im Konstantleistungsbereich gilt $P(n) = T(n) \cdot \omega(n) = P_{\max}$,
    also
    \[
...
\begin{proof}
    ...
    Beide einseitigen Grenzwerte stimmen Ã¼berein.
\end{proof}
...
\begin{figure}[H]
    ...
    \caption{Drehmoment-Drehzahl-Kennlinie (orange) und Leistungs-Drehzahl-Kennlinie
             (blau gestrichelt) des PMSM-Referenzmotors. Der Ãœbergang von konstantem
...
Tabelle~\ref{tab:messdaten} listet die im Infografik angegebenen StÃ¼tzpunkte und
vergleicht sie mit den Werten aus Satz~\ref{satz:kennlinie}.

\begin{table}[H]
    \centering
    \caption{Numerische ÃœberprÃ¼fung der Kennlinie. $T_{\mathrm{theor.}} =
             T_{\max} \cdot n_{\mathrm{base}} / n$ fÃ¼r $n > n_{\mathrm{base}}$.}
    \label{tab:messdaten}
...
\begin{lemma}[Ãœbereinstimmung von Messdaten und Modell]
...
\begin{proof}
    ...
    $|83 - 83{,}\overline{3}| = 0{,}\overline{3}\,\si{\newton\metre}$. FÃ¼r alle
    anderen StÃ¼tzpunkte ist die Abweichung kleiner oder gleich null, da
    Ganzzahlrundung der angegebenen Werte mit der theoretischen Kurve Ã¼bereinstimmt.
    Die relativen Fehler bleiben stets unter $0{,}6\,\%$, was die ModellgÃ¼te bestÃ¤tigt.
\end{proof}
...
\label{satz:leistung}
    Die mechanische Leistung $P(n)$ als Funktion der Drehzahl betrÃ¤gt:
    \[
        P(n) =
        \begin{cases}
...
\begin{proof}
    ...
    was mit dem Tabellenwert $\SI{104{,}7}{\kilo\watt}$ Ã¼bereinstimmt.
\end{proof}
...
\begin{figure}[H]
    ...
    \caption{Leistungs-Drehzahl-Kennlinie des Referenzmotors. Die Leistung steigt
             ...
             bleibt danach annÃ¤hernd konstant bei $P_{\max} = \SI{104{,}7}{\kilo\watt}$.}
...
\label{satz:radmoment}
    Sei $r$ der dynamische Reifenradius, $G$ das GesamtÃ¼bersetzungsverhÃ¤ltnis und
    $\eta_{\mathrm{DT}}$ der Antriebsstrangwirkungsgrad. Die Fahrzeuggeschwindigkeit
    $v$ ergibt sich aus der Motordrehzahl Ã¼ber
    \[
    ...
    \]
    Das Radmoment als Funktion der Geschwindigkeit betrÃ¤gt dann:
    \[
        T_W(v) = T(n(v)) \cdot G \cdot \eta_{\mathrm{DT}}.
    \]
\end{satz}

Abbildung~\ref{fig:wheel_torque} veranschaulicht das Radmoment Ã¼ber der
Fahrzeuggeschwindigkeit fÃ¼r $G = 9$ und $\eta_{\mathrm{DT}} = 0{,}95$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{plot6_wheel_torque.pdf}
    \caption{Radmoment $T_W$ als Funktion der Fahrzeuggeschwindigkeit $v$ fÃ¼r
             Ãœbersetzung $G = 9$ und Antriebsstrangwirkungsgrad $\eta_{\mathrm{DT}} = 0{,}95$.
             Das maximale Radmoment betrÃ¤gt $T_{W,\max} = 250 \cdot 9 \cdot 0{,}95 \approx
             \SI{2.1}{\kilo\newton\metre}$.}
    \label{fig:wheel_torque}
\end{figure}

% ============================================================
\section{FeldschwÃ¤chung}
\label{sec:feldschwaechung}
% ============================================================

\subsection{Grundprinzip der FeldschwÃ¤chung}

Die FeldschwÃ¤chung (engl.\ \emph{field weakening}) ist das entscheidende Verfahren,
das den Betrieb des PMSM oberhalb der Eckdrehzahl ermÃ¶glicht. Ohne FeldschwÃ¤chung
wÃ¼rde die induzierte Spannung bei $n > n_{\mathrm{base}}$ die zulÃ¤ssige
Versorgungsspannung $U_{\max}$ Ã¼bersteigen.

\begin{satz}[FeldschwÃ¤chungsstrom]
\label{satz:feldschwaechung}
    Im FeldschwÃ¤chbereich wird durch EinprÃ¤gung eines negativen $d$-Achsenstroms
    ...
    \[
        U_s = \omega \cdot \sqrt{(\psi_{\mathrm{PM}} + L_d \cdot I_d)^2 + (L_q \cdot I_q)^2}
            \leq U_{\max}
    \]
    eingehalten wird. Dabei bezeichnen $L_d, L_q$ die InduktivitÃ¤ten in $d$- und
    $q$-Achse des PMSM.
\end{satz}

\begin{proof}
    Im stationÃ¤ren Betrieb des PMSM in $dq$-Koordinaten gelten die Spannungsgleichungen
    \begin{align*}
        U_d &= R_s I_d - \omega L_q I_q, \\
        U_q &= R_s I_q + \omega (L_d I_d + \psi_{\mathrm{PM}}).
    \end{align*}
    FÃ¼r ohmsch vernachlÃ¤ssigbaren Statorwiderstand ($R_s \approx 0$, was bei hohen
    ...
    \]
    Um $U_s \leq U_{\max}$ zu gewÃ¤hrleisten, muss bei steigendem $\omega$ der Term
    $(L_d I_d + \psi_{\mathrm{PM}})$ reduziert werden, was durch $I_d < 0$ erreicht wird.
\end{proof}

\begin{lemma}[Drehmomentverlust durch FeldschwÃ¤chung]
    Im FeldschwÃ¤chbereich reduziert sich das maximal erzielbare Drehmoment, weil
    $I_d \neq 0$ und die StrombeschrÃ¤nkung $I_s^2 = I_d^2 + I_q^2 \leq I_{\max}^2$
    aktiv ist:
    \[
        I_q = \sqrt{I_{\max}^2 - I_d^2} < I_{\max}.
    \]
    Das verfÃ¼gbare Drehmoment fÃ¤llt daher auf
    $T = \frac{3}{2} p [\psi_{\mathrm{PM}} I_q + (L_d - L_q) I_d I_q]$.
\end{lemma}

Abbildung~\ref{fig:field_weakening} zeigt die Stromkomponenten $I_d$ und $I_q$
im FeldschwÃ¤chbereich.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{plot5_field_weakening.pdf}
    \caption{Normierte Stromkomponenten im FeldschwÃ¤chbereich. Mit zunehmender
             Drehzahl wÃ¤chst $|I_d|$ (FeldschwÃ¤chstrom), wÃ¤hrend $I_q$
             (Drehmomentstrom) sinkt. Der Gesamtstromzeiger $|I_{\mathrm{total}}|$
             bleibt konstant gleich dem maximal zulÃ¤ssigen Strom.}
...
\begin{satz}[ReversibilitÃ¤t des PMSM als Generator]
\label{satz:rekuperation}
    ...
    Die rekuperierte Leistung betrÃ¤gt
    \[
...
\begin{proof}
    ...
    die Batterie zurÃ¼ckgespeist wird. Die Drehmomentkennlinie ist anti-symmetrisch
    ...
    PMSM im selben Betriebspunkt arbeitet â€” nur mit umgekehrter Drehmomentrichtung â€”
...
\begin{figure}[H]
    ...
    \caption{Vier-Quadranten-Darstellung des PMSM. Der grÃ¼ne Bereich zeigt das
...
\begin{lemma}[EnergierÃ¼ckgewinnung bei einem Bremsvorgang]
    Beim Bremsvorgang eines Fahrzeugs der Masse $m$ von Geschwindigkeit $v_1$ auf
    $v_2 < v_1$ betrÃ¤gt die theoretisch rÃ¼ckgewinnbare Energie:
    \[
...
\begin{beispiel}
    FÃ¼r $m = \SI{1800}{\kilo\gram}$, $v_1 = \SI{100}{\kilo\metre\per\hour} = \SI{27{,}78}{\metre\per\second}$,
    ...
    und zeigt die signifikante ReichweitenerhÃ¶hung durch Rekuperation.
\end{beispiel}
...
\label{satz:verluste}
    ...
    \]
    wobei $k_{\mathrm{Fe}}$ und $k_{\mathrm{mech}}$ materialabhÃ¤ngige Konstanten sind.
\end{satz}

\begin{proof}
    \textit{Kupferverluste} $P_{\mathrm{Cu}} = R_s I_s^2$: Joulsche WÃ¤rme in den
    ...
    und Wirbelstrom) in den Blechpaketen; steigen Ã¼berproportional mit der Frequenz.
    ...
    sowie Durchlassverluste; abhÃ¤ngig von Strom und Schaltfrequenz.

    Die Summe dieser vier Terme liefert ein hinreichend genaues Verlustmodell fÃ¼r die
...
\begin{figure}[H]
    ...
    \caption{Wirkungsgradkarte des PMSM-Referenzmotors. Das Hocheffizienzgebiet
             ($\eta > 95\,\%$, gelb-grÃ¼ner Bereich) liegt typischerweise im mittleren
...
\begin{lemma}[MTPA-Bedingung fÃ¼r PMSM mit Reluktanzanteil]
    FÃ¼r einen PMSM mit Reluktanzanteil ($L_d \neq L_q$) lautet die MTPA-Bedingung:
    \[
        I_d^* = \frac{\psi_{\mathrm{PM}}}{4(L_q - L_d)}
               - \sqrt{\left(\frac{\psi_{\mathrm{PM}}}{4(L_q - L_d)}\right)^2 + \frac{I_s^2}{2}}.
    \]
    FÃ¼r einen reinen PMSM ohne Reluktanz ($L_d = L_q$) gilt $I_d^* = 0$, d.\,h.
    der gesamte Strom flieÃŸt in die $q$-Achse.
\end{lemma}
...
\begin{definition}[Feldorientierte Regelung]
    Die FOC transformiert die dreiphasigen StatorgrÃ¶ÃŸen $(a, b, c)$ Ã¼ber die
...
Im $dq$-System kÃ¶nnen $I_d$ und $I_q$ unabhÃ¤ngig mit klassischen PI-Reglern
geregelt werden, was eine entkoppelte Drehmoment- und Flussregelung ermÃ¶glicht.
...
\begin{itemize}
    \item \textbf{MTPA (Maximum Torque Per Ampere):} Minimiert Kupferverluste
          durch optimale Stromzeigerwahl im ungeschwÃ¤chten Betrieb.
    \item \textbf{FWC (Field Weakening Control):} Regelt $I_d$ im Ãœberdrehzahlbereich
          zur Einhaltung der Spannungsgrenze.
    \item \textbf{RTO (Real-Time Optimization):} Kombination beider Verfahren
          mit BerÃ¼cksichtigung von Temperaturmodellen und Wechselrichterverlusten
...
\begin{itemize}
    \item Im Konstantmomentbereich ($0 \leq n \leq n_{\mathrm{base}}$) liefert der
          Motor bei maximalem Strom konstantes Drehmoment; die Leistung wÃ¤chst linear.
    ...
    \item Die FeldschwÃ¤chung durch negativen $d$-Achsenstrom ermÃ¶glicht den Betrieb
          oberhalb der Eckdrehzahl ohne Ãœberschreitung der Wechselrichterspannung.
    \item Regeneratives Bremsen nutzt den Maschinenbetrieb im Generatormodus zur
          EnergierÃ¼ckgewinnung mit typisch $60$--$80\,\%$ Wirkungsgrad.
...
eine deutlich hÃ¶here Leistungsdichte bei reduzierter axialer BaulÃ¤nge. Die
Drehmoment-Drehzahl-Charakteristik von AFMs unterscheidet sich durch kÃ¼rzere
Flusslinien und grÃ¶ÃŸere aktive LuftspaltflÃ¤chen. Aktuelle Forschung fokussiert
auf die Optimierung der Scheibenmagnete aus NdFeB-Material sowie auf
additiv gefertigte Wicklungsgeometrien fÃ¼r minimale KupferfÃ¼llfaktor-Verluste.
Unternehmen wie YASA (Mercedes-Benz) und Magnax treiben die Skalierbarkeit von
AFMs fÃ¼r $>\SI{200}{\kilo\watt}$ voran~\cite{chalmers2011axial}.
...
und ermÃ¶glichen bei gleicher Leistung deutlich kleinere und leichtere Motoren.
...
und das FeldschwÃ¤chverhalten unterscheiden sich grundlegend von reinen PMSM.
Hybride aus SyRM und PMSM (IPMSM) erlauben durch Optimierung des VerhÃ¤ltnisses
$L_d/L_q$ eine maÃŸgeschneiderte Kennlinie fÃ¼r spezifische Fahrzeugklassen.
...
Faktor $5$--$10$ hÃ¶here Leistungsdichte. Die Haupthindernisse sind die
KÃ¼hlinfrastruktur (flÃ¼ssiger Stickstoff oder Helium) und die mechanische
Robustheit der Supraleiter unter dynamischen Betriebsbedingungen. NASA und
das EU-Projekt HYFLYERS entwickeln Prototypen fÃ¼r die Luftfahrt, wobei
Erkenntnisse auf EV-Anwendungen Ã¼bertragbar sind~\cite{filippenko2017superconducting}.

\subsection{Regelungsverfahren der nÃ¤chsten Generation}

\subsubsection{ModellprÃ¤diktive Regelung (MPC)}

Die modellprÃ¤diktive Regelung (Model Predictive Control, MPC) lÃ¶st online ein
Optimierungsproblem Ã¼ber einen PrÃ¤diktionshorizont und bietet gegenÃ¼ber
klassischer FOC folgende Vorteile:
\begin{itemize}
    \item Explizite Behandlung von Strom- und SpannungsbeschrÃ¤nkungen
          in der Optimierung,
    \item natÃ¼rliche Einbeziehung von MehrgrÃ¶ÃŸenabhÃ¤ngigkeiten
          ($dq$-Kopplung, SÃ¤ttigung),
    \item direkte Minimierung von Verlusten als Kostenfunktion
          statt indirekte MTPA-Approximation.
\end{itemize}
Finite-Control-Set MPC (FCS-MPC) ist echtzeitfÃ¤hig auf modernen DSPs und FPGAs
...
dass sie MTPA- und FeldschwÃ¤chungsstrategien adaptiv und ohne explizites
Motormodell erlernen kÃ¶nnen. Wesentliche Forschungsthemen sind:
\begin{itemize}
    \item Transferierbarkeit von Sim-to-Real: Lernstrategien aus Simulationsmodellen
          mÃ¼ssen robust gegenÃ¼ber Parameterunsicherheiten (SÃ¤ttigungscharakteristik,
          TemperaturabhÃ¤ngigkeit von $R_s$, $\psi_{\mathrm{PM}}$) sein.
...
Digitale Zwillinge (Digital Twins) des Traktionsmotors ermÃ¶glichen Echtzeit-
ZustandsschÃ¤tzung und prÃ¤diktive Wartung. Hochgenaue Finite-Elemente-Modelle
...
\subsubsection{Direkte FlÃ¼ssigkeitskÃ¼hlung der Statorwicklung}

Konventionelle WassermÃ¤ntel um das MotorgehÃ¤use fÃ¼hren WÃ¤rme ineffizient Ã¼ber
mehrere Schichten ab. Direkte KÃ¼hlung durch KÃ¼hlmittel im Wickelkopf (Hair-Pin-
Wicklung mit KÃ¼hlkanÃ¤len) oder durch dielektrische ImmersionskÃ¼hlung reduziert
die Wicklungstemperatur drastisch und erlaubt dauerhaft $30$--$50\,\%$ hÃ¶here
Strombelastung bei gleicher Motorgeometrie.

\subsubsection{WÃ¤rmemanagement der Permanentmagnete}

Permanentmagnete aus NdFeB verlieren reversibel KoerzitivfeldstÃ¤rke und
Remanenz bei erhÃ¶hter Temperatur und kÃ¶nnen irreversibel entmagnetisiert werden
($T > \SI{150}{\degreeCelsius}$ typisch). Thermische Modelle auf Basis
von Lumped-Parameter-Netzwerken (LPTN) mit $> 20$ Knoten ermÃ¶glichen die
EchtzeitÃ¼berwachung der Magnettemperatur aus messbaren GrÃ¶ÃŸen (Strom,
Spannung, GehÃ¤usetemperatur)~\cite{nategh2017thermal}.

\subsubsection{WÃ¤rmepumpenintegration}

Im Winterbetrieb kann der Traktionsmotor als WÃ¤rmelieferant fÃ¼r den
Fahrzeuginnenraum genutzt werden. Optimierte Verlustverteilung (gezielt erhÃ¶hte
Kupferverluste durch leicht suboptimale MTPA-Strategie) bei niedrigen Lasten
reduziert den Heizbedarf der WÃ¤rmepumpe und verbessert die Winterreichweite
um bis zu $15\,\%$.
...
deutlich hÃ¶here Schaltfrequenzen ($>\SI{100}{\kilo\hertz}$) bei geringeren
Durchlassverlusten als konventionelles IGBT-Silizium. Folgen fÃ¼r die
...
und dÃ¼nnere Kabeldurchmesser ermÃ¶glicht.
...
DrehmomentstÃ¶ÃŸe bei Gangwechsel (in mehrstufigen E-Getrieben) mÃ¼ssen durch
...
Fahrzeuge mit je einem Motor pro Rad oder Achse ermÃ¶glichen elektromagnetisches
Torque Vectoring: individuell steuerbares Rad-Drehmoment verbessert
FahrstabilitÃ¤t, Kurvengeschwindigkeit und Traktion ohne mechanisches
Sperrdifferential. Die Drehmoment-Drehzahl-Charakteristik jedes Motors muss
dann im Zusammenspiel optimiert werden; dies erfordert MehrgrÃ¶ÃŸen-Optimierer
auf Fahrzeugebene.

\subsubsection{Autonomes Fahren und prÃ¤diktives Energiemanagement}

Autonome Fahrzeuge kennen ihre Route, Topographie und den umgebenden
Verkehrsfluss im Voraus. PrÃ¤diktives Energiemanagement (look-ahead control)
kann Rekuperations- und Beschleunigungsmomente Ã¼ber den gesamten Fahrtweg
...
Die AbhÃ¤ngigkeit von NdFeB-Magneten stellt ein Versorgungsrisiko dar.
...
KoerzitivfeldstÃ¤rke eine Alternative. Forschung an hochkoerzitiven Ferritmagneten
...
Verfahren zur RÃ¼ckgewinnung von Nd, Dy und Pr kÃ¶nnten die VerfÃ¼gbarkeit
...
sich kontinuierlich weiter. ZukÃ¼nftige Normen werden voraussichtlich:
\begin{itemize}
    \item standardisierte Kennlinienformate fÃ¼r digitale Fahrzeugdatenbanken
          (ASAM MDF) definieren,
    \item Wirkungsgradklassen analog zu IE1--IE5 fÃ¼r EV-Motoren einfÃ¼hren,
    \item Mindestanforderungen an die Rekuperationseffizienz Ã¼ber standardisierte
          Fahrzyklen (WLTP, NEDC) festlegen,
    \item Sicherheitsanforderungen fÃ¼r ML-basierte Motorregler im Kontext
...
O.\ Gutfleisch, M.\,A.\ Willard, E.\ BrÃ¼ck, C.\,H.\ Chen, S.\,G.\ Sankar,
J.\,P.\ Liu,
