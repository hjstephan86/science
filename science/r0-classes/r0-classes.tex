...
\item $\beta$ = Transmissionsrate (Kontakte pro Kopf Ã— Übertragungswahrscheinlichkeit)
...
V & (4, âˆž) & Katastrophale Pandemie & Tage & Sehr schwierig \\
\hline
\end{tabular}
\end{table}
...
\caption{Klassifikationsspektrum zeigt die fünf Zonen mit Bifurkationspunkten (rot gestrichelt bei $\mathcal{R}_0 = 1$). Reale Pathogene sind markiert: HIV (2.0), EBOV (2.2), SARS-CoV-2 (2.5). Die farbliche Kodierung (rotâ†’gelbâ†’blauâ†’dunkelblau) zeigt steigende Kontrollschwierigkeit.}
\label{fig:plot1}
\end{figure}
...
\caption{Links: Vergleich von Antiviralia (15--75\%), Impfung (10--90\%), Kombinationstherapie (35--96\%) und Isolation (25--94\%) pro Zone. Rechts: Kosteneffizienz (â‚¬/QALY) zeigt exponentiellen Anstieg von Zone I zu V. Zeigt, dass höhere Zonen deutlich teurere Interventionen erfordern.}
\label{fig:plot3}
\end{figure}
...
\caption{Vier-Panel: (TL) $\mathcal{R}_0 âˆ \beta$ linear. (TR) $\mathcal{R}_0 âˆ 1/\gamma$ linear (längere Infektionsdauer â†’ höhere $\mathcal{R}_0$). (BL) Within-Host-Replikation satturierend. (BR) 2D-Heatmap mit Bifurkationskurve bei $\mathcal{R}_0 = 1$ (rot). Zeigt, dass mehrere Parameter-Kombinationen gleiche $\mathcal{R}_0$ erreichen.}
\label{fig:plot4}
\end{figure}
...
\caption{Vier-Panel: (TL) S-I Phasenebene für $\mathcal{R}_0 = 1.5$ mit Vektorfeld. (TR) Bifurkationsdiagramm: $I^* âˆ (\mathcal{R}_0 - 1) / \mathcal{R}_0$. (BL) Stabilitätskarte: DFE stabil (grün) für $\mathcal{R}_0 < 1$, endemic stabil (rot) für $\mathcal{R}_0 > 1$. (BR) Lyapunov-Exponenten: $\lambda < 0$ (stabil) für $\mathcal{R}_0 < 1$, $\lambda > 0$ (instabil) für $\mathcal{R}_0 > 1$.}
\label{fig:plot5}
\end{figure}
...
\begin{itemize}
\item Isolationskapazität pro Land (50 Betten Ã— 20 Länder): USD 50--100 Millionen Kapitalinvestment
...
\item Summe: 65\% â†’ $\mathcal{R}_0^{\text{effect}} = 0.35 \times 2.5 = 0.875$ (nahe Elimination)
\end{itemize}
...
\item Summe: 80\% â†’ $\mathcal{R}_0^{\text{effect}} = 0.20 \times 10 = 2.0$ (noch endemisch, aber kontrolliert)
\end{itemize}
...
\item \textbf{Variantenhertzstellung:} Bei längeren Epidemien entstehen Varianten mit erhöhtem R-null (wie SARS-CoV-2 Alpha â†’ Delta â†’ Omicron). Das Rahmenwerk kann dies nur statisch behandeln.
...
\item \textbf{Public Communication:} Die R-null-Klassifikation als Kommunikations-Tool zur Öffentlichkeit nutzen (ähnlich Hurricane-Kategorie-System). ``Wir sind in Zone IV â†’ verstärkte Maßnahmen ab morgen''.
