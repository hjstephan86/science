...
Abschnitt~\ref{sec:foundations} legt die mathematischen Grundlagen (Agenten, MAS, Trajektoriendefinitionen). Abschnitt~\ref{sec:trajectory} definiert die Drosophila-Trajektorie und ihre Lemniskatenform formal. Abschnitt~\ref{sec:energy} entwickelt das Energiemodell mit Fahrtwind-Induktionsfedern. Abschnitt~\ref{sec:cfta} entwickelt das MAS-Koalitionsmodell für LEO-Satelliten und die zugehörige Koalitionsbildungsstrategie. Abschnitt~\ref{sec:subgraph} führt die polynomielle Reduktion auf den Subgraph Algorithmus durch. Abschnitt~\ref{sec:lsat} beschreibt das LSAT-Entscheidungsverfahren. Abschnitt~\ref{sec:simulation} präsentiert Simulation und experimentelle Ergebnisse mit matplotlib-Visualisierungen. Abschnitt~\ref{sec:outlook} gibt einen sehr ausführlichen Ausblick. Die Zusammenfassung schließt die Arbeit ab.
