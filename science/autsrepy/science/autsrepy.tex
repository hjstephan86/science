...
Steigerung der SoftwareproduktivitÃ¤t sowie eine breitere Entwicklerbasis.
...
Diese Arbeit argumentiert auf Basis technischer, Ã¶konomischer, Ã¶kosystemischer und
sicherheitstechnischer Analyse, dass \emph{Python} -- insbesondere in der AusprÃ¤gung
\emph{MicroPython}~\cite{micropython_main} -- die strategisch Ã¼berlegene Wahl fÃ¼r
eine AUTOSAR-Standardisierung darstellt.
Python bietet die weltgrÃ¶ÃŸte Entwicklercommunity, konkurrenzlose
Ã–kosystemreife, erstklassige KI/ML-Integration, ausgezeichnete Testbarkeit
und etablierte Einsatznachweise in sicherheitskritischen Systemen.
MicroPython ermÃ¶glicht dabei den direkten Einsatz auf ressourcenbeschrÃ¤nkten
Mikrocontrollern, wie sie in der \gls{autosar}-Classic-Plattform typisch sind,
wÃ¤hrend CPython in der Adaptive Platform nahtlos integrierbar ist.

Wir entwerfen eine vollstÃ¤ndige Standardisierungsarchitektur, definieren
...
umsetzbarer Fahrplan fÃ¼r die Aufnahme von Python/MicroPython als normierte
Sprache in AUTOSAR~R24 und darÃ¼ber hinaus.

Die Geschichte der Automobilelektronik ist eine kontinuierliche Eskalation
der SoftwarekomplexitÃ¤t. WÃ¤hrend ein Fahrzeug der 1980er-Jahre kaum
Software enthielt, umfassen moderne Fahrzeuge Ã¼ber 100 \glspl{ecu},
...
Der Ãœbergang zum Software-defined Vehicle (SDV) intensiviert diesen Trend
weiter: Fahrzeugfunktionen sollen kÃ¼nftig primÃ¤r durch Software definiert,
...
\gls{autosar} -- gegrÃ¼ndet 2003 von BMW, Bosch, Continental, DaimlerChrysler,
Siemens VDO und Volkswagen -- definiert eine offene, standardisierte
Softwarearchitektur fÃ¼r SteuergerÃ¤te im Automobil~\cite{autosar_overview2017}.
Die Classic Platform (\gls{cp}) zielt auf Mikrocontroller mit begrenzten
Ressourcen; die 2017 eingefÃ¼hrte Adaptive Platform (\gls{ap}) adressiert
leistungsfÃ¤hige Hochleistungsrechner mit POSIX-kompatiblem Betriebssystem~\cite{autosar_ap_explained2019}.

Beide Plattformen spezifizieren bislang C (fÃ¼r CP) bzw.\ C++14/17 (fÃ¼r AP)
als primÃ¤re Implementierungssprachen.
Angesichts bekannter SchwÃ¤chen dieser Sprachen bezÃ¼glich
...
Rust besitzt zweifellos VorzÃ¼ge in Bezug auf Speichersicherheit durch sein
...
unreifes Tooling und fehlende Normnachweise fÃ¼r sicherheitskritische Anwendungen.

\textbf{Diese Arbeit positioniert Python/MicroPython als die Ã¼berlegene Alternative.}
WÃ¤hrend Rust eine nische Systemprogrammiersprache bleibt, ist Python die
meistgenutzte Programmiersprache der Welt~\cite{tiobe2025, stackoverflow_survey2024}
mit einem Ã–kosystem, das Machine Learning, formale Verifikation,
Testautomatisierung, Protokoll-Scripting und Hardware-nahe Programmierung
gleichermaÃŸen abdeckt.
MicroPython~\cite{micropython_main, george2014micropython} -- eine schlanke,
auf Mikrocontroller optimierte Python-3-Implementierung -- beweist tÃ¤glich
in Millionen industrieller und akademischer Anwendungen, dass Python
auch auf ressourcenbeschrÃ¤nkter Hardware praktikabel ist.
...
\begin{enumerate}
  \item Welche technischen Anforderungen stellt \gls{autosar} an eine Programmiersprache,
        und inwiefern erfÃ¼llt Python/MicroPython diese?
  \item Wie lÃ¤sst sich ein Python-Sprachprofil fÃ¼r AUTOSAR sicherheitstechnisch
        spezifizieren, das ISO~26262 konform ist?
  \item Welche Architektur ermÃ¶glicht die Integration von MicroPython in die
        AUTOSAR Classic Platform und Python in die Adaptive Platform?
  \item Welche wirtschaftlichen und Ã¶kosystemischen Vorteile entstehen durch
        eine Python-Standardisierung?
  \item Wie verhÃ¤lt sich Python/MicroPython im Vergleich zu Rust hinsichtlich
        der fÃ¼r AUTOSAR relevanten Kriterien?
\end{enumerate}

Der Rest dieser Arbeit ist wie folgt strukturiert:
Abschnitt~\ref{sec:autosar} erlÃ¤utert die \gls{autosar}-Architektur im Detail.
...
Abschnitt~\ref{sec:oekosystem} diskutiert das Ã–kosystem.
...
Abschnitt~\ref{sec:fazit} schlieÃŸt mit Fazit und Ausblick.
...
\gls{autosar} wurde 2003 als industrielles Konsortium gegrÃ¼ndet~\cite{autosar_overview2017},
um die wachsende Fragmentierung der SteuergerÃ¤te-Software zu beenden.
Vor AUTOSAR pflegte jeder OEM und jeder Tier-1-Zulieferer eigene,
inkompatible Software-Stacks fÃ¼r jede Derivatefamilie.
...
CP spezifiziert C (ISO~C99, ergÃ¤nzt durch MISRA~C~\cite{misra_c2012})
als Implementierungssprache. Der AUTOSAR~CP-Stack umfasst Ã¼ber 80~Module,
...
\begin{itemize}
  ...
  \item Ethernet/ SOME/IP als primÃ¤rem Kommunikationsbus
...
Aus den AUTOSAR-Spezifikationen und den einschlÃ¤gigen Normen
...
\caption{Anforderungskatalog fÃ¼r Programmiersprachen in AUTOSAR}
...
R3  & Statische Analyse mÃ¶glich (TypprÃ¼fung, Kontrollfluss) & MISRA~C \\
R4  & Definiertes Speichermodell; keine unkontrollierten Heap-Operationen & ISO~26262 \\
R5  & Sprachsubset definierbar fÃ¼r ASIL-B/C/D-Anwendungen & ISO~26262-6 \\
R6  & Formale oder halbformale Verifikation unterstÃ¼tzt & IEC~61508 \\
R7  & Breite Toolchain-UnterstÃ¼tzung (Debugger, Profiler) & Praxis \\
R8  & Hardware-nahe Programmierung mÃ¶glich (Register, DMA, Interrupts) & CP-Anforderung \\
R9  & InteroperabilitÃ¤t mit C/C++ (FFI) & Migration \\
R10 & Ausreichend groÃŸe, in Automotive erfahrene Entwicklercommunity & OEM-Anforderung \\
R11 & Offene Lizenzierung (keine proprietÃ¤ren Lock-ins) & AUTOSAR-Statut \\
...
% ================================================================
\section{Python und MicroPython -- Ãœberblick und Eigenschaften}
\label{sec:python_micropython}

\subsection{Python -- Die universelle Hochsprache}
\label{subsec:python}

Python wurde 1991 von Guido van Rossum verÃ¶ffentlicht~\cite{vanrossum1995python}
und hat sich zur meistgenutzten Programmiersprache der Welt entwickelt~\cite{tiobe2025}.
Die Sprache vereint Prinzipien von Lesbarkeit, AusdrucksstÃ¤rke und
ProduktivitÃ¤t. Charakteristisch sind:
...
\begin{itemize}
  ...
  \item \textbf{Garbage Collected Memory Management} mit ReferenzzÃ¤hlung als primÃ¤rem
        Mechanismus (deterministischer als Mark-and-Sweep fÃ¼r kurzlebige Objekte);
  \item \textbf{Klare, erzwungene Syntaxstruktur} durch einrÃ¼ckungsbasierte BlÃ¶cke;
  \item \textbf{Erstklassige Funktionen, Closures, Generatoren} und
        \textbf{async/await}-Coroutinen (seit Python~3.5);
  \item \textbf{Umfangreiches Standardbibliotheks-Ã–kosystem} (Ã¼ber 200~000 Pakete
        auf PyPI~\cite{pypi_stats2024});
  \item \textbf{C-Erweiterbarkeit} via CPython C-API oder Cython~\cite{cython_book};
  \item \textbf{VollstÃ¤ndige Unicode-UnterstÃ¼tzung} (PEP~3120).
\end{itemize}

\subsubsection{Statische TypprÃ¼fung und Verifikation}
...
und \texttt{pyre} (Meta) bieten vollstÃ¤ndige statische TypprÃ¼fung.
Kombiniert mit strikten Typ-Annotationen (PEP~484, PEP~526, PEP~544)
lÃ¤sst sich Python-Code auf einem Niveau typprÃ¼fen, das C++
mit modernen Compilern vergleichbar ist~\cite{gros2021python_types}.

FÃ¼r sicherheitskritische Anwendungen ist dies entscheidend:
Ein AUTOSAR-SWC kÃ¶nnte mit vollstÃ¤ndiger Typ-Annotation versehen werden,
und die statische Analyse via mypy/pyright wÃ¼rde Typfehler bereits
...
\begin{itemize}
  \item \textbf{MicroPython}: FÃ¼r Mikrocontroller (s.\ Abschnitt~\ref{subsec:micropython});
...
Insbesondere \textbf{Nuitka} und \textbf{mypyc} sind fÃ¼r die AUTOSAR-Adaptive-Platform
hochrelevant: Sie ermÃ¶glichen die Erstellung von nativen ELF-Binaries aus
Python-Quellcode, die direkt in POSIX-ECUs deploybar sind, ohne
einen Interpreter im Produktionssystem laufen zu lassen.

\subsection{MicroPython -- Python fÃ¼r Mikrocontroller}
...
Es ist eine vollstÃ¤ndige, schlanke Python-3-Implementierung
speziell fÃ¼r ressourcenbeschrÃ¤nkte Hardware~\cite{micropython_main}:

\begin{itemize}
  \item \textbf{Minimaler Flash-Bedarf}: ca.\ 256~kB (Basis-Interpreter);
        typische Deployment-GrÃ¶ÃŸe 256--512~kB Flash;
  \item \textbf{Minimaler RAM-Bedarf}: lauffÃ¤hig ab 16~kB RAM,
        typisch 64--256~kB;
  \item \textbf{Python-3-KompatibilitÃ¤t}: Kern-Sprachfeatures vollstÃ¤ndig
  ...
  \item \textbf{Inline-Assembler} fÃ¼r ARM Thumb2, Xtensa und STM32;
  ...
  \item \textbf{Viper-Code-Emitter}: \texttt{@micropython.viper}-Decorator
        mit C-Ã¤hnlichem Typsystem fÃ¼r hÃ¶chste Performance;
  \item \textbf{Integrierter Garbage Collector} (Mark-and-Sweep mit
        konfigurierbarer Heap-GrÃ¶ÃŸe und manuell triggerbarer Kollektion).
\end{itemize}

\subsubsection{UnterstÃ¼tzte Mikrocontroller-Plattformen}
MicroPython unterstÃ¼tzt eine breite Plattformpalette~\cite{micropython_ports2023}:

\begin{table}[h]
\centering
\caption{Von MicroPython offiziell unterstÃ¼tzte Plattformen (Auswahl)}
...
Die Performance-Eigenschaften von MicroPython sind fÃ¼r eingebettete Systeme
gut untersucht~\cite{micropython_perf_papanastasiou2021, python_embedded_benchmark2022}:

\begin{itemize}
  \item StandardmÃ¤ÃŸiger bytecode-interpretierter Python-Code:
        ca.\ 10--50$\times$ langsamer als Ã¤quivalenter C-Code;
  \item Mit \texttt{@micropython.native}: ca.\ 2--5$\times$ langsamer als C;
  \item Mit \texttt{@micropython.viper}: annÃ¤hernd C-Geschwindigkeit (1--2$\times$);
  \item Inline-Assembler: gleichwertig zu handoptimiertem Assembler.
\end{itemize}

Dies ist fÃ¼r nicht-zeitkritische Schichten von AUTOSAR-Software
(Diagnose, Konfiguration, OTA-Management, Kommunikationsprotokoll-Glue-Code)
vollkommen ausreichend und fÃ¼r zeitkritische Schichten durch
native/viper-Dekoratoren oder C-Extensions lÃ¶sbar.

\subsubsection{MicroPython Speicherverwaltung im Detail}
\label{subsubsec:gc}

Das Speicherverwaltungsmodell von MicroPython ist fÃ¼r Embedded Systems ausgelegt:
...
\begin{itemize}
  ...
  \item \textbf{Deterministisches GC}: GC kann deaktiviert und manuell Ã¼ber
        \texttt{gc.collect()} ausgelÃ¶st werden;
  \item \textbf{Worst-Case Execution Time (WCET)}: Mit deaktiviertem GC und
        viper-Decorator ist WCET analysierbar;
  \item \textbf{Pre-Allokation}: Kritische Objekte kÃ¶nnen zur Boot-Zeit
        pre-allokiert und aus dem GC ausgeschlossen werden (\texttt{micropython.const});
  \item \textbf{Frozen Modules}: Python-Module kÃ¶nnen als Bytecode in Flash
        eingefroren werden (kein RAM fÃ¼r Quellcode nÃ¶tig).
\end{itemize}

\begin{lstlisting}[language=MicroPython,caption={MicroPython Speichermanagement fÃ¼r zeitkritische Funktionen},label=lst:gc_control]
import gc
import micropython
...
\subsection{Die Rust-Bewegung im Ãœberblick}
Rust~\cite{rust_book2022} wurde von Mozilla Research entwickelt und 2015
in Version~1.0 verÃ¶ffentlicht. Seine Haupteigenschaft ist das Ownership-System,
das Speichersicherheit ohne Garbage Collector zur Compilezeit garantiert.
Dies macht Rust attraktiv fÃ¼r Systemprogrammierung.

Im Automobilbereich wurde eine AUTOSAR-Arbeitsgruppe fÃ¼r Rust gebildet~\cite{autosar_wg_rustsafety},
...
\subsection{Kritische Analyse der Rust-Eignung fÃ¼r AUTOSAR}
\label{subsec:rust_kritik}

Trotz dieser AktivitÃ¤ten bestehen fundamentale Hindernisse:
...
WÃ¤hrend Python nach TIOBE~\cite{tiobe2025} und dem
Stack Overflow Survey~\cite{stackoverflow_survey2024} weltweit die
meistgenutzte Sprache ist (ca.\ 30~Mio. aktive Entwickler), zÃ¤hlt
Rust nur ca.\ 3~Mio. Entwickler weltweit -- davon ein Bruchteil
mit Automotive-Erfahrung. Der resultierende Entwicklermangel
verteuert Projekte und verlÃ¤ngert Time-to-Market erheblich.

\subsubsection{Fehlendes Safety-Zertifizierungs-Ã–kosystem}
ISO~26262-8 verlangt fÃ¼r Sprachtools einen
\emph{Tool Confidence Level}~\cite{iso26262_2018}.
FÃ¼r Rust existiert (Stand 2025) kein zertifizierter Compiler
(nach ISO~26262 ASIL-D qualifiziert), wÃ¤hrend GCC und LLVM/Clang
seit Jahren entsprechend qualifiziert werden (z.B.\ durch
AbsInt, AdaCore, ETAS).
MicroPython und CPython kÃ¶nnen auf qualifizierte C-Backends
...
diagnostischer Speicherzugriff) existieren fÃ¼r C Ã¼ber Jahrzehnte.
FÃ¼r Rust sind automotive-grade BSW-Bindungen weitgehend nicht existent
und mÃ¼ssten von Grund auf neu entwickelt werden -- ein
Jahrzehnt-Projekt fÃ¼r die gesamte Industrie.

\subsubsection{`unsafe` Rust: kein Gewinn gegenÃ¼ber C}
Viele Hardware-nahe Operationen in Automotive-ECUs erfordern
zwingend \texttt{unsafe}-Rust-BlÃ¶cke~\cite{rust_unsafe2021}.
...
\begin{table}[h]
\centering
\caption{Vergleich Rust vs.\ Python/MicroPython fÃ¼r AUTOSAR-Standardisierung}
...
Compiler-Qualifizierung (ISO~26262) & nicht vorhanden & via GCC/LLVM mÃ¶glich \\
AUTOSAR BSW-Bindungen & keine & C-FFI, erste Bibliotheken \\
MISRA-Ã¤quivalentes Profil & MISRA Rust (Draft) & PyMISRA-AnsÃ¤tze (forschbar) \\
Embedded-Footprint & 50--500~kB (no\_std) & 256~kB (MicroPython) \\
KI/ML-Integration & rudimentÃ¤r & erstklassig (PyTorch, TensorFlow) \\
OTA-Scripting & nicht vorgesehen & nativ via Python-Scripts \\
TestÃ¶kosystem & wachsend & ausgereift (pytest, hypothesis) \\
Statische Analyse & sehr gut & sehr gut (mypy, pyright) \\
Formale Verifikation & experimentell & SPARK-Ã¤hnlich via typsichere Teilmengen \\
...
% ================================================================
\section{Technische Eignung von Python/MicroPython fÃ¼r AUTOSAR}
\label{sec:technische_eignung}

\subsection{ErfÃ¼llung des Anforderungskatalogs}
\label{subsec:anforderungen_erfuellung}

Wir prÃ¼fen systematisch, inwieweit Python/MicroPython die in
Tabelle~\ref{tab:anforderungen} definierten Anforderungen erfÃ¼llt:
...
Inline-Assembler fÃ¼r zeitkritische Pfade.
...
erzielbar. FÃ¼r ASIL-D-Funktionen wÃ¤re eine static-alloc-only-Bibliothek
...
Ã¼bersetzt wird. MicroPython wird ebenfalls mit GCC/Clang kompiliert.
...
ermÃ¶glichen vollstÃ¤ndige statische TypprÃ¼fung. ZusÃ¤tzlich bieten
\texttt{pylint}~\cite{pylint2023}, \texttt{flake8}, \texttt{bandit}
(Sicherheitsanalyse) und \texttt{pyflakes} umfangreiche Codeanalyse.
FÃ¼r AUTOSAR-spezifische Regeln wÃ¤re ein \emph{AUTOSAR Python Checker}
...
Keine Interaktion mit OS-Heap. VollstÃ¤ndig kontrollierbar.
FÃ¼r Adaptive Platform: Python mit speziell allokierten arenas
(CPython configurierbar via \texttt{pymalloc} oder
custom allocator~\cite{cpython_allocator2022}).

\paragraph{R5 -- Sprachsubset fÃ¼r ASIL}
Python besitzt eine klar definierte, kleine Kernsprache.
Ein ASIL-Sprachsubset (``PySafety-Profile'') ist eindeutig definierbar:
Verboten wÃ¤ren z.B.\ \texttt{eval()}, \texttt{exec()}, dynamischer Import,
und unbeschrÃ¤nkte Rekursion. Erlaubt wÃ¤ren typisierte Funktionen,
Pre-allokierte Datenstrukturen und native-Dekoratoren.
(s.~Abschnitt~\ref{sec:sprachprofil})

\paragraph{R6 -- Formale Verifikation}
FÃ¼r typisierte Python-Teilmengen existieren AnsÃ¤tze zu
...
Python verfÃ¼gt Ã¼ber ausgezeichnete Toolchain-UnterstÃ¼tzung:
Eclipse/VS~Code-Integration, REPL fÃ¼r interaktives Debugging,
...
\paragraph{R9 -- InteroperabilitÃ¤t mit C/C++}
MicroPython unterstÃ¼tzt native C-Module (\texttt{.c}-Dateien, die als
MicroPython-Module kompiliert werden)~\cite{micropython_cmodules2023}.
CPython bietet die vollstÃ¤ndige C-API sowie \texttt{ctypes},
\texttt{cffi} und Cython. Bestehende AUTOSAR-BSW-C-Bibliotheken
kÃ¶nnen direkt von Python aus aufgerufen werden.

\paragraph{R10--R15}
Alle weiteren Anforderungen werden in den folgenden Abschnitten
(Sicherheit, Architektur, Ã–kosystem) detailliert behandelt.
...
die MicroPython komfortabel unterstÃ¼tzen:
...
Die Ressourcen moderner Automotive-MCUs Ã¼bertreffen die MicroPython-Mindestanfor\-derungen
um einen Faktor von 16--64$\times$ (Flash) bzw.\ 8--128$\times$ (RAM).
Selbst ohne die gesamte Python-Stdlib ist genÃ¼gend Platz fÃ¼r
...
Interrupt-Service-Routinen (\glspl{isr}) sind der hardwarenÃ¤chste Teil des Systems.
MicroPython bietet mehrere MÃ¶glichkeiten:
...
\begin{itemize}
  \item C-ISR Latenz: $<$1~Âµs
  \item Native Python-ISR Latenz: 2--5~Âµs
  \item Deferred Python-Callback: 10--50~Âµs (abhÃ¤ngig von Scheduling)
\end{itemize}

FÃ¼r AUTOSAR-CP-Anwendungen mit typischen Zykluszeiten von
1~ms (Task~Class~1) bis 100~ms (Task~Class~3) sind diese Latenzen
vollkommen akzeptabel. Nur harte Echtzeit-ISRs ($<$1~Âµs) wÃ¼rden
...
und ihrer Laufzeitumgebung flieÃŸt in den Safety Case ein.
...
\begin{definition}[PySafety Level]
  ...
  von Python/MicroPython, das fÃ¼r eine bestimmte ASIL-Klasse einsetzbar ist.
\end{definition}
...
\bottomrule
\endfoot
PSL-0 & VollstÃ¤ndiges Python & -- & QM \\
PSL-1 & Python ohne eval/exec, ohne dynamischen Import & eval, exec, \_\_import\_\_ & ASIL-A \\
PSL-2 & PSL-1 + vollstÃ¤ndige Typ-Annotationen, kein \texttt{*args}/\texttt{**kwargs} ohne Typen & Dynamische Attribute, getattr ohne Typcheck & ASIL-B \\
PSL-3 & PSL-2 + nur pre-allokierte Container, kein unbeschrÃ¤nkter Heap & list(), dict() ohne GrÃ¶ÃŸenbeschrÃ¤nkung, offene Rekursion & ASIL-C \\
...
\subsubsection{Coding Guidelines und statische PrÃ¼fung}
Analog zu MISRA~C~\cite{misra_c2012} wÃ¤ren \textbf{AUTOSAR~Python~Guidelines} zu definieren.
Erste EntwÃ¼rfe kÃ¶nnten auf bestehenden Arbeiten aufbauen:
\begin{itemize}
  ...
  \item \textbf{NASA PySafe}: NASA's Python-Codierungsrichtlinien
        fÃ¼r sicherheitskritische Systeme~\cite{nasa_python2011};
  \item \textbf{CERT~Secure Coding Standard fÃ¼r Python}~\cite{cert_python2019}.
\end{itemize}
...
Python bietet hier signifikante Vorteile gegenÃ¼ber C/C++:
...
\begin{itemize}
  ...
  \item \textbf{Integer-Overflow frei}: Python's \texttt{int} ist beliebig groÃŸ
...
FÃ¼r Automotive gelten besondere Anforderungen:
\begin{itemize}
...
\subsection{WCET-Analyse und EchtzeitfÃ¤higkeit}
\label{subsec:wcet}

Worst-Case Execution Time Analysis (WCET) ist fÃ¼r ASIL-C/D-Funktionen
zwingend erforderlich~\cite{wilhelm2008wcet}.
FÃ¼r Python/MicroPython-Code ist WCET auf drei Ebenen mÃ¶glich:

\begin{enumerate}
  \item \textbf{Viper-Level}: Code mit \texttt{@micropython.viper} ist
        strukturell Ã¤quivalent zu C-Code und kann mit etablierten
        ...
        wie Ã¤quivalentes C kompiliert;
  \item \textbf{Bytecode-Level}: FÃ¼r bytecode-interpretierten Python-Code
  ...
  \item \textbf{Partitionierung}: Zeitkritische Funktionen in C, zeitunkritische
        in Python -- bewÃ¤hrter Praxis-Ansatz.
\end{enumerate}
...
\subsection{Ãœberblick: Python in der AUTOSAR-Schichtenarchitektur}
...
FÃ¼r die AUTOSAR Classic Platform wird folgende Integrationsarchitektur vorgeschlagen:
...
Die MicroPython-\gls{vm} wird als eigenstÃ¤ndiger AUTOSAR-Task unter
...
CPython (oder Nuitka-kompiliertes Python) lÃ¤uft als nativer POSIX-Prozess.
...
Die Behauptung, Python sei fremd fÃ¼r das AUTOSAR-Ã–kosystem, ist falsch.
Das Open-Source-Projekt \texttt{autosar}~\cite{pyautosar_gh} (frÃ¼her \texttt{pyautosar})
...
Eines der wichtigsten Anwendungsszenarien fÃ¼r Python in AUTOSAR ist
das Over-the-Air-Update-Management~\cite{ota_autosar_ap2021}.
Python's StÃ¤rken kommen hier besonders zum Tragen:
...
% ================================================================
\section{Das Python-Ã–kosystem im Automotive-Kontext}
\label{sec:oekosystem}

\subsection{WeltgrÃ¶ÃŸtes Entwickler-Ã–kosystem}
\label{subsec:entwickler}

Python ist gemÃ¤ÃŸ Stack Overflow Developer Survey 2024~\cite{stackoverflow_survey2024}
die meistgenutzte Programmiersprache unter professionellen
Entwicklern (ca.\ 51\% General-Use).
Der Python Package Index (PyPI)~\cite{pypi_stats2024} zÃ¤hlt Ã¼ber
...
Python ist die Standardsprache fÃ¼r Testautomatisierung im Automotive-Bereich:

\begin{itemize}
  \item \textbf{pytest}~\cite{pytest2023}: Defacto-Standard fÃ¼r Unit- und
        Integrationstests; erweiterbar Ã¼ber Plugins;
  \item \textbf{hypothesis}~\cite{hypothesis2019}: Property-based Testing,
        Ã¤quivalent zu QuickCheck in Haskell;
...
FÃ¼r AUTOSAR-Anwendungen bedeutet dies: Eine in Python implementierte
SWC kann mit denselben Testetools getestet werden wie das Ã¼brige
Python-Produktionscode-Ã–kosystem -- ohne zusÃ¤tzliche Adapterschichten.
...
(ADAS, prÃ¤diktive Wartung, Spracherkennung, Anomalie-Detektion)
erfordert eine Sprache, die ML-Frameworks nativ unterstÃ¼tzt.
Python ist die Sprache des Machine Learning~\cite{ml_python_survey2023}.

FÃ¼r Embedded wird das Ã–kosystem durch folgende Projekte ergÃ¤nzt:
\begin{itemize}
  \item \textbf{TensorFlow Lite for Microcontrollers}~\cite{tflite_micro2021}:
        C-Backend, Python-Frontend fÃ¼r Training und Konvertierung.
        MicroPython-kompatibles Inference-Interface verfÃ¼gbar;
  \item \textbf{ONNX Runtime}~\cite{onnxruntime2019}: Python-API fÃ¼r
...
Ein durchgÃ¤ngiger Python-Stack von der Datenaufzeichnung im Fahrzeug
(MicroPython) Ã¼ber Cloud-Training (PyTorch) bis zur Deployment-Verifikation
(pytest) und RÃ¼ckspielen des Modells (ONNX/TFLite) wÃ¤re
...
\begin{itemize}
  ...
  \item \textbf{Vector CANoe Python}: Scripting-Schnittstelle fÃ¼r
        Automotive-Testinfrastruktur;
  \item \textbf{INCA/MCD Python API}: ETK/XETK-Zugriff fÃ¼r Kalibrierung;
  \item \textbf{Autosar-Explorer}: ARXML-Analyse und Visualisierung.
\end{itemize}

Eine Standardisierung von Python in AUTOSAR wÃ¼rde diese bereits
...
\subsection{EntwicklerproduktivitÃ¤t}
...
bei Ã¤quivalenter FunktionalitÃ¤t~\cite{prechelt2000productivity, lutz2013learning}.
FÃ¼r AUTOSAR-Projekte, die Millionen von Euro in Software-Entwicklung
investieren, bedeutet dies erhebliche Kosteneinsparungen.

Eine konservative SchÃ¤tzung fÃ¼r ein mittelgroÃŸes AUTOSAR-Projekt
(5 ECUs, 20 SWCs, 2 Jahre Entwicklungszeit, 10 Ingenieure):
\begin{itemize}
  \item \textbf{C/C++ Baseline}: 10 Ingenieure $\times$ 2 Jahre $\times$ 150~kâ‚¬/Jahr = 3~Mio.\ â‚¬
  \item \textbf{Python (50\% schneller)}: 10 Ingenieure $\times$ 1.3 Jahre = 2~Mio.\ â‚¬ (\textbf{--33\%})
  \item \textbf{Fehlerreduktion} (weniger Buffer-Overflows, Typfehler): zus.\ --15~\%
  \item \textbf{Gesamteinsparung}: $\approx$1.5~Mio.\ â‚¬ pro Projekt
\end{itemize}

\subsection{Talentpool und Rekrutierung}
\label{subsec:talentpool}

Der globale Entwicklermangel ist ein kritisches Problem fÃ¼r die Automobilindustrie.
Mit Python als AUTOSAR-Sprache:
\begin{itemize}
  \item Zugang zu 30+ Millionen Python-Entwicklern weltweit;
  \item KÃ¼rzere Einarbeitungszeit neuer Mitarbeiter (Python bekannt aus
  ...
  \item AttraktivitÃ¤t fÃ¼r junge Absolventen (Python meistgelehrte
        Erstkurssprache an UniversitÃ¤ten~\cite{python_courses2023}).
\end{itemize}

Mit Rust als AUTOSAR-Sprache wÃ¤re der Talentpool auf $<$3~Mio.\ Entwickler
beschrÃ¤nkt, von denen nur ein kleiner Prozentsatz Automotive-Interesse hat.

\subsection{Reduzierte Toolkost}
\label{subsec:toolkosten}

Python-Entwicklung erfordert keine teuren proprietÃ¤ren Tools:
\begin{itemize}
  ...
  \item Vergleich: C++-Toolchain fÃ¼r Automotive inkl.\ Compiler-Qualifizierung:
        50~000--500~000~â‚¬/Lizenz (ETAS, Wind River, Green Hills).
\end{itemize}
...
\begin{itemize}
  \item \textbf{AUTOSAR\_SWS\_PythonRuntime}: Software Specification fÃ¼r
        Python/MicroPython Runtime in AUTOSAR;
  \item \textbf{AUTOSAR\_SWS\_PythonRTE}: Python-Binding fÃ¼r AUTOSAR RTE;
...
\begin{itemize}
  \item AusgewÃ¤hlte nicht-sicherheitskritische SWCs (QM, ASIL-A/B)
...
\begin{itemize}
  \item Aufnahme in AUTOSAR~R30 als normative, optionale Sprache
        (gleichrangig zu C++ fÃ¼r AP, als Alternative zu C fÃ¼r CP);
  \item Zertifizierungspfad fÃ¼r MicroPython-Interpreter (Tool~TCL2);
  \item AUTOSAR-Python-Zertifizierung fÃ¼r Tools und Schulungen.
\end{itemize}
...
kÃ¼nftigen AUTOSAR Software Specification fÃ¼r Python Runtime:
...
\bottomrule
\endfoot
PY\_00001 & SPEC & Die Python-Runtime fÃ¼r AUTOSAR CP SHALL MicroPython~1.25 oder neuer implementieren. \\
...
PY\_00004 & SPEC & Python-SWCs SOLLEN vollstÃ¤ndige Typ-Annotationen (PEP~484) enthalten. \\
PY\_00005 & SPEC & FÃ¼r ASIL-B und hÃ¶her MUSS PSL-2 oder hÃ¶her angewendet werden. \\
PY\_00006 & SPEC & FÃ¼r ASIL-D MUSS PSL-4 angewendet werden und ein WCET-Nachweis fÃ¼r alle kritischen Pfade vorliegen. \\
PY\_00007 & SPEC & Die PySafety-Runtime SHALL eine sichere Sandbox fÃ¼r SWC-AusfÃ¼hrung bereitstellen. \\
...
PY\_00010 & SPEC & Frozen-Module (Bytecode im Flash) SOLLEN fÃ¼r alle produktiven SWCs verwendet werden. \\
\end{longtable}

\subsection{Referenz-Implementierung: PyAutosar}
\label{subsec:pyautosar_impl}

Als KernstÃ¼ck des Standardisierungsvorschlags empfehlen wir die
...
\begin{enumerate}
  ...
  \item \textbf{pyautosar-rte}: Python-Bindings fÃ¼r alle normativen RTE-APIs;
  \item \textbf{pyautosar-bsw}: Python-Wrapper fÃ¼r ausgewÃ¤hlte BSW-Module
        (COM, NVM, DET, DEM, DCM);
  \item \textbf{pyautosar-tools}: Code-Generator fÃ¼r SWC-Stubs aus ARXML,
        PSL-Checker (Lint-Plugin), Coverage-Integration;
  \item \textbf{pyautosar-ap}: ara-Bindings fÃ¼r die Adaptive Platform.
\end{enumerate}
...
Die Verwendung von Python auf ressourcenbeschrÃ¤nkten Systemen ist
aktiv erforscht. Papanastasiou et al.~\cite{papanastasiou2021micropython} analysierten
Performance und Eignung von MicroPython fÃ¼r industrielle Steuerungssysteme
und kamen zu dem Schluss, dass MicroPython fÃ¼r nicht-zeitkritische Aufgaben
vollstÃ¤ndig geeignet ist. Tolentino und Galvez~\cite{tolentino2019micropython} zeigten,
dass MicroPython erfolgreich in der GebÃ¤udeautomation eingesetzt werden kann.

\subsection{Sprachsicherheit und Safety}

Gros et al.~\cite{gros2021python_types} untersuchen systematisch die MÃ¶glichkeiten
statischer TypprÃ¼fung in Python und zeigen, dass modernes getyptes Python
bezÃ¼glich statischer AnalysequalitÃ¤t mit Java vergleichbar ist.
Eilers und MÃ¼ller~\cite{nagini2018} prÃ¤sentieren Nagini, ein formales Verifikationssystem
fÃ¼r typisiertes Python basierend auf der Viper-IntermediÃ¤rsprache.
...
AnsÃ¤tze. Das AUTOSAR-Konsortium selbst hat mit der EinfÃ¼hrung von C++14/17
in der Adaptive Platform 2017 gezeigt, dass SprachÃ¤nderungen mÃ¶glich sind~\cite{autosar_ap_explained2019}.
...
die Ã¼berlegene Wahl fÃ¼r eine AUTOSAR-Sprachstandardisierung ist:

\begin{enumerate}
  \item \textbf{Technische Eignung}: MicroPython ist auf aktuellen
        Automotive-MCUs ressourcentechnisch problemlos einsetzbar.
        Mit native/viper-Dekoratoren und Inline-Assembler kÃ¶nnen Performance-
        kritische Funktionen mit annÃ¤hernd C-Geschwindigkeit realisiert werden.
  \item \textbf{Sicherheit}: Die definierten PySafety-Level ermÃ¶glichen
        ISO~26262-konforme Implementierungen bis ASIL-D.
        Statische Typisierung, formale VerifikationsansÃ¤tze (Nagini)
        und umfangreiche Testautomatisierung ergÃ¤nzen das Safety-Case-Argument.
  ...
  \item \textbf{Ã–kosystem}: Python bietet das weltweit grÃ¶ÃŸte Entwickler-
        und BibliotheksÃ¶kosystem, einschlieÃŸlich erstklassiger KI/ML-,
        Test- und Diagnose-Tools.
  \item \textbf{Wirtschaftlichkeit}: Python-Standardisierung verspricht
        signifikante ProduktivitÃ¤tssteigerungen (30--50\%), Kosteneinsparungen
        und Zugang zu einem 10$\times$ grÃ¶ÃŸeren Talentpool als Rust.
  \item \textbf{Machbarkeit}: Existierende Projekte (pyautosar, python-can,
        MicroPython-Ports fÃ¼r Automotive-MCUs) belegen die praktische
        Machbarkeit.
\end{enumerate}
...
Unterfangen -- sie erÃ¶ffnet ein weitreichendes Forschungsfeld, das mehrere
Disziplinen der Informatik, des Ingenieurwesens und der Normungspraxis berÃ¼hrt.
Die folgenden Fragen sind nicht nur akademischer Natur, sondern bilden die
notwendige Grundlage fÃ¼r eine industriell belastbare und normkonforme Umsetzung.

\subsubsection{Echtzeit und WCET-Analyse}

\begin{itemize}
  \item \textbf{Formale WCET-Analyse fÃ¼r MicroPython-Viper-Code}:
        Der \texttt{@micropython.\newline viper}-Decorator erzeugt nativen Maschinencode,
        der strukturell C-Ã¤hnlich ist. Dennoch fehlt bislang ein formaler Nachweis,
        dass der erzeugte Code mit bestehenden WCET-Werkzeugen (AbsInt aiT,
        TriCore-WCET-Analyzer) vollstÃ¤ndig analysierbar ist.
  ...
  \item \textbf{WCET des Bytecode-Interpreters}:
        FÃ¼r bytecode-interpretierten Python-Code ist eine Worst-Case-Analyse
        auf Basis eines abstrakten Interpreter-Modells denkbar.
        Ein Ansatz wÃ¤re die Bestimmung einer oberen Schranke fÃ¼r die
        AusfÃ¼hrungszeit jedes Bytecode-Opcodes auf einer spezifischen Zielplattform,
        kombiniert mit statischer Kontrollfluss\-analyse auf Bytecode-Ebene.
        Erste AnsÃ¤tze existieren in der akademischen Literatur~\cite{micropython_wcet2023},
        erfordern aber industriellen Reifegrad.

  \item \textbf{GC-Pausezeiten als WCET-StÃ¶rgrÃ¶ÃŸe}:
        Auch wenn der GC manuell steuerbar ist, bedarf es eines formalen
        Modells der maximalen GC-Pausezeit in AbhÃ¤ngigkeit von Heap-GrÃ¶ÃŸe,
        Objektanzahl und Fragmentierungsgrad.
        Dieses Modell wÃ¤re Basis fÃ¼r ASIL-B/C-Zulassungen von SWCs mit
        erlaubtem GC.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Tool Confidence Level (TCL) fÃ¼r MicroPython}:
        ISO~26262-8 verlangt einen Qualifizierungsnachweis fÃ¼r Software-Tools,
        die sicherheitskritischen Code erzeugen oder ausfÃ¼hren.
        FÃ¼r den MicroPython-Interpreter als Laufzeitumgebung
        ...
        realistisch.
        Ein Qualifizierungskit analog zu GCC-QualifizierungslÃ¶sungen (z.B.\ von
        LDRA, AdaCore) wÃ¤re zu entwickeln.
        Dies umfasst: Interpreter-Testsuite mit nachweisbarer Abdeckung,
        Freeze-Mechanismus fÃ¼r zertifizierte Interpreter-Versionen,
        Ã„nderungsmanagement und Bug-Fix-Prozess nach IEC~61508.

  \item \textbf{Compositional Qualification}:
        Da MicroPython auf qualifizierten C-Compilern (GCC/Clang mit
        Qualifizierungskit) aufbaut, kÃ¶nnte eine kompositionelle
...
\begin{itemize}
  \item \textbf{VollstÃ¤ndiges PSL-4-Regelwerk}:
        Das in Abschnitt~\ref{sec:sprachprofil} eingefÃ¼hrte PySafety-Level-4-Profil
        ist bislang als konzeptionelle Spezifikation beschrieben.
        FÃ¼r einen industriellen Standard mÃ¼sste das vollstÃ¤ndige Regelwerk
        -- analog zu MISRA~C mit ca.\ 143~Regeln -- ausgearbeitet werden,
        mit BegrÃ¼ndung, Rationale, Beispielen und automatisch prÃ¼fbaren
        TestfÃ¤llen. Dies ist ein Normungsvorhaben fÃ¼r die AUTOSAR-Working-Group.

  \item \textbf{PySafety Lint-Plugin (Referenzimplementierung)}:
        Ein Open-Source-\texttt{pylint}-Plugin oder eigenstÃ¤ndiger Checker,
        der alle PSL-Regeln automatisch prÃ¼ft, ist als Referenzimplementierung
        zu entwickeln. Dies ermÃ¶glicht OEMs und Tier-1-Zulieferern, PSL-KonformitÃ¤t
        in ihrer \gls{ci}/\gls{cd}-Pipeline nachzuweisen.

  \item \textbf{MC/DC-Coverage fÃ¼r Python}:
        ASIL-D verlangt modifizierte BedingungsÃ¼berdeckung (MC/DC).
        FÃ¼r Python existiert \texttt{coverage.py} mit Branch-Coverage; MC/DC ist
        Ã¼ber das Plugin \texttt{pytest-cov-mcdc} fÃ¼r CPython adressierbar, fÃ¼r
        MicroPython-Bytecode jedoch nicht vollstÃ¤ndig gelÃ¶st.
        Forschungsbedarf: automatische MC/DC-Instrumentierung des
        MicroPython-Bytecode-Compilers.

  \item \textbf{Formale Verifikation fÃ¼r PySafety-Profile}:
        Nagini~\cite{nagini2018} ermÃ¶glicht Verifikation typisierter Python-Funktionen
        gegen Viper-Spezifikationen.
        Die Integration von Nagini in einen AUTOSAR-Verifikationsworkflow
        (analog zu Frama-C fÃ¼r C) ist ein offenes Forschungsfeld.
        Insbesondere wÃ¤re zu untersuchen, ob Nagini auf MicroPython-typisierte
        Teilmengen anwendbar ist, oder ob ein dedizierter Verifikator fÃ¼r
        MicroPython-Viper-Code entwickelt werden mÃ¼sste.
\end{itemize}

\subsubsection{Performance und Portierung}

\begin{itemize}
  \item \textbf{MicroPython-Ports fÃ¼r dominante Automotive-MCUs}:
        TriCore (Infineon TC3xx/TC4xx), PowerPC/e200 (NXP MPC5xxx)
        und RISC-V-basierte Automotive-Cores (z.B.\ SiFive im
        Renesas-Ã–kosystem) sind aktuell nicht als offizielle MicroPython-Ports
        verfÃ¼gbar.
        Die Portierung auf diese Architekturen -- insbesondere die
        Implementierung des \texttt{@viper}-Compilers fÃ¼r TriCore-ISA --
        ist eine wichtige Voraussetzung fÃ¼r den AUTOSAR-CP-Einsatz.

  \item \textbf{Benchmark-Suite fÃ¼r Automotive-Workloads}:
        Bestehende Benchmarks (MicroPython Benchmark Suite, PyPerformance)
        sind nicht fÃ¼r Automotive-Workloads optimiert.
        ...
        Diese wÃ¤re Grundlage fÃ¼r datenbasierte Argumentationen
        gegenÃ¼ber Standardisierungsgremien.

  \item \textbf{Statische Kompilierung fÃ¼r CP-Deployment}:
        Eine Variante von MicroPython, die Python-Bytecode vollstÃ¤ndig
        zur Linkzeit in Maschinencode kompiliert (Ã¤hnlich wie mypyc oder Nuitka),
        wÃ¼rde den Interpreter im Produktionssystem Ã¼berflÃ¼ssig machen.
        Dies wÃ¼rde WCET-Analyse erheblich vereinfachen und das TCL-Argument stÃ¤rken.
        Das Projekt \texttt{micropython-compiler} (experimentell) adressiert
        diesen Ansatz, benÃ¶tigt jedoch erhebliche Engineering-Investitionen
        fÃ¼r Automotive-Grade-QualitÃ¤t.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Sandboxing mehrerer Python-SWCs}:
        ...
        Eine privilegierte SWC kÃ¶nnte versehentlich oder bÃ¶swillig Zustand
        einer anderen SWC lesen oder Ã¼berschreiben.
        Forschungsbedarf: leichtgewichtige Isolation zwischen Python-Modulen
        (analog zu WebAssembly-Sandboxing), mÃ¶glicherweise durch
        separate MicroPython-VMs mit getrennten Heaps pro ASIL-Partition.

  \item \textbf{Supply-Chain-Sicherheit fÃ¼r Python-SWCs}:
        Python-Pakete von PyPI~\cite{pypi_stats2024} bergen Supply-Chain-Risiken
        (Typosquatting, kompromittierte AbhÃ¤ngigkeiten).
        FÃ¼r AUTOSAR-Python-SWCs mÃ¼ssen Prozesse definiert werden:
        erlaubte Paket-Whitelists, Hash-Verifikation aller AbhÃ¤ngigkeiten,
        SBOM (Software Bill of Materials) fÃ¼r Python-Komponenten nach ISO~5230.
        Die Werkzeuge \texttt{pip-audit} und \texttt{cyclonedx-python} bieten
        Ausgangspunkte, sind aber fÃ¼r Automotive-Prozesse anzupassen.

  \item \textbf{Runtime Integrity Checking}:
        Python's ReflexionsmÃ¶glichkeiten (\texttt{inspect}, \newline \texttt{\_\_dict\_\_}-Manipulation)
        kÃ¶nnten deaktiviert werden, sind aber auch Angriffsvektoren.
        Ein gehÃ¤rtetes MicroPython-Build fÃ¼r ASIL-Umgebungen, das alle
        zur Laufzeit nicht benÃ¶tigten Reflexions-APIs entfernt, wÃ¤re
...
\begin{itemize}
  \item \textbf{KompatibilitÃ¤t zwischen CPython und MicroPython}:
        Unterschiede zwischen CPython und MicroPython (fehlende Stdlib-Module,
        leicht abweichendes Verhalten bei Randpunkten) kÃ¶nnen zu Portierungsfehlern
        fÃ¼hren, wenn Code zwischen AP und CP Ã¼bertragen wird.
        Eine formale KompatibilitÃ¤tsmatrix sowie ein
        \texttt{micropython-compat}-Testlauf auf CPython wÃ¤ren zu standardisieren.

  \item \textbf{AUTOSAR-Python-Modul-Repository}:
        Analog zu crates.io (Rust) oder npm (JavaScript) wÃ¤re ein
        kuratiertes, qualitÃ¤tsgesichertes Repository fÃ¼r AUTOSAR-konforme
        Python-Pakete sinnvoll.
        Pakete wÃ¼rden PSL-Level-Metadaten, Testergebnisse und
        Safety-Zertifizierungsnachweise tragen.

  \item \textbf{InteroperabilitÃ¤t mit bestehenden AUTOSAR-Toolketten}:
        ...
        Open-Source-BeitrÃ¤ge zur pyautosar-Community~\cite{pyautosar_gh}.
\end{itemize}
...
In den nÃ¤chsten zwei Jahren liegt die PrioritÃ¤t auf dem Aufbau belastbarer
technischer Grundlagen und dem Einwerben industrieller UnterstÃ¼tzung:

\paragraph{Referenzimplementierung PyAutosar}
Das dringendste Deliverable ist eine vollstÃ¤ndige Open-Source-Referenzimplementierung
\textbf{PyAutosar} (s.~Abschnitt~\ref{subsec:pyautosar_impl}).
Nur ein funktionierender Prototyp -- lauffÃ¤hig auf einem realen Automotive-Evaluation-Board
...
als Python-SWCs, vollstÃ¤ndig im Rahmen des AUTOSAR-OS-Schedulers.

\paragraph{Erstes PSL-Regelwerk (Draft)}
Die AUTOSAR-Working-Group sollte bis Ende 2027 einen Draft des
PySafety-Level-1/2-Regelwerks verabschieden, der Ã¶ffentlich zur Kommentierung
freigegeben wird. Dieser Draft schafft die Grundlage fÃ¼r industrielle Pilotprojekte
und gibt Werkzeugherstellern die notwendige Spezifikation fÃ¼r
Checker-Implementierungen.

\paragraph{MicroPython-Port fÃ¼r TriCore}
Infineon TriCore (TC3xx/TC4xx) ist die meistgenutzte MCU-Architektur
im Automotive-CP-Segment. Ein stabiler MicroPython-Port fÃ¼r TriCore --
mÃ¶glichst mit \texttt{@viper}-UnterstÃ¼tzung fÃ¼r TriCore-Operationen --
wÃ¼rde den grÃ¶ÃŸten Adressierbarkeitsgrad der AUTOSAR-CP-Installationsbasis
erschlieÃŸen.
Dies ist ein Engineering-Projekt mit Ã¼berschaubarem Aufwand (ca.\ 6--12 Monate
Vollzeit-Entwicklung), das durch ein Konsortium aus Infineon,
Bosch und der MicroPython Foundation realisierbar wÃ¤re.

\paragraph{Akademische Validierungsstudien}
Parallel zur industriellen Arbeit sollten UniversitÃ¤ten und Forschungsinstitute
vergleichende Studien auf Basis standardisierter Benchmarks durchfÃ¼hren:
...
breiter industrieller UnterstÃ¼tzung kann die normative Phase beginnen.
...
Sprache -- zunÃ¤chst fÃ¼r QM und ASIL-A/B-Anwendungen auf der Classic Platform
...
Parallel zur Normierung sollte der Qualifizierungsprozess fÃ¼r den
MicroPython-Interpreter nach ISO~26262-8 initiiert werden.
Dieser wÃ¼rde durch einen Zusammenschluss aus Infineon, Bosch, einem
Qualifizierungsdienstleister (z.B.\ LDRA, Parasoft) und der
MicroPython Foundation durchgefÃ¼hrt.
Das Ergebnis -- ein Ã¶ffentlich verfÃ¼gbares Qualifizierungskit --
wÃ¤re ein strategisch wichtiger Meilenstein, der MicroPython in
dieselbe Liga wie qualifizierte C-Compiler hebt.

\paragraph{Toolketten-Integration}
MarktfÃ¼hrende AUTOSAR-Toolhersteller wie Vector, EB oder dSPACE mÃ¼ssten
Python-UnterstÃ¼tzung in ihren Code-Generatoren und Konfigurationstools verankern:
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Body/Comfort-DomÃ¤ne}: Fensterheber-SteuergerÃ¤t, Klimaanlage,
        Sitzverstellung -- geringe Sicherheitsanforderungen (QM/ASIL-A),
        ideal fÃ¼r erste Python-SWC-Serien;
...
Auf einem Zeithorizont von zehn Jahren kÃ¶nnte Python/MicroPython
zur bevorzugten Implementierungssprache in weiten Teilen des
AUTOSAR-Ã–kosystems werden.
...
\gls{ota}-Updatebarkeit und breite EntwicklerzugÃ¤nglichkeit ermÃ¶glicht.
Python erfÃ¼llt alle drei Anforderungen besser als C oder C++:
\begin{itemize}
  ...
  \item \textbf{OTA-Updates}: Python-Bytecode (\texttt{.mpy}-Dateien)
        kann inkrementell Ã¼ber OTA eingespielt werden, ohne
        einen vollstÃ¤ndigen Flash-Zyklus zu erfordern;
  \item \textbf{KI-Integration}: Das nÃ¤chste Jahrzehnt wird von
        KI-gestÃ¼tzten Fahrzeugfunktionen geprÃ¤gt sein.
        Python als Ende-zu-Ende-Sprache -- von der Datengenerierung im
        Fahrzeug bis zur Modell-Inference auf dem \gls{ecu} -- schafft
        eine einzigartige WertschÃ¶pfungskette, die mit keinem anderen
        Sprach-Stack erreichbar ist.
\end{itemize}

\paragraph{LLM-gestÃ¼tzte Automotive-Entwicklung}
...
um GrÃ¶ÃŸenordnungen hÃ¶her als fÃ¼r C~\cite{stackoverflow_survey2024}.
Dies bedeutet: LLM-gestÃ¼tzte Code-Generierung, Bugfixing und
Dokumentation werden fÃ¼r Python-AUTOSAR-Code deutlich Ã¼berlegene
QualitÃ¤t liefern als fÃ¼r C/C++- oder Rust-Code.
Bis 2035 wird dieser Faktor ein signifikanter Wettbewerbsvorteil sein.

\paragraph{Python als Lehrsprache fÃ¼r Automotive-Ingenieure}
Bereits heute vermitteln UniversitÃ¤ten Python als erste
...
direkt AUTOSAR-SWCs entwickeln kann, wÃ¼rde den historischen
FachkrÃ¤ftemangel in der Automotive-Softwareentwicklung lindern.
Ausbildungsprogramme fÃ¼r ``Python AUTOSAR Developer'' kÃ¶nnten
-- Ã¤hnlich wie AUTOSAR-Entwickler-Schulungen heute -- von
Zertifizierungsinstituten (wie dem AUTOSAR Learning Program)
angeboten werden.

\paragraph{Open-Source-Ã–kosystem und Community-Governance}
Langfristig sollte die PyAu\-tosar-Infrastruktur unter einer
Open-Source-Governance stehen, die Ã¤hnlich wie GENIVI/COVESA
oder der Eclipse-Foundation Automotive-Working-Group organisiert ist.
Eine gemeinnÃ¼tzige ``Python AUTOSAR Foundation'' wÃ¼rde:
\begin{itemize}
  \item die Pflege des MicroPython-AUTOSAR-Ports Ã¼bernehmen;
  \item InteroperabilitÃ¤tstests zwischen Toolketten sicherstellen;
...
\paragraph{Perspektive AUTOSAR~R35 und darÃ¼ber hinaus}
In einer optimistischen Langfristprojektion kÃ¶nnte AUTOSAR ab Release~R35
Python/MicroPython als \emph{bevorzugte} Sprache fÃ¼r Applikationsschichten
(SWC-Ebene) auf beiden Plattformen spezifizieren, wÃ¤hrend C
auf die untersten HAL/MCAL-Schichten zurÃ¼ckgedrÃ¤ngt wird.
Dies wÃ¼rde den Traum einer vollstÃ¤ndig in Python entwickelten
Fahrzeugsoftware greifbar machen -- von der Kalibrierungslogik Ã¼ber
...
Der Erfolg der Python-Standardisierung in AUTOSAR hÃ¤ngt nicht allein
von technischer Exzellenz ab, sondern ebenso von aktiver Beteiligung
in Normungsgremien:

\begin{itemize}
  \item \textbf{ISO~TC22/SC32/WG8}: ZustÃ¤ndig fÃ¼r ISO~26262;
        Python-spezifische ErgÃ¤nzungen (Annex zu Sprach-Subsets)
        wÃ¤ren hier einzubringen;
  \item \textbf{MISRA}: Entwicklung von \textit{MISRA Python} analog zu
        MISRA~C und MISRA~C++23 -- ein SchlÃ¼sseldokument fÃ¼r die
        Akzeptanz von Python in der Automobilindustrie;
  \item \textbf{SAE International (J3061, J3101)}: Cybersecurity-Normung;
        Python-spezifische Sicherheitsleitlinien fÃ¼r embedded Anwendungen;
  \item \textbf{IEC~SC65A (IEC~61508)}: FÃ¼r industrielle Anwendungen
        jenseits des Automobils -- Python in Maschinen- und Prozesssicherheit --
        wÃ¼rden Parallelentwicklungen synergetisch wirken;
  \item \textbf{AUTOSAR Foundation}: Formale Aufnahme in die
        Technical Overview als normative Sprache, verpflichtende
        UnterstÃ¼tzung durch AUTOSAR-konforme Toolchain-Hersteller.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Laufzeitanalyse}: Entwicklung von WCET-Analyseverfahren
        fÃ¼r MicroPython-Viper-Code und bytecode-interpretierten Python-Code,
        publiziert als Open-Source-Werkzeuge;
  \item \textbf{Sicherheitsnachweise}: Formale Sicherheitsbeweise fÃ¼r
  ...
  \item \textbf{Standardisierungstheorie}: Untersuchung von Governance-Modellen
        fÃ¼r Open-Source-Sprachen in sicherheitskritischen Industrie-Standards --
...
in den nÃ¤chsten zwanzig Jahren das RÃ¼ckgrat automobiler SteuergerÃ¤tesoftware
bilden, wird heute in Standardisierungsgremien, auf Konferenzen und
in den Strategieabteilungen der OEMs getroffen.

Diese Arbeit hat argumentiert -- technisch fundiert, Ã¶konomisch belegt
und Ã¶kosystemisch begrÃ¼ndet --, dass \textbf{Python und MicroPython}
die Ã¼berlegene Wahl fÃ¼r eine AUTOSAR-Standardisierung sind.
Nicht als Ersatz fÃ¼r C in den untersten Hardware-Schichten, sondern
als erstklassige Sprache fÃ¼r alle Schichten oberhalb des \gls{mcal}:
Diagnose, Konfiguration, Kommunikationsmanagement, OTA-Updates,
ADAS-Applikationslogik, KI-Inferenz und die gesamte AnwendungsdomÃ¤ne.
...
Die AUTOSAR-Community hat bewiesen, dass Standardisierungswandel mÃ¶glich ist --
von proprietÃ¤ren Stacks zu AUTOSAR~Classic, dann zu AUTOSAR~Adaptive.
Was fehlt, ist der strategische Wille, diese Erkenntnisse zu einer
kohÃ¤renten, zukunftsweisenden Sprachstrategie zu verbinden.

\medskip

Die nÃ¤chste Generation von Fahrzeugen wird Software-defined sein.
Die nÃ¤chste Generation von Automotive-Ingenieuren wird Python kennen.
Die nÃ¤chste Generation von Fahrzeugfunktionen wird KI-gestÃ¼tzt sein.
...
und zukunftsfÃ¤hige Sprache fÃ¼r das Software-defined Vehicle.}
...
\bibitem{pep484}
  G.~van~Rossum, J.~Lehtosalo und Å.~Langa,
...
\bibitem{pypy_bolz2009}
  C.~F.~Bolz, A.~Cuni, M.~FijaÅ‚kowski und A.~Rigo,
...
\bibitem{rust_unsafe2021}
  V.~Astrauskas, C.~Matheja, F.~Poli, P.~MÃ¼ller und A.~J.~Summers,
...
\bibitem{pretschner2007software}
  A.~Pretschner, M.~Broy, I.~H.~KrÃ¼ger und T.~Stauner,
...
\bibitem{python_formal_verification2020}
  A.~Fromherz, T.~Gehr und P.~MÃ¼ller,
