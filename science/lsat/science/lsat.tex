...
\section{Einführung}
%==============================================================
Konsequente Konstruktion polynomieller Abbildungen für alle Probleme aus NP hat die Austauschbarkeit aller Probleme aus NP bei einer effizienten polynomiellen Lösung zum Glück. Diese Arbeit verbindet zwei vorherige Arbeiten: Die Masterarbeit \textit{Learning Monotone DNF in Boolean Circuits} \cite{Epp13} und die Arbeit \textit{Der Subgraph Algorithmus} \cite{Epp26}. Erstere entwickelt einen Algorithmus zum Lernen einer versteckten monotonen DNF-Formel $H \in \text{M-DNF}$ innerhalb eines Boolean Circuits mittels Query-Learning und SAT-Solver-Aufrufen. Letztere zeigt, dass das Subgraph-Isomorphismus-Problem in $O(n^3)$ Zeit lösbar ist, woraus $\text{P} = \text{NP}$ folgt.
...
Dieses Kapitel wiederholt die notwendigen Grundlagen aus der Masterarbeit \cite{Epp13} und ergänzt sie durch die relevanten Konzepte aus \cite{Epp26, Epp26}.
...
Der Subgraph Algorithmus \cite{Epp26, Epp26} löst das Subgraph-Isomorphismus-Problem in $O(n^3)$ Zeit. Die Kernkomponenten sind:
...
\begin{enumerate}
    ...
    \]
    Die Injektivität dieser Funktion wird in \cite{Epp26} bewiesen.
...
Dieses Verfahren, welches zu Beginn LBV für die eine und dann für die andere Hälfte der aussagenlogischen Formel prüft, und dann den Subgraph-SAT-Solver anwendet, falls durch LBV noch keine erfüllende Belegung gefunden worden ist, ist das effizienteste Verfahren. Ein effizienteres Verfahren zur Lösung des SAT-Problems gibt es nicht. Denn nur noch für die \textit{Kombinationspyramide} wird der Subgraph-SAT-Solver angewendet, welche das LBV Verfahren nicht geprüft hat. Das beudeutet, es ist zu erwarten, dass dieses Verfahren für jede zweite aussagenlogische Formel nur logarithmische Laufzeit und für jede zweite aussagenlogische Formel Laufzeit $O(n^3)$ hat.
...
\begin{enumerate}
	\item\label{res:reduktion}
	  ...
	  polynomielle Lösung des SAT-Problems durch den Subgraph Algorithmus \cite{Epp26,
	  Epp26} ermöglicht.
...
\begin{itemize}
	...
	\item \textbf{Unteren-Schranken-Forschung}: Theoretische Analyse ob Sub-\cite{Epp26}-Zeit SAT-Solving möglich ist oder $\Omega(\log m)$ Iterations-Schranke fundamental ist.
...
\bibitem[Epp26]{Epp26}
S.\ Epp. Der Subgraph Algorithmus. 2026. Verfügbar unter \url{https://github.com/hjstephan86/subgraph}.
