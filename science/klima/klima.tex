...
ausflieÃƒÅ¸enden Massenstr\"ome.}
\end{satz}
...
\]
(symmetrische EinstrÃ¶mung in die innertropische Konvergenzzone).
\end{satz}
...
bis $20\%$ sind durch das Wandern der ITCZ erkl\"arbar.
\end{proof}
...
\begin{itemize}
    \item $S(t)$: \emph{Stabiler Anteil} -- Fl\"achenanteil mit normalem atmosph\"arischem
          Zustand (neutraler ENSO, moderate Temperaturen, regelm\"aÃŸige Zirkulation)
...
\begin{itemize}
    ...
    \item Erhaltung: $(dS+dE+dA+dR)/dt = 0$ ist durch direkte Addition pr\"ufbar.
\end{itemize}
\end{proof}
...
\begin{itemize}
    ...
    \item $\lambda_2 > 0 \Leftrightarrow \mathcal{R}_0^{\mathrm{klima}} > 1$: instabil,
          exponentielle Zunahme von $E(t)$ initial.
\end{itemize}
\end{proof}
...
der Kreis selbst $(J_1 \to J_2 \to \cdots \to J_K \to J_1)$ ist ein Hamiltonkreis.
\end{proof}
...
diesen kritischen Wert erreicht. GrÃ¶{\ss}ere Wellenzahlen erfordern schw\"achere
...
\caption{Oben: Jet-Stream-M\"aander mit Rossby-Wellenzug ($k=4$). Orange: mÃ¤andrierender
...
Indien (ENSO $\to$ MonsunschwÃ¤chung),
...
Damit ist der Graph stark zusammenh\"angend.
\end{proof}
...
entspricht einer negativen in $G_{\mathrm{La\,Nina}}$ und umgekehrt.
\end{proof}
