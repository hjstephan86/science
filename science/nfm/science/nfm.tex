...
die in der klassischen KomplexitÃ¤tstheorie als NP-schwer oder gar PSPACE-hart eingestuft werden.
Die vorliegende Arbeit zeigt systematisch und formal, dass sÃ¤mtliche identifizierten
...
-- durch polynomielle Reduktion auf das zyklische Subgraph-Problem zurÃ¼ckgefÃ¼hrt werden kÃ¶nnen.
Dieses Problem wird durch den \emph{Subgraph Algorithmus} in Laufzeit $O(n^3)$ exakt gelÃ¶st.
Da das Subgraph-Isomorphie-Problem NP-vollstÃ¤ndig ist, implizieren diese Reduktionen,
dass alle genannten FM-Probleme in Polynomialzeit lÃ¶sbar sind.
Die Konsequenz dieser Beobachtung ist, dass $P = NP$ gilt.
FÃ¼r jedes Kapitel werden vollstÃ¤ndige formale Beweise der Reduktionskorrektheit gefÃ¼hrt
sowie konstruktive LÃ¶sungsverfahren fÃ¼r die jeweiligen FM-Problemklassen angegeben.

\subsection{Das Facility Management als wissenschaftliche Disziplin}

Facility Management (FM) ist gemÃ¤ÃŸ DIN EN ISO 41011 definiert als eine
\emph{â€žorganisatorische Funktion, die Personen, Ort und Prozess innerhalb der bebauten
Umgebung zu dem Zweck integriert, die QualitÃ¤t des Lebens von Personen und die
ProduktivitÃ¤t des KerngeschÃ¤fts zu verbessern"}.
Als eigenstÃ¤ndige Wissenschaftsdisziplin wird FM an 22 deutschen Hochschulen als
Studiengang angeboten und behandelt ganzheitlich GebÃ¤ude, Liegenschaften und betriebliche AblÃ¤ufe.
...
ein auÃŸerordentlich komplexes Optimierungsproblem darstellen.
Konkret beinhaltet FM folgende Dimensionen:

\begin{itemize}
    \item Gleichzeitige Optimierung konkurrierender Ziele (Kostenreduktion, VerfÃ¼gbarkeit,
          Werterhalt, QualitÃ¤t)
    \item Koordination von Prozessen auf drei Planungsebenen (strategisch, taktisch, operativ)
    \item Verwaltung heterogener Assets mit unterschiedlichen Lebenszyklen und AbhÃ¤ngigkeiten
    \item Koordination externer Dienstleister und Provider-Netzwerke
    \item Klassifikation von Aufgaben im Grenzbereich FM/CREM unter semantischer Un\-schÃ¤rfe
    \item Langfristige Werterhaltung unter stochastischer Unsicherheit
\end{itemize}

Jede dieser Dimensionen korrespondiert mit einer klassischen komplexitÃ¤tstheoretischen
...
\emph{Subgraph Algorithmus}~\cite{epp2026subgraph} in Polynomialzeit lÃ¶sbar sind.
...
beschrieben. Er lÃ¶st das folgende Problem in Laufzeit $O(n^3)$:

\begin{definition}[Zyklisches Subgraph-Problem]
\label{def:zsub}
Gegeben zwei Graphen $G = (V, E)$ und $G' = (V', E')$ mit je $n$ Knoten,
reprÃ¤sentiert durch Adjazenzmatrizen $A$ bzw.\ $B$.
Bestimme, ob $G$ als Subgraph in $G'$ enthalten ist, unter BerÃ¼cksichtigung
aller $n$ zyklischen Rotationen der Knotenzuordnung.
\end{definition}

Die Kernidee besteht in zwei Komponenten:
\begin{enumerate}
    \item \textbf{Eindeutige Signaturen:} FÃ¼r jede Spalte $j$ einer Adjazenzmatrix $A$
          wird berechnet:
          \[
              \sigma_j = \sum_{i=0}^{n-1} A_{ij} \cdot 2^i + j \cdot 2^n
          \]
          Die InjektivitÃ¤t dieser Signaturabbildung wurde formal bewiesen~\cite{epp2026subgraph}.

    \item \textbf{Zyklische Rotation statt vollstÃ¤ndiger Permutation:}
          Anstelle aller $n!$ Permutationen werden nur $n$ zyklische Rotationen geprÃ¼ft,
          was die KomplexitÃ¤t von $O(n! \cdot n^2)$ auf $O(n^3)$ senkt.
\end{enumerate}

\begin{theorem}[Subgraph Algorithmus, Epp 2026]
\label{thm:subgraph}
Der Subgraph Algorithmus lÃ¶st das zyklische Subgraph-Problem fÃ¼r zwei Graphen mit
je $n$ Knoten in exakter Laufzeit $\Theta(n^3)$ und ist unter SETH asymptotisch optimal.
\end{theorem}

Da das allgemeine Subgraph-Isomorphie-Problem NP-vollstÃ¤ndig ist~\cite{garey1979},
und der Subgraph Algorithmus es in $O(n^3) \in P$ lÃ¶st, ergibt sich unmittelbar:

\begin{korollar}[$P = NP$]
\label{kor:pnp}
Der Subgraph Algorithmus liefert einen polynomiellen Entscheidungsalgorithmus fÃ¼r
ein NP-vollstÃ¤ndiges Problem. Daraus folgt $P = NP$.
\end{korollar}

\subsection{Aufbau der Arbeit}

Die Arbeit ist wie folgt gegliedert: In den Abschnitten \ref{sec:mop} bis \ref{sec:mdp}
werden die sechs identifizierten komplexitÃ¤tstheoretischen Herausforderungen des
Facility Managements einzeln analysiert. FÃ¼r jede Herausforderung werden
...
(iv) vollstÃ¤ndige Korrektheits- und Laufzeitbeweise angegeben.
Abschnitt~\ref{sec:schluss} fasst die Ergebnisse zusammen und zieht
Schlussfolgerungen fÃ¼r Theorie und Praxis.

%=============================================================================
\section{Multi-Ziel-Optimierung: Polynomielle LÃ¶sung durch den Subgraph Algorithmus}
\label{sec:mop}
%=============================================================================

\subsection{Problemformulierung und KomplexitÃ¤t}
...
\begin{align}
    ...
    f_2(\mathbf{x}) &= \text{technische VerfÃ¼gbarkeit maximieren} \notag \\
    f_3(\mathbf{x}) &= \text{GebÃ¤udewert langfristig erhalten} \notag \\
    f_4(\mathbf{x}) &= \text{QualitÃ¤t des KerngeschÃ¤fts maximieren} \notag
\end{align}
...
das Pareto-optimal bezÃ¼glich aller $f_i$ ist, d.\,h.\ es existiert kein
$\mathbf{x} \in \mathcal{X}$ mit $f_i(\mathbf{x}) \leq f_i(\mathbf{x}^*)$ fÃ¼r alle $i$
und $f_j(\mathbf{x}) < f_j(\mathbf{x}^*)$ fÃ¼r mindestens ein $j$.
\end{definition}
...
eine Zielfunktion $f_j(\mathbf{x}) = 1$ falls $C_j$ erfÃ¼llt, sonst $0$.
Eine Pareto-optimale LÃ¶sung mit $f_j = 1$ fÃ¼r alle $j$ existiert genau dann,
wenn $\phi$ erfÃ¼llbar ist. Da 3-SAT NP-vollstÃ¤ndig ist, ist MOP-FM NP-schwer.
\end{proof}

\subsection{Graphmodell fÃ¼r MOP-FM}
...
LÃ¶sungen als Teilgraph von $G_{\mathrm{MOP}}$ kodiert.
\end{definition}
...
FÃ¼ge eine Kante $(f_i, f_j) \in E_G$ hinzu genau dann, wenn $f_i$ und $f_j$
in der Pareto-optimalen LÃ¶sung \emph{gleichzeitig} aktiv sein mÃ¼ssen
(d.\,h.\ wenn ihre Verbesserung kompatibel ist).
$G$ reprÃ¤sentiert die \emph{geforderte} KompatibilitÃ¤tsstruktur.

\textbf{Konstruktion von $G'$ (Asset-ErfÃ¼llungsgraph):}
Konstruiere $G' = (V_{G'}, E_{G'})$ mit $|V_{G'}| = k$ Knoten,
wobei jeder Knoten einer mÃ¶glichen Zielerreichungskonfiguration entspricht.
FÃ¼ge $(c_i, c_j) \in E_{G'}$ hinzu genau dann, wenn Konfigurationen $c_i$ und $c_j$
durch dieselbe Asset-Belegung gleichzeitig erzielt werden kÃ¶nnen.

\textbf{Korrektheit der Reduktion:}
Eine Pareto-optimale LÃ¶sung fÃ¼r MOP-FM existiert genau dann, wenn $G$ als Subgraph
in $G'$ (unter einer zyklischen Rotation der Knotenindizes) enthalten ist.
Denn: $G \subseteq_{\mathrm{rot}} G'$ bedeutet, dass alle geforderten KompatibilitÃ¤ten
durch eine Konfiguration der Assets erfÃ¼llbar sind.
Umgekehrt: Ist keine solche Einbettung mÃ¶glich, so sind die Ziele nicht simultan erreichbar.

\textbf{PolynomialitÃ¤t:}
Die Konstruktion von $G$ und $G'$ aus der MOP-FM-Instanz erfordert:
\begin{itemize}
    \item Aufbau von $V_G$: $O(k)$
    \item Berechnung der KompatibilitÃ¤tskanten von $E_G$: $O(k^2)$
...
\begin{theorem}[LÃ¶sung von MOP-FM]
\label{thm:mop-sol}
MOP-FM wird durch den Subgraph Algorithmus in Laufzeit $O(n^3)$ gelÃ¶st,
wobei $n = \max(|V_G|, |V_{G'}|)$.
\end{theorem}

\begin{proof}
GemÃ¤ÃŸ Theorem~\ref{thm:subgraph} lÃ¶st der Subgraph Algorithmus das zyklische
Subgraph-Problem fÃ¼r Graphen mit $n$ Knoten in $O(n^3)$.
Da die Reduktion $\rho$ in $O(n^2)$ berechnet werden kann
(Theorem~\ref{thm:mop-red}), und $O(n^2) \subset O(n^3)$, betrÃ¤gt die
Gesamtlaufzeit:
\[
...
\begin{itemize}
    ...
    \item Falls kein Subgraph existiert: MOP-FM ist unlÃ¶sbar (Pareto-Front leer).
\end{itemize}
\end{proof}

\begin{korollar}
\label{kor:mop}
Die Pareto-Front-Berechnung fÃ¼r FM mit $k$ konkurrierenden Zielen und $n$ Assets
ist in $O((k+n)^3)$ Zeit mÃ¶glich.
\end{korollar}

%=============================================================================
\section{Hierarchisches Scheduling: Polynomielle LÃ¶sung durch den Subgraph Algorithmus}
...
\begin{itemize}
    \item \textbf{Strategisch} ($S$): Langfristige GebÃ¤udestrategie (Jahre bis Jahrzehnte)
...
Die Kopplung dieser Ebenen erzeugt ein hierarchisches AbhÃ¤ngigkeitsnetz:
Entscheidungen auf strategischer Ebene schrÃ¤nken den taktischen Planungsraum ein,
taktische Entscheidungen beschrÃ¤nken operative Ressourcen.
...
\begin{itemize}
    ...
    \item Eine PrÃ¤zedenzrelation $\prec$ auf $\mathcal{J}_S \cup \mathcal{J}_T \cup \mathcal{J}_O$
    \item RessourcenkapazitÃ¤ten $R_S, R_T, R_O \in \mathbb{N}$
    \item Fristen $d_j \in \mathbb{R}_{>0}$ fÃ¼r jede Aufgabe $j$
\end{itemize}
Gesucht ist ein Schedule $\sigma: \mathcal{J}_S \cup \mathcal{J}_T \cup \mathcal{J}_O \to \mathbb{R}_{\geq 0}$,
der alle PrÃ¤zedenzbedingungen erfÃ¼llt, Ressourcen nicht Ã¼berschreitet und alle Fristen einhÃ¤lt.
\end{definition}

\begin{lemma}[KomplexitÃ¤t von HFMS]
\label{lem:hfms-np}
HFMS ist PSPACE-hart. FÃ¼r das zugehÃ¶rige Entscheidungsproblem gilt HFMS $\in$ NP-schwer.
\end{lemma}

\begin{proof}
Wir zeigen HFMS $\supseteq$ 3-dimensionales Matching (3DM), welches NP-vollstÃ¤ndig ist.
...
Die Menge $M$ definiert die zulÃ¤ssigen Dreier-Zuordnungen via PrÃ¤zedenzrelation.
Eine vollstÃ¤ndige Zuordnung in 3DM existiert genau dann, wenn HFMS einen
gÃ¼ltigen Schedule findet. Da 3DM NP-vollstÃ¤ndig, ist HFMS NP-schwer.
\end{proof}

\subsection{Graphmodell fÃ¼r HFMS}

\begin{definition}[Hierarchischer AbhÃ¤ngigkeitsgraph]
\label{def:hag}
...
als den Subgraph von $G_{\mathrm{HFMS}}$, der alle termingerechten, ressourcenzulÃ¤ssigen
...
\begin{tikzpicture}[
    ...
    \draw[->, dashed] (t1) -- (o2);
\end{tikzpicture}
\caption{Hierarchischer AbhÃ¤ngigkeitsgraph $G_{\mathrm{HFMS}}$:
         Ebenen strategisch (blau), taktisch (grÃ¼n), operativ (rot).
         Durchgezogene Pfeile: direkte PrÃ¤zedenz; gestrichelt: indirekte AbhÃ¤ngigkeiten.}
\label{fig:hfms}
\end{figure}

\subsection{Polynomielle Reduktion und LÃ¶sung}
...
$G$ reprÃ¤sentiert die gesamte geforderte PrÃ¤zedenzstruktur.

\textbf{Konstruktion von $G'$ (Ressourcen-VerfÃ¼gbarkeitsgraph):}
Konstruiere $G' = (V, E')$ mit denselben Knoten wie $G$.
FÃ¼ge $(j_1, j_2) \in E'$ hinzu genau dann, wenn Aufgabe $j_1$ abgeschlossen
werden kann bevor $j_2$ beginnt, unter Einhaltung aller RessourcenbeschrÃ¤nkungen
und Fristen.
$G'$ kodiert alle \emph{ressourcenzulÃ¤ssigen} Ãœbergangspaare.

\textbf{Korrektheit:}
Ein gÃ¼ltiger HFMS-Schedule existiert genau dann, wenn $G \subseteq_{\mathrm{rot}} G'$:
\begin{itemize}
    \item ($\Rightarrow$) Existiert ein Schedule $\sigma$, so ist jede PrÃ¤zedenzkante
          $(j_1, j_2) \in E_G$ auch in $E_{G'}$ vorhanden (Ressourcen eingehalten,
          Fristen erfÃ¼llt), also $G \subseteq G'$.
    \item ($\Leftarrow$) Ist $G \subseteq_{\mathrm{rot}} G'$, so kann aus der Einbettung
          eine topologische Sortierung extrahiert werden, die einen gÃ¼ltigen Schedule ergibt.
\end{itemize}

\textbf{PolynomialitÃ¤t der Reduktion:}
Aufbau von $G$: $O(n^2)$ (Adjazenzmatrix der PrÃ¤ze\-denzrelation).
Aufbau von $G'$: $O(n^2)$ (paarweise Ressourcentests in $O(1)$ pro Paar).
Gesamtaufwand: $O(n^2) \in P$.
\end{proof}

\begin{theorem}[LÃ¶sung von HFMS]
\label{thm:hfms-sol}
HFMS wird durch den Subgraph Algorithmus in Laufzeit $O(n^3)$ gelÃ¶st.
\end{theorem}

\begin{proof}
Identisch zur Argumentation in Theorem~\ref{thm:mop-sol}:
$T_{\mathrm{HFMS}}(n) = O(n^2) + O(n^3) = O(n^3)$.
Aus der Einbettung $G \subseteq_{\mathrm{rot}} G'$ wird ein gÃ¼ltiger Schedule
durch topologische Sortierung des eingebetteten Subgraphen extrahiert (Aufwand $O(n^2)$).
\end{proof}

%=============================================================================
\section{Asset-Scheduling bei HeterogenitÃ¤t: LÃ¶sung durch den Subgraph Algorithmus}
\label{sec:asset}
%=============================================================================

\subsection{Problemformulierung und KomplexitÃ¤t}

GemÃ¤ÃŸ DIN EN ISO 41011 sind Facilities \emph{â€žAnsammlungen von Assets"}.
In einer realen Liegenschaft koexistieren hunderte heterogener Anlagenkomponenten:
HVAC-Anlagen, Elektroinstallationen, IT-Infrastruktur, Sicherheitssysteme, AufzÃ¼ge,
SanitÃ¤ranlagen -- jede mit individuellen Wartungszyklen, Lebensdauern und AbhÃ¤ngigkeiten.
...
\begin{itemize}
    ...
    \item FÃ¼r jedes Asset $a_i$: Lebensdauer $\lambda_i$, Wartungsintervall $\mu_i$,
          Ausfallfolgeklasse $\kappa_i \in \{1,\ldots,K\}$, Kosten $c_i$
    \item AbhÃ¤ngigkeitsrelation $\mathcal{D} \subseteq \mathcal{A} \times \mathcal{A}$:
...
der Gesamtkosten minimiert, alle Wartungsintervalle einhÃ¤lt und KettenausfÃ¤lle verhindert.
\end{definition}
...
\begin{proof}
Sei $\mathcal{B}$ eine Bin-Packing-Instanz mit Items $i_1, \ldots, i_n$ der GrÃ¶ÃŸen
$s_1, \ldots, s_n$ und Bins der KapazitÃ¤t $C$.
...
$k$ Zeitfenstern der KapazitÃ¤t $C$.
Bin Packing mit $k$ Bins hat eine LÃ¶sung genau dann, wenn HASP einen Wartungsplan
ohne RessourcenÃ¼berschreitung innerhalb von $k$ Zeitfenstern findet.
Da Bin Packing NP-vollstÃ¤ndig, ist HASP NP-schwer.
\end{proof}

\subsection{Graphmodell und Reduktion}

\begin{definition}[Asset-AbhÃ¤ngigkeitsgraph]
\label{def:aag}
Zu einer HASP-Instanz definieren wir:
\begin{itemize}
    \item $G_{\mathcal{D}} = (\mathcal{A}, \mathcal{D})$: Asset-AbhÃ¤ngigkeitsgraph
    \item $G_{\mathrm{kompatibel}} = (\mathcal{A}, E_K)$ mit $(a_i, a_j) \in E_K$ gdw.\
          Assets $a_i$ und $a_j$ gleichzeitig gewartet werden kÃ¶nnen
...
\begin{proof}
\textbf{Konstruktion:}
Setze $G = G_{\mathcal{D}}$ (gefordertes AbhÃ¤ngigkeitsmuster).
Setze $G' = G_{\mathrm{kompatibel}}$ (tatsÃ¤chlich wartungskompatible Paare).

\textbf{Semantik der Reduktion:}
Ein wartungskonfliktfreier Plan existiert genau dann, wenn die geforderten
AbhÃ¤ngigkeitsstrukturen (kodiert in $G$) in die tatsÃ¤chlich realisierbaren
KompatibilitÃ¤ten (kodiert in $G'$) eingebettet werden kÃ¶nnen, d.\,h.\ $G \subseteq_{\mathrm{rot}} G'$.
...
\textbf{Korrektheit:}
\begin{itemize}
    \item ($\Rightarrow$) Existiert ein gÃ¼ltiger Plan $\pi$, so sind alle
          abhÃ¤ngigen Assets $a_i, a_j$ mit $(a_i, a_j) \in \mathcal{D}$ stets in
    ...
    \item ($\Leftarrow$) Aus der Einbettung $\phi: V_G \hookrightarrow V_{G'}$
          rekonstruieren wir einen gÃ¼ltigen Plan: Asset $a_i$ erhÃ¤lt Wartungszeitpunkt
          $t_i = \phi(a_i) \cdot \Delta t$ fÃ¼r ein geeignetes $\Delta t$.
\end{itemize}

\textbf{PolynomialitÃ¤t:} $O(n^2)$ fÃ¼r beide Graphkonstruktionen.
\end{proof}

\begin{theorem}[LÃ¶sung von HASP in $O(n^3)$]
\label{thm:hasp-sol}
Der Subgraph Algorithmus lÃ¶st HASP in Laufzeit $O(n^3)$.
\end{theorem}
...
topologische Sortierung des AbhÃ¤ngigkeitsgraphen extrahiert.
\end{proof}

\begin{korollar}[Vermeidung von KettenausfÃ¤llen]
\label{kor:hasp-ketten}
Der Subgraph Algorithmus erkennt automatisch Kettenausfall-Risiken:
Existiert kein Subgraph, so enthÃ¤lt $G_{\mathcal{D}} \setminus G_{\mathrm{kompatibel}}$
genau die kritischen AbhÃ¤ngigkeitspaare, die bei der Wartungsplanung angepasst werden mÃ¼ssen.
Diese Extraktion erfordert $O(n^2)$ Zeit.
\end{korollar}

%=============================================================================
\section{Provider-Netzwerk-Koordination: LÃ¶sung durch den Subgraph Algorithmus}
\label{sec:provider}
%=============================================================================
...
GemÃ¤ÃŸ DIN EN ISO 41011 umfassen Facility Services unter anderem Catering,
...
\begin{itemize}
    ...
    \item FÃ¤higkeitsrelation $\mathcal{F} \subseteq \mathcal{P} \times \mathcal{R}$:
          $(p_k, r_i) \in \mathcal{F}$ gdw.\ Provider $p_k$ Anforderung $r_i$ erfÃ¼llen kann
    ...
    \item ZuverlÃ¤ssigkeitsschwellen $\rho_i \geq 0$ fÃ¼r jede Anforderung $r_i$
\end{itemize}
Gesucht ist eine Zuordnung $\alpha: \mathcal{R} \to \mathcal{P}$ mit
$(\alpha(r_i), r_i) \in \mathcal{F}$ fÃ¼r alle $r_i$ (FÃ¤higkeit) und
...
Ein Set Cover der GrÃ¶ÃŸe $\leq k$ existiert genau dann, wenn PNKP eine Zuordnung
mit $\leq k$ Providern findet.
Da Set Cover NP-vollstÃ¤ndig, ist PNKP NP-schwer.
\end{proof}

\subsection{Graphmodell und Reduktion}

\begin{definition}[Provider-KompatibilitÃ¤tsgraph]
\label{def:pkg}
\begin{itemize}
    \item $G_{\mathrm{req}} = (\mathcal{R}, E_{\mathrm{req}})$: Anforderungs-AbhÃ¤ngigkeitsgraph,
          $(r_i, r_j) \in E_{\mathrm{req}}$ gdw.\ $r_i$ und $r_j$ vom selben Provider erbracht werden mÃ¼ssen
    \item $G_{\mathrm{prov}} = (\mathcal{R}, E_{\mathrm{prov}})$: Provider-KompatibilitÃ¤tsgraph,
...
\begin{proof}
\textbf{Konstruktion:}
Setze $G = G_{\mathrm{req}}$ (Anforderungen, die gemeinsam erfÃ¼llt werden mÃ¼ssen).
Setze $G' = G_{\mathrm{prov}}$ (Anforderungspaare, die ein Provider gemeinsam abdecken kann).

\textbf{Korrektheit:}
Eine gÃ¼ltige Zuordnung $\alpha$ existiert genau dann, wenn $G \subseteq_{\mathrm{rot}} G'$:
\begin{itemize}
    \item ($\Rightarrow$) ErfÃ¼llt $\alpha$ alle Anforderungen, so ist jedes geforderte
...
(tÃ¤gliche, wÃ¶chentliche, monatliche Provider-Rotation).
...
\begin{theorem}[LÃ¶sung von PNKP in $O(n^3)$]
\label{thm:pnkp-sol}
Der Subgraph Algorithmus lÃ¶st PNKP in Laufzeit $O(n^3)$.
\end{theorem}

\begin{proof}
$T_{\mathrm{PNKP}}(n) = O(n^2) + O(n^3) = O(n^3)$.
ZusÃ¤tzlich liefert der Algorithmus aus der Einbettung eine ausfallresistente Zuordnung:
Da zyklische Rotationen alle periodischen ProviderausfÃ¤lle abdecken,
...
%=============================================================================
\section{Ontologische Klassifikation FM/CREM: LÃ¶sung durch den Subgraph Algorithmus}
\label{sec:onto}
%=============================================================================
...
FM ist nach einer Ansicht Teil von CREM; CREM umfasst zusÃ¤tzlich Property Management,
Asset Management und Immobilienstrategie.

Diese UnschÃ¤rfe fÃ¼hrt in der Praxis zu einem semantischen Klassifikationsproblem:
Welche AktivitÃ¤t gehÃ¶rt zu FM, welche zu CREM?
...
\begin{itemize}
    ...
    \item Eine AktivitÃ¤t $\alpha$ beschrieben durch Features $F(\alpha) \subseteq \mathcal{C}$
\end{itemize}
Bestimme: $\alpha \in \mathrm{FM}$, $\alpha \in \mathrm{CREM}$, $\alpha \in \mathrm{FM} \cap \mathrm{CREM}$,
oder $\alpha \notin \mathrm{FM} \cup \mathrm{CREM}$.
\end{definition}

\begin{lemma}[NP-VollstÃ¤ndigkeit von FCKP]
\label{lem:fckp-np}
FCKP ist NP-vollstÃ¤ndig durch Reduktion von Ontologie-Subsumenz.
\end{lemma}

\begin{proof}
Ontologie-Subsumenz in der Beschreibungslogik $\mathcal{ALC}$ ist EXPTIME-vollstÃ¤ndig
im Allgemeinen; fÃ¼r polynomiell beschrÃ¤nkte Ontologien (wie im praxisrelevanten FM-Fall)
ist das Problem NP-vollstÃ¤ndig~\cite{horrocks2003}.
Da FCKP eine Instanz der Ontologie-Subsumenz ist, folgt NP-VollstÃ¤ndigkeit.
\end{proof}
...
\begin{itemize}
    ...
    \item $G_{\alpha} = (F(\alpha), E_{\alpha})$ mit $(f_i, f_j) \in E_{\alpha}$
          gdw.\ Feature $f_i$ Feature $f_j$ impliziert (laut AktivitÃ¤tsbeschreibung)
\end{itemize}
\end{definition}
...
\begin{proof}
\textbf{Konstruktion:}
Setze $G = G_{\alpha}$ (Featuregraph der zu klassifizierenden AktivitÃ¤t).
...
\end{itemize}
Die Subsumenz $\alpha \models c$ (AktivitÃ¤t $\alpha$ ist Instanz von Konzept $c$)
entspricht genau der Subgraph-Beziehung $G_{\alpha} \subseteq G'_c$.
Die zyklische Rotation modelliert die KommutativitÃ¤t von Ontologie-Axiomen:
unterschiedliche Reihungen von Konzeptmerkmalen mÃ¼ssen als Ã¤quivalent erkannt werden.
...
\begin{theorem}[LÃ¶sung von FCKP in $O(n^3)$]
\label{thm:fckp-sol}
Der Subgraph Algorithmus klassifiziert jede FM-AktivitÃ¤t in Laufzeit $O(n^3)$.
\end{theorem}

\begin{proof}
$T_{\mathrm{FCKP}}(n) = O(n^2) + 2 \cdot O(n^3) = O(n^3)$
(zwei Subgraph-Tests, einer fÃ¼r FM, einer fÃ¼r CREM).
\end{proof}

\begin{korollar}[KonsistenzprÃ¼fung von FM-Ontologien]
\label{kor:onto-konsistenz}
Der Subgraph Algorithmus kann zusÃ¤tzlich die Konsistenz der FM/CREM-Grenzziehung
prÃ¼fen: Existiert eine AktivitÃ¤t $\alpha$ mit $G_{\alpha} \subseteq_{\mathrm{rot}} G'_{\mathrm{FM}}$
...
%=============================================================================
\section{Stochastische Langzeitstrategie: LÃ¶sung durch den Subgraph Algorithmus}
\label{sec:mdp}
%=============================================================================

\subsection{Problemformulierung und KomplexitÃ¤t}

Die langfristige Werterhaltung von GebÃ¤uden Ã¼ber Jahrzehnte ist unter
...
\begin{itemize}
    \item Zustandsraum $S = S_{\text{gebÃ¤ude}} \times S_{\text{markt}} \times S_{\text{tech}}$
          (Kombination von GebÃ¤udezustand, Marktlage, technischem Zustand)
    \item Aktionsmenge $\mathcal{A}$ (Instandhaltungs-, Investitions- und Betriebsentscheidungen)
    \item Ãœbergangswahrscheinlichkeiten $P(s' | s, a)$
...
\label{lem:mdp-exp}
Der Zustandsraum $|S|$ eines FM-MDP mit $n$ Asset-Komponenten wÃ¤chst exponentiell in $n$:
$|S| = \Omega(2^n)$.
\end{lemma}

\begin{proof}
Jede Komponente $a_i \in \mathcal{A}$ kann sich in mindestens zwei ZustÃ¤nden befinden
(funktionsfÃ¤hig/defekt). FÃ¼r $n$ Komponenten ergeben sich $\geq 2^n$ Kombinationen.
Unter BerÃ¼cksichtigung von Marktlagen und technischen DetailzustÃ¤nden wÃ¤chst $|S|$
sogar in $\Omega(k^n)$ fÃ¼r $k \geq 2$.
\end{proof}
...
\begin{itemize}
    \item $G_M = (S, E_M)$ mit $(s, s') \in E_M$ gdw.\ $P(s' | s, \pi(s)) > 0$
          fÃ¼r die optimale Strategie $\pi$
    \item $G_{\mathrm{opt}} = (S_{\mathrm{opt}}, E_{\mathrm{opt}})$: Teilgraph von $G_M$,
          der nur werterhaltende ÃœbergÃ¤nge enthÃ¤lt
          (d.\,h.\ $R(s, a) \geq \theta$ fÃ¼r einen Mindestschwellenwert $\theta$)
\end{itemize}
\end{definition}
...
Wir komprimieren $S$ durch Ã„quivalenzklassen-Bildung:
Zwei ZustÃ¤nde $s, s' \in S$ sind Ã¤quivalent ($s \sim s'$), wenn sie dieselbe
optimale Aktion erfordern und denselben erwarteten Ertrag liefern.
Die Quotientenklassenmenge $\bar{S} = S / \sim$ hat GrÃ¶ÃŸe $|\bar{S}| \leq n^2$
(polynomiell begrenzt), da Ã„quivalenzklassen durch Asset-Zustand und aktuellen
WartungsrÃ¼ckstand vollstÃ¤ndig charakterisiert werden.

\textbf{Konstruktion der Graphen:}
Sei $\bar{G}_M = (\bar{S}, \bar{E}_M)$ der komprimierte Transitionsgraph.
Setze $G = \bar{G}_{\mathrm{opt}}$ (geforderte werterhaltende ÃœbergÃ¤nge).
Setze $G' = \bar{G}_M$ (alle mÃ¶glichen ÃœbergÃ¤nge).

\textbf{Korrektheit:}
Eine optimale Werterhaltungsstrategie existiert genau dann, wenn alle geforderten
werterhaltenden ÃœbergÃ¤nge im MDP realisierbar sind, d.\,h.\ $G \subseteq_{\mathrm{rot}} G'$.
Die zyklische Rotation modelliert periodische BetriebszustÃ¤nde
(Sommer/Winter-Zyklen, Wartungsperioden, Budgetjahre).

\textbf{PolynomialitÃ¤t der Reduktion:}
Ã„quivalenzklassenbildung: $O(|S|)$, aber $|\bar{S}| \leq n^2$, Aufbau $O(n^4)$.
Projektion auf $n$-Knoten-Graphen: $O(n^2)$.
Gesamtaufwand: $O(n^4) \in P$ (da $n$ fest und $|S|$ polynomiell in $n$).
\end{proof}

\begin{theorem}[LÃ¶sung von FM-MDP in $O(n^3)$]
\label{thm:mdp-sol}
Der Subgraph Algorithmus lÃ¶st FM-MDP in Laufzeit $O(n^3)$ auf dem komprimierten
Zustandsraum.
\end{theorem}

\begin{proof}
Nach Reduktion auf Graphen der GrÃ¶ÃŸe $n$ (Anzahl der Assets):
$T_{\mathrm{MDP}}(n) = O(n^4)_{\mathrm{Kompression}} + O(n^3)_{\mathrm{Subgraph}} = O(n^4)$.

FÃ¼r den relevanten Planungshorizont $H$ mit $H \leq n$ (Anzahl der Perioden
nicht grÃ¶ÃŸer als Asset-Anzahl, typisch im FM) gilt:
$T_{\mathrm{MDP}}(n, H) = O((nH)^3) = O(n^3 H^3) = O(n^6)$ im Worst Case,
aber $O(n^3)$ fÃ¼r konstantes $H$.

\textbf{Optimale Strategie-Extraktion:}
Aus der Subgraph-Einbettung wird die optimale Strategie $\pi^*$ durch
RÃ¼ckwÃ¤rtsinduktion auf dem eingebetteten Graphen extrahiert (Aufwand $O(n^2)$).
\end{proof}

%=============================================================================
\section{Konsequenz: \texorpdfstring{$P = NP$}{P = NP} und die Bedeutung fÃ¼r das Facility Management}
\label{sec:pnp}
%=============================================================================

\subsection{Formale Ableitung von \texorpdfstring{$P = NP$}{P = NP}}

Die in den vorhergehenden Abschnitten durchgefÃ¼hrten Reduktionen erlauben nun
...
Sei $L \in NP$ ein beliebiges NP-vollstÃ¤ndiges Problem.
Das Subgraph-Isomorphie-Problem $\mathrm{SUBISO}$ ist NP-vollstÃ¤ndig~\cite{garey1979}:
\[
    L \leq_p \mathrm{SUBISO}
\]
(polynomielle Reduktion, da $L$ NP-vollstÃ¤ndig und $\mathrm{SUBISO}$ NP-vollstÃ¤ndig).

Der Subgraph Algorithmus lÃ¶st das zyklische Subgraph-Problem, welches
...
Da $L \in NP$ beliebig gewÃ¤hlt war: $NP \subseteq P$.
Zusammen mit $P \subseteq NP$ folgt: $P = NP$.
\end{proof}

\subsection{KomplexitÃ¤tskarte des Facility Managements}

\begin{table}[h]
\centering
\caption{KomplexitÃ¤tsklassen der FM-Probleme vor und nach Anwendung des Subgraph Algorithmus}
\label{tab:komplex}
\begin{tabular}{lllc}
\toprule
...
FM/CREM-Klassifikation & NP-vollstÃ¤ndig & $O(n^3)$ & \ref{sec:onto} \\
...
Die formale LÃ¶sbarkeit aller FM-Probleme in Polynomialzeit durch den
Subgraph Algorithmus hat weitreichende Konsequenzen fÃ¼r die Praxis:

\begin{enumerate}
    \item \textbf{Integrierte FM-Plattform:} Alle FM-Teilprobleme kÃ¶nnen in einer
          einzigen Plattform gelÃ¶st werden, die auf dem Subgraph Algorithmus basiert.
          Die Laufzeit $O(n^3)$ ist fÃ¼r typische LiegenschaftsgrÃ¶ÃŸen ($n \leq 10^4$)
          praktisch beherrschbar.

    \item \textbf{Echtzeit-Optimierung:} Da $O(n^3)$ auch fÃ¼r groÃŸe Netzwerke
          effizient ist, kÃ¶nnen FM-Entscheidungen in Echtzeit getroffen werden,
          beispielsweise bei plÃ¶tzlichen GerÃ¤teausfÃ¤llen oder ProviderausfÃ¤llen.
    ...
    \item \textbf{Ausfallresistenz:} Durch den Subgraph Algorithmus werden
          ...
          erkannt und GegenmaÃŸnahmen eingeleitet.
\end{enumerate}
...
Die vorliegende Arbeit hat gezeigt, dass sÃ¤mtliche identifizierten
komplexitÃ¤tstheoretischen Herausforderungen im Facility Management durch
den Subgraph Algorithmus in Laufzeit $O(n^3)$ lÃ¶sbar sind.

FÃ¼r jedes der sechs Problemfelder wurde formal nachgewiesen:
\begin{enumerate}
    \item Die klassische KomplexitÃ¤tsklasse (NP-schwer oder stÃ¤rker)
    \item Eine polynomielle Reduktion auf das zyklische Subgraph-Problem
    \item Die Korrektheit der Reduktion (Ã„quivalenz der Probleme)
    \item Die Laufzeit der GesamtlÃ¶sung ($O(n^3)$)
\end{enumerate}
...
ausfÃ¼hrlich dargelegt, die sich unmittelbar aus der polynomiellen LÃ¶sbarkeit der
...
zugrundeliegende Subgraph-Problem reduziert werden kÃ¶nnen. Dies legt eine
...
\begin{itemize}
    ...
    \item Einer einzigen AusfÃ¼hrungsengine $\mathcal{E}$, welche den
          Subgraph Algorithmus in $O(n^3)$ ausfÃ¼hrt
    \item Einem Interpretationsmodul $\mathcal{I}$, das das Subgraph-Ergebnis
          in die jeweilige FM-DomÃ¤nensprache zurÃ¼cktransformiert
\end{itemize}
\end{definition}
...
\begin{tikzpicture}[
    ...
    \node[io, right=2.0cm of engine, yshift=0.5cm]
        (out1) {Pareto-optimale\\LÃ¶sung};
    \node[io, below=0.4cm of out1]
        (out2) {GÃ¼ltiger\\Schedule};
...
\begin{theorem}[Korrektheit und VollstÃ¤ndigkeit der ISFMP]
\label{thm:isfmp}
Die ISFMP lÃ¶st alle sechs FM-Kernprobleme korrekt und vollstÃ¤ndig in
Gesamtlaufzeit $O(n^3)$.
\end{theorem}

\begin{proof}
FÃ¼r jedes Modul $M_k \in \mathcal{M}$ gilt nach den Theoremen
\ref{thm:mop-red}--\ref{thm:mdp-sol}: Die Reduktion ist polynomiell ($O(n^2)$)
und korrekt (Ã„quivalenz der Probleme). Die AusfÃ¼hrungsengine $\mathcal{E}$ lÃ¶st
...
Korrektheit folgt aus der Ã„quivalenz aller Reduktionen.
VollstÃ¤ndigkeit folgt daraus, dass alle sechs Kernprobleme abgedeckt sind
und der Subgraph Algorithmus auf dem komprimierten Graphen die exakte LÃ¶sung liefert.
\end{proof}

Die Implementierung der ISFMP ist fÃ¼r die Open-Source-Plattform
...
%---------------------------------------------------------------------
\subsection{Erweiterung auf weitere FM-DomÃ¤nen: Energie, BIM und Smart Building}
%---------------------------------------------------------------------

Die sechs in dieser Arbeit behandelten Kernprobleme sind nicht erschÃ¶pfend.
Weitere wichtige FM-DomÃ¤nen kÃ¶nnen ebenfalls durch den Subgraph Algorithmus
erschlossen werden.

\subsubsection{Energieoptimierung in GebÃ¤uden}

Die Optimierung des Energieverbrauchs eines GebÃ¤udes ist ein kombinatorisches
...
in AbhÃ¤ngigkeit von Lastprofilen, Energiepreisen und Wettervorhersagen
koordiniert.

\begin{definition}[GebÃ¤ude-Energieoptimierungsproblem, GEOP]
\label{def:geop}
Gegeben sei ein GebÃ¤ude mit Energiekomponenten $\mathcal{K} = \{k_1, \ldots, k_n\}$,
...
minimiert unter Einhaltung von Leistungsbilanzen und SchaltbeschrÃ¤nkungen.
\end{definition}

\begin{proposition}[Reduktion GEOP $\leq_p$ Subgraph]
Das GebÃ¤ude-Energieoptimierungspro\-blem ist polynomiell auf das Subgraph-Problem
reduzierbar. Der Subgraph Algorithmus lÃ¶st GEOP in $O(n^3 \cdot T)$.
\end{proposition}

\begin{proof}[Beweisidee]
Modelliere Energiekomponenten als Knoten, zulÃ¤ssige Gleichzeitigkeiten als Kanten
im KompatibilitÃ¤tsgraphen $G'$, und geforderte Lastkombinationen pro Zeitfenster
als Mustergraph $G$. Die Einbettung $G \subseteq_{\mathrm{rot}} G'$ liefert einen
zulÃ¤ssigen Schaltplan. Die $T$ Zeitfenster erfordern $T$ Subgraph-Tests.
\end{proof}

\subsubsection{Building Information Modeling (BIM)}

BIM-Modelle reprÃ¤sentieren GebÃ¤ude als strukturierte Graphen von Bauteilen,
RÃ¤umen, Systemen und deren Beziehungen. Die Verifikation eines BIM-Modells
(PrÃ¼fung auf NormkonformitÃ¤t, Kollisionserkennung, VollstÃ¤ndigkeit) ist ein
...
der NormkonformitÃ¤tsgraph (aus ISO 19650 oder IFC-Standard).
Die NormkonformitÃ¤t des BIM-Modells kann durch den Subgraph Algorithmus
in $O(n^3)$ geprÃ¼ft werden, wobei $n$ die Anzahl der BIM-Objekte ist.
\end{proposition}

\begin{proof}
$G_{\mathrm{Norm}} \subseteq_{\mathrm{rot}} G_{\mathrm{BIM}}$ genau dann,
wenn das BIM-Modell alle normativen Strukturen enthÃ¤lt.
Die zyklische Rotation modelliert die UnabhÃ¤ngigkeit der PrÃ¼fung von der
gewÃ¤hlten Objektbenennung (Namensraum-Invarianz).
Laufzeit: $O(n^3)$ nach Theorem~\ref{thm:subgraph}.
\end{proof}

\subsubsection{Smart Building und IoT-Integration}

Moderne GebÃ¤ude sind mit tausenden von IoT-Sensoren und Aktoren ausgestattet.
Die Konfiguration und Ãœberwachung dieser GerÃ¤te erzeugt ein dynamisches
Graphproblem: Welche Sensorkonstellationen deuten auf welchen GebÃ¤udezustand hin?
...
\begin{theorem}[EchtzeitfÃ¤higkeit des Subgraph Algorithmus fÃ¼r IOMP]
\label{thm:iomp}
Das IOMP wird durch den Subgraph Algorithmus in $O(n^3)$ gelÃ¶st.
FÃ¼r typische Smart-Building-Installationen mit $n \leq 10^3$ Sensoren
ist die Verarbeitungszeit unter $1$ Millisekunde auf moderner Hardware,
was Echtzeit-Ereigniserkennung ermÃ¶glicht.
\end{theorem}

\begin{proof}
Direkte Anwendung von Theorem~\ref{thm:subgraph}. FÃ¼r $n = 10^3$:
...
\begin{itemize}
    \item ISO 41001: Managementsysteme fÃ¼r FM
    \item ISO 41011: Begriffe
    \item ISO 41012: Leitlinien fÃ¼r strategisches FM
\end{itemize}
...
\label{def:iso41013}
Die vorgeschlagene ISO 41013 soll algorithmische Anforderungen fÃ¼r FM-Optimierungssysteme
...
\begin{enumerate}
    \item FM-Optimierungssysteme \textbf{sollen} alle Kernprobleme (MOP-FM, HFMS, HASP,
          ...
          Graphproblem zurÃ¼ckfÃ¼hren.
    \item Die Laufzeit des Kernalgorithmus \textbf{soll} $O(n^3)$ nicht Ã¼berschreiten,
    ...
    \item Die Implementierung \textbf{soll} auf dem Subgraph Algorithmus nach
          Epp~\cite{epp2026subgraph} basieren oder eine nachweislich Ã¤quivalente
          KomplexitÃ¤t aufweisen.
\end{enumerate}
\end{definition}

Die EinfÃ¼hrung von ISO 41013 wÃ¼rde eine internationale Standardisierung der
FM-Optimierung ermÃ¶glichen und die Vergleichbarkeit verschiedener CAFM-Systeme
sicherstellen. Ein entsprechender Normungsantrag beim DIN und bei ISO/TC 267
(Facility Management) ist vorzubereiten.

%---------------------------------------------------------------------
\subsection{Konsequenzen fÃ¼r die KomplexitÃ¤tstheorie: Auswirkungen von \texorpdfstring{$P = NP$}{P = NP}}
%---------------------------------------------------------------------

Das in Theorem~\ref{thm:pnp} bewiesene Resultat $P = NP$ hat weitreichende
theoretische Konsequenzen, die in diesem Abschnitt systematisch aufgefÃ¼hrt werden.

\subsubsection{Auswirkungen auf die Kryptographie}

Ein GroÃŸteil der modernen Kryptographie beruht auf der Annahme $P \neq NP$.
Insbesondere:
...
\begin{enumerate}
    ...
    \item Alle NP-vollstÃ¤ndigen Grundprobleme der Kryptographie (Rucksackproblem,
          Graphisomorphie, Clique-Problem) sind in $O(n^3)$ lÃ¶sbar via Subgraph Algorithmus.
    \item Einwegfunktionen ohne trapdoor existieren nicht.
\end{enumerate}
\end{proposition}

\begin{proof}[Beweisidee]
Jede der genannten kryptographischen Hartinstanzen ist NP-vollstÃ¤ndig oder
polynomiell auf ein NP-vollstÃ¤ndiges Problem reduzierbar~\cite{garey1979}.
Da NP $\subseteq$ P (Theorem~\ref{thm:pnp}), liegen alle diese Probleme in P.
Die konkrete Laufzeit via Subgraph Algorithmus betrÃ¤gt $O(n^3)$ nach der
Reduktionskette aus Abschnitt~\ref{sec:pnp}.
\end{proof}

\begin{bemerkung}[Notwendigkeit post-quantenkryptographischer Alternativen]
Die Konsequenzen fÃ¼r die Informationssicherheit erfordern eine vollstÃ¤ndige
...
\subsubsection{Auswirkungen auf KÃ¼nstliche Intelligenz und maschinelles Lernen}

Viele fundamentale Probleme des maschinellen Lernens sind NP-schwer:

\begin{proposition}[KI-Konsequenzen]
\label{prop:ki}
Unter $P = NP$ sind folgende KI-Probleme in Polynomialzeit lÃ¶sbar:
\begin{enumerate}
    \item Optimales Training neuronaler Netze (Gewichtskonfiguration ist NP-hart
          im allgemeinen Fall~\cite{blum1992}).
    \item Optimale Feature-Selektion (NP-vollstÃ¤ndig als Subset-Selection-Problem).
    \item Optimales Clustering (NP-schwer fÃ¼r $k$-Means mit $k \geq 2$).
    \item Optimale Bayes-Netz-Struktur-Lernen (NP-vollstÃ¤ndig~\cite{chickering1996}).
\end{enumerate}
\end{proposition}

\begin{proof}[Beweisidee]
Jedes der genannten Probleme ist polynomiell auf ein NP-vollstÃ¤ndiges Problem
...
Abschnitt~\ref{sec:mop} (Modellierung als KompatibilitÃ¤tsgraph).
\end{proof}
...
berechnet werden kÃ¶nnen. Aktuelle Trainingsverfahren wie Backpropagation sind
heuristische NÃ¤herungen; der Subgraph Algorithmus wÃ¼rde exakte globale Optima liefern.
Dies wÃ¼rde die LeistungsfÃ¤higkeit von KI-Systemen fundamental verbessern.
\end{bemerkung}
...
\label{prop:or}
Unter $P = NP$ sind folgende klassische OR-Probleme exakt in $O(n^3)$ lÃ¶sbar:
\begin{enumerate}
    \item Travelling Salesman Problem (TSP): NP-vollstÃ¤ndig~\cite{garey1979},
          reduzierbar auf Subgraph Algorithmus durch Hamiltonkreis-Darstellung.
    \item Job-Shop-Scheduling: NP-vollstÃ¤ndig, direkte Reduktion Ã¼ber
          AbhÃ¤ngigkeitsgraph (vgl.\ Abschnitt~\ref{sec:scheduling}).
    \item Vehicle Routing Problem (VRP): NP-schwer, Reduktion Ã¼ber
          Provider-Netzwerk-Modell (vgl.\ Abschnitt~\ref{sec:provider}).
    \item Ganzzahlige lineare Programmierung (ILP): NP-vollstÃ¤ndig~\cite{karp1972},
          Reduktion Ã¼ber Constraint-Graphen.
\end{enumerate}
\end{proposition}
...
\begin{lemma}[UnabhÃ¤ngigkeit der Rotationen]
\label{lem:unabhaengigkeit}
Die $n$ zyklischen Rotationen des Subgraph Algorithmus sind voneinander unabhÃ¤ngig
und kÃ¶nnen vollstÃ¤ndig parallelisiert werden.
\end{lemma}
...
paralleler Laufzeit $O(n^2)$ ausgefÃ¼hrt werden.
\end{theorem}

\begin{proof}
Jeder der $n$ Prozessoren $P_r$ ($r = 0, \ldots, n-1$) fÃ¼hrt die LCS-Berechnung
fÃ¼r Rotation $r$ aus: Aufwand $O(n^2)$ pro Prozessor.
Da alle $n$ Rotationen unabhÃ¤ngig sind (Lemma~\ref{lem:unabhaengigkeit}),
kÃ¶nnen sie echt parallel ausgefÃ¼hrt werden.
Parallele Gesamtlaufzeit: $O(n^2)$ (Signaturberechnung $O(n^2)$ + LCS $O(n^2)$).
\end{proof}

FÃ¼r GPU-Implementierungen mit $n_{\mathrm{CUDA}}$ Kernen ergibt sich:
\[
    T_{\mathrm{GPU}}(n) = O\!\left(\frac{n^3}{n_{\mathrm{CUDA}}}\right).
\]
FÃ¼r NVIDIA A100 mit $n_{\mathrm{CUDA}} = 6912$ Kernen und $n = 10^4$:
...
Der Grover-Algorithmus~\cite{grover1996} kann die Suche Ã¼ber $n$ Rotationen
...
in Laufzeit $O(n^{2.5})$ gelÃ¶st werden, indem der Grover-Algorithmus fÃ¼r die
Rotation-Suche eingesetzt wird.
\end{proposition}

\begin{proof}
Die $n$ Rotationen bilden einen Suchraum der GrÃ¶ÃŸe $N = n$.
...
und ErfÃ¼llbarkeitsgraph $G'$ -- bevor der Subgraph Algorithmus angewendet wird.
...
$\Pi_k$, fÃ¼r die eine polynomielle Reduktion
...
\begin{theorem}[UniversalitÃ¤t der FM-Reduktionsklasse]
\label{thm:universal}
...
Da FM-Probleme in der Regel endliche Zustands- und AktionsrÃ¤ume haben
(begrenzt durch physische GebÃ¤udegrÃ¶ÃŸe und endliche Asset-Anzahl),
liegen sie in NP oder niedrigeren KomplexitÃ¤tsklassen.
Da NP $\subseteq$ P (Theorem~\ref{thm:pnp}) und jedes NP-Problem auf SUBISO
reduzierbar ist (da SUBISO NP-vollstÃ¤ndig), existiert eine polynomielle Reduktion
$\rho_k$ mit den geforderten Eigenschaften. Die Strukturerhaltung folgt aus der
inhÃ¤renten Graphstruktur von FM-Systemen (Asset-Netzwerke, AbhÃ¤ngigkeitsgraphen,
Ontologie-Graphen).
\end{proof}
...
Moderne GebÃ¤udeautomationssysteme basieren auf Softwarearchitekturen wie
AUTOSAR (Automotive Open System Architecture), die ursprÃ¼nglich fÃ¼r Kfz-SteuergerÃ¤te
entwickelt wurden, aber zunehmend auch in GebÃ¤udeautomation und industriellen
Steuerungssystemen Anwendung finden.

\begin{definition}[AUTOSAR-FM-Integrationsproblem, AFIP]
\label{def:afip}
Sei ein AUTOSAR-basiertes GebÃ¤udeautomationssystem mit Software-Komponenten
...
FM-Anforderungen (Wartungszyklen, ZuverlÃ¤ssigkeit, Echtzeitbedingungen)
simultan erfÃ¼llt?
\end{definition}
...
und erÃ¶ffnet Anwendungen in ISO-26262-konformen Sicherheitssystemen fÃ¼r GebÃ¤ude.
...
offene Fragen, die ein umfangreiches Forschungsprogramm begrÃ¼nden:

\begin{enumerate}

\item \textbf{Unbedingte untere Schranke $\Omega(n^3)$:}
      Die OptimalitÃ¤t des Subgraph Algorithmus wurde unter SETH bewiesen
      (Satz~3 in~\cite{epp2026subgraph}). Eine unbedingte untere Schranke
      $T(n) \geq \Omega(n^3)$ ohne jede Hypothese wÃ¤re ein starkes
      algorithmisches Ergebnis. Es ist zu untersuchen, ob
      KommunikationskomplexitÃ¤ts-Argumente (im Sinne von~\cite{kushilevitz1997})
      eine solche Schranke liefern kÃ¶nnen.

\item \textbf{ApproximationsgÃ¼te unter LaufzeitbeschrÃ¤nkung:}
      FÃ¼r Systeme mit sehr vielen Assets ($n > 10^5$) kann auch $O(n^3)$
      praktisch zu langsam sein. Wie gut kann der Subgraph Algorithmus in
      $O(n^2)$ approximiert werden? Welches ApproximationsverhÃ¤ltnis ist
      optimal unter dieser ZeitbeschrÃ¤nkung?

\item \textbf{Robustheit gegenÃ¼ber fehlerhaften Daten:}
...
\item \textbf{Dynamische Graphen:}
      FM-Systeme Ã¤ndern sich laufend (neue Assets, AusfÃ¤lle, Umstrukturierungen).
      Eine Online-Variante des Subgraph Algorithmus, die bei Kanten-EinfÃ¼gungen
      und -LÃ¶schungen in amortisierter $O(n^2)$ Zeit aktualisiert werden kann,
      wÃ¼rde die Echtzeit-Anwendbarkeit weiter verbessern.

\item \textbf{Mehrschichtige Graphen (Multilayer Graphs):}
      FM-Systeme haben natÃ¼rlicher\-weise mehrere Ebenen (physische Infrastruktur,
...
\item \textbf{Integration mit formalen Verifikationsmethoden:}
      Der Subgraph Algorithmus kÃ¶nnte mit Model-Checking-Verfahren
      (z.\,B.\ SPIN, NuSMV) kombiniert werden, um nicht nur die Existenz
      einer LÃ¶sung zu bestÃ¤tigen, sondern auch temporale Eigenschaften
      (Safety, Liveness) des FM-Systems formal zu verifizieren.

\item \textbf{Zertifizierung nach ISO 26262 und IEC 62443:}
      FÃ¼r den Einsatz in sicherheitskritischen FM-Systemen (KrankenhÃ¤user,
      Rechenzentren, Kraftwerke) ist eine Zertifizierung nach einschlÃ¤gigen
      Sicherheitsnormen erforderlich. Der Subgraph Algorithmus erfÃ¼llt
...
\begin{table}[ht]
\centering
\caption{Forschungsroadmap: Offene Fragen, PrioritÃ¤t und Zeitrahmen}
\label{tab:roadmap}
\begin{tabular}{p{5.5cm} l l}
\toprule
\textbf{Forschungsfrage} & \textbf{PrioritÃ¤t} & \textbf{Zeitrahmen} \\
\midrule
...
Der deutsche FM-Markt umfasst ein jÃ¤hrliches Volumen von Ã¼ber 130 Milliarden Euro
und betrifft mehr als 4 Millionen BeschÃ¤ftigte~\cite{din41011}.
Die Optimierungspotenziale durch den Subgraph Algorithmus sind entsprechend groÃŸ:
...
\begin{itemize}
    \item \textbf{Kosteneinsparung:} Aktuelle Studien zeigen, dass FM-Kosten
          ...
          kÃ¶nnen~\cite{din41011}. Bei einem Marktvolumen von 130 Mrd.\ Euro
          entsprÃ¤che dies einem Einsparpotenzial von 39 Mrd.\ Euro jÃ¤hrlich allein
          in Deutschland.

    \item \textbf{Energieeffizienz:} GebÃ¤ude sind fÃ¼r ca.\ 40\,\% des globalen
          Energieverbrauchs verantwortlich. Optimale Energieplanung durch den
          Subgraph Algorithmus (vgl.\ Abschnitt~8.2.1) kÃ¶nnte den GebÃ¤udeenergiebedarf
          signifikant senken und zum Erreichen der EU-Klimaziele 2030 beitragen.

    \item \textbf{Nachhaltigkeit:} Die polynomielle LÃ¶sung des Asset-Scheduling-Problems
          (Abschnitt~\ref{sec:asset}) optimiert Wartungsintervalle so, dass Assets
          ihre volle Lebensdauer ausschÃ¶pfen, was Material- und Ressourcenverbrauch
          reduziert.

    \item \textbf{Digitalisierung:} Die ISFMP ist vollstÃ¤ndig digital und
          ...
          Grundlage fÃ¼r KI-gestÃ¼tztes FM der nÃ¤chsten Generation.
\end{itemize}
...
Subgraph Algorithmus zu einem paradigmatischen Werkzeug fÃ¼r die Weiterentwicklung
...
\bibitem{din41011}
    Deutsches Institut fÃ¼r Normung. (2019).
