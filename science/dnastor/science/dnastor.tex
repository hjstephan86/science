...
\begin{definition}[QuaternÃ¤res DNA-Alphabet]
...
\begin{definition}[Shannon-Entropie und Kanalkapazit\"{a}t]
    ...
    Die Kapazit\"{a}t des DNA-Speicherkanals (quaternÃ¤rer symmetrischer
...
\begin{definition}[DNA-Graph eines Datenblockes]
    ...
    Sei $\phi: \mathcal{B}^{2k} \to \Sigma^k$ die quaternÃ¤re Kodierung
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=0.55cm]
      ...
      \node[layer=mainblue,  below=of os]  (cod)
            {QuaternÃ¤rer Kodierer $\phi$ \quad|\quad GC-Balance \quad|\quad Reed-Solomon ECC};
    ...
    Schicht~2: QuaternÃ¤rer Kodierer $\phi$, GC-Balance und Reed-Solomon-ECC;
...
\subsection{QuaternÃ¤re Kodierungsfunktion}

\begin{definition}[QuaternÃ¤re Kodierung $\phi$]
    \label{def:kodierung}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Bijektives quaternÃ¤res Kodierungsschema $\phi: \mathcal{B}^2 \to \Sigma$.
...
\begin{figure}[H]
    ...
    \caption{QuaternÃ¤res Kodierungsschema und Effizienzvergleich.
    ...
    23\% mehr als die rein quaternÃ¤re Grenze von 2\,Bit/Base.}
    \label{fig:encoding}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{QuaternÃ¤re Kodierung:}
...
\begin{proof}
    ...
    \textbf{Korrektheit.}
    Die quaternÃ¤re Kodierung ist bijektiv (Satz~\ref{satz:kodierung}),
...
\begin{definition}[DNA-Kanalmodell]
    \label{def:kanal}
    Der DNA-Speicherkanal ist ein gestÃ¶rter quaternÃ¤rer Kanal mit drei
    Fehlertypen:
    \begin{itemize}
...
\begin{proof}
    ...
    Der quaternÃ¤re symmetrische Kanal mit Fehlerrate $\varepsilon$ hat
...
\begin{enumerate}
    ...
    \item \textbf{Sequenzierung:} Rufe PCR-Amplifikation der
          Ziel-Oligomere durch komplementÃ¤re Primer auf.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=0.9cm and 1.8cm]
      ...
      \node[sysbox=darkgreen, below=0.85cm of verify, minimum width=3.0cm]
            (decode) {QuaternÃ¤re\\Dekodierung $\psi$};
    ...
    quaternÃ¤re Dekodierung zum rekonstruierten Datenblock $D_i$.}
...
\begin{figure}[h]
    ...
    \textbf{Links:} DatenintegritÃ¤t als Funktion der Zeit fÃ¼r verschiedene
    ...
    --- bei $-196$\,Â°C (fl\"{u}ssiger Stickstoff) sind
...
(jede Base kann mit bis zu $k-1$ Gegenbases eine komplementÃ¤re
...
Die Implementierung des Subgraph Algorithmus ist verfÃ¼gbar unter:
