...
\begin{bemerkung}
Definition~\ref{def:integriert} formalisiert die im Einleitungszitat beschriebene Beobachtung: Die Wahrnehmung des Gesamtsystems als â€žmächtig und wirkungsvoll'' entspricht der Emergenz $F(\mathcal{S}^+) \supset F(\mathcal{A}) \cup F(\mathcal{D})$. Die technische Korrektheit der Betrachtung erfordert jedoch die separate Analyse von $\mathcal{A}$ und $\mathcal{D}$.
\end{bemerkung}
...
Jede Entscheidung für die ausgelagerte Topologie muss explizit, begründet und mit dem Bewusstsein ihrer außergewöhnlichen Natur getroffen werden. Eine implizite, unreflektierte Wahl dieser Topologie -- etwa weil â€ždas halt so gemacht wird'' -- stellt einen architektonischen Fehler dar, der zu inkonsistenten Konfigurationen, ungenutzten DBMS-Fähigkeiten und suboptimaler Benutzerzufriedenheit führt.
\end{korollar}
...
\begin{bemerkung}
Satz~\ref{satz:informationsverlust} ist die formale Entsprechung des Einleitungszitats: â€žAls Ganzes wirkt dieses Softwaresystem [..] sehr mächtig und sehr wirkungsvoll'' beschreibt $\mathcal{V}_U$. â€žAus technischer Sicht aber ist dies nicht richtig'' beschreibt den Übergang zu $\mathcal{V}_T$.
\end{bemerkung}
...
Die in dieser Arbeit entwickelten Definitionen und Sätze erlauben nun, den â€žaußergewöhnlichen Weg'' präzise zu charakterisieren. Er ist kein Sonderfall und keine Ausnahme im negativen Sinne, sondern das Ergebnis einer vollständigen, informierten architektonischen Entscheidung.
