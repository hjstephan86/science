...
Abschnitt~\ref{sec:grundlagen} legt die mathematischen Grundlagen der Distributionentheorie (Testfunktionen, Schwartz-Raum, temperierte Distributionen). Abschnitt~\ref{sec:definition} definiert den Dirac-Impuls rigoros und beweist die Normierungseigenschaft. Abschnitt~\ref{sec:eigenschaften} leitet alle wesentlichen Eigenschaften formal her (Skalierungs-, Symmetrie- und Siebeigenschaft). Abschnitt~\ref{sec:fourier} analysiert den Dirac-Impuls im Frequenzbereich mittels Fourier-Transformation. Abschnitt~\ref{sec:naherungen} behandelt Approximationsfolgen mit Visualisierungen. Abschnitt~\ref{sec:kamm} fÃ¼hrt den Dirac-Kamm ein und verbindet ihn mit diskreter Abtasttheorie. Abschnitt~\ref{sec:lti} zeigt Anwendungen in Signal- und Systemtheorie (Faltung, LTI-Systeme). Abschnitt~\ref{sec:quantenmechanik} behandelt den Dirac-Impuls in der Quantenmechanik. Abschnitt~\ref{sec:zusammenfassung} fasst die zentralen Eigenschaften zusammen. Abschnitt~\ref{sec:ausblick} enthÃ¤lt einen ausfÃ¼hrlichen Forschungsausblick.
...
(weiÃŸes Rauschen / weiÃŸes Spektrum), und die Phase ist identisch $0$. Dies bedeutet,
...
\begin{itemize}
    ...
    \item \textbf{Quantenchaos und Dirac-Spektren:} Die Verteilung der Nullstellen der
          ...
          Vermutung und der ungelÃ¶sten Riemannschen Vermutung.
