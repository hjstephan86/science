...
aufzusuchen, anzuschließen, zu messen und zu korrigieren â€” ein Prozess, der
...
(CTIS â€“ Central Tire Inflation System bei Militärfahrzeugen und Landwirtschaft).
...
\begin{definition}[Zentrales Reifendruckregelsystem â€“ ZRRS]
...
Kapitel beweist formal, dass ein \emph{lastadaptives} ZRRS â€” das heißt ein
ZRRS, das den Solldruck $p^*_i$ individuell in Abhängigkeit der aktuellen
Radlast $F_i$ setzt â€” gegenüber einem System mit konstantem Solldruck eine
...
\begin{figure}[H]
    ...
    \caption{Links: Reifenkontaktfläche $A_{\text{K}}$ als Funktion der Radlast bei drei
             ...
             Radlasten â€” das Minimum verschiebt sich mit zunehmender Last nach rechts
...
dem konstanten ZRRS in diesem Szenario â€” eine substantielle Einsparung
...
% ---------------------------------------------------------------------------
\subsection{Anwendungsfall I: Produktion â€” Fertigungstransport und Logistik}
% ---------------------------------------------------------------------------
...
\begin{figure}[H]
    ...
    \caption{Monatlicher Verschleißindex $W_{i,m}$ für drei Nutzungsszenarien über 12 Monate
             ...
             Die Farbskala zeigt die relative Belastung â€” trotz starker Asymmetrie
...
\]
mit $\mathbb{E}[\sigma^2_F] = k^2 \sum_{j}\phi_{ij}^2 \sigma_m^2 \cdot n_{\text{Ã˜}}$
für die erwartete Radlastvarianz bei $n_{\text{Ã˜}}$ durchschnittlichen Fahrgästen.
\end{satz}
...
\begin{figure}[H]
    ...
    \caption{Quantitativer Verschleißvergleich: ZRRS konstant vs. ZRRS adaptiv
             ...
             Links: Mittlerer Verschleißindex $\bar{W}$ â€” adaptive Kurve (grün)
...
\begin{itemize}
    \item \textbf{Unterdruck:} Randabrieb â€” das Gummi wölbt sich nach innen,
    die Lauffläche trägt außen mehr Last.
    \item \textbf{Überdruck:} Mittenabrieb â€” der Reifen kontaktiert die Straße
    nur in der Mitte.
...
\begin{figure}[H]
    ...
    \caption{Reifenabriefprofil (schematisch) über die Laufflächenbreite.
             Links: Ohne ZRRS-Adaption bei asymmetrischer Beladung â€” HL
             (grün, durchgezogen) zeigt erhöhten Mittenabrieb (Überlastung),
             HR (grau, gepunktet) zeigt Randabrieb (Unterlastung).
             Rechts: Mit lastadaptivem ZRRS â€” alle Profile konvergieren gegen
...
\begin{figure}[H]
    ...
    \caption{Links: Rollwiderstandskoeffizient $c_r$ als Funktion des Reifendrucks.
             ...
             $\bar{\Delta p}$ â€” asymmetrische Beladung (rot) erzeugt doppelt so hohe
...
\begin{figure}[H]
    ...
    \caption{Stabilitätsanalyse des lastadaptiven ZRRS.
             ...
             für symmetrische (grün), asymmetrisch-konstante (rot) und adaptive (blau) Regelung â€”
...
\begin{figure}[H]
    ...
    \caption{Monte-Carlo-Simulation ($N = 50\,000$ Fahrten, $\text{seed} = 42$).
             ...
             Mitte: Histogramm der Verschleißindizes â€” die adaptive Verteilung
             ...
             (Mittelwert rot gestrichelt) â€” im Schnitt werden ca.\ $15$--$25\,\%$
...
sind in allen drei betrachteten Nutzungskontexten â€” industrielle Produktion,
Personenbeförderung und Gütertransport â€” der maßgebliche Treiber für
...
\begin{figure}[H]
    ...
    \caption{Heatmap der Druckabweichungen aller vier Räder über 12 Monate.
             Oben: manuelles Befüllen â€” deutliche räumliche und zeitliche Inhomogenitäten.
             Unten: ZRRS â€” nahezu uniforme, blaue Felder nahe Null.}
    \label{fig:heatmap}
\end{figure}
...
$\approx 19\,\text{Wh}$ für alle vier Räder â€” ein vernachlässigbarer Anteil
...
\begin{tabular}{lrrr}
\toprule
\textbf{Kostenart} & \textbf{Manuell [â‚¬]} & \textbf{ZRRS [â‚¬]} & \textbf{Einsparung [â‚¬]} \\
\midrule
...
adressierbares Marktvolumen von ca. $4{,}4\,\text{Mrd.}\,\text{â‚¬/Jahr}$
...
Die dort gewonnenen Erkenntnisse â€” insbesondere die nahezu sichere Überlegenheit
...
Druckanpassung (Satz~\ref{satz:stabfahrwerk}) â€” werfen eine Reihe weiterführender
...
\begin{itemize}
    ...
    Schätzung $\hat{F}_i(t)$ aus diesen Signalen â€” ohne dedizierten
    Kraftsensor â€” wäre kosteneffizienter. Das erweiterte Kalman-Filter (EKF)
...
\begin{itemize}
    ...
    reagieren und die Solldrücke entsprechend anpassen â€” auch wenn die
...
formale Analyse â€” insbesondere die Robustheitseigenschaften bei
Frachtplanung-Unsicherheit â€” ein eigenständiges Forschungsthema darstellt.
...
\end{align*}
Die Analyse der Stabilität dieses nichtlinearen gekoppelten Systems â€”
insbesondere die Frage, ob ein einziger Lyapunov-Kandidat für das gesamte
thermo-mechanische Modell existiert â€” stellt eine wesentliche Erweiterung
...
Solldruck propabilistisch vorpositionieren â€” noch bevor ein Sensor den
...
$\mathcal{A} \in [0{,}15, 0{,}45]$ â€” deutlich höher als beim PKW.
...
für moderate laterale Lastungleichgewichte. Für extreme Szenarien â€”
etwa einseitiges Auffahren auf ein Hindernis, Beladung nur auf
einer Fahrzeugseite oder akutes Reifenplatzen benachbarter Räder â€” sind
...
explizit auf die formalen Ergebnisse aus Kapitel~\ref{sec:asymm} verweisen â€”
insbesondere Satz~\ref{satz:druckopt_last} als Nachweis der Optimalität
des lastadaptiven Drucks â€” und eine standardisierte Formel für $p^*_i(F_i)$
...
â€” einschließlich einer Fehlerabschätzung für nichtlineare Wechselwirkungs\-terme â€”
...
\textit{Road vehicles â€“ Interchange of digital information on electrical connections between towing and towed vehicles},
...
\bibitem{iso4000}
DIN ISO~4000-1:2010,
\textit{Passenger car tyres â€“ Part 1: Tyre designation and dimensions},
Beuth Verlag, Berlin, 2010.

\bibitem{nhtsa_tpms}
National Highway Traffic Safety Administration (NHTSA),
\textit{Final Rule: Federal Motor Vehicle Safety Standard No.~138 â€“ Tire Pressure Monitoring Systems},
...
\textit{Road vehicles â€“ Functional safety â€“ Part 1: Vocabulary},
...
\textit{Road vehicles â€“ Controller area network (CAN) â€“ Part 1: Data link layer and physical signalling},
...
\textit{UN Regulation No.~64 â€“ Spare wheel / temporary-use spare wheel / run-flat tyre
...
\textit{Metallic tube connections for fluid power and general use â€“ Part 1: 24-degree
...
\bibitem{iso7241}
DIN ISO~7241-A:2021,
\textit{Hydraulic fluid power â€“ Quick-action couplings â€“ Part 1: Dimensions and
requirements for hydraulic couplings},
Beuth Verlag, Berlin, 2021.

\bibitem{ece_r13}
United Nations Economic Commission for Europe (UNECE),
\textit{UN Regulation No.~13H â€“ Uniform provisions concerning the approval of passenger
cars with regard to braking},
ECE/TRANS/WP.29, Genf, 2015.

\bibitem{conti_ctis}
Continental AG,
\textit{CTIS â€“ Central Tire Inflation System: Product Description and Performance Data},
...
Systems â€“ Part 1: General Requirements},
Beuth Verlag, Berlin, 2010.
\end{thebibliography}
