...
\title{\textbf{Optimierung von Dieselkraftstoff und Dieselmotor fÃ¼r maximale Lebensdauer}}
\author{Stephan Epp}
\date{05. MÃ¤rz 2026}
...
%% ------------------------------------------------------------------
Diese Arbeit prÃ¤sentiert einen neuartigen Dieselkraftstoff sowie einen
dazu konzipierten Dieselmotor, die gemeinsam eine maximale Motorlebensdauer
bei hohem thermischen Wirkungsgrad ermÃ¶glichen. Auf der Basis des
Archard-Verschlei\ss{}modells, der Dowson-Higginson-Gleichung fÃ¼r
...
rigoros optimale Parameter fÃ¼r Kraftstoff- und Motorauslegung hergeleitet
und mit umfangreichen Plots visualisiert. Der optimierte Kraftstoff weist
eine Cetan-Zahl von \CN{} = 55, eine kinematische ViskositÃ¤t von
...
Dieselmotor besitzt ein VerdichtungsverhÃ¤ltnis von $\rc = 18$, einen
...
Kombination die Verschlei\ss{}rate um bis zu \SI{28}{\percent} gegenÃ¼ber
...
Herausforderung, bei steigenden Umweltauflagen gleichzeitig hÃ¶chste
ZuverlÃ¤ssigkeit und maximale Lebens\-dauer sicherzustellen. Ein Dieselmotor
interagiert mit seinem Kraftstoff auf vielfÃ¤ltige Weise: Kraftstoff ist
nicht nur EnergietrÃ¤ger, sondern auch Schmiermittel fÃ¼r Einspritzpumpe
und DÃ¼senbaugruppen, Transportmedium fÃ¼r Additive und direkter Einflussfaktor
auf die VerbrennungsqualitÃ¤t~\cite{heywood1988}.

\textbf{Problemstellung.} Die bisherige Forschung hat Kraftstoff- und
Motoroptimierung hÃ¤ufig getrennt betrachtet. Diese Arbeit verfolgt einen
...
\textbf{BeitrÃ¤ge dieser Arbeit:}
\begin{enumerate}[label=(\roman*)]
  \item Mathematischer Nachweis der optimalen Cetan-Zahl, ViskositÃ¤t,
        Schmierfaehigkeit und OxidationsstabilitÃ¤t des neuen Kraftstoffs
        (Abschnitt~\ref{sec:kraftstoff}).
  \item Konstruktive Auslegung des dazu passenden Dieselmotors mit
        VerdichtungsverhÃ¤ltnis, Einspritzstrategie und Lagersystem
        (Abschnitt~\ref{sec:motor}).
  \item Thermodynamischer Beweis der WirkungsgradoptimalitÃ¤t
...
\begin{enumerate}
  ...
  \item \textbf{2$\to$3}: Isobare WÃ¤rmezufuhr bei konstantem Druck~$p_2 = p_3$,
  \item \textbf{3$\to$4}: Isentrope Expansion ($V_3 \to V_4 = V_1$),
  \item \textbf{4$\to$1}: Isochore WÃ¤rmeabfuhr bei konstantem Volumen~$V_1$.
\end{enumerate}
...
wobei $\rc = V_1 / V_2$ das VerdichtungsverhÃ¤ltnis,
$\rcut = V_3 / V_2$ das AbschneideverhÃ¤ltnis und
...
Der thermische Wirkungsgrad $\etath(\rc, \rcut)$ ist fÃ¼r $\rc > 1$ streng
...
und $f(1)=0$, sodass $f(\rcut)>0$ fÃ¼r $\rcut > 1$, also
...
$F$ die Normalkraft, $s$ der Gleitweg und $H$ die VickershÃ¤rte des weicheren
...
mit effektiver KontaktflÃ¤che $A_{\mathrm{eff}}$.

\subsection{Elastohydrodynamische Schmierung (EHD)}
\label{sec:grundlagen_ehd}

Die Dowson-Higginson-Gleichung fÃ¼r die minimale Schmierfilmdicke~$h_{\min}$
...
wobei $\alpha$ der DruckviskositÃ¤tskoeffizient, $E' = 2\left[
\frac{1-\nu_1^2}{E_1} + \frac{1-\nu_2^2}{E_2}\right]^{-1}$ der
reduzierte ElastizitÃ¤tsmodul, $u_s$ die Gleit\-geschwindigkeit,
$\eta_0$ die dynamische ViskositÃ¤t bei Umgebungsdruck und $R_x$
der Ã¤quivalente KrÃ¼mmungsradius ist.
...
und VollstÃ¤ndigkeitskoeffizient $a = 6{,}908$. Die
WÃ¤rmefreisetzungsrate lautet:
...
Ein Dieselkraftstoff, der fÃ¼r maximale Motorlebensdauer ausgelegt ist,
muss folgende Eigenschaften optimieren:

\begin{enumerate}[label=(\alph*)]
  \item \textbf{Hohe Cetan-Zahl}: KÃ¼rzerer ZÃ¼ndverzug reduziert den
        Druckanstiegsgradienten und damit die mechanische Belastung.
  \item \textbf{Optimale ViskositÃ¤t}: Sowohl zu geringe als auch zu hohe
        ViskositÃ¤t erhÃ¶hen den Verschlei\ss{} von Einspritzpumpe und DÃ¼sen.
  \item \textbf{Hohe Schmierfaehigkeit}: Direkte Reduzierung des
        Archard-Koeffizienten~$K$ in Gleichung~\eqref{eq:archard}.
  \item \textbf{Hohe OxidationsstabilitÃ¤t}: Verhindert Ablagerungen,
        die EinspritzdÃ¼sen verstopfen und den Reibwert erhÃ¶hen.
  \item \textbf{Niedriger Schwefelgehalt}: Reduziert sÃ¤urebildende
        Reaktionen im SchmierÃ¶l.
\end{enumerate}

\subsection{Mathematische Optimierung der Cetan-Zahl}
\label{sec:kraftstoff_cetan}

Der ZÃ¼ndverzug $\tau_{id}$ hÃ¤ngt exponentiell von der Cetan-Zahl ab~\cite{heywood1988}:
...
ist proportional zur quadratischen ZÃ¼ndverzugsrate:
...
$\CN^* = 55$ das Optimum fÃ¼r das Gesamtsystem.
\end{theorem}
...
\end{equation}
also eine Verbesserung von \SI{36{,}2}{\percent}. FÃ¼r $\CN = 60$ wÃ¤re
die weitere Reduktion auf $e^{-0{,}9} \approx 0{,}407$; jedoch erhÃ¶hen
sich dann die Kraftstoffproduktionskosten und die Hydrierungstiefe erheblich,
sodass $\CN^* = 55$ den besten Kompromiss darstellt.
\end{proof}

\subsection{Mathematische Optimierung der ViskositÃ¤t}
\label{sec:kraftstoff_viskositaet}

Die Walther-Gleichung beschreibt die TemperaturabhÃ¤ngigkeit der
kinematischen ViskositÃ¤t~\cite{astm_d341}:
...
\begin{definition}[Optimaler ViskositÃ¤tsbereich]
Der optimale Bereich fÃ¼r Einspritzpumpen liegt bei
$\nu_{40} \in [2{,}0;\ 4{,}5]\ \si{\mm\squared\per\s}$,
wie von ISO~4406 und ASTM~D975 festgelegt.
\end{definition}

FÃ¼r den neuen Kraftstoff wird $\nu_{40} = \SI{3{,}5}{\mm\squared\per\s}$
gewÃ¤hlt. Dies liegt im Zentrum des optimalen Bereichs und gewÃ¤hrleistet
eine EHD-Filmdicke:
...
benÃ¶tigte Mindestkonzentration ergibt sich durch Invertierung:
...
FÃ¼r den neuen Kraftstoff wird $c^* = \SI{300}{\mg\per\kg}$ gewÃ¤hlt,
was eine $\WSD = \SI{205}{\micro\m}$ ergibt -- weit unterhalb beider Grenzwerte.

\subsection{Optimierung der OxidationsstabilitÃ¤t}
\label{sec:kraftstoff_oxidation}

Die OxidationsstabilitÃ¤t wird durch die Rancimat-Induktionszeit $t_{\mathrm{ind}}$
...
\SI{200}{\mg\per\kg} BHT-Ã¤quivalent) wird $t_{\mathrm{ind}} = \SI{22}{\hour}$
erreicht -- 10\,\% Ã¼ber dem EN~590-Minimum von \SI{20}{\hour}.
...
\begin{table}[H]
  ...
  \begin{tabular}{lllS[table-format=3.1]}
    ...
    Kinematische ViskositÃ¤t (40Â°C) & $\si{\mm\squared\per\s}$ & $2{,}0\ldots 4{,}5$ & 3{,}5 \\
    ...
    Dichte (15Â°C) & $\si{\kg\per\m\cubed}$ & $820\ldots 845$ & 838{,}0 \\
...
\begin{itemize}
  \item VerdichtungsverhÃ¤ltnis $\rc = 18$ (optimaler Wirkungsgrad, s.\
        Abschnitt~\ref{sec:thermodynamik}),
  \item Common-Rail-Einspritzdruck $p_{\mathrm{inj}} = \SI{1800}{\bar}$,
  \item Mehrfacheinspritzung: Voreinspritzung ($-15Â°$ KW) + Haupteinspritzung ($-5Â°$ KW),
  \item Turbolader mit LadeluftkÃ¼hler (LadedruckverhÃ¤ltnis~1{,}8),
...
Durch eine Voreinspritzung von 15\,\% der Gesamtmasse bei $\varphi_{\mathrm{pre}} = -15Â°$ KW
...
wobei $F_{\mathrm{gas}}$ die Gaskraft, $F_{\mathrm{traeg}}$ die MassentrÃ¤gheitskraft,
$d_L$ der Lagerdurchmesser und $b_L$ die Lagerbreite ist. FÃ¼r maximale Lebensdauer
wird $p_{\mathrm{Lager}} \leq \SI{20}{\MPa}$ angestrebt.

Die Mindestsommerfeld-Zahl $\mathrm{So}$ fÃ¼r Vollschmierung lautet~\cite{DIN31651}:
...
mit Lagerspiel $c$, Drehzahl $n$ und ViskositÃ¤t $\eta$ des SchmierÃ¶ls.

%% ------------------------------------------------------------------
\section{Thermodynamischer Nachweis der WirkungsgradoptimalitÃ¤t}
\label{sec:thermodynamik}
%% ------------------------------------------------------------------

\subsection{Optimales VerdichtungsverhÃ¤ltnis}
...
\begin{theorem}[Optimales VerdichtungsverhÃ¤ltnis]
\label{thm:rc_opt}
Unter den Nebenbedingungen $T_3 \leq \SI{2500}{\K}$ und
$p_{\max} \leq \SI{200}{\bar}$ sowie fÃ¼r $\CN^* = 55$,
$\rcut = 2{,}0$ und $\gamma = 1{,}35$ ist das optimale
VerdichtungsverhÃ¤ltnis $\rc^* = 18$.
\end{theorem}
...
thermischen Grenze. Der thermische Wirkungsgrad betrÃ¤gt:
\begin{equation}
...
Der optimierte Kraftstoff hat eine hÃ¶here untere Heizwert-Effizienz
...
$\eta_c = 0{,}98$ betrÃ¤gt der effektive indizierte Wirkungsgrad:
...
\subsection{Archard-Nachweis fÃ¼r Einspritzkomponenten}
...
wobei $v_s$ die Gleitgeschwindigkeit ist. FÃ¼r den optimierten Kraftstoff
...
\subsection{EHD-Nachweis fÃ¼r Kurbelwellenlager}
\label{sec:tribologie_ehd}

Mit dem neuen Kraftstoff (hÃ¶here ViskositÃ¤t im Betrieb durch geringere
VerdÃ¼nnung) ergibt sich nach Dowson-Higginson (Gl.~\eqref{eq:dhg}):
...
Die Lagerlebensdauer wird also um \SI{72}{\percent} erhÃ¶ht.

\subsection{Kolbenringverschlei\ss{} -- Integrales Modell}
\label{sec:tribologie_kolben}

Das integrale Verschlei\ss{}modell fÃ¼r Kolbenringe kombiniert
Einlaufverschlei\ss{} und stationÃ¤ren Betrieb~\cite{taylor1998}:
...
Einlaufexponent $b = 0{,}4$ und stationÃ¤rer Rate $c$
...
Der Gewinn betrÃ¤gt $\Delta L = \SI{12\,000}{\hour}$ = \SI{44}{\percent}
mehr Lebensdauer durch den optimierten Kraftstoff.

\subsection{Gesamtlebensdauernachweis}
\label{sec:tribologie_gesamt}

Die Gesamtmotorlebensdauer $L_{\mathrm{ges}}$ wird durch das schwÃ¤chste Glied
bestimmt. Mit Hilfe der S-N-Kurven-Methode (WÃ¶hler) und Monte-Carlo-Simulation
ergibt sich:
...
Dieselzyklus bei $\rc = 18$ und $\rcut = 2{,}2$. Die schraffierte FlÃ¤che
entspricht der nutzbaren Arbeit. Der spÃ¼r\-bare Anstieg der ArbeitsflÃ¤che
mit steigendem VerdichtungsverhÃ¤ltnis (Abb.~\ref{fig:thermal_efficiency})
bestÃ¤tigt Satz~\ref{thm:eta_monoton} numerisch.
...
\begin{figure}[h!]
  ...
  \caption{P-V-Diagramm des idealisierten Dieselzyklus bei $\rc = 18$,
           ...
           Zustandspunkte; die schraffierte FlÃ¤che entspricht der
           indizierten Arbeit.}
...
\begin{figure}[h!]
  ...
  \caption{Thermischer Wirkungsgrad $\etath$ als Funktion des
           VerdichtungsverhÃ¤ltnisses $\rc$ fÃ¼r verschiedene
           AbschneideverhÃ¤ltnisse $\rcut$. Der rote Stern markiert
           das gewÃ¤hlte Optimum bei $\rc = 18$, $\rcut = 2{,}0$.}
...
ZÃ¼ndverzÃ¶gerung und die daraus folgende Reduktion der relativen
Verschlei\ss{}rate mit steigender Cetan-Zahl. Bei CN = 55 betrÃ¤gt
...
\begin{figure}[h!]
  ...
  \caption{Links: ZÃ¼ndverzug $\tau_{id}$ vs. Cetan-Zahl. Rechts:
...
\subsection{ViskositÃ¤t und Schmierfaehigkeit}
\label{sec:ergebnis_visko}

Abbildung~\ref{fig:viscosity_temp} zeigt die Walther-ViskositÃ¤tskurven
...
\begin{figure}[h!]
  ...
  \caption{Kinematische ViskositÃ¤t als Funktion der Temperatur nach
           Walther-Gleichung. Der grÃ¼n schraffierte Bereich markiert den
...
\begin{figure}[h!]
  ...
  \caption{HFRR-Verschlei\ss{}narbe~$\WSD$ als Funktion der
           Additivkonzentration fÃ¼r Standard- und optimierten Diesel.
...
GrÃ¶\ss{}en fÃ¼r die Einspritzdruckwahl: SpruehqualitÃ¤t, Verbrennungswirkungsgrad
...
\subsection{WÃ¤rmefreisetzung und Zylinderdruckverlauf}
\label{sec:ergebnis_waerme}

Die Wiebe-Analyse (Abb.~\ref{fig:heat_release}) belegt, dass der optimierte
Kraftstoff mit CN = 55 eine frÃ¼here, gleichmÃ¤\ss{}igere WÃ¤rmefreisetzung
...
\begin{figure}[h!]
  ...
  \caption{Links: WÃ¤rmefreisetzungsrate $dQ/d\varphi$ nach Wiebe-Funktion.
           Rechts: Kumulierter Umsatzgrad $x_Q$ fÃ¼r Standard- und
           optimierten Diesel.}
...
\begin{figure}[h!]
  ...
  \caption{Zylinderdruckverlauf wÃ¤hrend der Verbrennung fÃ¼r Standard-
...
\begin{figure}[h!]
  ...
  \caption{Links: Emissionen als Funktion des Einspritzzeitpunkts.
           Rechts: $\NOx$-PM-Tradeoff-Kurven fÃ¼r Standard- und
           optimierten Diesel.}
  \label{fig:nox_pm}
\end{figure}

\subsection{OxidationsstabilitÃ¤t}
\label{sec:ergebnis_oxidation}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_oxidation_stability.pdf}
  \caption{OxidationsstabilitÃ¤t: Peroxidwert-Zeitverlauf (Rancimat,
...
Die EHD-Analyse (Abb.~\ref{fig:ehd_film}) bestÃ¤tigt, dass die optimierte
ViskositÃ¤t von \SI{4{,}5}{\milli\Pa\s} eine Schmierfilmdicke im sicheren
Bereich hydrodynamischer Schmierung ($\Lambda > 2$) gewÃ¤hrleistet.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_ehd_film.pdf}
  \caption{Links: Minimale EHD-Schmierfilmdicke $h_{\min}$ nach
           Dowson-Higginson als Funktion der dynamischen ViskositÃ¤t.
...
\begin{figure}[h!]
  ...
  \caption{Stribeck-Kurve fÃ¼r Standard- und optimierten Diesel.
...
Spinnennetz-Plot zusammen. Der optimierte Diesel Ã¼bertrifft den EN~590-Standard
...
\begin{figure}[h!]
  ...
  \caption{Bewertungsmatrix aller Kraftstoffe nach technischen und
           wirtschaftlichen Kriterien (Farbe: Rot = schlecht, GrÃ¼n = gut).}
  \label{fig:score_matrix}
\end{figure}

%% ------------------------------------------------------------------
\section{Quantitativer Gesamtnachweis der LebensdauerverlÃ¤ngerung}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textit{Thermische Belastung}: HÃ¶here CN reduziert
        ...
        wÃ¤rmebeanspruchte Bauteile bedeutet.
\end{enumerate}
...
Die wichtigsten Erkenntnisse im Ã¼berblick:
...
\begin{enumerate}
  ...
  \item \textbf{Schmierfaehigkeit}: $\WSD = \SI{205}{\micro\m}$ verlÃ¤ngert
...
Archards Modell basiert auf der Annahme, dass AsperitÃ¤te abgleiten und
hemisphÃ¤rische Verschlei\ss{}partikel der Radien~$a_i$ hinterlassen. FÃ¼r
...
AsperitÃ¤tradius $\bar{a}$ ergibt sich:
...
%% ------------------------------------------------------------------
\section{Numerische Parameter fÃ¼r Simulationen}
\label{app:parameter}
%% ------------------------------------------------------------------
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    OberflÃ¤chenrauheit $\sigma$ & \SI{300}{\nm} & Angenommen \\
    \bottomrule
  \end{tabular}
\end{table}
...
\bibitem{kerschbaumer2002}
  H.~G. Kerschbaumer.
  SchmierfÃ¤higkeit von Dieselkraftstoffen -- Einfluss der Kraftstoffzusammensetzung
...
\bibitem{mollenhauer2010}
  K.~Mollenhauer und H.~TschÃ¶ke (Hrsg.).
...
\bibitem{DIN31651}
  ...
  \textit{DIN~31651: Berechnung von Gleitlagerungen -- KreisfÃ¶rmige Zylindergleitlager}.
