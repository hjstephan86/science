...
\begin{figure}[h!]
	...
	\caption{Direkter visueller Beweis der Äquivalenz $\nabla f = J_f^T$: Links das Gradientenfeld $\nabla f$ (grüne Pfeile), rechts das transponierte Jacobi-Feld $J_f^T$ (rote Pfeile). Beide Vektorfelder sind identisch â€” jeder Pfeil hat dieselbe Richtung und Länge. Dies illustriert Theorem~\ref{thm:gradient_jacobi}: Der Gradient ist kein eigenständiges Konzept, sondern die Transponierte der Jacobi-Matrix.}
	\label{fig:plot03}
\end{figure}
...
\newpage
\subsection{Plot 5: Gradientenabstieg â€” äquivalente Jacobi-Formulierung}
...
\begin{figure}[h!]
	...
	\caption{Visualisierung des impliziten Funktionssatzes anhand des Einheitskreises $F(x,y) = x^2+y^2-1=0$: Die Gradienten $\nabla F = (2x,2y)^T = J_F^T$ (blaue Pfeile) stehen überall senkrecht auf der Kurve. Die implizite Ableitung $dy/dx = -J_{F,x}/J_{F,y} = -x/y$ ist eine reine Jacobi-Formulierung, für die kein Gradienten-Analogon im Mehrkomponentenfall existiert â€” ein Fall, in dem die Jacobi-Matrix strukturell unersetzlich ist.}
	\label{fig:plot07}
\end{figure}

\subsection{Plot 8: Zusammenfassung â€” Äquivalenz und Verallgemeinerung}
...
\begin{proof}
	...
	\]
	Diese Aktualisierung benötigt ausschließlich die Matrix $G^{(l)} \in \mathbb{R}^{n_l \times n_{l-1}}$ â€” keine anderen partiellen Ableitungen. Die blockdiagonale Struktur:
	\[
