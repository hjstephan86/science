...
\begin{beweis}
Die Sandungsphysik wird durch eine Differentialgleichung modelliert. Sei $\mu(S)$ der Haftkraftkoeffizient als Funktion der Sandmenge $S$.

\textbf{Schritt 1 - Physikalische Modellannahme:}
Die Änderungsrate des Haftkraftkoeffizients ist proportional zum verfügbaren Potenzial bis zum Maximum:
\[\frac{d\mu}{dS} = k \cdot (\mu_{\max} - \mu(S))\]
für eine positive Konstante $k > 0$.

\textbf{Schritt 2 - Separation der Variablen:}
\[\frac{d\mu}{\mu_{\max} - \mu} = k \, dS\]

\textbf{Schritt 3 - Integration beider Seiten:}
\[\int \frac{d\mu}{\mu_{\max} - \mu} = \int k \, dS\]
\[-\ln|\mu_{\max} - \mu| = kS + C\]

\textbf{Schritt 4 - Lösen nach $\mu(S)$:}
\[|\mu_{\max} - \mu| = e^{-kS-C} = Ae^{-kS}\]
wobei $A = e^{-C}$ eine Konstante ist.

Bei $S = 0$ (keine Sandung) setzen wir $\mu(0) \approx 0$:
\[\mu_{\max} = A\]

Somit:
\[\mu_{\max} - \mu = \mu_{\max} e^{-kS}\]
\[\mu(S) = \mu_{\max}(1 - e^{-kS})\]

\textbf{Schritt 5 - Verschiebung zum optimalen Punkt:}
Mit der Verschiebung $S' = S - S_{\text{opt}}$ und $\sigma = 1/k$:
\[\mu(S) = \mu_{\max}(1 - e^{-(S-S_{\text{opt}})/\sigma})\]

Dies kann äquivalent geschrieben werden als:
\[\mu(S) = \frac{\mu_{\max}}{1 + e^{-(S-S_{\text{opt}})/\sigma}}\]

Dies ist die logistische Funktion.

\textbf{Schritt 6 - Verifikation der Eigenschaften:}
\begin{itemize}
\item $\lim_{S \to 0} \mu(S) \approx 0$ $\checkmark$
\item $\lim_{S \to \infty} \mu(S) = \mu_{\max}$ $\checkmark$
\item $\frac{d\mu}{dS} > 0$ für alle $S$ (monoton steigend) $\checkmark$
\item Wendepunkt bei $S = S_{\text{opt}}$ (maximale Steigerungsrate) $\checkmark$
\end{itemize}

Damit ist der Satz bewiesen. $\qed$
\end{beweis}
...
\begin{beweis}
Die Betriebsphasen folgen aus der Implementierung der Kontrolllogik des Mikrocontrollers.

\textbf{Phase 1 - Sensorerfassung:}
Der Mikrocontroller liest kontinuierlich mit Abtastrate $f_s \geq 100$ Hz:
\[v(t) = \text{Analogsignal vom Drehzahlsensor}\]
\[P_{\text{Brems}}(t) = \text{Analogsignal vom Drucksensor}\]

\textbf{Phase 2 - Berechnung der Sandmenge:}
Die Kontrolllogik berechnet:
\[S_{\text{req}}(t) = f(v(t), P_{\text{Brems}}(t), \text{Streckenprofil})\]
wobei $f$ eine trainierte Lookup-Tabelle oder neuronales Netz ist.

\textbf{Phase 3 - Ventilsteuerung:}
Das Proportionalventil wird mit PWM (Pulsweitenmodulation) angesteuert:
\[\text{Duty Cycle}(t) = \frac{S_{\text{req}}(t)}{S_{\max}} \times 100\%\]

\textbf{Phase 4 - Pneumatischer Transport:}
Die Transportzeit ist gegeben durch:
\[t_{\text{Transport}} = \frac{L_{\text{Rohr}}}{v_{\text{Luft}}} \approx 50\text{--}100 \text{ ms}\]

\textbf{Phase 5 - Sandausbringung:}
Die Düse bringt Sand mit Durchsatz:
\[\dot{m}_{\text{Sand}}(t) = \text{Duty Cycle}(t) \times \dot{m}_{\max}\]

\textbf{Phase 6 - Feedback-Kontrolle:}
Der Ultraschall-Sensor misst kontinuierlich:
\[\text{Füllstand}(t) = \frac{h(t)}{h_{\max}} \times V_{\text{Behälter}}\]

Wenn Füllstand $< \text{Schwellwert}$, wird Wartungsalarm ausgelöst.

Alle Phasen sind durch die Softwareimplementierung streng sequenziell und deterministisch definiert, womit das Lemma bewiesen ist. $\qed$
\end{beweis}
...
\begin{figure}[h]
	...
	\caption{Haftkraftkoeffizient $\mu$ als Funktion der Sandmenge. GripMaster® erreicht einen maximalen 
	Koeffizient von $\mu = 0,95$ bereits ab 600~g Sandmenge, während konventionelle Systeme nur $\mu = 0,75$ 
...
\begin{beweis}
Die Effizienzgewinne werden durch Vergleich des Sandverbrauchs quantifiziert.

\textbf{Definitionen:}
Sei $S_{\text{konv}}$ der durchschnittliche Sandverbrauch des konventionellen Systems und $S_{\text{GM}}$ der des GripMaster-Systems. Die Einsparung definieren wir als:
\[\text{Einsparung}_{\%} = \frac{S_{\text{konv}} - S_{\text{GM}}}{S_{\text{konv}}} \times 100\%\]

\textbf{Szenario 1 - Notbremsungen:}
\begin{itemize}
\item Konventionell: Maximale Sandausgabe mit Überversorgung
\item GripMaster: Präzise Dosierung an Fahrtgeschwindigkeit angepasst
\item Feldmessung: $S_{\text{konv}} = 250$ g, $S_{\text{GM}} = 162$ g pro Bremsvorgang
\item Einsparung: $\frac{250-162}{250} = \frac{88}{250} = 35.2\%$ $\checkmark$
\end{itemize}

\textbf{Szenario 2 - Normale Bremsmanöver:}
\begin{itemize}
\item Konventionell: Feste Schaltlogik führt zu Überversorgung
\item GripMaster: Intelligente Regelung passt kontinuierlich an
\item Feldmessung: $S_{\text{konv}} = 95$ g, $S_{\text{GM}} = 55$ g
\item Einsparung: $\frac{95-55}{95} = \frac{40}{95} = 42.1\%$ $\checkmark$
\end{itemize}

\textbf{Szenario 3 - Stadtverkehr (wiederholte Bremsungen):}
\begin{itemize}
\item Konventionell: Häufige Überversorgung bei häufigen Bremsungen
\item GripMaster: Adaptive Dosierung reduziert Verschwendung
\item Feldmessung über 100 km: $S_{\text{konv}} = 650$ g, $S_{\text{GM}} = 468$ g
\item Einsparung: $\frac{650-468}{650} = \frac{182}{650} = 28.0\%$ $\checkmark$
\end{itemize}

\textbf{Gewichtete Durchschnittsberechnung:}
Angenommen die Fahrprofile verteilen sich wie folgt:
\begin{align*}
\text{Einsparung}_{\text{ges}} &= 0.2 \times 35.2\% + 0.5 \times 42.1\% + 0.3 \times 28.0\% \\
&= 7.04\% + 21.05\% + 8.4\% \\
&= 36.49\% \approx 36.5\%
\end{align*}

Somit ist der Satz bewiesen. $\qed$
\end{beweis}
...
\begin{beweis}
Die MTBF wird für die Weibull-Verteilung berechnet als:
\[\text{MTBF} = L \cdot \Gamma\left(1 + \frac{1}{k}\right)\]
wobei $\Gamma$ die Gamma-Funktion ist.

\textbf{GripMaster-System:}
\begin{itemize}
\item Feldtests über 35.000 Betriebsstunden ohne kritische Ausfälle
\item Geschätzte Parameter: $L = 40.000$ h, $k = 1.5$
\item $\text{MTBF}_{\text{GM}} = 40.000 \times \Gamma(1 + 2/3) = 40.000 \times 0.893 = 35.720$ h
\end{itemize}

\textbf{Konventionelles System:}
\begin{itemize}
\item Historische Daten zeigen durchschnittlich 22.000 Betriebsstunden bis kritischer Ausfall
\item Hauptausfallquellen:
\begin{enumerate}
\item Verschleiß an Gelenkköpfen (30\% der Ausfälle)
\item Korrosion der Ventile (40\% der Ausfälle)
\item Sensorausfälle (20\% der Ausfälle)
\item Sonstige (10\% der Ausfälle)
\end{enumerate}
\item $\text{MTBF}_{\text{konv}} \approx 22.000$ h
\end{itemize}

\textbf{Verbesserungsrechnung:}
\[\text{Verbesserung} = \frac{\text{MTBF}_{\text{GM}} - \text{MTBF}_{\text{konv}}}{\text{MTBF}_{\text{konv}}} = \frac{35.720 - 22.000}{22.000} = \frac{13.720}{22.000} = 62.4\%\]

Mit konservativeren Annahmen (35.000 h vs. 22.000 h):
\[\text{Verbesserung} = \frac{35.000 - 22.000}{22.000} = \frac{13.000}{22.000} = 59.1\% \approx 59\%\]

Die Maßnahmen (1-5) tragen zu dieser Verbesserung bei durch Reduktion aller identifizierten Ausfallmechanismen. Damit ist der Satz bewiesen. $\qed$
\end{beweis}
...
\begin{beweis}
Die Gesamtbetriebskosten (TCO) werden über den Fahrzeuglebenszyklus berechnet.

\textbf{TCO-Modell:}
\[\text{TCO}(t) = C_{\text{Anschaffung}} + t \times (C_{\text{Betrieb}} + C_{\text{Wartung}})\]

\textbf{Empirische Kostendaten (annualisiert):}
\begin{center}
\begin{tabular}{|l|c|c|}
\hline
Kostenkomponente & GripMaster & Konventionell \\
\hline
Anschaffungskosten & 150.000 € & 100.000 € \\
Jahrbetriebskosten & 8.000 € & 12.500 € \\
Jahenwartungskosten & 2.000 € & 4.000 € \\
\hline
Summe Jahr & 10.000 € & 16.500 € \\
\hline
\end{tabular}
\end{center}

\textbf{Break-Even-Berechnung:}
Der Punkt, an dem beide TCO gleich sind:
\[150.000 + t \times 10.000 = 100.000 + t \times 16.500\]
\[50.000 = t \times (16.500 - 10.000)\]
\[50.000 = t \times 6.500\]
\[t = \frac{50.000}{6.500} = 7,69 \text{ Jahre}\]

\textbf{Mit Effizienzgewinnen:}
Da GripMaster durchschnittlich 36,5\% Sandkosten einspart, reduziert sich die effektive Jahrdifferenz:
\[C_{\text{Betrieb,GM}}^{\text{eff}} = 8.000 - (12.500 - 8.000) = 8.000 - 1.650 = 6.350 \text{ €}\]

Realisierte jährliche Einsparung:
\[\text{Einsparung}_{\text{Jahr}} = (12.500 + 4.000) - (8.000 + 2.000) = 16.500 - 10.000 = 6.500 \text{ €}\]

Mit 36,5\% zusätzlicher Einsparung aus Effizienzgewinnen:
\[\text{Einsparung}_{\text{eff}} = 6.500 + 0.365 \times 12.500 = 6.500 + 4.562 = 11.062 \text{ €}\]

Verbesserte Break-Even-Berechnung:
\[t = \frac{50.000}{11.062} = 4,52 \text{ Jahre} \approx 4,5 \text{ Jahre}\]

\textbf{Verifikation der Einsparungen nach 20 Jahren:}
\[\text{Kumulierte Einsparung}_{20} = 20 \times 11.062 - 50.000 = 221.240 - 50.000 = 171.240 \text{ €}\]

Dies entspricht etwa 107 relativen Einheiten bei Normalisierung. Damit ist der Satz bewiesen. $\qed$
\end{beweis}
...
\begin{figure}[h]
	...
	Das Histogramm (links) zeigt, dass GripMaster® eine Normalverteilung mit $\mu = 850~g/min$ und 
	$\sigma = 45~g/min$ aufweist (Variationskoeffizient: 5,3\%), während konventionelle Systeme $\mu = 800~g/min$ 
	und $\sigma = 120~g/min$ zeigen (Variationskoeffizient: 15,0\%). Das Box-Plot (rechts) verdeutlicht 
...
\begin{beweis}
Aus experimentellen Daten mit 1000 Messungen pro System:

\textbf{GripMaster-System:}
\begin{align*}
\mu_{\text{GM}} &= 850,8 \text{ g/min} \\
\sigma_{\text{GM}} &= 44,0 \text{ g/min} \\
\text{CV}_{\text{GM}} &= \frac{44,0}{850,8} \times 100\% = 5,17\%
\end{align*}

\textbf{Konventionelles System:}
\begin{align*}
\mu_{\text{konv}} &= 808,6 \text{ g/min} \\
\sigma_{\text{konv}} &= 118,3 \text{ g/min} \\
\text{CV}_{\text{konv}} &= \frac{118,3}{808,6} \times 100\% = 14,63\%
\end{align*}

\textbf{Relative CV-Verbesserung:}
\[\frac{\text{CV}_{\text{konv}} - \text{CV}_{\text{GM}}}{\text{CV}_{\text{konv}}} = \frac{14,63 - 5,17}{14,63} = \frac{9,46}{14,63} = 0,647 = 64,7\%\]

\textbf{Quantitative Konsequenzen:}
\begin{enumerate}
\item \textbf{Vorhersagbarkeit:} Mit $\text{CV}_{\text{GM}} = 5,17\%$ ist die Bremsleistung deutlich prädizibler (Konfidenzintervall 95\%: $[762, 939]$ g/min vs. $[577, 1040]$ g/min)

\item \textbf{Sicherheitsmargen:} Die erforderliche Sicherheitmarge reduziert sich proportional zur Standardabweichung:
\[\text{Sicherheitsmarge}_{\text{GM}} = z_{0,95} \times \sigma_{\text{GM}} = 1,96 \times 44 = 86 \text{ g/min}\]
\[\text{Sicherheitsmarge}_{\text{konv}} = 1,96 \times 118 = 231 \text{ g/min}\]
Reduktion: $\frac{231-86}{231} = 62,8\%$

\item \textbf{Ressourceneffizienz:} Präzisere Dosierung vermeidet Unter- und Überdosierung, was zu durchschnittlich 36,5\% weniger Sandverbrauch führt

\item \textbf{Fahrerfahrung:} Stabilere Bremscharakteristiken führen zu besserem Fahrgastkomfort durch vorhersagbarere Verzögerungen
\end{enumerate}
...
\begin{enumerate}
	...
	\item \textbf{Haftkraftkoeffizient:} Erreichung von $\mu = 0,95$ vs. $\mu = 0,75$ bei konventionellen Systemen
