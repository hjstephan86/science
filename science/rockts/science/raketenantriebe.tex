...
des MassenverhÃ¤ltnisses. Ein $\Dv$ von $\SI{9.4}{km/s}$ (niedrige Erdumlaufbahn)
erfordert bei $\ve = \SI{4.4}{km/s}$ (kryogenes $\mathrm{H_2/O_2}$-Triebwerk)
ein MassenverhÃ¤ltnis von etwa 8.}
\label{fig:tsiol}
\end{figure}
...
\begin{tikzpicture}[scale=1.2]
  ...
  % StrÃ¶mungslinien im DÃ¼senkanal
  ...
  % Dreieckige Pfeile fÃ¼r StrÃ¶mungsrichtung
  ...
  \fill[pattern=north east lines, pattern color=gray!50]
    ...
    -- (-3.5,-\Rc-0.15) -- cycle;
\end{tikzpicture}
\caption{Meridianschnitt einer konvergent-divergenten LavaldÃ¼se. In der Brennkammer
herrschen hoher Druck $p_c$ und hohe Temperatur $T_c$. Am DÃ¼senhals wird Schallgeschwindigkeit
($M=1$) erreicht; im divergenten Teil wird die StrÃ¶mung Ã¼berschallartig und $v_e$ maximiert.}
\label{fig:laval}
\end{figure}

\subsection{Isentrope StrÃ¶mungsbeziehungen}

FÃ¼r ein ideales Gas mit Isentropenexponent $\gamma$ gelten entlang der DÃ¼senstrÃ¶mung
...
Die \textbf{AusstrÃ¶mgeschwindigkeit} ergibt sich aus dem Energiesatz (EnthalpiegefÃ¤lle):
...
\]
Mit $v_c \approx 0$ (DÃ¼senhals-Approx.) und $h = c_p T$:
\[
...
und $A_e$ die AustrittsflÃ¤che ist. Optimale Expansion: $p_e = p_a$.
\end{satz}

\begin{definition}[Spezifischer Impuls]
Der spezifische Impuls $\Isp$ ist das MaÃŸ fÃ¼r die Treibstoffeffizienz eines Triebwerks:
\[
...
Ein FlÃ¼ssigraketentriebwerk (engl. \emph{Liquid Rocket Engine}, LRE) verbrennt
flÃ¼ssige Treibstoffe --- meist Brennstoff (Fuel) und Oxidator (Oxidizer) getrennt ---
...
\begin{tikzpicture}[scale=0.95]
  ...
  \node[font=\scriptsize] at (-4.35, 3.1) {flÃ¼ssiger};
  \node[font=\scriptsize] at (-4.35, 2.7) {Sauerstoff};
  \node[font=\scriptsize] at (-4.35, 2.2) {$T=-183\,Â°C$};
  % FÃ¼llstand
  ...
  \node[font=\scriptsize] at (0.65, 3.1) {flÃ¼ssiger};
  \node[font=\scriptsize] at (0.65, 2.7) {Wasserstoff};
  \node[font=\scriptsize] at (0.65, 2.2) {$T=-253\,Â°C$};
  ...
  % RegenerativkÃ¼hlung (Pfeile an DÃ¼senwand)
  \foreach \y in {-1.4,-1.8,-2.2,-2.6,-3.0,-3.4} {
    \draw[-{Stealth[length=3pt]}, gray!60, line width=0.4pt]
      (-3.75,\y) -- (-3.55,\y);
  }
  \node[font=\scriptsize, gray!80, rotate=90] at (-3.9,-2.4) {KÃ¼hlung};
\end{tikzpicture}
\caption{Schematischer Aufbau eines kryogenen FlÃ¼ssig-Raketentriebwerks (LOX/LH$_2$).
Beide Treibstoffkomponenten werden durch Turbopumpen in die Brennkammer gefÃ¶rdert,
dort verbrannt und durch die LavaldÃ¼se expandiert.}
\label{fig:lre}
\end{figure}

\subsection{Verbrennungstemperatur und Heizwert}

Die Verbrennungstemperatur $T_c$ beeinflusst $v_e$ maÃŸgeblich. FÃ¼r die Reaktion
von Wasserstoff mit Sauerstoff:
\[
  \mathrm{2\,H_2 + O_2 \longrightarrow 2\,H_2O},\quad
  \Delta H_R = -\SI{483.6}{kJ/mol}
\]
Die adiabate Flammentemperatur ergibt sich aus der WÃ¤rmebilanz:
\[
  \sum_i n_i\,c_{p,i}\,(T_c - T_{\text{ref}}) = -\Delta H_R
  \quad\Rightarrow\quad T_c \approx \SI{3500}{K}
\]

\subsection{Schubkammerauslegung --- KenngrÃ¶ÃŸen}

\begin{table}[H]
\centering
\caption{AuslegungsgrÃ¶ÃŸen fÃ¼r ein typisches LOX/LH$_2$-Triebwerk (Vakuumbetrieb)}
\label{tab:triebwerk}
\begin{tabular}{lrrl}
\toprule
...
DÃ¼senhalsdurchmesser    & $d^*$     & 0.20       & m\\
...
\textbf{Berechnung des AusstrÃ¶mimpulses:}
\begin{align}
...
sondern durch elektrische oder elektromagnetische Felder. Sie erzielen weit hÃ¶here
AusstrÃ¶mgeschwindigkeiten ($v_e = 20\ldots\SI{200}{km/s}$), haben aber sehr geringe
MassenflÃ¼sse und damit extrem kleine SchubkrÃ¤fte.
...
Ionenstromfluss ist. Die AusstrÃ¶mgeschwindigkeit ergibt sich aus:
\[
...
FÃ¼r Xenon ($m_{\mathrm{Xe}} = \SI{2.18e-25}{kg}$, $q = \SI{1.6e-19}{C}$,
$\Delta V = \SI{1200}{V}$):
\[
...
\begin{tikzpicture}[scale=1.0]
  ...
  % Elektronendrift (azimuthal, EÃ—B)
...
FÃ¼r einen Satelliten auf einer Kreisbahn im Radius $r$ um die Erde gilt
KrÃ¤ftegleichgewicht zwischen Gravitationskraft und Zentripetalkraft:
...
geostationÃ¤rem Orbit (GEO, $r_2$). Die Transferellipse hat ihre
Periapsis bei $r_1$ und Apoapsis bei $r_2$. Es werden zwei ImpulsmanÃ¶ver benÃ¶tigt.}
\label{fig:hohmann}
\end{figure}
...
\]
PerigÃ¤umsgeschwindigkeit der Transferellipse:
\[
...
\]
FÃ¼r LEO ($r_1 = \SI{6778}{km}$) $\to$ GEO ($r_2 = \SI{42164}{km}$):
\begin{align*}
...
\begin{table}[H]
\centering
\caption{Typisches $\Dv$-Budget fÃ¼r eine Mondmission (Low-Thrust vernachlÃ¤ssigt)}
\label{tab:dv_mond}
\begin{tabular}{lrl}
\toprule
...
als fÃ¼r eine Mondmission benÃ¶tigt.}
\label{fig:stufen}
\end{figure}
...
und durch eine DÃ¼se expandiert. Der Reaktor erzeugt keinerlei chemische Energie ---
der Wasserstoff verbrennt nicht, er wird nur erhitzt.

Da nur Wasserstoff ($\bar{M} = \SI{2}{g/mol}$) ausgestoÃŸen wird, ergibt sich ein
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[font=\scriptsize] at (0,-1.2) {UO$_2$/UC-BrennstÃ¤be};
  ...
  % Erhitzter H2-Auslass (rechts -> DÃ¼se)
  \draw[thick, triebwerkorange, -{Stealth}]
    (2.0,0) -- (3.2,0) node[midway, above, font=\scriptsize] {H$_2$, heiÃŸ};
  \node[font=\scriptsize, triebwerkorange] at (3.1,-0.3) {$T\approx2800\,K$};

  % DÃ¼se (konvergent-divergent)
  \draw[thick, raumblau, line width=1.5pt]
    (3.2,0.4) -- (3.2,-0.4);
  \draw[thick, raumblau, line width=1.5pt]
...
\end{tikzpicture}
\caption{Kernthermal-Raketenantrieb (NTR). FlÃ¼ssiger Wasserstoff wird durch den Reaktor
auf $\sim\SI{2800}{K}$ erhitzt und durch die DÃ¼se expandiert. Es findet \emph{keine}
Verbrennung statt --- der Reaktor liefert nur WÃ¤rme. $\Isp \approx \SI{850}{s}$.}
\label{fig:ntr}
\end{figure}
...
(doppellogarithmisch). Chemische Triebwerke liefern hohen Schub fÃ¼r die Startphase,
elektrische Antriebe hohen $\Isp$ fÃ¼r Tiefraummissionen --- ein fundamentaler Kompromiss.}
\label{fig:vergleich}
\end{figure}
...
Chemische Triebwerke bleiben fÃ¼r die Startphase alternativlos, weil sie die einzige
Technologie darstellen, die SchubkrÃ¤fte von mehreren Meganewton bei ausreichend hohem
...
FÃ¼r Missionen jenseits der Erdumlaufbahn --- Mond, Mars, Ã¤uÃŸeres Sonnensystem ---
gewinnt der spezifische Impuls zunehmend an Bedeutung. Ein Hall-Effekt-Triebwerk
mit $\Isp = \SI{1500}{s}$ benÃ¶tigt fÃ¼r dasselbe $\Dv$ nur ein MassenverhÃ¤ltnis von
$m_0/m_1 = e^{\Dv/(g_0\cdot\Isp)}$, was die benÃ¶tigte Treibstoffmasse dramatisch
reduziert --- der Preis ist jedoch die sehr geringe Schubkraft und damit lange
ManÃ¶verdauer.

Kernthermische Antriebe ($\Isp \approx 850$ s) stellen eine vielversprechende BrÃ¼cke dar:
Sie Ã¼bertreffen chemische Systeme um etwa den Faktor~2 im $\Isp$, bei noch nutzbaren
SchubkrÃ¤ften. Das NASA-Programm DRACO testet derartige Systeme als Kandidaten fÃ¼r
bemannte Marsreisen.

Die Zukunft liegt in der optimalen Kombination aller Konzepte: chemischer Hochschub
fÃ¼r den Start, elektrische Hocheffizienzantriebe fÃ¼r interplanetare Cruise-Phasen,
und mÃ¶glicherweise Fusions- oder Antimaterie-Antriebe fÃ¼r interstellare Distanzen ---
jenseits des gegenwÃ¤rtigen Technologiestands, aber innerhalb der Physik.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Die \textbf{LavaldÃ¼se} wandelt thermische Energie isentrop in gerichteten
    Impuls --- die de Laval-Kontraktion erzwingt ÃœberschallstrÃ¶mung.
  \item Chemische Triebwerke: $\Isp = 250\ldots460$ s --- optimal fÃ¼r \textbf{Schub}.
  \item Elektrische Antriebe: $\Isp = 1000\ldots30000$ s --- optimal fÃ¼r \textbf{Effizienz}.
  \item Der \textbf{Hohmann-Transfer} ist das energieoptimale ManÃ¶ver zwischen
    koplanaren Kreisbahnen: $\Dv_{\mathrm{LEO\to GEO}} \approx \SI{3.95}{km/s}$.
  \item \textbf{Mehrstufige Raketen} Ã¼berwinden die Massentyrannei durch sukzessives
