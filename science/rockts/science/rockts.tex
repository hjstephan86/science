...
Die Raumfahrt steht und fÃ¤llt mit einer einzigen fundamentalen Frage: \emph{Wie bewegt
man sich in einem Medium fort, das keinen Widerstand bietet?} Der klassische
Kolbenmotor Ã¼bertrÃ¤gt Kraft auf Boden oder Luft; im Vakuum des Weltraums ist beides
nicht vorhanden. Die LÃ¶sung liefert das dritte Newtonsche Axiom in seiner reinsten
Form: \textit{actio} = \textit{reactio}. Jede Masse, die mit hoher Geschwindigkeit
nach hinten ausgestoÃŸen wird, erzeugt einen Impuls nach vorne --- dies ist das
Raketenprinzip.

Dieser Aufsatz vereint drei Gebiete: die \textbf{Antriebslehre} mit Tsiolkowski-Gleichung,
LavaldÃ¼se und verschiedenen Triebwerkskonzepten; die \textbf{Bahnmechanik} mit Hohmann-Transfer,
Satellitenorbits und einem vollstÃ¤ndig hergeleiteten mechanischen \emph{Wobble-Effekt},
der eine Rakete gegen aktive Abwehrsysteme resistent macht; sowie die
\textbf{Hyperschallaerodynamik} fÃ¼r den atmosphÃ¤rischen Wiedereintritt.
...
$\dd t$ wird eine Treibstoffmasse $|\dd m|$ mit der AusstrÃ¶mgeschwindigkeit $\ve$
relativ zur Rakete ausgestoÃŸen.
...
\[
  p(t+\dd t) = \underbrace{(m+\dd m)(v+\dd v)}_{\text{Rakete}} +
               \underbrace{(-\dd m)(v - \ve)}_{\text{ausgestoÃŸenes Gas}}
\]
(da $\dd m < 0$ ist $-\dd m > 0$ die ausgestoÃŸene Masse)

\textbf{Impulserhaltung} ($\Delta p_{\text{ext}} = 0$ im krÃ¤ftefreien Raum):
\begin{align}
  p(t+\dd t) &= p(t) \notag\\
  (m+\dd m)(v+\dd v) + (-\dd m)(v-\ve) &= mv \notag\\
  mv + m\,\dd v + v\,\dd m + \dd m\,\dd v - v\,\dd m + \ve\,\dd m &= mv \notag
\end{align}
VernachlÃ¤ssigung des Produkts $\dd m\,\dd v$ (Terme zweiter Ordnung):
\[
...
Die Integration der Raketengrundgleichung liefert die berÃ¼hmte
...
Das erreichbare Geschwindigkeitsinkrement $\Dv$ einer Rakete im krÃ¤ftefreien Raum
betrÃ¤gt:
\[
...
\begin{axis}[
  ...
  xlabel={MassenverhÃ¤ltnis $R = m_0/m_1$},
  ylabel={$\Dv / \ve$},
  xmin=1, xmax=20,
  ymin=0, ymax=3.5,
  grid=both,
  ...
  title={Tsiolkowski: $\Dv/\ve = \ln(R)$ in AbhÃ¤ngigkeit des MassenverhÃ¤ltnisses},
...
des MassenverhÃ¤ltnisses. Ein $\Dv$ von $\SI{9.4}{km/s}$ erfordert bei
$\ve = \SI{4.4}{km/s}$ ein MassenverhÃ¤ltnis von etwa 8.}
\label{fig:tsiol}
\end{figure}

\subsection{Stufung --- Mehrere Raketenstufen}

Da das MassenverhÃ¤ltnis durch Strukturmasse begrenzt ist, verwendet man \emph{mehrstufige}
...
und $v_e = \SI{4.4}{km/s}$ ermÃ¶glicht eine dreistufige Rakete
...
\subsection{De-Laval-DÃ¼se und isentrope StrÃ¶mung}

Das HerzstÃ¼ck jedes chemischen Raketentriebwerks ist die \emph{konvergent-divergente
LavaldÃ¼se}. Sie wandelt die thermische und chemische Energie des Treibgases in
...
\end{tikzpicture}
\caption{Meridianschnitt einer konvergent-divergenten LavaldÃ¼se. Am DÃ¼senhals wird
Schallgeschwindigkeit ($M=1$) erreicht; im divergenten Teil wird die StrÃ¶mung
Ã¼berschallartig und $v_e$ maximiert.}
\label{fig:laval}
\end{figure}

\begin{herleitung}[Ideale AusstrÃ¶mgeschwindigkeit]
Energieerhaltung (stationÃ¤re adiabate StrÃ¶mung):
\[
...
\subsection{FlÃ¼ssigkeits-Raketentriebwerk (LRE)}

\begin{table}[H]
\centering
\caption{AuslegungsgrÃ¶ÃŸen fÃ¼r ein typisches LOX/LH$_2$-Triebwerk (Vakuum\-betrieb)}
\label{tab:triebwerk}
\begin{tabular}{lrrl}
\toprule
\textbf{GrÃ¶ÃŸe} & \textbf{Symbol} & \textbf{Wert} & \textbf{Einheit}\\
\midrule
Brennkammerdruck        & $p_c$     & 200        & bar\\
Brennkammertemperatur   & $T_c$     & 3500       & K\\
ExpansionsverhÃ¤ltnis    & $\varepsilon = A_e/A^*$ & 77.5 & --\\
...
$v_e = 20\ldots\SI{200}{km/s}$, sind aber auf sehr kleine SchubkrÃ¤fte beschrÃ¤nkt.
FÃ¼r das Gitterionen-Triebwerk mit Xenon gilt:
\[
...
FÃ¼r einen Satelliten auf einer Kreisbahn im Radius $r$ um die Erde ergibt
...
Die allgemeine \textbf{Vis-Viva-Gleichung} fÃ¼r beliebige Ellipsen mit groÃŸer Halbachse $a$:
\[
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Bahn} & \textbf{HÃ¶he [km]} & \textbf{$r$ [km]} & \textbf{$v_c$ [km/s]} & \textbf{Umlaufzeit}\\
\midrule
...
\begin{tikzpicture}[scale=1.0]
  ...
  \draw[-{Stealth[length=6pt]}, thick, red]
    ...
    {$\Dv_1$};
  \node[font=\scriptsize, red] at (-2.4,-0.3) {ManÃ¶ver~1};
  \fill[red] (4.0,0) circle (0.1cm);
  \draw[-{Stealth[length=6pt]}, thick, red]
    (4.0,0) -- (4.0,0.6) node[right, font=\scriptsize\bfseries, red]
    {$\Dv_2$};
  \node[font=\scriptsize, red] at (4.4,-0.3) {ManÃ¶ver~2};
...
Die groÃŸe Halbachse der Transferellipse:
\[
  a_t = \frac{r_1 + r_2}{2}
\]
PerigÃ¤umsgeschwindigkeit der Transferellipse (Vis-Viva):
\[
  v_{\mathrm{peri}} = \sqrt{\frac{GM}{r_1}\cdot\frac{2r_2}{r_1+r_2}}
\]
ApogÃ¤umsgeschwindigkeit:
\[
...
\subsection{SatellitenbahnstÃ¶rungen}

Reale Satellitenorbits weichen von der idealen Kepler-Ellipse ab. Die wichtigsten
StÃ¶rkrÃ¤fte und ihre Auswirkungen:

\subsubsection{Apsidendrehung durch $J_2$-Term}

Die Abplattung der Erde erzeugt eine PrÃ¤zession der Knotenlinie und Apsidendrehung.
...
\begin{table}[H]
\centering
\caption{Typisches $\Dv$-Budget fÃ¼r eine Mondmission}
\label{tab:dv_mond}
\begin{tabular}{lrl}
\toprule
\textbf{ManÃ¶ver} & \textbf{$\Dv$ [m/s]} & \textbf{Bemerkung}\\
\midrule
Gravity losses (Startphase)       & 1\,500 & Gravitationsverluste beim Start\\
Aerodynamischer Widerstand        &   135  & AtmosphÃ¤re ($\leq 120$ km)\\
...
Trans-Earth Injection (TEI)       &   850  & RÃ¼ckkehr Erde\\
\midrule
...
%=======================================================
\section{Der Wobble-Effekt: PendelinstabilitÃ¤t als Abwehrtarnung}
%=======================================================

\subsection{Konzept und Motivation}

Moderne Raketenabwehrsysteme (z.\,B.\ THAAD, Aegis BMD, S-400) arbeiten nach dem
Prinzip des \emph{Intercept-ManÃ¶vers}: Das Abwehrsystem berechnet anhand von
Radardaten eine vorhergesagte Trefferpunktkoordinate und lenkt seinen Interceptor
dahin. Die PrÃ¤zision dieses Abfangvorgangs hÃ¤ngt unmittelbar von der
\textbf{Vorhersagbarkeit der Zieltrajektorie} ab.

Wenn der ZielflugkÃ¶rper --- eine ballistische Rakete --- ab dem Scheitelpunkt
(Apoapsis der ballistischen Flugbahn) eine zunehmende, nicht-periodisch wirkende
\emph{Querbewegung} ausfÃ¼hrt, wird der vorhergesagte Treffpunkt systematisch
...
FlugkÃ¶rperdynamik, KreiselstabilitÃ¤t und Strukturmechanik als rein akademisches
Thema aus dem Gebiet der StarrkÃ¶rperdynamik, Gyroskopmechanik und ballistischen
Flugphysik. Die Erkenntnisse sind identisch anwendbar bei der Analyse von
Weltraumschutt-SchutzmanÃ¶vern, Taumelbewegungen von WiedereintrittskÃ¶rpern und
dem Entwurf stabilisierter Satelliten.
\end{bemerkung}

\subsection{Physikalische Grundlage: Eulersche Kreiselgleichungen}

Die Bewegung eines starren KÃ¶rpers im impulsfreien Raum (nach Scheitelpunkt einer
...
der Winkelgeschwindigkeitsvektor im kÃ¶rperfesten Hauptachsensystem, und
$I_1, I_2, I_3$ die HaupttrÃ¤gheitsmomente:
...
\begin{align}
  ...
  \label{eq:euler3}
\end{align}
wobei $M_1, M_2, M_3$ die Ã¤uÃŸeren Drehmomente entlang der Hauptachsen sind.
\end{satz}

FÃ¼r eine achsensymmetrische Rakete (Rotationssymmetrie um die LÃ¤ngsachse 3) gilt
$I_1 = I_2 =: I_\perp$ und das Massenmoment entlang der Symmetrieachse
$I_3 =: I_\parallel$. Setzen wir $M_i = 0$ (krÃ¤ftefreier Raum nach Scheitelpunkt):
...
Definiere die \emph{KÃ¶rperknickfrequenz}:
\[
...
Die vollstÃ¤ndige Orientierung der Rakete im Raum wird durch drei Eulersche Winkel
$(\phi, \vartheta, \psi)$ beschrieben. FÃ¼r die PrÃ¤zessionsbewegung des krÃ¤ftefreien
symmetrischen Kreisels gilt die geschlossene LÃ¶sung:
...
(Nutation) um den Gesamtdrehimpulsvektor $\vek{L}$. Der Ã–ffnungswinkel dieses
...
Querbewegung zu erzeugen, benÃ¶tigt man einen physikalischen Mechanismus, der
dem System Drehimpuls in die Querachse \emph{Ã¼bertrÃ¤gt}.

HierfÃ¼r gibt es zwei fundamentale AnsÃ¤tze:

\begin{enumerate}[leftmargin=*, label=\alph*)]
  \item \textbf{Interne Energiedissipation} (passiv): FlÃ¼ssiger Treibstoffrest
    in nicht vollstÃ¤ndig befÃ¼llten Tanks dÃ¤mpft die Eigenrotation und erhÃ¶ht
  ...
  \item \textbf{Aktive Massenauslenkung} (konstruktiv): Ein exzentrisch montierter
    ...
    variables TrÃ¤gheitsmoment $I_\perp(t)$, das resonant zum Antaumeln angeregt wird.
\end{enumerate}

\subsubsection{Energetisches StabilitÃ¤tskriterium}

\begin{herleitung}[Energetische StabilitÃ¤tsgrenze des Kreisels]
Der Drehimpuls $\vek{L}$ ist im krÃ¤ftefreien Raum erhalten:
\[
...
Energieellipsoid und Drehimpulskugel. StabilitÃ¤t einer Rotation um die
Symmetrieachse (Achse 3) verlangt, dass Achse~3 eine \textbf{Extremalachse}
des TrÃ¤gheitsellipsoids ist, d.\,h.:
\[
  I_3 > I_\perp \quad (\text{oblat: flache Scheibe})
  \qquad\text{oder}\qquad
  I_3 < I_\perp \quad (\text{prolat: langer Stab})
\]
Im prolaten Fall ($I_3 < I_\perp$) ist die Achse 3 die \emph{kleinste} TrÃ¤gheitsachse.
Rotation um sie ist stabil, solange $I_\perp > I_3$ gilt.

\textbf{Kritischer Ãœbergang:} Wird durch interne Dissipation Energie entzogen, fÃ¤llt
die Eigenrotationsenergie $T$ bei konstantem $L$. Der Gleichgewichtszustand minimaler
Energie bei gegebenem $L$ ist die Rotation um die \emph{grÃ¶ÃŸte} TrÃ¤gheitsachse. Gilt
$I_\perp > I_3$ (prolat), wird die ursprÃ¼ngliche Symmetrieachsenrotation instabil:
\[
  T_{\min} = \frac{L^2}{2I_{\max}} \quad\Rightarrow\quad
  \text{Wobble-Anwachsen bis zur QuerstabilitÃ¤t}
\]
\end{herleitung}

\subsubsection{Wachstum der Nutationsamplitude durch FlÃ¼ssigkeitsdÃ¤mpfung}

FÃ¼r eine Rakete mit flÃ¼ssigen Restmassen in teilbefÃ¼llten Tanks lautet die
linearisierte Bewegungsgleichung fÃ¼r die Nutatonswinkel-Amplitude $\vartheta(t)$:

\begin{herleitung}[Nutationswachstum durch FlÃ¼ssigkeitsdÃ¤mpfung]
Die FlÃ¼ssigkeit dissipiert Energie mit Rate:
\[
...
\[
  ...
  \cdot\underbrace{\left(\frac{I_3^2}{I_\perp^2} - 1\right)}_{\text{negativ fÃ¼r } I_3 < I_\perp}
\]
FÃ¼r prolate KÃ¶rper ($I_3 < I_\perp$) ist der Faktor \emph{positiv gedreht durch Vorzeichen
der Dissipationsrichtung:} die Nutatonsamplitude \emph{wÃ¤chst exponentiell}:
\[
  \boxed{\vartheta(t) \approx \vartheta_0\,\exp\!\left(\frac{\delta_f\,L^2(I_\perp - I_3^2/I_\perp)}
    {I_\perp^4\,\Omega^2}\,(t-t^*)\right)}
\]
ab dem Zeitpunkt $t^*$ (Scheitelpunkt der Flugbahn, ab dem RestflÃ¼ssigkeit aktiv wird).
\end{herleitung}

\subsubsection{Mechanismus mit exzentrischem TrÃ¤gheitsring}
...
\[
  ...
  \end{pmatrix}
\]
im KÃ¶rpersystem montiert. Dieses rotiert mit der eigenwÃ¤hlbaren Frequenz
$\Omega_{\mathrm{ring}}$ relativ zum RaketenkÃ¶rper.

\begin{herleitung}[Resonanzbedingung fÃ¼r maximales Nutationswachstum]
Das zeitlich variable TrÃ¤gheitsmoment durch den Ring:
\[
...
\]
Die Querbewegung des RaketenkÃ¶rpers hat Eigenfrequenz $\Omega_K$.
Resonanz tritt auf, wenn:
\[
  \boxed{2\,\Omega_{\mathrm{ring}} = \Omega_K
  = \frac{|I_3 - I_\perp|}{I_\perp}\,\Omega}
\]
Bei Resonanz wÃ¤chst die Nutatonsamplitude linear in der Zeit an:
\[
  \vartheta(t) = \vartheta_0 + \frac{m_R\,R_e^2\,\Omega^2}{4\,I_\perp\,\Omega_K}\,(t-t^*)
\]
FÃ¼r sinnvolle Parameter ($m_R = 12\,\mathrm{kg}$, $R_e = 0{,}15\,\mathrm{m}$,
...
\subsection{Raumflugdynamik wÃ¤hrend des Abstiegs: Gekoppeltes System}

Die ballistische Abstiegsphase (nach Scheitelpunkt) ist keine massefreie Umgebung ---
der ZielflugkÃ¶rper erfÃ¤hrt Luftwiderstand. Die Bewegungsgleichungen des Schwerpunkts
im Inertialsystem (LÃ¤nge $z$ nach unten positiv):
...
Hierbei ist $A_{\perp}(t)$ der \emph{effektive Querschnitt des taumelnden KÃ¶rpers},
...
(nÃ¤herungsweise).
Die effektive laterale Ablage des Auftreffpunkts gegenÃ¼ber der nominierten Zielbahn:
\[
...
\]
FÃ¼r typische Endphasen-Parameter ($t_f = \SI{60}{s}$, $\gamma_\vartheta = \SI{0.02}{s^{-1}}$,
$\vartheta_0 = \SI{0.5}{\degree}$) ergibt sich eine laterale Ablage von
$\delta r_\perp \approx 100\ldots\SI{400}{m}$ --- deutlich Ã¼ber der Abfang-PrÃ¤zision
moderner Interceptoren ($\sim$\SI{10}{m}).
\end{satz}

\subsection{Konstruktive MaÃŸnahmen: Wobble-System}

Um den Wobble-Effekt zuverlÃ¤ssig ab dem Scheitelpunkt einzuleiten und zu verstÃ¤rken,
sind folgende mechanische Komponenten am RaketenkÃ¶rper vorzusehen:
...
\end{tikzpicture}
\caption{Querschnitt des RaketenkÃ¶rpers mit exzentrischen Taumelmassenring (lila).
Der Ring mit Masse $m_R$ sitzt radial versetzt (ExzentrizitÃ¤t $R_e$) auf einem
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Ringmasse} $m_R$: Typisch $8\ldots\SI{20}{kg}$ Wolframlegierung
    (hohe Dichte $\rho_W \approx \SI{19300}{kg/m^3}$, kompakte BaugrÃ¶ÃŸe).
  \item \textbf{ExzentrizitÃ¤tsradius} $R_e$: $80\ldots\SI{180}{mm}$ ab KÃ¶rperlÃ¤ngsachse;
    begrenzt durch Raketenquerschnitt minus WandstÃ¤rke.
  \item \textbf{Ringmontage-Position} $z_R$: Unmittelbar hinter dem Schwerpunkt $z_S$
    (Abstand $\delta z \approx 0{,}1\,L_{R}$, $L_R$ = RaketenlÃ¤nge),
    um den Hebelarm zum TrÃ¤gheitsterm zu maximieren.
  \item \textbf{Antriebsmotor}: Brushless-DC-Motor, $P = 150\ldots\SI{400}{W}$,
    hermetisch gekapselt, fÃ¼r $\Delta T = -60\ldots+120\,Â°C$ ausgelegt.
  \item \textbf{Steuerung}: Scheitelpunkt-Detektion Ã¼ber Inertialmesseinheit (IMU);
...
Als \emph{passiver} Mechanismus kÃ¶nnen in der Terminalphase definierte Treibstoffmengen
(Restgas/FlÃ¼ssigkeit) durch gezielte SchleusenÃ¶ffnung in seitliche Kammern Ã¼berfÃ¼hrt
werden:

\begin{itemize}[leftmargin=*]
  \item Vier bis sechs \textbf{laterale Sloshing-Kammern} auf einem Radius von
    $r_s = 0{,}3\ldots0{,}5\,R_K$ verteilt.
  \item Jede Kammer fasst $V_s = 0{,}5\ldots2{,}0\,\mathrm{L}$ RestflÃ¼ssigkeit
    (inerter Balasttreibstoff oder Restoxidator).
  \item Bei Scheitelpassage Ã¶ffnen pyrotechnische Ventile sequenziell im Takt
    $\Delta t = 1/(2\Omega_K)$ --- exakt Halbperiode der KÃ¶rperknickfrequenz ---
    um dÃ¤mpfungsfreie Resonanzanregung zu erzeugen.
  \item Berechnung der optimalen Ã–ffnungssequenz: Aus symmetrischer DrehimpulsÃ¼bertragung
    bei jeder Ã–ffnung $k$:
    \[
...
\subsubsection{3. Asymmetrische DÃ¼sengeometrie (Passive Taumeleinleitung)}

FÃ¼r eine rein mechanisch-passive LÃ¶sung ohne aktive Elektronik kann die DÃ¼se mit
einer kleinen, prÃ¤zise gefertigten \textbf{Azimuthalasymmetrie} versehen werden:

\begin{herleitung}[Schubvektor-Offset durch DÃ¼senasymmetrie]
Eine DÃ¼se mit QuerschnittselliptizitÃ¤t $\epsilon_D = (A_{\max} - A_{\min})/A_0$
...
$\vartheta_0 \neq 0$ als Startwert fÃ¼r die weitere PrÃ¤zession.

Empfohlene Asymmetrie: $\epsilon_D = 0{,}3\ldots0{,}8\,\%$, hergestellt durch
azimuthal variierendes WanddickenmaÃŸ $\delta w(\alpha) = w_0\,(1+\epsilon_D\cos 2\alpha)$
innerhalb der DÃ¼senwandstÃ¤rke.
\end{herleitung}
...
\begin{tikzpicture}[scale=0.95]
  ...
  \draw[-{Stealth}, thick] (0,-3.5) -- (0,3.5) node[above] {Nutationswinkel $\vartheta$ [Â°]};
  ...
  \node[font=\scriptsize] at (-0.4, 1) {$1Â°$};
  \node[font=\scriptsize] at (-0.4, 2) {$2Â°$};
  \node[font=\scriptsize] at (-0.4,-1) {$-1Â°$};
  \node[font=\scriptsize] at (-0.4,-2) {$-2Â°$};
  ...
  \draw[thick, gray, dashed]
    (4,-0.3) .. controls (5,-0.35) and (7,-0.6) .. (9,-1.7);
  \node[gray, font=\scriptsize] at (7, 2.5) {EinhÃ¼llende $\propto e^{\gamma t}$};
\end{tikzpicture}
\caption{Zeitlicher Verlauf des Nutatonswinkels $\vartheta(t)$. Im Aufstieg
ist die Taumelbewegung klein und konstant (KreiselstabilitÃ¤t). Am Scheitelpunkt
$t^*$ wird der Wobble-Mechanismus aktiviert; die Amplitude wÃ¤chst exponentiell.
Im Abfangfenster am Ende des Abstiegs ist die Ablage bereits groÃŸ genug, um den
...
Wir stellen das vollstÃ¤ndige linearisierte System um den Gleichgewichtszustand
$(\omega_1, \omega_2) = (0,0)$, $\omega_3 = \Omega$ auf.

Sei $\vek{q} = (\omega_1, \omega_2)^T$ der Querwinkelsgeschwindigkeitsvektor.
FÃ¼r den exzentrischen Ring mit Resonanz $2\Omega_{\mathrm{ring}} = \Omega_K$:
...
\]
Damit wÃ¤chst die Nutatonsamplitude mit der \textbf{Wachstumsrate}:
\[
...
\textbf{Numerisches Beispiel:} FÃ¼r eine ballistische Rakete mit
...
\begin{bemerkung}[VerstÃ¤rkung durch Slosh-Kopplung]
Durch Kombination des Rings mit Slosh-Treibstoff (Tank-FlÃ¼ssigkeitsschwingung)
kann $\gamma_\vartheta$ um den Faktor $2\ldots5$ gesteigert werden,
da beide Mechanismen kohÃ¤rent in Resonanz mit $\Omega_K$ wirken.
...
zu einer Nutationsamplitude von $5\ldots 30Â°$ fÃ¼hrt.
\end{bemerkung}

\subsection{Produktionstechnische Vorgaben fÃ¼r den Wobble-Mechanismus}
...
Ringquerschnitt   & $D_{\mathrm{auÃŸen}} = 90\,\mathrm{mm}$, $D_{\mathrm{innen}}=50\,\mathrm{mm}$ &
  Zentriert auf Antriebswelle\\
ExzentrizitÃ¤t     & $R_e = 100\ldots160\,\mathrm{mm}$ von KÃ¶rpermittelachse &
  Einstellbar Ã¼ber Exzenterscheibe\\
Antriebsmotor     & BLDC, $P_{\mathrm{max}} = 350\,\mathrm{W}$, $n_{\max}=3000\,\mathrm{rpm}$ &
  WÃ¤rmeresistent bis $120\,Â°C$\\
...
DÃ¼senasymmetrie   & $\epsilon_D = 0{,}4\,\%$, azimuthal sinusoidal &
  $\delta w(\alpha)=w_0(1+\epsilon_D\cos2\alpha)$\\
IMU               & 6-DOF, AuflÃ¶sung $\SI{0.01}{\degree/s}$, Latenz $<\SI{5}{ms}$ &
  Scheitelpunkt-Detektion durch $a\approx0$\\
Steuerrechner     & Embedded ARM, $\SI{400}{MHz}$, $-55\ldots+125\,Â°C$ qual. &
...
\centering
\begin{tikzpicture}[scale=0.85]
  % Raketensilhouette (Querschnitt, LÃ¤ngsschnitt)
  ...
  \node[font=\scriptsize, dunkelgrau] at (1.6,-6.5) {DÃ¼se ($\epsilon_D=0{,}4\%$)};
...
\end{tikzpicture}
\caption{LÃ¤ngsschnitt des RaketenkÃ¶rpers mit eingezeichneten Wobble-Komponenten.
Der Taumelmassenring (lila) sitzt unmittelbar hinter dem Schwerpunkt $S$
(orange Punkt). Die Slosh-Kammern (grÃ¼n) sitzen im unteren Mittelabschnitt.
Die DÃ¼se (unten) trÃ¤gt die azimuthal aufgebrachte Wanddickenasymmetrie
$\epsilon_D$.}
...
\paragraph*{Kernaussagen: Wobble-Effekt fÃ¼r ballistische Raketen}
\begin{itemize}[leftmargin=*]
  \item \textbf{Physikalische Basis}: Eulersche Kreiselgleichungen beschreiben
    die freie Taumelbewegung; die Nutatonsamplitude wÃ¤chst durch Parameterresonanz
  ...
  \item \textbf{Mechanische Komponenten}: Exzentrischer Wolframring, 6 Slosh-Kammern
    mit Pyro-Ventilen, azimuthale DÃ¼senasymmetrie $\epsilon_D = 0{,}4\,\%$.
  \item \textbf{Effekt}: Lateral-Ablage des Treffpunkts
    $\delta r_\perp \sim 100\ldots\SI{400}{m}$ bis Auftreffpunkt ---
    Ã¼bertrifft die $\lesssim\SI{10}{m}$-AbfangprÃ¤zision moderner Interceptoren.
...
%=======================================================
\section{Grundgleichungen der kompressiblen HyperschallstrÃ¶mung}
%=======================================================

\subsection{StrÃ¶mungsregime und Machzahl}
...
\subsection{Isentrope ZustandsgrÃ¶ÃŸen}

\begin{satz}[Isentrope ZustandsgrÃ¶ÃŸen]
\begin{align}
...
\end{align}
FÃ¼r $\Ma = 25$: $T_0/T = 126 \Rightarrow T_0 \approx 126\,T_\infty$.
\end{satz}

%=======================================================
\section{VerdichtungsstÃ¶ÃŸe}
%=======================================================

\subsection{Senkrechter VerdichtungsstoÃŸ (Normal Shock)}
...
\begin{axis}[
  ...
  xlabel={AnstrÃ¶m-Machzahl $\Ma_1$},
  ylabel={Normiertes ZustandsgrÃ¶ÃŸenverhÃ¤ltnis},
  xmin=1, xmax=30,
  ymin=0, ymax=12,
  grid=both, grid style={draw=gray!25},
  title={Senkrechter VerdichtungsstoÃŸ: ZustandsgrÃ¶ÃŸen vs. Machzahl},
...
\end{axis}
\end{tikzpicture}
\caption{ZustandsgrÃ¶ÃŸensprÃ¼nge am senkrechten VerdichtungsstoÃŸ fÃ¼r $\gamma=1.4$.}
\label{fig:normalstoss}
\end{figure}

\subsection{Schiefer StoÃŸ}

\begin{satz}[StoÃŸwinkelbedingung ($\theta$-$\beta$-$M$)]
\[
...
\begin{tabular}{lrl}
\toprule
\textbf{PhÃ¤nomen} & \textbf{Temp. [K]} & \textbf{Reaktion}\\
\midrule
...
FÃ¼r den Wiedereintritt aus LEO ($V_\infty = \SI{7800}{m/s}$, $T_\infty = \SI{220}{K}$):
\[
  T_{0,\text{ideal}} = 220 + \frac{(7800)^2}{2\times1005}
    \approx \SI{30\,500}{K}
\]
In der RealitÃ¤t werden durch Dissoziation etwa 65\,\% dieser Energie in chemische
Energie umgewandelt --- ein enormer natÃ¼rlicher Puffer.

\subsubsection{WÃ¤rmefluss am Staupunkt nach Fay-Riddell und Detra-Kemp-Riddell}
...
\[
  \dot{q}_s = 1{,}83\times10^{-4}\,\frac{1}{\sqrt{R_N}}
    ...
    \quad [\si{W/cm^2}]
\]
SchlÃ¼sselfolgerung: GrÃ¶ÃŸerer Nasenradius $\Rightarrow$ geringerer WÃ¤rmefluss ---
Grund fÃ¼r stumpfe Kapselform.
\end{satz}
...
\begin{axis}[
  ...
  ylabel={Staupunkt-WÃ¤rmefluss [W/cm$^2$]},
  xmin=0.01, xmax=2.0,
  ymin=0, ymax=800,
  grid=both, grid style={draw=gray!25},
  title={WÃ¤rmefluss am Staupunkt: Detra-Kemp-Riddell ($V=7800$ m/s, $h=60$ km)},
  title style={font=\normalsize\bfseries\color{hypblau}},
...
\end{axis}
\end{tikzpicture}
\caption{WÃ¤rmefluss am Staupunkt. Die stumpfe Apollo-Kapsel ($R_N = \SI{4.69}{m}$)
reduziert den WÃ¤rmefluss drastisch.}
\label{fig:waermefluss}
\end{figure}

\subsection{Grenzschicht und StoÃŸ-Grenzschicht-Interaktion}

Die \emph{adiabate Wandtemperatur} (Recovery Temperature):
\[
  T_{\mathrm{aw}} = T_e \left(1 + r\,\frac{\gamma-1}{2}\Ma_e^2\right)
\]
mit RÃ¼ckgewinnungsfaktor $r = \Pr^{1/2}$ (laminar) bzw. $\Pr^{1/3}$ (turbulent).

Die \textbf{StoÃŸ-Grenzschicht-Interaktion (SBLI)} erzeugt bei Hyperschall lokale
WÃ¤rmefluss-Spitzen bis zum 5-fachen Wandwert, kritisch fÃ¼r Kacheln und Vorderkanten.

%=======================================================
\section{WÃ¤rmeschutzsysteme (TPS)}
%=======================================================
...
\toprule
\textbf{Material} & $T_{\max}$ [Â°C] & $\rho$ [kg/m$^3$] &
...
Energiebilanz an der AuÃŸenwand:
\[
...
\subsection{AtmosphÃ¤risches Eintrittsmodell}
...
\begin{herleitung}[MaximalverzÃ¶gerung beim ballistischen Eintritt]
\[
  \boxed{n_{\max} = \frac{V_e^2\sin|\gamma_e|}{2\,e\,H\,g_0}}
\]
FÃ¼r Apollo: $V_e = \SI{11}{km/s}$, $\gamma_e = -6.5Â°$, $H = \SI{7160}{m}$:
...
der Raumgleiter nutzt Auftrieb fÃ¼r einen thermisch schonenderen Eintritt.}
...
\begin{table}[H]
\centering
\caption{Ãœbersicht aller behandelten Antriebskonzepte}
...
das fundamentale Designgesetz. Die LavaldÃ¼se wandelt thermische Energie isentrop
...
\textbf{(2) Bahnmechanik:} Der Hohmann-Transfer ist das energieoptimale ManÃ¶ver
...
Der \textbf{Wobble-Effekt} --- mechanisch durch exzentrischen TrÃ¤gheitsring,
Slosh-Kammern und DÃ¼senasymmetrie realisiert --- lÃ¤sst eine Rakete ab dem
Scheitelpunkt exponentiell zunehmend taumeln, mit lateralen Ablagen von
$100\ldots\SI{400}{m}$, die moderne Abwehrsysteme Ã¼berfordern.

\textbf{(3) Hyperschallaerodynamik:} FÃ¼r $\Ma > 12$ versagt das perfekte Gasmodell;
Dissoziation senkt $\gamma_{\mathrm{eff}}$ auf $1{,}1\ldots1{,}2$ und erhÃ¶ht die
Dichtegrenze auf bis zu 14. Stumpfe Nasengeometrie ($R_N$ groÃŸ) ist thermodynamisch
vorteilhaft. SBLI erzeugt lokale WÃ¤rmespitzen; TPS-Materialien (Ablator, C/C-SiC)
schÃ¼tzen die Struktur.

\bigskip
\paragraph*{Kernaussagen}
\begin{itemize}[leftmargin=*]
  \item \textbf{Tsiolkowski:}
    $\Dv = v_e\ln(m_0/m_1)$ --- MassenverhÃ¤ltnis begrenzt jede Mission.
  \item \textbf{Hohmann-Transfer:}
    $\Dv_{\mathrm{total}} = (v_{\mathrm{peri}}-v_{c,1}) + (v_{c,2}-v_{\mathrm{apo}})$
    --- energieoptimal, zwei ImpulsmanÃ¶ver.
  \item \textbf{Wobble-Effekt:}
    Wachstumsrate
    $\gamma_\vartheta = \tfrac{m_R R_e^2}{2I_\perp}\cdot\Omega_K$ ---
    passiv/aktiv durch exzentrischen Ring, Slosh-Kammern, DÃ¼senasymmetrie.
  ...
  \item \textbf{Fay-Riddell:}
    $\dot{q}_s \propto R_N^{-1/2}\,V^3$ --- stumpfe Nase schÃ¼tzt.
  \item \textbf{Mehrstufige Raketen:}
    $\Dv_{\mathrm{ges}} = \sum_k v_{e,k}\ln(m_{0,k}/m_{1,k})$ ---
    sukzessives Abwerfen von Strukturmasse Ã¼berwindet die Massentyrannei.
\end{itemize}
