...
% ======================================================
\title{StrÃ¶mungsmechanik und Hyperschallaerodynamik\\[0.4em]
...
Hyperschall bezeichnet StrÃ¶mungen mit Machzahlen $\Ma \geq 5$. In diesem Regime
treten physikalische PhÃ¤nomene auf, die in der klassischen Unterschall- und
Ãœberschallaerodynamik keine Rolle spielen: extreme Aerothermodynamik,
Dissoziation und Ionisation der Luft, dominante ViskositÃ¤tseffekte und starke
Interaktionen zwischen StoÃŸwellen und Grenzschichten.

FÃ¼r den zivilen Raumfahrtbereich ist das Hyperschallregime unausweichlich: Jedes
Fahrzeug, das aus dem Orbit oder von einer suborbitalen Bahn in die ErdatmosphÃ¤re
eintritt, durchquert dieses Regime. Die Mission des Space Shuttle, der Apollo-Kapsel,
der Sojus-Kapsel, des SpaceX Dragon sowie zukÃ¼nftiger Konzepte wie dem Raumgleiter
Hermes oder der IXV-Demonstratorkonfiguration der ESA basieren sÃ¤mtlich auf der
Beherrschung der Hyperschallphysik.

\begin{definition}[Machzahl und StrÃ¶mungsregime]
Die \textbf{Machzahl} ist definiert als VerhÃ¤ltnis der lokalen StrÃ¶mungsgeschwindigkeit
$v$ zur lokalen Schallgeschwindigkeit $a$:
\[
  \Ma = \frac{v}{a}, \qquad a = \sqrt{\gamma R T}
\]
Einteilung der StrÃ¶mungsregime:
\[
...
%=======================================================
\section{Grundgleichungen der kompressiblen StrÃ¶mungsmechanik}
%=======================================================

\subsection{Navier-Stokes-Gleichungen in KompressibilitÃ¤t}

Die Bewegungsgleichungen fÃ¼r ein kompressibles, viskoses Fluid lauten in
Indexschreibweise (Einsteinsche Summenkonvention):

\begin{align}
  \frac{\partial \rho}{\partial t} + \frac{\partial (\rho u_i)}{\partial x_i} &= 0
  \tag{KontinuitÃ¤t}\label{eq:kont}\\[4pt]
...
Der viskose Spannungstensor fÃ¼r ein Newton'sches Fluid:
\[
...
innere Energie ist. Der WÃ¤rmestromvektor folgt dem Fourier'schen Gesetz:
$q_i = -\lambda\,\partial T/\partial x_i$.

\subsection{Isentrope StrÃ¶mungsbeziehungen}

FÃ¼r eine reibungsfreie, adiabate (isentrope) StrÃ¶mung gelten die
...
\end{align}
FÃ¼r $\Ma = 25$ und $\gamma = 1.4$ (kalt, ohne Dissoziation):
$T_0/T = 1 + 0.2 \times 625 = 126 \quad\Rightarrow\quad T_0 = 126\,T_\infty$.
\end{satz}

\subsection{Senkrechter VerdichtungsstoÃŸ (Normal Shock)}

An senkrechten VerdichtungsstÃ¶ÃŸen gelten die Rankine-Hugoniot-Bedingungen.
Sei $\Ma_1$ die Machzahl vor dem StoÃŸ:

\begin{herleitung}[Rankine-Hugoniot-Sprungbedingungen]
Aus Erhaltung von Masse, Impuls und Energie Ã¼ber die StoÃŸfront ($\perp$ zur StoÃŸflÃ¤che):
\begin{align*}
...
\]
FÃ¼r $\Ma_1 \to \infty$ (Hyperschall-Grenzfall):
\[
...
\]
Die \emph{Dichtegrenze} bei 6 ist charakteristisch fÃ¼r Hyperschall --- die
StrÃ¶mung kann sich trotz beliebig hoher Machzahl nur bis zum Faktor 6 verdichten
(fÃ¼r $\gamma = 1.4$, ohne Hochtemperatureffekte).
\end{herleitung}
...
% DruckverhÃ¤ltnis p2/p1 (geteilt durch 20 zur Darstellung)
...
Die Dichte nÃ¤hert sich asymptotisch dem Grenzwert~6, wÃ¤hrend Druck und Temperatur
quadratisch mit $\Ma$ anwachsen.}
\label{fig:normalstoss}
\end{figure}

\subsection{Schiefer StoÃŸ --- StoÃŸpolaren (Oblique Shocks)}

Im praktischen Fall trifft eine StrÃ¶mung auf eine keilfÃ¶rmige Kontur (Rampe,
Nasenspitze). Es entsteht ein schiefer StoÃŸ mit StoÃŸwinkel $\beta$ bei
Ablenkwinkel $\theta$:
...
\]
Die Machzahl hinter dem schiefen StoÃŸ:
\[
...
die StoÃŸkompression in thermische Energie umgewandelt. Die dabei erreichten
Temperaturen Ã¼berschreiten die Dissoziationsenergien molekularer Spezies:
...
Vibrationserregung N$_2$        & 1\,000 & VollstÃ¤ndige Vibration aktiv\\
...
erheblich: WÃ¤hrend fÃ¼r kalte Luft $\gamma = 1.4$ gilt, fÃ¤llt dieser Wert bei
...
FÃ¼r reales Gas mit effektivem $\gamma_{\text{eff}}$:
\[
...
\end{tabular}
\end{center}
Die \emph{StoÃŸlage am Bug} eines Wiedereintrittsfahrzeugs ist somit fÃ¼r reales Gas
\textbf{deutlich nÃ¤her am KÃ¶rper} als klassisch berechnet.
\end{herleitung}

\subsection{Staupunkttemperatur und Aeroheating}

Die kinetische Enthalpie einer StrÃ¶mung mit Geschwindigkeit $V_\infty$ wird am
Staupunkt vollstÃ¤ndig in WÃ¤rme umgewandelt. Im Idealfall (kein WandwÃ¤rmestrom):
...
Diese Zahl zeigt, warum Wiedereintrittsfahrzeuge ohne effektiven WÃ¤rmeschutz
augenblicklich verdampfen wÃ¼rden. In der RealitÃ¤t werden durch Dissoziation etwa
65\,\% dieser Energie in chemische Energie (statt WÃ¤rme) umgewandelt --- ein
enormer ``natÃ¼rlicher'' Puffer.

\subsubsection{WÃ¤rmefluss am Staupunkt nach Fay-Riddell}

Das Fay-Riddell-Modell liefert den konvektiven WÃ¤rmefluss am Staupunkt:
...
In vereinfachter Form (Detra-Kemp-Riddell fÃ¼r kugelfÃ¶rmigen Bug):
\[
...
Dabei ist $R_N$ der Nasenradius in Metern. \textbf{SchlÃ¼sselfolgerung}: Ein
grÃ¶ÃŸerer Nasenradius reduziert den WÃ¤rmefluss erheblich --- dies ist der Grund,
...
\begin{axis}[
  ...
  ylabel={Staupunkt-WÃ¤rmefluss $\dot{q}_s$ [W/cm$^2$]},
  xmin=0.01, xmax=2.0,
  ymin=0, ymax=800,
  grid=both, grid style={draw=gray!25},
...
\end{axis}
\end{tikzpicture}
\caption{WÃ¤rmefluss am Staupunkt nach dem Detra-Kemp-Riddell-Modell bei LEO-Wiedereintritt.
Die stumpfe Bauform der Apollo-Kapsel ($R_N = \SI{4.69}{m}$) reduziert den WÃ¤rmefluss
gegenÃ¼ber der spitzen Shuttle-Nase drastisch.}
\label{fig:waermefluss}
\end{figure}

%=======================================================
\section{Grenzschicht und WandwÃ¤rmeÃ¼bergang}
%=======================================================

\subsection{Hyperschall-Grenzschichttheorie}

Die Grenzschicht bei HyperschallstrÃ¶mungen weist charakteristische Besonderheiten auf.
Die \emph{viskose Interaktion} beschreibt die starke Kopplung zwischen der
Grenzschicht und der AuÃŸenstrÃ¶mung, die durch die Verdichtungswirkung der Grenzschicht
auf die AuÃŸenstrÃ¶mung entsteht.

\subsubsection{Adiabate Wandtemperatur (Recovery Temperature)}

Selbst ohne WÃ¤rmezufuhr stellt sich an einer isolierten Wand eine sogenannte
RÃ¼ckgewinnungstemperatur ein:
\[
  T_{\mathrm{aw}} = T_e \left(1 + r\,\frac{\gamma-1}{2}\Ma_e^2\right)
\]
wobei $r$ der RÃ¼ckgewinnungsfaktor ist: $r = \Pr^{1/2}$ (laminare Grenzschicht)
bzw. $r = \Pr^{1/3}$ (turbulente Grenzschicht), und $\Pr \approx 0.71$ fÃ¼r Luft.

\subsubsection{Stanton-Zahl und Reynolds-Analogie}

Der lokale konvektive WÃ¤rmefluss zur Wand:
\[
  \dot{q}_w = St\,\rho_e u_e\,(h_{\mathrm{aw}} - h_w)
\]

Die Stanton-Zahl $St$ verknÃ¼pft WÃ¤rme- und ReibungsÃ¼bertragung Ã¼ber die
Reynolds-Colburn-Analogie:
\[
  St \cdot \Pr^{2/3} = \frac{C_f}{2}
\]

FÃ¼r die laminare Grenzschicht Ã¼ber eine ebene Platte (Blasius/Chapman):
\[
  C_f = \frac{0.664}{\sqrt{Re_x}}, \qquad
  Re_x = \frac{\rho_e u_e x}{\mu_e}
\]

\subsection{StoÃŸ-Grenzschicht-Interaktion}

Ein charakteristisches PhÃ¤nomen der Hyperschallaerodynamik ist die
\emph{StoÃŸ-Grenzschicht-Interaktion} (Shock-Boundary-Layer Interaction, SBLI).
Wenn ein schiefer StoÃŸ auf eine Grenzschicht trifft, entsteht ein komplexes
StrÃ¶mungssystem mit mÃ¶glicher lokaler AblÃ¶sung.
...
\begin{tikzpicture}[scale=1.1]
  ...
  % Einfallender StoÃŸ (von oben)
  \draw[hyprot, very thick] (1.0, 3.0) -- (3.5, 0.30);
  \node[hyprot, font=\scriptsize\bfseries] at (1.5, 2.6) {Einfallender StoÃŸ};

  % Reflexion / AblÃ¶sung
  \draw[hitzeorange, very thick] (3.5, 0.30) -- (5.5, 2.0);
  \node[hitzeorange, font=\scriptsize\bfseries] at (5.2, 2.4) {Reflektierter StoÃŸ};
  ...
  \node[hyprot, font=\scriptsize\bfseries] at (3.6,-0.18) {AblÃ¶seblase};

  % Trennlinie / Wiederanlege-StoÃŸ
  \draw[schockgelb!80!black, thick] (3.0,0) -- (3.0,1.5)
    node[above, font=\scriptsize] {AblÃ¶sestoÃŸ};
  \draw[schockgelb!80!black, thick] (4.2,0) -- (4.2,1.5)
    node[above, font=\scriptsize] {WiederanlegestoÃŸ};

  % WÃ¤rmefluss-Spitze (Pfeil)
  \draw[-{Stealth[length=5pt]}, very thick, hyprot]
    (4.2,0.5) -- (4.2,-0.05);
  \node[hyprot, font=\scriptsize\bfseries] at (5.5,0.4) {$\dot{q}_{w,\max}$};
  \node[font=\scriptsize] at (5.5,0.15) {(lokal $5\times$ erhÃ¶ht)};

  % Koordinaten
  \draw[-{Stealth}] (-1.0,-0.6) -- (9.2,-0.6) node[right, font=\scriptsize] {$x$};
  \draw[-{Stealth}] (-1.0,-0.6) -- (-1.0,3.3) node[above, font=\scriptsize] {$y$};
\end{tikzpicture}
\caption{StoÃŸ-Grenzschicht-Interaktion (SBLI) bei Hyperschall. Der einfallende StoÃŸ
lÃ¶st die Grenzschicht ab; am WiederanlegestoÃŸ entsteht ein lokales WÃ¤rmeflussmaxium,
das den 5-fachen Wert der ungestÃ¶rten WandwÃ¤rme erreichen kann.}
\label{fig:sbli}
\end{figure}

%=======================================================
\section{WÃ¤rmeschutzsysteme (Thermal Protection Systems, TPS)}
%=======================================================
...
\subsection{WÃ¤rmeÃ¼bertragungsgleichung an der Wand}

Die Energiebilanz an der AuÃŸenwand ergibt:
\[
...
Die \textbf{Strahlungsabgabe} nach auÃŸen:
\[
...
\begin{tikzpicture}[scale=0.95]
  ...
  \begin{scope}[xshift=0cm]
    ...
    \node[font=\tiny, dunkelgrau] at (0,2.0) {FRSI (weiÃŸ)};
    % WÃ¤rmefluss-Pfeil
  ...
  \begin{scope}[xshift=5cm]
    ...
    % HeiÃŸe Zonen (Bug + FlÃ¼gelvorderkanten)
    \fill[hyprot!80]
    ...
    \node[font=\tiny, hyprot] at (-2.0,0.25) {$>1600Â°$C};
  \end{scope}
\end{tikzpicture}
\caption{Drei WÃ¤rmeschutzkonzepte: (links) Einweg-Ablationshitzeschutz der Apollo-Kapsel;
(Mitte) wiederverwendbare Keramikkacheln (HRSI/FRSI) des Space Shuttle;
(rechts) integraler C/C-SiC-Komposit fÃ¼r Raumgleiter.}
...
\subsection{AblationswÃ¤rmetransfer -- Massenverlustrate}

Beim Ablationshitzeschutz wird Wandmaterial pyrolytisch zersetzt und ausgeblasen.
Die effektive WÃ¤rmeschutzmenge ergibt sich aus der Bilanz:
\[
...
Pyrolyse-GaskÃ¼hl\-wirkung) ist. FÃ¼r AVCOAT: $h_{\mathrm{eff}} \approx \SI{21}{MJ/kg}$.
...
Das vereinfachte Gleichungssystem fÃ¼r einen ballistischen Wiedereintritt
(vernachlÃ¤ssigter Auftrieb, konstante Ballistische Koeffiziente
...
FÃ¼r ballistischen Eintritt ($C_L = 0$) und exponentielles AtmosphÃ¤renmodell
$\rho(h) = \rho_0\,e^{-h/H}$ (SkalenhÃ¶he $H = \SI{7.16}{km}$) lÃ¤sst sich eine
analytische NÃ¤herung fÃ¼r die maximale VerzÃ¶gerung ableiten:

\begin{herleitung}[MaximalverzÃ¶gerung beim ballistischen Eintritt]
Chapman (1958) zeigt, dass fÃ¼r $C_L=0$, $\gamma = \text{const}$ (flacher Eintritt):
\[
  \frac{\dd V}{\dd h} = \frac{\rho(h) V C_D A}{2m\sin\gamma}
\]
Die maximale VerzÃ¶gerung tritt bei der Dichte
$\rho^* = \frac{2m\sin\gamma}{C_D A\,H} = \frac{2\beta_c\sin\gamma}{H}$
auf. Die zugehÃ¶rige VerzÃ¶gerung:
\[
...
\]
FÃ¼r Apollo-Wiedereintritt: $V_e = \SI{11}{km/s}$, $\gamma_e = -6.5Â°$, $H = \SI{7160}{m}$:
\[
  n_{\max} = \frac{(11000)^2 \times \sin 6.5Â°}{2\times 2.718\times 7160\times 9.81}
...
\end{axis}
\end{tikzpicture}
\caption{Vergleich von Wiedereintrittstrajektorien in der HÃ¶hen-Geschwindigkeitsebene.
Die ballistische Kapsel folgt einer steilen Bahn mit hoher SpitzenverzÃ¶gerung;
der Raumgleiter nutzt Auftrieb fÃ¼r einen flacheren, thermisch schonenderen Eintritt.}
\label{fig:trajektorien}
\end{figure}

\subsection{Auftriebskraft und Gleitzahl beim Raumgleiter}

FÃ¼r einen Raumgleiter mit Auftriebsbeiwert $C_L$ und Widerstandsbeiwert $C_D$
gilt im stationÃ¤ren Gleitflug:
\[
...
Die \emph{Gleitzahl} $L/D$ bestimmt maÃŸgeblich die ManÃ¶vrierfÃ¤higkeit und
Landereichweite des Raumgleiters. FÃ¼r das Space Shuttle: $L/D \approx 4.5$
...
\begin{align}
  \gamma_{\min} &: \quad \text{Ãœberhitzung (zu steiler Eintritt)} \\
  \gamma_{\max} &: \quad \text{Ãœberfliegen / kein Wiedereintritt (zu flach)}
\end{align}

Aus der WÃ¤rmeflussbeschrÃ¤nkung $\dot{q}_w \leq \dot{q}_{\max}$:
\[
...
\subsection{StumpfkÃ¶rper-Kapsel (Blunt Body)}

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=1.1]
  % StrÃ¶mungspfeile (AnstrÃ¶mung von rechts)
  ...
  % StoÃŸwelle (weit vor der Kapsel)
  ...
  % HeiÃŸes Gas (zwischen StoÃŸ und Kapsel)
...
Die groÃŸe Nasenradius-KrÃ¼mmung hÃ¤lt die Bugwelle weit vom KÃ¶rper und reduziert
den WÃ¤rmefluss; die Ablationsschicht (rot) schÃ¼tzt die Struktur.}
\label{fig:kapsel}
\end{figure}

\subsection{Raumgleiter --- Delta-FlÃ¼gel-Konfiguration}

Raumgleiter wie der Space Shuttle oder das ESA-IXV nutzen Auftrieb fÃ¼r einen
kontrollierten Wiedereintritt mit groÃŸer Querreichweite. Die aerothermodynamischen
...
\begin{tabular}{llrrl}
\toprule
\textbf{Zone} & \textbf{TPS-Mat.} & $T_{\max}$ [Â°C] & $\dot{q}_{\max}$ [W/cm$^2$] & \textbf{Auslegungskriterium}\\
\midrule
Nasenvorderkante   & RCC          & 1\,650 & 60  & Strahlungsgleichgewicht\\
FlÃ¼gelvorderkante         & RCC          & 1\,430 & 45  & StoÃŸ-Grenzschicht-SBLI\\
Unterseite (heiÃŸ)         & HRSI  & 1\,260 & 35  & WÃ¤rmeleit. + Strahlung\\
SeitenflÃ¤chen             & FRCI-12      & 760    & 15  & Konvektion gemÃ¤ÃŸigt\\
...
%=======================================================
\section{Numerische Simulation: CFD fÃ¼r Hyperschall}
%=======================================================

\subsection{DSMC vs. CFD -- Knudsen-Zahl-Abgrenzung}

Die GÃ¼ltigkeit der Kontinuumsmechanik-Annahme hÃ¤ngt von der Knudsen-Zahl ab:
\[
  Kn = \frac{\lambda}{L} = \frac{\mu}{\rho\,a\,L}\sqrt{\frac{\pi\gamma}{2}}
\]
wobei $\lambda$ die freie WeglÃ¤nge und $L$ die charakteristische LÃ¤nge ist.
...
\begin{axis}[
  width=13cm, height=7cm,
  xlabel={FlughÃ¶he $h$ [km]},
  ylabel={Knudsen-Zahl $Kn$},
  xmin=60, xmax=200,
  ymin=1e-5, ymax=100,
  ymode=log,
  grid=both, grid style={draw=gray!25},
  title={Knudsen-Zahl vs. FlughÃ¶he: GÃ¼ltigkeitsgrenzen der StrÃ¶mungsmodelle},
  title style={font=\normalsize\bfseries\color{hypblau}},
  legend pos=north west, legend style={font=\small},
]
% Knudsen-Zahl (mittlere freie WeglÃ¤nge / 1m)
\addplot[thick, hypblau, domain=60:200, samples=100]
  {exp(0.145*(x-80)) * 1e-5};
\addlegendentry{$Kn$ fÃ¼r $L=1$ m (Kapsel)}

\addplot[thick, hyprot, domain=60:200, samples=100]
  {exp(0.145*(x-80)) * 1e-4};
\addlegendentry{$Kn$ fÃ¼r $L=0.1$ m (Vorderkante)}
...
\node[schockgelb!60!black, font=\scriptsize] at (axis cs:100,0.005)
  {GleitstrÃ¶mung (slip CFD)};
\fill[hyprot!8] (axis cs:60,0.1) rectangle (axis cs:200,100);
\node[hyprot!70!black, font=\scriptsize] at (axis cs:100,1.0)
  {Rarefiziert (DSMC)};
\end{axis}
\end{tikzpicture}
\caption{Knudsen-Zahl-Karte als Funktion der FlughÃ¶he. Oberhalb von ca. \SI{90}{km}
(fÃ¼r $L=1$ m) verliert die Kontinuumsmechanik ihre GÃ¼ltigkeit; dort ist die
...
Der \emph{Morkovin'sche Skalierungshypothese} zufolge kÃ¶nnen inkompressible
Turbulenzmodelle fÃ¼r $\Ma < 5$ mit einer Dichteskalierung verwendet werden:

\[
  \tilde{u}_i = \sqrt{\frac{\bar{\rho}}{\rho_\infty}}\, u_i'
\]

FÃ¼r $\Ma > 5$ versagen Standardmodelle (k-$\omega$, k-$\varepsilon$) aufgrund
von KompressibilitÃ¤tskorrektionen und Realgas-Effekten. Empfohlene Modelle:

\begin{itemize}[leftmargin=*]
  \item \textbf{k-$\omega$-SST} mit Sarkar-Dilatationsdissipation fÃ¼r $\Ma < 10$
  \item \textbf{LES} (Large Eddy Simulation) fÃ¼r Transitionsbereiche
  \item \textbf{DSMC} (Direct Simulation Monte Carlo) fÃ¼r rarefizierte Bereiche
  \item \textbf{DSMC-CFD-Hybridmethode} fÃ¼r den Ãœbergangsbereich
\end{itemize}
...
Das IXV der ESA demonstrierte 2015 erstmals europÃ¤ische Wiedereintrittstechnologie
mit einem Lifting-Body-Raumgleiter. Die aerodynamische Konfiguration erreichte
$L/D \approx 0.7$ im Hyperschall --- bewusst konservativ fÃ¼r den ersten Flug.

\subsection{SpaceX Starship -- Belly-Flop Wiedereintritt}

Das Starship-Konzept von SpaceX (ab 2023 in TestflÃ¼gen) nutzt einen neuartigen
``Bauch-Flop''-Wiedereintritt: Das Fahrzeug tritt mit flachem KÃ¶rper (hoher $C_D$,
...
TranspirationskÃ¼hl-(Header-Tank-)Systeme beherrscht.

\subsection{Zusammenfassung: $\Dv$-Budget und WÃ¤rmeschutzaufwand}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    ylabel={Max. WÃ¤rmefluss Staupunkt [W/cm$^2$]},
    xmin=5, xmax=35,
    ymin=10, ymax=2000,
    ymode=log,
    grid=both, grid style={draw=gray!25},
    title={WÃ¤rmefluss vs. Eintrittsmachzahl -- zivile Missionen},
    title style={font=\normalsize\bfseries\color{hypblau}},
    legend pos=north west, legend style={font=\small},
  ]
  ...
  \end{axis}
\end{tikzpicture}
\caption{Maximaler konvektiver WÃ¤rmefluss am Staupunkt als Funktion der
Eintrittsmachzahl fÃ¼r reale zivile Missionen, verglichen mit der
...
Die Hyperschallaerodynamik verbindet fundamentale StrÃ¶mungsmechanik,
...
eintretenden Fahrzeugs Ã¼bersteigt die Schmelzenthalpie jedes bekannten
Konstruktionswerkstoffs um ein Vielfaches. Nur durch geeignete Geometrie
(stumpfer Bug, groÃŸer Nasenradius), fortgeschrittene Materialien (Ablator,
C/C-SiC) und optimierte Trajektorien ist ein sicherer Wiedereintritt mÃ¶glich.

\textbf{(2) Realgaseffekte:} FÃ¼r $\Ma > 12$ verliert die Annahme eines
kalorisch perfekten Gases ihre GÃ¼ltigkeit. Dissoziation und Ionisation senken
den effektiven $\gamma$, verschieben die StoÃŸlage und verÃ¤ndern die
WÃ¤rmeÃ¼bertragungscharakteristik grundlegend.

\textbf{(3) Kontrollierter Wiedereintritt:} Der Eintrittskorridor ist durch
thermische Constraints nach unten und durch die Notwendigkeit des AtmosphÃ¤renfangs
nach oben begrenzt. Raumgleiter mit hoher $L/D$-Zahl kÃ¶nnen diesen Korridor
aktiv manÃ¶vrieren; ballistische Kapseln mÃ¼ssen ihn passiv -- durch prÃ¤zise
Initialbahnausrichtung -- treffen.

\bigskip
\paragraph*{Kernaussagen der Hyperschallaerodynamik}
\begin{itemize}[leftmargin=*]
  \item \textbf{Dichtegrenze}: Am senkrechten StoÃŸ kann sich die Luft maximal
    um den Faktor $(\gamma+1)/(\gamma-1)$ verdichten; fÃ¼r $\gamma=1.4$ ergibt
    das~6; mit Dissoziation bis zu~14.
  \item \textbf{Fay-Riddell}: WÃ¤rmefluss am Staupunkt skaliert mit
    $\dot{q}_s \propto R_N^{-1/2}\,V^3$ --- stumpfer Bug ist thermisch gÃ¼nstiger.
  \item \textbf{Realgaseffekte} beginnen bei $T > \SI{2000}{K}$ (O$_2$-Dissoziation)
    und verÃ¤ndern alle aerothermodynamischen KenngrÃ¶ÃŸen wesentlich.
  \item \textbf{SBLI} (StoÃŸ-Grenzschicht-Interaktion) erzeugt lokale
    WÃ¤rmefluss-Spitzen bis zum 5-fachen Wert --- kritisch fÃ¼r Kacheln und
    Vorderkanten.
  \item \textbf{Knudsen-Zahl}: Oberhalb von ca. \SI{90}{km} (fÃ¼r $L\sim$1 m)
    ist Kontinuums-CFD ungÃ¼ltig; DSMC-Methodik erforderlich.
  \item \textbf{Eintrittskorridor}: Zwischen Ãœberhitzungsgrenze (zu steil)
    und atmosphÃ¤rischem Ãœberfliegen (zu flach) liegt nur ein enger Bereich
    sicherer Eintrittswinkel ($\pm 1\ldots2Â°$ fÃ¼r Kapseln).
\end{itemize}
