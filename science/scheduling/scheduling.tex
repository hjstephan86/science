...
einerseits die prÃ¤zise Darstellung der klassischen Theorie, die seit dem
...
hier erstmals ausfÃ¼hrlich prÃ¤sentierten EDF$^+$-Algorithmus \cite{Epp2026} --
in einen einheitlichen theoretischen Rahmen.

Die Hauptgrundlage ist das international maÃŸgebliche Lehrbuch von Giorgio C.\
Buttazzo \cite{Buttazzo2011}, das seit seiner ersten Auflage 1997 als
Standardreferenz der Echtzeit-Scheduling-Theorie gilt. Es wird ergÃ¤nzt durch
eine Vielzahl von PrimÃ¤rquellen, darunter die Originalarbeiten von Horn
\cite{Horn1974}, Dertouzos \cite{Dertouzos1974}, Jackson \cite{Jackson1955},
Lehoczky, Sha und Ding \cite{Lehoczky1989} sowie die einflussreichen BeitrÃ¤ge
...
statischen PrioritÃ¤tszuweisung (RM) und der dynamischen PrioritÃ¤tszuweisung
(EDF):
...
Die kÃ¼rzeste AusfÃ¼hrungszeit aller Tasks findet EDF sofort.
EDF ist scharf optimal.}
\end{quote}

Diese Aussage ist nicht trivial. Sie unterscheidet zwei fundamentale Konzepte
der OptimalitÃ¤t: \emph{schwache} OptimalitÃ¤t, die RM innerhalb der Klasse
fester PrioritÃ¤tsalgorithmen auszeichnet, und \emph{scharfe} OptimalitÃ¤t,
die EDF Ã¼ber alle prÃ¤emptiven Einprozessor-Scheduling-Algorithmen hinweg
kennzeichnet. Die SchÃ¤rfe von EDF -- im Sinne von: ``genau die schedulearbaren
Task-Sets werden gefunden, alle anderen nicht'' -- ist das stÃ¤rkste
OptimalitÃ¤tsergebnis, das fÃ¼r Einprozessor-Echtzeit-Scheduling bekannt ist.

Der Beweis der scharfen OptimalitÃ¤t von EDF, der auf Arbeiten von Horn
\cite{Horn1974} und Dertouzos \cite{Dertouzos1974} zurÃ¼ckgeht und spÃ¤ter
durch Baruah et al.\ \cite{Baruah1990} fÃ¼r periodische Tasks prÃ¤zisiert wurde,
ist mathematisch elegant und bildet das HerzstÃ¼ck von Kapitel~3 dieses Buches.

\section*{Historischer Kontext}

Die Echtzeit-Scheduling-Theorie entstand in den frÃ¼hen 1970er Jahren als
Antwort auf die wachsende Verbreitung von Prozesssteuerungssystemen, die
zuverlÃ¤ssige zeitliche Garantien erforderten. Die Pionierarbeiten von Liu und
...
\begin{itemize}
	...
	\item Abeni und Buttazzo \cite{Abeni1998} entwickelten den Constant Bandwidth
	      Server als Grundlage fÃ¼r ressourcenisoliertes Scheduling.
\end{itemize}

Im neuen Jahrtausend rÃ¼ckte das Mixed-Criticality-Problem (Vestal \cite{Vestal2007},
...
\section*{Ãœber den EDF$^+$-Algorithmus}

Eine wichtige Erweiterung der klassischen EDF-Theorie ist der EDF$^+$-Algorithmus
\cite{Epp2026}, der in Kapitel~10 ausfÃ¼hrlich behandelt wird. EDF$^+$ Ã¼bertrÃ¤gt
die scharfe OptimalitÃ¤t von EDF auf Systeme mit stochastischen Blocking-Ereignissen:
...
optimalen EDF-Schedule fÃ¼r die verbleibenden nicht-blockierten Tasks.

Der Beweis der OptimalitÃ¤t von EDF$^+$ beruht direkt auf dem EDF-OptimalitÃ¤tstheorem
und ist konstruktiv: Durch vollstÃ¤ndige Induktion Ã¼ber die Anzahl der
Blocking-Ereignisse wird gezeigt, dass EDF$^+$ stets einen Deadline-freien
Schedule liefert, sofern die Systemauslastung $U \leq 1$ betrÃ¤gt. Dies macht
...
\textbf{Kapitel~2 -- Grundlegende Konzepte:} VollstÃ¤ndiges Task-Modell mit
allen Zeitparametern (Lateness, LaxitÃ¤t, Slack), Klassifikation von Scheduling-Problemen
nach der Graham-Notation $\alpha|\beta|\gamma$, GÃ¼tekriterien, Scheduling-Anomalien
und der Dhall-Effekt auf Mehrprozessoren.

\textbf{Kapitel~3 -- Aperiodisches Task-Scheduling:} Jacksons EDD-Algorithmus
\cite{Jackson1955}, vollstÃ¤ndiger Dertouzos-Beweis der EDF-OptimalitÃ¤t
\cite{Dertouzos1974}, Bratley's Branch-and-Bound fÃ¼r nicht-preemptives Scheduling
\cite{Bratley1971} und Lawlers LDF-Algorithmus \cite{Lawler1973} fÃ¼r Tasks
mit Vorrangrelationen.

\textbf{Kapitel~4 -- Periodisches Task-Scheduling:} VollstÃ¤ndige Analyse von
Rate Monotonic (Liu \& Layland \cite{Liu1973}) einschlieÃŸlich aller Beweise,
Hyperbolic Bound \cite{Bini2001}, Response Time Analysis \cite{Audsley1993},
EDF-Schedulierbarkeitstheorem \cite{Baruah1990} sowie der prÃ¤zise Vergleich
zwischen schwacher (RM) und scharfer (EDF) OptimalitÃ¤t.

\textbf{Kapitel~5 -- Fixed-Priority Server:} VollstÃ¤ndige Analyse von
...
DSS, TBS, CBS \cite{Abeni1998} und EDL-Server -- mit OptimalitÃ¤tsnachweisen
und dem fundamentalen Nicht-Existenz-Ergebnis fÃ¼r optimale Server \cite{Tia1995}.

\textbf{Kapitel~7 -- Ressourcenzugriffsprotokolle:} PrioritÃ¤tsinversion, NPP,
PIP und PCP \cite{Sha1990}, Stack Resource Policy \cite{Baker1991}, vollstÃ¤ndige
...
\textbf{Kapitel~9 -- Ãœberlastbehandlung:} Value-Based Scheduling, Cervin's
Theorem Ã¼ber EDF bei permanenter Ãœberlast \cite{Cervin2003}, Elastic Scheduling
und Admission Control.

\textbf{Kapitel~10 -- EDF$^+$-Algorithmus:} VollstÃ¤ndiger OptimalitÃ¤tsbeweis,
Laufzeitanalyse, Korollare (Dominanz Ã¼ber RM, Minimax-Eigenschaft) und
...
\textbf{Kapitel~13 -- Zusammenfassung und Ausblick:} GesamtÃ¼berblick,
praktische Empfehlungen und ein sehr ausfÃ¼hrlicher Ausblick auf offene
...
Scheduling). FÃ¼r ein vollstÃ¤ndiges VerstÃ¤ndnis der Kapitel Ã¼ber Ressourcenprotokolle
und CBS sind Grundkenntnisse Ã¼ber NebenlÃ¤ufigkeit hilfreich.

Mathematisch werden Kenntnisse der Analysis (Grenzwerte, Ableitungen) benÃ¶tigt,
die fÃ¼r das VerstÃ¤ndnis des Liu-Layland-Beweises (Kapitel~4) und des
Cervin-Theorems (Kapitel~9) notwendig sind.

\section*{Danksagung}

Der Autor dankt den BegrÃ¼ndern der Echtzeit-Scheduling-Theorie -- insbesondere
C.\,L.\ Liu, J.\,W.\ Layland, Giorgio C.\ Buttazzo, Lui Sha und Sanjoy Baruah --
fÃ¼r ihre grundlegenden BeitrÃ¤ge, ohne die dieses Buch nicht mÃ¶glich wÃ¤re.
Besonderer Dank gilt der fÃ¼r die wissenschaftliche
Infrastruktur und UnterstÃ¼tzung.
...
Echtzeit-Systeme sind Rechnersysteme, die innerhalb prÃ¤ziser Zeitschranken auf Ereignisse
in ihrer Umgebung reagieren mÃ¼ssen. Die Korrektheit solcher Systeme hÃ¤ngt nicht nur
vom Wert des Ergebnisses, sondern auch vom Zeitpunkt ab, zu dem das Ergebnis
erzeugt wird \cite{Stankovic1988}. Eine Reaktion, die zu spÃ¤t erfolgt, kann wertlos
oder sogar gefÃ¤hrlich sein. Echtzeit-Computing spielt heute eine entscheidende Rolle
in unserer Gesellschaft, da eine wachsende Anzahl komplexer Systeme vollstÃ¤ndig oder
...
vom logischen Ergebnis der Berechnung, sondern auch vom Zeitpunkt abhÃ¤ngt, zu dem
die Ergebnisse produziert werden.

Das Wort \emph{real} zeigt an, dass die Reaktion der Systeme auf externe Ereignisse
wÃ¤hrend deren Ablauf erfolgen muss. Folglich muss die Systemzeit (interne Zeit) mit
...
\begin{definition}[Echtzeit-System]
	Ein \textbf{Echtzeit-System} ist ein Rechnersystem, das innerhalb prÃ¤ziser,
	vorher festgelegter Zeitgrenzen auf Ereignisse in seiner Umgebung reagieren muss.
	Die Korrektheit hÃ¤ngt sowohl vom Wert als auch vom Zeitpunkt der erzeugten
	Ergebnisse ab.
\end{definition}

\subsection{Typen von Echtzeit-Tasks}

AbhÃ¤ngig von den Konsequenzen einer versÃ¤umten Deadline unterscheidet man:

\begin{definition}[Harte Echtzeit-Task]
	Eine Echtzeit-Task heiÃŸt \textbf{hart}, wenn die Erzeugung von Ergebnissen nach
	ihrer Deadline katastrophale Folgen fÃ¼r das System unter Kontrolle haben kann.
\end{definition}

\begin{definition}[Weiche Echtzeit-Task]
	Eine Echtzeit-Task heiÃŸt \textbf{weich}, wenn die Erzeugung von Ergebnissen nach
...
und im Voraus garantieren kÃ¶nnen, dass alle kritischen Zeitbedingungen eingehalten werden.
...
\begin{itemize}
	\item \textbf{DMA}: Der Prozessor muss warten, wenn ein DMA-GerÃ¤t einen Speicherzyklus
	      stiehlt. Die Anzahl solcher Wartezeiten ist nicht vorhersagbar.
	\item \textbf{Cache-Speicher}: Cache-Fehlzugriffe (Cache Misses) fÃ¼hren zu variablen
	      Zugriffszeiten. PrÃ¤emptionen verschlechtern die Cache-LokalitÃ¤t erheblich.
	\item \textbf{Interrupts}: KÃ¶nnen unbeschrÃ¤nkte VerzÃ¶gerungen in die Task-AusfÃ¼hrung
	      einfÃ¼hren, wenn sie nicht ordnungsgemÃ¤ÃŸ behandelt werden.
	\item \textbf{Systemaufrufe}: UnbeschrÃ¤nkte AusfÃ¼hrungszeiten von Kernelprimitiven
	      verhindern prÃ¤zise WCET-AbschÃ¤tzungen.
	\item \textbf{Semaphore und PrioritÃ¤tsinversion}: Klassische Semaphore kÃ¶nnen
	      hochpriore Tasks durch niederpriore Tasks fÃ¼r unbeschrÃ¤nkte Zeit blockieren.
	\item \textbf{Speicherverwaltung}: Demand-Paging verursacht groÃŸe und unvorhersagbare
	      VerzÃ¶gerungen durch Seitenfehler.
\end{itemize}
...
Dieses Kapitel legt das vollstÃ¤ndige begriffliche Fundament fÃ¼r alle
Scheduling-Algorithmen der folgenden Kapitel. Es definiert prÃ¤zise die
...
grundlegenden GÃ¼tekriterien. Besonderes Gewicht liegt auf den Konzepten
der \emph{Lateness}, \emph{Laxity} und \emph{Slack}, die die Basis fÃ¼r
die OptimalitÃ¤tsbeweise in Kapitel~\ref{chap:aperiodisch} und
...
\begin{definition}[Job (Aperiodische Task)]
	Ein \textbf{Job} $J_i$ ist eine atomare AusfÃ¼hrungseinheit mit folgenden
	Zeitparametern:
	\begin{itemize}
		\item $a_i$ (Arrival Time, Ankunftszeit): Zeitpunkt, ab dem $J_i$ zur
		      AusfÃ¼hrung bereit ist. Auch: \emph{release time} $r_i$.
		\item $C_i$ (Worst-Case Execution Time, WCET): Maximale AusfÃ¼hrungszeit
		      unter allen mÃ¶glichen Eingaben. UnverÃ¤nderlich fÃ¼r jede Instanz.
		\item $d_i$ (Absolute Deadline): SpÃ¤tester Fertigstellungszeitpunkt.
		\item $D_i$ (Relative Deadline): $D_i = d_i - a_i$. Zeitdauer ab
		      Ankunft bis zur Deadline.
		\item $f_i$ (Finishing Time, Fertigstellungszeit): TatsÃ¤chlicher
		      Abschluss der AusfÃ¼hrung im konkreten Schedule.
		\item $s_i$ (Start Time, Startzeitpunkt): Erster Zeitpunkt, zu dem
		      $J_i$ tatsÃ¤chlich ausgefÃ¼hrt wird.
	\end{itemize}
\end{definition}

\begin{definition}[Abgeleitete ZeitgrÃ¶ÃŸen]
	Aus den Grundparametern werden folgende GrÃ¶ÃŸen abgeleitet:
	\begin{align*}
		R_i &= f_i - a_i \quad\text{(Response Time, Antwortzeit)}\\
		L_i &= f_i - d_i \quad\text{(Lateness: positiv = zu spÃ¤t, negativ = Puffer)}\\
		E_i &= \max(0, f_i - d_i) \quad\text{(Tardiness: VerspÃ¤tung, nie negativ)}\\
...
\begin{bemerkung}[ZusammenhÃ¤nge der ZeitgrÃ¶ÃŸen]
	Die LaxitÃ¤t $X_i = D_i - C_i$ gibt an, wie lange $J_i$ insgesamt warten
	darf, ohne ihre Deadline zu verpassen. Der Slack $\text{slack}_i(t)$ ist
	die verbleibende WartekapazitÃ¤t zum Zeitpunkt $t$:
	\begin{itemize}
...
\begin{beispiel}[Zeitparameter]
	...
	\begin{itemize}
		\item $D_1 = 8-2 = 6$, $X_1 = 6-3 = 3$ (LaxitÃ¤t: darf 3 Einheiten warten)
		\item Wenn $s_1=4$, $f_1=7$: $R_1=5$, $L_1=-1$ (1 Einheit Puffer), $E_1=0$
		\item Wenn $s_1=6$, $f_1=9$: $R_1=7$, $L_1=1$ (1 Einheit zu spÃ¤t), $E_1=1$
...
\begin{definition}[Periodische Task]
	Eine \textbf{periodische Task} $\tau_i$ erzeugt in regelmÃ¤ÃŸigen AbstÃ¤nden
	Jobs. Sie ist durch folgende Parameter vollstÃ¤ndig beschrieben:
	\begin{itemize}
...
\begin{definition}[Prozessorauslastung]
	...
	\]
	$U$ gibt den Anteil der Prozessorzeit an, der fÃ¼r die periodischen Tasks
	\emph{im Durchschnitt} benÃ¶tigt wird. Wenn $U > 1$, ist kein machbarer
	Schedule unter irgendeinem Algorithmus mÃ¶glich.
\end{definition}
...
\begin{definition}[Synchroner vs. asynchroner Task-Set]
	...
	\end{itemize}
	Die kritische Analyse erfolgt stets fÃ¼r synchrone Aktivierungen (kritischer
	Augenblick -- schlimmster Fall fÃ¼r RM, vgl.\ Kapitel~\ref{chap:periodisch}).
\end{definition}

\subsection{Sporadische und aperiodische Tasks}

\begin{definition}[Sporadische Task]
	Eine \textbf{sporadische Task} ist eine Task mit unregelmÃ¤ÃŸigen Aktivierungen,
	jedoch mit einer Mindestankunftsabstand (Minimum Interarrival Time) $T_i^{\min}$:
	$a_{i,k+1} - a_{i,k} \geq T_i^{\min}$. Sporadische Tasks kÃ¶nnen offline
...
\begin{definition}[Firme aperiodische Task]
	...
	garantiert: Beim Eintreffen prÃ¼ft ein Admission-Controller, ob sie ohne
...
\begin{definition}[Graham-Notation $\alpha|\beta|\gamma$]
	...
	\begin{itemize}
		...
		\item $\gamma$: \textbf{GÃ¼tekriterium} -- z.\,B.\ $L_{\max}$ (maximale Lateness),
...
\subsection{GÃ¼tekriterien}

\begin{definition}[GÃ¼tekriterien im Ãœberblick]
	\begin{itemize}
		\item $L_{\max} = \max_i L_i$: \textbf{Maximale Lateness}. Negativ $\Rightarrow$
		      alle Deadlines eingehalten. OptimalitÃ¤tsmaÃŸ fÃ¼r EDF und EDD.
		...
		\item $n_{\text{late}} = |\{J_i \mid f_i > d_i\}|$: \textbf{Anzahl verspÃ¤teter
		      Tasks}. NP-schwer fÃ¼r beliebige Ankunftszeiten.
		\item \textbf{Feasibility}: Existenz eines Schedules mit $L_{\max} \leq 0$.
	\end{itemize}
\end{definition}

\begin{bemerkung}[Zusammenhang GÃ¼tekriterien]
	...
	von $L_{\max}$ das stÃ¤rkere und nÃ¼tzlichere Kriterium gegenÃ¼ber
	bloÃŸer Machbarkeit.
\end{bemerkung}
...
\begin{definition}[Taxonomie der Scheduling-Algorithmen]
	\begin{itemize}
		\item \textbf{Preemptiv vs. Nicht-Preemptiv}: Darf eine laufende Task
		      unterbrochen werden?
		\item \textbf{Statisch vs. Dynamisch}: Werden PrioritÃ¤ten vor oder
		      wÃ¤hrend der AusfÃ¼hrung berechnet?
		...
		\item \textbf{Optimal vs. Heuristisch}: Findet der Algorithmus stets
		      die beste LÃ¶sung, oder nur eine gute NÃ¤herung?
		\item \textbf{Clairvoyant vs. Non-Clairvoyant}: Hat der Algorithmus
		      vollstÃ¤ndige Kenntnis aller zukÃ¼nftigen Ankunftszeiten?
	\end{itemize}
\end{definition}

\begin{definition}[OptimalitÃ¤tsbegriffe -- Zusammenfassung]
	...
	\begin{itemize}
		\item \textbf{Schwach optimal} (innerhalb einer Klasse): Findet den
		      ...
		      Beispiel: RM ist schwach optimal unter festen PrioritÃ¤ten.
		\item \textbf{Scharf optimal} (global): Findet jeden machbaren Schedule,
		      der Ã¼berhaupt existiert. Beispiel: EDF ist scharf optimal fÃ¼r
		      preemptive Einzelprozessoren.
		\item \textbf{Optimal bezÃ¼glich $\gamma$}: Minimiert das GÃ¼tekriterium
		      $\gamma$ unter allen mÃ¶glichen Schedules.
	\end{itemize}
\end{definition}

\section{Vorrangrelationen und RessourcenabhÃ¤ngigkeiten}
...
\begin{definition}[Vorranggraph (Precedence Graph)]
	...
	wenn $J_i$ vollstÃ¤ndig abgeschlossen ist. Notationen:
	\begin{itemize}
		\item $J_i \to J_j$: $J_i$ ist unmittelbarer VorgÃ¤nger von $J_j$
		\item $J_i \prec J_j$: $J_i$ ist (direkter oder indirekter) VorgÃ¤nger
	\end{itemize}
\end{definition}

\begin{definition}[Ressourcenkonflikt]
	Zwei Tasks $J_i$ und $J_j$ stehen in einem \textbf{Ressourcenkonflikt},
	wenn sie dieselbe exklusive Ressource $R_k$ benÃ¶tigen. Ressourcenkonflikte
	fÃ¼hren zu Blockierung und sind der Hauptgrund fÃ¼r PrioritÃ¤tsinversion
...
\begin{definition}[Scheduling-Anomalie nach Graham]
	...
	Eingabeparameter -- weniger Tasks, kÃ¼rzere WCETs, schwÃ¤chere Vorrangrelationen,
	mehr Prozessoren -- zu einem \emph{schlechteren} Schedule-Ergebnis (z.\,B.\
	lÃ¤ngerer Makespan) fÃ¼hrt.
\end{definition}
...
\begin{beispiel}[Graham-Anomalie auf Mehrprozessorsystemen]
	...
	EinschrÃ¤nkungen. Graham (1969) bewies: Solche Anomalien sind bei allgemeinen
	Mehrprozessorsystemen unter List Scheduling unvermeidbar.
\end{beispiel}

\begin{beispiel}[Anomalie bei Selbstsuspension]
	Task $\tau_1$ mit $C_1=1$, $T_1=4$, Deadline $d_1=4$ hat Slack~3. FÃ¼hrt
	$\tau_1$ eine Selbstsuspension von 2 Einheiten aus, verpasst sie ihre Deadline.
	Obwohl $\tau_1$ selbst weniger Rechenzeit benÃ¶tigt (wegen Suspension), wird
	das System schlechter -- ein direktes Beispiel fÃ¼r eine Anomalie bei
	Selbstsuspensionen unter RM.
\end{beispiel}

\begin{bemerkung}[Anomalien und Konsequenzen fÃ¼r den Systementwurf]
	Scheduling-Anomalien haben direkte praktische Konsequenzen:
	\begin{enumerate}
		\item \textbf{WCET-ErhÃ¶hung} einer Task kann durch zusÃ¤tzliche Preemptionen
		      zu Deadline-Verletzungen fÃ¼hren, obwohl das System ursprÃ¼nglich
		      schedulierbar war.
		\item \textbf{Selbstsuspensionen} (z.\,B.\ fÃ¼r I/O) mÃ¼ssen explizit in
		      der Schedulierbarkeitsanalyse berÃ¼cksichtigt werden.
...
\begin{proof}[Beweisidee]
	...
	fÃ¼r $i=1,\ldots,m$ (kurze Tasks, frÃ¼he Deadlines) und $C_{m+1} = 1$,
	$T_{m+1} = 1+\varepsilon$ (lange Task, spÃ¤tere Deadline). Auslastung:
	$U = m \cdot \varepsilon/(\varepsilon+\delta) + 1/(1+\varepsilon) \to 1$
	fÃ¼r kleine $\varepsilon, \delta$. EDF belegt alle $m$ Prozessoren mit
	den $m$ kurzperiodischen Tasks, da diese immer die frÃ¼heste Deadline haben,
	und lÃ¤sst die lange Task verhungern. \hfill$\blacksquare$
\end{proof}
...
\section{VollstÃ¤ndige Notation und Annahmen}

Zur Vereinheitlichung gelten in diesem Buch folgende Standardannahmen,
sofern nicht ausdrÃ¼cklich anders angegeben:
...
\begin{itemize}
	...
	\item[\textbf{A4}] Tasks sind unabhÃ¤ngig (keine Vorrangrelationen, keine gemeinsamen Ressourcen).
...
Annahmen A3 und A4 werden in spÃ¤teren Kapiteln gezielt aufgehoben:
...
\begin{table}[H]
	\centering
	\caption{Ãœbersicht: Zeitparameter periodischer Tasks}
	\begin{tabular}{cll}
		\toprule
		...
		$C_i$ & WCET & Max.\ AusfÃ¼hrungszeit pro Instanz \\
...
auf einem Einzelprozessorsystem. Jeder Algorithmus stellt eine LÃ¶sung fÃ¼r ein
bestimmtes Scheduling-Problem dar.

Der fundamentale Kernsatz gilt bereits fÃ¼r aperiodische Tasks:
...
Das durch Jackson gelÃ¶ste Problem ist $1 \mid \text{sync} \mid L_{\max}$: Eine Menge
...
\begin{theorem}[Jackson 1955]
	\label{thm:jackson}
	Gegeben sei eine Menge von $n$ unabhÃ¤ngigen Tasks. Jeder Algorithmus, der die
	Tasks in Reihenfolge nichtabsteigender Deadlines ausfÃ¼hrt, ist optimal bezÃ¼glich
...
\begin{proof}
	...
	$\sigma$ unmittelbar vor $J_a$ lÃ¤uft. Sei $\sigma'$ ein Schedule, der durch
	Vertauschen von $J_a$ und $J_b$ in $\sigma$ entsteht. Das Vertauschen kann die
	maximale Lateness nicht erhÃ¶hen: In beiden FÃ¤llen ($L'_a \geq L'_b$ oder
...
\subsection{Das EDF-OptimalitÃ¤tstheorem}
...
\begin{quote}
\textbf{Wenn es einen machbaren Schedule gibt, findet EDF diesen sofort. Die kÃ¼rzeste
AusfÃ¼hrungszeit aller Tasks findet EDF sofort. EDF ist scharf optimal.}
\end{quote}

\begin{theorem}[Horn 1974 / Dertouzos 1974 -- EDF OptimalitÃ¤t]
	\label{thm:edf_optimal}
	Gegeben sei eine Menge von $n$ unabhÃ¤ngigen Tasks mit beliebigen Ankunftszeiten.
	Jeder Algorithmus, der zu jedem Zeitpunkt die Task mit der frÃ¼hesten absoluten
	Deadline unter allen bereiten Tasks ausfÃ¼hrt, ist optimal bezÃ¼glich der
...
\begin{proof}[Beweis (Dertouzos 1974)]
	...
	Zeitscheiben ausgefÃ¼hrt werden.

	\textbf{Notationen:}
	\begin{itemize}
		\item $\sigma(t)$: Task, die in der Scheibe $[t, t+1)$ ausgefÃ¼hrt wird.
		\item $E(t)$: Bereite Task, die zum Zeitpunkt $t$ die frÃ¼heste Deadline hat.
		\item $t_E(t)$: Zeitpunkt ($\geq t$), zu dem die nÃ¤chste Scheibe von $E(t)$
		      in $\sigma$ ausgefÃ¼hrt wird.
	\end{itemize}

	Wenn $\sigma \neq \sigma_{\text{EDF}}$, existiert ein Zeitpunkt $t$ mit
	$\sigma(t) \neq E(t)$. Das Vertauschen der Scheiben bei $t$ und $t_E$ kann die
	maximale Lateness nicht erhÃ¶hen: Wird eine Scheibe vorgezogen, bleibt die
	...
	$d_E \leq d_i$ fÃ¼r alle $i$, gilt $t_E + 1 \leq d_i$, was die Machbarkeit
	der verschobenen Scheibe garantiert.

	Durch Anwendung des Vertauschungsalgorithmus auf alle Zeitscheiben von $t=0$
	bis $t=D$ (dem spÃ¤testen Deadline-Zeitpunkt) wird $\sigma$ in $\sigma_{\text{EDF}}$
	umgewandelt, ohne die maximale Lateness zu erhÃ¶hen. Da EDF die maximale
...
\begin{definition}[Scharf optimal]
	Ein Scheduling-Algorithmus $\mathcal{A}$ heiÃŸt \textbf{scharf optimal}, wenn er
	fÃ¼r jede Task-Klasse $\mathfrak{S}$ gilt:
	\[
		\forall\, \Sigma \in \mathfrak{S}: \quad
		\Sigma \text{ ist schedulierbar} \iff \mathcal{A} \text{ findet einen machbaren Schedule.}
	\]
	Die Bedingung ist eine Ã„quivalenz, nicht nur eine Implikation.
\end{definition}
...
\begin{enumerate}
	...
	\item Wenn EDF keinen machbaren Schedule findet, existiert keiner -- EDF scheitert
	      ...
	      strukturell nicht erfÃ¼llbar sind.
	\item EDF minimiert $L_{\max}$ unter allen Algorithmen.
\end{enumerate}

Die SchÃ¤rfe bedeutet: EDF nutzt die gesamte verfÃ¼gbare ProzessorkapazitÃ¤t aus.
Die kÃ¼rzeste AusfÃ¼hrungszeit aller Tasks findet EDF sofort, da EDF immer die Task
mit der frÃ¼hesten Deadline priorisiert -- also diejenige, die am dringlichsten ist.

\subsection{Garantiebedingung fÃ¼r EDF}

Wenn Tasks dynamische Aktivierungszeiten haben, muss der Garantietest
dynamisch bei jeder neuen Task-Ankunft durchgefÃ¼hrt werden:

\begin{satz}[EDF-Garantietest]
	Sei $J$ die aktuell garantierte Task-Menge. Eine neu ankommende Task $J_{\text{new}}$
	kann garantiert werden, wenn fÃ¼r die Vereinigung $J' = J \cup \{J_{\text{new}}\}$
	gilt:
	\[
		\forall i = 1, \ldots, n: \quad \sum_{k=1}^{i} c_k(t) \leq d_i,
	\]
	wobei $c_k(t)$ die verbleibende AusfÃ¼hrungszeit von $J_k$ zum Zeitpunkt $t$ und
...
EDF ist in diesem Modell \textbf{nicht mehr optimal}: Es kann FÃ¤lle geben, in
denen ein machbarer Schedule existiert, EDF diesen aber nicht findet. Dies
unterstreicht die Bedeutung der PreemptivitÃ¤t fÃ¼r die OptimalitÃ¤t von EDF.

\subsection{Bratley's Algorithmus}

Bratley's Algorithmus (1971) lÃ¶st das nicht-preemptive Scheduling durch
...
\begin{itemize}
	\item Ein Ast wird abgebrochen, wenn jede Erweiterung des aktuellen Pfades
	      zu einer versÃ¤umten Deadline fÃ¼hrt.
	\item Ein Ast wird abgebrochen, wenn ein machbarer Schedule gefunden wurde.
\end{itemize}
Die KomplexitÃ¤t ist $O(n \cdot n!)$ im schlimmsten Fall.

\section{Scheduling mit Vorrangrelationen}

\begin{theorem}[Lawler 1973 -- LDF OptimalitÃ¤t]
	Der Latest Deadline First (LDF) Algorithmus minimiert die maximale Lateness
	fÃ¼r eine Menge von Tasks mit Vorrangrelationen und synchronen Ankunftszeiten.
\end{theorem}

FÃ¼r Tasks mit Vorrangrelationen und dynamischen Aktivierungszeiten existiert der
EDF*-Algorithmus von Chetto et al. (1990), der eine Transformation der
Zeitparameter durchfÃ¼hrt, um EDF anwendbar zu machen.
...
In vielen Echtzeit-Kontrollanwendungen stellen periodische AktivitÃ¤ten den
...
Diese Aussage wird durch die Schedulierbarkeitsanalyse prÃ¤zisiert: RM kann nur
Auslastungen bis $\ln 2 \approx 69\%$ im schlimmsten Fall garantieren, wÃ¤hrend
EDF die volle ProzessorkapazitÃ¤t bis $U = 1$ ausschÃ¶pft.
...
\begin{itemize}
	...
	\item $C_i$: Worst-Case AusfÃ¼hrungszeit von $\tau_i$
...
LÃ¤nge eines Slots (GCD der Perioden), der \textbf{Major Cycle} ist der
Hyperperiod (LCM der Perioden).

\textbf{Vorteile:} Einfach zu implementieren, kein Overhead durch Context Switches,
deterministische AusfÃ¼hrungsreihenfolge.

\textbf{Nachteile:} SprÃ¶de bei Ãœberlastbedingungen, sensibel gegenÃ¼ber
AnwendungsÃ¤nderungen, schwierig fÃ¼r aperiodische Tasks.

\section{Rate Monotonic Scheduling -- Detaillierte Analyse}

\subsection{EinfÃ¼hrung und Grundidee}

Der Rate Monotonic (RM) Scheduling-Algorithmus wurde 1973 von Liu und Layland
eingefÃ¼hrt \cite{Liu1973} und gehÃ¶rt heute zu den meistgenutzten Algorithmen fÃ¼r
periodische Echtzeit-Tasks in eingebetteten Systemen. Die Grundidee ist denkbar
einfach: Tasks werden statische PrioritÃ¤ten \emph{umgekehrt proportional} zu ihrer
Periode zugewiesen. Eine Task mit kurzer Periode (also hoher Anforderungsrate) erhÃ¤lt
eine hÃ¶here PrioritÃ¤t als eine Task mit langer Periode.

\begin{definition}[Rate Monotonic PrioritÃ¤tszuweisung]
	Der \textbf{Rate Monotonic} Algorithmus weist jeder periodischen Task $\tau_i$
	eine PrioritÃ¤t $P_i$ zu, die umgekehrt proportional zur Periode ist:
	\[
		T_i < T_j \implies P_i > P_j.
	\]
	Tasks mit gleicher Periode erhalten beliebige, aber feste PrioritÃ¤ten.
\end{definition}

Da die Perioden konstant sind, ist RM eine \textbf{feste PrioritÃ¤tszuweisung}: Die
PrioritÃ¤t einer Task Ã¤ndert sich wÃ¤hrend der AusfÃ¼hrung nicht. RM ist auÃŸerdem
...
\begin{beispiel}[Einfache RM-Zuweisung]
	...
	\]
	RM ordnet die PrioritÃ¤ten: $P_1 > P_2 > P_3$, da $T_1 < T_2 < T_3$.
...
Das wichtigste Konzept fÃ¼r die Analyse von RM ist der \emph{kritische Augenblick}
(critical instant). Er gibt an, unter welchen Bedingungen eine Task die grÃ¶ÃŸte
Antwortzeit erfÃ¤hrt.

\begin{definition}[Kritischer Augenblick]
	Der \textbf{kritische Augenblick} einer Task $\tau_i$ ist derjenige
	Ankunftszeitpunkt, der die grÃ¶ÃŸte Antwortzeit von $\tau_i$ produziert.
	Die zugehÃ¶rige \textbf{kritische Zeitzone} ist das Intervall zwischen dem
...
\begin{lemma}[Liu \& Layland 1973 -- Kritischer Augenblick]
	...
	Der kritische Augenblick fÃ¼r eine Task $\tau_i$ unter RM tritt auf, wenn
	$\tau_i$ \emph{gleichzeitig} mit allen Tasks hÃ¶herer PrioritÃ¤t aktiviert wird.
\end{lemma}

\begin{proof}
	Wir zeigen durch sukzessives VorrÃ¼cken der Ankunftszeitpunkte hochpriorer Tasks,
	dass die Antwortzeit von $\tau_i$ maximal wird, wenn alle gleichzeitig ankommen.

	Sei $\tau_n$ die Task mit der niedrigsten PrioritÃ¤t und $\tau_i$ eine Task mit
	hÃ¶herer PrioritÃ¤t. Die Antwortzeit von $\tau_n$ wird durch die Interferenz von
	$\tau_i$ verzÃ¶gert. Wie in Abbildung (a) dargestellt, verzÃ¶gert eine Instanz von
	$\tau_i$ die AusfÃ¼hrung von $\tau_n$ um $C_i$. Das Vorziehen der Ankunftszeit von
	$\tau_i$ kann die Fertigstellungszeit von $\tau_n$ weiter erhÃ¶hen (Abbildung b),
	da eine zusÃ¤tzliche Instanz von $\tau_i$ in das Interferenzintervall fallen kann.

	Durch wiederholtes VorrÃ¼cken aller hochprioren Tasks wird die Antwortzeit von
	$\tau_n$ maximiert, wenn alle gleichzeitig ankommen. Dieses Argument gilt fÃ¼r
	jede Task $\tau_n$ mit niedrigster PrioritÃ¤t, und durch Rekursion fÃ¼r jede
	Task im System. \hfill$\blacksquare$
\end{proof}

\begin{bemerkung}
	Lemma~\ref{lem:critical_instant} hat eine weitreichende praktische Konsequenz:
	Um zu prÃ¼fen, ob ein Task-Set unter RM schedulierbar ist, genÃ¼gt es, den
	...
	ungÃ¼nstigsten Fall ihre Deadlines einhalten, sind sie \emph{unter allen}
	mÃ¶glichen Ankunftsbedingungen schedulierbar.
\end{bemerkung}

\subsection{VollstÃ¤ndiger OptimalitÃ¤tsbeweis von RM}

\begin{definition}[Schwach optimal]
	\label{def:schwach_optimal}
	Ein Scheduling-Algorithmus $\mathcal{A}$ heiÃŸt \textbf{schwach optimal}
...
\begin{theorem}[Liu \& Layland 1973 -- RM OptimalitÃ¤t]
	\label{thm:rm_optimal}
	RM ist \textbf{schwach optimal} unter allen Algorithmen mit fester PrioritÃ¤t:
	Wenn ein Task-Set $\Gamma$ von \emph{irgendeiner} festen PrioritÃ¤tszuweisung
	scheduliert werden kann, so kann es auch von RM scheduliert werden.
\end{theorem}

\begin{proof}
	Der Beweis wird durch ein direktes Austauschargument fÃ¼r zwei Tasks gefÃ¼hrt
	und anschlieÃŸend auf $n$ Tasks verallgemeinert.

	\textbf{Schritt 1: Zwei Tasks, $T_1 < T_2$.}

	Angenommen, die PrioritÃ¤ten seien \emph{nicht} gemÃ¤ÃŸ RM zugewiesen, d.\,h.\
	$\tau_2$ hat eine hÃ¶here PrioritÃ¤t als $\tau_1$. Am kritischen Augenblick
	($t = 0$) sieht der Schedule aus wie in Abbildung~\ref{fig:rm_non_rm_priority}:
	$\tau_2$ wird zuerst ausgefÃ¼hrt, dann $\tau_1$. FÃ¼r Machbarkeit muss gelten:
	\begin{equation}
		C_1 + C_2 \leq T_1.
		\label{eq:non_rm_feasible}
	\end{equation}

	Nun weisen wir die PrioritÃ¤ten gemÃ¤ÃŸ RM zu, d.\,h.\ $\tau_1$ erhÃ¤lt die hÃ¶here
	PrioritÃ¤t ($T_1 < T_2$). Wir mÃ¼ssen zeigen, dass die Machbarkeitsbedingung
	(\ref{eq:non_rm_feasible}) auch unter RM ausreicht. Dazu unterscheiden wir
	zwei FÃ¤lle, je nachdem, wie $C_1$ und $T_2$ sich zueinander verhalten. Sei
	$F = \lfloor T_2 / T_1 \rfloor$ die Anzahl vollstÃ¤ndiger $T_1$-Perioden
	innerhalb von $T_2$.
	...
	womit (\ref{eq:rm_case1}) erfÃ¼llt ist.

	\textbf{Fall 2:} $C_1 \geq T_2 - F T_1$.

	Hier Ã¼berlappt die AusfÃ¼hrung von $\tau_1$ mit der zweiten Instanz von $\tau_2$.
	Die Machbarkeitsbedingung fÃ¼r $\tau_2$ unter RM ist:
	\begin{equation}
		F C_1 + C_2 \leq F T_1.
		\label{eq:rm_case2}
	\end{equation}
	...
	womit (\ref{eq:rm_case2}) erfÃ¼llt ist.

	In beiden FÃ¤llen gilt: Wenn das Task-Set unter der nicht-RM-Zuweisung
	...
	beliebigen festen PrioritÃ¤tszuweisung $\pi$ scheduliert wird. Wenn $\pi$
	nicht die RM-Reihenfolge ist, existieren zwei Tasks $\tau_i, \tau_j$ mit
	$T_i < T_j$, aber $P_i^{\pi} < P_j^{\pi}$ (invertierte PrioritÃ¤t). Durch
	Anwendung des Zwei-Task-Argumentes aus Schritt 1 kÃ¶nnen die PrioritÃ¤ten
	von $\tau_i$ und $\tau_j$ vertauscht werden, ohne die Schedulierbarkeit zu
	verletzen. Da das Task-Set endlich ist, fÃ¼hrt die iterierte Anwendung dieser
...
\subsection{Berechnung des Least Upper Bound fÃ¼r zwei Tasks}
...
FÃ¼r $n = 2$ Tasks $\tau_1$ und $\tau_2$ mit $T_1 < T_2$ berechnen wir $U_{\text{lub}}$
wie folgt:
\begin{enumerate}
	\item Zuweisung gemÃ¤ÃŸ RM: $\tau_1$ erhÃ¤lt die hÃ¶here PrioritÃ¤t.
	\item ErhÃ¶hung von $C_2$ bis der Prozessor vollstÃ¤ndig ausgelastet ist.
	\item Minimierung der oberen Schranke $U_{\text{ub}}$ Ã¼ber alle Periodenrelationen.
\end{enumerate}
...
Der grÃ¶ÃŸtmÃ¶gliche Wert von $C_2$ betrÃ¤gt $C_2 = T_2 - C_1(F+1)$, und die
...
ist $U_{\text{ub}}$ monoton fallend in $C_1$. Das Minimum wird also bei dem grÃ¶ÃŸten
erlaubten $C_1$ erreicht, nÃ¤mlich $C_1 = T_2 - F T_1$.

\subsubsection{Fall 2: $C_1 \geq T_2 - F T_1$}

Der grÃ¶ÃŸtmÃ¶gliche Wert von $C_2$ betrÃ¤gt $C_2 = (T_1 - C_1) F$, und:
\[
...
In beiden FÃ¤llen wird das Minimum bei $C_1 = T_2 - T_1 F$ erreicht. Einsetzen liefert
mit der AbkÃ¼rzung $G = T_2/T_1 - F$ (sodass $0 \leq G < 1$):
\[
	U_{\text{ub}} = \frac{T_1}{T_2}F + \frac{(T_2 - T_1 F)}{T_2}
	\cdot\frac{T_2/T_1 - F}{1}
	= \frac{F + G^2}{F + G}.
\]
FÃ¼r $F = 1$ (den ungÃ¼nstigsten Fall) vereinfacht sich das zu:
\[
	U = \frac{1 + G^2}{1 + G}.
\]
Minimierung Ã¼ber $G \in [0,1)$ durch Ableiten ergibt:
\[
	\frac{dU}{dG} = \frac{2G(1+G) - (1+G^2)}{(1+G)^2} = \frac{G^2 + 2G - 1}{(1+G)^2} = 0,
\]
also $G = -1 + \sqrt{2}$ (positive LÃ¶sung). Einsetzen liefert:
\[
...
\begin{bemerkung}
	...
	$U_{\text{ub}} = 1$. Das bedeutet: FÃ¼r harmonische Perioden ($T_2 = k T_1$)
	kann RM den Prozessor vollstÃ¤ndig auslasten.
\end{bemerkung}

\subsection{Berechnung des Least Upper Bound fÃ¼r $n$ Tasks}

\begin{theorem}[Liu \& Layland 1973 -- RM Least Upper Bound]
	\label{satz:rm_lub}
	FÃ¼r eine beliebige Menge von $n$ periodischen Tasks unter den Annahmen A1--A4
	ist der Least Upper Bound der Prozessorauslastung unter dem Rate Monotonic
	Algorithmus:
	\[
		U_{\text{lub}}(\text{RM}, n) = n\left(2^{1/n} - 1\right).
	\]
	FÃ¼r $n \to \infty$ gilt $U_{\text{lub}} \to \ln 2 \approx 0{,}693$.
\end{theorem}

\begin{proof}
	Die schlechteste Bedingung fÃ¼r die Schedulierbarkeit, d.\,h.\ das globale
	Minimum der oberen Schranke $U_{\text{ub}}$ Ã¼ber alle Task-Sets, die den
	Prozessor vollstÃ¤ndig auslasten, wird durch folgende Parameterwahl erreicht
	\cite{Liu1973}:
	\begin{equation}
		\begin{cases}
			T_1 < T_n < 2 T_1 \\
	...
	\end{equation}
	Die resultierende Prozessorauslastung lÃ¤sst sich mit den AbkÃ¼rzungen
	$R_i = T_{i+1}/T_i$ schreiben als:
	\[
		U = \sum_{i=1}^{n-1} R_i + \frac{2}{R_1 R_2 \cdots R_{n-1}} - n.
	\]
	Minimierung Ã¼ber alle $R_i$ durch partielles Ableiten:
	\[
		\frac{\partial U}{\partial R_k} = 1 - \frac{2}{R_k^2 \cdot \prod_{i \neq k} R_i} = 0.
	\]
	Mit $P = \prod_{i=1}^{n-1} R_i$ erhÃ¤lt man $R_k P = 2$ fÃ¼r alle $k$, also:
	\[
...
\begin{table}[H]
	\centering
	\caption{Werte von $U_{\text{lub}}$ fÃ¼r RM als Funktion der Taskanzahl $n$}
	\label{tab:rm_lub}
	\begin{tabular}{cc|cc}
		\toprule
...
\begin{bemerkung}
	...
	monoton gegen $\ln 2 \approx 0{,}693$ fÃ¤llt. Das bedeutet: FÃ¼r sehr groÃŸe
	...
	um bis zu 31 Prozentpunkte hÃ¶here Nutzung der ProzessorkapazitÃ¤t.
\end{bemerkung}
...
\begin{beispiel}[Anwendung des Liu-Layland-Tests]
	...
	Task-Set kÃ¶nnte dennoch schedulierbar sein.
\end{beispiel}
...
\begin{bemerkung}[Test ist hinreichend, nicht notwendig]
	...
	wenn die Perioden gÃ¼nstig zueinander liegen (z.\,B.\ bei harmonischen Perioden).
...
\subsection{Hyperbolic Bound -- Engere Schranke fÃ¼r RM}

Der Hyperbolic Bound von Bini und Buttazzo \cite{Bini2001} ist eine schÃ¤rfere
...
\begin{proof}
	...
	Dort gilt $U_i = R_i - 1$ fÃ¼r $i = 1,\ldots,n-1$, also $R_i = U_i + 1$. Damit ist
	$\prod_{i=1}^{n-1} R_i = T_n/T_1$. Die Schedulierbarkeitsbed.~(\ref{eq:rm_case2})
	fÃ¼r $\tau_n$ lautet $U_n \leq 2T_1/T_n - 1$, d.\,h.\ $(U_n+1) \leq 2 / \prod_{i=1}^{n-1}(U_i+1)$,
...
\begin{bemerkung}[Vergleich der Schranken]
	...
	hÃ¤ufig besser. Er akzeptiert Task-Sets, die der Liu-Layland-Bound ablehnen wÃ¼rde.
	Formell gilt:
	\[
		\prod_{i=1}^{n}(U_i + 1) \leq 2 \text{ ist eine striktere Bedingung als } \sum_{i=1}^n U_i \leq n(2^{1/n}-1)
	\]
	in dem Sinne, dass die zulÃ¤ssige Schedulierbarkeitsregion durch den Hyperbolic
	Bound grÃ¶ÃŸer ist. Der Gewinn wÃ¤chst mit der Anzahl der Tasks und nÃ¤hert sich
	$\sqrt{2}$ fÃ¼r $n \to \infty$.
\end{bemerkung}
...
notwendig. FÃ¼r einen \emph{exakten} Test benÃ¶tigt man die Response Time Analysis
...
eigenen AusfÃ¼hrungszeit $C_i$ und der Interferenz $I_i$ durch alle Tasks mit hÃ¶herer
PrioritÃ¤t. Die Interferenz im Intervall $[0, R_i]$ ist die Summe aller AusfÃ¼hrungen
hochpriorer Tasks, die in dieses Intervall fallen:
\[
	I_i = \sum_{j \in hp(i)} \left\lceil \frac{R_i}{T_j} \right\rceil C_j,
\]
wobei $hp(i) = \{\tau_j \mid P_j > P_i\}$ die Menge der Tasks mit hÃ¶herer PrioritÃ¤t
...
\begin{definition}[Worst-Case Antwortzeit unter RM]
	...
	LÃ¶sung der impliziten Gleichung:
	\begin{equation}
...
geschlossene LÃ¶sung. Die Berechnung erfolgt iterativ.

\subsubsection{Iterativer Berechnungsalgorithmus}

\begin{algorithmic}[1]
	\Require Task $\tau_i$, geordnet nach RM-PrioritÃ¤t
	\State $R_i^{(0)} \leftarrow \sum_{j=1}^{i} C_j$ \Comment{StartschÃ¤tzung: frÃ¼hester Fertigstellungszeitpunkt}
	\Repeat
...
durch $T_i$ nach oben beschrÃ¤nkt. Sie konvergiert daher entweder zu einem Fixpunkt
(der Task schedulierbar) oder Ã¼berschreitet $T_i$ (Task nicht schedulierbar).

\begin{beispiel}[Response Time Analysis -- Schritt fÃ¼r Schritt]
	Gegeben:
	\begin{center}
		\begin{tabular}{cccc}
			\toprule
			Task    & $C_i$ & $T_i$ & PrioritÃ¤t (RM) \\
			\midrule
			$\tau_1$ & 1     & 4     & 1 (hÃ¶chste) \\
			$\tau_2$ & 2     & 6     & 2 \\
	...
	\textbf{Task $\tau_1$} (keine hÃ¶herpriore Task):
...
Konstruktion des Schedules am kritischen Augenblick Ã¼berprÃ¼ft werden.
...
\begin{satz}[Schedulierbarkeit am kritischen Augenblick]
	...
	$\tau_i$ ihre Deadline an ihrem kritischen Augenblick einhÃ¤lt, d.\,h.\ wenn alle
...
\begin{beispiel}[Schedulierbarkeit direkt am kritischen Augenblick]
	...
	\begin{itemize}
		\item $[0,1)$: $\tau_1$ lÃ¤uft (hÃ¶here PrioritÃ¤t, $T_1 < T_2$).
		\item $[1,3)$: $\tau_2$ lÃ¤uft (keine andere Task bereit).
		\item $[3,4)$: $\tau_2$ lÃ¤uft weiter -- aber $\tau_1$ hat neue Instanz bei $t=4$!
...
Lehoczky, Sha und Ding \cite{Lehoczky1989} entwickelten eine prÃ¤zisere Methode
zur Schedulierbarkeitsanalyse unter fester PrioritÃ¤t, die auf dem Konzept der
...
\begin{definition}[Level-$i$-Arbeitslast]
	...
	die im Intervall $(0, t]$ von Task $\tau_i$ und allen Tasks mit hÃ¶herer PrioritÃ¤t
	angefordert wird:
	\[
...
\begin{theorem}[Lehoczky, Sha, Ding 1989]
	Eine Menge vollstÃ¤ndig preemptiver periodischer Tasks ist unter einem
	Algorithmus mit fester PrioritÃ¤t genau dann schedulierbar, wenn:
	\[
		\forall i = 1,\ldots,n: \quad \exists t \in (0, T_i]: \quad W_i(t) \leq t.
	\]
\end{theorem}

\subsection{Warum RM schwach optimal ist -- PrÃ¤zisierung}

\begin{definition}[Schwach optimal -- PrÃ¤zisierung]
	...
	\begin{enumerate}
		\item RM ist optimal \emph{innerhalb} der Klasse aller Algorithmen mit
		      fester PrioritÃ¤tszuweisung.
		\item Es gibt Task-Mengen mit $\ln 2 < U \leq 1$, die \emph{nicht} unter
		      RM schedulierbar sind, aber unter EDF schedulierbar sind.
		\item RM ist \emph{nicht} optimal Ã¼ber alle Scheduling-Algorithmen.
	\end{enumerate}
\end{definition}

\begin{satz}[Grenzen der SchwÃ¤che von RM]
	FÃ¼r jede Auslastung $U$ mit $U_{\text{lub}}(\text{RM},n) < U \leq 1$ existiert
...
\begin{proof}
	WÃ¤hle $\Gamma_U$ gemÃ¤ÃŸ den Worst-Case-Parametern (\ref{eq:worst_case_params})
...
\subsection{VollstÃ¤ndiges Beispiel: Vergleich RM und EDF bei hoher Auslastung}
...
\begin{beispiel}[RM versagt, EDF scheduliert]
	...
	verlÃ¤uft ohne Deadline-Verletzungen, da EDF zu jedem Zeitpunkt die Task mit der
	frÃ¼hesten absoluten Deadline ausfÃ¼hrt und dabei die volle ProzessorkapazitÃ¤t nutzt.
	...
	\end{quote}
	FÃ¼r $\tau_2$ existiert ein machbarer Schedule (unter EDF), aber RM findet ihn nicht.
\end{beispiel}

\subsection{Ãœbungsaufgaben zu Rate Monotonic}

\begin{beispiel}[Ãœbungsaufgabe 1 -- Schedulierbarkeitstest]
	Bestimme, ob folgendes Task-Set unter RM schedulierbar ist:
	\[
		\tau_1: C=2, T=6 \quad \tau_2: C=2, T=8 \quad \tau_3: C=2, T=12.
	\]
	\textit{LÃ¶sung:} $U = 2/6 + 2/8 + 2/12 = 1/3 + 1/4 + 1/6 = 3/4 = 0{,}75$.
	$U_{\text{lub}}(3) = 3(2^{1/3}-1) \approx 0{,}780 > 0{,}75$. Schedulierbar. \checkmark
\end{beispiel}

\begin{beispiel}[Ãœbungsaufgabe 2 -- Nicht schedulierbar]
	Bestimme, ob folgendes Task-Set unter RM schedulierbar ist:
	\[
		\tau_1: C=1, T=4 \quad \tau_2: C=2, T=6 \quad \tau_3: C=3, T=8.
	\]
	\textit{LÃ¶sung:} $U = 1/4 + 2/6 + 3/8 = 0{,}25 + 0{,}333 + 0{,}375 = 0{,}958$.
...
EDF ist eine dynamische Scheduling-Regel: Tasks mit frÃ¼heren absoluten Deadlines
werden zuerst ausgefÃ¼hrt. EDF ist typischerweise preemptiv.

\begin{quote}
\textbf{EDF ist scharf optimal: Wenn ein machbarer Schedule existiert, findet EDF diesen sofort. EDF findet die kÃ¼rzeste AusfÃ¼hrungszeit aller Tasks sofort.}
\end{quote}

\subsection{Schedulierbarkeitsanalyse fÃ¼r EDF}
...
\begin{proof}
	\textbf{``Nur wenn'' Teil:} Wenn $U > 1$, Ã¼berschreitet der Gesamtrechenaufwand
	in der Hyperperiode $H$ die verfÃ¼gbare Zeit $H$. Kein Algorithmus kann diesen
	...
	Deadline versÃ¤umt wird, und $[t_1, t_2]$ das lÃ¤ngste Intervall kontinuierlicher
	Auslastung vor $t_2$, in dem nur Instanzen mit Deadline $\leq t_2$ ausgefÃ¼hrt werden.
	...
	Da eine Deadline versÃ¤umt wird, muss $C_p(t_1, t_2) > (t_2 - t_1)$ gelten,
	also $U > 1$ -- Widerspruch. \hfill$\blacksquare$
\end{proof}

\subsection{Die SchÃ¤rfe von EDF im Vergleich zu RM}

Der fundamentale Unterschied:

\begin{satz}[SchÃ¤rfe von EDF gegenÃ¼ber RM]
	\label{satz:edf_scharf}
	Es gibt Task-Mengen, die EDF schedulieren kann, RM aber nicht. Insbesondere gilt
	fÃ¼r alle $U$ mit $\ln 2 < U \leq 1$:
	\begin{itemize}
...
\begin{beispiel}
	...
	\emph{nicht} garantieren -- und erzeugt tatsÃ¤chlich eine Deadline-Verletzung.
	Da $U \approx 0{,}97 \leq 1$, ist der Task-Satz unter EDF schedulierbar.
	Dies zeigt die SchwÃ¤che von RM und die SchÃ¤rfe von EDF.
\end{beispiel}

\section{Deadline Monotonic Scheduling}

Deadline Monotonic (DM) ist eine Erweiterung von RM fÃ¼r Tasks mit Deadlines
kleiner als ihre Perioden ($D_i \leq T_i$). DM weist PrioritÃ¤ten umgekehrt
proportional zur relativen Deadline zu.

DM ist optimal unter allen Algorithmen mit fester PrioritÃ¤t fÃ¼r Tasks mit
$D_i \leq T_i$ -- analog zu RM ist DM damit ebenfalls \emph{schwach optimal}
im Sinne dieser Klasse.

\section{EDF mit eingeschrÃ¤nkten Deadlines}

FÃ¼r Tasks mit $D_i \leq T_i$ kann die Schedulierbarkeit unter EDF durch das
Processor Demand Criterion (Baruah et al. 1990) getestet werden:

\begin{satz}[Schedulierbarkeit unter EDF mit eingeschrÃ¤nkten Deadlines]
...
\begin{table}[H]
	...
	\begin{tabular}{p{4cm}p{5cm}p{5cm}}
		...
		\midrule
		PrioritÃ¤t            & Fest (umgekehrt proportional zu $T_i$) & Dynamisch (absolutes Deadline) \\
		OptimalitÃ¤t          & \textbf{Schwach optimal} (innerhalb fester PrioritÃ¤t) & \textbf{Scharf optimal} (insgesamt) \\
		$U_{\text{lub}}$     & $\ln 2 \approx 0{,}693$ (worst case) & $1{,}000$ \\
		Schedulierbarkeitstest & Pseudo-polynomiell (RTA) & $O(n)$ (Auslastungstest) \\
		Ãœberlastverhalten    & Niederpriore Tasks blockieren & Periodenskalierung (Cervin) \\
		Impl.-Aufwand & $O(1)$ (Multi-Level Queue) & $O(\log n)$ (Heap) \\
		PrÃ¤emptionsanzahl    & HÃ¶her & Niedriger \\
		\bottomrule
	\end{tabular}
\end{table}

\begin{bemerkung}
	Dass RM schwach optimal ist, bedeutet: Wenn ein Task-Satz Ã¼berhaupt
	mit \emph{irgendeiner} festen PrioritÃ¤tszuweisung schedulierbar ist, so
	ist er auch mit RM schedulierbar. Das schlieÃŸt jedoch nicht aus, dass
	...
	Ã¼berhaupt schedulierbar sind ($U \leq 1$). Es gibt keine Task-Menge
...
\section{EinfÃ¼hrung und Problemstellung}

In realen Echtzeit-Systemen treten stets \emph{hybride Task-Sets} auf: harte
periodische Tasks (Sensor-Abfragen, Regelschleifen) mÃ¼ssen zusammen mit weichen
aperiodischen Tasks (Benutzereingaben, Diagnoseereignisse) eingeplant werden.

\textbf{Das zentrale Ziel} dieses Kapitels: FÃ¼r ein hybrides System sollen
\begin{enumerate}
	...
	\item die Antwortzeiten der weichen aperiodischen Tasks minimiert werden --
	      ohne die Garantien der periodischen Tasks zu gefÃ¤hrden.
\end{enumerate}
...
\textbf{Grundidee Server:} Ein \emph{Server} ist eine kÃ¼nstliche periodische
Task mit Periode $T_s$ und KapazitÃ¤t (Budget) $C_s$, die aperiodische Anfragen
...
Aperiodische Tasks werden nur dann ausgefÃ¼hrt, wenn der Prozessor von keiner
periodischen Task benÃ¶tigt wird. Dies entspricht einer impliziten PrioritÃ¤t
unterhalb aller periodischen Tasks.

\begin{algorithm}[H]
	\caption{Background Scheduling}
	\begin{algorithmic}[1]
		\State Verwalte periodische Tasks in HochprioritÃ¤tswarteschlange (RM)
		\State Verwalte aperiodische Tasks in NiedrigprioritÃ¤tswarteschlange (FCFS)
		\Loop
		\If{periodische Task bereit}
		\State FÃ¼hre hÃ¶chstpriore periodische Task aus (RM)
		\ElsIf{aperiodische Task bereit}
		\State FÃ¼hre nÃ¤chste aperiodische Task aus (FCFS)
		\Else
		\State Leerlauf
		\EndIf
		\EndLoop
...
\subsection{Korrektheit und OptimalitÃ¤tsklassifikation}
...
\begin{proof}
	Aperiodische Tasks werden nur in Leerlaufzeiten ausgefÃ¼hrt. Da sie keine
	periodische Task preemptieren oder verzÃ¶gern kÃ¶nnen, ist der Schedule fÃ¼r
	die periodischen Tasks identisch mit dem RM-Schedule ohne aperiodische Tasks.
	Die RM-Schedulierbarkeitsanalyse bleibt unverÃ¤ndert gÃ¼ltig. \hfill$\blacksquare$
\end{proof}

\textbf{OptimalitÃ¤tsklassifikation:} Background Scheduling ist bezÃ¼glich
aperiodischer Antwortzeiten \emph{nicht optimal}: Bei hoher periodischer Last
$U_p \approx \ln 2$ bleibt nur $1 - \ln 2 \approx 0{,}307$ fÃ¼r aperiodische
Tasks Ã¼brig, und diese Bandbreite wird ineffizient genutzt (Tasks mÃ¼ssen auf
groÃŸe Leerlaufintervalle warten).
...
regelmÃ¤ÃŸig aktiviert wird und beim Aufwachen aperiodische Anfragen bedient
-- aber nur bis zur KapazitÃ¤tsgrenze $C_s$. Wenn keine aperiodischen Anfragen
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State Setze $c_s \leftarrow C_s$ \Comment{Budget auffÃ¼llen}
		\While{$c_s > 0$ \textbf{und} aperiodische Anfrage vorhanden}
		\State Bediene nÃ¤chste aperiodische Anfrage fÃ¼r $\min(C_{\text{ape}}, c_s)$ Einheiten
		\State $c_s \leftarrow c_s - $ verbrauchte Zeit
		\EndWhile
		\State Verwerfe verbleibendes Budget $c_s$; wird erst zur nÃ¤chsten Periode aufgefÃ¼llt
	\end{algorithmic}
\end{algorithm}
...
\begin{proof}
	Der Polling Server verhÃ¤lt sich \emph{exakt} wie eine periodische Task mit
	Periode $T_s$ und WCET $C_s$: UnabhÃ¤ngig von der Anzahl der aperiodischen
...
\subsubsection{Engerer Bound: Hyperbolic Test fÃ¼r PS}
...
Im schlimmsten Fall muss eine aperiodische Anfrage bis zur nÃ¤chsten Serveraktivierung
warten (wenn der Server gerade sein Budget aufgebraucht hat oder keine Anfrage
vorlag). FÃ¼r eine Anfrage $J_a$ mit $C_a \leq C_s$:
\[
	R_a^{\max} \leq 2T_s - \varepsilon + C_a.
\]
FÃ¼r beliebige $C_a$ mit exakt $\lceil C_a/C_s \rceil$ Serverzugriffen:
\[
	R_a \leq T_s + \left\lceil \frac{C_a}{C_s} \right\rceil T_s.
\]

\subsection{OptimalitÃ¤tsklassifikation und Laufzeit}

\textbf{OptimalitÃ¤tsklassifikation:} PS ist \emph{schwach optimal} innerhalb
der Klasse der periodischen Server unter RM: Bei vorgegebenem $(C_s, T_s)$
garantiert PS dieselbe periodische Schedulierbarkeit wie jede Ã¤quivalente
periodische Task. PS ist jedoch \emph{nicht scharf optimal} bezÃ¼glich aperiodischer
...
\textbf{Laufzeit:} $O(\log n)$ fÃ¼r Warteschlangenoperationen (RM-Heap),
$O(1)$ fÃ¼r Budget-Verwaltung pro Aktivierung.

\section{Deferrable Server -- KapazitÃ¤tserhaltung}

\subsection{Informelle Beschreibung}

Der Deferrable Server (DS) verbessert den PS, indem das Budget bei InaktivitÃ¤t
...
das Budget sofort zur VerfÃ¼gung.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $c_s \leftarrow C_s$ \Comment{Budget auf Maximum auffÃ¼llen}
		\State
		\State \textbf{Wenn aperiodische Anfrage $J_a$ ankommt und $c_s > 0$:}
		\State Bediene $J_a$ sofort (wenn keine hÃ¶herpriore Task aktiv)
		\State $c_s \leftarrow c_s - $ verbrauchte Zeit
		\State
		\State \textbf{Wenn Server aktiviert und $c_s > 0$, aber keine Anfrage:}
		\State Behalte Budget $c_s$ (nicht verwerfen!)
		\State Server schlÃ¤ft bis zur nÃ¤chsten Anfrage oder nÃ¤chsten Periode
	\end{algorithmic}
\end{algorithm}

\subsection{Warum DS KEINE Ã¤quivalente periodische Task ist}

Der DS verletzt Annahme A5 (Tasks suspendieren sich nicht), da er sich
suspendiert, obwohl er der hÃ¶chstpriore Task wÃ¤re. Dies hat eine \emph{niedrigere}
Schedulierbarkeitsschranke zur Folge.

\begin{satz}[DS Schedulierbarkeitsschranke -- Strosnider, Lehoczky, Sha]
	\label{satz:ds}
	Die Schedulierbarkeitsschranke fÃ¼r $n$ periodische Tasks mit einem
	Deferrable Server mit Auslastung $U_s$ ist:
	\[
		U_p \leq n \left(\left(\frac{U_s+2}{2U_s+1}\right)^{1/n} - 1\right).
	\]
	Das Minimum Ã¼ber alle $U_s$ ergibt $U_p^* \approx 0{,}652$ bei $U_s^* \approx 0{,}186$.
\end{satz}

\begin{proof}
	Im schlimmsten Fall fÃ¼hrt der DS dreimal innerhalb einer Periode $T_1$ der
	hÃ¶chstprioren periodischen Task aus: am Ende seiner eigenen Periode (aufgehoben)
	und zu Beginn der nÃ¤chsten Periode. Daraus folgt die Worst-Case-Parameterisierung:
	\[
		C_s = \frac{T_1 - T_s}{2}, \quad C_1 = T_2 - T_1, \ldots, \quad
		C_n = \frac{3T_s + T_1 - 2T_n}{2}.
	\]
	Minimierung der Auslastung Ã¼ber $R_i = T_{i+1}/T_i$ analog zum RM-Beweis
	(Abschnitt~\ref{satz:rm_lub}) ergibt:
	\[
		K = \frac{U_s + 2}{2U_s + 1}, \qquad U_{\text{lub}} = U_s + n(K^{1/n} - 1).
	\]
	Da DS die periodischen Tasks stÃ¤rker beeintrÃ¤chtigt als PS (wegen der
	KapazitÃ¤tserhaltung), gilt $U_{\text{lub}}^{DS} \leq U_{\text{lub}}^{PS}$
	fÃ¼r $U_s < 0{,}4$. \hfill$\blacksquare$
\end{proof}

\subsubsection{Vergleich PS vs.\ DS}

FÃ¼r $U_p = 0{,}6$: $U_s^{\max}(\text{PS}) = 2/P - 1$ und $U_s^{\max}(\text{DS}) = (2-P)/(2P-1)$
...
\textbf{OptimalitÃ¤tsklassifikation:} DS ist \emph{nicht scharf optimal}
bezÃ¼glich aperiodischer Antwortzeiten, aber besser als PS. Die niedrigere
Schedulierbarkeitsschranke gegenÃ¼ber PS ist der Preis fÃ¼r bessere aperiodische
ResponsivitÃ¤t.
...
gegenÃ¼ber DS, indem Budget nicht einfach erhalten, sondern \emph{getauscht} wird:
Wenn keine aperiodischen Anfragen vorliegen, tauscht der Server seine AusfÃ¼hrungszeit
gegen die Laufzeit der hÃ¶chstprioren aktiven periodischen Task. Das Budget wird
dabei auf das PrioritÃ¤tsniveau der getauschten Task degradiert.

\begin{algorithm}[H]
	\caption{Priority Exchange Server}
	\begin{algorithmic}[1]
		\State \textbf{Zu Beginn der Serverperiode:} $c_s \leftarrow C_s$ bei ServerprioritÃ¤t
		\State
		\State \textbf{Wenn Server hÃ¶chste PrioritÃ¤t hat und $c_s > 0$:}
		\If{aperiodische Anfrage vorhanden}
		\State Bediene Anfrage fÃ¼r $\min(C_{\text{ape}}, c_s)$ Einheiten
		\ElsIf{periodische Task $\tau_k$ aktiv}
		\State FÃ¼hre $\tau_k$ fÃ¼r 1 Einheit aus (Tausch)
...
\begin{proof}
	Im schlimmsten Fall verhÃ¤lt sich PE wie eine periodische Task mit Periode $T_s$
	und AusfÃ¼hrungszeit $C_s$: Die Tauschoperationen verschieben Rechenzeit nur
	zwischen PrioritÃ¤tsniveaus, Ã¤ndern aber nicht die GesamtausfÃ¼hrungszeit. Das
...
\subsection{OptimalitÃ¤tsklassifikation und Vergleich}

PE ist damit gegenÃ¼ber DS \emph{besser schedulierbar}, aber auf Kosten
komplexerer Implementierung. FÃ¼r eine gegebene periodische Last $U_p$:
\begin{itemize}
	\item PE erlaubt mehr Serverbandbreite als DS.
	\item Die aperiodischen Antwortzeiten von PE sind vergleichbar mit DS.
\end{itemize}

\textbf{Laufzeit:} $O(\log n)$ pro Ereignis, zusÃ¤tzlich $O(1)$ fÃ¼r Tauschoperationen.
...
Der Sporadic Server (SS), von Sprunt, Sha und Lehoczky, hat die SchlÃ¼sseleigenschaft:
\emph{Das verbrauchte Budget wird erst $T_s$ Zeiteinheiten nach dem Verbrauch
wiederhergestellt}. Dadurch verhÃ¤lt sich SS -- im Gegensatz zu DS -- exakt wie
eine normale periodische Task in Bezug auf die Schedulierbarkeitsschranke.

\textbf{Replenishment-Regeln:}
\begin{itemize}
	\item \textbf{WiederauffÃ¼llzeitpunkt} RT: Wird gesetzt, wenn SS aktiv wird
	      ($P_{\text{exe}} \geq P_s$) und $c_s > 0$. Sei $t_A$ dieser Zeitpunkt:
	      $\mathrm{RT} = t_A + T_s$.
	\item \textbf{WiederauffÃ¼llmenge} RA: Wird gesetzt, wenn SS idle wird ($P_{\text{exe}} < P_s$)
	      oder $c_s = 0$. Sei $t_I$ dieser Zeitpunkt:
	      $\mathrm{RA} = $ verbrauchte KapazitÃ¤t in $[t_A, t_I]$.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State Plane WiederauffÃ¼llung RA zu Zeitpunkt RT
		\State RT $\leftarrow \infty$ \Comment{NÃ¤chster Aktivierungszeitpunkt wird neu gesetzt}
		\State
		\State \textbf{Zum geplanten WiederauffÃ¼llzeitpunkt RT:}
		\State $c_s \leftarrow c_s + \mathrm{RA}$
	\end{algorithmic}
\end{algorithm}

\subsection{Hauptsatz: SS verhÃ¤lt sich wie eine periodische Task}

\begin{theorem}[Sprunt, Sha, Lehoczky -- SS Ã„quivalenz]
	\label{thm:ss}
...
\begin{proof}
	Wir analysieren das Verhalten von SS in den drei mÃ¶glichen FÃ¤llen (Abbildung
	nach Buttazzo \cite{Buttazzo2011}):

	\textbf{Fall 1 -- Kein Verbrauch in $[t_A, t_I]$:}
	In diesem Intervall ist keine aperiodische Anfrage aktiv. SS behÃ¤lt sein Budget,
	aber keine WiederauffÃ¼llung findet vor $t_I + T_s$ statt. Das bedeutet: In
	$[t_A, t_I + T_s]$ kann SS maximal $C_s$ Zeiteinheiten fÃ¼r aperiodische Tasks
	verbrauchen -- identisch mit einer periodischen Task $\tau_s$ mit verzÃ¶gertem
	Release-Zeitpunkt $t_I$ statt $t_A$. Da verzÃ¶gerte Releases die Antwortzeiten
	anderer Tasks nicht verschlechtern, bleibt Schedulierbarkeit erhalten.

	\textbf{Fall 2 -- VollstÃ¤ndiger Verbrauch:}
	$c_s$ wird in $[t_A, t_I]$ vollstÃ¤ndig verbraucht ($\mathrm{RA} = C_s$).
	WiederauffÃ¼llung bei $t_A + T_s$. SS verhÃ¤lt sich exakt wie $\tau_s$ mit
	Release-Zeit $t_A$.

	\textbf{Fall 3 -- Teilweiser Verbrauch:}
	$\mathrm{RA} = C_R$ (verbrauchter Teil) wird bei $t_A + T_s$ wiederaufgefÃ¼llt;
	verbleibendes Budget $C_s - C_R$ bleibt erhalten. SS verhÃ¤lt sich wie zwei
	periodische Tasks $\tau_x(C_R, T_s)$ und $\tau_y(C_s - C_R, T_s)$, wobei
	$\tau_x$ bei $t_A$ und $\tau_y$ bei $t_I$ released wird. Da $\tau_y$ verzÃ¶gert
	ist, schadet es der Schedulierbarkeit nicht (analog zu Fall 1).

	In allen FÃ¤llen ist der Beitrag von SS zur Auslastung $U_s = C_s/T_s$, genau
...
\begin{satz}[SS Schedulierbarkeitsschranke]
	...
	\[
		\prod_{i=1}^n (U_i + 1) \leq \frac{2}{U_s + 1}.
	\]
	Die maximale Serverauslastung, die die Schedulierbarkeit erhÃ¤lt, ist:
	\[
...
\begin{proof}
	...
	(Theorem~\ref{thm:hyperbolic}) fÃ¼r $n+1$ Tasks lautet $\prod_{i=1}^{n+1}(U_i+1) \leq 2$.
	Einsetzen von $U_{n+1} = U_s$ liefert die Behauptung. \hfill$\blacksquare$
\end{proof}

\subsection{OptimalitÃ¤tsklassifikation und Laufzeit}

\textbf{OptimalitÃ¤tsklassifikation:} SS ist \emph{schwach optimal} fÃ¼r den
Klasse fester PrioritÃ¤tsserver: Es gibt keinen Server unter RM, der eine
hÃ¶here Schedulierbarkeitsschranke \emph{und} bessere aperiodische Antwortzeiten
gleichzeitig bietet (Theorem~\ref{thm:nooptserver}).

\textbf{Laufzeit:} Jede Ereignisbehandlung (Serveraktivierung, Idle-Wechsel,
WiederauffÃ¼llung) erfordert $O(1)$ Operationen. Pro aperiodischer Anfrage
entstehen hÃ¶chstens $\lceil C_a/C_s \rceil$ WiederauffÃ¼llungen, daher
Gesamtoverhead pro Anfrage $O(\lceil C_a/C_s \rceil)$.

\section{Slack Stealing -- Maximale ResponsivitÃ¤t}

\subsection{Informelle Beschreibung}

Der Slack Stealing Algorithmus (Lehoczky \& Ramos-Thuel 1992) nutzt nicht
eine feste Serverbandbreite, sondern ``stiehlt'' alle verfÃ¼gbaren Slack-Zeiten
der periodischen Tasks fÃ¼r aperiodische Anfragen.

\textbf{Grundidee:} Wenn $\tau_i$ eine Slack-Zeit $\text{slack}_i(t) > 0$ hat,
bedeutet das, dass sie nicht \emph{sofort} ausgefÃ¼hrt werden muss. Der
Slack Stealer kann diese Zeit fÃ¼r aperiodische Anfragen nutzen, ohne die
Deadline von $\tau_i$ zu gefÃ¤hrden.

\begin{algorithm}[H]
	\caption{Slack Stealing (vereinfachte Darstellung)}
	\begin{algorithmic}[1]
		\State Berechne offline $A(0, t)$ = Slack-Funktion fÃ¼r alle $t \in [0, H]$
		\State
		\State \textbf{Wenn aperiodische Anfrage $J_a$ bei $t=r_a$ ankommt:}
		\State Berechne aktuellen Slack $A(r_a, t')$ fÃ¼r $t' > r_a$
		\State Finde frÃ¼hestes $t'$ mit $A(r_a, t') \geq C_a$
		\State Plane $J_a$ so frÃ¼h wie mÃ¶glich innerhalb verfÃ¼gbarer Slack-Zeiten
		\State
...
\begin{definition}[Slack-Funktion $A(s,t)$]
	...
	\]
	$A(s, \cdot)$ ist eine nichtfallende Treppenfunktion mit SprÃ¼ngen an den
...
\begin{satz}[Slack Stealing -- Schedulierbarkeit]
	...
	gefÃ¤hrdet Slack Stealing die Schedulierbarkeit nie: Aperiodische Tasks werden
	ausschlieÃŸlich in Zeiten eingeplant, in denen kein periodischer Task laufen muss.
\end{satz}

\begin{proof}
	Per Konstruktion: $A(s, t)$ gibt exakt die Zeitdauer an, die fÃ¼r periodische
	Tasks \emph{nicht} benÃ¶tigt wird. Jede aperiodische Zuteilung innerhalb von
	$A(s, t)$ lÃ¤sst mindestens genug Prozessorzeit fÃ¼r alle periodischen Tasks
...
\begin{theorem}[Tia, Liu, Shankar 1995 -- Kein optimaler Server]
	...
	FÃ¼r jeden festen PrioritÃ¤tsscheduler und jede Ankunftsdisziplin fÃ¼r
...
\begin{proof}
	...
	\begin{itemize}
		\item Option A: $J_1$ sofort (bei $t=2$) einplanen $\Rightarrow$ $J_1$ fertig bei $t=3$, minimale Antwortzeit fÃ¼r $J_1$.
		\item Option B: $J_1$ auf $t=3$ verschieben $\Rightarrow$ $J_1$ fertig bei $t=4$.
	\end{itemize}

	\textbf{Ankunft $J_2$ bei $t=3$ ($C=1$):}
	\begin{itemize}
		\item Bei Option A (A sofort): Kein Slack in $[3,6]$; $J_2$ frÃ¼hestens bei $t=7$.
...
hat KomplexitÃ¤t $O(N)$ mit $N$ = Anzahl Deadlines in der Hyperperiode. Die
Online-Aktualisierung bei jeder aperiodischen Ankunft hat KomplexitÃ¤t $O(n)$.

\section{Dimensionierung von Servern}

\begin{satz}[Optimale Server-Dimensionierung]
	FÃ¼r ein gegebenes periodisches Task-Set mit Hyperbolic-Produkt $P = \prod_i(U_i+1)$
	...
	$T_s = T_1$ (hÃ¶chste PrioritÃ¤t), mit $C_s = U_s \cdot T_s$.
\end{satz}
...
\begin{table}[H]
	\centering
	\caption{Vergleich der Fixed-Priority Server -- VollstÃ¤ndige Ãœbersicht}
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{lllll}
		\toprule
		...
		Sporadic (SS) & VerzÃ¶g.\ AuffÃ¼llen & \textbf{Wie PS} & Gut & $O(\log n)$ \\
	...
	Slack Stealing minimiert Antwortzeiten soweit mÃ¶glich.}
\end{table}

%=============================================================
\chapter{Dynamic Priority Server -- Dynamische PrioritÃ¤ts-Server unter EDF}
\label{chap:dynserver}
%=============================================================

\section{EinfÃ¼hrung und Motivation}

GegenÃ¼ber Fixed-Priority-Servern aus Kapitel~\ref{chap:server} bieten dynamische
PrioritÃ¤ts-Server einen entscheidenden Vorteil: Da EDF den Prozessor bis $U=1$
auslasten kann, steht fÃ¼r aperiodische Tasks eine weit grÃ¶ÃŸere Bandbreite zur
VerfÃ¼gung. FÃ¼r eine Menge periodischer Tasks mit Auslastung $U_p$ verbleiben
$(1-U_p)$ fÃ¼r aperiodische Server -- gegenÃ¼ber deutlich weniger unter RM.

\begin{bemerkung}[Vorteil dynamischer Server gegenÃ¼ber RM-basierten Servern]
	...
	\end{itemize}
	EDF erlaubt damit mehr als doppelt so viel Bandbreite fÃ¼r aperiodische Tasks.
\end{bemerkung}
...
\begin{itemize}
	...
	\item Aperiodische Tasks $J_i$ haben keine Deadlines, sollen aber so frÃ¼h wie
	      mÃ¶glich fertiggestellt werden.
	\item Alle periodischen Tasks starten synchron bei $t=0$.
	\item Die AusfÃ¼hrungszeit jeder aperiodischen Task ist bekannt.
\end{itemize}
...
\cite{Spuri1996}, Ã¼bertrÃ¤gt das Prinzip des Fixed-Priority-Exchange-Servers auf
EDF: Der Server tauscht seine Laufzeit mit niederprioren periodischen Tasks
(d.\,h.\ solchen mit spÃ¤terer Deadline), anstatt sie zu verbrauchen. Dadurch
bleibt KapazitÃ¤t erhalten, auch wenn keine aperiodischen Anfragen vorliegen.

\textbf{Informelle Beschreibung:}

\begin{itemize}
	\item Der DPE-Server hat eine Periode $T_s$ und KapazitÃ¤t $C_s$.
	\item Zu Beginn jeder Serverperiode mit Deadline $d$ wird die aperiodische
	      KapazitÃ¤t $C_S^d \leftarrow C_s$ gesetzt.
	\item Wenn die hÃ¶chste PrioritÃ¤t im System eine aperiodische KapazitÃ¤t $C$
	      hat, wird entweder eine aperiodische Task (falls vorhanden) oder die
	      periodische Task mit kÃ¼rzester Deadline ausgefÃ¼hrt. Im letzteren Fall
	      wird deren Laufzeit in eine aperiodische KapazitÃ¤t auf dem PrioritÃ¤tsniveau
	      der periodischen Task Ã¼bertragen (Tausch).
	\item Leerlauf verbraucht die KapazitÃ¤t bis auf null.
\end{itemize}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State
		\State \textbf{Bei AusfÃ¼hrung der hÃ¶chsten PrioritÃ¤t (aperiodische KapazitÃ¤t $C$):}
		\If{aperiodische Anfrage $J$ vorhanden}
		\State Bediene $J$ bis Fertigstellung oder KapazitÃ¤t $C$ erschÃ¶pft
		\ElsIf{periodische Task $\tau_k$ mit frÃ¼hester Deadline bereit}
		\State FÃ¼hre $\tau_k$ fÃ¼r 1 Zeiteinheit aus
		\State $C_S^{d_k} \leftarrow C_S^{d_k} + 1$; $C \leftarrow C - 1$
		\Comment{Tausch: KapazitÃ¤t auf Niveau $d_k$ Ã¼bertragen}
		\Else
...
\begin{proof}
	\textbf{Konstruktion des Ã¤quivalenten Schedules $\sigma'$:}
	...
	\begin{itemize}
		\item Ersetze den DPE-Server durch eine periodische Task $\tau_s$
		      mit Periode $T_s$ und WCET $C_s$: In $\sigma'$ lÃ¤uft $\tau_s$ genau dann,
		      wenn in $\sigma$ ServerkapazitÃ¤t verbraucht wird.
	...
	EDF-Schedule fÃ¼r die Menge $\{\tau_1,\ldots,\tau_n,\tau_s\}$ mit
	Gesamtauslastung $U_p + U_s$ -- unabhÃ¤ngig von aperiodischen Anfragen.

	\textbf{Ã„quivalenz $\sigma \leftrightarrow \sigma'$:} In $\sigma$ sind die
	...
	Da $\sigma'$ ein reines EDF-Schedule Ã¼ber periodische Tasks mit Auslastung
...
\begin{satz}[DPE Laufzeit]
	Die Laufzeit von DPE pro Ereignis (Ankunft, Fertigstellung, KapazitÃ¤tserschÃ¶pfung)
	betrÃ¤gt $O(\log n)$ fÃ¼r die Verwaltung der EDF-PrioritÃ¤tswarteschlange als Heap.
...
\subsection{OptimalitÃ¤tsklassifikation von DPE}

DPE ist \textbf{nicht scharf optimal} bezÃ¼glich der Antwortzeit aperiodischer Tasks:
Es gibt Szenarien, in denen ein clairvoyanter Algorithmus bessere Antwortzeiten
liefern wÃ¼rde. DPE ist jedoch \textbf{schwach optimal} in dem Sinne, dass fÃ¼r jeden
Auslastungswert $U_p + U_s \leq 1$ die Schedulierbarkeit aller periodischen Tasks
garantiert wird -- genau wie ein Ã¤quivalenter rein-periodischer EDF-Schedule.
...
erweitert den Sporadic Server auf dynamische PrioritÃ¤ten. Die KapazitÃ¤t $C_s$ wird
nicht zu festen Zeiten aufgefÃ¼llt, sondern nach dem Verbrauch.

\textbf{Deadline- und WiederauffÃ¼llungsregeln:}
...
\begin{itemize}
	...
	\item Wenn der Server idle wird (Anfrage abgeschlossen oder $C_s = 0$)
	      zum Zeitpunkt $t_I$: Plane WiederauffÃ¼llung $\mathrm{RA} = \text{(verbrauchte KapazitÃ¤t in }[t_A, t_I])$ zu Zeitpunkt $\mathrm{RT}$.
\end{itemize}

\begin{algorithm}[H]
	\caption{DSS Server -- KapazitÃ¤tsverwaltung}
	\begin{algorithmic}[1]
		...
		\State Bediene $J$ mit Deadline $d_s$; decrementiere $c_s$ bei AusfÃ¼hrung
		\State
		\State \textbf{Wenn Server idle wird (Zeitpunkt $t_I$):}
		\State $\mathrm{RA} \leftarrow$ verbrauchte KapazitÃ¤t in $[t_A, t_I]$
		\State Plane WiederauffÃ¼llung $c_s \mathrel{+}= \mathrm{RA}$ bei Zeitpunkt $\mathrm{RT}$
	\end{algorithmic}
\end{algorithm}

\subsection{Schedulierbarkeitsnachweis}

\begin{lemma}[DSS verhÃ¤lt sich wie eine periodische Task]
...
\begin{proof}
	Da die WiederauffÃ¼llungsmenge stets gleich der verbrauchten Menge ist und
	das WiederauffÃ¼llen frÃ¼hestens $T_s$ nach dem Aktivierungszeitpunkt erfolgt,
	kann in jedem $T_s$-langen Intervall maximal $C_s$ Zeit fÃ¼r aperiodische
...
\begin{proof}
	...
	es gibt einen Ãœberlauf bei $t$. Dann gibt es ein $t' < t$ mit kontinuierlicher
	Auslastung. FÃ¼r die Gesamtanforderung $C$ in $[t', t]$ gilt nach
	...
	$U_s$ verhÃ¤lt, folgt aus dem EDF-Schedulierbarkeitstheorem direkt $U_p + U_s \leq 1$. \hfill$\blacksquare$
\end{proof}

\subsection{Laufzeitanalyse und OptimalitÃ¤tsklassifikation}

\begin{satz}[DSS Laufzeit]
	Jede Ereignisbehandlung (Ankunft, Fertigstellung, WiederauffÃ¼llung) erfordert
	$O(\log n)$ fÃ¼r Heap-Operationen. Pro aperiodischer Anfrage entstehen maximal
	$\lceil C_a/C_s \rceil$ WiederauffÃ¼llungen, sodass der Gesamtoverhead
	pro Anfrage in $O(\lceil C_a/C_s \rceil \cdot \log n)$ liegt.
\end{satz}

DSS ist wie DPE \textbf{nicht scharf optimal} bezÃ¼glich Antwortzeiten, aber
...
$r_k$ mit AusfÃ¼hrungszeit $C_k$ ankommt:
\[
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State FÃ¼ge $(C_k, d_k)$ in EDF-Warteschlange ein
		\State EDF-Schedule lÃ¤uft weiter
	\end{algorithmic}
\end{algorithm}
...
\subsection{Korrektheitsbeweis: AuslastungsbeschrÃ¤nkung}

\begin{lemma}[TBS Auslastungsschranke]
	\label{lem:tbs_util}
	In jedem Intervall $[t_1, t_2]$ ist die von TBS bediente AusfÃ¼hrungszeit
	$C_{\mathrm{ape}}$ durch $(t_2 - t_1) \cdot U_s$ nach oben beschrÃ¤nkt:
	\[
...
\begin{proof}
	...
	Ãœberlauf bei $t$. Sei $[t', t]$ das lÃ¤ngste Intervall kontinuierlicher Auslastung
	vor $t$, in dem nur Tasks mit Deadline $\leq t$ ausgefÃ¼hrt werden. Die Gesamtanforderung:
	\[
	...
	und AusfÃ¼hrungszeit $C_s = T_s U_s$ ankommen, verhÃ¤lt sich TBS wie eine
	periodische Task. Dann folgt aus EDF-Schedulierbarkeit $U_p + U_s \leq 1$. \hfill$\blacksquare$
\end{proof}

\subsection{OptimalitÃ¤tsklassifikation von TBS}
...
eine Erweiterung TB* (Total Bandwidth, optimiert), die durch iterative Deadline-VerkÃ¼rzung die minimale Antwortzeit annÃ¤hert.

\begin{satz}[TB* OptimalitÃ¤t -- Buttazzo \& Sensini]
...
fÃ¼r EDF-Heap-EinfÃ¼gung. Gesamtoverhead vernachlÃ¤ssigbar.
...
d.\,h.\ eine Task kann den Server nicht Ã¼ber seine zugewiesene Bandbreite $U_s$
hinaus belasten -- selbst wenn sie mehr Rechenzeit beansprucht als erwartet.
Dies unterscheidet CBS grundlegend von TBS, der die Ãœberschreitung von WCETs
nicht abfangen kann.
...
\begin{definition}[CBS-Parameter und Zustandsvariablen]
	...
	\begin{itemize}
		...
		\item $k$: ZÃ¤hler der erzeugten Chunks
	\end{itemize}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $n \leftarrow n+1$; fÃ¼ge $J_j$ in Serverwarteschlange ein
		...
		\If{$n \neq 0$} starte nÃ¤chsten Job mit Deadline $d_{s,k}$ \EndIf
		\State
		\State \textbf{Bei jeder AusfÃ¼hrungseinheit ($c = c - 1$):}
		\If{$c = 0$} \Comment{Regel 3: Budget erschÃ¶pft}
...
\begin{theorem}[CBS Isolationseigenschaft -- Abeni \& Buttazzo]
	...
	Die CPU-Auslastung eines CBS $S$ mit Parametern $(Q_s, T_s)$ betrÃ¤gt
	exakt $U_s = Q_s/T_s$, unabhÃ¤ngig von AusfÃ¼hrungszeiten und Ankunftsmustern.
\end{theorem}

\begin{proof}
	Durch Regel 3 wird das Budget $c_s$ bei jedem Verbrauch sofort auf $Q_s$
	aufgefÃ¼llt und die Serverdeadline um $T_s$ nach hinten verschoben. In jedem
	Intervall der LÃ¤nge $T_s$ verbraucht der CBS genau $Q_s$ Zeiteinheiten fÃ¼r
	ServerausfÃ¼hrung. Die AusfÃ¼hrungsrate ist damit konstant $Q_s/T_s = U_s$.
	...
	Budget verbraucht (sonst wÃ¼rde Regel 3 nicht auslÃ¶sen). Damit ist die
	Bandbreite exakt $Q_s/T_s$ fÃ¼r jeden Beobachtungszeitraum. \hfill$\blacksquare$
\end{proof}
...
\begin{proof}
	...
	periodische Task mit Auslastung $U_{s,i}$ verhÃ¤lt. Die Gesamtauslastung
	betrÃ¤gt $U_p + \sum_i U_{s,i} = U_p + U_s$, und EDF-Schedulierbarkeit
	gilt genau dann, wenn dieser Wert $\leq 1$. \hfill$\blacksquare$
\end{proof}

\subsection{Worst-Case-Antwortzeit und optimale CBS-Parameter}

Die worst-case Antwortzeit $R_i$ eines Jobs mit AusfÃ¼hrungszeit $C_i$ unter CBS mit
Budget $Q_s$ und Periode $T_s$ betrÃ¤gt:
\[
	R_i = C_i + \left\lfloor \frac{C_i}{Q_s} \right\rfloor (T_s - Q_s).
\]

Das Budget $Q_s$, das die mittlere Antwortzeit minimiert (unter BerÃ¼cksichtigung
...
\begin{table}[H]
	\centering
	\caption{Vergleich dynamischer PrioritÃ¤ts-Server}
	\label{tab:dynserver}
	\begin{tabular}{p{2.5cm}p{2cm}p{2cm}p{2cm}p{3cm}}
		...
		DSS  & $U_p+U_s\leq 1$ & Gut & Mittel & Dynamische WiederbefÃ¼llung \\
...
(EDF-Variante, in der Tasks so spÃ¤t wie mÃ¶glich eingeplant werden) fÃ¼r aperiodische
Anfragen.

\begin{theorem}[Chetto \& Chetto -- EDL OptimalitÃ¤t]
	FÃ¼r jede Online-Algorithmus $A$ und jeden aperiodischen Job $J_i$ gilt:
	\[
...
\begin{proof}[Beweisidee]
	...
	fÃ¼r aperiodische Tasks stets die meisten FreirÃ¤ume zur VerfÃ¼gung. Jeder andere
	Algorithmus kann somit hÃ¶chstens dieselben FreirÃ¤ume nutzen. \hfill$\blacksquare$
\end{proof}

\begin{satz}[EDL-Server Laufzeit]
	Die Berechnung der Leerlaufzeiten unter EDL hat KomplexitÃ¤t $O(N \cdot n)$,
	wobei $n$ die Anzahl der periodischen Tasks und $N$ die Anzahl der
	Instanzen in der Hyperperiode ist. Im schlimmsten Fall kann $N$ sehr groÃŸ
	sein (bei nicht-harmonischen Perioden), was EDL fÃ¼r viele praktische Systeme
	ungeeignet macht.
\end{satz}
...
\begin{theorem}[Tia, Liu, Shankar 1995]
	FÃ¼r jeden Task-Set mit fester PrioritÃ¤tszuweisung und jede aperiodische
...
\begin{proof}[Beweis durch Gegenbeispiel]
	...
	\begin{itemize}
		\item Option 1: FÃ¼hre $J_1$ sofort aus. Wenn dann $J_2$ bei $t=3$ ankommt,
		      ist kein Slack bis $t=6$ verfÃ¼gbar; $J_2$ terminiert frÃ¼hestens bei $t=7$.
...
\begin{bemerkung}
	Dieses fundamentale Ergebnis zeigt, dass die in diesem Kapitel prÃ¤sentierten
	Server keine scharf optimalen LÃ¶sungen sind: Es gibt kein ``perfektes'' Server-
	Verfahren. Die vorgestellten Algorithmen sind stattdessen Kompromisse zwischen
	Antwortzeit, Overhead, Speicherbedarf und ImplementierungskomplexitÃ¤t.
\end{bemerkung}

%=============================================================
\chapter{Ressourcenzugriffsprotokolle -- PrioritÃ¤tsinversion und LÃ¶sungsstrategien}
\label{chap:ressourcen}
%=============================================================

\section{Das PrioritÃ¤tsinversionsproblem}

\begin{definition}[Kritischer Abschnitt]
	Ein \textbf{kritischer Abschnitt} ist ein Codebereich, der unter gegenseitiger
	Ausschluss-Bedingung ausgefÃ¼hrt wird. Nur eine Task darf einen kritischen
	Abschnitt, der eine Ressource $R_k$ schÃ¼tzt, gleichzeitig betreten.
	Gesichert durch Semaphor $S_k$ mit \texttt{wait}($S_k$) beim Eintritt und
	\texttt{signal}($S_k$) beim Austritt.
\end{definition}

\begin{definition}[PrioritÃ¤tsinversion]
	\textbf{PrioritÃ¤tsinversion} tritt auf, wenn eine hochpriore Task $\tau_H$
	blockiert ist, wÃ¤hrend eine niederpriore Task $\tau_M$ (ohne gemeinsame
	Ressource mit $\tau_H$) ausgefÃ¼hrt wird. Die Blockierzeit ist dann nicht
	auf die Dauer des kritischen Abschnitts begrenzt, sondern hÃ¤ngt von der
	gesamten AusfÃ¼hrungszeit aller Tasks mit $P_M < P < P_H$ ab.
\end{definition}

\begin{beispiel}[Klassische PrioritÃ¤tsinversion -- 3 Tasks]
	$\tau_1 > \tau_2 > \tau_3$ in PrioritÃ¤t. $\tau_1$ und $\tau_3$ teilen Ressource $R$.
	\begin{enumerate}
		\item $t_0$: $\tau_3$ startet, betritt kritischen Abschnitt auf $R$.
		\item $t_1$: $\tau_2$ kommt, preemptiert $\tau_3$ (hÃ¶here PrioritÃ¤t).
		\item $t_2$: $\tau_1$ kommt, preemptiert $\tau_2$ (hÃ¶here PrioritÃ¤t).
		\item $t_3$: $\tau_1$ versucht $R$ zu betreten -- blockiert (von $\tau_3$ gehalten).
		\item $t_4$: $\tau_2$ lÃ¤uft weiter und preemptiert $\tau_3$ -- $\tau_1$ wartet!
	\end{enumerate}
	$\tau_1$ wartet also nicht auf $\tau_3$'s kritischen Abschnitt, sondern auf die
	\emph{gesamte} AusfÃ¼hrungszeit von $\tau_2$. Im schlimmsten Fall unbeschrÃ¤nkt.
\end{beispiel}

\textbf{Terminologie:}
\begin{itemize}
	\item $B_i$: maximale Blockierzeit von $\tau_i$
	\item $\delta_{j,k}$: Dauer des lÃ¤ngsten kritischen Abschnitts von $\tau_j$ auf $S_k$
	\item $C(S_k) = \max\{P_i \mid \tau_i$ benutzt $S_k\}$: PrioritÃ¤tsgrenze von $S_k$
	\item $\gamma_i$: Menge der kritischen Abschnitte, die $\tau_i$ blockieren kÃ¶nnen
\end{itemize}
...
\begin{definition}[NPP]
	Bei \textbf{NPP} wird die PrioritÃ¤t einer Task beim Eintritt in jeden
	kritischen Abschnitt auf das systemweite Maximum angehoben:
	\[
		p_i(R_k) = \max_h \{P_h\}.
	\]
	Bei Verlassen des kritischen Abschnitts wird $p_i$ auf $P_i$ zurÃ¼ckgesetzt.
\end{definition}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $p_i \leftarrow \max_h\{P_h\}$ \Comment{PrioritÃ¤t auf Maximum}
		\State Belege Ressource $R_k$
		\State
		\State \textbf{signal($S_k$):}
		\State Gib $R_k$ frei
		\State $p_i \leftarrow P_i$ \Comment{Nominal-PrioritÃ¤t wiederherstellen}
	\end{algorithmic}
\end{algorithm}

\subsection{Blockierzeit unter NPP}

Da unter NPP nie zwei Tasks gleichzeitig in kritischen Abschnitten sein kÃ¶nnen
(die PrioritÃ¤tsanhebung verhindert Preemption), ist die maximale Blockierzeit:
\[
	B_i = \max\{\delta_{j,k} - 1 \mid P_j < P_i,\; k=1,\ldots,m\}.
\]

\textbf{Nachteil:} NPP verursacht \emph{unnÃ¶tige} Blockierung: Wenn $\tau_1$ keine
...
\begin{definition}[PIP -- Sha, Rajkumar, Lehoczky 1990]
	...
	Ressource hÃ¤lt, vorÃ¼bergehend die hÃ¶chste PrioritÃ¤t aller Tasks, die auf diese
	Ressource warten:
	\[
	...
	und $\tau_2$ von $\tau_1$, erbt $\tau_3$ die PrioritÃ¤t von $\tau_1$.
\end{definition}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $p_j \leftarrow \max(p_j, P_i)$ \Comment{PrioritÃ¤tsvererbung}
		\State Setze $\tau_j$ fort mit neuer PrioritÃ¤t
		\Else
		\State $\tau_i$ belegt $R_k$
		\EndIf
		\State
		\State \textbf{signal($S_k$):}
		\State Gib $R_k$ frei; wecke hÃ¶chstpriore wartende Task
		\If{keine Tasks mehr von $\tau_j$ blockiert}
		\State $p_j \leftarrow P_j$ \Comment{Nominal-PrioritÃ¤t}
		\Else
...
\begin{lemma}
	...
	auslÃ¶sen, wenn $S_k$ sowohl von einer Task mit $P < P_i$ als auch von einer
	Task mit $P > P_i$ benutzt wird.
\end{lemma}

\begin{lemma}
	Transitive PrioritÃ¤tsvererbung tritt nur bei \emph{geschachtelten}
	kritischen Abschnitten auf.
\end{lemma}

\begin{theorem}[Sha, Rajkumar, Lehoczky -- PIP Blockierzeit]
	\label{thm:pip}
	Unter PIP kann $\tau_i$ fÃ¼r hÃ¶chstens $\alpha_i = \min(l_i, s_i)$ kritische
...
\begin{proof}
	...
	ist. Sobald $\tau_j$ diesen verlÃ¤sst, kann er $\tau_i$ nicht mehr blockieren.
	Pro niederpriorer Task also maximal ein Blockiervorgang.

	\textbf{Bound durch $s_i$ (Lemma~7.4 in Buttazzo):} Da Semaphore binÃ¤r sind,
...
\begin{algorithmic}[1]
	\Require Tasks geordnet nach absteigender PrioritÃ¤t; keine Schachtelung
...
\begin{definition}[PCP -- Sha, Rajkumar, Lehoczky 1990]
	Jeder Ressource $R_k$ wird eine \textbf{PrioritÃ¤tsgrenze} (Priority Ceiling)
	zugewiesen:
	\[
		C(R_k) = \max\{P_i \mid \tau_i \text{ benutzt } R_k\}.
	\]
	Eine Task $\tau_i$ darf $R_k$ nur betreten, wenn ihre aktive PrioritÃ¤t
	\emph{strikt hÃ¶her} ist als die PrioritÃ¤tsgrenze aller aktuell gesperrten
	Ressourcen:
	\[
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Comment{PrioritÃ¤t auf Ceiling}
		\Else
		\State $\tau_i$ wird blockiert (von Task mit hÃ¶chster Ceiling-PrioritÃ¤t)
		\EndIf
		\State
		\State \textbf{signal($S_k$):}
		\State Gib $R_k$ frei; $p_i \leftarrow P_i$ \Comment{Nominal-PrioritÃ¤t}
		\State Wecke hÃ¶chstpriore wartende Task
	\end{algorithmic}
\end{algorithm}
...
\begin{theorem}[PCP Eigenschaften]
	...
	Unter PCP gilt:
	\begin{enumerate}
		\item Jede Task $\tau_i$ wird hÃ¶chstens einmal blockiert.
		\item Deadlocks sind unmÃ¶glich.
		\item Gegenseitiger Ausschluss ist gewÃ¤hrleistet.
	\end{enumerate}
\end{theorem}
...
\begin{proof}
	...
	Damit beide gleichzeitig ausgefÃ¼hrt werden kÃ¶nnten, mÃ¼sste eine von ihnen die
	...
	PrioritÃ¤t als $\tau_i$), fÃ¼hrt das zu einem Widerspruch.

	\textbf{Zu 2 (Deadlock-Freiheit):} Ein Deadlock erfordert zyklisches Warten.
	Nach PCP kann $\tau_j$ $R_k$ nur belegen, wenn $p_j > C(R_k)$. FÃ¼r einen Zyklus
	$\tau_a \to \tau_b \to \tau_a$ mÃ¼sste $P_a > C(R_b) \geq P_b > C(R_a) \geq P_a$
...
geprÃ¼ft werden (Sha et al.):
\[
	R_i = C_i + B_i + \sum_{j \in hp(i)} \left\lceil \frac{R_i}{T_j} \right\rceil C_j,
\]
wobei $B_i = \max\{\delta_{j,k}-1 \mid P_j < P_i,\; C(R_k) \geq P_i\}$ die maximale
Blockierzeit ist. Da $\tau_i$ hÃ¶chstens einmal blockiert wird, ist $B_i$ durch einen
einzigen kritischen Abschnitt beschrÃ¤nkt.
...
\begin{definition}[Preemptionsgrenze (Preemption Level)]
	...
	$\pi_i$ zugewiesen (unter RM gleich der PrioritÃ¤t). Jeder Ressource $R_k$ wird
...
\begin{definition}[SRP-Zugriffsregel]
	...
	$\tau_i$ darf $R_k$ betreten, ohne zusÃ¤tzliche PrÃ¼fung -- die Zugangskontrolle
...
\begin{satz}[SRP Eigenschaften]
	Unter SRP gilt:
	\begin{itemize}
		\item Jede Task wird hÃ¶chstens \emph{einmal} blockiert (bei der Aktivierung,
		      nicht beim Ressourcenzugriff).
		\item Stack-Sharing ist mÃ¶glich: Alle Tasks kÃ¶nnen denselben Stack nutzen,
		      da niemals zwei Tasks gleichzeitig im kritischen Abschnitt sein kÃ¶nnen.
		\item SRP ist sowohl fÃ¼r feste als auch fÃ¼r dynamische PrioritÃ¤ten anwendbar
		      (OSEK, AUTOSAR, POSIX-Ã¤hnliche Systeme).
	\end{itemize}
\end{satz}
...
\begin{proof}[Beweis der einmaligen Blockierung unter SRP]
	...
	werden. Damit ist genau eine Blockierphase mÃ¶glich, nÃ¤mlich vor der ersten
	AusfÃ¼hrung. \hfill$\blacksquare$
\end{proof}
...
\begin{table}[H]
	...
	\begin{tabular}{p{2.0cm}p{1.9cm}p{1.9cm}p{2.0cm}p{3.0cm}}
		...
		\midrule
		NPP    & 1 Abschnitt & Ja & Ja & UnnÃ¶tige Blockierung \\
		...
		SRP    & 1 Abschnitt & Ja & \textbf{Ja} & Optimal fÃ¼r Einzel-CPU \\
		\bottomrule
	\end{tabular}
	}
\end{table}
...
VollstÃ¤ndig preemptives Scheduling maximiert die Schedulierbarkeit, verursacht aber
erheblichen Overhead: Cache-Misses durch zerstÃ¶rte LokalitÃ¤t, Pipeline-Flushes,
...
\begin{definition}[Preemptionskosten-Komponenten]
	...
	\begin{enumerate}
		...
		\item \textbf{Cache-Kosten (CRPD)}: Cache-Related Preemption Delay --
		      Reload der Cache-Lines, die die preemptierende Task verdrÃ¤ngt hat.
		\item \textbf{Bus-Kosten}: ZusÃ¤tzliche RAM-Zugriffe durch Cache-Misses.
	\end{enumerate}
	Empirisch: Auf PowerPC MPC7410 mit 2 MB L2-Cache kann CRPD bis zu 33\,\%
	der nicht-preemptiven WCET betragen \cite{Buttazzo2011}.
\end{definition}

Zentrales Ergebnis dieses Kapitels: FÃ¼r ein prÃ¤emptiv machbares Task-Set lÃ¤sst sich
stets ein Preemptionsschema finden, das die Schedulierbarkeit erhÃ¤lt und gleichzeitig
...
Ohne Preemption ergibt sich fÃ¼r jede Task $\tau_i$ eine Blockierzeit durch die
lÃ¤ngste niederpriore Task:
\begin{equation}
...
\begin{proof}
	...
	$\tau_2$ ($C_2 = T_1 + \varepsilon = 3\varepsilon$, $T_2 = M$ fÃ¼r groÃŸes $M$).
	Auslastung: $U = \varepsilon/(2\varepsilon) + 3\varepsilon/M = 0{,}5 + 3\varepsilon/M$,
	beliebig nah an 0{,}5 fÃ¼r groÃŸes $M$. Aber: Nicht-preemptiv kann $\tau_2$
	nicht unterbrochen werden, also $\tau_1$ muss $C_2 = 3\varepsilon > T_1 = 2\varepsilon$
	warten -- Deadline-Verletzung. \hfill$\blacksquare$
\end{proof}

\subsection{Machbarkeitsanalyse fÃ¼r nicht-preemptives Scheduling}

Das \emph{Self-Pushing-PhÃ¤nomen} (Bril et al.\ 2009) zeigt, dass unter
nicht-preemptivem Scheduling die worst-case Antwortzeit nicht beim ersten Job
(kritischer Augenblick), sondern bei einem spÃ¤teren Job auftreten kann. Daher
muss die Analyse innerhalb der lÃ¤ngsten Level-$i$-AktivitÃ¤tsperiode $L_i$
...
FÃ¼r alle Jobs $k \in [1, K_i]$ mit $K_i = \lfloor L_i/T_i \rfloor$ wird der Startzeitpunkt
...
\begin{definition}[Preemption Threshold -- Wang \& Saksena 1999]
	Jede Task $\tau_i$ erhÃ¤lt zwei PrioritÃ¤tsniveaus:
	\begin{itemize}
		\item Nominale PrioritÃ¤t $P_i$ (fÃ¼r die Ready-Queue und Einplanung),
		\item Preemption Threshold $\theta_i \geq P_i$ (fÃ¼r die AusfÃ¼hrung).
	\end{itemize}
	$\tau_h$ kann $\tau_i$ \emph{wÃ¤hrend der AusfÃ¼hrung} nur preemptieren, wenn
	$P_h > \theta_i$.
\end{definition}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For{$i \leftarrow n$ \textbf{downto} $1$} \Comment{Von niedrigster zu hÃ¶chster PrioritÃ¤t}
...
\]
Antwortzeit (innerhalb der lÃ¤ngsten Level-$i$-AktivitÃ¤tsperiode):
\begin{equation}
...
\begin{satz}[OptimalitÃ¤t des Threshold-Algorithmus]
...
\subsection{OptimalitÃ¤tsklassifikation}

Preemption Thresholds sind \textbf{nicht scharf optimal}: Es gibt Task-Sets, die
weder vollstÃ¤ndig preemptiv noch vollstÃ¤ndig nicht-preemptiv, aber mit geeigneten
Thresholds schedulierbar sind. Innerhalb der Klasse der Threshold-Scheduler ist der
Algorithmus \textbf{schwach optimal} (findet LÃ¶sung, wenn eine existiert).
...
\begin{definition}[Maximales nicht-preemptives Intervall]
	...
	wird beim Eintreffen einer hÃ¶herprioren Task ein Timer auf $q_i$ gesetzt.
\end{definition}

Blockierzeit: $B_i = \max_{j:\,P_j < P_i}\{q_j - 1\}$.

\subsection{LÃ¤ngste machbare nicht-preemptive Intervalle}
...
\begin{theorem}[Yao, Buttazzo, Bertogna -- LÃ¤ngste nicht-preemptive Intervalle]
	Die lÃ¤ngsten nicht-preemptiven Intervalle $Q_i$, die die Schedulierbarkeit
	erhalten, kÃ¶nnen rekursiv berechnet werden:
	\[
...
\begin{proof}
	$Q_i$ ist das minimale $\beta_k + 1$ Ã¼ber alle Tasks mit hÃ¶herer PrioritÃ¤t als
...
\begin{algorithm}[H]
	\caption{Berechnung der lÃ¤ngsten nicht-preemptiven Intervalle}
	\begin{algorithmic}[1]
...
fÃ¼r die Berechnung von $\beta_i$. Im ungÃ¼nstigsten Fall ist $|\mathit{TS}_i|$
pseudopolynomiell (abhÃ¤ngig von den Perioden).
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Require Task $\tau_i$ mit BasisblÃ¶cken $\delta_1,\ldots,\delta_N$, maximales NPR $Q$
		...
		\EndFor
		\State Rekonstruiere EPPs rÃ¼ckwÃ¤rts von $\delta_{\mathrm{prev}}(\delta_N)$
		\State \Return FEASIBLE
	\end{algorithmic}
\end{algorithm}

Dabei ist die Bedingung fÃ¼r nicht-preemptive Regionen:
\begin{equation}
	\xi_{j-1} + \sum_{\ell=j}^{k} b_\ell \leq Q.
	\label{eq:npr_cond}
\end{equation}

\begin{theorem}[Bertogna et al.\ 2011 -- OptimalitÃ¤t von SelectEPP]
	...
	LÃ¤nge $Q$ nicht Ã¼berschreiten.
\end{theorem}

\begin{proof}
	Der Algorithmus implementiert eine dynamische Programmierung Ã¼ber die
	BasisblÃ¶cke. FÃ¼r jedes $\delta_k$ berechnet er das global optimale $\hat{C}_k$
	(kleinste kumulative WCET der ersten $k$ BlÃ¶cke). Da $\hat{C}_k$ das Minimum
	Ã¼ber alle machbaren VorgÃ¤nger $\delta_j$ bildet, werden alle mÃ¶glichen
	machbaren EPP-Platzierungen fÃ¼r die ersten $k$ BlÃ¶cke berÃ¼cksichtigt.
	Durch Induktion Ã¼ber $k$ ist $\hat{C}_N = C_i^{\min}$ die minimale WCET
...
\begin{satz}[Laufzeit SelectEPP]
	SelectEPP hat eine Laufzeit von $O(N^2)$, wobei $N$ die Anzahl der BasisblÃ¶cke
	ist. FÃ¼r jeden der $N$ BasisblÃ¶cke wird Ã¼ber die Menge $\mathit{Prev}_k$ (hÃ¶chstens
	$N$ EintrÃ¤ge) iteriert. Der Gesamtaufwand fÃ¼r das gesamte Task-Set mit $n$
	Tasks ist $O(\sum_i N_i^2)$.
\end{satz}

\section{Vergleich der AnsÃ¤tze}
...
\begin{table}[H]
	...
	\begin{tabular}{p{3cm}p{2cm}p{2.5cm}p{2.5cm}p{2.5cm}}
		\toprule
		\textbf{Methode} & \textbf{Impl.-Aufwand} & \textbf{Vorhersage\-barkeit} & \textbf{EffektivitÃ¤t} & \textbf{OptimalitÃ¤t} \\
		\midrule
...
%=============================================================
\chapter{Ãœberlastbehandlung -- Scheduling unter Ãœberlast}
\label{chap:uberlast}
%=============================================================

\section{EinfÃ¼hrung: Arten und Ursachen der Ãœberlast}

Ãœberlast ($U > 1$) tritt auf, wenn die Gesamtrechenanforderung die verfÃ¼gbare
ProzessorkapazitÃ¤t Ã¼berschreitet. Ursachen sind:
\begin{itemize}
	\item \textbf{Schlechtes Systemdesign}: Keine Worst-Case-Analyse durchgefÃ¼hrt.
	\item \textbf{Gleichzeitige Ereignisse}: Peak-Load-Szenarien Ã¼berschÃ¤tzt.
	\item \textbf{Hardwarefehler}: Anomale Interrupt-Sequenzen sÃ¤ttigen CPU.
	\item \textbf{UmgebungsverÃ¤nderungen}: Unvorhergesehene Lastspitzen.
\end{itemize}
...
\begin{definition}[Instantane Last]
	...
	\]
	wobei $c_k(t)$ die verbleibende AusfÃ¼hrungszeit von Job $J_k$ ist.
	Ein System ist Ã¼berlastet, wenn $\rho(t) > 1$.
\end{definition}

\section{Aperiodische Ãœberlast -- Value-Based Scheduling}

Bei aperiodischer Ãœberlast ist es nicht mÃ¶glich, alle Deadlines einzuhalten.
...
Unter Ãœberlast versucht der \textbf{Robust Earliest Deadline (RED)} Algorithmus,
den Gesamtwert zu maximieren, indem Tasks mit niedrigem VerhÃ¤ltnis $v_i/C_i$
abgelehnt werden (Value Density):

\begin{algorithm}[H]
	\caption{Value-Based Scheduling unter Ãœberlast (vereinfacht)}
	\begin{algorithmic}[1]
...
\section{Ãœberlastbehandlung unter RM vs. EDF}

\subsection{Permanente Ãœberlast unter RM}

Unter RM mit fester PrioritÃ¤t bei $U > 1$:
\begin{itemize}
	\item \textbf{Alle niederprioren Tasks werden vollstÃ¤ndig blockiert.}
	\item Tasks mit $P_i < P_j$ fÃ¼r alle $j$ erhalten keinen Prozessor mehr.
	\item Das System degeneriert: Nur hochpriore Tasks laufen; alle anderen
	      verpassen ihre Deadlines.
\end{itemize}

\subsection{Permanente Ãœberlast unter EDF -- Cervin-Theorem}

\begin{theorem}[Cervin 2003 -- EDF bei permanenter Ãœberlast]
	\label{thm:cervin}
	Bei $n$ periodischen Tasks mit $U > 1$ unter EDF gilt im eingeschwungenen
	Zustand: Die effektive mittlere Periode jeder Task $\tau_i$ betrÃ¤gt
	\[
		\bar{T}_i = T_i \cdot U.
	\]
	EDF fÃ¼hrt also eine \textbf{automatische Periodenskalierung} durch.
\end{theorem}

\begin{proof}[Beweisidee]
	In einem EDF-Schedule wird jede Task mit relativer HÃ¤ufigkeit $C_i/T_i$ ausgefÃ¼hrt
	(proportional zu ihrer Auslastung). Bei $U > 1$ wird die verfÃ¼gbare Zeit $1$ auf
	die Tasks im VerhÃ¤ltnis ihrer Auslastungen aufgeteilt. Task $\tau_i$ erhÃ¤lt den
	Anteil $U_i/U = C_i/(T_i U)$ der Prozessorzeit, was einer effektiven Rate von
	$1/\bar{T}_i = 1/(T_i U)$ entspricht. \hfill$\blacksquare$
\end{proof}

\begin{satz}[Vergleich RM vs. EDF bei Ãœberlast]
	\begin{itemize}
		\item \textbf{RM}: Niederpriore Tasks werden vollstÃ¤ndig blockiert;
		      hochpriore Tasks laufen normal. Ungerecht bei Ãœberlast.
		\item \textbf{EDF}: Alle Tasks skalieren gleichmÃ¤ÃŸig; keine Task wird
		      vollstÃ¤ndig blockiert. Fairer Umgang mit Ãœberlast.
	\end{itemize}
	Dies ist ein weiterer Vorteil von EDF gegenÃ¼ber RM, zusÃ¤tzlich zur hÃ¶heren
	Auslastungsschranke.
\end{satz}
...
\begin{definition}[Elastic Task]
	...
	$[T_i^{\min}, T_i^{\max}]$ und einen ElastizitÃ¤tskoeffizienten $e_i \geq 0$.
	Bei Ãœberlast kann die Periode erhÃ¶ht (und damit die Auslastung reduziert) werden,
	mit einem ``Kostenfaktor'' proportional zu $e_i$.
\end{definition}

\begin{satz}[Elastic Scheduling Feasibility]
	Ein elastisches Task-Set ist unter EDF schedulierbar, wenn fÃ¼r die optimalen
	Perioden $T_i^*$ gilt:
	\[
		\sum_{i=1}^n \frac{C_i}{T_i^*} \leq 1, \quad T_i^{\min} \leq T_i^* \leq T_i^{\max}.
	\]
	Die optimalen Perioden kÃ¶nnen durch einen iterativen Algorithmus in $O(n^2)$
	berechnet werden.
\end{satz}

\section{$(\pi, \lambda)$-Algorithmen fÃ¼r harte Ãœberlast}

FÃ¼r Systeme, in denen nicht alle Tasks weich sind, werden \textbf{Overload Manager}
eingesetzt, die Entscheidungen Ã¼ber Akzeptanz oder Ablehnung neuer Tasks treffen.
...
\begin{satz}[GÃ¼te von EDF-basiertem Overload Management]
	EDF maximiert unter allen Online-Algorithmen bei harter Ãœberlast den
...
Dieses Kapitel prÃ¤sentiert \textbf{EDF$^+$} (Earliest Deadline First Plus),
...
Der fundamentale Kernsatz gilt auch fÃ¼r EDF$^+$:

\begin{quote}
\textbf{Wenn es einen machbaren Schedule gibt, findet EDF diesen sofort.
Die kÃ¼rzeste AusfÃ¼hrungszeit aller Tasks findet EDF sofort. EDF ist scharf optimal.}
\end{quote}

EDF$^+$ erweitert diese scharfe OptimalitÃ¤t auf Systeme mit stochastischen
Blocking-Ereignissen. WÃ¤hrend klassisches EDF bei Blocking-Ereignissen seine
OptimalitÃ¤t verlieren kann, stellt EDF$^+$ sicher, dass nach jedem
Blocking-Ereignis der optimale EDF-Schedule fÃ¼r die verbleibenden Tasks
berechnet wird.
...
\begin{definition}[Geometrische Verteilung der Blockings]
	Die Anzahl der AusfÃ¼hrungsversuche bis zur erfolgreichen AusfÃ¼hrung von $T_i$
...
\textbf{Phase 2: AusfÃ¼hrung} -- Tasks werden in Reihenfolge $\sigma^{(k)}$ ausgefÃ¼hrt.
...
\section{OptimalitÃ¤tsbeweis fÃ¼r EDF$^+$}

\subsection{Fundament: EDF-OptimalitÃ¤tstheorem}

Das gesamte OptimalitÃ¤tsargument fÃ¼r EDF$^+$ beruht auf dem in Kapitel~\ref{chap:aperiodisch}
bewiesenen EDF-OptimalitÃ¤tstheorem (Satz~\ref{thm:edf_optimal}):

\begin{satz}[EDF-OptimalitÃ¤tstheorem -- Wiederholung]
	\label{satz:edf_opt_wieder}
	Sei $\Sigma$ ein prÃ¤emptives Single-Processor-Task-System mit $U \leq 1$ und
	Blocking-Wahrscheinlichkeit $p = 0$. Dann findet EDF stets einen gÃ¼ltigen Schedule:
	\[
...
\begin{proof}
	...
	$D_i^{\text{eff}} = D_i$ fÃ¼r alle $i$. Das System erfÃ¼llt $U \leq 1$ und $p = 0$.
	...
	ausschlieÃŸt. Die verbleibende Task-Menge
	$\mathcal{T}' = \{T_j \in \mathcal{T} \mid D_j^{\text{eff}} < +\infty\}$
	hat Auslastung:
	\[
		U' = U - \frac{C_i}{P_i} < U \leq 1.
	\]
	FÃ¼r alle $T_j \in \mathcal{T}'$ gilt zum Zeitpunkt $t^*$: $p_j = 0$.
	Damit erfÃ¼llt $\mathcal{T}'$ die Voraussetzungen von Satz~\ref{satz:edf_opt_wieder}:
	\[
	...
	Per vollstÃ¤ndiger Induktion Ã¼ber $R$: Beim $R$-ten Blocking von $T_{i_R}$
	...
	optimal. Die Gesamtaussage folgt durch vollstÃ¤ndige Induktion. \hfill$\blacksquare$
\end{proof}
...
\begin{proof}
	RMS ist nach Liu \& Layland \cite{Liu1973} nur fÃ¼r $U \leq \ln 2$
	deadline-safe. FÃ¼r $U > \ln 2$ kÃ¶nnen RMS-Schedules Deadlines verfehlen.
...
\begin{satz}[Laufzeit von EDF$^+$]
	Die Laufzeit von EDF$^+$ fÃ¼r $N = 40$ Tasks mit Blocking-Wahrscheinlichkeit $p$:
	\begin{itemize}
		\item \textbf{Initialisierung:} $\Theta(n \log n)$ durch Merge Sort.
		\item \textbf{Pro Blocking-Ereignis:} $\Theta(n \log n)$ fÃ¼r Neuplanung.
...
\begin{table}[H]
	\centering
	\caption{Erwartete Neuplanungen fÃ¼r EDF$^+$ bei $N=40$ Tasks}
...
\section{EDF$^+$ fÃ¼r dynamische Task-Mengen}

Die Erweiterung auf dynamisch wachsende Task-Mengen ist mÃ¶glich. Bei Ankunft
einer neuen Task $T_{n+1}$ zum Zeitpunkt $a_{n+1}$ wird die laufende AusfÃ¼hrung
von $T_{\text{cur}}$ preemptiert, falls $D_{n+1} < D_{\text{cur}}$, und der
Schedule fÃ¼r die erweiterte Menge neu berechnet.

\begin{satz}[OptimalitÃ¤t von EDF$^+$ fÃ¼r dynamische Task-Mengen]
	EDF$^+$ ist optimal fÃ¼r dynamische Task-Mengen mit stochastischen
	Blocking-Ereignissen und dynamischen Task-AnkÃ¼nften. Bei jedem
	Ankunftsereignis oder Blocking-Ereignis erzeugt EDF$^+$ eine Teilmenge
	aktiver Tasks, die die Voraussetzungen des EDF-OptimalitÃ¤tstheorems
	(Satz~\ref{satz:edf_opt_wieder}) erfÃ¼llt.
\end{satz}
...
\begin{itemize}
	...
	\item \textbf{ISO~26262 (ASIL-D):} Der analytisch begrenzte WCET-Overhead
	      durch Neuplanungen ermÃ¶glicht eine zertifizierbare Implementierung.
	\item \textbf{Vorteil gegenÃ¼ber RM:} Bei Auslastungen $U > \ln 2 \approx 0{,}69$
	      (typisch in modernen ECUs) Ã¼bertrifft EDF$^+$ RM systematisch.
\end{itemize}

%=============================================================
\chapter{Kernel-Design-Aspekte fÃ¼r Echtzeit-Systeme}
\label{chap:kernel}
%=============================================================
...
\begin{definition}[Kernelprimitiven fÃ¼r Echtzeit]
	Ein Echtzeit-Kernel muss folgende Primitive mit \emph{beschrÃ¤nkter AusfÃ¼hrungszeit}
	bereitstellen:
	\begin{itemize}
		...
		\item \textbf{delay}($\tau$, $\delta$): Task fÃ¼r $\delta$ Zeiteinheiten suspendieren.
		\item \textbf{wait}($S$): Semaphorzugriff mit PrioritÃ¤tsprotokoll.
		\item \textbf{signal}($S$): Semaphor freigeben mit PrioritÃ¤tsreset.
	\end{itemize}
	Alle Primitive mÃ¼ssen \emph{preemptierbar} sein und dÃ¼rfen keine unbeschrÃ¤nkten
	Schleifen enthalten.
\end{definition}

\subsection{ProzesszustÃ¤nde in einem Echtzeit-Kernel}
...
\begin{figure}[H]
	...
	\begin{tabular}{|c|l|}
		...
		\hline
		RUNNING   & Task wird aktuell ausgefÃ¼hrt \\
		READY     & Task bereit, wartet auf CPU-Zuteilung \\
		BLOCKED   & Task wartet auf Ressource oder Ereignis \\
		SUSPENDED & Task deaktiviert (bis zum nÃ¤chsten Aktivierungszeitpunkt) \\
		IDLE      & Leerlauf-Task (niedrigste PrioritÃ¤t) \\
		\hline
	\end{tabular}
	\caption{ProzesszustÃ¤nde in einem Echtzeit-Kernel}
\end{figure}

\subsection{Warteschlangen-Implementierung}

\begin{satz}[KomplexitÃ¤t der Scheduling-Warteschlange]
	\begin{itemize}
		\item \textbf{Feste PrioritÃ¤ten (RM)}: Multi-Level-Queue mit $P$ PrioritÃ¤tsstufen.
		      EinfÃ¼gen und Entnehmen: $O(1)$.
		\item \textbf{Dynamische PrioritÃ¤ten (EDF)}: Heap (balancierter BinÃ¤rbaum).
		      EinfÃ¼gen und Entnehmen: $O(\log n)$.
	\end{itemize}
	FÃ¼r kleine Task-Sets ($n < 100$) ist der $O(\log n)$-Overhead von EDF
	vernachlÃ¤ssigbar. RM bietet nur bei sehr groÃŸen Task-Sets ($n > 1000$) einen
	wesentlichen Vorteil.
\end{satz}

\section{Interrupt-Behandlung in Echtzeit-Systemen}

\begin{satz}[Interrupt-Ansatz C -- Empfohlene Methode fÃ¼r Echtzeit]
	Der beste Ansatz fÃ¼r Echtzeit-Systeme: Interrupts aktivieren eine dedizierte
	\emph{Task} zur GerÃ¤tebehandlung, anstatt das GerÃ¤t direkt im Interrupt-Handler
	...
	\begin{algorithmic}[1]
		...
		\State return
	\end{algorithmic}
	Die GerÃ¤tebehandlungs-Task $J_E$ wird dann vom Scheduler mit normaler PrioritÃ¤t
	verwaltet und kann von hÃ¶herprioren Tasks preemptiert werden. Damit ist die
	AusfÃ¼hrungszeit von Interrupt-Handlern vollstÃ¤ndig in die Schedulierbarkeitsanalyse
	integrierbar.
\end{satz}
...
da Page Faults zu unvorhersagbaren VerzÃ¶gerungen fÃ¼hren. Empfohlene Alternativen:
\begin{itemize}
	\item \textbf{Statische Speicherpartitionierung}: Feste Segmente fÃ¼r jede Task,
	      keine VerdrÃ¤ngung. Vorhersagbar, aber weniger flexibel.
...
In der Praxis mÃ¼ssen Kernel-Overheads in der Schedulierbarkeitsanalyse berÃ¼cksichtigt
...
Anzahl von Interrupts wÃ¤hrend der AusfÃ¼hrung von $\tau_i$ sind.

%=============================================================
\chapter{Anwendungsdesign fÃ¼r Echtzeit-Systeme}
...
\begin{definition}[Nyquist-Rate fÃ¼r Regelungsanwendungen]
	FÃ¼r eine Regelschleife mit Systemdynamik bis zur Frequenz $f_{\max}$ (Hz) muss
	...
	empfohlen, um Implementierungstoleranzen zu berÃ¼cksichtigen.
\end{definition}
...
\begin{definition}[Hierarchische Dekomposition]
	...
	\begin{enumerate}
		\item \textbf{Sensor-Ebene}: Datenerfassung, hÃ¶chste Frequenz ($T \approx 1\,\text{ms}$).
		\item \textbf{Filter-/Vorverarbeitungs-Ebene}: RauschunterdrÃ¼ckung, Kalman-Filter.
		...
		\item \textbf{Ãœberwachungs-Ebene}: Zustandsanzeige, Logging ($T \approx 1\,\text{s}$).
	\end{enumerate}
\end{definition}

\section{Robotersteuerungsbeispiel -- VollstÃ¤ndige Analyse}
...
\begin{table}[H]
	...
	\begin{tabular}{lccccl}
		...
		$\tau_5$ Monitor    & 2{,}0  & 100  & 0{,}020 & 1 & Ãœberwachung \\
		\midrule
		$\Sigma$ & & & 0{,}820 & & \\
		\bottomrule
	\end{tabular}
...
Analog fÃ¼r $\tau_3, \tau_4, \tau_5$: Alle scheduleierbar unter RM.

Da $U = 0{,}82 \leq 1$: Das System ist auch unter EDF schedulierbar -- mit mehr
Spielraum fÃ¼r aperiodische Tasks (TBS mit $U_s = 1-0{,}82 = 0{,}18$ mÃ¶glich).

\section{AUTOSAR-Anwendungsbeispiel}

In AUTOSAR Classic wird EDF Ã¼blicherweise nicht direkt verwendet; stattdessen
nutzt AUTOSAR feste PrioritÃ¤ten (RM-basiert). Mit EDF$^+$ (Kapitel~\ref{chap:edfplus})
kann jedoch ein optimales Scheduling auch mit Blocking-Ereignissen erreicht werden.

\begin{table}[H]
	\centering
	\caption{AUTOSAR-Task-Konfiguration fÃ¼r ein MotorsteuergerÃ¤t (ECU)}
	\begin{tabular}{lccl}
		\toprule
		\textbf{Task (OSEK-Kategorie)} & $C_i$ ($\mu$s) & $T_i$ (ms) & \textbf{Funktion} \\
		\midrule
		PreemptTask10ms & 200  & 10   & Einspritzregelung (ASIL-D) \\
		PreemptTask20ms & 500  & 20   & ZÃ¼ndzeitpunkt (ASIL-C) \\
...
komfortabler Sicherheitspuffer fÃ¼r Spitzenlasten und aperiodische Diagnose-Tasks.
...
\section{Schwache vs.\ Scharfe OptimalitÃ¤t -- GesamtÃ¼berblick}

\begin{definition}[Schwache OptimalitÃ¤t von RM]
	RM ist \textbf{schwach optimal}: optimal innerhalb fester PrioritÃ¤ten,
	$U_{\text{lub}} = \ln 2 \approx 0{,}693$ im worst case.
\end{definition}

\begin{definition}[Scharfe OptimalitÃ¤t von EDF]
	EDF ist \textbf{scharf optimal}: $U_{\text{lub}} = 1$.
	EDF scheduliert \emph{jedes} Task-Set mit $U \leq 1$.
\end{definition}

\section{EDF$^+$ -- Erweiterung der SchÃ¤rfe auf Blocking-Szenarien}

EDF$^+$ \cite{Epp2026} Ã¼bertrÃ¤gt die scharfe OptimalitÃ¤t von EDF auf Systeme
mit stochastischen Blocking-Ereignissen. Bei jedem Blocking erzeugt EDF$^+$ eine
Teilmenge nicht-blockierter Tasks, auf die das EDF-OptimalitÃ¤tstheorem anwendbar ist.
EDF$^+$ dominiert RM fÃ¼r alle $U > \ln 2$ und minimiert die erwartete Anzahl
verpasster Deadlines unter allen neuplanenden Algorithmen.

\section{Praktische Empfehlungen}

\begin{enumerate}
	\item \textbf{Verwende EDF statt RM} wenn $U > 0{,}693$ und dynamische PrioritÃ¤ten
	      akzeptabel sind.
	\item \textbf{Verwende EDF$^+$ statt EDF} wenn Blocking-Ereignisse mÃ¶glich sind.
	...
	\item \textbf{PrÃ¼fe Schedulierbarkeit offline} (RTA fÃ¼r RM, Auslastungstest fÃ¼r EDF).
	\item \textbf{Verwende CBS} bei variablen AusfÃ¼hrungszeiten und temporaler Isolation.
\end{enumerate}

%=============================================================
\section{Ausblick: Offene Forschungsfragen und ZukÃ¼nftige Entwicklungen}
%=============================================================

\subsection{Mehrprozessor-Scheduling -- Das grÃ¶ÃŸte offene Problem}

Das fundamentalste ungelÃ¶ste Problem der Echtzeit-Theorie ist die Erweiterung
...
\begin{definition}[Globales Scheduling]
	Beim \textbf{globalen Scheduling} kÃ¶nnen Tasks auf jedem der $m$ Prozessoren
	ausgefÃ¼hrt werden. Vorteil: maximale FlexibilitÃ¤t. Nachteil: Task-Migrationen
	und Cache-Invalidierungen erhÃ¶hen den Overhead erheblich.
\end{definition}
...
\begin{definition}[Partitioniertes Scheduling]
	...
	Auf jedem Prozessor lÃ¤uft ein unabhÃ¤ngiger Uniprocessor-Scheduler.
\end{definition}

Vielversprechende AnsÃ¤tze: \emph{Semi-Partitioned Scheduling} (bestimmte Tasks
dÃ¼rfen auf zwei Prozessoren migrieren) und \emph{Clustered Scheduling} (Gruppen
...
\begin{definition}[Pfair-Scheduling (Baruah et al.\ 1996)]
	...
	Mehrprozessorsystemen: Jede Task $\tau_i$ erhÃ¤lt in jedem Zeitquantum
...
pro Zeitquantum) macht Pfair fÃ¼r praktische Systeme kaum einsetzbar.
...
RM-basiert fÃ¼r Lo-Mode, Wechsel zu modifizierten Deadlines im Hi-Mode.
Bekannte Grenzen: Keine Garantien fÃ¼r niedrigkritische Tasks im Hi-Mode;
ModusÃ¼bergÃ¤nge kÃ¶nnen Deadline-Verletzungen bei mittelkritischen Tasks
verursachen.
...
hochkritischen Tasks virtuelle (kÃ¼rzere) Deadlines zu. Scharf optimal fÃ¼r
zwei KritikalitÃ¤tsstufen auf Uniprocessoren. Erweiterungen auf $k > 2$ Stufen
und Mehrprozessoren sind aktive Forschungsthemen.

\textbf{Offene Frage:} Gibt es einen universell optimalen MCS-Algorithmus,
der fÃ¼r alle Systeme mit $U^{\mathrm{LO}} \leq 1$ und $U^{\mathrm{HI}} \leq 1$
machbar ist?
...
Echtzeit-Garantien. Dennoch gibt es vielversprechende AnsÃ¤tze:

\subsubsection{ML-basierte WCET-SchÃ¤tzung}

\begin{itemize}
	\item \textbf{Extreme Value Theory (EVT)}: Statistische Analyse seltener
	      AusfÃ¼hrungspfade fÃ¼r probabilistische WCET-Schranken (pWCET).
	      ErmÃ¶glicht engere WCET-SchÃ¤tzungen als statische Analyse.
	\item \textbf{Random Forest / Gradient Boosting}: SchÃ¤tzen WCET aus
	      Programm-Features (Schleifenanzahl, Cachestruktur, AbhÃ¤ngigkeitstiefe).
	\item \textbf{Messdaten-Regression}: Nutzung historischer AusfÃ¼hrungsdaten
	      zur Verbesserung der Analysegenauigkeit bei seltenen Pfaden.
\end{itemize}

\subsubsection{Reinforcement Learning fÃ¼r adaptives Scheduling}

In Systemen mit nicht vollstÃ¤ndig bekannten Task-Parametern (Robotik, autonome
...
Der RL-Agent schlÃ¤gt Entscheidungen vor; ein formaler Verifikator prÃ¼ft
...
Jitter in der AusfÃ¼hrung eines Regelalgorithmus verschlechtert direkt die
RegelgÃ¼te. Klassische Echtzeit-Theorie minimiert nur Deadlines -- nicht
Jitter. \emph{Control-Aware Scheduling} (Cervin, Henriksson, Lincoln 2003--2010)
berÃ¼cksichtigt regelungstechnische GÃ¼tekriterien direkt im Scheduling-Algorithmus
und optimiert gemeinsam Scheduling und RegelgÃ¼te.
...
\begin{itemize}
	...
	\item Verteiltes EDF Ã¼ber mehrere Knoten mit NetzwerkverzÃ¶gerungen.
...
\subsubsection{ModellprÃ¼fung}

Werkzeuge wie UPPAAL (zeitbehaftete Automaten) und TLA+ ermÃ¶glichen die
formale Verifikation:
\begin{itemize}
	\item Nachweis der Deadline-Freiheit fÃ¼r konkrete Task-Sets.
	\item Verifikation von PIP/PCP auf Korrektheit und Deadlock-Freiheit.
	\item Verifikation von EDF$^+$ fÃ¼r endliche Blocking-Szenarien.
\end{itemize}

\textbf{Herausforderung:} Zustandsexplosion bei $n > 10$ Tasks.
Abstraktion und parametrische Verifikation (fÃ¼r beliebiges $n$) sind
offene Forschungsfragen.

\subsubsection{Theorem-Prover}

Isabelle/HOL, Coq und Lean ermÃ¶glichen maschinengeprÃ¼fte Beweise:
\begin{itemize}
	\item Der Liu-Layland-Bound wurde von Paulin (2006) in Isabelle formalisiert.
	\item Das EDF-OptimalitÃ¤tstheorem ist fÃ¼r Formalisierung zugÃ¤nglich
	      (Austauschargument als induktiver Beweis).
	\item \textbf{Ziel:} VollstÃ¤ndig verifizierte Scheduling-Bibliothek fÃ¼r
...
\cite{Epp2026} bietet mit seiner $O(n^3)$-KomplexitÃ¤t neue MÃ¶glichkeiten:

\begin{itemize}
	\item \textbf{Topologieanalyse von Task-AbhÃ¤ngigkeitsgraphen}: Effiziente
	      Erkennung von Subgraph-Beziehungen zwischen Task-Sets.
	\item \textbf{Deduplizierung}: Identifikation Ã¤quivalenter Task-Konfigurationen
	      in groÃŸen Scheduling-Problemen.
	\item \textbf{Verteilte Systeme}: Schnelle Topologieanpassung bei KnotenausfÃ¤llen
	      oder Neukonfigurationen in Echtzeit-Netzwerken.
\end{itemize}

ZukÃ¼nftige GPU-Parallelisierung (Tensoroperationen fÃ¼r Matrixberechnungen)
kÃ¶nnte die KomplexitÃ¤t auf $O(n^2 \log n)$ reduzieren.

\subsection{Hardware-Beschleunigung fÃ¼r Scheduling}

FÃ¼r Systeme mit sehr vielen Tasks ($n > 10.000$) oder extrem kurzen Perioden
...
\begin{itemize}
	...
	\item \textbf{Near-Memory Computing}: Scheduling-Logik direkt im
	      Speichercontroller fÃ¼r datenintensive Echtzeit-Workloads.
\end{itemize}
...
ISO~26262 ASIL-D fordert Nachweis der Schedulierbarkeit, beschrÃ¤nkte
Blockierzeiten und temporale Isolation. EDF$^+$ \cite{Epp2026} erfÃ¼llt diese
Anforderungen durch analytisch beschrÃ¤nkten Neuplanungs-Overhead
($E[R] = np/(1-p)$). Dies macht EDF$^+$ zum aussichtsreichen Kandidaten
fÃ¼r ASIL-D-Zertifizierung in AUTOSAR Adaptive Platform.
...
Fortschritte gemacht. Die hier behandelten Algorithmen -- von RM Ã¼ber EDF
bis zu EDF$^+$ -- bilden eine solide Grundlage, die tÃ¤glich in realen
Systemen eingesetzt wird.

Dennoch bestehen zentrale Herausforderungen: Die LÃ¼cke zwischen theoretisch
optimalen Algorithmen (Pfair) und praktisch einsetzbaren LÃ¶sungen bleibt
offen. Autonome Systeme, vernetzte Plattformen und KI-Anwendungen stellen
die Echtzeit-Theorie vor fundamental neue Fragen.

Der Kernsatz bleibt die bestÃ¤ndige Richtschnur:
...
Forschungsfeld mit direkten Auswirkungen auf Sicherheit und ZuverlÃ¤ssigkeit
technischer Systeme.
...
%--- Ãœberlast und Elastic Scheduling ---

\bibitem{Cervin2003}
A.\ Cervin,
\textit{Integrated Control and Real-Time Scheduling},
Diss., Lunds Tekniska HÃ¶gskola, 2003.
...
\bibitem{Epp2026}
S.\ Epp,
``Optimaler Scheduling-Algorithmus EDF$^+$: PrioritÃ¤tsstrafe und
stochastische Analyse bei 40 Tasks,'', 2026.
