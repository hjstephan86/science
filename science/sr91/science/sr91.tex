...
\title{\textbf{SR-91 Aurora~II:	Formale Aerodynamik- und Leistungsanalyse
	eines Hochgeschwindigkeits-AufklÃ¤rungsflugzeugs}}
\author{Stephan Epp}
\date{29. MÃ¤rz 2026}
...
Hochgeschwin\-digkeits-AufklÃ¤rungsflugzeugs \SR{}, das fÃ¼r eine Zwei-Piloten-Besatzung
ohne zivile Passagiere ausgelegt ist.
Das Flugzeug erreicht eine HÃ¶chstgeschwindigkeit von \Mach{5{,}0} bei einer
DienstgipfelhÃ¶he von \SI{30}{\kilo\metre}.
Ein besonderer Schwerpunkt liegt auf der formalen mathematischen Herleitung der
Wendeleistung im Ãœberschallregime:
Minimaler Kurvenradius, maximale Winkelrate und Lastvielfaches werden fÃ¼r das
gesamte Flugzustandsgebiet analytisch nachgewiesen.
ZwÃ¶lf matplotlib-Abbildungen illustrieren die quantitativen Ergebnisse.

AufklÃ¤rungsflugzeuge der Hochleistungsklasse vereinen drei scheinbar widersprÃ¼chliche
Anforderungen: maximale Reisegeschwindigkeit im Ãœberschallbereich, eine minimale
Radarsignatur sowie eine fÃ¼r menschliche Besatzungen ertragbare Dynamik.
Der \SR{} ist als konzeptionelles Nachfolgemuster des historischen SR-71 Blackbird
angelegt, dessen Auslegung auf \Mach{3{,}2} und \SI{25}{\kilo\metre} GipfelhÃ¶he
...
\begin{enumerate}[label=\roman*.]
  ...
  \item \textbf{Wendigkeit:} Aggressives WendemanÃ¶ver (\Mach{2{,}0}, $H = \SI{20}{\kilo\metre}$)
...
\Cref{sec:config} legt die geometrischen und massenbezogenen KenngrÃ¶ÃŸen fest.
\Cref{sec:aero} entwickelt die aerodynamische Theorie.
\Cref{sec:antrieb} beschreibt den TBCC-Antrieb.
\Cref{sec:wende} enthÃ¤lt den formalen Nachweis der Wendeleistung.
\Cref{sec:therm} behandelt aerodynamische ErwÃ¤rmung.
\Cref{sec:pilot} analysiert die physiologischen Belastungen der Besatzung.
\Cref{sec:stabilitaet} widmet sich der Flugdynamik und StabilitÃ¤t.
\Cref{sec:recon} bewertet den AufklÃ¤rungsnutzen.
...
\subsection{Geometrische HauptmaÃŸe}

\begin{table}[H]
\centering
\caption{Geometrische und massenbezogene KenngrÃ¶ÃŸen des \SR{}}
\label{tab:geo}
\begin{tabular}{llcl}
\toprule
GrÃ¶ÃŸe & Symbol & Wert & Einheit \\
\midrule
Spannweite & $b$ & 16{,}8 & \si{\metre} \\
FlÃ¼geltiefe (Wurzel) & $c_r$ & 24{,}0 & \si{\metre} \\
FlÃ¼geltiefe (Spitze) & $c_t$ & 2{,}5 & \si{\metre} \\
FlÃ¼gelflÃ¤che & $S$ & 78{,}5 & \si{\metre\squared} \\
Streckung & $\Lambda$ & 1{,}8 & â€” \\
Pfeilwinkel ($25\,\%$-Linie) & $\varphi_{25}$ & 67 & \si{\degree} \\
GesamtlÃ¤nge & $l_{ges}$ & 32{,}5 & \si{\metre} \\
HÃ¶he & $h_{ges}$ & 5{,}6 & \si{\metre} \\
\midrule
...
\subsection{FlÃ¼gelgeometrie: Delta-Konfiguration}

Der \SR{} verwendet einen \emph{geschnittenen Doppeldelta} (double-delta) mit
einem inneren Pfeilwinkel $\varphi_1 = 80^\circ$ und einem Ã¤uÃŸeren Winkel
$\varphi_2 = 60^\circ$.

\begin{definition}[Trapezgeometrische Kennzahlen]
  Sei $S$ die FlÃ¼gelreferenzflÃ¤che, $b$ die Spannweite, $\bar{c}$ die mittlere
  aerodynamische Tiefe (MAC) und $\lambda = c_t/c_r$ das ZuspitzungsverhÃ¤ltnis.
...
\subsection{KompressibilitÃ¤tskorrektur nach Prandtl-Glauert und Ackeret}

\begin{theorem}[Auftriebsanstieg nach Prandtl-Glauert und Ackeret]
  \label{thm:cla}
  FÃ¼r einen TragflÃ¼gel endlicher Streckung $\Lambda$ mit linearer Profiltheorie gilt
  fÃ¼r den Auftriebsanstieg $C_{L\alpha}$ (bezogen auf den Anstellwinkel in Bogenmass):
  \begin{equation}
    C_{L\alpha} =
    \begin{cases}
...
\begin{proof}
  ...
  den Term $2/\Lambda$ (Oswald-Faktor $e=1$, Delft-NÃ¤herung) erfasst.
  Im Ãœberschall liefert die lineare Ackeret-Theorie den Wert $4/\beta_A$ fÃ¼r
  den unendlichen FlÃ¼gel; der Korrekturfaktor fÃ¼r $\Lambda < \infty$ folgt
  analog aus der Randbedingung der TragflÃ¤che.
\end{proof}
...
\begin{definition}[Quadratische Polare]
  ...
  \begin{equation}
    ...
    \label{eq:polar}
  \end{equation}
  wobei $e \approx 0{,}85$ der Oswald-Wirkungsgrad des DeltaflÃ¼gels ist.
\end{definition}
...
\begin{figure}[H]
  ...
  \caption{Aerodynamische Polaren des \SR{}-DeltaflÃ¼gels bei Mach $0{,}5$ bis $5{,}0$.
           ...
           gemÃ¤ÃŸ \cref{eq:cd0} zurÃ¼ckzufÃ¼hren.}
  \label{fig:polar}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Links: Maximale Gleitzahl $E_{\max}(M)$ nach \cref{eq:ldmax}.
           ...
           Ãœberschallmaximum bei $M \approx 2{,}5$.
...
\begin{theorem}[Breguet-Reichweite]
  FÃ¼r konstante Gleitzahl $E$ und spezifischen Impuls $I_{sp}$ gilt:
  \begin{equation}
...
\begin{proof}
  ...
  Der Kraftstoffmassenstrom betrÃ¤gt $\dot{m}_f = T/(I_{sp}\,g_0)$, also
...
FÃ¼r $\zeta_f = 0{,}45$, $I_{sp} = \SI{3000}{\second}$ (Scramjet-Betrieb), $E = 5{,}5$:
\begin{equation}
...
\begin{definition}[TBCC-Arbeitsprinzip]
  Ein TBCC-Triebwerk vereint einen HochleistungsturbolÃ¼fter (Modus~I,
  $M_\infty \leq 3{,}0$) mit einem Dual-Mode-Scramjet (Modus~II,
  $M_\infty \geq 2{,}5$). Im Ãœbergangsbereich $2{,}5 \leq M_\infty \leq 3{,}0$
...
\begin{figure}[H]
  ...
  \caption{Schubkraft $F_T$ (kN, linke Achse) und spezifischer Impuls $I_{sp}$
           ...
           (gestrichelte Linie). Der Scramjet-Modus erzielt deutlich hÃ¶here $I_{sp}$-Werte.}
  \label{fig:schub}
\end{figure}
...
\begin{theorem}[Rankine-Hugoniot Stagnationsbedingungen]
  In einem Scramjet mit stationÃ¤rer, eindimensionaler, adiabater StrÃ¶mung gilt
  fÃ¼r das StagnationsdruckverhÃ¤ltnis Ã¼ber den Einlaufschock:
  \begin{equation}
...
Der spezifische Impuls $I_{sp}$ des Scramjet lÃ¤sst sich als Funktion der
...
\end{equation}
wobei $\eta_c$ der Verbrennungswirkungsgrad, $f_{st}$ das stÃ¶chiometrische
Kraftstoff-Luft-VerhÃ¤ltnis und $M_{cc}$ die Mach-Zahl in der Brennkammer ist.

% -----------------------------------------------------------------------
\section{WendemanÃ¶ver: Formaler Nachweis der Wendeleistung}
\label{sec:wende}
% -----------------------------------------------------------------------
...
\begin{theorem}[Kinematische Wendebedingungen]
  \label{thm:wende}
  FÃ¼r ein Luftfahrzeug der Masse $m$ im horizontalen, koordinierten Kurvenflug
...
\begin{proof}
  Die KrÃ¤ftebilanz im koordinierten Kurvenflug lautet in der vertikalen Ebene:
...
\begin{theorem}[Limitierendes Lastvielfaches]
  \label{thm:nlim}
  Das tatsÃ¤chlich erreichbare Lastvielfache $n_{\max}(V, H)$ ist das Minimum
...
\begin{proof}
  ...
  Da beide Grenzen unabhÃ¤ngig sind, bestimmt das Minimum das Erreichbare.
\end{proof}
...
FÃ¼r $H = \SI{20}{\kilo\metre}$ (ISA) ergibt die numerische Auswertung
$M_{\text{opt}} \approx 2{,}1$ mit $r_{\min} \approx \SI{32}{\kilo\metre}$
und einer 180Â°-Wendezeit von $t_{180} \approx \SI{52}{\second}$.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/fig05_Vn.pdf}
  \caption{V-n-ManÃ¶vrierdiagramm bei $H = \SI{20}{\kilo\metre}$.
           ...
           und $n_{\min} = -2{,}5$.
           $V_A$ ist die ManÃ¶vriergeschwindigkeit (Schnittpunkt Auftriebsgrenze
           mit $n_s$), $V_D$ die Konstruktionstauchgeschwindigkeit.}
  \label{fig:Vn}
\end{figure}

\subsection{WendemanÃ¶ver im Zeitbereich}

Das koordinierte KurvenmanÃ¶ver wird im Zustandsraummodell beschrieben.
Sei $\mathbf{x} = (V,\, \gamma,\, \chi,\, x,\, y,\, H)^T$ der Zustandsvektor
(Geschwindigkeit, Bahnneigung, Kurswinkel, Lage, HÃ¶he), dann lauten die
Bewegungsgleichungen fÃ¼r den Punktmassenschwerpunkt:
\begin{align}
...
% -----------------------------------------------------------------------
\section{Steigflug und FlugzustandshÃ¼llkurve}
\label{sec:steig}
% -----------------------------------------------------------------------
...
\begin{figure}[H]
  ...
  \caption{Steigfluganalyse fÃ¼r drei Steigwinkel $\gamma \in \{8^\circ, 15^\circ, 25^\circ\}$.
           Links: HÃ¶he $H(t)$ Ã¼ber der Zeit.
           Mitte: Mach-Verlauf $M(t)$ wÃ¤hrend des Steigflugs.
           Rechts: Flugbahn $H(x)$ im LÃ¤ngsschnitt.
           Der steile Steigflug ($\gamma = 25^\circ$) erreicht $H = \SI{30}{\kilo\metre}$
           in deutlich kÃ¼rzerer Zeit als der flache Steigkurs.}
  \label{fig:steig}
\end{figure}

\begin{theorem}[Spezifische Ãœberschussleistung]
  Die spezifische Ãœberschussleistung (SEP) ist definiert als:
  \begin{equation}
...
\begin{figure}[H]
  ...
  \caption{FlugzustandshÃ¼llkurve des \SR{} im $M$-$H$-Diagramm.
           Die blaue Abrisslinie begrenzt den Bereich nach unten-links,
           die orange thermische Grenze ($T_{aw} = 600\,{}^\circ$C Ti-Legierung)
           nach oben-rechts.
           Der Auslegungspunkt bei $M = 3{,}5$, $H = \SI{20}{\kilo\metre}$ (grÃ¼ne Raute)
           liegt komfortabel innerhalb der HÃ¼llkurve.}
  \label{fig:envelope}
\end{figure}

% -----------------------------------------------------------------------
\section{Aerodynamische ErwÃ¤rmung}
\label{sec:therm}
% -----------------------------------------------------------------------
...
\begin{theorem}[Stagnationstemperatur]
  In einem adiabaten, kompressiblen StrÃ¶mungsfeld gilt fÃ¼r die
...
\begin{proof}
  Aus dem ersten Hauptsatz der Thermodynamik fÃ¼r adiabate, stationÃ¤re StrÃ¶mung:
...
\begin{equation}
  ...
  \label{eq:Taw}
\end{equation}
mit dem RÃ¼ckgewinnungsfaktor $r$. FÃ¼r turbulente Grenzschicht: $r \approx Pr^{1/3}$
...
\begin{figure}[H]
  ...
  \caption{Links: Stagnationstemperatur $T_0$ (rot) und adiabatische Wandtemperatur
           ...
           Die horizontalen Linien markieren die Dauertemperaturgrenzen fÃ¼r
           Ti-6Al-4V (blau, 593Â°C) und keramische Verbundwerkstoffe CMC (grÃ¼n, 1200Â°C).
           Der rote Schraffurbereich kennzeichnet den Ãœberhitzungsbereich fÃ¼r Titan.
           Rechts: Aerodynamischer WÃ¤rmefluss an der Vorderkante ($x_c = 0{,}5$ m).}
  \label{fig:heizung}
\end{figure}

\begin{proposition}[Strukturmaterial-Wahl]
  FÃ¼r $M_\infty > 2{,}7$ bei $H = \SI{20}{\kilo\metre}$ Ã¼berschreitet $T_{aw}$
  die Dauerfestigkeitstemperatur von Ti-6Al-4V ($593\,{}^\circ$C). Oberhalb
  $M_\infty = 4{,}5$ versagt auch CMC unterhalb von $1200\,{}^\circ$C.
  Folgerung: Die PrimÃ¤rstruktur des \SR{} erfordert keramische Verbundwerkstoffe
  (CMC) im vorderen Rumpfbereich und aktive KÃ¼hlung der Vorderkanten.
\end{proposition}
...
Das \SR{} ist fÃ¼r eine Besatzung von genau zwei Personen ausgelegt:
\textbf{Pilot} (vorderes Cockpit) und \textbf{Waffensystemoffizier / AufklÃ¤rungsoperator}
...
\begin{definition}[Okularer Blutdruck unter g-Last]
  ...
  (Gz-Achse, kopfwÃ¤rts positiv) betrÃ¤gt:
  \begin{equation}
...
\begin{theorem}[G-LOC-Schwelle]
  ...
  Ohne SchutzmaÃŸnahmen folgt aus \cref{eq:pocular}:
  \begin{equation}
...
bindende EinschrÃ¤nkung â€” die Piloten kÃ¶nnen das volle Lastvielfache
...
\begin{figure}[H]
  ...
  \caption{Pilotenbelastungsanalyse.
           Oben links: g-Profil eines typischen WendemanÃ¶vers.
           Die G-LOC-Grenze (rot gestrichelt) bei $6{,}3\,g$ wird nicht Ã¼berschritten.
           ...
           Unten links: Anti-G-Anzug Druckkennlinie fÃ¼r Standard- und ATAGS-Anzug.
           Unten rechts: Kabinendruckprofil versus FlughÃ¶he (Normalbetrieb: 2400 m Ã¤quiv.,
           Notbetrieb: 6000 m Ã¤quiv.).}
  \label{fig:pilot}
\end{figure}

\subsection{Druckkabine und Lebenserhaltungssystem}

Die Druckkabine hÃ¤lt ein Ã„quivalent von $H_{ekv} = \SI{2400}{\metre}$
(Kabinendruck $\approx \SI{752}{\hecto\pascal}$) bis zur DienstgipfelhÃ¶he
...
% -----------------------------------------------------------------------
\section{Dynamische StabilitÃ¤t}
\label{sec:stabilitaet}
% -----------------------------------------------------------------------

\subsection{Lineare Flugdynamik}

Das linearisierte KleinstÃ¶rungs-Zustandsraummodell fÃ¼r die LÃ¤ngsdynamik lautet:
\begin{equation}
...
\begin{definition}[Eigenfrequenz und DÃ¤mpfung der Kurzperiode]
  Die Kurzperioden-Eigenfrequenz und -dÃ¤mpfung sind:
  \begin{equation}
...
\begin{figure}[H]
  ...
  \caption{Dynamische StabilitÃ¤tsanalyse.
           Oben links: Phygoiden-Periode als Funktion der Mach-Zahl.
           Oben rechts: Kurzperioden-Eigenfrequenz und DÃ¤mpfung;
           ...
           Unten rechts: Dutch-Roll StabilitÃ¤t â€” $\zeta_{DR}\,\omega_{DR} \geq 0{,}35\;\text{rad/s}$
           gemÃ¤ÃŸ MIL-F-8785C wird fÃ¼r alle $M > 1{,}2$ eingehalten.}
  \label{fig:stabilitaet}
\end{figure}

\subsection{Fly-by-Wire und aktive StabilitÃ¤tsaugmentation}

Da der \SR{} im transonischen Bereich und bei hohen Mach-Zahlen statisch
neutral bis leicht instabil ist (fÃ¼r geringe Wellenwiderstandszunahme),
...
\begin{theorem}[Robustheitsbedingung FBW]
  FÃ¼r das geschlossene Regelkreissystem mit Regler $K(s)$ und Strecke $G(s)$
  ...
  \begin{equation}
    ...
    \label{eq:robust}
  \end{equation}
  mit der minimalen Strukturellen SingulÃ¤rwert-Schranke $\Delta_{\min}$.
\end{theorem}

% -----------------------------------------------------------------------
\section{AufklÃ¤rungsnutzen und Sensorsysteme}
\label{sec:recon}
% -----------------------------------------------------------------------

\subsection{Optische AufklÃ¤rung: BodenauflÃ¶sung}

Die geometrische BodenauflÃ¶sung (Ground Sample Distance, GSD) einer
optischen Kamera mit Brennweite $f$ und PixelgrÃ¶sse $p$ aus der FlughÃ¶he $H$ betrÃ¤gt:
\begin{equation}
  \text{GSD} = \frac{p\,H}{f}
  \label{eq:gsd}
\end{equation}
FÃ¼r $f = \SI{1800}{\milli\metre}$, $p = \SI{5{,}2}{\micro\metre}$, $H = \SI{20}{\kilo\metre}$:
\begin{equation}
  \text{GSD} = \frac{5{,}2 \times 10^{-6} \cdot 20\,000}{1{,}8}
  = \SI{0{,}058}{\metre} \approx \SI{5{,}8}{\centi\metre}
\end{equation}
Damit lassen sich Fahrzeuge (LÃ¤nge $> \SI{3}{\metre}$) mit einem AuflÃ¶sungsfaktor
$> 50$ sicher klassifizieren.

\subsection{Radar-Horizont und SIGINT}

Der geometrische Radar-Horizont (ErdkrÃ¼mmung einbezogen):
\begin{equation}
...
\begin{figure}[H]
  ...
  \caption{AufklÃ¤rungsleistung des \SR{}.
           Links: Radar-Horizont als Funktion der FlughÃ¶he nach \cref{eq:rhorizon}.
           Mitte: GSD der optischen AufklÃ¤rungskamera nach \cref{eq:gsd};
           bei $H = \SI{20}{\kilo\metre}$ wird eine GSD von $< \SI{6}{\centi\metre}$
           erzielt.
           Rechts: Payload-Reichweiten-Kompromiss â€” erhÃ¶hte Sensorlast reduziert
...
\begin{figure}[H]
  ...
  \caption{Radar-Querschnitt (RCS) des \SR{} bei $f = \SI{10}{\giga\hertz}$.
           ...
           Rechts: Linearer Verlauf Ã¼ber den Azimutwinkel.
           ...
           Kantenbeugung an den schrÃ¤gen FlÃ¼gelvorderkanten auf $+5\;\text{dBsm}$ begrenzt.}
  \label{fig:rcs}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Leistungszusammenfassung \SR{} â€” formal nachgewiesene KenngrÃ¶ÃŸen}
\label{tab:summary}
\begin{tabular}{lcc}
\toprule
KenngrÃ¶ÃŸe & Wert & Formel/Verweis \\
\midrule
HÃ¶chstgeschwindigkeit & $M_{\max} = 5{,}0$ & \cref{fig:envelope} \\
Reisegeschwindigkeit & $M_{cruise} = 3{,}5$ & â€” \\
DienstgipfelhÃ¶he & $H_{max} = \SI{30}{\kilo\metre}$ & \cref{fig:envelope} \\
...
\begin{enumerate}
  \item Die formalen Beweise der \cref{thm:cla,thm:wende,thm:nlim,thm:gloc}
        bilden das Fundament fÃ¼r die quantitative Leistungsvorhersage.
  \item Das WendemanÃ¶ver (SÃ¤tze~\ref{thm:wende},~\ref{kor:opt_M}) ist bei
        $M \approx 2{,}1$ optimal; der minimale Kurvenradius betrÃ¤gt
  ...
  \item Die physiologische Analyse (\cref{thm:gloc}) belegt, dass die
        ...
        $n_s = 6$ ausschÃ¶pfen kann.
  \item Die thermische Analyse zeigt, dass CMC-Materialien ab $M > 4{,}5$
        zwingend erforderlich sind.
\end{enumerate}

\paragraph{Ausblick.}
WeiterfÃ¼hrende Arbeiten sollten die AeroelastizitÃ¤t des DeltaflÃ¼gels bei
hohen Mach-Zahlen (Flattern), die prÃ¤zise Triebwerksmodellierung des TBCC
mittels partieller Differentialgleichungen der reaktiven StrÃ¶mung sowie
die vollstÃ¤ndige 6-DOF-Flugdynamiksimulation adressieren.
...
\nomenclature{$\Lambda$}{FlÃ¼gelstreckung}
...
\nomenclature{$\text{GSD}$}{Geometrische BodenauflÃ¶sung (m)}
...
\bibitem{mil8785}
MIL-F-8785C,
\textit{Military Specification â€” Flying Qualities of Piloted Airplanes}.
...
\bibitem{holtz2020}
J.~Holtz,
\textit{Thermische Schutzschichten fÃ¼r Hochgeschwindigkeitsflugzeuge}.
DLR-Forschungsbericht~FB~2020-17, KÃ¶ln, 2020.
