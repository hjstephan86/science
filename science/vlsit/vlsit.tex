...
effiziente Struktur\-prÃ¼fung durch zyklische Signaturrotation}
...
BrÃ¼cken\-fehler oder Leitungs\-unter\-brechungen geprÃ¼ft werden.
...
reduzierbar ist.
Der Subgraph Algorithmus lÃ¶st dieses Problem in $O(n^3)$ durch zyklische
Signaturrotation und LCS-Vergleich.
Wir beweisen formal die Korrektheit dieser Reduktion, analysieren die
KomplexitÃ¤t mit mehreren neuen Lemmata und Korollaren, stellen eine
vollstÃ¤ndige Python-Implementierung vor, und vergleichen den Ansatz mit
klassischen BIST-Methoden (Built-In Self-Test) auf Basis von LFSRs
(MLFSRs und SLFSRs).
Experimente auf ISCAS-85 Benchmark-Schaltkreisen sowie eine ausfÃ¼hrliche
...
Die Herstellung integrierter Schaltkreise (ICs) im VLSI-MaÃŸstab ist ein
hochkomplexer Prozess mit nichtvern\-achlÃ¤ssigbaren Fehlerquoten. Moderne
System-on-Chip (SoC)-Designs umfassen Milliarden von Transistoren auf einer
FlÃ¤che von wenigen Quadrat\-millimetern. Selbst kleinste
Fertigungs\-abweichungen -- etwa durch Verunreinigungen, Litho\-grafie\-fehler
oder thermische Effekte -- kÃ¶nnen zu Funktions\-fehlern fÃ¼hren, die das
gesamte Bauteil unbrauchbar machen.

VLSI-Testing bezeichnet den Prozess, bei dem hergestellte Chips auf solche
Hardware-Defekte Ã¼berprÃ¼ft werden, \emph{bevor} sie in Endprodukte verbaut
...
\begin{definition}[VLSI-Testing]
  ...
  Prozess der Ãœber\-prÃ¼fung physisch gefertigter integrierter Schaltkreise
...
\begin{definition}[Hardware-Verifikation]
  ...
  gegebene Spezifikation erfÃ¼llt. Sie operiert auf dem logischen Entwurf,
  nicht auf physischen Exemplaren.
\end{definition}

Die zentrale Herausforderung beim Testing liegt in der Effizienz: Ein
Schaltkreis mit $n$ Flip-Flops besitzt $2^n$ mÃ¶gliche ZustÃ¤nde -- eine
vollstÃ¤ndige Enumeration ist bereits fÃ¼r $n = 100$ praktisch unmÃ¶glich.
Daher sind intelligente Testmethoden erforderlich, die mit mÃ¶glichst wenigen
Testvektoren eine hohe Fehlerdeckung (engl.\ \emph{fault coverage}) erzielen.

\subsection{Beitrag dieser Arbeit}

Diese Arbeit leistet folgende BeitrÃ¤ge:
...
\begin{itemize}
  ...
  \item Neue Lemmata zur SignaturstabilitÃ¤t unter Knotenrelabeling und zur
        Fehlerortung durch Rotationsindex
  \item VollstÃ¤ndige Python-Implementierung mit Integrations\-test auf
        ISCAS-85-Netze
  \item Analyse der Effizienz des Subgraph Algorithmus~\cite{epp2026subgraph}
        gegenÃ¼ber klassischen LFSR-basierten BIST-Methoden
...
Abschnitt~\ref{sec:grundlagen} fÃ¼hrt die theoretischen Grundlagen ein:
Fehlermodelle, LFSR-Varianten und den Subgraph Algorithmus.
Abschnitt~\ref{sec:modellierung} prÃ¤sentiert die formale Graphenmodellierung
...
und zeigt die vollstÃ¤ndige Implementierung.
Abschnitt~\ref{sec:analyse} analysiert KomplexitÃ¤t, Korrektheit und
OptimalitÃ¤t mit neuen formalen Ergebnissen.
Abschnitt~\ref{sec:experimente} wertet experimentelle Ergebnisse mit
erweiterter Benchmark-Tabelle aus.
Abschnitt~\ref{sec:ausblick} schlieÃŸt mit einem ausfÃ¼hrlichen Ausblick.
...
\begin{definition}[Stuck-at-Fault (SAF)]
  ...
  den logischen Wert~$0$ annimmt, unabhÃ¤ngig vom tatsÃ¤chlichen
  Schaltkreis\-verhalten. Analog fixiert ein Stuck-at-1-Fault (SA1) den Wert
  dauerhaft auf~$1$.
\end{definition}

\begin{definition}[BrÃ¼ckenfehler (Bridging Fault)]
  Ein BrÃ¼ckenfehler beschreibt einen unerwÃ¼nschten elektrischen Kurzschluss
...
\begin{definition}[Leitungsunterbrechung (Open Fault)]
  ...
  kein logisches Signal Ã¼bertragen werden kann.
\end{definition}

\begin{definition}[Delay Fault]
  Ein Delay Fault liegt vor, wenn ein Signal die korrekte logische Funktion
  ausfÃ¼hrt, aber die geforderten Zeitbedingungen (Setup- oder Hold-Times)
  verletzt.
\end{definition}
...
\begin{figure}[ht]
  ...
  \caption{Links: Typische Verteilung von Fertigungs\-fehlern in
    ...
    dominieren mit Ã¼ber 60\,\%. Rechts: Fehlerdeckungskurven fÃ¼r verschiedene
    Testmethoden in AbhÃ¤ngigkeit von der Testvektormenge. Der
...
Built-In Self-Test (BIST) bezeichnet die Integration von TestfunktionalitÃ¤t
...
\begin{itemize}
  ...
  \item \textbf{Circuit Under Test (CUT):} Der zu prÃ¼fende Schaltkreis
...
\begin{definition}[LFSR]
  ...
  $b_{n-1}, \ldots, b_0$ und einer RÃ¼ckkopplungsfunktion
  \[
    b_n(t+1) = \bigoplus_{i \in F} b_i(t),
  \]
  wobei $F \subseteq \{0, \ldots, n-1\}$ die Menge der
  RÃ¼ckkopplungs\-anzapfungen (Feedback Taps) ist und $\oplus$ die
  XOR-VerknÃ¼pfung bezeichnet.
\end{definition}
...
\begin{itemize}
  \item \textbf{SLFSR (Standard LFSR):}
    \begin{itemize}
      \item \emph{Test-per-scan:} + FÃ¼gt sich gut in den Design-Flow ein;
            $-$ Niedrigere Schiebegeschwindigkeit
      \item \emph{Test-per-clock:} + FÃ¼gt sich gut in den Design-Flow ein;
            $-$ Starke Korrelation zwischen aufeinanderfolgenden
            Testmustern, was die Fehlerdeckung begrenzt
    \end{itemize}
  \item \textbf{MLFSR (Modified LFSR):}
    \begin{itemize}
      \item \emph{Test-per-scan:} + HÃ¶here Geschwindigkeit
      \item \emph{Test-per-clock:} + HÃ¶here Geschwindigkeit;
            + HÃ¶here Perturbation der Testmuster (geringere Korrelation),
            was zu besserer Fehlerdeckung fÃ¼hrt
    \end{itemize}
\end{itemize}

Die starke Musterkorrelation beim SLFSR im Test-per-clock-Modus ist eine
fundamentale EinschrÃ¤nkung: Aufeinanderfolgende LFSR-ZustÃ¤nde unterscheiden
...
Der MLFSR adressiert dieses Problem durch modifizierte RÃ¼ckkopplungspolynome,
die grÃ¶ÃŸere Hamming-Distanzen zwischen aufeinanderfolgenden Mustern erzwingen.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{plot3_lfsr_comparison.pdf}
  \caption{Links: Ausgabesequenzen von SLFSR und MLFSR Ã¼ber 32 Taktzyklen
    (4-Bit-Register). Die MLFSR-Sequenz zeigt grÃ¶ÃŸere Perturbation. Rechts:
    Doppel-logarithmischer Vergleich der TestzeitkomplexitÃ¤t. Der
    Subgraph Algorithmus skaliert mit $O(n^2)$ fÃ¼r die Signaturberechnung,
    wÃ¤hrend LFSR-basierte Methoden linear mit der SchaltkreisgrÃ¶ÃŸe wachsen.}
...
Der Algorithmus prÃ¼ft fÃ¼r jede der $n$ zyklischen Rotationen von
$\boldsymbol{\sigma}^B$, ob die LÃ¤nge der lÃ¤ngsten gemeinsamen Teilsequenz
(LCS) mit $\boldsymbol{\sigma}^A$ mindestens~2 betrÃ¤gt.
Die Laufzeit betrÃ¤gt $O(n^3)$~\cite{epp2026subgraph}.
...
Ein kombinatorischer VLSI-Schaltkreis lÃ¤sst sich natÃ¼rlich als gerichteter
...
\begin{definition}[Schaltkreisgraph]
  \sloppy
  Sei~$C$ ein kombinatorischer Schaltkreis mit PrimÃ¤r\-eingÃ¤ngen
  $\mathcal{PI} = \{i_1, \ldots, i_k\}$, Logik\-gattern
  $\mathcal{G} = \{g_1, \ldots, g_m\}$ und PrimÃ¤r\-ausgÃ¤ngen
  $\mathcal{PO} = \{o_1, \ldots, o_l\}$.
  Der Schaltkreisgraph ist $G_C = (V, E)$ mit
  \[
    V = \mathcal{PI} \cup \mathcal{G} \cup \mathcal{PO}, \qquad
    E = \{(u, v) \mid \text{Signal von~$u$ nach~$v$ flieÃŸt}\}.
  \]
\end{definition}

\begin{definition}[Fehlerfreier Referenzgraph]
  Sei $G_{\mathrm{ref}} = (V, E_{\mathrm{ref}})$ der Schaltkreisgraph des
  spezifizierten, fehlerfreien Entwurfs. Die zugehÃ¶rige Adjazenzmatrix
  $A_{\mathrm{ref}}$ heiÃŸt \emph{Referenzmatrix}.
\end{definition}

\begin{definition}[Fehlerhafter Schaltkreisgraph]
  Sei $G_{\mathrm{fault}} = (V, E_{\mathrm{fault}})$ der Schaltkreisgraph
  eines konkreten, mÃ¶glicherweise fehlerhaften Chips. Die zugehÃ¶rige
  Adjazenzmatrix $A_{\mathrm{fault}}$ heiÃŸt \emph{Testmatrix}.
\end{definition}

\begin{definition}[GatterkomplexitÃ¤t und Netlistentiefe]
  Die \emph{GatterkomplexitÃ¤t} eines Schaltkreisgraphen $G_C$ ist die
  Knotenzahl $|V| = k + m + l$.
  Die \emph{Netlistentiefe} $d(G_C)$ ist die LÃ¤nge des lÃ¤ngsten Pfades
  (in Kantenanzahl) von einem PrimÃ¤reingangs\-knoten zu einem
  PrimÃ¤rausgangs\-knoten.
\end{definition}

Die Netlistentiefe bestimmt maÃŸgeblich, wie viele Testvektoren zur
Aktivierung tiefliegender Gatter erforderlich sind.
Ein Schaltkreis mit Tiefe~$d$ benÃ¶tigt mindestens $d$ Taktstufen, um eine
vollstÃ¤ndige Stimulation der AusgÃ¤nge zu erreichen.
...
\begin{figure}[ht]
  ...
  \caption{Links: Modellierung eines kombinatorischen Schaltkreises (angelehnt
    ...
    PrimÃ¤r\-eingÃ¤ngen (grÃ¼n), internen Gattern (blau/orange) und
    PrimÃ¤r\-ausgÃ¤ngen (rot). Rechts: Adjazenzmatrix des Schaltkreisgraphen
    fÃ¼r die inneren Gatter\-knoten. Einsen reprÃ¤sentieren Signalleitungen.}
...
\begin{proof}
  ...
  Signal\-Ã¼bertragung von~$g_i$ zu seinen Nachfolgern~$v$.
  Da kein effektives Signal Ã¼bertragen werden kann (der Wert ist
...
\begin{lemma}[SA1 als KantenhinzufÃ¼gung]
  \label{lem:sa1_edge}
  Ein Stuck-at-1-Fault an einem internen Signal kann im erweiterten
  Graphen\-modell als HinzufÃ¼gen einer Pseudo-Kante modelliert werden,
  die eine zusÃ¤tzliche AbhÃ¤ngigkeit von einem Konstantwert-Knoten
  $g_{\mathrm{vcc}}$ simuliert.
\end{lemma}

\begin{proof}
  Ein SA1-Fault an Signal~$s$ zwischen Gattern~$g_i$ und~$g_j$ bewirkt,
  dass~$g_j$ stets den Eingangswert~$1$ von~$s$ liest, unabhÃ¤ngig von
  $g_i$'s tatsÃ¤chlichem Ausgangswert.
  Im erweiterten Fehlermodellgraphen wird ein Pseudo-Knoten
  $g_{\mathrm{vcc}}$ mit konstantem Ausgangswert~$1$ eingefÃ¼hrt und
  eine Kante $(g_{\mathrm{vcc}}, g_j)$ hinzugefÃ¼gt.
  Gleichzeitig wird die originale Kante $(g_i, g_j)$ entfernt, sofern
  $g_i$'s Einfluss auf~$g_j$ vollstÃ¤ndig durch den Konstantwert ersetzt
  wird.
  Damit gilt $E_{\mathrm{ref}} \subsetneq E_{\mathrm{fault}}$, d.\,h.\
  $G_{\mathrm{ref}} \subseteq G_{\mathrm{fault}}$ im erweiterten Modell.
\end{proof}

\begin{lemma}[BrÃ¼ckenfehler als KantenzusammenfÃ¼hrung]
  \label{lem:bridge_edge}
  Ein BrÃ¼ckenfehler zwischen den Signalleitungen $(g_i, v_1)$ und
  $(g_j, v_2)$ entspricht einer Kantenmodifikation, bei der die
  Ausgangsgraphen der betroffenen Gatter zusammengefÃ¼hrt werden.
  Im Allgemeinen gilt weder $G_{\mathrm{fault}} \subseteq G_{\mathrm{ref}}$
  noch $G_{\mathrm{ref}} \subseteq G_{\mathrm{fault}}$.
\end{lemma}

\begin{proof}
  Ein BrÃ¼ckenfehler zwischen zwei Leitungen erzeugt einen elektrischen
  Kurzschluss.
  Im Wired-AND/Wired-OR-Modell resultiert dies in einem verÃ¤nderten
  logischen Wert auf beiden betroffenen Leitungen.
  Im Graphenmodell fÃ¼hrt dies dazu, dass die Knoten~$g_i$ und~$g_j$
  im fehlerhaften Graphen strukturell Ã¤quivalent behandelt werden.
  ...
  Da die ursprÃ¼ngliche Graphenstruktur verÃ¤ndert wird, kann keine der
  beiden Inklusionsrichtungen im Allgemeinen garantiert werden.
  Der BrÃ¼ckenfehler ist daher der schwierigste Fall fÃ¼r die
...
\begin{lemma}[Mehrfachfehler und Superposition]
  \label{lem:mehrfach}
  Seien $F_1, \ldots, F_k$ unabhÃ¤ngige Stuck-at-Faults vom Typ SA0 an
  Gattern $g_{i_1}, \ldots, g_{i_k}$.
  Der resultierende Fehler\-graph $G_{\mathrm{mf}}$ erfÃ¼llt
  \[
    E_{\mathrm{mf}}
    = E_{\mathrm{ref}}
...
\begin{proof}
  ...
  Da die Faults unabhÃ¤ngig voneinander auftreten, ist der Effekt mehrerer
...
\begin{korollar}[Erkennbarkeit von Mehrfachfehlern]
  ...
  wird, wÃ¤hrend $G_{\mathrm{ref}} \subseteq G_{\mathrm{mf}}$ (da
...
\begin{proof}
  ...
  $G_{\mathrm{ref}}$ enthÃ¤lt.
  Der Subgraph Algorithmus~\cite{epp2026subgraph} prÃ¼ft beide Richtungen
...
\begin{lemma}[Hierarchische Netzlisten-Dekomposition]
  ...
  Sei ein VLSI-Schaltkreis~$C$ hierarchisch aufgebaut: $C$ enthÃ¤lt
  Submodule $M_1, \ldots, M_r$ mit Schaltkreisgraphen
  $G_{M_1}, \ldots, G_{M_r}$.
  Dann ist der Gesamtschaltkreisgraph $G_C$ eine disjunkte Vereinigung
  der Submodul\-graphen, ergÃ¤nzt um Verbindungsknoten und -kanten:
  \[
    G_C
...
\begin{proof}
  ...
  was fÃ¼r gleichmÃ¤ÃŸige ModulgrÃ¶ÃŸen $|M_l| = n/r$ den Ausdruck
...
\begin{satz}[Reduktion: VLSI-Test $\leq_p$ Subgraph-Erkennung]
  ...
  Das VLSI-Stuck-at-Fehlertestproblem fÃ¼r kombinatorische Schaltkreise
...
\begin{proof}
  ...
  \begin{itemize}
    \item \emph{Fehlerfreier Chip:} $G_{\mathrm{test}} \cong G_{\mathrm{ref}}$,
          d.\,h.\ beide Subgraph-Inklusionen sind erfÃ¼llt.
    ...
    \item \emph{SA1-Fault (erweitertes Modell):}
          ...
          (zusÃ¤tzliche Kanten). Es gilt
          $G_{\mathrm{ref}} \subseteq G_{\mathrm{test}}$,
          aber nicht umgekehrt.
    \item \emph{BrÃ¼ckenfehler:} Weder
  ...
  \begin{enumerate}
    ...
    \item[(ii)]  $G_{\mathrm{ref}} \subseteq G_{\mathrm{test}}$:
                 Test auf SA1-Faults
    \item[(iii)] Beide Inklusionen erfÃ¼llt: Fehlerfrei
                 ($G_{\mathrm{test}} \cong G_{\mathrm{ref}}$)
  \end{enumerate}

  \textbf{Polynomialzeit:} Graphkonstruktion in $O(n^2)$; der
  Subgraph Algorithmus lÃ¶st jede Anfrage in $O(n^3)$.
  Gesamtaufwand: $O(n^3)$.
...
\begin{lemma}[SignaturstabilitÃ¤t unter Knotenrelabeling]
...
\begin{proof}
  ...
  $(j+k) \bmod n$ in~$A$.
  Da die Rotation nur die Indexzuweisung der Spalten Ã¤ndert, bleibt
...
\begin{lemma}[Fehlerortung durch Rotationsindex]
  ...
  Referenzgraphen Ã¼berein (nach Knotenrelabeling).
...
\begin{proof}
  ...
  enthÃ¤lt genau die Signaturen der Spalten, die im Testgraphen fehlen
  oder verÃ¤ndert sind.
  Da jede Signatur nach dem InjektivitÃ¤ts\-lemma aus~\cite{epp2026subgraph}
...
FÃ¼r beide Matrizen werden die Signaturvektoren
...
FÃ¼r jede der $n$ Rotationen $r \in \{0, \ldots, n-1\}$ von
...
Die folgende Implementierung realisiert den vollstÃ¤ndigen
Subgraph-basierten VLSI-Test in Python. Sie stÃ¼tzt sich auf die
Kernmethoden aus~\cite{epp2026subgraph} und ergÃ¤nzt diese um die
Fehlerdiagnosefunktion.

\begin{lstlisting}[language=Python,
                   caption={Signaturberechnung und Rotationssuche fÃ¼r VLSI-Schaltkreisgraphen}]
import numpy as np
...
LCS-Berechnung Ã¼ber alle Rotationen sowie den KomplexitÃ¤tsvergleich
mit dem naiven Permutationsansatz.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{plot4_subgraph_vlsi.pdf}
  \caption{Links: LCS-Werte des Subgraph Algorithmus Ã¼ber alle
    zyklischen Rotationen beim Vergleich zweier Schaltkreisgraphen
    ($n=6$). Rotation~3 liefert den maximalen LCS-Wert~6 (vollstÃ¤ndige
    Ãœbereinstimmung). Rechts: KomplexitÃ¤tsvergleich auf logarithmischer
    Skala. Die Schere zwischen $O(n^3)$ (Subgraph) und $O(n!)$ (naive
    Permutation) Ã¶ffnet sich ab $n=7$ dramatisch.}
...
\begin{figure}[ht]
  ...
  \caption{Links: Reduktionskette vom VLSI-Fehlermodell zum
    Subgraph-Erkennungsproblem in fÃ¼nf Transformationsschritten.
    Rechts: Fehlerdeckungskurven fÃ¼r die ISCAS-85-Benchmarks c17 und
...
\begin{proof}
  ...
  \textbf{InjektivitÃ¤t der Signaturfunction.}
  ...
  Signaturvektoren hat LÃ¤nge $\geq 2$.
  ...
  Rotation mit LCS $= n$ (vollstÃ¤ndige SignaturÃ¼bereinstimmung), und
  der Algorithmus gibt \emph{pass} aus.

  \textbf{Fehlerfreiheit von Fehlklassifikationen.}
  Aus der InjektivitÃ¤t folgt: Verschiedene KantensÃ¤tze erzeugen
...
\begin{satz}[Laufzeit des VLSI-Tests via Subgraph]
  ...
  betrÃ¤gt $O(n^3)$, wobei $n$ die Anzahl der Logikgatter ist.
\end{satz}

\begin{proof}
  Die vier Phasen haben folgende KomplexitÃ¤ten:
  \begin{itemize}
    ...
    \item Phase~4 (DiagnoserÃ¼ckverfolgung): $O(n)$
  \end{itemize}
  Gesamt: $O(n^2) + O(n^2) + O(n^3) + O(n) = O(n^3)$. $\square$
\end{proof}

\begin{korollar}[Vorteil gegenÃ¼ber naivem Ansatz]
  \label{kor:speedup}
  Der naive Ansatz, der alle $n!$ Knotenpermu\-tationen prÃ¼ft und fÃ¼r
  jede eine $n \times n$ MatrixÃ¼bereinstimmung in $O(n^2)$ testet,
  hat Laufzeit $O(n!\cdot n^2)$. Der Subgraph Algorithmus ist um den
  Faktor
  \[
    \frac{n!\cdot n^2}{n^3} = \frac{n!}{n} = (n-1)!
  \]
  schneller. FÃ¼r $n=10$ ergibt sich ein Speedup-Faktor von
  $9! = 362\,880$.
\end{korollar}
...
\begin{satz}[Optimale Laufzeit unter SETH]
  ...
  fÃ¼r das zyklische Subgraph-Problem auf Schaltkreisgraphen:
  Kein Algorithmus kann das Problem in $O(n^{3-\varepsilon})$ fÃ¼r
  ein festes $\varepsilon > 0$ lÃ¶sen.
\end{satz}

\begin{proof}
  Der Beweis gliedert sich in drei Schritte nach~\cite{epp2026subgraph}.

  \textbf{Untere Schranke fÃ¼r die Signaturberechnung:~$\Omega(n^2)$.}
  Jeder korrekte Algorithmus muss alle $n^2$ EintrÃ¤ge beider
  ...
  liefert. Der Algorithmus wÃ¼rde dann fÃ¤lschlicherweise \emph{kein
  ...
  kann LCS zweier Sequenzen der LÃ¤nge~$n$ unter SETH nicht in
  ...
  \[
    ...
    \quad\text{fÃ¼r alle } \varepsilon>0.
  \]
  Da der Algorithmus $\Theta(n^3)$ benÃ¶tigt, ist er optimal. $\square$
\end{proof}

\begin{lemma}[Speicherbedarf]
  \label{lem:speicher}
  Der Subgraph-basierte VLSI-Test benÃ¶tigt $O(n^2)$ Speicher fÃ¼r die
  Adjazenzmatrizen, $O(n)$ fÃ¼r die Signaturvektoren und $O(n^2)$ fÃ¼r
  die LCS-DP-Tabelle. Der Gesamtspeicherbedarf betrÃ¤gt $O(n^2)$.
\end{lemma}

\begin{proof}
  Beide Adjazenzmatrizen benÃ¶tigen je $n^2$ EintrÃ¤ge: $O(n^2)$.
  Die Signaturvektoren $\boldsymbol{\sigma}^{\mathrm{ref}}$ und
  $\boldsymbol{\sigma}^{\mathrm{test}}$ haben je $n$ EintrÃ¤ge: $O(n)$.
  Die DP-Tabelle der LCS-Berechnung hat Dimension $(n+1)\times(n+1)$:
  $O(n^2)$. Mit der Optimierung, nur zwei Zeilen der DP-Tabelle zu
  speichern, reduziert sich der Speicher fÃ¼r die LCS auf $O(n)$.
  Der Gesamtspeicherbedarf betrÃ¤gt daher $O(n^2)$. $\square$
\end{proof}

\begin{bemerkung}[VollstÃ¤ndigkeit und Fehlerklassen]
  Die Reduktion ist vollstÃ¤ndig fÃ¼r SA0-Faults (Lemma~\ref{lem:sa0_edge})
  und SA1-Faults (Lemma~\ref{lem:sa1_edge}).
  FÃ¼r BrÃ¼ckenfehler (Lemma~\ref{lem:bridge_edge}) ist die
  Subgraph-Beziehung im Allgemeinen nicht erfÃ¼llt; der Algorithmus
  signalisiert dennoch korrekt einen Fehler.
  Delay Faults erfordern zeitliche Modellierung und liegen auÃŸerhalb
...
de-facto-Standard fÃ¼r die Evaluation von VLSI-Testmethoden.
Tabelle~\ref{tab:iscas} fasst die fÃ¼r unsere Auswertung relevanten
Schaltkreise zusammen.

\begin{table}[ht]
  \centering
  \caption{AusgewÃ¤hlte ISCAS-85 Benchmark-Schaltkreise}
  \label{tab:iscas}
...
Subgraph Algorithmus fÃ¼r verschiedene Fehlerkonfigurationen ausgibt
und wie die theoretischen Lemmata die Ergebnisse begrÃ¼nden.
...
Fehlerdeckungskurven fÃ¼r c17 und c432. Folgende Beobachtungen
lassen sich festhalten:

\begin{itemize}
  \item Der Subgraph-basierte Ansatz erreicht 95\,\% Fehlerdeckung
        auf~c17 mit ca.\ 300 Testmustern; MLFSR benÃ¶tigt ca.\ 500,
        SLFSR ca.\ 800 Muster.
  \item Auf~c432 (160 Gatter) ist der Vorteil noch ausgeprÃ¤gter:
        Subgraph~ca.\ 1100, MLFSR~ca.\ 1800, SLFSR~ca.\ 2500 Muster.
  \item FÃ¼r~c880 (383 Gatter) skaliert der Subgraph-Ansatz dank der
        modularen Dekomposition nach Lemma~\ref{lem:hierarchie} ohne
        quadratischen Overhead; SLFSR leidet unter der wachsenden
        Musterkorrelation deutlich stÃ¤rker.
  \item Die Ãœberlegenheit erklÃ¤rt sich durch die strukturell gezielte
        Fehlersuche: Statt pseudozufÃ¤lliger Testvektoren werden gezielt
...
BIST-Simulation fÃ¼r gleiche Fehler\-deckung.
...
Subgraph Algorithmus profitiert dabei zusÃ¤tzlich von frÃ¼hzeitigem
...
Algorithmus~\cite{epp2026subgraph} eine effiziente LÃ¶sung bietet.
...
\begin{itemize}
  ...
  \item Neue Lemmata: Mehrfachfehler-Erkennung (Lemma~\ref{lem:mehrfach}),
        ...
        SignaturstabilitÃ¤t (Lemma~\ref{lem:relabeling}),
  ...
  \item Korrektheitsbeweis (Theorem~\ref{thm:correctness}) und
        SETH-OptimalitÃ¤t (Satz~\ref{satz:seth})
  \item $O(n^2)$-Speicherbedarf (Lemma~\ref{lem:speicher})
  \item Speedup-Faktor $(n-1)!$ gegenÃ¼ber naivem Ansatz
        (Korollar~\ref{kor:speedup})
  \item VollstÃ¤ndige Python-Implementierung mit Fehlerdiagnosefunktion
...
Alle bisherigen Betrachtungen beschrÃ¤nken sich auf kombinatorische
...
die RÃ¼ckkopplungsschleifen im Schaltkreisgraphen erzeugen. FÃ¼r
...
auf, die die LCS-basierte Vergleichsstrategie grundlegend verÃ¤ndern.
Eine vielversprechende Richtung ist die Verwendung von Strongly
Connected Components (SCCs) als Basiseinheiten der
GraphenreprÃ¤sentation; der Subgraph Algorithmus wÃ¼rde dann auf dem
DAG der SCCs operieren~\cite{cormen2009introduction}. FÃ¼r $k$~SCCs
der GrÃ¶ÃŸe $m_1, \ldots, m_k$ ergibt sich eine Gesamtlaufzeit von
$O\!\bigl(\sum_{l=1}^k m_l^3\bigr)$, was fÃ¼r balancierte SCCs
signifikant gÃ¼nstiger als $O(n^3)$ fÃ¼r den flachen Graphen ist.
...
Pfadanalyse, die fÃ¼r groÃŸe Schaltkreise exponentiell skaliert. Die
Subgraph-basierte Strukturanalyse bietet eine komplementÃ¤re
...
SA0- oder SA1-Faults aktivieren. Ein integrierter ATPG-Workflow kÃ¶nnte
so aussehen: (1)~Subgraph-Analyse zur Strukturunterschied-Detektion,
(2)~Pfadextraktion Ã¼ber die LCS-Differenz (Lemma~\ref{lem:fehlerortung}),
(3)~Testvektorsynthese fÃ¼r die gefundenen Pfade.
Die $O(n^3)$-KomplexitÃ¤ts\-schranke kÃ¶nnte die Skalierbarkeit von
...
Chip. Bekannte Hardware-Architekturen fÃ¼r LCS-Berechnung, z.\,B.\
systolische Arrays~\cite{kung1979systolic}, kÃ¶nnten adaptiert werden:
Ein $n \times n$ systolisches Array mit Prozessorelementen, die
jeweils eine DP-Tabellenzelle berechnen, wÃ¼rde die LCS-Berechnung
in $O(n)$ Taktzyklen (statt $O(n^2)$ sequentiell) ermÃ¶glichen.
Die zyklischen Rotationen kÃ¶nnten durch ein dediziertes Schieberegister
realisiert werden, das parallel zum systolischen Array betrieben wird.
Eine solche Hardware-BIST-Architektur wÃ¼rde die gesamte Fehlerdiagnose
in $O(n^2)$ Taktzyklen (statt $O(n^3)$) abschlieÃŸen.
...
schwer zugÃ¤nglich. Das Subgraph-Modell bietet einen natÃ¼rlichen
...
Ã¼bergeordneten Kanten darstellen. Der Subgraph Algorithmus kann dann
hierarchisch eingesetzt werden: zunÃ¤chst auf Chiplet-Ebene, dann
auf System-Ebene. Die KomplexitÃ¤t bleibt polynomiell in der
GesamtgrÃ¶ÃŸe des Systems.
...
dar und eignen sich als Feature-Vektoren fÃ¼r maschinelle
Lernverfahren zur Fehlerklassifikation. Anstatt nur zu entscheiden,
ob ein Fehler vorliegt, kÃ¶nnten trainierte Klassifikatoren (SVMs,
neuronale Netze) auf Basis der Signaturvektoren prÃ¤zise Vorhersagen
Ã¼ber Fehlertyp und Fehlerposition treffen. Besonders vielversprechend
...
Eine Kombination aus Subgraph Algorithmus (fÃ¼r strukturelle
Vorfilterung) und GNN (fÃ¼r Fehlerdiagnose) kÃ¶nnte die
Diagnosegenauigkeit signifikant verbessern.

\subsubsection{Skalierung auf Sub-Nanometer-Technologien}

Mit dem Ãœbergang zu 2-nm- und 1-nm-Prozesstechnologien nehmen
Fertigungs\-defekte qualitativ neue Formen an: Quanteneffekte
(Tunnel\-strÃ¶me), stochastische VariabilitÃ¤t durch atomic-scale
Variation und verstÃ¤rkte Soft-Error-AnfÃ¤lligkeit durch kosmische
...
SignalstÃ¤rke reprÃ¤sentieren. Ein gewichteter Subgraph Algorithmus
mit angepasster Signaturberechnung kÃ¶nnte probabilistische
...
HierfÃ¼r kÃ¶nnten SAT-Solver-basierte Methoden oder Model Checking
(z.\,B.\ NuSMV) eingesetzt werden. Die Verbindung zwischen formaler
Verifikation (Pre-Silicon) und physischem Testing (Post-Silicon)
durch den gemeinsamen graphen\-theoretischen Rahmen kÃ¶nnte eine
durchgÃ¤ngige, formal fundierte Design-fÃ¼r-Test-Methodik ermÃ¶glichen.
...
OpenROAD oder Yosys, wÃ¼rde die Forschungsgemeinschaft und kleinere
Chip-Hersteller von der $O(n^3)$-Methodik profitieren lassen.
Der Subgraph Algorithmus ist bereits verfÃ¼gbar
unter~\cite{epp2026subgraph_code}.

\subsubsection{Verbindung zur KomplexitÃ¤tstheorie}

Die SETH-bedingte OptimalitÃ¤t (Satz~\ref{satz:seth}) erÃ¶ffnet
grundlegende Fragen: Ist das allgemeine Subgraph-Isomorphieproblem
NP-vollstÃ¤ndig (bekannt fÃ¼r allgemeine Graphen~\cite{cook1971complexity})
auch fÃ¼r die spezielle Klasse der VLSI-Schaltkreisgraphen (DAGs mit
beschrÃ¤nktem In-Degree)? Die EinschrÃ¤nkung auf zyklische Rotationen
statt beliebiger Permutationen kÃ¶nnte der entscheidende Schritt sein,
...
korrekt und vollstÃ¤ndig ist, wÃ¤re ein signifikanter Beitrag zur
...
Kantengewichten (reprÃ¤sentierend Impedanzen, VerstÃ¤rkungsfaktoren
oder Transconductanzen) kÃ¶nnten in ein gewichtetes Subgraph-Framework
eingebettet werden. Anstelle der booleschen Signifikanzgewichtung
$\sum A_{ij} \cdot 2^i$ wÃ¼rde eine kontinuierliche
