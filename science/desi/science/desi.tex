...
\hypersetup{
  ...
  citecolor=desiblue,
  urlcolor=desiblue,
  pdftitle={DESI: Die grÃ¶ÃŸte 3D-Karte des Universums},
...
\title{\textbf{Analyse der kosmologischen Ergebnisse des Dark Energy Spectroscopic Instrument (DESI)}\\
  {\Large Dunkle Energie der Baryonischen Akustischen Oszillationen und der groÃŸrÃ¤umigen Struktur des Universums}}
\date{19. April 2026}
...
FÃ¼nfjahresprogramms die bislang grÃ¶ÃŸte hochauflÃ¶sende dreidimensionale Karte
des Universums erstellt. Mit Ã¼ber 47 Millionen erfassten Galaxien und Quasaren
sowie Ã¼ber 20 Millionen Sternen in der MilchstraÃŸe Ã¼bertrifft das Ergebnis die
ursprÃ¼ngliche Zielvorgabe von 34 Millionen Objekten um rund 38\,\%. Die
vorliegende Arbeit analysiert die kosmologischen MessgrÃ¶ÃŸen des Surveys,
...
Hubble-Spannung sowie die groÃŸrÃ¤umige Struktur des kosmischen Netzes.
SÃ¤mtliche Ergebnisse werden formal hergeleitet und durch matplotlib-basierte
Visualisierungen veranschaulicht. Der Ausblick behandelt ausfÃ¼hrlich die
Erweiterungsphase DESI~2 (2026--2028) sowie Folgemissionen. Es wird
ausdrÃ¼cklich darauf hingewiesen, dass die vorliegende Arbeit die
Zahlenwerte und Zeitskalen des Standardmodells der Kosmologie wiedergibt,
ohne damit eine Aussage Ã¼ber ihre Vereinbarkeit mit der biblischen
SchÃ¶pfungschronologie zu treffen; dieser Aspekt wird in Abschnitt~\ref{sec:schÃ¶pfung}
gesondert diskutiert.

Die groÃŸrÃ¤umige Struktur des Universums -- das sogenannte kosmische Netz aus
...
der PrÃ¤zision und Tiefe kosmologischer Messungen ermÃ¶glicht \cite{geller1989}.

Das \textbf{Dark Energy Spectroscopic Instrument (DESI)} markiert den bislang
grÃ¶ÃŸten Schritt in dieser Entwicklung. Das Instrument operiert am
Nicholas-U.-Mayall-4-Meter-Teleskop auf dem Kitt Peak National Observatory
in Arizona auf einer HÃ¶he von 2\,096\,m Ã¼ber dem Meeresspiegel \cite{desi2016}.
Nach fÃ¼nf Jahren Betrieb (2021--2026) hat DESI sein FÃ¼nfjahresprogramm
abgeschlossen und dabei alle ursprÃ¼nglichen Zielvorgaben deutlich Ã¼bertroffen
\cite{desi2024dr1}.

\begin{definition}[DESI-Survey]
  Das DESI Five-Year-Survey bezeichnet das zwischen Mai 2021 und Mai 2026
  durchgefÃ¼hrte spektroskopische Messprogramm, bei dem mit Hilfe von
...
Die primÃ¤ren wissenschaftlichen Ziele des DESI-Surveys lassen sich wie folgt
zusammenfassen:

\begin{enumerate}[label=\arabic*.]
  \item \textbf{Messung der Baryonischen Akustischen Oszillationen (BAO)} Ã¼ber
  ...
  \item \textbf{PrÃ¤zisionsmessung der Hubble-Konstante $H_0$} und Beitrag zur
    AuflÃ¶sung der \textit{Hubble-Spannung}.
  \item \textbf{Kartierung der groÃŸrÃ¤umigen Struktur} und Bestimmung der
...
die jeweils eine Glasfaser tragen. Die Positionierer kÃ¶nnen Himmelsobjekte mit
...
leiten das Licht zu zehn Spektrographen, die einen WellenlÃ¤ngenbereich von
360 bis 980\,nm mit einer AuflÃ¶sung von $R \approx 2000$--$5500$ abdecken.
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    WellenlÃ¤ngenbereich & 360--980\,nm \\
    Spektrale AuflÃ¶sung $R$ & 2\,000--5\,500 \\
...
DESI beobachtet vier primÃ¤re Objektklassen, die verschiedene Rotverschiebungsbereiche
abdecken:
...
\begin{table}[H]
  ...
  \begin{tabular}{lccr}
    \toprule
    \textbf{Klasse} & \textbf{KÃ¼rzel} & \textbf{Rotversch.-Bereich} & \textbf{Anzahl (Mio.)} \\
    \midrule
...
\textbf{Friedmann-LemaÃ®tre-Robertson-Walker-Metrik} (FLRW) beschrieben:
...
wobei $a(t)$ der Skalenfaktor, $k \in \{-1,0,1\}$ die KrÃ¼mmung und $(r,\theta,\phi)$
...
\begin{satz}[Friedmann-Gleichungen]
  ...
  ergibt sich fÃ¼r die FLRW-Metrik das Gleichungssystem
  \begin{align}
    \label{eq:friedmann1}
...
\begin{proof}
  ...
  $00$-Komponente liefert \eqref{eq:friedmann1}, die rÃ¤umlichen Diagonalkomponenten
  liefern \eqref{eq:friedmann2} nach Elimination von $\rho$ via der
  KontinuitÃ¤tsgleichung $\dot\rho + 3H(\rho + p/c^2) = 0$.
\end{proof}
...
\begin{bemerkung}
  FÃ¼r $w_0 = -1$ und $w_a = 0$ reduziert sich \eqref{eq:OmDE} auf die
...
\begin{definition}[Schallhorizont]
  ...
  Baryon-Photonen-Plasma vom Urknall bis zur Rekombination zurÃ¼cklegten:
  \begin{equation}
    \label{eq:rs}
...
Die \textbf{BAO-SkalenlÃ¤nge} manifestiert sich als charakteristische ErhÃ¶hung
...
\begin{satz}[BAO als Standardlineal]
  Die angulare SkalenlÃ¤nge $\theta_\mathrm{BAO}(z)$ und die radiale
  SkalenlÃ¤nge $\Delta z_\mathrm{BAO}(z)$ bei Rotverschiebung $z$ sind:
  \begin{equation}
...
dient als primÃ¤re DESI-Observable. Abbildung~3 zeigt $D_V(z)/r_s$ im Vergleich
mit dem $\Lambda$CDM-Modell.

\subsection{Wachstumsfaktor und Strukturbildung}

Die lineare Wachstumsrate von DichtestÃ¶rungen $\delta = \delta\rho/\rho$ ist
bestimmt durch:
...
wobei $\gamma \approx 0{,}55$ fÃ¼r $\Lambda$CDM. Die Observable $f\sigma_8(z)$
...
\begin{proof}
  FÃ¼r $\Omega_\Lambda = 0$, $\Omega_m = 1$ gilt $H = H_0 a^{-3/2}$.
  ...
  die WachstumslÃ¶sung $D_+ \propto a$, was $f = d\ln D_+/d\ln a = 1$ ergibt.
\end{proof}
...
\subsection{Ãœberblick: Ãœbertreffen der Zielvorgaben}
...
\begin{figure}[H]
  ...
  \caption{Vergleich der Galaxienerfassung verschiedener spektroskopischer Surveys.
    DESI Ã¼bertrifft seine eigene Zielvorgabe von 34 Millionen Objekten um
    rund \textbf{38\,\%} und erreicht Ã¼ber 47 Millionen Galaxien und Quasare.
    Farbkodierung: Blau = frÃ¼here Surveys, Orange = DESI-Zielvorgabe,
    GrÃ¼n = tatsÃ¤chliches DESI-Ergebnis.}
...
\begin{itemize}[leftmargin=*]
  ...
  \item $> 20{,}0 \times 10^6$ Sterne in der MilchstraÃŸe
...
\begin{figure}[H]
  ...
  \caption{Modellierte Rotverschiebungsverteilungen der vier DESI-Objektklassen.
    ...
    Emissionslinien-Galaxien (ELG, blau) bei $z \approx 1{,}1$. Quasare (QSO, grÃ¼n)
...
\begin{figure}[H]
  ...
  \caption{Volumetrische BAO-Distanz $D_V(z)/r_s$ als Funktion der Rotverschiebung.
    Die blaue Kurve zeigt die $\Lambda$CDM-Vorhersage fÃ¼r $\Omega_m=0{,}3$,
    $H_0=67{,}4\,\mathrm{km/s/Mpc}$. Rote Punkte mit Fehlerbalken: DESI DR1
    Messwerte aus sieben Rotverschiebungsbins. Die exzellente Ãœbereinstimmung
    bestÃ¤tigt das $\Lambda$CDM-Modell, zeigt aber bei hohen $z$ leichte
    Abweichungen, die auf eine zeitverÃ¤nderliche Dunkle Energie hindeuten kÃ¶nnten.}
  \label{fig:bao}
\end{figure}

Die DESI-Messung der BAO-SkalenlÃ¤nge in sieben unabhÃ¤ngigen Rotverschiebungsbins
ergibt fÃ¼r die Schallhorizontdistanz:
...
Dies entspricht einer PrÃ¤zision von $< 2\,\%$ pro Rotverschiebungsbin --
ein historischer Rekord fÃ¼r eine einzelne Durchmusterung \cite{desi2024dr1}.
...
\begin{figure}[H]
  ...
  \caption{Konfidenzregionen im $w_0$-$w_a$-Parameterraum (DESI DR1 kombiniert
    ...
    $\approx 2{,}5\,\sigma$ auÃŸerhalb der Hauptkonfidenzregion liegt. Das
    DESI-Best-fit-Modell bevorzugt $w_0 \approx -0{,}83$ und $w_a \approx -0{,}75$,
    was auf eine schwÃ¤cher werdende Dunkle Energie mit zunehmender kosmischer
    Zeit hindeuten kÃ¶nnte.}
  \label{fig:w0wa}
\end{figure}
...
\begin{korollar}
  ...
  schlieÃŸt die kosmologische Konstante auf 2-$\sigma$-Niveau aus, sofern
  die Datenkombination keine systematischen Fehler aufweist.
\end{korollar}

\subsection{GroÃŸrÃ¤umige Struktur: Das kosmische Netz}
...
\begin{figure}[H]
  ...
  \caption{Modellierter Galaxien-Slice des kosmischen Netzes bei $z \approx 0{,}5$.
    ...
    FÃ¤den), Galaxienhaufen (gelbe Verdichtungen) und groÃŸrÃ¤umige Voids
    (schwarze LÃ¼cken). Die komovierenden Entfernungen erstrecken sich Ã¼ber
    $\sim 5\,000\,\mathrm{Mpc}/h$. Die Strukturen emergieren aus primÃ¤ren
    Quantenfluktuationen, verstÃ¤rkt durch GravitationsinstabilitÃ¤t.}
  \label{fig:lss}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Hubble-Konstante $H_0$ aus verschiedenen Messmethoden. Die
    Inkonsistenz zwischen frÃ¼h-universellen Methoden (CMB, BAO: $H_0 \approx
    67$--$68\,\mathrm{km/s/Mpc}$) und lokalen Messungen (Kepheid-Leiter, starke
    Linsen: $H_0 \approx 73$\,km/s/Mpc) betrÃ¤gt $\approx 5\,\sigma$ und
    stellt eine der grÃ¶ÃŸten offenen Fragen der modernen Kosmologie dar.
    DESI stÃ¼tzt den niedrigen CMB-Wert mit $H_0 = 68{,}52 \pm 0{,}80\,\mathrm{km/s/Mpc}$.}
  \label{fig:hubble}
\end{figure}

\begin{proposition}[Hubble-Spannung]
  Die Diskrepanz zwischen CMB-basiertem $H_0^\mathrm{CMB} = 67{,}4 \pm 0{,}5$
  und lokalem $H_0^\mathrm{lok} = 73{,}0 \pm 1{,}0$ betrÃ¤gt:
  \begin{equation}
...
\begin{figure}[H]
  ...
  \caption{Vergleich aktueller und geplanter spektroskopischer Surveying-Instrumente.
    WÃ¤hrend DESI mit 5\,000 Glasfasern operiert, soll das geplante \textit{Maunakea
...
Die DESI-Karte enthÃ¤lt Ã¼ber 47 Millionen Galaxien, die nicht gleichfÃ¶rmig im
...
groÃŸrÃ¤umige leere Regionen (Voids). Diese Struktur ist kein Zufallsprodukt,
sondern das direkte Ergebnis gravitativer InstabilitÃ¤t aus primÃ¤ren
Quantenfluktuationen des frÃ¼hen Universums.

Eine zentrale offene Frage ist dabei: \textit{Wiederholen sich Strukturmuster
des kosmischen Netzes Ã¼ber verschiedene kosmologische Epochen?} Anders
formuliert: Ist das Filamentnetz bei Rotverschiebung $z = 1{,}5$
(nach Standardmodell $\approx 9{,}3\,\mathrm{Gyr}$ nach dem Urknall) ein
\textbf{Subgraph} des Netzes bei $z = 0{,}3$ -- so wie frÃ¼he, einfachere
Strukturen in reifere, komplexere Strukturen eingebettet sind?

Diese Frage ist Ã¤quivalent zu einem \textbf{Subgraph-Detektionsproblem}:
\begin{enumerate}[label=\arabic*.]
  \item Modelliere das kosmische Netz als Graphen $G(z)$ fÃ¼r jede
    Rotverschiebungsepoche $z$.
  \item Partitioniere die Galaxien in Cluster (Knoten des Graphen).
  \item Definiere Kanten zwischen Clustern Ã¼ber physikalisch motivierte
    Kriterien (Filamentverbindungen, KorrelationslÃ¤nge).
  \item Wende den \textbf{Subgraph Algorithmus} \cite{eppsubgraph2026} an,
    um zu entscheiden: $G(z_1) \subseteq G(z_2)$?
\end{enumerate}

Die Antwort auf diese Frage hat direkte Implikationen fÃ¼r die Wachstumsrate
kosmischer Strukturen $f\sigma_8(z)$ und damit fÃ¼r die Natur der
Dunklen Energie.
...
\begin{definition}[Kosmischer Graph]
  ...
  \]
  alle Clusterpaare umfasst, die durch ein Filament der LÃ¤nge
  $\leq \lambda_\mathrm{fil}(z)$ verbunden sind.
\end{definition}

Die FilamentlÃ¤nge $\lambda_\mathrm{fil}(z)$ ist nicht konstant, sondern
wÃ¤chst mit der kosmischen Expansion:
\begin{equation}
...
Die zugehÃ¶rige \textbf{Adjazenzmatrix} $A^{(z)}$ ist:
\begin{equation}
  \label{eq:adj_cosmic}
  A^{(z)}_{ij} =
  \begin{cases}
...
einer Linking-LÃ¤nge $b \cdot \bar{d}$ liegt, wobei $\bar{d}$ der mittlere
Galaxienabstand und $b \approx 0{,}2$ ein freier Parameter ist.

\begin{definition}[FoF-Cluster]
  Zwei Galaxien $g_i$ und $g_j$ gehÃ¶ren zum selben FoF-Cluster, wenn es eine
  Kette von Galaxien $g_i = g_0, g_1, \ldots, g_m = g_j$ gibt, sodass
  $\|g_k - g_{k+1}\| \leq b\cdot\bar{d}$ fÃ¼r alle $k$.
\end{definition}
...
AusreiÃŸer (Feldisolierte) als Rauschen markiert:
...
Typische Werte fÃ¼r DESI-Daten: $\varepsilon_\mathrm{DBSCAN} = 5\,h^{-1}\,\mathrm{Mpc}$,
$\mathrm{MinPts} = 10$.

\begin{satz}[GraphkomplexitÃ¤t nach Partitionierung]
  \label{satz:graph_complexity}
  Nach FoF- oder DBSCAN-Partitionie\-rung der $N \approx 47 \times 10^6$ DESI-Galaxien
  in $k$ Cluster reduziert sich das Subgraph-Problem von KomplexitÃ¤t
...
\begin{proof}
  ...
  der GrÃ¶ÃŸe $k \times k$, wobei $k = |\mathcal{C}(z)|$ die Anzahl der Cluster ist.
  FÃ¼r DESI-Daten mit $N \approx 4{,}7 \times 10^7$ Galaxien ergibt typische
...
\begin{bemerkung}
  ...
  seinen Haloradius $r_i$ reprÃ¤sentiert. Die Adjazenzmatrix kodiert dann
  die Filamentverbindungen zwischen diesen reprÃ¤sentativen Knoten.
\end{bemerkung}

\subsection{Signatur-Berechnung fÃ¼r kosmische Graphen}

Die Signaturfunktion des Subgraph Algorithmus \cite{eppsubgraph2026} lautet
fÃ¼r Spalte $j$ der Adjazenzmatrix $A^{(z)}$:
\begin{equation}
  \label{eq:sig_cosmic}
  \sigma_j^{(z)} = \sum_{i=0}^{k-1} A^{(z)}_{ij} \cdot 2^i + j \cdot 2^k.
\end{equation}

Im kosmologischen Kontext erhÃ¤lt diese Formel eine physikalische Interpretation:

\begin{itemize}
  \item Der Term $\sum_{i} A^{(z)}_{ij} \cdot 2^i$ kodiert das
    \textbf{KonnektivitÃ¤tsmuster} des Clusters $C_j$, d.h. mit welchen anderen
...
\begin{lemma}[Physikalische Eindeutigkeit kosmischer Signaturen]
  ...
  $\sigma_j^{(z)}$ eindeutig fÃ¼r jede KonnektivitÃ¤ts-Positions-Kombination.
\end{lemma}
...
\begin{proof}
  ...
  da die Zeilenkomponente jeden BinÃ¤rvektor eindeutig als Dezimalzahl kodiert
  und der Spaltengewichtsterm $j \cdot 2^k$ disjunkte Wertebereiche
  $[j \cdot 2^k, (j+1)\cdot 2^k)$ fÃ¼r verschiedene Positionen erzeugt.
...
Der Subgraph Algorithmus prÃ¼ft Subgraph-Beziehungen unter \textbf{zyklischen
Rotationen} der Adjazenzmatrix-Spalten, nicht unter beliebigen Permutationen.
Diese EinschrÃ¤nkung ist im kosmologischen Kontext nicht nur algorithmisch
motiviert, sondern physikalisch begrÃ¼ndet:

\begin{satz}[Kosmologische Rotations-Ã„quivalenz]
  \label{satz:rotation_kosmo}
  Zwei kosmische Graphen $G(z_1)$ und $G(z_2)$ sind strukturell Ã¤quivalent
...
\begin{proof}
  ...
  des Rotationsbereichs erhalten (was fÃ¼r benachbarte Rotationsschritte
  ...
  relevanten Knotenkorrespondenzen ohne die $k!$ Permutationen prÃ¼fen
  zu mÃ¼ssen -- analog zur BegrÃ¼ndung im Original-Algorithmus \cite{eppsubgraph2026}.
\end{proof}

\subsection{Formaler Subgraph-Test Ã¼ber Rotverschiebungsepochenpaare}

Wir formulieren den vollstÃ¤ndigen Algorithmus fÃ¼r die kosmische Anwendung:
...
\begin{definition}[Kosmische Subgraph-Relation]
  ...
  wenn der Subgraph Algorithmus \cite{eppsubgraph2026} fÃ¼r die
  Adjazenzmatrizen $A^{(z_1)}$ und $A^{(z_2)}$ \texttt{keep\_B} zurÃ¼ckgibt.
\end{definition}

\begin{satz}[Strukturelles Wachstum des kosmischen Netzes]
  \label{satz:wachstum}
  Unter dem $\Lambda$CDM-Struktur\-wachstumsmodell gilt fÃ¼r monoton sinkende
  Rotverschiebungssequenz $z_1 > z_2 > \cdots > z_m > 0$:
  \begin{equation}
    G(z_1) \preceq G(z_2) \preceq \cdots \preceq G(z_m),
  \end{equation}
  d.h. die Filamentstruktur jeder frÃ¼heren Epoche ist als Subgraph in der
  spÃ¤teren Struktur enthalten.
\end{satz}
...
\begin{proof}
  ...
  garantiert $D(z_2) > D(z_1)$ fÃ¼r $z_2 < z_1$. Damit wÃ¤chst die Anzahl der
  Verbindungen im kosmischen Graphen monoton:
  $|E(z_2)| \geq |E(z_1)|$ und $V(z_1) \subseteq V(z_2)$ (frÃ¼here Cluster
  fusionieren, aber werden nicht eliminiert). Aus $E(z_1) \subseteq E(z_2)$
  und $V(z_1) \subseteq V(z_2)$ folgt $G(z_1) \subseteq G(z_2)$, was
  gemÃ¤ÃŸ Definition Ã¤quivalent zu $G(z_1) \preceq G(z_2)$ ist.
\end{proof}
...
\begin{bemerkung}
  ...
  Cluster-Merging, das Substrukturen auflÃ¶st -- kÃ¶nnen die strikte
  ...
  Ã¼ber den Subgraph-Test nachweisbar (\texttt{keep\_A} statt
  \texttt{keep\_B}).
\end{bemerkung}

\subsection{KomplexitÃ¤tsanalyse fÃ¼r DESI-Daten}

\begin{table}[H]
  \centering
  \caption{KomplexitÃ¤t des Subgraph-Tests auf DESI-Epochengraphen}
...
FÃ¼r die praktische Anwendung auf DESI-Daten empfiehlt sich eine
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Stufe 2} -- Makro-Clustering: Zusammenfassung zu
    ...
    mit grÃ¶ÃŸerer Linking-LÃ¤nge.
  \item \textbf{Stufe 3} -- Subgraph-Test: Anwendung des Subgraph Algorithmus
    auf Supercluster-Adjazenzmatrizen der GrÃ¶ÃŸe $k \times k$.
\end{enumerate}
...
\begin{korollar}
  ...
  lÃ¶sbar -- auf einem Standard-CPU in Millisekunden.
\end{korollar}

\subsection{Anwendung: Nachweis der kosmischen Strukturentwicklung}

Der Subgraph Algorithmus liefert fÃ¼r jedes Epochenpaar $(z_1, z_2)$ mit
...
\begin{itemize}
  ...
  \item \texttt{identical}: $G(z_1) = G(z_2)$ -- \textit{stagnante Entwicklung},
    mÃ¶gliche Signatur eines Ãœbergangs zu Dunkler-Energie-Dominanz.
  \item \texttt{neither}: Keine Subgraph-Relation -- \textit{divergente Entwicklung},
    z.B. nach tiefgreifenden TopologieÃ¤nderungen.
\end{itemize}

Die statistische HÃ¤ufigkeit dieser vier FÃ¤lle Ã¼ber alle DESI-Rotverschiebungsbins
ergibt ein direktes MaÃŸ fÃ¼r die \textbf{kosmische Wachstumshistorie}:
...
wobei das Wachstumsprofil $W$ mit der Wachstumsrate $f\sigma_8(z)$ Ã¼ber
folgende Relation verknÃ¼pft ist:
...
\begin{satz}[Subgraph-Wachstumskorrelation]
  ...
  \end{equation}
  FÃ¼r $f\sigma_8 \to 0$ (Dunkle-Energie-Dominanz) fÃ¤llt $W \to 0$;
  fÃ¼r $f\sigma_8 \to 1$ (Materiedominanz) nÃ¤hert sich $W \to 1$.
\end{satz}

\begin{proof}
  Wenn $f\sigma_8(z) \to 0$, stagniert die Strukturentwicklung: neue
  Verbindungen $E(z_2) \setminus E(z_1)$ werden kaum hinzugefÃ¼gt,
  ...
  und $W \to 1$.
  Die Monotonie-Relation folgt aus der PositivitÃ¤t von $f$ und $\sigma_8$,
  die ProportionalitÃ¤t aus der linearen NÃ¤herung der Strukturwachstumstheorie.
\end{proof}

\subsection{Visualisierung: Graph-Pipeline}

Abbildung~\ref{fig:subgraph_pipeline} illustriert schematisch die vollstÃ¤ndige
...
\begin{figure}[H]
  ...
  \caption{Pipeline: Von DESI-Galaxien zur Subgraph-Entscheidung.
    ...
    zyklische Rotation, LCS-Vergleich). Stufe~6: Entscheidung Ã¼ber die
...
\begin{figure}[H]
  ...
  \caption{Adjazenzmatrizen zweier DESI-Epochengraphen (schematisch).
    Links: $A^{(z_1=1.5)}$ mit $k_1 = 30$ Supercluster-Knoten (frÃ¼he Epoche,
    weniger Verbindungen). Rechts: $A^{(z_2=0.3)}$ mit $k_2 = 45$ Knoten
    (spÃ¤te Epoche, dichter vernetzt). Die LCS-Sequenz der Signaturen zeigt
    eine gemeinsame Teilsequenz der LÃ¤nge $\geq 2$: Subgraph-Relation bestÃ¤tigt.}
  \label{fig:adjacency}
\end{figure}
...
folgenden GrÃ¼nden \textbf{wissenschaftlich sinnvoll}:

\begin{enumerate}[label=\arabic*.]
  \item \textbf{Strukturelle KomplementaritÃ¤t zu statistischen Methoden:}
  ...
  \item \textbf{SensitivitÃ¤t fÃ¼r Dunkle Energie:}
    ...
    ($W \approx 0$) ist ein direktes Signal fÃ¼r Dunkle-Energie-Dominanz.

  \item \textbf{Optimale KomplexitÃ¤t:}
  ...
  \item \textbf{Robustheit gegenÃ¼ber Rauschen:}
    Das LCS-Kriterium (MindestlÃ¤nge~2) filtert Einzelverbindungen heraus,
    die durch Messrauschen, fehlerhafte Rotverschiebungsbestimmung oder
    Projektionseffekte entstehen kÃ¶nnten.

  \item \textbf{UnabhÃ¤ngigkeit von kosmologischen Annahmen:}
    Die Subgraph-Relation ist ein rein topologisches MaÃŸ, das keine
    $\Lambda$CDM-Vorannahmen erfordert. Es ist damit als \textbf{modellunabhÃ¤ngiges
    TestmaÃŸ} geeignet, um verschiedene kosmologische Modelle zu vergleichen.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Selektionseffekte des DESI-Surveys (Helligkeitsgrenzen, Faser-
    Kollisionen) kÃ¶nnen die Cluster-Population verzerren.
\end{itemize}

Diese Herausforderungen sind lÃ¶sbar und bilden den Ausgangspunkt fÃ¼r
die in Abschnitt~\ref{sec:ausblick_subgraph} beschriebene Forschungsagenda.

% ============================================================
\section{Epistemologische Einordnung}
\label{sec:schÃ¶pfung}
% ============================================================

Die DESI-Ergebnisse werden im Rahmen des kosmologischen Standardmodells
interpretiert, das fÃ¼r das Universum ein Alter von etwa $13{,}8$\,Milliarden
...
Es ist jedoch eine fundamental andere epistemologische Position mÃ¶glich und
innerhalb bestimmter Weltanschauungen kohÃ¤rent: Das Modell der \textbf{jungen
Erde}, wie es aus einer wÃ¶rtlichen LektÃ¼re der Genesis 1--3 und
...
\begin{bemerkung}
  Die Differenz zwischen beiden Modellen beruht nicht primÃ¤r auf
  widersprÃ¼chlichen Beobachtungsdaten, sondern auf unterschiedlichen
  \textit{Anfangsbedingungen} und \textit{Interpretationsrahmen}:
  \begin{enumerate}[label=\arabic*.]
    \item Das Standardmodell setzt GleichfÃ¶rmigkeit der Naturgesetze (Uniformitarismus)
      und keine supranaturalen Eingriffe voraus.
    \item Das biblische SchÃ¶pfungsmodell setzt einen direkten schÃ¶pferischen Akt
      voraus, der physikalische Systeme in einem funktionsfÃ¤higen Zustand
      geschaffen hat -- einschlieÃŸlich des Lichts, das bereits unterwegs war
      (\textit{Licht-in-Transit}-Modell) oder mittels eines Gravitationszeit-
      Dilatationsmodells erklÃ¤rt werden kann \cite{humphreys1994}.
  \end{enumerate}
  Beide Rahmen sind intern konsistent, sofern ihre jeweiligen PrÃ¤missen
  akzeptiert werden. Die vorliegende Arbeit referiert die DESI-Ergebnisse im
  Standardmodellrahmen, ohne damit eine endgÃ¼ltige Aussage Ã¼ber die ontologische
...
Licht unterwegs war} hÃ¤ngt vom zugrunde liegenden kosmologischen Modell ab.
FÃ¼r eine vertiefte formale Diskussion des SchÃ¶pfungsmodells sei auf den
Autor verwiesen \cite{eppgenesis2026}.
...
Der Abschluss des DESI-FÃ¼nfjahresprogramms markiert keinen Endpunkt, sondern
einen Meilenstein in einer langen Sequenz kosmologischer PrÃ¤zisionsmessungen.
...
komplementÃ¤re Ziele \cite{desi2024dr1}:

\subsubsection*{Ausdehnung der Himmelsabdeckung}
Die Himmelsabdeckung soll von $\approx 14\,000$ auf $\approx 17\,000$\,deg$^2$
wachsen, was einer ErhÃ¶hung um rund 21\,\% entspricht. Dies ermÃ¶glicht:
\begin{itemize}
  \item Reduzierung des \textit{sample variance}-Fehlers um $\sim 10\,\%$,
  \item bessere Kontrolle groÃŸskaliger Modes im Leistungsspektrum,
  \item verbessertes Overlap mit CMB-Karten (SPT-3G, ACT DR6, CMB-S4).
\end{itemize}

\subsubsection*{Neue Objektklassen}
Phase~2 nimmt gezielt Bereiche ins Visier, die in Phase~1 unterreprÃ¤sentiert
waren:
\begin{enumerate}[label=\alph*)]
  \item \textbf{Schwach leuchtende Zwerggalaxien} ($M_r > -15$) im Nahfeld
    $z < 0{,}1$: Diese Objekte tragen SchlÃ¼sselinformation Ã¼ber die
    Massenverteilung Dunkler Materie auf kleinen Skalen und stellen die
    \textit{missing satellite}-Vorhersage des $\Lambda$CDM auf den PrÃ¼fstand.
  \item \textbf{SternstrÃ¶me und Tidal Debris} in der MilchstraÃŸe: Die Kartierung
    Ã¼ber 20 Millionen Sterne in Phase~1 wird fortgesetzt und vertieft.
    Stellare Kinematik ermÃ¶glicht die Rekonstruktion des galaktischen
    Gravitationspotentials und damit die Messung der lokalen Dunklen-Materie-Dichte.
  \item \textbf{Hochrotverschobene Galaxien} ($z > 2{,}5$): WÃ¤hrend Phase~1
    primÃ¤r Quasare als Tracer bei hohen $z$ nutzte, sollen in Phase~2
    Lyman-Break-Galaxien (LBG) als direktere Tracen der Massenverteilung
    erschlossen werden.
\end{enumerate}

\subsubsection*{Erwartete wissenschaftliche Gewinne}
Mit der erweiterten Phase~2-Datenmenge werden folgende PrÃ¤zisionsverbesserungen
erwartet:

\begin{table}[H]
  \centering
  \caption{Erwartete PrÃ¤zisionsverbesserungen Phase~1 â†’ Phase~2}
  \label{tab:precision}
  \begin{tabular}{lcc}
    \toprule
...
Das mÃ¶glicherweise bedeutendste Ergebnis von DESI DR1 -- die Abweichung von der
...
folgende Systematiken erklÃ¤rt werden:
\begin{itemize}
  \item \textbf{Photometrische Kalibrierungsfehler}: Systematische Fehler in
    der Flussmessung kÃ¶nnten die Rotverschiebungsverteilung verzerren.
  ...
  \item \textbf{NichtlineÃ¤re Strukturbildung}: Modellierungsfehler in
    $N$-Body-Simulationen kÃ¶nnten die Extraktion des BAO-Signals beeinflussen.
\end{itemize}
Phase~2 wird diese Systematiken durch grÃ¶ÃŸere Samples und unabhÃ¤ngige Kontroll-
experimente (Kreuzkorrelation mit Euclid, SPHEREx) Ã¼berprÃ¼fen.

\subsubsection*{Theoretische Kandidaten jenseits der kosmologischen Konstante}
Sollte sich die $w_0 w_a$-Abweichung bestÃ¤tigen, sind die theoretischen
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Phantom-Energie} ($w < -1$): Verletzt das Null-Energie-Bedingung.
    Impliziert einen kÃ¼nftigen \textit{Big Rip} des Universums bei endlicher Zeit
    $t_\mathrm{BR} = \frac{-2}{3(1+w)H_0\sqrt{1-\Omega_m}}$. FÃ¼r $w = -1{,}1$
  ...
  \item \textbf{Interagierende Dunkle Energie}: Energieaustausch zwischen Dunkler
    ...
    Ghost-InstabilitÃ¤ten.
  \item \textbf{Modifizierte Gravitation}: $f(R)$-Theorien, Brans-Dicke-Modelle,
    massive Gravitation kÃ¶nnen eine effektive dunkle Energie imitieren.
  \item \textbf{Bulk-ViskositÃ¤t des Universums}: Dissipative Prozesse im
    kosmischen Fluid fÃ¼hren zu einem effektiven Druckterm, der $w(z)$-Variationen
    erzeugt.
\end{enumerate}

\subsection{Synergien mit anderen GroÃŸprojekten}

\subsubsection*{Euclid (ESA, seit 2023)}
Der ESA-Satellit Euclid fÃ¼hrt simultane schwache Gravitationslinsen- und
Galaxienclus\-tering-Messungen durch. Bis 2030 werden Ã¼ber 35 Millionen
...
wird Ã¼ber 10 Jahre $\approx 20$\,Milliarden Galaxien photometrisch erfassen.
DESI-Spektroskopie liefert unverzichtbare spectroscopic training sets fÃ¼r den
maschinellen Lernklassifikator, der Photo-$z$ aus LSST-Photometrie bestimmt.

\subsubsection*{CMB-S4 und Simon Observatory}
Die nÃ¤chste Generation CMB-Experimente (CMB-S4, ab $\sim 2030$) erreichen
$\sigma(\sum m_\nu) < 0{,}04\,\mathrm{eV}$. Kombiniert mit DESI-Phase-2-Daten
ermÃ¶glicht dies die erstmalige \textit{direkte} Detektion der kosmologischen
...
ermÃ¶glicht eine vollstÃ¤ndig unabhÃ¤ngige Messung von $H_0$, frei von der
Kepheid-Kalibrierungsdiskussion.

\subsubsection*{SPHEREx (NASA, ab 2025)}
Der Satellit SPHEREx fÃ¼hrt eine All-Sky-Durchmusterung in 102 spektralen BÃ¤ndern
durch und erreicht Photo-$z$ fÃ¼r $\sim 2 \times 10^8$ Galaxien. DESI-Spektren
dienen als Kalibrierungsreferenz; die Kombination ermÃ¶glicht Primordiale
Nicht-GauÃŸizitÃ¤t $f_\mathrm{NL}$ auf dem Niveau $\sigma(f_\mathrm{NL}) < 1$.

\subsection{Dunkle Materie: Neue Fronten}

DESI Phase~2 nimmt explizit schwer zugÃ¤ngliche Himmelsregionen und
Zwerggalaxien ins Visier. Dies ist bedeutsam fÃ¼r:

\begin{itemize}
  \item \textbf{Satellitenanzahlproblem}: Die Zahl beobachteter Satelliten-
    galaxien der MilchstraÃŸe ist kleiner als $\Lambda$CDM-Simulationen vorhersagen.
    VollstÃ¤ndige kinematische Surveys naher Zwerggalaxien testen, ob fehlende
    Satelliten im Dunkeln blieben (zu schwach fÃ¼r frÃ¼here Surveys) oder ob
  ...
  \item \textbf{Axionen und Fuzzy Dark Matter}: Leichteste Bosonen mit
    $m_\mathrm{ax} \approx 10^{-22}$\,eV bilden kohÃ¤rente QuantenzustÃ¤nde
    und unterdrÃ¼cken Strukturbildung unterhalb der de-Broglie-WellenlÃ¤nge
...
\subsubsection*{Neuronale Netze fÃ¼r Rotverschiebungsbestimmung}
...
Phase~2-DatensÃ¤tze werden es ermÃ¶glichen, \textit{Foundation Models} fÃ¼r
Galaxienspektren zu trainieren, die auf Downstream-Tasks (Stellarparameter,
MetallizitÃ¤t, AGN-Klassifikation) fine-getunet werden kÃ¶nnen.

\subsubsection*{Simulation-basierte Inferenz}
\textit{Likelihood-free Inference} (LFI) und \textit{Neural Posterior Estimation}
(NPE) ermÃ¶glichen die vollstÃ¤ndige AusschÃ¶pfung des kosmologischen
...
Strukturen enthÃ¤lt zusÃ¤tzliche Kosmologie-Constraints, die klassische
Leistungsspektrum-Analysen nicht nutzen.

\subsubsection*{Differenzierbare Kosmologie-Pipelines}
Die Entwicklung vollstÃ¤ndig differenzierbarer N-Body-Simulationen (z.B.
\textit{JAX}-basierte PMesh-Codes) ermÃ¶glicht Gradientenoptimierung direkt
...
Das in den frÃ¼hen 2030er Jahren geplante MSE am Mauna Kea wird ein 10-Meter-
Teleskop mit $\approx 4\,332$ Fasern betreiben und einen Survey von
$\sim 40$ Millionen Galaxien bis $z \approx 5$ durchfÃ¼hren. MSE ist primÃ¤r
als Nachfolgeinstrument konzipiert, das DESI-Phase-2-Ergebnisse mit deutlich
grÃ¶ÃŸerer Tiefe und statistischer Kraft Ã¼berprÃ¼ft.

\subsubsection*{Square Kilometre Array (SKA)}
Das SKA wird ab den 2030er Jahren Milliarden von Galaxien durch
\textbf{21-cm-HI-Emissions\-spektroskopie} kartieren -- ein vÃ¶llig
unabhÃ¤ngiger Tracer vom optischen Universum. Die IntensitÃ¤tskartierung
(\textit{IM}) mit SKA-MID hat das Potential, BAO bei $0 < z < 3$ mit
$<0{,}5\,\%$-PrÃ¤zision pro Bin zu messen. Systematiken der optischen
Spektroskopie (Faserkalibrierung, Photometrie) sind bei 21-cm-IM nicht
relevant, was eine kraftvolle Kreuzvalidierung ermÃ¶glicht.
...
Hochrot\-verschiebungs-Survey ($1 < z < 3$) durchfÃ¼hren, der DESI-Ergebnisse
aus dem Weltraum kreuzvalidiert. Besonders wertvoll: Roman operiert oberhalb
der ErdatmosphÃ¤re, was stÃ¶rende OH-Airglow-Emissionen eliminiert, die bei
ELG-Spektroskopie von der Erde aus Systematiken verursachen.

\subsection{Primordiale Kosmologie: Inflation und Urknall}

DESI-Phase-2-Daten tragen zur Physik der frÃ¼hen Universums bei:

\subsubsection*{Primordiale Nicht-GauÃŸizitÃ¤t}
Inflationsmodelle sagen verschiedene Levels primordialer Nicht-GauÃŸizitÃ¤t
...
kombinierte DESI+\- CMB-S4-Analysen auf $\sigma(n_s) < 0{,}002$ prÃ¤zisiert werden.
...
Die in Abschnitt~\ref{sec:subgraph} entwickelte Methodik erÃ¶ffnet eine
vollstÃ¤ndig neue Forschungsrichtung an der Schnittstelle von Graphentheorie
und observationeller Kosmologie. Der folgende Ausblick skizziert die
wichtigsten Entwicklungslinien fÃ¼r die nÃ¤chsten zehn Jahre.

\subsubsection*{VollstÃ¤ndige Umsetzung auf DESI DR2 (2026--2028)}

Mit DESI Phase~2 stehen erstmals ausreichend Cluster-Statistiken in sieben
unabhÃ¤ngigen Rotverschiebungsbins (je $\Delta z \approx 0{,}3$) zur VerfÃ¼gung,
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Subgraph-Test fÃ¼r alle $\binom{7}{2} = 21$ Epochenpaare:}
...
\end{equation}
Dieses Observable ist komplementÃ¤r zu den Standard-Redshift-Space-Distortion
(RSD)-Messungen von $f\sigma_8(z)$ und liefert unabhÃ¤ngige Constraints auf
das Wachstum kosmischer Strukturen. Besonders in der Ãœbergangszone von
Materie- zu Dunkler-Energie-Dominanz ($z \approx 0{,}3$--$0{,}7$) kÃ¶nnte
$\mathcal{W}(z)$ die SensititivÃ¤t gegenÃ¼ber dem $w_0 w_a$-Parameter
deutlich verbessern.
...
ausschlieÃŸlich auf Zweipunkt-Statistiken. Eine Integration des Subgraph-Tests
wÃ¼rde folgende Architektur erfordern:
...
\begin{itemize}
  ...
  \item \textbf{VorwÃ¤rtsmodell:} Schnelle analytische Vorhersage von
...
\begin{itemize}
  \item \textbf{Gewichtete Kanten:} Filamentdichte oder -temperatur als
    Kantengewicht $w_{ij}$ einbeziehen, was das Detektionsverhalten fÃ¼r
  ...
  \item \textbf{Gerichtete Graphen:} Materiefluss entlang Filamenten
    (Akkretion in Richtung des Potentialminimums) definiert eine natÃ¼rliche
...
kÃ¶nnen trainiert werden, Subgraph-Relationen im kosmischen Netz
...
\begin{itemize}
  ...
  \item \textbf{Anwendung:} Echtzeit-Subgraph-Screening auf dem vollen
    DESI-Datensatz, gefolgt von exakter Verifikation fÃ¼r Kandidatenpaare.
\end{itemize}
...
$\beta_2$ (HohlrÃ¤ume/Voids) \cite{weinberg2013}. Der Subgraph-Test
...
\begin{proof}
  ...
  und $E(z_1) \subseteq E(z_2)$. HinzufÃ¼gen von Knoten und Kanten kann
...
Die Kombination von Subgraph-Test und TDA bietet damit eine vollstÃ¤ndige
...
Langfristig kÃ¶nnte der Subgraph-Test zu einem standardisierten
\textbf{topologischen Kosmologietest} weiterentwickelt werden, der
neben den klassischen Probes (BAO, RSD, Schwache Linse) als eigenstÃ¤ndige
SÃ¤ule der Kosmologiemessung steht. Die StÃ¤rken dieser Methode:

\begin{itemize}
  \item \textbf{ModellunabhÃ¤ngigkeit:} Kein $\Lambda$CDM-Vorwissen notwendig.
  \item \textbf{Topologische Robustheit:} Invariant gegenÃ¼ber Translationen,
    Rotationen und kleinen Deformationen des kosmischen Netzes.
  \item \textbf{KomplementaritÃ¤t:} Erfasst Strukturinformation jenseits
    der Zwei-Punkt-Statistik.
  \item \textbf{Skalierbarkeit:} Durch Cluster-Partitionierung auf beliebige
    DatensatzgrÃ¶ÃŸen (Euclid, SKA) anwendbar.
\end{itemize}

Die DESI Phase~2-Daten (2026--2028) bieten die erste reale Gelegenheit,
diesen Test mit statistisch signifikanter Stichprobe durchzufÃ¼hren
und zu kalibrieren -- ein direktes Anschlussprojekt an die vorliegende Arbeit.

\subsection{Zusammenfassung des Ausblicks}

Die nÃ¤chsten zehn Jahre (2026--2036) werden eine Ã„ra der \textit{PrÃ¤zisionskosmologie}
einlÃ¤uten, in der einzelne Parameter auf $< 1\,\%$ Genauigkeit bestimmt werden.
...
mit Euclid, Roman und CMB-S4 mit $> 5\,\sigma$-SensitivitÃ¤t adressiert werden.

Gleichzeitig Ã¶ffnet DESI Phase~2 neue Fenster auf die Dunkle Materie,
Neutrinomassen und primordiale Physik -- Fenster, die vor zehn Jahren noch
auÃŸerhalb der Reichweite lagen. Die Synergien zwischen bodengebundener
Spektroskopie, Weltraum-Photometrie und Radioastronomie werden zu einem
kohÃ¤renten, hochprÃ¤zisen Bild der kosmischen Struktur und Entwicklung
fÃ¼hren -- und mÃ¶glicherweise die Physik jenseits des Standardmodells aufdecken.
...
\begin{enumerate}[label=\arabic*.]
  \item DESI erfasste $>47$ Millionen Galaxien und Quasare sowie $>20$ Millionen
    Milchstra\-ÃŸen-Sterne -- eine Ãœberbietung der Zielvorgabe um 38\,\%.
  \item Die BAO-Messung in sieben Rotverschiebungsbins erreicht $< 2\,\%$-PrÃ¤zision
    und bestÃ¤tigt das $\Lambda$CDM-Standardmodell, zeigt aber Abweichungen,
    die auf dynamische Dunkle Energie hindeuten kÃ¶nnten.
  \item Das kombinierte $w_0 w_a$-Fit ergibt $w_0 = -0{,}83 \pm 0{,}09$,
    $w_a = -0{,}75 \pm 0{,}27$, was die kosmologische Konstante mit
    $\approx 2{,}5\,\sigma$ ausschlieÃŸt.
  \item DESI stÃ¼tzt $H_0 = 68{,}52 \pm 0{,}80\,\mathrm{km/s/Mpc}$ und damit
  ...
  \item Phase~2 (2026--2028) wird die Himmelsabdeckung auf 17\,000\,deg$^2$
    ausdehnen und neue Objektklassen erschlieÃŸen.
\end{enumerate}

Die DESI-Daten werden in den kommenden Jahren zusammen mit Euclid, Roman,
SKA und CMB-S4 die Grundlage fÃ¼r eine neue Generation kosmologischer
PrÃ¤zisionsmessungen bilden.
...
\bibitem{eppsubgraph2026}
  S. Epp, 
  \textit{Der Subgraph Algorithmus}. Januar 2026, VerfÃ¼gbar auf \url{https://github.com/hjstephan86/subgraph}
