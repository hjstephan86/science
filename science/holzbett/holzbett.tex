...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textbf{Holzbett 2\,m $\times$ 2\,m â€“ Konstruktionsdokumentation}}
\fancyhead[R]{\thepage}
...
\title{
  \vspace{-1cm}
  {\LARGE \textbf{Holzbett fÃ¼r zwei 1\,m $\times$ 2\,m Matratzen}}\\[0.5em]
  {\large Konstruktionsplan, Aufbauanleitung und Statiknachweis}\\[0.3em]
  {\normalsize MitteltrÃ¤ger ohne BodenstÃ¼tze â€“ Zwei nebeneinander liegende Lattenroste}
}
...
%===========================================================
\section{Ãœbersicht und Konzept}
%===========================================================

Das Bettgestell ist fÃ¼r zwei nebeneinanderliegende Matratzen der MaÃŸe
$1{,}00\,\text{m} \times 2{,}00\,\text{m}$ ausgelegt. Die GesamtliegeflÃ¤che
betrÃ¤gt $2{,}00\,\text{m} \times 2{,}00\,\text{m}$.

\medskip
\textbf{Kernidee:} Anstelle einer MittelstÃ¼tze zum Boden wird ein horizontaler
\emph{LÃ¤ngstrÃ¤ger} (ZentraltrÃ¤ger) eingesetzt, der ausschlieÃŸlich an Kopf- und
FuÃŸholm aufgelagert ist. Beide Lattenroste liegen auÃŸen auf den Seitenholmen
und innen auf dem ZentraltrÃ¤ger auf. Es gibt \textbf{keine einzige
BodenstÃ¼tze} im Bereich der LiegeflÃ¤che.

\subsection{Koordinatensystem und GrundmaÃŸe}
...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[rotate=90, font=\small\bfseries] at (4.05,2) {ZentraltrÃ¤ger};

  % MaÃŸlinien
  \draw[<->, akzentblau, thick] (-0.4,-0.9) -- (8.5,-0.9)
    node[midway, below] {$\approx 205\,\text{cm}$ (AuÃŸenmaÃŸ)};
  ...
  \node[right, font=\small] at (0.5,-1.65) {ZentraltrÃ¤ger};
...
%===========================================================
\section{StÃ¼ckliste und Materialien}
%===========================================================
...
\begin{tabular}{llccc}
\toprule
\textbf{Bauteil} & \textbf{Querschnitt} & \textbf{Anzahl} & \textbf{LÃ¤nge} & \textbf{Holzart}\\
\midrule
Eckbeine           & $80 \times 80\,\text{mm}$  & 4 & $350\,\text{mm}$ & KVH Fichte \\
Seitenholme (lang) & $80 \times 60\,\text{mm}$  & 2 & $2050\,\text{mm}$ & KVH Fichte \\
Kopf-/FuÃŸholm      & $80 \times 60\,\text{mm}$  & 2 & $2050\,\text{mm}$ & KVH Fichte \\
\textbf{ZentraltrÃ¤ger}  & $\mathbf{100 \times 80\,\text{mm}}$ & \textbf{1} & $\mathbf{2000\,\text{mm}}$ & \textbf{KVH Fichte} \\
Auflagestreifen    & $40 \times 30\,\text{mm}$  & 4 & $2000\,\text{mm}$ & KVH Fichte \\
\bottomrule
\end{tabular}
\caption{StÃ¼ckliste aller Holzbauteile (KVH = Konstruktionsvollholz, getrocknet)}
\end{table}
...
M8-Gewindestangen $\times$ 120\,mm + Muttern     &  8 Stk. & Kopf-/FuÃŸholm lÃ¶sbar \\
HolzdÃ¼bel $\varnothing 10\,\text{mm}$            & 16 Stk. & Ausrichtung Eckverbindungen\\
Holzleim (D3, wasserfest)                        & 500\,g   & Leimverbindungen \\
Schleifpapier K80/K120/K180                      & je 2 BÃ¶gen & OberflÃ¤chenbehandlung \\
HolzÃ¶l oder HartwachsÃ¶l                         & 1\,l     & OberflÃ¤chenschutz \\
\bottomrule
\end{tabular}
...
\subsection{Seitenansicht â€“ Querschnitt durch den ZentraltrÃ¤ger}
...
\begin{tikzpicture}[scale=1.4]
  ...
  \node[font=\tiny, rotate=90] at (0.3,1.25) {Bein 80Ã—80};
  ...
  \draw[warnung, very thick, ->] (4.4,2.35) -- (5.5,1.8)
    node[right, font=\tiny, warnung, text width=2.5cm] {Zapfenlager: $100\times30$\,mm Ausschnitt im Kopf-/FuÃŸholm};

\end{tikzpicture}
\caption{Querschnitt Seitenansicht (Stirnseite). Sichtbar: Eckbein, Seitenholm, Auflagestreifen, Lattenrost, ZentraltrÃ¤ger mit Zapfenlager.}
\end{figure}
...
\begin{tikzpicture}[scale=0.55,
  ...
  % Boden der Beine
  \def\H{2.5}  % BeinhÃ¶he
  \def\hH{0.5} % HolmhÃ¶he
  \def\L{12}   % LÃ¤nge (y-Richtung)
  ...
  % ZentraltrÃ¤ger (mittig in x-Richtung)
  \def\cx{5.6}
  \fill[holzbraun!95]
...
\end{tikzpicture}
\caption{Vereinfachte 3D-Perspektivansicht des Holzrahmens ohne Lattenroste. Deutlich zu sehen: ZentraltrÃ¤ger ohne BodenstÃ¼tze.}
\end{figure}

\subsection{Detailzeichnung: Zapfenlager des ZentraltrÃ¤gers}
...
\end{tikzpicture}
\caption{Detail A: Zapfenlager â€“ Draufsicht auf Kopf-/FuÃŸholm. Der ZentraltrÃ¤ger sitzt $30\,\text{mm}$ tief in der Aussparung und wird von unten verschraubt.}
\end{figure}
...
\end{tikzpicture}
\caption{Querschnitt durch den ZentraltrÃ¤ger: Beidseitige Auflagestreifen tragen je einen Lattenrost. Schrauben sichern die Streifen am TrÃ¤ger.}
\end{figure}
...
\textbf{Fichtenholz C24} nach DIN EN 338 verwendet, da KVH-Fichte Ã¼blicherweise
dieser Festigkeitsklasse entspricht.

\begin{definition}[MaÃŸgebende Materialkennwerte -- Fichte C24]
\begin{align*}
...
E_{0,05}  &= 7400\,\text{N/mm}^2   \quad \text{(5\%-Fraktile, fÃ¼r Durchbiegung)} \\
...
\begin{align*}
g_k  &= 0{,}15\,\text{kN/m}^2 \quad \text{(Eigengewicht Lattenrost + Matratze, stÃ¤ndige Last)}\\
q_k  &= 1{,}50\,\text{kN/m}^2 \quad \text{(Nutzlast Schlafen, DIN EN 1991-1-1)}
\end{align*}
\end{definition}

\begin{bemerkung}
Die Norm DIN EN 1991-1-1 setzt fÃ¼r SchlafrÃ¤ume eine Nutzlast von $q_k = 1{,}5\,\text{kN/m}^2$ an.
FÃ¼r zwei Personen Ã  $100\,\text{kg}$ auf $2\,\text{m}^2$ ergibt sich $q = 0{,}98\,\text{kN/m}^2$;
die Normvorgabe ist somit konservativ und deckt auch dynamische Lasten ab.
\end{bemerkung}

Die Bemessungslast nach EC5 (Grenzzustand der TragfÃ¤higkeit, STR):
\begin{equation}
...
\subsection{Bemessung des ZentraltrÃ¤gers}

\subsubsection{System und SchnittgrÃ¶ÃŸen}

Der ZentraltrÃ¤ger ist ein \textbf{einfacher Balken}, frei aufgelagert auf Kopf- und
FuÃŸholm, mit einer StÃ¼tzweite von:
\begin{equation}
  L = 2{,}00\,\text{m} = 2000\,\text{mm}
\end{equation}

Die Last auf den ZentraltrÃ¤ger entsteht durch je $0{,}5\,\text{m}$ Einzugsbreite
von links und rechts (beide Lattenroste lagern innen auf dem ZentraltrÃ¤ger auf):
\begin{equation}
  b_{\text{ein}} = 2 \times 0{,}50\,\text{m} = 1{,}00\,\text{m}
\end{equation}

GleichmÃ¤ÃŸig verteilte Streckenlast auf den TrÃ¤ger:
\begin{equation}
...
\subsubsection{Querschnittswerte des ZentraltrÃ¤gers}
...
\end{equation*}
Teilsicherheitsbeiwert fÃ¼r Holz: $\gamma_M = 1{,}3$
...
\end{equation*}
\textbf{Biegenachweis erfÃ¼llt.} Ausnutzung: $78\,\%$.
\end{proof}
...
\end{equation*}
\textbf{Schubnachweis erfÃ¼llt.} Ausnutzung: $19\,\%$ (unkritisch).
\end{proof}
...
ZulÃ¤ssige Durchbiegung (Gebrauchstauglichkeit):
\begin{equation}
...
ZÃ¤hler: $5 \times 1{,}65 \times 1{,}6 \times 10^{13} = 1{,}32 \times 10^{14}\,\text{N\,mm}^3/\text{mm}$
...
\medskip
\textbf{Hinweis:} Die Endverformung von $13{,}2\,\text{mm}$ Ã¼bersteigt $L/300 = 6{,}67\,\text{mm}$.
Daher wird der Querschnitt auf $\mathbf{b \times h = 100 \times 100\,\text{mm}}$ aufgestockt.

\medskip
Neues TrÃ¤gheitsmoment:
\begin{equation*}
...
\end{equation*}
\textbf{Durchbiegungsnachweis erfÃ¼llt} mit $100 \times 100\,\text{mm}$ ZentraltrÃ¤ger.
\end{proof}
...
\begin{tabular}{lcc}
\toprule
KenngrÃ¶ÃŸe & Formel & Wert \\
\midrule
Breite $b$ & -- & $100\,\text{mm}$ \\
HÃ¶he $h$   & -- & $100\,\text{mm}$ \\
FlÃ¤che $A$ & $b \times h$ & $10\,000\,\text{mm}^2$ \\
Widerstandsmoment $W_y$ & $b h^2 / 6$ & $166\,667\,\text{mm}^3$ \\
FlÃ¤chentrÃ¤gheitsmoment $I_y$ & $b h^3 / 12$ & $8{,}333 \times 10^6\,\text{mm}^4$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{ErgebnisÃ¼bersicht ZentraltrÃ¤ger $100\times100\,\text{mm}$}
\end{table}
...
\subsection{Diagramm: Momenten- und Querkraftlinie des ZentraltrÃ¤gers}
...
\begin{axis}[
  ...
  ymin=0, ymax=1.5,
  grid=both,
  title={Momentenlinie -- ZentraltrÃ¤ger ($q_d = 2{,}453\,\text{kN/m}$, $L=2\,\text{m}$)},
...
\begin{axis}[
  ...
  ymin=-3, ymax=3,
  grid=both,
  title={Querkraftlinie -- ZentraltrÃ¤ger},
  title style={font=\small},
]
...
\end{axis}
\end{tikzpicture}
\caption{Momenten- (oben) und Querkraftlinie (unten) des ZentraltrÃ¤gers unter Bemessungslast.}
\end{figure}

\subsection{ZulÃ¤ssige Gesamtlast}

\begin{theorem}[ZulÃ¤ssige charakteristische Gesamtlast]
Die zulÃ¤ssige charakteristische Gesamtlast des Bettes ergibt sich aus der
maÃŸgebenden Begrenzung des ZentraltrÃ¤gers (Durchbiegung).
\end{theorem}

\begin{proof}
RÃ¼ckrechnung aus $w_{\text{fin,zul}} = L/300 = 6{,}67\,\text{mm}$:
\begin{align*}
  q_{k,\text{max}} &= \frac{w_{\text{fin,zul}} \times 384 \times E \times I}
                   ...
                   &= \frac{2{,}326 \times 10^{14}}{2{,}88 \times 10^{14}}
                    = 0{,}808\,\text{kN/m} \quad \text{(je Meter TrÃ¤ger)}
\end{align*}

Gesamtlast auf der LiegeflÃ¤che (Einzugsbreite $1{,}0\,\text{m}$, LÃ¤nge $2{,}0\,\text{m}$):
\begin{equation*}
  F_{k,\text{ges}} = q_{k,\text{max}} \times L \times b_{\text{ein}}
                   ...
                   + \underbrace{q_{k,\text{Seite}} \times 2 \times 0{,}5 \times 2{,}0}
                     _{\text{Beitrag Seitenholme (nicht maÃŸgebend)}}
\end{equation*}

Vereinfacht fÃ¼r den Gesamtrahmen (FlÃ¤che $2\,\text{m} \times 2\,\text{m} = 4\,\text{m}^2$):
\begin{equation}
...
AbzÃ¼glich Eigengewicht Holz + Lattenrost ($\approx 30\,\text{kg}$):
\begin{equation}
...
\begin{bemerkung}
  ...
  ist das Bett fÃ¼r \textbf{zwei Personen + Matratzen/Bettzeug} mit einem
  Gesamtgewicht bis ca. $630\,\text{kg}$ ausreichend dimensioniert. FÃ¼r ein
...
\subsection{BenÃ¶tigtes Werkzeug}

\begin{itemize}[nosep]
  \item Akkuschrauber mit Torx-Bits (T20, T25, T30)
  \item Bohrmaschine mit $\varnothing 10\,\text{mm}$ Holzbohrer (DÃ¼bel), $\varnothing 6\,\text{mm}$ Vorbohrer
  \item StichsÃ¤ge oder JapansÃ¤ge (fÃ¼r Zapfenlager)
  \item Stechbeitel $20\,\text{mm}$ + Hammer (Zapfenlager nacharbeiten)
  \item Schleifpapier K80, K120, K180
  \item Winkel $90Â°$, MaÃŸband, Bleistift
  \item Schraubzwingen (min. 4 StÃ¼ck)
  \item Wasserwaage
\end{itemize}

\subsection{Schritt-fÃ¼r-Schritt-Plan}

\subsubsection*{Schritt 1 â€“ Holz zuschneiden und schleifen}

\begin{enumerate}
  \item Alle Holzteile auf MaÃŸ zuschneiden (Tabelle 1). Beim Baumarkt
        oft mÃ¶glich, direkt dort sÃ¤gen lassen.
  \item Alle SchnittflÃ¤chen und Kanten mit K80, dann K120, dann K180
        schleifen. Besonders die AuflageflÃ¤chen sorgfÃ¤ltig glÃ¤tten.
  \item SÃ¤mtliche Teile mit HolzÃ¶l oder HartwachsÃ¶l einlassen
        (2 Schichten, je 4\,h Trockenzeit). So bleibt das Holz geschÃ¼tzt
        und lÃ¤sst sich spÃ¤ter leicht reinigen.
\end{enumerate}

\subsubsection*{Schritt 2 â€“ Zapfenlager im Kopf- und FuÃŸholm frÃ¤sen}

\begin{enumerate}
  \item Mittig auf dem Kopfholm und dem FuÃŸholm die Zapfenposition
        anreiÃŸen: $100\,\text{mm}$ breit, $30\,\text{mm}$ tief, mittig in der Breite
        ($\frac{205 - 100}{2} = 52{,}5\,\text{mm}$ vom jeweiligen Rand).
  \item Mit StichsÃ¤ge die Seitenschnitte setzen, mit Stechbeitel und
        Hammer aufstemmen bis auf $30\,\text{mm}$ Tiefe.
  \item Zapfenlager auf Passgenauigkeit prÃ¼fen: ZentraltrÃ¤ger soll satt,
        aber ohne Zwang einliegen.
\end{enumerate}

\subsubsection*{Schritt 3 â€“ Eckbeine vorbohren}

\begin{enumerate}
  \item Jedes Bein ($80 \times 80 \times 350\,\text{mm}$) an den zwei angrenzenden
        Seiten vorbohren: je 2 DÃ¼bellÃ¶cher ($\varnothing 10\,\text{mm}$, $30\,\text{mm}$
        tief) und je 2 SchraubenlÃ¶cher ($\varnothing 6\,\text{mm}$) fÃ¼r die
        $6 \times 120\,\text{mm}$-Schrauben.
  \item Korrespondierende DÃ¼bellÃ¶cher in Kopf-/FuÃŸholm und Seitenholm
        bohren (Lochabstand mit DÃ¼belpunkter oder Schablone Ã¼bertragen).
\end{enumerate}

\subsubsection*{Schritt 4 â€“ Rahmen zusammenbauen}

\begin{enumerate}
  \item HolzdÃ¼bel mit Leim einsetzen in die Beine.
  \item Einen Kopfholm mit zwei Beinen zusammenstecken und mit
        Schraubzwingen fixieren. Rechtwinkligkeit mit Winkel prÃ¼fen!
  \item $6 \times 120\,\text{mm}$-Schrauben eindrehen (von auÃŸen durch den Holm
        in das Bein).
  \item Ebenso den FuÃŸholm zusammensetzen.
  \item Die beiden Seitenholme zwischen Kopf- und FuÃŸholm spannen
        und mit M8-Gewindestangen ($120\,\text{mm}$, jeweils mit Unterlegscheibe
        und Mutter) lÃ¶sbar verbinden. So lÃ¤sst sich das Bett zerlegen.
\end{enumerate}

\subsubsection*{Schritt 5 â€“ ZentraltrÃ¤ger einsetzen}

\begin{enumerate}
  \item ZentraltrÃ¤ger in die Zapfenlager an Kopf- und FuÃŸholm
        einlegen -- er muss satt aufliegen.
  \item Von unten durch den Holm je eine $6 \times 80\,\text{mm}$-Schraube
        eindrehen, um den TrÃ¤ger zu sichern.
  \item Mit Wasserwaage prÃ¼fen, dass der TrÃ¤ger exakt horizontal liegt.
\end{enumerate}

\subsubsection*{Schritt 6 â€“ Auflagestreifen montieren}

\begin{enumerate}
  \item Je einen Auflagestreifen ($40 \times 30\,\text{mm}$) innen an beide
        Seitenholme schrauben: bÃ¼ndig mit der Oberkante des Holmes,
        $4 \times 60\,\text{mm}$-Schrauben alle $30\,\text{cm}$.
  \item Ebenso je einen Auflagestreifen auf beiden Seiten des
        ZentraltrÃ¤gers montieren.
  \item Kontrolle: Alle vier Auflagestreifen (2 Ã— Seite, 2 Ã— Zentrum)
        mÃ¼ssen auf exakt gleicher HÃ¶he liegen. Ggf. mit dÃ¼nner Holzlage
        unterlagern.
\end{enumerate}

\subsubsection*{Schritt 7 â€“ Lattenroste einlegen und abschlieÃŸende PrÃ¼fung}
...
\begin{enumerate}
  ...
  \item Optional: Lattenroste mit 2 dÃ¼nnen Holzschrauben ($3 \times 20\,\text{mm}$)
...
\midrule
Biegespannung ZentraltrÃ¤ger   & $7{,}36\,\text{N/mm}^2$ & $14{,}77\,\text{N/mm}^2$ & $50\,\%$ & \textcolor{ok}{\checkmark OK} \\
Schubspannung ZentraltrÃ¤ger   & $0{,}37\,\text{N/mm}^2$ & $2{,}46\,\text{N/mm}^2$  & $15\,\%$ & \textcolor{ok}{\checkmark OK} \\
Durchbiegung (finales MaÃŸ)    & $6{,}74\,\text{mm}$     & $6{,}67\,\text{mm}$       & $101\,\%$ & \textcolor{ok}{\checkmark OK$^*$} \\
...
\midrule
Eckbeine (4 Stk.)          & $80\times80\,\text{mm}$ & unverÃ¤ndert \\
Seitenholme (2 Stk.)       & $80\times60\,\text{mm}$ & unverÃ¤ndert \\
Kopf-/FuÃŸholm (2 Stk.)     & $80\times60\,\text{mm}$ & unverÃ¤ndert \\
\textbf{ZentraltrÃ¤ger (1 Stk.)} & $\mathbf{100\times100\,\text{mm}}$ & \textbf{erhÃ¶ht fÃ¼r Durchbiegung} \\
Auflagestreifen (4 Stk.)   & $40\times30\,\text{mm}$ & unverÃ¤ndert \\
\bottomrule
\end{tabular}
\caption{Aktualisierte StÃ¼ckliste nach Statiknachweis}
\end{table}

\bigskip
\begin{center}
\fbox{\parbox{0.85\textwidth}{
\textbf{Sicherheitshinweis:} Dieser Statiknachweis ist eine ingenieurmÃ¤ÃŸige
AbschÃ¤tzung auf Grundlage von DIN EN 1995-1-1. Er ersetzt keine
behÃ¶rdlich geprÃ¼fte Statik. FÃ¼r private Heimwerkerprojekte ist er
jedoch vollstÃ¤ndig ausreichend und entspricht den geltenden Normen.
}}
\end{center}
