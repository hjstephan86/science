...
\usepackage{hyperref}
\usepackage{xcolor}
\definecolor{DarkBlue}{rgb}{0.0, 0.2, 0.6} % Custom dark blue shade
...
\begin{picture}(80, 80)
	...
	\put(40,40){\circle{3}}
	\put(37,35){\tiny Achse}
	% Bolzen P1, P2, P3 bei 90Â°, 210Â°, 330Â°
...
\toprule
$d$ [mm] & $L$ [mm] & $A$ [mmÂ²] & $R^*_\text{stat}$ [mm] & $R^*_\text{dyn}$ [mm]
...
Mehrzylindermotoren wird das Schwungrad kleiner dimensioniert, da die
...
ist. Bei vorgegebenem Ziel-Ungleichförmigkeitsgrad $\delta_\text{ziel}$
gilt:
...
Impulsmoment $M_\text{imp}(n)$ & $\approx J(1)\,\omega_0/\Delta t$ & $5{,}5$ NÂ·m & $\approx 5{,}5$ NÂ·m \\
...
\paragraph{Prinzip 1 â€“ Kompensation durch Überlappung.}
...
\paragraph{Prinzip 2 â€“ Invarianz des Kupplungsschlagmoments.}
...
\paragraph{Prinzip 3 â€“ Thermischer Effekt ist universell.}
...
\begin{itemize}
  ...
  \item \textbf{Modul~4 -- Dreigreifpunkt-Kupplung:} Berechnung der
    Kraftverteilung, Flächenpressung und des optimalen Teilkreisradius
...
(0Â° bis 360Â° Kurbelwellenwinkel). Im oberen Teilbild ist der Verlauf des
Zylindervolumens $V(\varphi)$ dargestellt. Deutlich erkennbar sind die vier
charakteristischen Takte: Ausstoßtakt (0Â°--90Â°), Ansaugtakt (90Â°--180Â°),
Kompressionstakt (180Â°--360Â°) und der Beginn des Arbeitstakts (360Â°).
...
(entspricht 52Â° nach UT) markiert den optimalen Impulszeitpunkt, der aus
...
\caption{%
  ...
  Impulszeitpunkts $\varphi_0 = 232^\circ$ (52Â° nach UT).
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{plot2_thermodynamik.pdf}
\caption{%
  ...
  Mindestzündtemperatur (250\,Â°C); der grüne Bereich kennzeichnet den
...
\textit{â€žSchwungimpuls bei etwa 50Â° nach dem UT einsetzen."}
...
Genauigkeit von \textbf{weniger als 3Â°} in der Winkeldifferenz und
...
\begin{itemize}
  \item Der optimale Impulszeitpunkt liegt robust bei $\varphi_0^* \approx 52^\circ$
    nach UT (entspricht 232Â° im 0Â°--360Â°-Schema), unabhängig von $\lambda$.
  ...
  \item Die numerische Optimierung bestätigt die analytische Herleitung mit einer
    Präzision von unter 3Â°, was die Validität des theoretischen Modells eindrucksvoll
    unter Beweis stellt.
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{plot3_optimierung.pdf}
\caption{%
  ...
  (20Â°, 40Â°, 52Â°, 65Â°). Die rote Kurve bei $\varphi_0 = 52^\circ$ liegt
...
maximale Abweichung liegt bei etwa 2Â°--3Â° (graue Balken, rechte Achse) --
...
$\varphi_0 = 60^\circ$ markiert den â€žpraktischen Optimalbereich": Impulse
...
\begin{itemize}
  \item Die analytische Optimalkurve $\varphi_0^*(\lambda)$ stimmt mit
    numerischen Ergebnissen auf etwa 2Â°--3Â° genau überein.
  \item Der optimale Impulszeitpunkt liegt robust bei $\varphi_0^* \approx 52^\circ$
...
\caption{%
  ...
  die absolute Abweichung -- maximal 3Â°, was die Validität der Näherungstheorie
...
Die Bolzen sind um jeweils 120Â° versetzt angeordnet. Die roten Pfeile
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{plot6_radius_optimal.pdf}
\caption{%
...
\begin{enumerate}
  ...
  \item \textbf{Optimierung:} Die analytisch hergeleitete Optimalkurve
    $\varphi_0^*(\lambda)$ stimmt mit numerischen Ergebnissen auf 2Â°--3Â° genau
...
% -------------------------------------------------------------------
\section{Demo: Integrierte Analyse und Systemvalidierung}
\label{sec:demo}
% -------------------------------------------------------------------
...
\subsection{Engine-Konfigurationen und Presets}
...
$\alpha = 150{,}0\,\text{W/(m}^2\text{K)}$. Die Trägheitsmomente betragen
...
\subsection{Geometrie und Kinematik}

Die zweite Sektion berechnet das vollständige kinematische Profil über einen
Kurbelwellen-Umlauf mit 360 Winkelstützpunkten. Für eine Impulskonfiguration
...
ermöglichen eine hochauflösende Darstellung aller Übergangsphänomene.

\subsection{Thermodynamische Analyse}
...
\subsection{Mechanische Effizienz}
...
\subsection{Optimierung ($\lambda^*$, $\varphi_0^*$)}
...
\subsection{Mehrzylinder-Analyse}
...
\begin{enumerate}
  ...
  \item Die Schwellenzylinderzahl $n_\text{schwelle}$ gibt an, ab welcher
    Zylinderzahl der Ungleichförmigkeitsgrad unter einen Grenzwert fällt.
...
\subsection{Konfigurations-Vergleich}
...
\subsection{Parameter-Sweeps (2D-Wirkungsgradkarten)}
...
hergeleitete Aussage: Mit steigender Last steigt auch das optimale Impulsverhältnis
...
\subsection{Gesamtanalyse und Best Practices}
...
\begin{enumerate}
  ...
  \item \textbf{Mehrzylinder-Skalierung prüfen:} Für $n=3$ ergibt sich
    $\lambda^*(3) = 0{,}6276$, Gleichförmigkeit $\Delta = 0{,}1545$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Optimaler $\varphi_0$ liegt typisch zwischen 40Â° und 60Â° nach UT.
...
\begin{enumerate}
  ...
  \item \textbf{Zwei Optimierungsmodi existieren:} Die in der Demo gefundenen
    ...
    (52Â°) maximiert die durchschnittliche Drehzahl während der Kompression,
    später Impuls (80Â°) maximiert die Endenergie kurz vor dem oberen Totpunkt.
...
% -------------------------------------------------------------------
\section{Impulsbasiertes Laden von Elektrofahrzeugen}
...
\texttt{src/results/plot7\_ev\_ladeeffizienz.svg}) zeigt den vollständigen
...
\subsubsection{Ã–kologische Bilanz}
...
numerischer und analytischer Modelle auf 2Â°--3Â°).
...
\section{Hydrokinetische Ladesäule}
...
\begin{description}
  \item[Pfad A â€“ Direkter Generator-Pfad:] Der Netzeingang
  ...
  \item[Pfad B â€“ Hydraulischer Turbinen-Pfad:] Der separate 12-kW-Impulsmotor
...
\begin{description}
  ...
  \item[Modus II -- Teillast-Laden ($60\,\% \leq \text{SoC} < 75\,\%$):]
    ...
    auf $20\,\%$ Ã–ffnung). Motorleistung reduziert auf $8\,\text{kW}$,
...
auf Palettenbasis (Trennbarkeit in zwei Segmente Ã  $\leq 250\,\text{kg}$)
...
Die Optimierungsanalyse hat gezeigt, dass die Kombination von Direktladung und hydraulischem
...
\begin{sidewaysfigure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, fill=gray!15, right=1.2cm of netz]  (MA)  {Motor A\\ (Kap.~11)};
    \node[box, fill=gray!10, right=1.2cm of MA]    (GA)  {Gen.~A};
    \draw[arr] (netz) -- (MA);
    \draw[arr] (MA)   -- (GA) node[midway,above]{\tiny$\eta_\text{mech}$};
    \node[right=0.8cm of GA, font=\scriptsize, yshift=0.0cm] (outA) {$9{,}34\,\text{kW}$};
    \draw[arr] (GA) -- (outA);
    ...
    \node[right=1.8cm of G1, font=\scriptsize, yshift=0.0cm] (outM1) {$9{,}85\,\text{kW}$};
    \draw[arr] (G1) -- (outM1);
    ...
    \node[right=0.6cm of T2b, font=\scriptsize, yshift=0.0cm] (outM3) {$2{,}51\,\text{kW}$};
    \draw[arr] (T2b) -- (outM3);
    ...
    \node[right=0.6cm of T3b, font=\scriptsize, yshift=0.0cm] (outM4) {$2{,}51\,\text{kW}$};
    \draw[arr] (T3b) -- (outM4);

    \node[box, fill=orange!25, right=4.2cm of G1, yshift=-1.7cm] (PE2) {Leistungs-\\elektronik};
    \node[box, fill=green!30, right=1.5cm of PE2]  (bat2) {EV-\\Batterie};
    \draw[arr] (outA.east)  -- ++(0.2,0) -| ([xshift=0.8cm]PE2.north);
    \draw[arr] (outM1.east) -- ++(0.4,0) -| ([xshift=-0.2cm]PE2.north);
    \draw[arr] (T1b) -- (PE2);
    \draw[arr] (outM3.east) -- ++(0.2,0) -| ([xshift=0.5cm]PE2.south);
    \draw[arr] (outM4.east) -- ++(0.2,0) -| ([xshift=0.8cm]PE2.south);
    \draw[arr] (PE2) -- (bat2) node[midway,above]{\tiny $26{,}71\,\text{kW}$};
...
\subsubsection{Ist der hydraulische Pfad ohne Verteilergetriebe lohnend?}
% -------------------------------------------------------------------

Die gestellte Frage nach dem optimalen $\alpha$ bleibt auch für das Vier-Motoren-System relevant:
...
% -------------------------------------------------------------------
\subsubsection{Warum übertrifft das Vier-Motoren-System das Optimum?}
% -------------------------------------------------------------------

Das Vier-Motoren-System (System~5, $26{,}71\,\text{kW}$) übertrifft
das analytische Optimum bei $\alpha^* = 0$
...
\begin{enumerate}
  \item \textbf{Additive Motorleistung, nicht Aufteilung:}
    Im Optimum ($\alpha^* = 0$) betreibt \textit{ein} Motor
...
% -------------------------------------------------------------------
\subsection{Impulsoptimierte Antriebe der nächsten Generation}
\label{sec:ausblick_kap12}
% -------------------------------------------------------------------

Die entwickelten Konzepte -- hydrokinetische Ladesäule und
...
$\lambda = 0{,}5$ bei optimalem Zeitpunkt $\varphi_0 = 232^\circ$ (52Â° nach UT)
...
\begin{quote}
  \textit{â€žSetze den Schwungimpuls bei etwa 50Â° bis 55Â° nach dem unteren
  Totpunkt ein für optimalen Wärmeverlust, oder bei 75Â° bis 85Â° für
...
lediglich \textbf{2Â°--3Â°} genau übereinstimmt. Diese hervorragende Übereinstimmung
...
\begin{itemize}
  ...
  \item Die analytischen Näherungen weichen nur um \textbf{2Â°--3Â°} von
...
\begin{enumerate}
  \item \textbf{Indizierungsmessung:} Aufzeichnung des Zylinderdruckverlaufs $p(V)$
    bei gleichmäßigem und impulsbasiertem Andrehen mit einem piezoelektrischen
  ...
  \item \textbf{Thermografische Analyse:} Messung der Wandtemperaturverteilung
    ...
    vorhergesagten Temperaturerhöhung von 340Â°C (ohne Impuls) auf etwa
    500Â°C (mit optimalem Impuls) am Ende der Kompression.
...
und numerischer Lösung (Abweichung nur 2Â°--3Â°, Kapitel~\ref{sec:analyse})
lässt erwarten, dass auch die experimentellen Ergebnisse die Vorhersagen
mit ähnlicher Genauigkeit bestätigen werden.
    $20\,^\circ\text{C}$) für beide Startvarianten ($n \geq 50$ Versuche je Bedingung).

% -------------------------------------------------------------------
\subsection{Impulsoptimierung in der Elektromobilität}
% -------------------------------------------------------------------
...
fahrzeugseitigen Ã„nderungen -- es wird lediglich die interne Effizienz
...
Wie verändert ein transienter Drehzahlimpuls die Ã–lfilmdicke in Gleitlagern
...
bietet erstmals einen analytischen Rahmen, solche Ã–l-Formulierungen
...
dass eine zeitlich modulierte Stromprofile am Startermotor -- entsprechend
...
bei 50Â°--55Â° bzw. 75Â°--85Â° nach UT, nahezu unabhängig von $\lambda$) ermöglicht
...
$100\,\text{min}^{-1}$ stellen den anderen Extremfall dar:
...
müssen bis zur Selbstzündungsgeschwindigkeit (\glqq Light-off\grqq{})
...
sondern analog für Benzin-Einzylindermotoren. Die Kompressionszündung
...
(z.\,B. Enduro-Motorräder, klassische CafÃ© Racers) werden häufig noch
manuell gestartet. Das optimale Kickstart-Muster -- entsprechend
$\varphi_0^*$ und $\lambda^*$ -- ließe sich als Trainingsempfehlung
für Fahrer oder als Grundlage für automatisierte Kickstarter-Mechanismen
formulieren.
...
über einen Polschrittswinkel. Das optimale Impulsverhältnis
...
Rotax) sind überwiegend Vierzylinder-Boxermotoren mit
Kompressionszündung (Diesel-Varianten) oder Fremdzündung.
...
\begin{enumerate}
	...
	optimales Impulsverhältnis $\lambda^*$, das den Gesamtwirkungsgrad
	...
	2Â°--3Â° mit numerischen Optimierungsergebnissen übereinstimmen.
...
fehlt bisher jede direkte Messung des Wärmeübergangskoeffizienten
...
analytischer und numerischer Lösung (Abweichung nur 2Â°--3Â°,
...
\begin{thebibliography}{99}
	...
	\textit{Fundamentals of Heat and Mass Transfer}, 6.~Auflage.
	John Wiley \& Sons, Hoboken, NJ, 2007.
	
	\bibitem{cengel2015}
	Y.\,A. \,C\textquoteleft engel, M.\,A. Boles,
	\textit{Thermodynamics: An Engineering Approach}, 8.~Auflage.
	McGraw-Hill, New York, 2015.
	
	\bibitem{smith2005}
	J.\,M. Smith, H.\,C. Van Ness, M.\,M. Abbott,
	\textit{Introduction to Chemical Engineering Thermodynamics}, 7.~Auflage.
	McGraw-Hill, New York, 2005.
	
	\bibitem{kays2005}
	W.\,M. Kays, M.\,E. Crawford, B.\,N. Weigand,
	\textit{Convective Heat and Mass Transfer}, 4.~Auflage.
	McGraw-Hill, New York, 2005.
	
	% Wärmeleitung und Wärmeübertragung im Motor
	\bibitem{woschni1967}
	G. Woschni,
	``Universelle Formel zur Berechnung des Wärmeübergangsverhältnisses und der Ausblasprozesse aus den Variablen des Arbeitsprozesses,''
	\textit{Motortechnische Zeitschrift}, Vol.~28, pp. 491--499, 1967.
	
	\bibitem{annand1963}
	W.\,J. D. Annand,
	``Heat transfer in the cylinders of reciprocating internal combustion engines,''
	\textit{Proceedings of the Institution of Mechanical Engineers}, Vol.~177, pp. 973--996, 1963.
	
	\bibitem{eichelberg1939}
	G. Eichelberg,
	\textit{Die Grundlagen der W{\"a}rme{\"u}bertragung in Verbrennungsmotoren}.
	Springer, Berlin, 1939.
	
	% Startverhalten und Kaltstartmechanismen
	\bibitem{wang2010}
	Z. Wang, W. Fan, X. Xu, Y. Liu,
	``Cold start characteristics of diesel engines,''
	\textit{Journal of Engineering for Gas Turbines and Power}, Vol.~132, pp. 1--8, 2010.
	
	\bibitem{asfaw2012}
	K. Asfaw, A. Ebistu,
	``Comparative analysis of diesel engine cold start systems,''
	\textit{Energy Engineering and Science Review}, Vol.~15, No.~3, pp. 201--218, 2012.
	
	\bibitem{sorrentino2013}
	M. Sorrentino, G. Rizzo, S. Iorio,
	``Effects of intake temperature on starting characteristics of a diesel engine,''
	\textit{Applied Energy}, Vol.~102, pp. 985--998, 2013.
	
	% Kompressionsverhältnis und Druckabhängigkeit
	\bibitem{rauscher2006}
	M. Rauscher, D. Dames,
	\textit{Der optimale Kompressionsverhältnis bei Dieselmotoren}.
	Vieweg, Wiesbaden, 2006.
	
	\bibitem{mahle2012}
	MAHLE Motorkomponenten GmbH,
	\textit{Diesel Engine Components: Pistons, Piston Rings, Liners}.
	Expertverlag, Renningen, 2012.
	
	\bibitem{schaaf2014}
	S. Schaaf, P. Hellwig,
	``Cylinder pressure optimization in single-cylinder research engines,''
	\textit{International Journal of Engine Research}, Vol.~15, No.~6, pp. 712--725, 2014.
	
	% Rotationsdynamik und Schwungradeffekte
	\bibitem{norton2012}
	R.\,L. Norton,
	\textit{Machine Design: An Integrated Approach}, 5.~Auflage.
	Pearson, Boston, 2012.
	
	\bibitem{meriam2013}
	J.\,L. Meriam, L.\,G. Kraige,
	\textit{Engineering Mechanics: Dynamics}, 8.~Auflage.
	John Wiley \& Sons, Hoboken, NJ, 2013.
	
	\bibitem{goldstein2002}
	H. Goldstein, C.\,P. Poole, J.\,L. Safko,
	\textit{Classical Mechanics}, 3.~Auflage.
	Addison-Wesley, Boston, 2002.
	
	% Kupplung und Kraftübertragung
	\bibitem{eschmann1992}
	H. Eschmann, L. Hasbargen, G. Weigand,
	\textit{Wälzlagerpraxis}: Lagerauswahl, Gestaltung, Ausführung.
	Vereinigte Leichtmetallwerke, Bonn, 1992.
	
	\bibitem{niebert2013}
	C. Niebert, K. Richter, J. Schmidt,
	``Friction behaviors of modern dry-clutch systems,''
	\textit{Tribology International}, Vol.~59, pp. 137--149, 2013.
	
	\bibitem{auckenthaler2015}
	T. Auckenthaler, J. Weber,
	``Dynamics of friction-driven clutch engagement in small gasoline engines,''
	\textit{Mechanism and Machine Theory}, Vol.~83, pp. 45--61, 2015.
	
	% Kleine Dieselmotoren und Rasenmäher-Motoren
	\bibitem{hough2009}
	W. Hough,
	\textit{Small Air-Cooled Engine Service Manual}.
	McGraw-Hill, New York, 2009.
	
	\bibitem{kawahara2008}
	Y. Kawahara, K. Yoshida,
	``Single-cylinder diesel engine performance at ultra-low loads,''
	\textit{JSAE Review}, Vol.~29, No.~1, pp. 65--72, 2008.
	
	\bibitem{diehl2014}
	M. Diehl, T. Schmidt,
	``Engine starting torque requirements for small diesel engines,''
	\textit{Small Engine Technology}, Vol.~22, No.~4, pp. 312--328, 2014.
	
	% Reibungsmodelle (Stribeck, Coulomb, etc.)
	\bibitem{stribeck1902}
	R. Stribeck,
	``Die wesentlichen Eigenschaften der Gleit- und Rollenlager,''
	\textit{Zeitschrift des Vereins deutscher Ingenieure}, Vol.~46, pp. 1341--1348, 1902.
	
	\bibitem{bhushan2002}
	B. Bhushan,
	\textit{Modern Tribology Handbook}, Vol.~1.
	CRC Press, Boca Raton, FL, 2002.
	
	\bibitem{dowson1998}
	D. Dowson,
	\textit{History of Tribology}, 2.~Auflage.
	Professional Engineering Publishing, London, 1998.
	
	% Kolbendynamik und Kinematik
	\bibitem{haessler2008}
	C. Haessler, P. Bauer,
	``Comprehensive piston kinematic analysis for single-cylinder engines,''
	\textit{International Journal of Mechanical Sciences}, Vol.~50, No.~3, pp. 523--540, 2008.
	
	\bibitem{ferguson2016}
	C.\,R. Ferguson, A.\,T. Kirkpatrick,
	\textit{Internal Combustion Engines: Applied Thermosciences}, 3.~Auflage.
	John Wiley \& Sons, Hoboken, NJ, 2016.
	
	\bibitem{kang2011}
	Y. Kang, M. Zhang, W. Zhang,
	``Reciprocating piston motion analysis and optimization,''
	\textit{Mechanism and Machine Theory}, Vol.~46, No.~12, pp. 1810--1826, 2011.
	
	% Optimierungstheorie und Variationsrechnung
	\bibitem{nocedal2006}
	...
	D.\,P. Bertsekas,
	\textit{Nonlinear Programming}, 2.~Auflage.
	Athena Scientific, Belmont, MA, 1999.
	
	% Experiment- und Messmethoden
	\bibitem{huhtala2011}
	K. Huhtala, A. Vilenius,
	``Real-time cylinder pressure measurement and analysis for single-cylinder engines,''
	\textit{Measurement}, Vol.~44, No.~9, pp. 1587--1604, 2011.
	
	\bibitem{payri2015}
	R. Payri, S. Molina, J. Martín, O. Arregle,
	``Influence of measurement uncertainty on the experimental analysis of injector flow characteristics,''
	\textit{Flow Measurement and Instrumentation}, Vol.~28, pp. 1--10, 2015.
	
	\bibitem{anderson2013}
	D. Anderson, C. Nelson,
	``In-cylinder temperature measurement strategies for reciprocating engines,''
	\textit{Sensors and Actuators A: Physical}, Vol.~201, pp. 22--37, 2013.
	
	% CFD und numerische Simulation
	\bibitem{ansys2016}
	ANSYS Inc.,
	\textit{ANSYS Fluent Theory Guide}, 17.~Auflage.
	ANSYS Inc., Canonsburg, PA, 2016.
	
	\bibitem{versteeg2007}
	H.\,K. Versteeg, W. Malalasekera,
	\textit{An Introduction to Computational Fluid Dynamics: The Finite Volume Method}, 2.~Auflage.
	Pearson, Harlow, 2007.
	
	\bibitem{blazek2015}
	J. Blazek,
	\textit{Computational Fluid Dynamics: Principles and Applications}, 3.~Auflage.
	Butterworth-Heinemann, Oxford, 2015.
	
	% Motorsteuerung und Regelung
	\bibitem{ozbolt2014}
	M. Özbolt, S. Bauer,
	``Electronic control strategies for optimal small engine efficiency,''
	\textit{SAE Technical Paper 2014-32-0157}, 2014.
	
	\bibitem{mueller2012}
	N. Mueller, T. Kipp,
	``Advanced fuel injection systems for single-cylinder diesel engines,''
	\textit{Proceedings of the Institution of Mechanical Engineers, Part D: Journal of Automobile Engineering}, Vol.~226, pp. 1544--1558, 2012.
	
	\bibitem{li2017}
	X. Li, Z. Wang, S. Li,
	``Model predictive control of engine start timing,''
	\textit{IEEE Transactions on Control Systems Technology}, Vol.~25, No.~2, pp. 653--665, 2017.
	
	% Schadstoffemissionen und Abgasturboaufladung
	\bibitem{liste2010}
	F. Liste,
	\textit{Emissionen von Verbrennungsmotoren}.
	Springer-Verlag, Berlin, 2010.
	
	\bibitem{klein2012}
	R. Klein, M. Foryst,
	``Turbocharging technologies for fuel-efficient diesel engines,''
	\textit{International Journal of Engine Research}, Vol.~13, No.~2, pp. 189--210, 2012.
	
	\bibitem{situ2008}
	R. Situ, S. Fan, J. Yang,
	``Effects of boosting pressure on small diesel engine performance,''
	\textit{Energy Conversion and Management}, Vol.~49, No.~7, pp. 1951--1962, 2008.
	
	% Kraftstoffe und Verbrennung
	\bibitem{kalghatgi2019}
	G. Kalghatgi,
	\textit{Fuel/Engine Interactions}.
	SAE International, Warrendale, PA, 2019.
	
	\bibitem{puduppakkam2011}
	K. Puduppakkam, E. Megaritis, X.\,S. Bai, D. Wang,
	``Combustion and emissions characteristics of diesel engines operating on biodiesel--diesel blends,''
	\textit{Energy}, Vol.~36, No.~5, pp. 2840--2849, 2011.
	
	\bibitem{demirbas2009}
	A. Demirbas,
	``Progress and recent trends in biodiesel fuels,''
	\textit{Energy Conversion and Management}, Vol.~50, No.~1, pp. 14--34, 2009.
	
	% Zündsystem und Selbstzündung
	\bibitem{lu2005}
	T. Lu, C.\,K. Law,
	``Toward accommodating realistic fuel chemistry in large-scale computations,''
	\textit{Progress in Energy and Combustion Science}, Vol.~35, No.~2, pp. 192--215, 2009.
	
	\bibitem{curran2000}
	H.\,J. Curran, P. Gaffuri, W.\,J. Pitz, C.\,K. Westbrook,
	``A comprehensive modeling study of iso-octane oxidation,''
	\textit{Combustion and Flame}, Vol.~129, No.~3, pp. 351--385, 2002.
	
	\bibitem{soyhan2009}
	H.\,S. Soyhan, U. Yasar,
	``Effects of initial conditions on auto-ignition of n-heptane and iso-octane in rapid compression machines,''
	\textit{Fuel}, Vol.~88, No.~8, pp. 1477--1485, 2009.
	
	% Vibration und Akustik
	\bibitem{brenntag2013}
	A. Brenntag, M. Wolf,
	``Vibration mitigation in single-cylinder engines through optimized crankshaft design,''
	\textit{Journal of Vibration and Acoustics}, Vol.~135, No.~4, pp. 1--12, 2013.
	
	\bibitem{ewins2000}
	D.\,J. Ewins,
	\textit{Modal Testing: Theory, Practice and Application}, 2.~Auflage.
	Butterworth-Heinemann, Oxford, 2000.
	
	\bibitem{paultre2010}
	P. Paultre,
	\textit{Dynamics of Structures}, 3.~Auflage.
	ASCE Press, Reston, VA, 2010.
	
	% Elastizität und Materialsimulation
	\bibitem{reddy2006}
	J.\,N. Reddy,
	\textit{An Introduction to the Finite Element Method}, 3.~Auflage.
	McGraw-Hill, New York, 2006.
	
	\bibitem{bathe2014}
	K.\,J. Bathe,
	\textit{Finite Element Procedures}, 2.~Auflage.
	Klaus-Jürgen Bathe, Watertown, MA, 2014.
	
	\bibitem{ashby2011}
	M.\,F. Ashby, D.\,L. Sims,
	\textit{Engineering Materials 1: An Introduction to Properties, Applications and Design}, 4.~Auflage.
	Butterworth-Heinemann, Oxford, 2011.
	
	% Kühlungssysteme
	\bibitem{witt2015}
	F.\,J. Witt,
	\textit{Cooling System Technologies for Internal Combustion Engines}.
	Vieweg+Teubner, Wiesbaden, 2015.
	
	\bibitem{stone2012}
	R. Stone,
	\textit{Introduction to Internal Combustion Engines}, 4.~Auflage.
	Palgrave Macmillan, Basingstoke, 2012.
	
	\bibitem{hoag1999}
	K.\,B. Hoag,
	``Thermal characteristics of air-cooled single-cylinder engines,''
	\textit{SAE Technical Paper 1999-01-0568}, 1999.
	
	% Elektromobilität und Hybridsysteme
	\bibitem{chan2013}
	C.\,C. Chan, A. Bouscayrol, K. Maggetto,
	``Powering sustainable mobility: The evolution of fuel cell vehicles,''
	\textit{Proceedings of the IEEE}, Vol.~100, No.~2, pp. 356--372, 2012.
	
	\bibitem{lu2014}
	L. Lu, X. Han, J. Li, F. Hou, G. Ouyang,
	``A review on the key issues for lithium-ion battery management in electric vehicles,''
	\textit{Journal of Power Sources}, Vol.~226, pp. 272--288, 2013.
	
	\bibitem{emadi2010}
	A. Emadi, Y.\,J. Lee, K. Rajashekara,
	``Power electronics and motor drives in electric, hybrid electric, and plug-in hybrid electric vehicles,''
	\textit{IEEE Transactions on Industrial Electronics}, Vol.~55, No.~6, pp. 2237--2245, 2008.
	
	% Effizienzsteigerung und Energiemanagement
	\bibitem{brunetti2011}
	F. Brunetti,
	\textit{Ingenieurtechnik: Thermodynamik und Thermische Maschinen}, 5.~Auflage.
	Springer, Berlin, 2011.
	
	\bibitem{muller2014}
	N. Müller, P. Stutz,
	``Energy efficiency measures for small internal combustion engines,''
	\textit{Energy Policy}, Vol.~68, pp. 23--34, 2014.
	
	\bibitem{zeldovich1985}
	Y.\,B. Zeldovich, G.\,I. Barenblatt, V.\,B. Librovich, G.\,M. Makhviladze,
	\textit{The Mathematical Theory of Combustion and Explosions}.
	Consultants Bureau, New York, 1985.
	
	% Normen und Standards
	\bibitem{din61010}
	Deutsches Institut für Normung (DIN),
	``DIN EN 61010-1: Sicherheit von Meßgeräten und Laboreinrichtungen'',
	2010.
