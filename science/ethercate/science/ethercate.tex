...
\title{
  ...
  \textbf{\Large App-basierte HMI als Ersatz fÃ¼r HMI-Controller}\\[0.4cm]
...
\label{sec:intro}
% =====================================================================
Diese Arbeit schlÃ¤gt eine Erweiterung des EtherCAT-Standards vor, die formal eine neue
...
wartungsintensiv und kÃ¶nnen die AnzeigequalitÃ¤t moderner Consumer-Hardware bei weitem
nicht erreichen.

Die vorgeschlagene Erweiterung fÃ¼hrt einen \emph{App-Based HMI Layer} (ABH) im
EtherCAT-Ã–kosystem ein, der es Tablets und Smartphones erlaubt, vollstÃ¤ndige
HMI-Verantwortung auf Maschinenebene zu Ã¼bernehmen, wÃ¤hrend eine erweiterte
Desktop-Variante fÃ¼r Inbetriebnahme und Diagnose durch Applikationsingenieure genutzt
...
Die quantitative Kosten-Nutzen-Analyse zeigt eine Einsparung von bis zu 81\,\% Ã¼ber
fÃ¼nf Jahre bei 20 HMI-Stationen. Der Proposal richtet sich an die EtherCAT Technology
...
$100\,\si{\micro\second}$ ermÃ¶glichen prÃ¤zise Bewegungssteuerung und synchronisierte
Multi-Achs-Applikationen, die mit klassischen Feldbussystemen nicht realisierbar wÃ¤ren
\cite{etg_spec}.
...
zur Entstehung einer heterogenen HMI-Landschaft gefÃ¼hrt, in der proprietÃ¤re LÃ¶sungen
verschiedener Hersteller parallel existieren, ohne gemeinsame Schnittstellen oder
InteroperabilitÃ¤t zu gewÃ¤hrleisten.

\subsection{Problemstellung}

Traditionell rÃ¼sten Maschinenbauer jede Produktionslinie (sog.\ \emph{Band}) mit einem
dedizierten HMI-Panel aus: einem eingebetteten Touchscreen-System, das eine proprietÃ¤re
Runtime ausfÃ¼hrt und direkt am MaschinengehÃ¤use montiert ist. Obwohl diese Systeme
ihren Zweck grundsÃ¤tzlich erfÃ¼llen, bringen sie erhebliche Nachteile mit sich:

\begin{itemize}[leftmargin=2em]
  \item \textbf{Hohe Anschaffungskosten:} Industrielle Panel-PCs liegen im Preisbereich
    von \EUR{800} bis Ã¼ber \EUR{5\,000} pro Einheit, abhÃ¤ngig von Schutzklasse,
    DisplaygrÃ¶\ss{}e und Rechnerleistung.
  \item \textbf{Geringe AnzeigequalitÃ¤t:} AuflÃ¶sungen zwischen 800$\times$600 und
    1920$\times$1080 bei eingeschrÃ¤nkter Helligkeit (typisch 350--600\,nit) sind der
    Standard.
  \item \textbf{ProprietÃ¤rer Runtime-Lock-in:} Jeder Hersteller nutzt eine eigene
    Entwicklungsumgebung und Runtime (z.\,B.\ Siemens WinCC, Beckhoff TwinCAT HMI,
    Wein\-tek EasyBuilder), was Portierbarkeit und HerstellerunabhÃ¤ngigkeit stark einschrÃ¤nkt.
  \item \textbf{Wartungsaufwand:} Ersatzteilbeschaffung und Software-KompatibilitÃ¤t bei
    veralteten Panels stellen langfristig erhebliche Herausforderungen dar.
  \item \textbf{Fehlende KonnektivitÃ¤t:} Integrierte Remote-Monitoring- und
    Cloud-Anbindungen sind nur mit proprietÃ¤ren ZusatzlÃ¶sungen realisierbar.
...
Gleichzeitig Ã¼bersteigen Consumer-Tablets die HMI-AnzeigequalitÃ¤t bei einem Bruchteil
der Kosten. Ein modernes iPad Pro erreicht AuflÃ¶sungen von 2752$\times$2064 Pixeln,
Bildwiederholraten von 120\,Hz sowie eine Helligkeit von bis zu 1600\,nit --- Werte, die
industrielle Panels nicht annÃ¤hernd bieten.

\subsection{Ziel dieser Arbeit}

Diese Arbeit schlÃ¤gt die \textbf{EtherCAT-HMI-App Extension (EHAE)} vor und definiert,
wie app-basierte HMI-Clients mit EtherCAT-Mastern integriert werden kÃ¶nnen, um
band-integrierte HMI-Controller vollstÃ¤ndig zu ersetzen. Die zentralen BeitrÃ¤ge dieser
Arbeit sind:
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Beschreibung eines auf OAuth~2.0, TLS~1.3 und RBAC aufbauenden
    Sicherheitsmodells.
  \item Quantitativer Nachweis der Kosteneinsparungen gegenÃ¼ber traditionellen AnsÃ¤tzen.
\end{enumerate}
...
\caption{5-Jahres-TCO-Vergleich: traditionelles HMI-Panel vs.\ app-basiertes HMI pro
  Station. Das Balkendiagramm zeigt fÃ¼nf Kostenkategorien (Hardware, Lizenzierung,
  Wartung, Ersatz und Gesamtkosten) jeweils nebeneinander fÃ¼r beide Varianten. Beim
  traditionellen HMI-Panel dominieren die Hardware-Kosten mit \EUR{3\,500} sowie
  Wartungskosten von \EUR{3\,000}. Die app-basierte LÃ¶sung reduziert alle
  Einzelpositionen drastisch: Hardware auf \EUR{600}, Wartung auf \EUR{300}. In der
  Gesamtbetrachtung Ã¼ber fÃ¼nf Jahre ergibt sich \EUR{10\,700} gegenÃ¼ber nur \EUR{1\,300}
  --- eine Einsparung von \textbf{87\,\%} pro Station.}
\label{fig:cost_comparison}
\end{figure}

Die Differenz zwischen den Varianten ist auf mehrere Faktoren zurÃ¼ckzufÃ¼hren: Industrielle
Panels erfordern WartungsvertrÃ¤ge, herstellerspezifische Schulungen und kostspielige
...
\subsection{EtherCAT-ProtokollÃ¼bersicht}

EtherCAT operiert auf einem Master-Slave-Prinzip \cite{etg_spec}. Ein einzelner Master
kommuniziert mit mehreren Slave-GerÃ¤ten in einer Ring- oder Linientopologie Ã¼ber
...
\emph{Processing-on-the-fly}-Ansatz: Jeder Slave liest die fÃ¼r ihn relevanten Daten aus
dem durchlaufenden Frame heraus und schreibt seine Prozessdaten hinein, wÃ¤hrend der
Frame den Knoten passiert --- ohne Pufferung und ohne zusÃ¤tzliche Latenz.
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Physical Layer (IEC~61158-2):} Definiert die elektrischen und mechanischen
    Eigenschaften der Ãœbertragung, typischerweise 100BASE-TX Ã¼ber Cat-5-Kabel.
  \item \textbf{Data Link Layer:} Beinhaltet das Field Memory Management Unit (FMMU) und
    den SyncManager. Das FMMU ermÃ¶glicht das flexible Mapping von Prozessdaten in den
    EtherCAT-Frame, wÃ¤hrend der SyncManager die synchronisierte Kommunikation zwischen
    Master und Slave verwaltet.
  \item \textbf{Application Layer:} Definiert hÃ¶here Protokolle wie CAN over EtherCAT
...
\caption{Standard-EtherCAT-Topologie mit traditionell angebundenem HMI-Panel. Das
  ...
  der Ã¼ber eine lineare Ethernet-Kette mit drei Slave-GerÃ¤ten verbunden ist: Drive~1,
  ...
  (rot markiert, unten links) ist \emph{nicht} Teil des EtherCAT-Rings, sondern Ã¼ber eine
  ...
  Bestandteil des EtherCAT-Standards, sondern eine proprietÃ¤re ErgÃ¤nzung.}
...
KompatibilitÃ¤tsproblemen, proprietÃ¤ren Protokollen und erhÃ¶htem Integrationsaufwand fÃ¼hrt.

\subsection{Consumer-Hardware-FÃ¤higkeiten}

Der rasante Fortschritt im Consumer-Elektronik-Markt hat eine technologische LÃ¼cke
gegenÃ¼ber industriellen HMI-Panels entstehen lassen, die sich Jahr fÃ¼r Jahr vergrÃ¶\ss{}ert.
...
Galaxy Tab S9)} \\
\midrule
AuflÃ¶sung          & 1024$\times$768 -- 1920$\times$1080  & 2560$\times$1600 --
2752$\times$2064 \\
Bildwiederholrate  & 60\,Hz                               & 60--120\,Hz (ProMotion) \\
Helligkeit         & 350--600\,nit                        & 600--1600\,nit \\
Touch-PrÃ¤zision    & $\pm$2--5\,mm                        & $\pm$0{,}5\,mm \\
Preis (EUR)        & 800--5\,000                          & 350--1\,300 \\
Prozessor          & ARM Cortex-A, 1--2\,GHz              & Apple M4 / Snapdragon 8 Gen 2 \\
RAM                & 512\,MB -- 2\,GB                     & 8--16\,GB \\
Akkulaufzeit       & entfÃ¤llt (Netzbetrieb)               & 10--12\,Stunden \\
\bottomrule
\end{tabularx}
\end{table}

Neben den Displayeigenschaften sind Consumer-Tablets in weiteren Dimensionen deutlich
Ã¼berlegen: ihre Prozessoren ermÃ¶glichen flÃ¼ssige Echtzeitvisualisierungen, Trend-Displays
und sogar lokale Datenanalyse. Die Integration von Wi-Fi~6, Bluetooth und Mobilfunk bietet
KonnektivitÃ¤tsoptionen, die industrielle Panels nicht besitzen.

\subsection{Bestehende AnsÃ¤tze und deren Grenzen}

Bereits heute gibt es Versuche, Consumer-GerÃ¤te als HMI einzusetzen. Diese basieren
typischerweise auf OPC-UA-KonnektivitÃ¤t oder proprietÃ¤ren HerstellerlÃ¶sungen wie
Siemens MindSphere oder Beckhoff TF2000. Sie weisen jedoch folgende SchwÃ¤chen auf:
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Keine spezifizierte Latenzanforderung fÃ¼r die HMI-App-KonnektivitÃ¤t.
  \item Vendor-Lock-in durch proprietÃ¤re Protokolle.
\end{itemize}

EHAE schlie\ss{}t diese LÃ¼cke durch eine vollstÃ¤ndige, herstellerunabhÃ¤ngige Spezifikation.
...
WebSocket-basierte API, mit der HMI-App-Clients Ã¼ber Wi-Fi oder kabelgebundenes Ethernet
verbinden kÃ¶nnen.

Abbildung~\ref{fig:ehae_architecture} zeigt das vollstÃ¤ndige Schichtenmodell der EHAE.
...
\caption{EHAE-Architekturdiagramm: Schichtenmodell der erweiterten EtherCAT-Protokollstack.
  ...
  Schichten. Ganz unten (grau) liegt der \emph{EtherCAT Physical Layer} gemÃ¤\ss{} IEC~61158-2.
  DarÃ¼ber (blau) befindet sich der \emph{Data Link Layer} mit FMMU und SyncManager.
  Die dritte Schicht (grÃ¼n) ist der \emph{Application Layer} mit CoE, FoE, EoE und AoE.
  ...
  von App-Clients Ã¼ber WebSocket-Verbindungen (Pfeile von oben): die Operator~App
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Keine Ã„nderungen an Slave-Firmware:} Alle Erweiterungen laufen
    ausschlie\ss{}lich im Master-Stack.
  \item \textbf{RÃ¼ckwÃ¤rtskompatibilitÃ¤t:} Bestehende EtherCAT-Netzwerke kÃ¶nnen ohne
    Modifikation der FeldgerÃ¤te auf EHAE migrieren.
  \item \textbf{Herstellerfreiheit:} Jeder EtherCAT-Master-Stack-Anbieter kann den HAG
    unabhÃ¤ngig implementieren.
\end{itemize}
...
\caption{Interne Modulstruktur des HAG. Das Blockdiagramm zeigt sieben funktionale Module
  ...
  Schnittstellen (grÃ¼n). Von links kommend empfÃ¤ngt der \emph{Config Loader} die
  Prozessdaten-Mappings aus dem EtherCAT Application Layer. Der \emph{PDO Mapper}
  Ã¼bersetzt rohe EtherCAT-Prozessdaten in benannte HMI-Variablen und speichert diese in
  der \emph{Variable Database (Var DB)}. Der \emph{WebSocket Server} in der Mitte ist
  das VerbindungsstÃ¼ck zu den App-Clients (rechts). Er kommuniziert bidirektional mit dem
  \emph{Auth Broker} (Token-Validierung) und der \emph{RBAC Engine}
  (ZugriffsrechtsprÃ¼fung). Der \emph{Event Bus} sammelt Ereignisse aus dem PDO Mapper
...
\begin{description}[leftmargin=2em]
  ...
  \item[PDO Mapper] FÃ¼hrt das zyklische Mapping von EtherCAT-Prozessdatenobjekten
    (PDOs) auf benannte HMI-Variablen durch. Erkennt WerteÃ¤nderungen und generiert
    entsprechende Events.
  \item[Variable Database] In-Memory-Datenbank, die den aktuellen Zustand aller
    HMI-Varia\-blen hÃ¤lt. Dient als konsistenter Snapshot-Speicher fÃ¼r neue
  ...
  \item[RBAC Engine] PrÃ¼ft vor jeder schreibenden Operation, ob die Rolle des Clients die
    erforderliche Berechtigung besitzt.
  \item[Event Bus] Internes Publish-Subscribe-System fÃ¼r lose Kopplung zwischen Modulen.
...
Codebase fÃ¼r beide Rollen zu nutzen und die FÃ¤higkeiten dynamisch auf Basis des
...
\node[font=\tiny, color=accentred] at (0,-1.1) {\texttimes~ParameterÃ¤nderung};
...
\end{tikzpicture}
\caption{Dual-Role-App-Modell: GegenÃ¼berstellung von Operator~App und Engineer~App.
  Das Diagramm zeigt drei GerÃ¤teklassen von links nach rechts, deren Capability-Sets
  ...
  Statusanzeige und das Quittieren von Alarmen; ParameterÃ¤nderungen sind explizit
  gesperrt (\texttimes). In der Mitte (grÃ¼n) die \emph{Operator App} auf einem Tablet
  mit erweiterter AnzeigeflÃ¤che: alle Smartphone-Features plus Trendanzeige und
  Rezeptauswahl sind verfÃ¼gbar. Rechts (grau) die \emph{Engineer App} auf Desktop
  oder Tablet: das vollstÃ¤ndige Capability-Set einschlie\ss{}lich SDO-Parametrierung,
  Antriebsinbetriebnahme, Firmware-Updates Ã¼ber FoE und Echtzeitoszilloskop.}
\label{fig:dual_role}
\end{figure}

\subsection{Engineer App: Topologie-Ansicht}

Eine der leistungsstÃ¤rksten Funktionen der Engineer App ist die Live-Topologie-Visuali\-sierung.
Sie zeigt das gesamte EtherCAT-Netzwerk in Echtzeit, inklusive GerÃ¤tezustÃ¤nde, Diagnose-
...
\end{tikzpicture}
\caption{Engineer-App-Topologieansicht: Live-EtherCAT-Netzwerkkarte mit GerÃ¤tezustÃ¤nden.
  Das Diagramm zeigt den EtherCAT-Master (blau) links, von dem zwei parallele StrÃ¤nge
  abzweigen. Der obere Strang fÃ¼hrt Ã¼ber Drive~1 (Achse~X, grÃ¼n $=$ Betrieb) zu Drive~3
  (Achse~Z, \textbf{rot $=$ Fehler}), der untere Ã¼ber Drive~2 (Achse~Y) zu I/O-Coupler
  EK1100 und I/O-Modul EL2008. Der Encoder EL5151 ist als Abzweigung von Drive~3
  dargestellt. Ãœber jedem Slave-Knoten ist ein farbiger Kreis mit StatuskÃ¼rzel
  dargestellt: \textbf{grÃ¼n OP} (Operational) fÃ¼r alle funktionierenden GerÃ¤te,
  \textbf{rot ERR} fÃ¼r Drive~3~(Achse~Z), das einen Fehler meldet. Die
  Statuszeile am unteren Rand zeigt Netzwerkparameter: Zykluszeit 250\,Âµs, Working
...
Er zÃ¤hlt, wie viele Slaves korrekt auf den umlaufenden Frame geantwortet haben. Ein WC
...
Engineer App visualisiert diesen Parameter dauerhaft und schlÃ¤gt Alarm bei
Unstimmigkeiten.

\subsection{Engineer App: Oszilloskop-Ansicht}

Die Engineer App bietet eine vollstÃ¤ndige Oszilloskop-Funktion, die Prozessvariablen
in Echtzeit mit einer konfigurierbaren Abtastrate aufzeichnet. Typische AnwendungsfÃ¤lle
sind:

\begin{itemize}[leftmargin=2em]
  \item Analyse von Regelkreisverhalten (Ãœberschwingen, Einregelzeit, StationÃ¤re
    Abweichung).
  ...
  \item Aufzeichnung von Anlaufkurven nach ParameterÃ¤nderungen.
\end{itemize}
...
Die EHAE definiert das \emph{HAG Communication Protocol (HCP)}, das Ã¼ber WebSocket
(RFC~6455) \cite{rfc6455} mit JSON- oder MessagePack-Serialisierung arbeitet. Die Wahl
von WebSocket Ã¼ber alternatives HTTP/REST ist bewusst getroffen: WebSocket unterstÃ¼tzt
Full-Duplex-Kommunikation und Server-seitige Push-Events, was fÃ¼r eine reaktive HMI mit
...
\texttt{ALARM\_EVENT}       & HAG $\to$ Client      & Alarm ausgelÃ¶st oder quittiert \\
...
Das Feld \texttt{quality} folgt der OPC-UA-QualitÃ¤tssemantik (GOOD, UNCERTAIN, BAD)
...
Datentyp, Skalierung, Einheit, Zugriffsrechte und die erforderliche Mindestrolle fÃ¼r
jeden Parameter.
...
Manager, GrenzwertÃ¼berschreitungen direkt aus der HVM-Konfiguration abzuleiten, ohne
...
Die Sicherheitsarchitektur der EHAE basiert auf drei aufeinander aufbauenden SÃ¤ulen:
Transport-VerschlÃ¼sselung (TLS~1.3), Authentifizierung (OAuth~2.0 mit PKCE) und
Autorisierung (Role-Based Access Control, RBAC).

\subsection{Authentifizierungsablauf}

Abbildung~\ref{fig:auth_flow} zeigt die vollstÃ¤ndige OAuth-2.0-PKCE-Sequenz fÃ¼r den
...
\caption{EHAE-Authentifizierungssequenz: OAuth~2.0 mit PKCE (Proof Key for Code
  ...
  Login-Seite (Schritt~2). Nach Eingabe der Credentials und Ãœbermittlung des Code-Verifiers
  (Schritt~3) stellt der Identity Provider ein JWT-Access-Token mit 1~Stunde Laufzeit und
  ein Refresh-Token aus (Schritt~4). In Schritt~5 verbindet sich die App Ã¼ber TLS~1.3
  mit dem HAG und Ã¼bergibt das Bearer-Token. Der HAG verifiziert das Token durch
  Token-Introspection beim Identity Provider (Schritte~6--7) und akzeptiert die Verbindung
  mit Ãœbertragung der initialen Variable-Datenbank (Schritt~8). Abschlie\ss{}end
  abonniert die App ihre benÃ¶tigten Variablen (Schritt~9).}
\label{fig:auth_flow}
\end{figure}

Die Verwendung von PKCE \cite{rfc7636} ist entscheidend fÃ¼r die Sicherheit auf mobilen
GerÃ¤ten, da diese keine Client-Secrets sicher speichern kÃ¶nnen. PKCE stellt sicher, dass
nur der Initiator des Auth-Flows das Token einlÃ¶sen kann, selbst wenn der Authorization
Code abgefangen wird.

\subsection{Token-Management}

JWTs enthalten folgende standardisierten Claims gemÃ¤\ss{} RFC~7519:

\begin{itemize}[leftmargin=2em]
  \item \textbf{sub:} Eindeutige Benutzer-ID.
  \item \textbf{iat/exp:} Ausstellungs- und Ablaufzeit (1 Stunde TTL).
  \item \textbf{ehae\_role:} EHAE-spezifischer Claim fÃ¼r die Rolle (VIEWER, OPERATOR,
    ENGINEER, ADMIN).
...
Tabelle~\ref{tab:rbac} zeigt die vollstÃ¤ndige RBAC-Matrix fÃ¼r alle vier EHAE-Rollen.
...
\toprule
\textbf{FÃ¤higkeit}                & \textbf{Viewer} & \textbf{Operator} & \textbf{Engineer} & \textbf{Admin} \\
\midrule
...
RollenÃ¤nderung                    &            &            &            & \checkmark \\
\bottomrule
\end{tabularx}
\end{table}

Die RBAC-Engine prÃ¼ft bei jeder eingehenden \texttt{WRITE\_VAR}- oder
...
Schreiboperation wird nur ausgefÃ¼hrt, wenn beide Bedingungen erfÃ¼llt sind.

\subsection{Netzwerksicherheit}

Alle WebSocket-Verbindungen sind obligatorisch Ã¼ber TLS~1.3 \cite{rfc8446} gesichert.
TLS~1.3 bietet gegenÃ¼ber TLS~1.2 wichtige Verbesserungen:

\begin{itemize}[leftmargin=2em]
  \item Reduzierter Handshake auf 1 Round-Trip-Time (1-RTT).
  \item Entfernung unsicherer Cipher Suites (RC4, 3DES, etc.).
  \item Forward Secrecy fÃ¼r alle Verbindungen durch ephemere Diffie-Hellman-SchlÃ¼ssel.
  \item 0-RTT-Wiederverbindung fÃ¼r bekannte Server (mit Replay-Schutz).
\end{itemize}
...
\end{tikzpicture}
\caption{Empfohlene Netzwerksegmentierung fÃ¼r EHAE mit drei VLANs. Das Diagramm
  zeigt die vollstÃ¤ndige Netzwerktopologie von oben nach unten. Ganz oben steht der
  \emph{Cloud Monitor} (blau, Ellipse), der Ã¼ber einen zentralen \emph{Firewall/Router}
  (grau) mit drei separaten VLANs verbunden ist: VLAN~10 fÃ¼r das
  Unternehmens-/BÃ¼ronetz (links), VLAN~20 fÃ¼r das Produktionsnetz (Mitte) und
  VLAN~30 fÃ¼r das HMI-Netz (rechts). Im Produktionsnetz (VLAN~20) befindet sich der
  ...
  802.11ax}, Ã¼ber den \emph{Operator-Tablet} und \emph{Engineer-Laptop}
  (gestrichelte Pfeile = WLAN) verbunden sind. Im BÃ¼ronetz (VLAN~10) ist ein
  \emph{Office PC} fÃ¼r die Viewer-Rolle angebunden. Die Firewall kontrolliert den
...
kompromittierte HMI-GerÃ¤te direkten Zugriff auf das EtherCAT-Produktionsnetz erhalten.
Der gesamte Datenfluss lÃ¤uft ausschlie\ss{}lich Ã¼ber den HAG, der als einziger
...
Tabelle~\ref{tab:latency} zeigt das End-to-End-Latenzbudget fÃ¼r die app-basierte HMI.

\begin{table}[H]
\centering
\caption{End-to-End-Latenzbudget fÃ¼r app-basiertes HMI.}
\label{tab:latency}
...
HAG-Verarbeitung       & Mapping, Ã„nderungserkennung    & 0{,}1\,ms                   & 0{,}5\,ms \\
WebSocket-Framing      & JSON-Serialisierung            & 0{,}5\,ms                   & 2\,ms \\
Wi-Fi~6                & Drahtlose Ãœbertragung          & 1--3\,ms                    & 10\,ms \\
...
Eine Gesamtlatenz von typisch 10\,ms ist fÃ¼r alle HMI-Anwendungsszenarien vollkommen
ausreichend. Die menschliche Wahrnehmungsschwelle fÃ¼r visuelle Reaktionen liegt bei ca.
100\,ms; damit bietet EHAE einen komfortablen 10-fachen Sicherheitsabstand. Lediglich
fÃ¼r hochdynamische Steuerungseingriffe (Not-Halt etc.) muss beachtet werden, dass
Not-Halt-Funktionen weiterhin direkt am Feldbus oder als Safety-Funktion (STO) im Antrieb
implementiert werden mÃ¼ssen --- nicht Ã¼ber die HMI-App.
...
%\node[card] at (0, 4.2) {};
%\node[font=\small\bfseries, color=appgreen]  at (-1.5, 4.4) {LÃ„UFT};
...
%  Informationskarten: (1)~Maschinenstatus in GrÃ¼n ("{}LÃ„UFT"{}) mit aktueller
%  Drehzahl 1250~rpm im Automatikmodus, (2)~eine orange Warnmeldung fÃ¼r Achse~Z mit
%  Temperatur 72\,\textdegree C bei einem Grenzwert von 80\,\textdegree C sowie einem "{}ACK"{}-Button zum
%  Quittieren, (3)~ein ProduktionszÃ¤hler mit aktuellem Stand 1\,284 Teilen gegenÃ¼ber
%  dem Tagesziel von 1\,500, (4)~ein Fortschrittsbalken fÃ¼r die Bandgeschwindigkeit
...
%wichtigsten Informationen (Status, kritische Alarme) sind sofort sichtbar, wÃ¤hrend
%detailliertere Ansichten (Trends, Parameterdetails) durch Navigation in weiteren
%Reitern erreichbar sind. Die Farbcodierung ist konsistent: GrÃ¼n fÃ¼r Normalbetrieb,
%Orange fÃ¼r Warnungen, Rot fÃ¼r Fehler und Notoperationen.
...
\caption{Vierstufige Migrationsstrategie von traditionellen HMI-Panels zu EHAE. Das
  ...
  PDO/SDO-Anforderungen analysiert und Benutzerrollen definiert. \textbf{Phase~2 (grÃ¼n,
  ...
  Operatoren verteilt, die Engineer App wird fÃ¼r alle Inbetriebnahmen genutzt, und die
...
entsteht. In Phase~2 laufen beide Systeme parallel, und erst nach vollstÃ¤ndiger Validierung
des app-basierten HMI werden die Panels abgebaut. Dies minimiert das Risiko und erlaubt
einen kontrollierten Ãœbergang auch in laufenden Produktionsumgebungen.
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Fallback-Plan:} WÃ¤hrend Phase~2 und~3 bleibt das traditionelle Panel als
    RÃ¼ckfalloption aktiv.
  ...
  \item \textbf{Schrittweise Rollenfreigabe:} Operators erhalten zunÃ¤chst nur
  ...
  \item \textbf{Audit-Log:} Alle Schreiboperationen und Alarmereignisse werden im HAG
    protokolliert und sind fÃ¼r Audits nachverfolgbar.
\end{itemize}
...
zugrunde: Die Mobile App fÃ¼r Android ist \emph{keine} eigenstÃ¤ndige Entwicklung,
sondern eine direkte Kapselung der bereits vollstÃ¤ndig funktionsfÃ¤higen Web-App in eine
...
erheblich: jede Ã„nderung an der Web-App steht automatisch auch in der Mobile App zur
VerfÃ¼gung, ohne dass eine zweite Codebase gepflegt werden muss.

\subsection{Das Capacitor-Prinzip}

Capacitor ist ein plattformÃ¼bergreifendes App-Laufzeit-Framework, das Web-Apps
(HTML, CSS, JavaScript) als vollwertige native Apps auf Android und iOS ausfÃ¼hrt.
...
(\texttt{android.webkit.WebView}) ein und stellt eine JavaScript-Bridge zur VerfÃ¼gung,
Ã¼ber die Web-Code auf native GerÃ¤teschnittstellen zugreifen kann.
...
\draw[decorate, decoration={brace,amplitude=8pt}, color=appgreen]
  ...
  {Web-Layer\\(plattform-\\unabhÃ¤ngig)};
\end{tikzpicture}
\caption{Capacitor-Schichtenmodell der EHAE-Android-App. Das Diagramm zeigt fÃ¼nf
  aufeinander aufbauende Schichten von unten nach oben. Ganz unten (grau) das
  \emph{Android-Betriebssystem (AOSP)}. DarÃ¼ber (blau) die \emph{Capacitor Native
  ...
  ermÃ¶glicht den bidirektionalen Datenaustausch zwischen nativem Kotlin/Java-Code
  und dem JavaScript der Web-App. Die vierte Schicht (grÃ¼n) ist die
  ...
  plattformunabhÃ¤ngigen Web-Layer.}
...
als dÃ¼nner Wrapper, der die Web-App in eine vollwertige Android-APK verwandelt.
...
Das Verzeichnis \texttt{www/} enthÃ¤lt die Web-App-Assets --- identisch mit der
Desktop-Browser-Version unter \texttt{src/index.html}. Capacitor liest diese Assets
beim Build-Vorgang ein und bÃ¼ndelt sie in das Android-App-Bundle, so dass die App
offline und ohne externe Netzwerkzugriffe auf die eigene UI-Logik lauffÃ¤hig ist.
...
\begin{description}[leftmargin=2em]
  \item[\texttt{appId}] Eindeutiger Android-App-Bezeichner nach Reverse-Domain-Notation
    (\texttt{de.epp.ehae}), der die App im Google Play Store und auf dem GerÃ¤t identifiziert.
  ...
  \item[\texttt{androidScheme: "https"}] Erzwingt das HTTPS-Schema fÃ¼r interne
  ...
  \item[\texttt{allowMixedContent}] Erlaubt gemischte HTTP/HTTPS-Verbindungen im
    Intranet-Kontext, damit die App auch Ã¼ber unverschlÃ¼sselte lokale HAG-Verbindungen
    kommunizieren kann.
  \item[\texttt{backgroundColor}] Setzt die initiale Hintergrundfarbe auf das
    EHAE-Dunkelblau (\texttt{\#07090f}) fÃ¼r einen nahtlosen Ãœbergang vom Splash-Screen
    zur vollstÃ¤ndig geladenen Web-App.
  \item[\texttt{overrideUserAgent}] Identifiziert App-Anfragen als
    \texttt{EHAE-HMI/1.0 Android}, was im HAG fÃ¼r Logging und
    gerÃ¤tetyp-spezifische Optimierungen genutzt werden kann.
\end{description}

\subsection{AbhÃ¤ngigkeiten und Framework-Version}

Tabelle~\ref{tab:npm_deps} listet die npm-AbhÃ¤ngigkeiten der Android-App auf.

\begin{table}[h]
\centering
\caption{npm-AbhÃ¤ngigkeiten der EHAE-Android-App (\texttt{package.json}).}
\label{tab:npm_deps}
\begin{tabularx}{\textwidth}{llX}
\toprule
...
Android~14 (\texttt{minSdkVersion}~22, \texttt{targetSdkVersion}~34) lauffÃ¤hig ist.
Das Plugin \texttt{@capawesome/\newline capacitor-android-edge-to-edge-support} ermÃ¶glicht die
vollkantige Darstellung ohne Statusbar- oder Navigationsbar-Unterbrechung --- kritisch
fÃ¼r HMI-Dashboards, die maximale BildschirmflÃ¤che benÃ¶tigen.
...
\caption{Build-Workflow der EHAE-Android-App. Das Ablaufdiagramm zeigt vier Schritte.
  ...
  getestet --- ohne Android-AbhÃ¤ngigkeiten. \textbf{Schritt~2 (blau):} Die fertigen
  Web-Assets (\texttt{index.html}) werden in das \texttt{www/}-Verzeichnis des
  Capacitor-Projekts kopiert. \textbf{Schritt~3 (grÃ¼n):} \texttt{cap sync android}
  ...
  assembleRelease} baut das vollstÃ¤ndige Android-App-Bundle. Das Ergebnis (rot) ist
  eine APK oder ein AAB, die direkt auf Android-GerÃ¤ten installiert oder im
  Google Play Store verÃ¶ffentlicht werden kann.}
...
Capacitor-Ansatzes gegenÃ¼ber einer nativen Android-Entwicklung zusammen.
...
\midrule
Codebase           & Eine gemeinsame Codebase fÃ¼r Web und Android
                   & Separate Android-Codebase (Kotlin/Java) \\
Wartungsaufwand    & Einmalige Pflege der Web-App
                   & Doppelter Aufwand (Web und Android) \\
Feature-ParitÃ¤t    & Automatisch: Web $\equiv$ App
                   & Manuelle Synchronisation erforderlich \\
UI-Tests           & Direkt im Browser ausfÃ¼hrbar
                   & Android-Emulator oder -GerÃ¤t notwendig \\
...
EinstiegshÃ¼rde     & Niedrig (Web-Kenntnisse genÃ¼gen)
...
und sofort im Browser getestet. Nach der Verifikation genÃ¼gt ein \texttt{cap sync
android} gefolgt von einem Gradle-Build, um die identische FunktionalitÃ¤t als
Android-APK auszuliefern. Es entsteht kein redundanter Code, keine Divergenz zwischen
Web- und App-Version und kein zusÃ¤tzlicher QualitÃ¤tssicherungsaufwand fÃ¼r eine zweite
Plattform. Dieses Prinzip spiegelt exakt das EHAE-Gesamtkonzept wider: maximale
Standardisierung und minimaler proprietÃ¤rer Aufwand.
...
\begin{description}[leftmargin=2em]
  \item[Edge-to-Edge-Darstellung] Das Plugin
    ...
    das gesamte Displayformat fÃ¼r das HMI-Dashboard zu nutzen --- entscheidend fÃ¼r
  ...
  \item[Querformat-Fixierung] Das Android-Manifest konfiguriert
    ...
    geÃ¶ffnet wird. HMI-Dashboards sind auf Querformat optimiert und wÃ¼rden im
    Hochformat wichtige Informationsdichte verlieren.
  \item[Intranet-Verbindungen] Im Produktionsnetz kommuniziert die App Ã¼ber
    unverschlÃ¼sselte HTTP-Verbindungen mit dem lokalen HAG.
    ...
    TLS-Anforderung fÃ¼r externe Verbindungen aufzuweichen.
  \item[Splash-Screen] Ein 1{,}5-sekÃ¼ndiger Splash-Screen in der EHAE-Hintergrundfarbe
    \texttt{\#07090f} Ã¼berbrÃ¼ckt die WebView-Ladezeit und vermittelt ein
    professionelles, natives Erscheinungsbild.
  \item[User-Agent-Identifikation] Der Ã¼berschriebene User-Agent
...
Abbildung~\ref{fig:cumulative_cost} zeigt die kumulative 5-Jahres-Kostenentwicklung fÃ¼r
...
\end{axis}
\end{tikzpicture}
\caption{Kumulative 5-Jahres-Kosten fÃ¼r 20 HMI-Stationen: traditionelles HMI-Panel vs.\
  EHAE-LÃ¶sung. Das Liniendiagramm zeigt zwei Kurven Ã¼ber fÃ¼nf Jahre. Die \textbf{blaue
  ...
  akkumulieren), und erreicht nach Jahr~5 \EUR{214\,000}. Die \textbf{grÃ¼ne Kurve}
  (EHAE app-basiert) verlÃ¤uft deutlich flacher: Jahr~1 \EUR{16\,000} (Tablets +
  HAG-Softwarelizenzen), danach nur geringe ZuwÃ¤chse durch minimale Wartungskosten,
  Endstand nach Jahr~5: \EUR{40\,000}. Dies entspricht einer \textbf{Kostenreduktion
  von 81\,\%}. Die Schere zwischen beiden Kurven Ã¶ffnet sich von Jahr zu Jahr weiter,
  da traditionelle Panels hohe laufende Wartungs- und Lizenzkosten verursachen.}
\label{fig:cumulative_cost}
\end{figure}

\subsection{Nicht-monetÃ¤re Vorteile}

Neben den direkten Kostenersparnissen bietet EHAE weitere Vorteile, die schwerer
monetÃ¤r quantifizierbar, aber operativ signifikant sind:

\begin{description}[leftmargin=2em]
  \item[Verbesserte DiagnosefÃ¤higkeit] Die Engineer App mit integriertem Topologie-Viewer,
    Oszilloskop und SDO-Browser ermÃ¶glicht Inbetriebnahmen und Fehlerdiagnosen, die mit
    traditionellen Panels nicht mÃ¶glich waren.
  \item[OrtsunabhÃ¤ngiges Monitoring] Fernzugriff Ã¼ber das Cloud-Monitoring-Modul erlaubt
    OEM-Support und Remote-Diagnose ohne Vor-Ort-Besuch.
  \item[KÃ¼rzere Update-Zyklen] App-Updates werden zentral Ã¼ber App-Stores ausgerollt;
    keine manuelle Panel-Aktualisierung notwendig.
  \item[Zukunftssicherheit] Consumer-Hardware-Ã–kosysteme (iOS, Android, Windows)
    werden langfristig von gro\ss{}en Unternehmen gepflegt und stÃ¤ndig verbessert.
\end{description}
...
zentralen BeitrÃ¤ge dieser Arbeit sind:

\begin{enumerate}[leftmargin=2em]
  \item Der \emph{HMI Application Gateway (HAG)}: ein neues Master-Stack-Modul, das
    keine Ã„nderungen an der Slave-Firmware erfordert und vollstÃ¤ndig rÃ¼ckwÃ¤rtskompatibel
    ist.
  \item Das \emph{HAG Communication Protocol (HCP)}: WebSocket-basierter,
    JSON-serialisierter Echtzeit-Variablenzugriff mit Alarm-Delivery, SDO-UnterstÃ¼tzung,
  ...
  \item Nachgewiesene Kosteneinsparungen von bis zu 81\,\% Ã¼ber 5 Jahre bei gleichzeitiger
    erheblicher Verbesserung der AnzeigequalitÃ¤t und DiagnosefÃ¤higkeit.
\end{enumerate}
...
Die in dieser Arbeit vorgestellte EHAE-Architektur bildet eine tragfÃ¤hige Basis fÃ¼r
eine Vielzahl weiterfÃ¼hrender Entwicklungen. Die folgenden Abschnitte beschreiben
...
industriellen EtherCAT-Umfeld vollstÃ¤ndig auszuschÃ¶pfen.

\subsubsection*{Funktionale Sicherheit -- EHAE Safety Extension}

Das aktuelle EHAE-Modell ist fÃ¼r informationally-kritische, aber nicht fÃ¼r
sicherheitsgerichtete Anwendungen ausgelegt. Eine zukÃ¼nftige \emph{EHAE Safety
Extension} wÃ¼rde einen separaten, redundant Ã¼bertragenen HMI-Kanal nach
IEC~61508\,SIL\,2 und EN~ISO~13849\,PLd definieren. Dabei sind folgende
Teilprobleme zu lÃ¶sen:

\begin{itemize}[leftmargin=2em]
  \item \textbf{Gesicherte BefehlsbestÃ¤tigung:} Jeder sicherheitsrelevante
    ...
    ergÃ¤nzen, das einen Hardware-Token (FIDO2/U2F) oder einen
    Biometrie-Nachweis des Bedieners einschlieÃŸt.
  \item \textbf{Latenz-Budget-Garantien:} FÃ¼r sicherheitsrelevante Kommandos
  ...
  \item \textbf{Watchdog-Mechanismus:} Der HAG muss bei Verbindungsverlust zur
    ...
    Ã¼bergehen und dies per \verb|SAFETY_TIMEOUT|-Alarm signalisieren.
  \item \textbf{Zertifizierungsweg:} Eine Zusammenarbeit mit einem PrÃ¼finstitut
    (TÃœV, DEKRA) ist notwendig, um eine typische EHAE-Safety-Installation nach
    IEC~62443-3-3 und EN~ISO~13849 zu zertifizieren. Das Ergebnis wÃ¤re ein
    standardisiertes PrÃ¼fverfahren fÃ¼r ETG-Mitglieder.
\end{itemize}
...
Time-Sensitive Networking (TSN, IEEE~802.1Q-2022) und Wi-Fi~7 (IEEE~802.11be) erÃ¶ffnen
neue MÃ¶glichkeiten:

\begin{itemize}[leftmargin=2em]
  \item \textbf{TSN-Integration:} Durch IEEE~802.1Qbv (Time-Aware Shaper) kÃ¶nnen
    HCP-Pakete in dedizierten Sendezeitfenstern (Guard Bands) priorisiert werden.
    Dies ermÃ¶glicht eine garantierte Latenz von unter $5\,\si{\milli\second}$ fÃ¼r
    PDO-Updates auch in Ã¼berlasteten Netzwerken -- relevant fÃ¼r
    Montagelinien mit mehreren parallelen EHAE-Clients.
  \item \textbf{Wi-Fi~7 Multi-Link Operation (MLO):} IEEE~802.11be erlaubt
    simultane Ãœbertragung Ã¼ber mehrere FrequenzbÃ¤nder (2,4\,GHz, 5\,GHz, 6\,GHz).
    FÃ¼r EHAE bedeutet dies eine drastische Reduktion des Jitters bei
    drahtlosen Tablets in der Fabrikhalle. Eine HAG-seitige Implementierung
    mÃ¼sste differenzierte DSCP-Markierungen fÃ¼r HCP-Nachrichten gegenÃ¼ber
    Hintergrundverkehr vergeben.
  \item \textbf{5G-Industrial-Private-Networks:} In weitlÃ¤ufigen Anlagen (z.\,B.\
    Logistikzentren) sind 5G-Campus-Netzwerke mit URLLC-Profil (\emph{Ultra
    Reliable Low Latency Communication}) eine realistische Option fÃ¼r
    ...
    unterstÃ¼tzen, ohne aktive WebSocket-Sessions zu unterbrechen (Session
    Migration via QUIC Connection ID Migration).
\end{itemize}

\subsubsection*{KÃ¼nstliche Intelligenz und prÃ¤diktive Wartung}

Die PDO-Zeitreihendaten, die der HAG bereits sammelt, stellen ein hochwertiges
Rohmaterial fÃ¼r maschinelles Lernen dar. Konkrete Ausbaustufen:
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{On-Device-Anomalieerkennung im HAG:} Ein leichtgewichtiges
    ...
    AusreiÃŸer in Stromaufnahme, Temperatur oder Positionsfehlern erkennen,
    bevor ein Alarm ausgelÃ¶st wird. Die Schwellenwerte werden durch
    unÃ¼berwachtes Lernen in der Anlaufphase der Maschine automatisch kalibriert.
  \item \textbf{Cloud-basiertes Fleet-Learning:} Mehrere Anlagen desselben
    Typs kÃ¶nnen aggregierte (anonymisierte) Degradationsdaten in ein zentrales
    ML-Backend (z.\,B.\ Azure Machine Learning, AWS SageMaker) einspeisen.
    Das daraus gewonnene globale Modell wird per FoE Ã¼ber den HAG auf alle
    Slaves verteilt, die ein lokales Inferenz-Modul enthalten.
  \item \textbf{Natural Language Interface:} Mittelfristig ist ein
    KI-Assistent denkbar, der auf dem mobilen HMI-Client lÃ¤uft und dem
    Bediener auf Basis aktueller Maschinendaten in natÃ¼rlicher Sprache
    Diagnosen und Handlungsempfehlungen liefert (z.\,B.:\ \emph{``Achse 3
    zeigt erhÃ¶hten Schleppfehler -- Kugelgewindetrieb prÃ¼fen''}).
    Large Language Models (LLMs) mit On-Device-Inferenz auf
    Apple-Silicon- bzw.\ Qualcomm-Snapdragon-Chips wÃ¤ren hier praktikabel.
  \item \textbf{Reinforcement Learning fÃ¼r Rezeptoptimierung:} FÃ¼r
    parametrierbare Prozesse (z.\,B.\ SchweiÃŸnÃ¤hte, Dosierungen) kÃ¶nnte
    ein RL-Agent auf Basis beobachteter QualitÃ¤tskennzahlen die
...
Die EHAE-App-Schnittstelle ist fÃ¼r 2D-Touchscreens ausgelegt. ZukÃ¼nftige
Interaktionsformen werden deutlich rÃ¤umlicher:
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Spatial Computing (Apple Vision Pro, Meta Quest Pro):}
    ...
    bereits auf einer Web-App basiert, genÃ¼gt eine Erweiterung der
    HMI-Komponenten um \texttt{<a-frame>}-Elemente oder Three.js, um
    3D-Maschinensymbole Ã¼ber den physischen Slaves schweben zu lassen.
    PDO-Echtzeitwerte wÃ¼rden direkt am Slave-GehÃ¤use visualisiert.
  \item \textbf{Microsoft HoloLens 2 / RealWear HMT-1:} FÃ¼r Wartungstechniker
    in industriellen Umgebungen ermÃ¶glicht eine EHAE-AR-Erweiterung
    hands-free Diagnose: QR-Code am Slave scannen, HAG liefert
    automatisch Topologie-Position, SDO-Werte und Fehlerliste im
    HoloLens-Overlay.
  \item \textbf{Gestensteuerung fÃ¼r sterile Umgebungen:} In der
    Pharma- und Lebensmittelproduktion, wo Touchscreens hygienisch
    problematisch sind, ermÃ¶glicht eine Integration von MediaPipe
    (Google) Hand Tracking den berÃ¼hrungslosen Betrieb des
    EHAE-HMI-Clients -- vollstÃ¤ndig browserbasiert ohne
    zusÃ¤tzliche Treiber.
  \item \textbf{Voice Control:} Die Web Speech API ist in allen
    modernen Browsern verfÃ¼gbar. Sprachkommandos wie \emph{``Zeige
    Fehlerhistorie Achse 2''} oder \emph{``Starte Teach-Modus''} kÃ¶nnten
    maschinenlesbar in HCP-Nachrichten Ã¼bersetzt werden,
    was den Bedienkomfort in LÃ¤rm- und Schutzhandschuh-Szenarien
...
vollstÃ¤ndigen Edge-Computing-Plattform ist ein natÃ¼rlicher nÃ¤chster Schritt:

\begin{itemize}[leftmargin=2em]
  \item \textbf{Plugin-Architektur:} Eine formale Plugin-API (z.\,B.\ auf Basis
    von WebAssembly-Modulen oder dynamisch geladenen Shared Libraries) wÃ¼rde
    es ETG-Mitgliedern erlauben, herstellerspezifische Protokoll-Adapter zu
    entwickeln (z.\,B.\ CoE-Profil-Decoder fÃ¼r SINAMICS-Antriebe, spezifische
    FoE-Handler). Das Plugin-System mÃ¼sste hot-reload-fÃ¤hig sein, um
    ProduktionsstillstÃ¤nde bei Updates zu vermeiden.
  \item \textbf{Containerisierung:} Der HAG als Docker-Container (OCI-konform)
    ...
    Edge Gateway) ermÃ¶glicht orchestrierte Deployments via Kubernetes~K3s.
    Dies vereinfacht Rolling Updates, A/B-Deployments und Health-Monitoring.
  \item \textbf{Multi-Master-UnterstÃ¼tzung:} Anlagen mit mehreren EtherCAT-Linien
    (z.\,B.\ parallele FertigungsstraÃŸen) benÃ¶tigen einen Ã¼bergeordneten
    \emph{EHAE Aggregation Gateway}, der die HAG-Instanzen mehrerer Master
    unter einer einheitlichen HCP-Session zusammenfÃ¼hrt. Der HMI-Client sieht
    dann eine einheitliche Topologie-Ansicht Ã¼ber alle Linien hinweg.
  \item \textbf{HAG-Redundanz (High Availability):} FÃ¼r missionskritische Anlagen
    ist ein Active-Standby-HAG-Paar mit Shared State via Redis oder
    Apache Kafka denkbar. Failover unter $1\,\si{\second}$ ohne
    Client-Session-Unterbrechung wÃ¤re das Zielkriterium.
\end{itemize}
...
sind ideale Eingaben fÃ¼r digitale Zwillinge und IIoT-Plattformen:
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{OPC~UA~FX (Field eXchange):} Die neue OPC~UA~Part~80
    ...
    zwischen FeldgerÃ¤ten Ã¼ber Hersteller- und Protokollgrenzen hinweg. Ein
    HAG-seitiger OPC~UA~FX-Publisher wÃ¼rde Ether\-CAT-PDOs als
    ...
    integrierbar machen.
  \item \textbf{Digital Twin nach ISO~23247:} Der ISO-Standard fÃ¼r digitale
    Zwillinge in der Fertigung definiert eine \emph{Observable Manufacturing
    Element (OME)}-Schnittstelle. Ein EHAE-KompatibilitÃ¤tsmodul kÃ¶nnte
    ...
    AAS (\emph{Asset Administration Shell}, IEC~63278) ausdrÃ¼cken.
  \item \textbf{MQTT-Sparkplug-B-Publisher:} Viele MES- und SCADA-Systeme
    ...
    Topic-Namespace publiziert, wÃ¼rde EHAE ohne weitere Middleware mit
  ...
  \item \textbf{Historisierung und Langzeitarchivierung:} Ein integrierter
    Time-Series-Adapter (InfluxDB, TimescaleDB) im HAG wÃ¼rde PDO-Zeitreihen
    direkt archivieren -- mit konfigurierbarer AuflÃ¶sung (z.\,B.\ $1\,\si{ms}$
    fÃ¼r Debugging, $1\,\si{s}$ fÃ¼r Langzeittrends) und automatischem Downsampling
    per Retention Policy. Die Engineer App kÃ¶nnte historische Daten im gleichen
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{iOS-App:} Capacitor unterstÃ¼tzt iOS nativ. Die vorhandene
    Web-App-Codebase kann ohne Anpassung fÃ¼r den Apple App Store kompiliert
    werden. Sicherheitsrelevante ErgÃ¤nzungen fÃ¼r iOS wÃ¤ren die Integration von
    Face~ID/Touch~ID als PKCE-ergÃ¤nzendem Second Factor sowie die Nutzung
    des iOS Secure Enclave fÃ¼r JWT-Refresh-Token-Speicherung.
  \item \textbf{Progressive Web App (PWA):} Durch ein Service Worker-Manifest
    ...
    Besonders relevant fÃ¼r Windows-Arbeitsstationen im BÃ¼ro/Engineering-Umfeld,
    die keine dedizierte App-Installation erlauben.
  \item \textbf{Desktop-App via Electron/Tauri:} FÃ¼r die Engineer App, die
    auf groÃŸen Monitoren mit Maus- und Tastaturinteraktion genutzt wird, bietet
    Tauri (Rust-basiert, $\sim$3\,MB BinÃ¤rgrÃ¶ÃŸe) eine schlanke Alternative zu
    Electron. Die gleiche Web-App-Codebase wird als native Desktop-Applikation
    fÃ¼r Windows, macOS und Linux verpackt. Native OS-Integrationen (Systemtray,
    Dateiexport, serieller Port fÃ¼r direkten EtherCAT-Zugriff) kÃ¶nnen per
    Tauri-Plugin ergÃ¤nzt werden.
  \item \textbf{Chromebook / Android-Enterprise-Deployment:} In
    Unternehmensumgebungen mit Google Workspace lassen sich EHAE-Apps
    Ã¼ber die Android-Enterprise-Management-API (AMAPI) zentral deployen,
    konfigurieren und in einem Kiosk-Modus sperren -- ohne manuelle
    GerÃ¤teeinrichtung durch den Anlagenbetreiber.
\end{itemize}

\subsubsection*{Standardisierung und ETG-Roadmap}

Der langfristig bedeutsamste Schritt fÃ¼r EHAE ist die formale Aufnahme in den
EtherCAT-Standard:

\begin{itemize}[leftmargin=2em]
  \item \textbf{ETG.1000-Erweiterung (Application Layer):} Die vorliegende
    Arbeit liefert alle notwendigen technischen Grundlagen fÃ¼r einen
    ETG-Proposal. Als nÃ¤chster Schritt ist die Einreichung bei der ETG
    Technical Working Group sowie die DurchfÃ¼hrung eines
    Multi-Vendor-InteroperabilitÃ¤tstests (IOT) mit mindestens drei
    unabhÃ¤ngigen HAG-Implementierungen anzustreben.
  \item \textbf{KonformitÃ¤tstest-Suite:} Analog zu den bestehenden
    ETG-Conformance-Tests fÃ¼r Slaves sollte eine EHAE-Conformance-Test-Suite
    entwickelt werden, die automatisiert prÃ¼ft, ob ein HAG alle HCP-Nachrichten
    korrekt und vollstÃ¤ndig implementiert. Dies ist Voraussetzung fÃ¼r ein
    offizielles ETG-KonformitÃ¤tszertifikat.
  \item \textbf{Open-Source-Referenzimplementierung:} Die VerÃ¶ffentlichung
    einer HAG-Refe\-renzimplementierung unter Apache-2.0-Lizenz (z.\,B.\ auf
    GitHub unter der ETG-Organisation) wÃ¼rde die Akzeptanz des Standards
    massiv erhÃ¶hen. Sie dient ETG-Mitgliedern als Integrationsgrundlage und
  ...
  \item \textbf{IEC-Normungsverfahren:} Mittelfristig sollte EHAE als
    ...
    networks over Ethernet}) eingereicht werden. Dies wÃ¼rde die
    InteroperabilitÃ¤t mit OPC~UA, PROFINET und EtherNet/IP auf normativer
    Ebene regeln und EHAE fÃ¼r GroÃŸunternehmen mit strikten
    Normierungsanforderungen (Automobilindustrie, Luftfahrt) Ã¶ffnen.
\end{itemize}
...
ist fÃ¼r den typischen Maschinenpark ausreichend. FÃ¼r hochsensible Umgebungen
(kritische Infrastruktur, RÃ¼stung, Pharma) sind folgende Erweiterungen zu
untersuchen:

\begin{itemize}[leftmargin=2em]
  \item \textbf{Zero-Trust-Netzwerkarchitektur:} Statt eines perimeterbasierten
    Sicherheitsmodells wÃ¼rde ein Zero-Trust-Ansatz (NIST SP~800-207) jeden
    HCP-Request unabhÃ¤ngig vom Netzwerkstandort des Clients authentifizieren
    und autorisieren. Dies erfordert eine kurzlebige, per-Request-Zertifikat-
    Ausstellung (SPIFFE/SPIRE) und eine kontinuierliche Anomalie-Ãœberwachung
    des Datenverkehrs.
  \item \textbf{Post-Quanten-Kryptografie:} NIST hat 2024 die ersten
    ...
    TLS-Konfiguration auf hybrid-SchlÃ¼sselaustausch (X25519 + ML-KEM-768)
    sollte mittelfristig erfolgen, um die Kommunikation gegenÃ¼ber zukÃ¼nftigen
    Quantencomputern zu schÃ¼tzen.
  \item \textbf{Hardware Security Module (HSM):} FÃ¼r den HAG in
    sicherheitskritischen Anlagen sollte der private TLS-SchlÃ¼ssel in einem
    dedizierten HSM (z.\,B.\ Infineon OPTIGA TPM, NXP SE050) gespeichert
    werden, das physische Tamper-Erkennung bietet und den SchlÃ¼ssel auch
    bei Kompromittierung des Betriebssystems schÃ¼tzt.
  \item \textbf{Audit Log mit Blockchain-Anker:} Zur unverÃ¤nderlichen
    Nachvollziehbarkeit aller Bedienereingriffe (wer hat wann welchen
    Sollwert gesetzt?) kÃ¶nnte ein Audit-Log-Modul Hashchains Ã¼ber
    alle HCP-Schreibkommandos bilden und regelmÃ¤ÃŸig auf einer permissioned
    Blockchain (Hyperledger Fabric) verankern -- relevant fÃ¼r
    regulierte Branchen mit GxP-Anforderungen (FDA 21 CFR Part 11).
\end{itemize}

\subsubsection*{Formale Verifikation und ZuverlÃ¤ssigkeitsanalyse}

FÃ¼r den Einsatz in sicherheitsrelevanten Maschinen ist formale Verifikation
ein wichtiges ZukÃ¼nftiges Arbeitsfeld:
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{TLA\textsuperscript{+}-Modell des HCP-Protokolls:} Das
    ...
    Deadlock- und Liveness-Eigenschaften geprÃ¼ft werden. Dies liefert einen
  ...
  \item \textbf{Fuzzing und Property-based Testing:} HAG-Implementierungen
    sollten einem strukturierten Fuzzing-Programm (AFL++, libFuzzer) fÃ¼r
    den WebSocket-Parser und den JSON-Deserializer unterzogen werden, da
    malformierte Eingaben in Industrieumgebungen durch defekte GerÃ¤te
    realistisch auftreten. ErgÃ¤nzend bietet sich Property-based Testing
    (Hypothesis, QuickCheck) fÃ¼r das Variable-Mapping-Subsystem an.
  \item \textbf{FMEA des Gesamtsystems:} Eine formale Failure Mode and Effects
    Analysis (FMEA) nach IEC~60812 fÃ¼r das EHAE-System wÃ¼rde die
    Schwachstellen-Priorisierung fÃ¼r zukÃ¼nftige HÃ¤rtungsmaÃŸnahmen ermÃ¶glichen
    und ist Voraussetzung fÃ¼r eine Maschinenzulassung nach dem EU \emph{Machinery
