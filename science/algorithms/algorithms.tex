...
Informatik. Zwei der mÃ¤chtigsten und universellsten Entwurfsprinzipien sind die
...
nachgewiesen und vollstÃ¤ndig analysiert: Die \emph{Levenshtein-Distanz} (Edit-Distanz)
reprÃ¤sentiert die Dynamische Programmierung mit einer quadratischen Laufzeit
$\mathcal{O}(m \cdot n)$; der \emph{Merge-Sort-Algorithmus} reprÃ¤sentiert
Teile-und-Herrsche mit der beweisbar optimalen Laufzeit $\mathcal{O}(n \log n)$
fÃ¼r das allgemeine vergleichsbasierte Sortierproblem.
Alle Laufzeitaussagen werden durch formale Definitionen, SÃ¤tze, Lemmata und vollstÃ¤ndige
...
ergÃ¤nzt, die das Wachstumsverhalten, die DP-Tabellenstruktur, den Rekursionsbaum
und den empirischen Laufzeitvergleich anschaulich machen.
Ein ausfÃ¼hrlicher Ausblick behandelt Erweiterungen, offene Probleme und
...
in den AnfÃ¤ngen der modernen Computerwissenschaft erkannte man, dass nicht nur die
\emph{Korrektheit}, sondern auch die \emph{Effizienz} eines Algorithmus -- gemessen
in Zeitbedarf (Laufzeit) und Speicherbedarf (RaumkomplexitÃ¤t) -- von zentraler
Bedeutung ist. Ein korrekt arbeitender, aber exponentiell langsamer Algorithmus ist
in der Praxis oft wertlos; eine quadratische LÃ¶sung kann fÃ¼r kleine Eingaben
akzeptabel, fÃ¼r grosse jedoch untragbar sein.

\textbf{Das Ziel des optimalen Algorithmenentwurfs.}
Gegeben ein Problem mit einer festen Klasse von Eingaben der GrÃ¶sse $n$, sucht man
einen Algorithmus, dessen Laufzeit $T(n)$ so klein wie mÃ¶glich ist -- idealerweise
beweisbar optimal in dem Sinne, dass kein Algorithmus fÃ¼r dieses Problem
asymptotisch schneller sein kann.

Zwei der mÃ¤chtigsten Methoden, mit denen Informatiker und Mathematiker optimale
oder nahezu optimale Algorithmen entwerfen, sind:
\begin{itemize}
    \item \textbf{Dynamische Programmierung (DP):} LÃ¶sung eines Problems durch
          Zerlegung in sich \emph{Ã¼berlappende} Teilprobleme, die jeweils nur
          einmal gelÃ¶st und in einer Tabelle gespeichert werden
    ...
    \item \textbf{Teile-und-Herrsche (Divide and Conquer, D\&C):}
          ...
          gleicher oder Ã¤hnlicher Struktur, deren unabhÃ¤ngige LÃ¶sungen anschliessend
          zu einer GesamtlÃ¶sung zusammengesetzt werden.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Levenshtein-Distanz} (Vladimir Levenshtein, 1965
          ...
          (EinfÃ¼gen, LÃ¶schen, Ersetzen eines Zeichens), um einen String $s_1$
    ...
    \item \textbf{Merge Sort} (John von Neumann, 1945 \cite{vonneumann1945};
          ...
          (Merge). Laufzeit $\mathcal{O}(n \log n)$ -- nachweisbar optimal fÃ¼r
...
formalisieren und alle Laufzeitaussagen vollstÃ¤ndig zu beweisen. Im Einzelnen:
\begin{enumerate}
    \item Formale Definition der Landau-Notation und Rekurrenzgleichungen
    \item VollstÃ¤ndiger formaler Beweis der Korrektheit beider Algorithmen
    \item Laufzeitbeweis fÃ¼r die Levenshtein-Distanz: $\mathcal{O}(m \cdot n)$
    \item Laufzeitbeweis fÃ¼r Merge Sort via Mastertheorem: $\mathcal{O}(n \log n)$
    ...
    \item AusfÃ¼hrlicher Ausblick auf Erweiterungen und offene Forschungsfragen
\end{enumerate}
...
\subsection{Landau-Notation und asymptotische KomplexitÃ¤t}
...
\begin{definition}[$\mathcal{O}$-Notation (obere Schranke)]
    ...
    gibt, sodass fÃ¼r alle $n \geq n_0$ gilt:
    \[
        f(n) \;\leq\; c \cdot g(n).
    \]
\end{definition}
...
\begin{bemerkung}
    Im Kontext der Algorithmenanalyse reprÃ¤sentiert $T(n)$ die (schlimmste-Fall--)
    Laufzeit als Funktion der EingabegrÃ¶sse $n$. Die Landau-Notation abstrahiert
...
\begin{definition}[Rekurrenzgleichung]
    Eine \emph{Rekurrenzgleichung} fÃ¼r die Laufzeit $T(n)$ eines
    ...
    Verkleinerungsfaktor und $f(n)$ der Aufwand fÃ¼r das Teilen und Zusammensetzen ist.
\end{definition}
...
\begin{satz}[Mastertheorem, \cite{cormen2009}]
    ...
    \begin{enumerate}
        \item \textbf{Fall 1:} Wenn $f(n) = \mathcal{O}(n^{\mu - \varepsilon})$ fÃ¼r
              ein $\varepsilon > 0$, dann $T(n) = \Theta(n^\mu)$.
        \item \textbf{Fall 2:} Wenn $f(n) = \Theta(n^\mu)$, dann
              $T(n) = \Theta(n^\mu \log n)$.
        \item \textbf{Fall 3:} Wenn $f(n) = \Omega(n^{\mu + \varepsilon})$ fÃ¼r
              ein $\varepsilon > 0$ und $a\,f(n/b) \leq c\,f(n)$ fÃ¼r $c < 1$
              und grosse $n$, dann $T(n) = \Theta(f(n))$.
    \end{enumerate}
\end{satz}

\begin{proof}[Beweisskizze]
    Die Laufzeit entfaltet sich als geometrische Summe Ã¼ber die Rekursionsebenen.
    Auf Ebene $k$ gibt es $a^k$ Teilprobleme der GrÃ¶sse $n/b^k$, der Gesamtaufwand
    auf Ebene $k$ betrÃ¤gt $a^k \cdot f(n/b^k)$. Die Rekursion endet bei
    ...
    $a^k \cdot c\,(n/b^k)^\mu = c\,n^\mu$, und die Summe Ã¼ber $\log_b n + 1$
    identische Terme liefert $T(n) = \Theta(n^\mu \log n)$. \qedhere
\end{proof}

\subsection{Das Bellman'sche OptimalitÃ¤tsprinzip}

\begin{definition}[OptimalitÃ¤tsprinzip nach Bellman]
    Ein Optimierungsproblem genÃ¼gt dem \emph{Bellman'schen OptimalitÃ¤tsprinzip}
    \cite{bellman1957}, wenn gilt: Jede optimale LÃ¶sung des Gesamtproblems
    enthÃ¤lt optimale LÃ¶sungen aller seiner Teilprobleme.
\end{definition}

\begin{bemerkung}
    Das OptimalitÃ¤tsprinzip ist die notwendige Voraussetzung fÃ¼r die Anwendbarkeit
    der Dynamischen Programmierung. Es sichert zu, dass das optimale Ergebnis eines
    Teilproblems unverÃ¤ndert in die GesamtlÃ¶sung Ã¼bernommen werden kann, ohne dass
    das Teilproblem erneut gelÃ¶st werden muss.
\end{bemerkung}
...
\begin{definition}[Editieroperation]
    ...
    \begin{itemize}
        \item \textbf{EinfÃ¼gung (Insert):} EinfÃ¼gen eines Zeichens $c \in \Sigma$
              an einer beliebigen Position.
        \item \textbf{LÃ¶schung (Delete):} LÃ¶schen eines Zeichens an einer
              beliebigen Position.
...
\begin{definition}[Levenshtein-Distanz]
    Seien $s_1 \in \Sigma^m$ und $s_2 \in \Sigma^n$ zwei Strings der LÃ¤ngen
    ...
    \[
        ...
        s_1 \text{ in } s_2 \text{ Ã¼berfÃ¼hrt}\bigr\}.
    \]
\end{definition}

\begin{beispiel}
    FÃ¼r $s_1 = \texttt{KITTEN}$ und $s_2 = \texttt{SITTING}$ gilt:
    \[
        \mathrm{lev}(\texttt{KITTEN}, \texttt{SITTING}) = 3,
    \]
    realisiert durch: $\texttt{K} \to \texttt{S}$ (Substitution),
    $\texttt{E} \to \texttt{I}$ (Substitution),
    EinfÃ¼gen von $\texttt{G}$ am Ende.
\end{beispiel}

\subsection{Rekursive Charakterisierung und das OptimalitÃ¤tsprinzip}

\begin{lemma}[OptimalitÃ¤tsprinzip fÃ¼r die Edit-Distanz]
    \label{lemma:opt}
    Seien $s_1[1..m]$ und $s_2[1..n]$ zwei Strings. Bezeichne $s_1[1..i]$
    das PrÃ¤fix der LÃ¤nge $i$ von $s_1$ (und analog fÃ¼r $s_2$). Dann gilt
    fÃ¼r $d[i][j] = \mathrm{lev}(s_1[1..i], s_2[1..j])$:
    \[
        d[i][j] \;=\; \begin{cases}
            i & \text{falls } j = 0 \\
            j & \text{falls } i = 0 \\
            ...
            1 + \min\!\begin{cases}
                d[i-1][j]   & \text{(LÃ¶schung)} \\
                d[i][j-1]   & \text{(EinfÃ¼gung)} \\
...
\begin{proof}
    ...
    $s_2[1..j]$ Ã¼berfÃ¼hrt. Wir betrachten die letzte Operation in $\mathcal{E}^*$:

    \textbf{Fall 1:} $s_1[i] = s_2[j]$.
    Die letzte Operation ist keine Substitution des letzten Zeichens
    (das wÃ¤re unnÃ¶tig teuer). Vielmehr kann $s_1[i]$ direkt $s_2[j]$ zugeordnet
    werden (Match, Kosten 0). Es verbleibt das Teilproblem, $s_1[1..i-1]$ in
    $s_2[1..j-1]$ zu Ã¼berfÃ¼hren. Das OptimalitÃ¤tsprinzip sichert:
    $d[i][j] = d[i-1][j-1]$.

    \textbf{Fall 2:} $s_1[i] \neq s_2[j]$. Es gibt drei MÃ¶glichkeiten:
    \begin{itemize}
        \item \emph{LÃ¶schung von $s_1[i]$:} Man lÃ¶scht $s_1[i]$ (Kosten 1) und
              Ã¼berfÃ¼hrt $s_1[1..i-1]$ in $s_2[1..j]$: Gesamtkosten $1 + d[i-1][j]$.
        \item \emph{EinfÃ¼gung von $s_2[j]$:} Man fÃ¼gt $s_2[j]$ am Ende von
              $s_1[1..i]$ ein (Kosten 1) und Ã¼berfÃ¼hrt $s_1[1..i]$ in
              $s_2[1..j-1]$: Gesamtkosten $1 + d[i][j-1]$.
        \item \emph{Substitution $s_1[i] \to s_2[j]$:} Man ersetzt $s_1[i]$ durch
              $s_2[j]$ (Kosten 1) und Ã¼berfÃ¼hrt $s_1[1..i-1]$ in $s_2[1..j-1]$:
              Gesamtkosten $1 + d[i-1][j-1]$.
    \end{itemize}
    Da jede optimale LÃ¶sung eine dieser drei Formen hat und das OptimalitÃ¤tsprinzip
    fÃ¼r Edit-Distanzen gilt (jede Editierfolge auf einem PrÃ¤fix ist Teil der
    optimalen GesamtlÃ¶sung), folgt die Rekurrenz durch Minimumbildung. \qedhere
\end{proof}
...
\begin{figure}[h!]
    ...
    \caption{
        ...
        \textit{Links:} Die $(m+1) \times (n+1)$-DP-Tabelle fÃ¼r
        $s_1 = \texttt{KITTEN}$ und $s_2 = \texttt{SITTING}$ ($m=6, n=7$).
        Blau hervorgehoben ist der optimale Traceback-Pfad von $d[6][7] = 3$
        zurÃ¼ck zu $d[0][0] = 0$; grÃ¼n markierte Zellen zeigen ZeichenÃ¼bereinstimmungen
        (Kosten 0).
        \textit{Rechts:} Die Rekurrenzformel mit den drei mÃ¶glichen VorgÃ¤ngerzellen
        und der zugehÃ¶rigen Laufzeitanalyse. Das Ergebnis $d[6][7] = 3$ bestÃ¤tigt
...
\begin{satz}[Korrektheit des Levenshtein-DP]
    ...
    Nach AusfÃ¼hrung von Algorithmus~\ref{alg:levenshtein} gilt fÃ¼r alle
...
\begin{proof}
    Per vollstÃ¤ndiger Induktion Ã¼ber $i + j$:
    ...
    (genau $j$ EinfÃ¼gungen erforderlich). Analog $d[i][0] = i$. \checkmark

    \textbf{Induktionsschritt} ($i + j > 0$, $i > 0$, $j > 0$):
    Per Induktionsvoraussetzung gilt die Aussage fÃ¼r alle Paare $(i', j')$
    mit $i' + j' < i + j$, insbesondere fÃ¼r $(i-1, j-1)$, $(i-1, j)$, $(i, j-1)$.

    \emph{Fall A ($s_1[i] = s_2[j]$):} Der Algorithmus setzt
    $d[i][j] = d[i-1][j-1]$. Per IV gilt $d[i-1][j-1] = \mathrm{lev}(s_1[1..i-1], s_2[1..j-1])$.
    Da $s_1[i] = s_2[j]$, wird kein Editierschritt fÃ¼r das letzte Zeichenpaar
    benÃ¶tigt, also $\mathrm{lev}(s_1[1..i], s_2[1..j]) = \mathrm{lev}(s_1[1..i-1], s_2[1..j-1])$.
    ...
    (LÃ¶schung, EinfÃ¼gung, Substitution) zuzÃ¼glich 1.
...
\begin{satz}[Laufzeit der Dynamischen Programmierung fÃ¼r Edit-Distanz]
    \label{satz:lev_runtime}
    Algorithmus~\ref{alg:levenshtein} hat eine ZeitkomplexitÃ¤t von
    $\mathcal{O}(m \cdot n)$ und eine RaumkomplexitÃ¤t von $\mathcal{O}(m \cdot n)$.
\end{satz}

\begin{proof}
    \textbf{ZeitkomplexitÃ¤t:}
    Der Algorithmus fÃ¼hrt folgende Schritte aus:
    \begin{enumerate}
        \item Initialisierung der Randzeile: $\mathcal{O}(n)$ Schritte.
        \item Initialisierung der Randspalte: $\mathcal{O}(m)$ Schritte.
        \item Die doppelte \textbf{for}-Schleife: $i$ lÃ¤uft von $1$ bis $m$,
              $j$ von $1$ bis $n$. In jeder Iteration werden genau
              $\mathcal{O}(1)$ Operationen ausgefÃ¼hrt (ein Vergleich,
              hÃ¶chstens drei Tabellenabfragen, eine Minimum-Berechnung,
              eine Zuweisung). Die Gesamtanzahl der Iterationen betrÃ¤gt:
              \[
    ...
    \[
        ...
        \;=\; \mathcal{O}(m \cdot n).
    \]
    FÃ¼r den Spezialfall gleich langer Strings ($m = n = N$) gilt somit
    $T_{\mathrm{lev}}(N) = \mathcal{O}(N^2)$.

    \textbf{RaumkomplexitÃ¤t:}
    Die DP-Tabelle $d$ hat $(m+1)(n+1)$ EintrÃ¤ge, also
...
\begin{figure}[h!]
    ...
    \caption{
        \textbf{Quadratische LaufzeitkomplexitÃ¤t der Dynamischen Programmierung.}
        \textit{Links:} Vergleich der Wachstumskurven $\mathcal{O}(n)$,
        $\mathcal{O}(n \log n)$ und $\mathcal{O}(n^2)$ fÃ¼r $n \in [1, 200]$.
        Die rot schattierte FlÃ¤che unterstreicht den quadratischen Ãœberschuss
        der DP-Methode gegenÃ¼ber subquadratischen Verfahren.
        \textit{Rechts:} Konkrete Zellanzahl der $(N \times N)$-DP-Tabelle
        fÃ¼r verschiedene StringlÃ¤ngen $N$ auf logarithmischer Skala.
        Bei $N = 1000$ umfasst die Tabelle bereits $10^6$ EintrÃ¤ge.
    }
    \label{fig:dp_comp}
\end{figure}

\subsection{Untere Schranke fÃ¼r das Edit-Distanz-Problem}

\begin{satz}[Untere Schranke fÃ¼r Edit-Distanz, \cite{masek1980}]
    Jeder Algorithmus, der die Edit-Distanz zweier Strings der LÃ¤nge $N$ im
    Worst Case korrekt berechnet, benÃ¶tigt $\Omega(N^2 / \log N)$ Zeit
...
\begin{bemerkung}
    ...
    Vier-Russen-Algorithmus \cite{masek1980} erzielt tatsÃ¤chlich
    $\mathcal{O}(N^2 / \log N)$ und ist damit (modulo konstante Faktoren) optimal.
    Ob $\mathcal{O}(N^{2-\varepsilon})$ fÃ¼r ein $\varepsilon > 0$ mÃ¶glich ist,
...
\begin{definition}[Vergleichsbasiertes Sortierverfahren]
    ...
    Universum $\mathcal{U}$ nur Ã¼ber paarweise Vergleiche der Form $a_i \leq a_j$
...
\begin{definition}[Merge-Sort]
    ...
    \begin{enumerate}
        \item \textbf{Teilen (Divide):} Teile das Array $A[l..r]$ in der Mitte:
              ...
              Erhalte linke HÃ¤lfte $A[l..\mathrm{mid}]$ und rechte HÃ¤lfte
              $A[\mathrm{mid}+1..r]$.
        \item \textbf{Herrschen (Conquer):} Sortiere rekursiv beide HÃ¤lften.
        \item \textbf{Zusammensetzen (Merge):} Verschmelze (merge) die beiden
              sortierten HÃ¤lften in $\mathcal{O}(n)$ Zeit zu einem sortierten Array.
    \end{enumerate}
\end{definition}
...
\begin{figure}[h!]
    ...
    \caption{
        ...
        \textit{Links:} VollstÃ¤ndiger Rekursionsbaum fÃ¼r $n = 8$.
        ...
        GrÃ¼n hervorgehoben sind die Merge-Schritte auf dem Weg zurÃ¼ck zur Wurzel.
...
\begin{satz}[Korrektheit von Merge Sort]
    ...
    Nach AusfÃ¼hrung von \textsc{MergeSort}$(A, 1, n)$ ist $A[1..n]$ aufsteigend
    sortiert.
\end{satz}

\begin{proof}
    Per vollstÃ¤ndiger Induktion Ã¼ber $n = r - l + 1$ (LÃ¤nge des Teilarrays):
    ...
    Es gilt $l \leq \mathrm{mid} < r$, sodass beide HÃ¤lften echt kÃ¼rzer als
    ...
    $L$ bzw.\ $R$ angehÃ¤ngt (diese sind bereits sortiert und grÃ¶sser als alle
...
\begin{lemma}[Laufzeit des Merge-Schritts]
    ...
    \textsc{Merge}$(A, l, \mathrm{mid}, r)$ mit $n = r - l + 1$ lÃ¤uft in
    $\Theta(n)$ Zeit.
\end{lemma}
...
\begin{proof}
    ...
    werden $\mathcal{O}(1)$ Vergleiche und Zuweisungen ausgefÃ¼hrt, und die Schleife
...
\begin{satz}[Laufzeit von Merge Sort]
    ...
    Merge Sort hat eine ZeitkomplexitÃ¤t von $\Theta(n \log n)$ im
    Best-, Average- und Worst-Case.
\end{satz}

\begin{proof}
    Die Rekurrenzgleichung fÃ¼r die Laufzeit $T(n)$ ergibt sich direkt aus der
    Struktur des Algorithmus:
    \begin{equation}
        T(n) \;=\; 2\,T\!\left(\frac{n}{2}\right) + \Theta(n), \qquad T(1) = \Theta(1).
        \label{eq:mergerecur}
    \end{equation}
    Der Term $2\,T(n/2)$ resultiert aus den zwei rekursiven Aufrufen auf HÃ¤lften
    der GrÃ¶sse $\lfloor n/2 \rfloor$ und $\lceil n/2 \rceil$ (der Unterschied ist
    ...
    gibt es $2^k$ Teilprobleme der GrÃ¶sse $n/2^k$, und das Mergen jedes
...
\begin{korollar}[StabilitÃ¤t von Merge Sort]
    Merge Sort ist stabil: Elemente mit gleichem SchlÃ¼ssel behalten ihre
...
\begin{proof}
    ...
    linke Elemente gehen rechten gleicher PrioritÃ¤t voran.
    Da $L$ Elemente aus kleineren Originalindizes enthÃ¤lt, bleibt die
...
\begin{figure}[h!]
    ...
    \caption{
        ...
        \textit{Links:} Doppelt-logarithmischer Vergleich der KomplexitÃ¤tsklassen
        $\mathcal{O}(n)$, $\mathcal{O}(n \log n)$, $\mathcal{O}(n^2)$ und
        $\mathcal{O}(n^3)$ fÃ¼r $n \in [2, 1000]$. Der Vorteil von $n \log n$
        gegenÃ¼ber $n^2$ betrÃ¤gt bei $n = 100$ einen Faktor $\approx 15$ und
        wÃ¤chst weiter linear mit $n/\log_2 n$.
        \textit{Rechts:} Empirische Laufzeitmessung der Python-Implementierung
        im Vergleich zur theoretischen Kurve $c \cdot n \log_2 n$.
        Die hervorragende Ãœbereinstimmung bestÃ¤tigt die theoretische Analyse.
    }
    \label{fig:ms_comp}
\end{figure}

\subsection{Untere Schranke fÃ¼r vergleichsbasiertes Sortieren}

\begin{satz}[Untere Schranke fÃ¼r vergleichsbasiertes Sortieren, \cite{cormen2009}]
    \label{satz:sort_lower}
    Jeder vergleichsbasierte Sortieralgorithmus benÃ¶tigt im Worst Case
    $\Omega(n \log n)$ Vergleiche.
\end{satz}

\begin{proof}
    Jeder vergleichsbasierte Sortieralgorithmus kann als Entscheidungsbaum
    (binÃ¤rer Baum) modelliert werden, dessen BlÃ¤tter den $n!$ mÃ¶glichen
    Permutationen der Eingabe entsprechen.

    Die HÃ¶he $h$ dieses Entscheidungsbaums (worst-case Anzahl Vergleiche)
    erfÃ¼llt:
    \[
        2^h \;\geq\; \text{Anzahl der BlÃ¤tter} \;\geq\; n!
    \]
    ...
    \[
        ...
        \;=\; \Omega(n \log n).
    \]
    Also ist $h = \Omega(n \log n)$ fÃ¼r jeden vergleichsbasierten Algorithmus. \qedhere
\end{proof}

\begin{korollar}[OptimalitÃ¤t von Merge Sort]
...
\begin{proof}
    ...
    -- die asymptotische OptimalitÃ¤t folgt. \qedhere
\end{proof}
...
\begin{figure}[h!]
    ...
    \caption{
        ...
        \textit{Links:} Detaillierter Merge-Schritt: Zwei sortierte HÃ¤lften
...
\begin{figure}[h!]
    ...
    \caption{
        \textbf{VollstÃ¤ndiger Vergleich: Dynamische Programmierung vs.\
        Teile-und-Herrsche.}
        ...
        Panel~(B): Das VerhÃ¤ltnis $n^2/(n \log_2 n) = n/\log_2 n$ wÃ¤chst linear;
        bei $n = 500$ ist $n \log n$ bereits einen Faktor $\approx 56$ schneller.
        Panel~(C): Vergleich der SpeicherkomplexitÃ¤ten -- die DP benÃ¶tigt
        $\mathcal{O}(N^2)$ Speicher, Merge Sort nur $\mathcal{O}(n)$.
        Panel~(D): Tabellarische GesamtÃ¼bersicht beider Algorithmen.
    }
    \label{fig:comparison}
\end{figure}

\begin{definition}[Ãœberlappende vs.\ disjunkte Teilprobleme]
    Seien $P_1, P_2, \ldots, P_k$ die Teilprobleme einer Rekursion.
    \begin{itemize}
        \item Die Teilprobleme heissen \emph{Ã¼berlappend}, wenn
              mindestens ein Teilproblem $P_i$ in mehreren rekursiven Ã„sten
              gemeinsam verwendet wird (d.\,h.\ mehrfach gelÃ¶st werden mÃ¼sste
              ohne Memoization).
        \item Die Teilprobleme heissen \emph{disjunkt}, wenn jedes
              Teilproblem hÃ¶chstens einmal auftritt.
    \end{itemize}
\end{definition}

\begin{bemerkung}
    Dynamische Programmierung ist besonders wirksam bei Ã¼berlappenden
    Teilproblemen: Die naive rekursive Berechnung der Levenshtein-Distanz
    hÃ¤tte ohne Memoization eine Laufzeit von $\mathcal{O}(3^{\max(m,n)})$
    (exponentiell). Die DP-Tabellierung reduziert dies auf $\mathcal{O}(m \cdot n)$,
    indem jedes der $\mathcal{O}(mn)$ Teilprobleme genau einmal gelÃ¶st wird.
...
\begin{satz}[Charakterisierung der Entwurfsprinzipien]
    ...
    Sei $P$ ein Optimierungsproblem der GrÃ¶sse $n$.
    \begin{enumerate}
        \item Wenn $P$ das Bellman'sche OptimalitÃ¤tsprinzip erfÃ¼llt und
              exponentiell viele Ã¼berlappende Teilprobleme hat, so ist
              \emph{Dynamische Programmierung} in der Regel das geeignete
              Entwurfsprinzip.
        \item Wenn $P$ sich in $a$ disjunkte Teilprobleme der GrÃ¶sse $n/b$
              zerlegen lÃ¤sst und der Zusammensetzungsschritt
...
\begin{proof}
    ...
    unreprÃ¤sentiert. Damit ist die Vollst\"{a}ndigkeit verletzt. \qedhere
\end{proof}
...
fÃ¼r jede Operation) ist die einfachste Variante der Edit-Distanz. In der Bioinformatik
und der Sprachverarbeitung sind gewichtete Varianten unerlÃ¤sslich:

\begin{itemize}
    \item \textbf{Gewichtete Edit-Distanz:} Den Operationen EinfÃ¼gen, LÃ¶schen und
          Ersetzen werden unterschiedliche Gewichte $w_I, w_D, w_S \geq 0$ zugeordnet.
          Die DP-Rekurrenz bleibt strukturell gleich, die Laufzeit betrÃ¤gt weiterhin
          $\mathcal{O}(mn)$. Diese Variante ist grundlegend fÃ¼r die Sequenzausrichtung
          in der Bioinformatik (Needleman-Wunsch-Algorithmus \cite{needleman1970},
          Smith-Waterman-Algorithmus \cite{smith1981}).
    \item \textbf{Affine LÃ¼ckenstrafe (Affine Gap Penalty):} In biologischen
          Sequenzen sind zusammenhÃ¤ngende LÃ¼cken (Gaps) hÃ¤ufiger als Punktmutationen.
    ...
    \item \textbf{Damerau-Levenshtein-Distanz:} Erweiterung um die Operation
          ...
          Tippfehlern hÃ¤ufig beobachtet. Die Rekurrenz erfordert zusÃ¤tzliche
          Hilfstabellen, bleibt aber polynomiell \cite{damerau1964}.
    \item \textbf{Longest Common Subsequence (LCS):} Eng verwandt mit der
          Edit-Distanz (nur EinfÃ¼gungen und LÃ¶schungen erlaubt). LCS ist die
...
Edit-Distanz subquadratisch ($\mathcal{O}(n^{2-\varepsilon})$ fÃ¼r $\varepsilon > 0$)
berechnet werden kann. Die \emph{Strong Exponential Time Hypothesis} (SETH)
\cite{williams2005} impliziert, dass dies nicht mÃ¶glich ist: Ein Algorithmus mit
$\mathcal{O}(n^{2-\varepsilon})$ fÃ¼r die Edit-Distanz wÃ¼rde SETH widerlegen.
...
KomplexitÃ¤tstheorie und sind Gegenstand aktueller Spitzenforschung.

\subsubsection{DP in der Graphentheorie und der Subgraph Algorithmus}

Die Dynamische Programmierung ist nicht auf Stringprobleme beschrÃ¤nkt. Im Kontext
der Graphentheorie ermÃ¶glicht DP die effiziente LÃ¶sung von Problemen wie:
\begin{itemize}
    \item \textbf{KÃ¼rzeste Wege:} Der Bellman-Ford-Algorithmus \cite{bellman1958}
          ($\mathcal{O}(VE)$) und der Floyd-Warshall-Algorithmus \cite{floyd1962}
          ($\mathcal{O}(V^3)$) sind prototypische DP-Algorithmen fÃ¼r Graphen.
    \item \textbf{Baumzerlegungen:} Viele NP-schwere Graphenprobleme
          (z.\,B.\ Vertex Cover, Maximum Independent Set) sind auf Graphen
          beschrÃ¤nkter Baumbreite ($\mathrm{treewidth} \leq k$) in
          $\mathcal{O}(f(k) \cdot n)$ lÃ¶sbar durch DP auf dem Zerlegungsbaum.
    \item \textbf{Subgraph Algorithmus} \cite{epp2026subgraph}: Der von Epp
          ...
          lÃ¶st das Subgraph-Isomorphieproblem fÃ¼r wichtige Graphklassen
          ...
          graphentheoretischer DP-AnsÃ¤tze.
\end{itemize}
...
Forschungsgebiet:
\begin{itemize}
    \item \textbf{Viterbi-Algorithmus:} DP-basierter Algorithmus fÃ¼r die
          Maximum-Likelihood-ZustandsschÃ¤tzung in Hidden-Markov-Modellen --
          grundlegend fÃ¼r Spracherkennung, Bioinformatik und Kommunikation.
    \item \textbf{Reinforcement Learning:} Bellman-Gleichungen und
          Q-Learning sind direkte Verallgemeinerungen des Bellman'schen
          OptimalitÃ¤tsprinzips auf stochastische Umgebungen \cite{sutton2018}.
    \item \textbf{Attention-Mechanismen in Transformers:} Die Berechnung der
          Attention-Matrix kann als verallgemeinerte Ã„hnlichkeitsabfrage
...
\begin{itemize}
    \item \textbf{Tim Sort:} Die von Tim Peters entwickelte und in Python und Java
          ...
          fÃ¼r kleine Subarrays ($k < 64$). Durch Ausnutzung natÃ¼rlicher Runs
    ...
    \item \textbf{Natural Merge Sort:} Erkennt und nutzt monotone Teilfolgen
          (Runs) in den Eingabedaten fÃ¼r adaptives, effizienzsteigerndes Mergen.
    \item \textbf{Polyphasisches Merge Sort:} FÃ¼r externes Sortieren (Daten
          passen nicht in den Hauptspeicher) minimiert polyphasisches Merging
          die Anzahl der I/O-Operationen -- fundamental fÃ¼r Datenbanksysteme
...
Algorithmik und hat zu bahnbrechenden Algorithmen gefÃ¼hrt:
...
\begin{itemize}
    ...
    \item \textbf{Closest Pair of Points}: NÃ¤chstes Punktpaar in $\mathcal{O}(n \log n)$
...
disjunkten Teilprobleme unabhÃ¤ngig voneinander berechnet werden kÃ¶nnen:
...
\begin{itemize}
    ...
    \item \textbf{MapReduce/Spark:} Das MapReduce-Paradigma \cite{dean2008}
          realisiert D\&C Ã¼ber Cluster: Die Map-Phase zerlegt, die Reduce-Phase
          setzt zusammen. Merge Sort ist ein natÃ¼rliches Lehrbeispiel.
    \item \textbf{GPU-Computing:} Auf NVIDIA-GPUs ermÃ¶glicht die CUDA-basierte
          Bitonic-Sort-Variante ($\mathcal{O}(\log^2 n)$ parallele Schritte)
          das Sortieren von Milliarden Elementen in Sekunden -- relevant fÃ¼r
          Echtzeit-Datenanalyse und maschinelles Lernen.
\end{itemize}

\subsection{Verbindungen zur KomplexitÃ¤tstheorie}

Die hier betrachteten Algorithmen sind eng mit fundamentalen Fragen der
KomplexitÃ¤tstheorie verknÃ¼pft:
...
\begin{itemize}
    \item \textbf{P vs.\ NP:} Sowohl Levenshtein-DP ($\mathcal{O}(n^2)$) als auch
          ...
          Allgemeinen ist NP-vollstÃ¤ndig; der Subgraph Algorithmus \cite{epp2026subgraph}
          lÃ¶st strukturell eingeschrÃ¤nkte Varianten in Polynomialzeit.
    \item \textbf{Raumhierarchie und KommunikationskomplexitÃ¤t:}
          Die RaumkomplexitÃ¤t $\mathcal{O}(n^2)$ der DP kann nicht auf
    ...
    \item \textbf{Vergleichsbasierte Sortierung:} Der Beweis der
          ...
          Entscheidungsbaum-Methode ist ein Paradebeispiel fÃ¼r Informationstheorie
          als Werkzeug der Komple\-xi\-tÃ¤ts\-theorie.
    \item \textbf{Untere Schranken via KommunikationskomplexitÃ¤t:}
          Moderne Untergrenzen fÃ¼r DP-Probleme verwenden das Framework der
          KommunikationskomplexitÃ¤t \cite{kushilevitz1997}, um zu zeigen, dass
...
\begin{itemize}
    \item \textbf{Genomische Sequenzausrichtung:} Das Alignment von DNA-Sequenzen
          ...
          wÃ¤ren ohne effiziente DP-Algorithmen nicht realisierbar.
    \item \textbf{Proteinhomologie:} Die Ã„hnlichkeit von Proteinsequenzen
          (BLOSUM-Matrizen als Substitutionskosten) wird mit DP bestimmt und
          erlaubt RÃ¼ckschlÃ¼sse auf evolutionÃ¤re Verwandtschaft und Funktion.
    \item \textbf{RNA-Strukturvorhersage:} Der Nussinov-Algorithmus und das
          ...
          $[i, j]$ des RNA-Strings zur Vorhersage von SekundÃ¤rstrukturen.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Rechtschreibkorrektur:} Suggestionsalgorithmen (z.\,B.\ in
          Hunspell/LibreOffice) verwenden Levenshtein-Distanz, um Ã¤hnliche
          WÃ¶rter im WÃ¶rterbuch zu finden. FÃ¼r ein WÃ¶rterbuch der GrÃ¶sse $|D|$
          und WÃ¶rter der LÃ¤nge $N$ kostet die naive Suche $\mathcal{O}(|D| \cdot N^2)$.
          BK-BÃ¤ume (Burkhard-Keller-BÃ¤ume \cite{burkhard1973}) reduzieren dies
    ...
    \item \textbf{Maschinelle Ãœbersetzung:} Word-Error-Rate (WER) und
          BLEU-Score-Berechnung verwenden Varianten der Edit-Distanz zur
          automatischen Bewertung von ÃœbersetzungsqualitÃ¤t.
...
Merge Sort ist in nahezu allen grossen Datenmanagementsystemen prÃ¤sent:
\begin{itemize}
    ...
    \item \textbf{Log-Structured Merge Trees (LSM-Trees):} Verwendet von
          LevelDB, RocksDB, Cassandra und HBase fÃ¼r hochperformante
...
\subsection{Theologisch-philosophische Reflexion: Ordnung und SchÃ¶pfung}

Das Sortierproblem -- die Herstellung von Ordnung aus Unordnung -- ist nicht nur
ein algorithmisches, sondern auch ein philosophisch tiefgrÃ¼ndiges Problem.
Aus der Perspektive einer SchÃ¶pfungstheologie im Sinne der Genesis-Exegese
stellt sich die Frage nach der Ordnung als fundamentales SchÃ¶pfungsprinzip:
Der SchÃ¶pfungsakt \emph{ex nihilo} ist in der jÃ¼disch-christlichen Tradition
...
wider: durch Zerlegung in Ã¼berschaubare Teilprobleme (Dividere), deren
getrennte Beherrschung (Conquere) und die ZusammenfÃ¼hrung zur Ordnung (Merge)
entsteht aus einer ungeordneten Eingabe eine vollstÃ¤ndig geordnete Ausgabe.
...
Die Dynamische Programmierung hingegen verkÃ¶rpert das Prinzip des geduldigen,
akkumulativen Aufbaus von Wissen: Jede Teilantwort wird sorgfÃ¤ltig gespeichert
und weiterverwendet -- eine Parallele zur Tradition des Lernens durch
Ãœberlieferung und Wiederholung (\textit{Deuteronomy}). Das Bellman'sche
OptimalitÃ¤tsprinzip formalisiert die Weisheit, dass ein in jedem Schritt optimal
...
\begin{itemize}
    \item Die Tabellierung der Teilgraphisomorphien Ã¼ber alle Knoten-Teilmengen
          entspricht dem DP-Paradigma: Ergebnisse fÃ¼r kleinere Teilmengen
          werden gespeichert und beim Aufbau grÃ¶sserer Teilmengen wiederverwendet.
    \item Die Zerlegung des Suchproblems in unabhÃ¤ngige Teilgraphen und deren
          separate Bearbeitung Ã¤hnelt dem Teile-und-Herrsche-Ansatz.
    \item Die $\mathcal{O}(n^3)$-Laufzeit des Subgraph Algorithmus liegt
          zwischen den $\mathcal{O}(n^2)$ der Edit-Distanz-DP und dem
          allgemeinen Subgraph-Isomorphieproblem (NP-vollstÃ¤ndig).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Quanten-Merge-Sort:} Quantenalgorithmen kÃ¶nnen das Sortierproblem
          nicht sublinear lÃ¶sen ($\Omega(\log n \cdot n)$ Untergrenzen gelten auch
          quantenmechanisch), da Quantensuchalgorithmen (Grover \cite{grover1996})
          nur eine quadratische Beschleunigung fÃ¼r unstrukturierte Suche liefern.
          Jedoch sind Quanten-Parallelisierungen von Merge Sort in
          $\mathcal{O}(\sqrt{n} \log n)$ Tiefe auf kohÃ¤renten Quantenregistern
          diskutiert worden.
    \item \textbf{Quantum-Edit-Distance:} Der Quantenalgorithmus von Childs et al.\
          ...
          Verbesserung gegenÃ¼ber dem klassischen $\mathcal{O}(n^2)$-Algorithmus,
          jedoch noch weit von der OptimalitÃ¤t entfernt.
    \item \textbf{Neuromorphes Computing:} Spike-Train-basierte Implementierungen
          ...
          fÃ¼r sequenzanalytische Anwendungen.
\end{itemize}
...
tiefe mathematische Strukturprinzipien wider: das OptimalitÃ¤tsprinzip, die
...
Programmierung und Teile-und-Herrsche als die zwei fundamentalen SÃ¤ulen des
modernen Algorithmenentwurfs nebeneinander entwickelt. Kleinberg und Tardos
betonen dabei einen Aspekt, der mit dem StrukturminimalitÃ¤tsprinzip (Kapitel~6
dieser Arbeit) in enger Beziehung steht: Ein Entwurfsparadigma ist dann optimal
gewÃ¤hlt, wenn die \emph{Teilproblemstruktur} des Problems isomorph zur
\emph{Zugriffsstruktur} der verwendeten Datenstruktur ist. FÃ¼r Teile-und-Herrsche
bedeutet dies, dass die rekursive Halbierbarkeit des Arrays unmittelbar aus
seiner linearen Indexstruktur $\{1, \ldots, n\}$ folgt; fÃ¼r die Dynamische
Programmierung entspricht die 2D-ZellenabhÃ¤ngigkeit $(i,j) \leftarrow
(i{-}1,j),\,(i,j{-}1),\,(i{-}1,j{-}1)$ genau der Gitterstruktur der
$(m \times n)$-Tabelle. Diese Ãœbereinstimmung zwischen Datenstrukturgeometrie
und Algorithmenlogik ist es, die nach Satz~\ref{satz:dualitaet} die strukturelle
OptimalitÃ¤t garantiert. Kleinberg und Tardos liefern damit aus didaktischer
Perspektive eine unabhÃ¤ngige BestÃ¤tigung des in dieser Arbeit formal bewiesenen
DualitÃ¤tsprinzips: \emph{die Form der minimalen Datenstruktur determiniert
das optimale Algorithmenparadigma}.

Der Subgraph Algorithmus \cite{epp2026subgraph} steht exemplarisch fÃ¼r die
produktive Verbindung dieser Paradigmen in der eigenen Forschung -- eine Verbindung,
die in zukÃ¼nftigen Arbeiten weiter ausgebaut und vertieft werden soll.
