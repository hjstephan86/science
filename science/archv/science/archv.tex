...
Positionsverschiebungen zu pflegen, wenn die zukÃ¼nftige Ankunftsreihenfolge
der einzuordnenden Dokumente \emph{a priori} unbekannt ist.
Anstatt die unbekannte Zukunft vorherzusagen, schlÃ¤gt diese Arbeit eine
\emph{rÃ¼ckwÃ¤rtsgewandte stochastische Wartestrategie} vor: Nach dem Eintreffen
eines Dokuments wird eine zufÃ¤llige Wartezeit abgewartet; trifft in dieser Zeit
ein weiteres Dokument ein, beginnt die Wartezeit neu. Erst wenn eine
vollstÃ¤ndige Warteperiode ohne neues Dokument verstreicht, wird die akkumulierte
...
geschlossene AusdrÃ¼cke fÃ¼r erwartete Wartezeiten und Verschiebungskosten her
und zeigen, welche Verteilung welches Systemziel optimiert. AbschlieÃŸend wird
...
bereits vorhandene Dokumente verschoben werden mÃ¼ssen â€“ im schlimmsten Fall
...
die zukÃ¼nftige Reihenfolge unbekannt. WÃ¼rde man alle kÃ¼nftig eintreffenden
Dokumente kennen (\emph{Offline-Szenario}), kÃ¶nnte man eine nahezu
...
Ankunftsrate zu schÃ¤tzen, und dann eine zufÃ¤llige Zeit zu warten, bevor man
einordnet. Trifft in dieser Wartezeit ein weiteres Dokument ein, wird die
Wartezeit zurÃ¼ckgesetzt â€“ das System sammelt Dokumente, bis eine
``ruhige'' Periode eintritt.

Diese Idee ist in der Informatik unter verschiedenen GewÃ¤ndern bekannt:
...
in der Rechnerarchitektur Ã¼bertragen lÃ¤sst.

\paragraph{BeitrÃ¤ge dieser Arbeit.}
\begin{itemize}[noitemsep]
  \item Formale Definition des Archiv-Problems als Online-Sortieraufgabe mit
        zufÃ¤lliger Wartestrategie (\cref{sec:model}).
  \item Umfassende mathematische Analyse der Wartezeit-Verteilungen
        ...
        geschlossenen AusdrÃ¼cken fÃ¼r Erwartungswert, Varianz und
        Verteilungsfunktion (\cref{sec:distributions}).
  \item Beweis der OptimalitÃ¤tseigenschaften und Zieldifferenzierung zwischen
...
$m$~Dokumenten $d_1 \prec d_2 \prec \cdots \prec d_m$ gemÃ¤ÃŸ einer totalen
Ordnung $\prec$ (z.\,B. alphabetisch nach SchlÃ¼ssel). Das \emph{EinfÃ¼gen}
eines neuen Dokuments~$d$ an Position~$k$ in eine Liste der LÃ¤nge~$m$
...
Die \emph{Verschiebungskosten} fÃ¼r das EinfÃ¼gen von Dokument $d$ in ein
Archiv der LÃ¤nge~$m$ sind
\[
  C(d, \mathcal{A}) = \bigl|\{i : d_i \prec d\}\bigr| \;=\; \mathrm{rank}_{\mathcal{A}}(d)
  \quad\in\{0, 1, \ldots, m\}.
\]
Die Gesamtkosten fÃ¼r das sequentielle EinfÃ¼gen einer Folge
$\sigma = (d^{(1)}, d^{(2)}, \ldots, d^{(N)})$ sind
\[
  C_{\mathrm{seq}}(\sigma) = \sum_{j=1}^{N} C\!\left(d^{(j)},
  \mathcal{A}_{j-1}\right),
\]
wobei $\mathcal{A}_{j-1}$ das Archiv vor dem $j$-ten EinfÃ¼gen bezeichnet.
\end{definition}

\subsection{Worst-Case-Analyse des naiven Online-EinfÃ¼gens}

\begin{proposition}[Worst-Case-Kosten]\label{prop:worstcase}
FÃ¼r eine Folge $\sigma$ der LÃ¤nge~$N$ gilt
\[
...
\begin{proof}
Dokument $d^{(j)}$ wird in ein Archiv der LÃ¤nge $j-1$ eingefÃ¼gt. Die maximalen
Kosten betragen $j-1$. Summation liefert
$\sum_{j=1}^{N}(j-1) = N(N-1)/2$. FÃ¼r absteigende Folge erreicht jedes
Dokument die erste Stelle und erzwingt alle $j-1$ Verschiebungen.
\end{proof}

\subsection{Batch-EinfÃ¼gen als Optimierungsstrategie}

Statt jedes Dokument sofort einzufÃ¼gen, sammeln wir Dokumente in einem
\emph{Puffer} $B$ und fÃ¼gen sie dann als sortierten Batch ein.

\begin{definition}[Batch-EinfÃ¼gen]\label{def:batch}
Sei $B = \{b_1, \ldots, b_k\}$ eine Menge von $k$ Dokumenten, die alle in das
Archiv $\mathcal{A}$ der LÃ¤nge~$m$ eingefÃ¼gt werden sollen. Die optimale
Strategie ist:
\begin{enumerate}[label=\arabic*.,noitemsep]
  \item Sortiere $B$ intern: $b_{\pi(1)} \prec \cdots \prec b_{\pi(k)}$.
  \item FÃ¼ge die sortierten Dokumente in einem einzigen Durchlauf ein
        (\emph{Merge-EinfÃ¼gen}).
\end{enumerate}
Die Kosten des Merge-EinfÃ¼gens sind $\mathcal{O}((m+k)\log(m+k))$ fÃ¼r die
interne Sortierung plus $\mathcal{O}(m+k)$ Verschiebungen fÃ¼r das physische
Merging.
\end{definition}

\begin{theorem}[Kosten-Reduktion durch Batching]\label{thm:batchreduction}
FÃ¼r $k$ Dokumente, die als Batch eingefÃ¼gt werden, gilt im Vergleich zum
sequentiellen EinfÃ¼gen:
\[
  \frac{C_{\mathrm{batch}}}{C_{\mathrm{seq}}} \;\leq\; \frac{2(m+k)}{k(k-1)+2m},
\]
was fÃ¼r groÃŸes~$k$ gegen $\mathcal{O}(1/k)$ geht. Batching skaliert also
linear mit der BatchgrÃ¶ÃŸe.
\end{theorem}

\begin{proof}
Sequentielles EinfÃ¼gen von $k$ Dokumenten in ein Archiv der LÃ¤nge $m$
kostet im Erwartungswert (bei zufÃ¤lliger Reihenfolge)
$\sum_{j=0}^{k-1}(m+j)/2 = k(2m+k-1)/4$. Das Batch-Merge benÃ¶tigt $m+k$
Vergleiche und $\mathcal{O}(m+k)$ Verschiebungen. Das VerhÃ¤ltnis ergibt die
obige Schranke.
\end{proof}
...
\begin{enumerate}[label=\arabic*.,noitemsep]
  ...
  \item \textbf{Ereignis â€“ Dokumentankunft:} Bei Ankunft von Dokument $d$
        lege $B := B \cup \{d\}$ und setze einen Zufallstimer
        $T \sim F$ neu.
  \item \textbf{Ereignis â€“ Timerablauf:} Falls kein Dokument wÃ¤hrend~$T$
        eingetroffen ist (\emph{ruhige Periode}), fÃ¼hre das
        Batch-EinfÃ¼gen aus: Sortiere $B$, fÃ¼ge in $\mathcal{A}$ ein,
        setze $B := \emptyset$.
  \item \textbf{Wiederholung:} Gehe zu Schritt~2.
\end{enumerate}
\end{definition}

Die zentrale GrÃ¶ÃŸe ist die \emph{Gesamtwartezeit}~$W$, also die Zeit vom
ersten Eintreffen eines Dokuments bis zum tatsÃ¤chlichen EinfÃ¼gen in das Archiv.
Diese Zeit ergibt sich als \emph{mehrfach zurÃ¼ckgesetzte Wartezeit} und besitzt
...
Rate $\lambda > 0$. Zwischen zwei aufeinanderfolgenden AnkÃ¼nften liegen
...
$T \sim F$ gezogen. Das nÃ¤chste Dokument kommt nach $X \sim \Exp(\lambda)$.

\begin{itemize}[noitemsep]
  \item \textbf{Batch schlieÃŸt}: Wenn $T < X$, also der Timer ablÃ¤uft, bevor
...
Die Anzahl der Resets bis zum SchlieÃŸen des Batches ist geometrisch verteilt:
...
(einschlieÃŸlich des abschlieÃŸenden erfolgreichen Timers) betrÃ¤gt
\[
...
\paragraph{GedÃ¤chtnislosigkeit.}
Die Exponentialverteilung ist die einzige stetige gedÃ¤chtnislose Verteilung:
$\Prob(T > t+s \mid T > s) = \Prob(T > t)$. Dies bedeutet, dass jeder Reset
statistisch gleichwertig ist â€“ die Vergangenheit spielt keine Rolle.

\begin{theorem}[Gesamtwartezeit bei Exponential-Timer]
\label{thm:expwait}
Sei $T \sim \Exp(\mu)$ und AnkÃ¼nfte $\sim \Pois(\lambda)$.
Die Gesamtwartezeit bis zum Batch-Abschluss ist
\[
  W_{\Exp} \;\sim\; \Exp(\mu + \lambda - \lambda)
\]
fÃ¼r den letzten erfolgreichen Timer, und die Gesamtwartezeit des gesamten
akkumulierten Prozesses (vom ersten Dokument bis zum EinfÃ¼gen) lautet:
\[
...
verhÃ¤lt sich gleichmÃ¤ÃŸig Ã¼ber alle Zeitpunkte. Sie ist optimal fÃ¼r
\emph{isotrope} Ankunftsprozesse ohne Tagesgang oder Burst-Struktur. Der
Parameter $\mu$ kann als Kehrwert der gewÃ¼nschten mittleren BatchgrÃ¶ÃŸe gewÃ¤hlt
...
\begin{theorem}[Erwartete Batch-GrÃ¶ÃŸe bei Geometrischem Timer]
\label{thm:geosize}
Sei $p_a$ die Ankunftswahrscheinlichkeit pro Takt.
Die erwartete Anzahl der Dokumente im Batch betrÃ¤gt
\[
...
Timer zurÃ¼ck. Der Prozess endet im ersten Takt ohne Dokument. Die Anzahl der
...
Die geometrische Verteilung ist gedÃ¤chtnislos (analog zur Exponentialverteilung
in diskreter Zeit) und eignet sich fÃ¼r \emph{taktgenaue digitale Systeme} wie
Prozessoren oder Netzwerkswitches. Sie minimiert die mittlere Latenz fÃ¼r
gleichmÃ¤ÃŸige Ankunftsprozesse bei diskretem Zeithorizont.

\subsection{Poisson-Verteilung als Batch-ZÃ¤hler}\label{subsec:poisson}

In einem Beobachtungsfenster fester LÃ¤nge $T_0$ folgt die Anzahl der
...
Die Strategie $\mathcal{W}(\Exp(\mu))$ ist Ã¤quivalent zu: Warte, bis ein
Poisson-Prozess in einem Fenster der zufÃ¤lligen LÃ¤nge $T \sim \Exp(\mu)$
kein Dokument liefert. Da Poisson-Prozesse stationÃ¤re, unabhÃ¤ngige Inkremente
haben, liefert diese Strategie besonders elegante AusdrÃ¼cke.
...
\paragraph{Erwartete Batch-GrÃ¶ÃŸe.}
...
Das Poisson-Modell ist \emph{optimal fÃ¼r kontinuierliche, stationÃ¤re
Ankunftsprozesse}. Es maximiert die BatchgrÃ¶ÃŸe bei vorgegebener maximaler
Wartezeit $1/\mu$ und ist die natÃ¼rliche Wahl fÃ¼r Netzwerkpaketverwaltung
...
Diese Variante ist optimal, wenn groÃŸe Batches gegenÃ¼ber kurzer Latenz
bevorzugt werden â€“ z.\,B. in Archivierungssystemen mit hohem Einzel-
Verschiebungsaufwand oder in Bulk-Insert-Operationen von Datenbanken.
Durch ErhÃ¶hung von $r$ kann der Anwender das \emph{Latenz-Durchsatz-Tradeoff}
fein steuern.
...
unvollstÃ¤ndige Gammafunktion ist. FÃ¼r $\lambda x_m \ll 1$:
\[
...
\label{thm:pareto}
FÃ¼r $\alpha > 1$ und kleine Ankunftsrate $\lambda x_m \to 0$:
\[
...
\]
FÃ¼r $1 < \alpha \leq 2$ ist die Varianz unendlich (\emph{infinite variance}).
\end{theorem}

\paragraph{Ziel der Pareto-Strategie.}
Heavy-Tail-Timer sind optimal fÃ¼r \emph{selbstÃ¤hnliche} Ankunftsprozesse mit
Burst-Struktur (fraktales Brownian Motion, Hurst-Exponent $H > 1/2$). Sie
ermÃ¶glichen kurze Reaktionszeiten in ruhigen Phasen und lange Akkumulationszeiten
in aktiven Phasen. Nachteil: Hohe Varianz in der Latenz macht sie fÃ¼r
Echtzeitsysteme ungeeignet.

\subsection{VergleichsÃ¼bersicht der Verteilungen}

\begin{table}[ht]
\centering
\caption{Vergleich der Wartezeitverteilungen fÃ¼r die Strategie $\mathcal{W}(F)$.
...
% ============================================================
\section{OptimalitÃ¤tsanalyse}\label{sec:optimality}
% ============================================================
...
EinfÃ¼gen) und $K = \E[|B|]$ die mittlere BatchgrÃ¶ÃŸe. FÃ¼r gegebenes $\lambda$
und Einzel-Verschiebungskosten $c$ ist das normierte Verschiebungsvolumen:
\[
  V = K^2 \cdot c - K \cdot c = c \cdot K(K-1).
\]
Die Einsparung gegenÃ¼ber sofortigem EinfÃ¼gen ist
$S = K \cdot c - c = c(K-1)$.
\end{definition}
...
\]
FÃ¼r $\mu \to \infty$: $S \to 0$, $L \to 0$ (sofortiges EinfÃ¼gen).
FÃ¼r $\mu \to 0$: $S \to \infty$, $L \to \infty$ (unendlich langer Batch).
\end{theorem}
...
gegenlÃ¤ufigen EinflÃ¼sse transparent macht:
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Verschiebungskosten}: $c_{\mathrm{shift}} \cdot \mu/\lambda$,
        proportional zum Kehrwert der mittleren BatchgrÃ¶ÃŸe
...
Das eindeutige Minimum von $J(\mu)$ Ã¼ber $\mu > 0$ ist
\[
...
FÃ¼r den Spezialfall $c_{\mathrm{wait}} = c_{\mathrm{shift}}$ gilt $\mu^* = \sqrt{\lambda}$.
\end{theorem}
...
\subsection{KompetitivitÃ¤tsanalyse}\label{subsec:competitive}

\begin{definition}[KompetitivitÃ¤tsverhÃ¤ltnis]\label{def:competitive}
Der \emph{competitive ratio} einer Online-Strategie $\mathcal{W}(F)$ gegenÃ¼ber
dem optimalen Offline-Algorithmus OPT ist
\[
  \rho(\mathcal{W}) = \sup_\sigma \frac{C_{\mathcal{W}}(\sigma)}{C_{\mathrm{OPT}}(\sigma)},
\]
wobei $\sigma$ alle mÃ¶glichen Dokumentfolgen durchlÃ¤uft.
\end{definition}

\begin{theorem}[Untere Schranke fÃ¼r randomisierte Strategien]\label{thm:lower}
FÃ¼r jede randomisierte Online-Strategie gilt:
\[
...
Diese Schranke ist der klassische Satz von Yao fÃ¼r das List-Update-Problem
und Ã¼bertrÃ¤gt sich direkt auf das Archiv-Problem.

\begin{theorem}[Obere Schranke fÃ¼r $\mathcal{W}(\Exp)$]\label{thm:upper}
...
die zufÃ¤llige Wartestrategie die Kosten gegenÃ¼ber dem Offline-Optimum.
\end{proof}
...
Das Archiv-Problem ist nicht nur eine abstrakte Kombinationsaufgabe â€“ es
ist ein prÃ¤zises Modell fÃ¼r zahlreiche VorgÃ¤nge in modernen Computersystemen.
...
Archiv $\mathcal{A}$   & Cache-Inhalt / TLB-EintrÃ¤ge        & L1â€“L3, TLB\\
Verschiebung           & Cache-Miss, Eviction, Writeback    & L1â€“L3\\
Batch-EinfÃ¼gen         & Write-combining, Prefetching       & L1/L2\\
...
in die nÃ¤chsttiefere Ebene geschrieben (Verschiebung). Die Eviction-Rate ist
analog zu den Verschiebungskosten im Archiv-Problem.

\paragraph{Write-Back-Strategie als Wartestrategie.}
Der \emph{Write-Back-Cache} verzÃ¶gert Schreiboperationen in den Hauptspeicher
...
fÃ¼r Seitentabellen-EintrÃ¤ge. Bei einem TLB-Miss muss ein Page-Walk durchgefÃ¼hrt
werden. Moderne Prozessoren (ARM, RISC-V, x86 mit Hardware-Table-Walker)
kÃ¶nnen TLB-Miss-Handler batchen.
...
$\mathcal{W}(\NBin(r, p))$ mit $r = $ ``Minimale BatchgrÃ¶ÃŸe vor Sweep''.
...
mittlere BatchgrÃ¶ÃŸe von $\E[|B|] = \lambda/\mu = 0.4$ â€“ also typischerweise
1 Request pro Batch, was dem beobachteten Verhalten entspricht. FÃ¼r
HDD-Systeme mit $\lambda = 500\,\mathrm{req/s}$ wÃ¤re
...
\begin{itemize}[noitemsep]
  ...
  \item Timerablauf: Interrupt wird ausgelÃ¶st, alle Pakete im Puffer werden
...
Die Formel lÃ¤sst sich umschreiben als
...
$5$--$50\,\mu\mathrm{s}$ fÃ¼r 1\,Gbps-NICs Ã¼berein.
...
\Cref{fig:results} zeigt die erwartete BatchgrÃ¶ÃŸe und Gesamtverschiebungsanzahl
fÃ¼r verschiedene Wartezeitstrategien in AbhÃ¤ngigkeit vom Timer-Parameter.
...
\caption{Normierte Verschiebungskosten (oben) und mittlere Latenz (unten)
         als Funktion des Timer-Parameters $\mu$ fÃ¼r drei Wartezeitstrategien
         bei $\lambda = 1$.}
\label{fig:results}
\end{figure}
...
\caption{Pareto-Front zwischen mittlerer Latenz und Verschiebungseinsparung
         fÃ¼r drei Wartezeitstrategien. Der schwarze Punkt markiert das
         theoretisch optimale $\mu^*$ fÃ¼r den Exp-Timer.}
\label{fig:paretofront}
\end{figure}

\subsection{Einfluss des Hurst-Exponenten (SelbstÃ¤hnliche Prozesse)}

FÃ¼r Ankunftsprozesse mit \emph{Long-Range Dependence} (LRD, Hurst-Exponent
$H > 0.5$) â€“ typisch fÃ¼r Web-Traffic und Datei-I/O â€“ gilt:

\begin{proposition}[Pareto-Timer bei LRD]\label{prop:lrd}
FÃ¼r einen fraktionalen Poisson-Prozess mit Hurst-Exponent $H \in (0.5, 1)$
ist der optimale Timer $T^*$ heavy-tailed mit Tailindex
\[
  \alpha^* = \frac{1}{H - 0.5} + 1.
\]
FÃ¼r $H = 0.75$ ergibt sich $\alpha^* = 5$, fÃ¼r $H = \frac{5}{6} \approx 0.833$
ergibt sich $\alpha^* = 4$.
\end{proposition}

Dies zeigt, dass \emph{Pareto-Timer mit $\alpha \in [3, 5]$} besonders robust fÃ¼r
selbstÃ¤hnliche Workloads mit typischen Hurst-Exponenten $H \in [0.75, 0.9]$ sind â€“
ein wichtiges Ergebnis fÃ¼r die Dimensionierung von Netzwerk- und
...
Anstatt den Timer-Parameter $\mu$ fest zu wÃ¤hlen, kann er \emph{adaptiv}
basierend auf der geschÃ¤tzten Ankunftsrate angepasst werden.
...
$k_{\mathrm{target}}$ die Ziel-BatchgrÃ¶ÃŸe ist. Diese Strategie konvergiert
im stationÃ¤ren Fall gegen den optimalen fixen Timer.

\subsection{Mehrklassen-PrioritÃ¤ten}

In realen Systemen haben verschiedene Dokumente (Prozesse, I/O-Requests)
verschiedene PrioritÃ¤ten. Eine natÃ¼rliche Erweiterung ist ein
\emph{Mehrklassen-Archiv} mit $R$ PrioritÃ¤tsklassen, wobei Klasse-$r$-Dokumente
einen eigenen Timer $T_r \sim F_r$ verwenden.

\begin{theorem}[Mehrklassen-Kostendekomposition]
Bei unabhÃ¤ngigen Ankunftsprozessen der Klassen $r = 1,\ldots,R$ mit Raten
$\lambda_r$ dekomponiert das Gesamtkostenfunktional additiv:
\[
  J_{\mathrm{total}} = \sum_{r=1}^R J_r(\mu_r),
\]
und jeder optimale $\mu_r^*$ kann unabhÃ¤ngig bestimmt werden.
\end{theorem}
...
ist $2$-kompetitiv. Das geordnete Archiv-Problem mit Batch-EinfÃ¼gen ergÃ¤nzt
...
formalisiert und die folgende rÃ¼ckwÃ¤rtsgewandte Wartestrategie analysiert: Da die
Zukunft unbekannt ist, schaut das System in die Vergangenheit (gemessene
Ankunftsrate $\hat\lambda$) und wartet eine zufÃ¤llige Zeit $T$. Bei Ankunft
eines neuen Dokuments wird der Timer zurÃ¼ckgesetzt. Erst nach einer vollstÃ¤ndigen
...
\begin{enumerate}[label=\arabic*.,noitemsep]
  ...
  \item Der optimale Timer-Parameter $\mu^*$ balanciert Latenzkosten und
        ...
        $\mu^* = \sqrt{c_{\mathrm{wait}}\,\lambda/c_{\mathrm{shift}}}$.
  \item FÃ¼r selbstÃ¤hnliche (LRD) Prozesse ist ein Pareto-Timer mit
        $\alpha = 1/(H-0.5) + 1$ optimal.
\end{enumerate}

\paragraph{Empfehlungen nach Systemtyp:}
\begin{itemize}[noitemsep]
  \item \textbf{GleichmÃ¤ÃŸige Last, geringe Latenzanforderung:}
...
\subsection{Offene Fragen und zukÃ¼nftige Arbeit}

\begin{itemize}[noitemsep]
  \item \textbf{Untere Schranke fÃ¼r randomisierte Strategien} verschÃ¤rfen:
        Kann $e/(e-1)$ fÃ¼r das spezifische Archiv-Problem Ã¼berwunden werden?
  \item \textbf{Maschinenlernbasierte Anpassung:} Reinforcement Learning fÃ¼r
        die adaptive Wahl von $F$ bei unbekannter Ankunftsverteilung.
  \item \textbf{Mehrdimensionale Archive} (B-BÃ¤ume, LSM-Trees): Das
        Batch-EinfÃ¼gen in LSM-Trees (\emph{Log-Structured Merge-Trees}) ist
  ...
  \item \textbf{Hardware-Implementierung:} FPGA- oder ASIC-Implementierung
        eines adaptiven Wartezeitgenerators fÃ¼r NIC Interrupt Coalescing.
\end{itemize}
...
Sei $Y = \min(T, X)$ mit $T \sim \Exp(\mu)$, $X \sim \Exp(\lambda)$ unabhÃ¤ngig.
Dann ist $Y \sim \Exp(\mu + \lambda)$.

Fall 1: $T < X$ (Batch schlieÃŸt, Wahrsch. $\mu/(\mu+\lambda)$). Gesamtwartezeit
nur dieser Periode: $W_0 = T$. FÃ¼r diesen Fall:
$\E[T \mid T < X] = 1/(\mu + \lambda)$ (Standard-Ergebnis fÃ¼r das Minimum).
...
\]
Da jedoch der erste Reset bereits eine Zeit $1/\lambda$ des nÃ¤chsten Dokuments
hinzufÃ¼gt:
\[
...
\]
FÃ¼r $\alpha > 1$ und $\lambda x_m \to 0$ dominiert der mittlere Term:
\[
