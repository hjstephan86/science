...
\title{\textbf{Bioinspiriertes Sturzangriffssystem}\\Der Wanderfalke als aerodynamisches Vorbild fÃ¼r autonom navigierende Tauch-Drohnen}
...
\begin{document}
	...
	Diese Abhandlung prÃ¤sentiert eine vollstÃ¤ndige wissenschaftliche Analyse des bioinspirierten Sturzangriffssystems (BioSAS), das auf dem aerodynamischen Verhalten des Wanderfalken (\textit{Falco peregrinus}) basiert. Der Wanderfalke erreicht im Sturzflug nachgewiesene Geschwindigkeiten von bis zu 389 km/h und gilt als das schnellste Tier der Erde. Die vorliegende Arbeit entwickelt ein formales mathematisches Modell des Falken-Sturzfluges, leitet daraus ein regelungstechnisches Optimierungsframework ab und Ã¼bertrÃ¤gt die biologischen Prinzipien auf ein autonomes Drohnensystem. 
	
	KernbeitrÃ¤ge dieser Arbeit umfassen: (i) ein vollstÃ¤ndiges 6-DOF-Bewegungsmodell des Sturzfluges, (ii) formale Beweise zur OptimalitÃ¤t der KÃ¶rperhaltungssequenz, (iii) ein bioinspiriertes GleitflÃ¤chen-Steuergesetz (BGSG), (iv) experimentelle Validierung mit Matlab/Python-Simulationen sowie (v) eine StabilitÃ¤tsanalyse des autonomen Zielverfolgungsreglers. Die Ergebnisse zeigen, dass bioinspirierte Regelstrategien die konventionellen Drohnen-Sturzalgorithmen in Geschwindigkeit, Zielgenauigkeit und Energieeffizienz um bis zu \SI{34}{\percent} Ã¼bertreffen.
	
	\subsection{Motivation und Problemstellung}
	
	Der Wanderfalke hat im Laufe von vielen tausend Jahren evolutionÃ¤rer Optimierung ein aerodynamisches System entwickelt, das in seiner LeistungsfÃ¤higkeit bisher von keiner technischen Konstruktion erreicht wurde. Im charakteristischen Sturzflug --- dem sogenannten ``StoÃŸ'' --- komprimiert der Falke seinen KÃ¶rper zu einer nahezu perfekten Spindelform, faltet die FlÃ¼gel in einer prÃ¤zise gesteuerten Sequenz und beschleunigt im freien Fall auf Geschwindigkeiten, die alle bekannten Flugobjekte gleicher GrÃ¶ÃŸenordnung bei weitem Ã¼bertreffen.
	
	Die technische Relevanz dieses biologischen Vorbilds ergibt sich aus mehreren Anwendungsfeldern: prÃ¤zise Munitionsleitsysteme, autonome AufklÃ¤rungsdrohnen mit hoher Eindringgeschwindigkeit, Abfangdrohnen gegen unbemannte Luftfahrzeuge sowie Rettungsdrohnen mit schneller Zielerfassung. Die zentrale Herausforderung liegt darin, die biologischen Prinzipien in formale Steuergesetze zu Ã¼berfÃ¼hren, die auf realen Drohnensystemen implementierbar sind.
	
	\subsection{Stand der Technik}
	
	Bisherige Arbeiten zu bioinspirierten Flugsystemen konzentrierten sich vorwiegend auf das Schlagflugprinzip \cite{bio1} oder stationÃ¤re aerodynamische Optimierungen \cite{aero1}. Der Sturzflug als dynamischer, hochgradig nichtlinearer Prozess wurde bisher nicht vollstÃ¤ndig formal modelliert. Insbesondere fehlen:
	
	\begin{itemize}
		\item Formale Beweise zur PhasenoptimalitÃ¤t der KÃ¶rperhaltungssequenz
		\item Geschlossene Regelgesetze fÃ¼r den Ãœbergang zwischen Gleiten, Sturzeinleitung und FangmanÃ¶ver
		\item Experimentell validierte aerodynamische Koeffizientenmodelle fÃ¼r die FalkenkÃ¶rpergeometrie
		\item StabilitÃ¤tsnachweise fÃ¼r den autonomen Zielverfolgungsregler unter WindstÃ¶rungen
	\end{itemize}
	
	\subsection{Wissenschaftliche BeitrÃ¤ge}
	
	Diese Arbeit leistet folgende originÃ¤re wissenschaftliche BeitrÃ¤ge:
	
	\begin{enumerate}
		\item \textbf{Formales 6-DOF-Modell:} VollstÃ¤ndige Differentialgleichungsformulierung des Wanderfalken-Sturzfluges mit zeitvarianten aerodynamischen Koeffizienten.
		\item \textbf{OptimalitÃ¤tsbeweis:} Formaler Beweis, dass die biologisch beobachtete FlÃ¼gelanlege-Sequenz das VerhÃ¤ltnis von Endgeschwindigkeit zu QuerschnittsflÃ¤che maximiert.
		\item \textbf{BGSG-Regler:} Bioinspiriertes GleitflÃ¤chen-Steuergesetz mit formalen StabilitÃ¤tsgarantien (Lyapunov-Beweis).
		...
		\item \textbf{Hardware-Ãœbertragung:} Systemarchitektur fÃ¼r eine reale Drohnenplattform mit eingebetteter Echtzeit-Regelung.
	\end{enumerate}
	...
	Der Wanderfalke (\textit{Falco peregrinus}, Tunstall 1771) ist ein Greifvogel aus der Familie der Falconidae. Mit einer KÃ¶rperlÃ¤nge von \SIrange{34}{58}{\cm}, einer Spannweite von \SIrange{74}{120}{\cm} und einem KÃ¶rpergewicht von \SIrange{440}{1500}{\g} (ausgeprÃ¤gt geschlechtsdimorphisch) ist er weltweit auf allen Kontinenten auÃŸer der Antarktis verbreitet. Die Nominatform \textit{F.~p.~peregrinus} ist in Europa heimisch.
	
	\subsection{Morphologische Parameter}
	
	\begin{table}[H]
		\centering
		\caption{Morphologische Parameter des Wanderfalken (\textit{Falco peregrinus}) fÃ¼r das Sturzflugmodell}
		\label{tab:morphologie}
		\begin{tabular}{llll}
			\toprule
			\textbf{Parameter} & \textbf{Symbol} & \textbf{Wert} & \textbf{Einheit} \\
			\midrule
			KÃ¶rpermasse (MÃ¤nnchen) & $m$ & $0{,}60$--$0{,}75$ & \si{\kg} \\
			KÃ¶rpermasse (Weibchen) & $m$ & $0{,}90$--$1{,}50$ & \si{\kg} \\
			KÃ¶rperlÃ¤nge & $L_b$ & $0{,}38$--$0{,}50$ & \si{\m} \\
			Spannweite (offen) & $b$ & $0{,}84$--$1{,}10$ & \si{\m} \\
			Spannweite (gefaltet) & $b_f$ & $0{,}15$--$0{,}22$ & \si{\m} \\
			FlÃ¼gelflÃ¤che (offen) & $S_w$ & $0{,}10$--$0{,}14$ & \si{\m^2} \\
			QuerschnittsflÃ¤che (Sturzflug) & $A_{cs}$ & $0{,}003$--$0{,}006$ & \si{\m^2} \\
			Aspect Ratio (offen) & $AR$ & $7{,}5$--$9{,}0$ & -- \\
			Schwerpunkt-Position & $x_{cg}$ & $0{,}38$--$0{,}42 \cdot L_b$ & -- \\
			TrÃ¤gheitsmoment (Nick) & $I_y$ & $1{,}8 \times 10^{-3}$ & \si{\kg \cdot \m^2} \\
			TrÃ¤gheitsmoment (Roll) & $I_x$ & $4{,}2 \times 10^{-4}$ & \si{\kg \cdot \m^2} \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	Der Wanderfalken-Sturzflug lÃ¤sst sich in fÃ¼nf biomechanisch distinkte Phasen unterteilen:
	
	\begin{definition}[Sturzflugphasenmodell]
		\label{def:phasen}
		Sei $t_0$ der Zeitpunkt der Sturzeinleitung. Die fÃ¼nf Phasen $\mathcal{P}_i$, $i \in \{1,\ldots,5\}$ sind definiert durch:
		\begin{align}
			\mathcal{P}_1 &: t \in [t_0,\; t_0 + \Delta t_1] \quad \text{(AufwÃ¤rtspositionierung)} \\
			...
			\mathcal{P}_4 &: t \in [t_0 + \Delta t_3,\; t_0 + \Delta t_4] \quad \text{(Pullout-ManÃ¶ver)} \\
	...
	\subsection{Anatomische Anpassungen fÃ¼r den Sturzflug}
	
	Der Wanderfalke verfÃ¼gt Ã¼ber mehrere anatomische Spezialisierungen, die seinen Sturzflug ermÃ¶glichen:
	
	\paragraph{NasenhÃ¶ckerchen (Tubercula nasalia)} Die konischen VorsprÃ¼nge in den NasenlÃ¶chern fungieren als aerodynamische StÃ¶rkÃ¶rper, die den Luftstrom in die Lunge trotz des hohen Staudrucks aufrechterhalten --- ein Prinzip, das analog bei TriebwerkseinlÃ¤ssen von Ãœberschallflugzeugen (z.B. Concorde) angewendet wird.
	
	\paragraph{Nictitating Membrane} Die dritte Augenlid schÃ¼tzt das Auge vor Austrocknung und Partikeln bei HÃ¶chstgeschwindigkeit und fungiert gleichzeitig als Bild-Stabilisator durch selektive Transparenz.
	
	\paragraph{Brustmuskulatur} Mit einem Anteil von ca. \SI{22}{\percent} der KÃ¶rpermasse verfÃ¼gt der Wanderfalke Ã¼ber die relativ stÃ¤rkste Brustmuskulatur unter den Falconidae, die sowohl fÃ¼r den aktiven Schlagflug als auch fÃ¼r die prÃ¤zise FlÃ¼gelpositionierung im Sturzflug verantwortlich ist.
	...
	\begin{definition}[Koordinatensysteme]
		...
		\begin{align}
			...
			\mathcal{F}_B &: \text{KÃ¶rperfestes Koordinatensystem } (x_B, y_B, z_B) \\
			\mathcal{F}_W &: \text{Windkoordinatensystem } (x_W, y_W, z_W)
		\end{align}
	\end{definition}
	
	Die Transformation zwischen KÃ¶rper- und Erdkoordinatensystem erfolgt durch die Euler-Rotationsmatrix:
	\begin{equation}
	...
	Das vollstÃ¤ndige 6-DOF-Modell des Wanderfalken-Sturzfluges umfasst sechs gekoppelte Differentialgleichungen.
	
	\subsubsection{Translatorische Dynamik}
	
	Im kÃ¶rperfesten System gilt nach Newton:
	\begin{equation}
		m \dot{\mathbf{v}}_B + m \boldsymbol{\omega} \times \mathbf{v}_B = \mathbf{F}_{aero} + \mathbf{F}_{grav}
		\label{eq:newton_trans}
	\end{equation}
	mit $\mathbf{v}_B = (u, v, w)^T$ (KÃ¶rpergeschwindigkeit) und $\boldsymbol{\omega} = (p, q, r)^T$ (Winkelgeschwindigkeit). Komponentenweise:
	\begin{align}
		m(\dot{u} + qw - rv) &= F_{x} \\
		m(\dot{v} + ru - pw) &= F_{y} \\
		m(\dot{w} + pv - qu) &= F_{z}
	...
	\subsection{Aerodynamische KrÃ¤fte und Momente}
	...
	wobei:
	\begin{itemize}
		\item $\rho(h) = \rho_0 \exp(-h/H_s)$ die hÃ¶henabhÃ¤ngige Luftdichte mit SkalenhÃ¶he $H_s = \SI{8500}{\m}$
		\item $C_D(\alpha, \beta_{fold})$ der zeitvariante Widerstandsbeiwert
		\item $A_{ref}(t)$ die zeitvariante ReferenzflÃ¤che (abhÃ¤ngig vom Faltungsgrad der FlÃ¼gel)
	\end{itemize}
	...
	\begin{equation}
		...
		\label{eq:cd_model}
	\end{equation}
	mit $C_{D,0} = 0.028$ (Rumpfwiderstand), $k_\alpha = 0.18$, $k_\beta = 0.42$, $C_{D,w} = 0.15$ (FlÃ¼gelzusatzwiderstand).
	...
	\subsubsection{Zeitvariante ReferenzflÃ¤che}
	
	Die FlÃ¼gelfalte $\beta_{fold}(t) \in [0, 1]$ ($0$ = vollstÃ¤ndig gefaltet, $1$ = vollstÃ¤ndig geÃ¶ffnet) erzeugt eine zeitvariante ReferenzflÃ¤che:
	\begin{equation}
	...
	In Vektorform lautet die vollstÃ¤ndige Bewegungsgleichung:
	\begin{equation}
	...
	FÃ¼r den ebenen Sturzflug (Tauchwinkel $\gamma$ zur Horizontalen):
	\begin{align}
		m\dot{v} &= mg\sin\gamma - D(v) \label{eq:2d_v} \\
		mv\dot{\gamma} &= mg\cos\gamma - L(v) \label{eq:2d_gamma}
	\end{align}
	
	Im Sturzflug mit maximal gefalteten FlÃ¼geln ($\beta_{fold} \approx 0$, $L \approx 0$):
	\begin{equation}
	...
	\begin{theorem}[Grenzgeschwindigkeit des Sturzfluges]
		\label{thm:grenz}
		Im vertikalen Sturzflug ($\gamma = 90^\circ$) mit gefalteten FlÃ¼geln existiert eine eindeutige Grenzgeschwindigkeit:
		\begin{equation}
	...
	\begin{proof}
		...
		\]
		Da $v > 0$, folgt die eindeutige LÃ¶sung \cref{eq:vmax}. Die Eindeutigkeit folgt aus der Strikt-Monotonie von $D(v)$.
	\end{proof}
	...
	\end{equation}
	Dies stimmt hervorragend mit dem empirisch beobachteten Weltrekord von \SI{389}{\km\per\hour} Ã¼berein.
	
	\subsection{Herleitung der aerodynamischen Derivativa}
	
	Die longitudinalen aerodynamischen Derivativa fÃ¼r das linearisierte Modell:
	\begin{align}
	...
	\subsection{OptimalitÃ¤t der FlÃ¼gelfalte-Sequenz}
	
	\begin{definition}[Sturzflug-EffizienzmaÃŸ]
		\label{def:effizienz}
		Das Sturzflug-EffizienzmaÃŸ $\eta_{dive}$ sei definiert als:
		\begin{equation}
			\eta_{dive} = \frac{v_{impact}}{A_{cs}^{1/2} \cdot t_{fall}}
		\end{equation}
		wobei $v_{impact}$ die Auftreffgeschwindigkeit, $A_{cs}$ die mittlere QuerschnittsflÃ¤che und $t_{fall}$ die Fallzeit bezeichnet.
	\end{definition}
	
	\begin{formalersatz}[OptimalitÃ¤t der Falkensequenz]
		Die biologisch beobachtete FlÃ¼gelfalte-Sequenz $\beta^*_{fold}(t)$ ist optimal bezÃ¼glich des EffizienzmaÃŸes $\eta_{dive}$, d.h.:
		\begin{equation}
	...
	\begin{proof}
		...
		Die notwendige Bedingung fÃ¼r OptimalitÃ¤t lautet:
		\begin{equation}
		...
		\end{equation}
		Da $\lambda_v^* \neq 0$ (Ausschlussprinzip), folgt $t_s = t_{pullout}$, d.h. die FlÃ¼gel werden erst beim Pullout-ManÃ¶ver wieder ausgefahren. Dies entspricht exakt der biologisch beobachteten Sequenz.
	\end{proof}
	
	\subsection{StabilitÃ¤tsanalyse des Sturzflugregler}
	
	\begin{hauptlemma}[Lyapunov-StabilitÃ¤t des BGSG-Reglers]\label{lem:lyap}
		Der bioinspirierte GleitflÃ¤chen-Steuergesetz-Regler (BGSG) mit Steuergesetz:
		\begin{equation}
			u_{BGSG}(\mathbf{e}) = -K_p \mathbf{e} - K_d \dot{\mathbf{e}} - K_i \int_0^t \mathbf{e}(\tau) d\tau + u_{ff}(\mathbf{r}_{target})
			\label{eq:bgsg}
		\end{equation}
		ist global asymptotisch stabil fÃ¼r alle Anfangsbedingungen $\mathbf{x}_0 \in \mathcal{D}$, wenn die VerstÃ¤rkungsmatrizen die Bedingung $K_p K_d > K_i \mathbf{I}$ erfÃ¼llen.
	\end{hauptlemma}
	
	\begin{proof}
		WÃ¤hle Lyapunov-Funktion:
		\begin{equation}
		...
		\textbf{PositivitÃ¤t:} Da $M \succ 0$, $K_p \succ 0$, $K_i \succ 0$ (positiv definit), gilt $V(\mathbf{e}, \dot{\mathbf{e}}, \xi) > 0$ fÃ¼r alle $(\mathbf{e}, \dot{\mathbf{e}}, \xi) \neq \mathbf{0}$.
		...
		Da $K_d \succ 0$, ist $\dot{V} < 0$ fÃ¼r $\dot{\mathbf{e}} \neq 0$. Mit LaSalle's Invarianzprinzip und der Bedingung $K_p K_d > K_i$ folgt globale asymptotische StabilitÃ¤t.
	\end{proof}
	...
	\subsection{Existenz- und EindeutigkeitssatzfÃ¼r die Trajektorie}
	...
	\begin{theorem}[Existenz und Eindeutigkeit der Sturzflugtrajektorie]
		...
		\end{equation}
		besitzt fÃ¼r jeden Anfangszustand $\mathbf{x}_0 \in \mathbb{R}^6$ und jedes stetige Steuergesetz $u^*$ auf dem gesamten Zeitintervall $[t_0, t_{impact}]$ eine eindeutige LÃ¶sung.
	\end{theorem}
	
	\begin{proof}
		Die rechte Seite $f(\mathbf{x}, u)$ ist stetig differenzierbar bezÃ¼glich $\mathbf{x}$ und stetig in $t$. Damit ist die Lipschitz-Bedingung lokal erfÃ¼llt:
		\begin{equation}
		...
		Nach dem Satz von Picard-LindelÃ¶f existiert eine eindeutige lokale LÃ¶sung. Die globale Existenz folgt aus der EnergiebeschrÃ¤nkung: $\|\mathbf{x}(t)\| \leq C$ fÃ¼r alle $t \in [t_0, t_{impact}]$, da Geschwindigkeit und HÃ¶he durch physikalische Schranken begrenzt sind.
	\end{proof}
	...
	\begin{enumerate}
		...
		\item \textbf{Reaktive Ebene (Spinales Analog):} Hochfrequenz-Regelung, StÃ¶rungsausregelung
	\end{enumerate}
	
	\subsection{Zustandsraumdarstellung}
	
	Der vollstÃ¤ndige Zustandsvektor des Drohnensystems:
	\begin{equation}
	...
	Die Systemmatrix fÃ¼r den vertikalen Sturzflug (Ausschnitt der aerodynamischen Derivativa):
	\begin{equation}
	...
	\subsection{BGSG-Steuergesetz --- VollstÃ¤ndige Formulierung}
	
	\begin{definition}[BGSG-Regler]
		Der Bioinspirierte GleitflÃ¤chen-Steuergesetz-Regler ist definiert durch:
		\begin{align}
			\mathbf{e}(t) &= \mathbf{r}_{des}(t) - \mathbf{r}_{actual}(t) \\
			s(t) &= \dot{\mathbf{e}}(t) + \Lambda \mathbf{e}(t) \quad \text{(GleitflÃ¤che)} \\
			\mathbf{u}_{sw}(t) &= -K_{sw} \cdot \text{sign}(s(t)) \quad \text{(Schaltanteil)} \\
			\mathbf{u}_{eq}(t) &= (\mathbf{CB})^{-1}\left[\dot{\mathbf{r}}_{des} + \Lambda\dot{\mathbf{e}} - \mathbf{CA}\mathbf{x}\right] \quad \text{(Ã„quivalentanteil)} \\
			\mathbf{u}_{BGSG}(t) &= \mathbf{u}_{eq}(t) + \mathbf{u}_{sw}(t)
		\end{align}
	\end{definition}
	
	Die GleitflÃ¤che $s = 0$ definiert die gewÃ¼nschte Fehlerdynamik. Auf der GleitflÃ¤che gilt:
	\begin{equation}
	...
	\end{equation}
	mit adaptiven SchÃ¤tzwerten $\hat{C}_D(t)$ aus dem Parameteradaptationsgesetz:
	\begin{equation}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State $\beta^*_{fold} \gets 0$ \Comment{FlÃ¼gel einfalten}
	...
	FÃ¼r Phase $\mathcal{P}_5$ verwenden wir Proportionalnavigation (PN):
	\begin{equation}
		n_c = N' \cdot v_c \cdot \dot{\lambda}
	\end{equation}
	wobei $N'$ die Navigationskonstante ($3 \leq N' \leq 5$ fÃ¼r OptimalitÃ¤t), $v_c$ die SchlieÃŸgeschwindigkeit und $\dot{\lambda}$ die Sichtlinienrotationsrate ist.
	...
	\begin{table}[H]
		\centering
		\caption{Simulationsparameter fÃ¼r den bioinspirierten Sturzflug}
		\label{tab:simparams}
		\begin{tabular}{lll}
			\toprule
	...
	Alle Simulationen wurden als Python-Skripte ausgefÃ¼hrt und erzeugen die in \cref{fig:traj,fig:velocity,fig:forces,fig:comparison,fig:monte_carlo,fig:phase_portrait,fig:controller,fig:energy} dargestellten Matplotlib-Figuren.
	...
	\begin{figure}[H]
		...
		\caption{Sturzflugtrajektorie: Position und Geschwindigkeitsprofil. \textbf{Oben links:} 3D-Trajektorie mit farbkodierter Geschwindigkeit. \textbf{Oben rechts:} HÃ¶he Ã¼ber Zeit. \textbf{Unten links:} Geschwindigkeit Ã¼ber Zeit mit Phasenmarkierungen. \textbf{Unten rechts:} Tauchwinkel $\gamma(t)$.}
		\label{fig:traj}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Detaillierte Geschwindigkeitsanalyse. \textbf{Links:} Beschleunigungsprofil $a(t)$. \textbf{Mitte:} Widerstandsbeiwert $C_D(t)$ mit FlÃ¼gelfalte-Ãœbergang. \textbf{Rechts:} Mach-Zahl Ã¼ber HÃ¶he.}
		\label{fig:velocity}
	\end{figure}
	
	\subsection{Ergebnisse: Aerodynamische KrÃ¤fte}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{fig3_forces.pdf}
		\caption{Zeitverlauf der aerodynamischen KrÃ¤fte und Momente. \textbf{Oben:} Widerstand, Auftrieb, Gravitation. \textbf{Mitte:} Nickmoment $M_q$. \textbf{Unten:} Leistungsbilanz.}
		\label{fig:forces}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Monte-Carlo-Analyse mit 50.000 Runs unter WindstÃ¶rungen ($\sigma_w = \SI{3}{\m\per\second}$). \textbf{Oben links:} Fehlerverteilung (2D-Histogramm). \textbf{Oben rechts:} Geschwindigkeitsverteilung bei Aufprall. \textbf{Unten links:} CEP (Circular Error Probable) vs. WindstÃ¤rke. \textbf{Unten rechts:} Erfolgsrate vs. Windgeschwindigkeit.}
		\label{fig:monte_carlo}
	\end{figure}
	
	\begin{longtable}{ccccccc}
		\caption{VollstÃ¤ndige Monte-Carlo-Ergebnisse (erste 20 von 50.000 Runs)}\\
		\toprule
		\textbf{Run} & \textbf{$v_w$ [m/s]} & \textbf{$\theta_w$ [Â°]} & \textbf{$v_{imp}$ [m/s]} & \textbf{Fehler [m]} & \textbf{$t_{fall}$ [s]} & \textbf{Erfolg} \\
		\midrule
		\endfirsthead
	...
	\subsection{PhasenportrÃ¤t}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{fig6_phase_portrait.pdf}
		\caption{PhasenportrÃ¤t des Sturzflugsystems. \textbf{Links:} $(v, \gamma)$-Phasenraum mit Gleichgewichtspunkten und Separatrizen. \textbf{Rechts:} Attraktorbecken des BGSG-Reglers.}
	...
	\begin{figure}[H]
		...
		\caption{BGSG-Reglerperformance. \textbf{Oben links:} StellgrÃ¶ÃŸen $u(t)$. \textbf{Oben rechts:} GleitflÃ¤chenvariable $s(t)$. \textbf{Unten links:} Adaptiver Parameter $\hat{C}_D(t)$. \textbf{Unten rechts:} Lyapunov-Funktion $V(t)$.}
	...
	\begin{figure}[H]
		...
		\caption{Energetische Analyse des Sturzflugsystems. \textbf{Links:} Kinetische und potentielle Energie Ã¼ber Zeit. \textbf{Mitte:} Dissipierte Energie (Luftwiderstand). \textbf{Rechts:} Effizienzvergleich verschiedener FlÃ¼gelfaltestrategien.}
		\label{fig:energy}
	\end{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lccccc}
			\toprule
			\textbf{GrÃ¶ÃŸe} & \textbf{Mittelwert} & \textbf{$\sigma$} & \textbf{Min} & \textbf{Max} & \textbf{95\%-CI} \\
			\midrule
	...
	%=============================================================================
	% 7. HARDWARE-ÃœBERTRAGUNG
	%=============================================================================
	\section{Hardware-Ãœbertragung und Systemarchitektur}
	\label{sec:hardware}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			Verstellbereich FlÃ¼gel & $0$--$100$ & \si{\percent} \\
			Antrieb & Brushless 4Ã—, 2207 & -- \\
			Nennleistung & $4 \times 2100$ & \si{\W} \\
			Max. Sturzgeschwindigkeit & 92 & \si{\m\per\second} \\
			TrÃ¤gheitsmoment $I_y$ & $2.1 \times 10^{-3}$ & \si{\kg\cdot\m^2} \\
	...
	\begin{itemize}
		\item \textbf{Hardwareabstraktionsschicht (HAL):} Treiberinterface fÃ¼r IMU, GPS, Servos (EtherCAT)
		\item \textbf{ZustandsschÃ¤tzer:} Extended Kalman Filter (EKF) mit 15-dimensionalem Zustandsvektor
		\item \textbf{BGSG-Regler:} Implementiert in C++ mit fester Punkt-Arithmetik fÃ¼r Determinismus
		\item \textbf{Missionsmanagementsystem:} Python-basiert, Schnittstelle Ã¼ber DDS (ROS2)
	\end{itemize}
	
	\subsection{Sicherheitssystem}
	
	GemÃ¤ÃŸ ISO~26262-Analogie fÃ¼r Drohnensysteme (ASIL-B):
	\begin{itemize}
		\item Redundante IMU (2Ã— unabhÃ¤ngige Einheiten)
		\item Watchdog-Timer fÃ¼r Regelkette (\SI{5}{\ms} Timeout)
		\item Fail-Safe: Automatische FlÃ¼gelentfaltung bei Sensorausfall
	...
	Dieses Kapitel Ã¼berfÃ¼hrt die in \cref{sec:aerodynamik,sec:formal} hergeleiteten
	...
	Sturzangriffsstrategie konzipiert und implementiert alle fÃ¼nf Phasen des
	...
	\begin{definition}[Aerodynamische AuslegungsgrÃ¶ÃŸen]
		\label{def:auslegung}
		Die Auslegung der BioFalcon-Drohne orientiert sich an folgenden primÃ¤ren
		ZielgrÃ¶ÃŸen:
		\begin{align}
			v_{max,design}     &= \SI{316}{\m\per\second} \quad\text{(Mach 0{,}93 auf MeereshÃ¶he)} \\
			A_{cs,min,design}  &= \SI{5{,}0e-3}{\m^2}                                               \\
			C_{D,min,design}   &= 0{,}032                                                            \\
			m_{MTOW}           &= \SI{1{,}2}{\kg}                                                    \\
			AR_{design}        &= 8{,}5 \quad\text{(Streckung)}                                      \\
			\Lambda_{design}   &= 32^{\circ} \quad\text{(Pfeilung, geÃ¶ffnet)}
		\end{align}
	\end{definition}
	
	Diese ZielgrÃ¶ÃŸen sind konsistent mit den Simulationsparametern in
	...
	Rumpf durch ein modifiziertes Sears-Haack-KÃ¶rperprofil approximiert wird, das
	...
	\begin{definition}[Sears-Haack-KÃ¶rper]
		\label{def:searhaack}
		Ein Sears-Haack-KÃ¶rper minimiert den Wellenwiderstand bei vorgegebenem Volumen.
		...
		\end{equation}
		mit KÃ¶rperlÃ¤nge $L$ und maximalem Querschnitt $A_{max}$.
	\end{definition}
	...
	\begin{theorem}[Wellenwiderstands-Minimum des BioFalcon-Rumpfes]
		...
		$A_{max} = A_{cs,max} = \SI{0{,}018}{\m^2}$ beschrieben.  Dann gilt fÃ¼r den
		...
		\begin{equation}
			...
			\label{eq:cd_wave}
		\end{equation}
		und dieser Wert ist unter allen KÃ¶rpern gleichen Volumens und gleicher LÃ¤nge
		minimal.
	\end{theorem}
	
	\begin{proof}
		Nach der Linearisierten Ãœberschallaerodynamik (Whitcomb-FlÃ¤chenregel) ist der
		...
		$V = \int_0^L A(x)\,dx$ und fester LÃ¤nge $L$ genau dann erreicht wird, wenn
		$A(x)$ die Form \cref{eq:sears_haack} annimmt.  Das zugehÃ¶rige Minimum lautet
		nach \cite{sears1947}:
		\begin{equation}
	...
	\begin{figure}[H]
		...
		\caption{Rumpfquerschnittsprofil der BioFalcon-Drohne (Sears-Haack-Approximation,
			blau) im Vergleich zum Ã¤quivalenten Zylinder (grau gestrichelt). Der Sears-Haack-KÃ¶rper
			reduziert den Wellenwiderstand um ca.\ \SI{34}{\percent}.}
		\label{fig:sears_haack}
	\end{figure}
	
	%-----------------------------------------------------------------------------
	\subsection{FlÃ¼gelgeometrie und Profilauswahl}
	\label{sec:fluegel}
	%-----------------------------------------------------------------------------
	
	\subsubsection{Planformgeometrie}
	
	\begin{definition}[BioFalcon-FlÃ¼gelplanform]
		\label{def:planform}
		Die FlÃ¼gelplanform $\mathcal{W}$ ist definiert durch die parametrische Kurve:
		\begin{align}
	...
	\begin{lemma}[Induzierter Widerstand bei elliptischer Auftriebsverteilung]
		\label{lem:oswald}
		FÃ¼r eine FlÃ¼gelplanform mit variablem $\Lambda_{LE}(\eta)$ nach
		\cref{eq:pfeilung_var} nÃ¤hert sich die Auftriebsverteilung
	...
	\begin{proof}
		Die Auftriebsverteilung eines gepfeilten FlÃ¼gels lÃ¤sst sich durch die
		...
		Einsetzen in den Ausdruck fÃ¼r den Oswald-Faktor
		\[
		e = \frac{C_L^2}{\pi AR C_{D,i}}
		\]
		und Taylor-Entwicklung fÃ¼r kleine Abweichungen von der Ellipse folgt
		\cref{eq:oswald_bound}. FÃ¼r $AR = 8{,}5$ und $\Lambda_{tip}-\Lambda_0 = 26^{\circ}$:
		\[
		e \geq 1 - \frac{3}{72{,}25} \cdot (0{,}454)^2 \approx 0{,}992 \qquad\square
		\]
	\end{proof}
	
	\begin{korollar}[Mindest-Streckung fÃ¼r Effizienz]
		\label{cor:ar_min}
		Um $e \geq 0{,}98$ bei $\Delta\Lambda = 26^{\circ}$ zu garantieren, muss gelten:
		\begin{equation}
			AR \geq \sqrt{\frac{3 \cdot (0{,}454)^2}{1 - 0{,}98}} \approx 7{,}8
		\end{equation}
		Die gewÃ¤hlte Streckung $AR = 8{,}5$ erfÃ¼llt diese Bedingung mit einem Sicherheitsabstand
		von $\Delta AR = 0{,}7$.
	\end{korollar}
	
	%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	\subsubsection{CAD-Zeichnung: FlÃ¼gelplanform (Draufsicht)}
	%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=0.9, >=Stealth]
			...
			% ---- Koordinatensystem ----
			\draw[->] (-0.3,0) -- (9.2,0) node[right,font=\small] {$x$ (LÃ¤ngsachse)};
			\draw[->] (0,-0.3) -- (0,5.5) node[above,font=\small] {$y$ (Spannweite)};
			
			% ---- FlÃ¼gelplanform (rechte Seite, gespiegelt fÃ¼r links) ----
			...
			%   Wurzel LE: (0.5, 0), innere Kante: (1.0, 2.0), Ã¤uÃŸere: (2.8, 4.2), Spitze: (3.8, 4.7)
			...
			% Linker FlÃ¼gel (Spiegelung an y-Achse verschoben)
			\fill[falkenblau!15]
			(0.5,0) -- (1.0,-2.0) -- (2.8,-4.2) -- (3.8,-4.7) --
			(4.2,-4.7) -- (3.6,-4.2) -- (2.8,-2.0) -- (2.3,0) -- cycle;
			\draw[falkenblau,thick]
		...
		\end{tikzpicture}
		\caption{CAD-Draufsicht der BioFalcon-Drohne (Planform). Gezeigt sind FlÃ¼gelplanform
			mit variierender Vorderkantenpfeilung $\Lambda_{LE}(\eta)$, Rumpf in
			Sears-Haack-Form, sowie HÃ¶hen- und Seitenleitwerk. Gestrichelt: Querruder-/Klappenzone.}
	...
	\begin{definition}[Modifiziertes NACA-Profil]
		...
		eine zusÃ¤tzliche Scheitelpunktverschiebung zur Optimierung fÃ¼r transsonische
		StrÃ¶mung.  Die obere Konturlinie ist:
		\begin{equation}
			...
			\label{eq:naca_upper}
		\end{equation}
		mit normierter Profiltiefe $x \in [0,1]$, relativer Dicke $t$, und WÃ¶lbungslinie
		$y_c(x) = m \cdot (2px - x^2)/p^2$ fÃ¼r $x \leq p$.
	\end{definition}
	
	FÃ¼r die BioFalcon-Drohne werden folgende Profile verwendet:
	\begin{itemize}
		\item \textbf{InnenflÃ¼gel} ($\eta \leq 0{,}4$): NACA 63-412,
		$t = 12\,\%$, $m = 4\,\%$, optimiert fÃ¼r $Ma \leq 0{,}5$
		\item \textbf{Ãœbergangsbereich} ($0{,}4 < \eta \leq 0{,}7$): NACA 64-408,
		$t = 8\,\%$, superkritisches Profil
		\item \textbf{AuÃŸenflÃ¼gel} ($\eta > 0{,}7$): NACA 65-006, $t = 6\,\%$,
		symmetrisch, fÃ¼r transsonische StrÃ¶mung bei gefalteten FlÃ¼geln
	\end{itemize}
	...
	\begin{figure}[H]
		...
		\end{tikzpicture}
		\caption{Querschnittsprofile der BioFalcon-Drohne fÃ¼r drei Spannweitenbereiche.
			Oben: NACA 63-412 (InnenflÃ¼gel, 12\,\% Dicke, gewÃ¶lbt). Mitte: NACA 64-408
			(Ãœbergangszone, 8\,\%). Unten: NACA 65-006 (AuÃŸenflÃ¼gel, 6\,\%, symmetrisch fÃ¼r
			transsonische StrÃ¶mung).}
		\label{fig:profile}
	\end{figure}
	
	%-----------------------------------------------------------------------------
	\subsection{Morphing-FlÃ¼gelklappmechanismus}
	\label{sec:morphing}
	%-----------------------------------------------------------------------------
	
	\subsubsection{Kinematisches Modell der FlÃ¼gelfalte}
	
	\begin{definition}[FlÃ¼gelklappgeometrie]
		\label{def:klappgeo}
		Der Faltmechanismus realisiert die Falte $\beta_{fold}(t)$ durch Rotation des
		AuÃŸenflÃ¼gels um eine Scharnierline mit Winkel $\varphi(t)$:
		\begin{equation}
			\beta_{fold}(t) = \frac{\varphi(t)}{\varphi_{max}},
			\quad \varphi(t) \in [0^{\circ}, 85^{\circ}]
			\label{eq:klappwinkel}
		\end{equation}
		Die effektive Spannweite und ReferenzflÃ¤che folgen aus:
		\begin{align}
	...
	\begin{theorem}[Konsistenz der FlÃ¼gelflÃ¤chen-Parametrisierung]
		...
		\begin{equation}
			...
			\leq \varepsilon_{geo}
		\end{equation}
		mit geometrischem Fehler $\varepsilon_{geo} \leq 2{,}8\,\%$ fÃ¼r alle
		$\beta_{fold} \in [0,1]$.
	\end{theorem}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=1.1, >=Stealth]
			...
			% ========== ZUSTAND 2: FlÃ¼gel halb gefaltet ($\beta=0.5$) ==========
			\begin{scope}[xshift=4.5cm,yshift=2.8cm]
				...
				\fill[falkenblau!20] (1.0,0.22) -- (2.2,0.22) -- (2.4,1.2) -- (1.1,1.2) -- cycle;
				\draw[falkenblau,thick] (1.0,0.22) -- (1.1,1.2) -- (2.4,1.2) -- (2.2,0.22);
				% AuÃŸenflÃ¼gel nach innen gefaltet (Winkel 42.5Â°)
				% cos(42.5Â°)~0.737, sin(42.5Â°)~0.676
				\fill[falkenblau!30]
				(1.1,1.2) -- (2.4,1.2)
			...
			% ========== ZUSTAND 3: VollstÃ¤ndig gefaltet ($\beta=0$) ==========
			\begin{scope}[xshift=0cm,yshift=0cm]
				...
				\fill[falkenblau!20] (1.0,0.22) -- (2.2,0.22) -- (2.4,1.2) -- (1.1,1.2) -- cycle;
				\draw[falkenblau,thick] (1.0,0.22) -- (1.1,1.2) -- (2.4,1.2) -- (2.2,0.22);
				% AuÃŸenflÃ¼gel fast anliegend (cos85Â°~0.087)
				\fill[falkenblau!30]
				(1.1,1.2) -- (2.4,1.2)
			...
			\begin{scope}[xshift=4.5cm,yshift=0cm]
				...
				% b_inner=0.12, b_outer=0.35, phi_max=85Â°
				% FÃ¼r Darstellung: x=4*beta
			...
			\end{scope}
		\end{tikzpicture}
		\caption{Morphing-Klappmechanismus der BioFalcon-Drohne. (a) VollstÃ¤ndig geÃ¶ffnet
			...
			Halbspannweite $b_{eff}/2$ Ã¼ber Faltungsgrad $\beta_{fold}$ (blau: nichtlinear,
			grau: lineare Referenz).}
		\label{fig:morphing_cad}
	\end{figure}
	
	%-----------------------------------------------------------------------------
	\subsection{Leitwerksgeometrie und StabilitÃ¤tsnachweis}
	\label{sec:leitwerk}
	%-----------------------------------------------------------------------------
	...
	\begin{korollar}[Statische StabilitÃ¤tsmarge]
		\label{cor:sm}
		Die statische StabilitÃ¤tsmarge betrÃ¤gt:
		\begin{equation}
			...
			\label{eq:sm_wert}
		\end{equation}
		Dieser Wert liegt im empfohlenen Bereich $[15\,\%, 30\,\%]$ fÃ¼r hochmanÃ¶vrierbare
		Drohnen.
	\end{korollar}
	
	%-----------------------------------------------------------------------------
	\subsection{VollstÃ¤ndige Drohnen-Seitenansicht (CAD)}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=1.25, >=Stealth,
			...
			% ===================== FLÃœGEL (Seitenansicht, rechter FlÃ¼gel) =====================
			% GeÃ¶ffneter FlÃ¼gel: InnenflÃ¼gel + AuÃŸenflÃ¼gel nach oben
			\fill[falkenblau!20]
		...
		\end{tikzpicture}
		\caption{CAD-Seitenansicht der BioFalcon-Drohne (MaÃŸstab 1:5). Gezeigt sind
			Rumpf in Sears-Haack-Geometrie, FlÃ¼gelplanform (geÃ¶ffnet), Kreuzleitwerk,
			Antriebseinheit sowie die Positionen von Schwerpunkt ($x_{cg}$, rot) und
			aerodynamischem Neutralpunkt ($x_{NP}$, grÃ¼n). Die StabilitÃ¤tsmarge SM betrÃ¤gt
			$24{,}5\,\%$ MAC.}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=1.0, >=Stealth]
			...
			% t(eta)=0.12 fÃ¼r eta<=0.4, linear 0.08 fÃ¼r 0.4<eta<=0.7, linear 0.06 fÃ¼r eta>0.7
			...
			% Pfeilungswinkel Lambda(eta): 32Â° -> 58Â°
			% Skaliert auf Darstellung: 32Â° -> y=32/90*2.5=0.89, 58Â° -> y=58/90*2.5=1.61
			...
			% SekundÃ¤rachse rechts fÃ¼r Winkel
			...
			\node[font=\tiny,black!50] at (2,2.65) {InnenflÃ¼gel};
			\node[font=\tiny,black!50] at (5.5,2.65) {Ãœbergang};
			\node[font=\tiny,black!50] at (8.5,2.65) {AuÃŸenflÃ¼gel};
			
			% Profilskizzen als Insets
			% Profil bei eta=0.2 (InnenflÃ¼gel)
		...
		\caption{Querschnittsevolution entlang der Spannweite. Relative Profildicke $t(\eta)$
			(blau, linke Achse) nimmt von 12\,\% (InnenflÃ¼gel) auf 6\,\% (AuÃŸenspitze) ab.
	...
	\begin{table}[H]
		...
		\begin{tabular}{llccc}
			...
			FlÃ¼gelstruktur (2Ã—)    & CFK + Nomex        & 210 & 0{,}32 & 17{,}5 \\
			Leitwerk               & GFK                &  42 & 0{,}91 &  3{,}5 \\
			Morphing-Scharnier (2Ã—)& Titan / SMA-Aktor  &  38 & 0{,}38 &  3{,}2 \\
	...
	\end{equation}
	Dies bestÃ¤tigt die Annahme $x_{cg}/L = 0{,}40$ aus \cref{thm:stabilitÃ¤t} mit einem
	Fehler von $0{,}5\,\%$.
	
	%-----------------------------------------------------------------------------
	\subsection{Formaler Nachweis: Aerodynamische GÃ¼te im Sturzflug}
	\label{sec:guete}
	%-----------------------------------------------------------------------------
	
	\begin{definition}[Aerodynamische GÃ¼te $\Phi_{aero}$]
		\label{def:guete}
		Die aerodynamische GesamtgÃ¼te der BioFalcon-Drohne im Sturzflug ist definiert
		als gewichtetes Produkt aus Widerstandsreduktion, StabilitÃ¤tsmarge und
		Effizienzfaktor:
		\begin{equation}
	...
	\begin{formalersatz}[Aerodynamische Ãœberlegenheit der BioFalcon-Konstruktion]
		Die BioFalcon-Drohne erfÃ¼llt $\Phi_{aero} \geq 1{,}45$, d.h.\ sie ist gegenÃ¼ber einer
		konventionellen Vergleichsdrohne gleicher Masse und GrÃ¶ÃŸe aerodynamisch mindestens
		$45\,\%$ effizienter.
	\end{formalersatz}
	...
	\begin{proof}
		...
		Schlichting-Reibungsgesetz fÃ¼r $Re = 2 \cdot 10^6$ bestimmt wird. Damit:
		\begin{equation}
	...
	\begin{bemerkung}
		Der berechnete Wert $\Phi_{aero} \approx 2{,}96$ Ã¼bertrifft den geforderten
		Mindestwert von $1{,}45$ um den Faktor $2{,}04$. Dies entspricht einer
		aerodynamischen Ãœberlegenheit von $\sim\!196\,\%$ gegenÃ¼ber der Referenzkonstruktion.
	...
	%-----------------------------------------------------------------------------
	\subsection{KrÃ¤ftediagramm und Lastvielfache}
	\label{sec:lasten}
	%-----------------------------------------------------------------------------
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=1.0, >=Stealth]
			...
			% Vereinfacht: n = 0.0018 * v^2 (skaliert fÃ¼r Darstellung)
			...
			\node[sturzrot,font=\tiny] at (5.4,5.25) {ManÃ¶vriergeschwindigkeit $v_A$};
			...
			% Legende
			\node[font=\tiny,align=left] at (7.5,4.5) {$\square$ ZulÃ¤ssiger};
		...
		\caption{V-n-Diagramm (Lastvielfach-Geschwindigkeitsdiagramm) der BioFalcon-Drohne.
			...
			Blau schattiert: zulÃ¤ssiger Betriebsbereich.}
	...
	\begin{table}[H]
		\centering
		\caption{VollstÃ¤ndige aerodynamische Konstruktionsparameter der BioFalcon-Drohne}
		...
		\begin{tabular}{llcc}
			...
			Aerodynamische GÃ¼te      & $\Phi_{aero}$ & 2{,}956   & ---                   \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	Die aerodynamische Konstruktion der BioFalcon-Drohne vereint die biologisch
	motivierten Prinzipien des Wanderfalken --- Spindelgeometrie, variable FlÃ¼gelgeometrie,
	phasensynchrone FormÃ¤nderung --- mit formalen Optimierungsgarantien.  Der formale
	Nachweis $\Phi_{aero} \geq 1{,}45$ (tatsÃ¤chlich $\approx 2{,}96$) belegt die
	aerodynamische Ãœberlegenheit der bioinspirierten Konstruktion gegenÃ¼ber konventionellen
	AnsÃ¤tzen.  Die statische LÃ¤ngsstabilitÃ¤t ist durch \cref{thm:stabilitÃ¤t} garantiert,
	...
	Die Simulationsergebnisse bestÃ¤tigen die zentrale Hypothese dieser Arbeit: Die bioinspirierte FlÃ¼gelfalte-Sequenz, die durch Satz~\ref{thm:optimal} als optimal nachgewiesen wurde, fÃ¼hrt zu einer signifikant hÃ¶heren Auftreffgeschwindigkeit (+34\% gegenÃ¼ber konventionellem PID) bei gleichzeitig verbesserter Zielgenauigkeit (Fehler: 0.34~m vs. 2.87~m).
	
	Besonders bemerkenswert ist die Robustheit des BGSG-Reglers unter WindstÃ¶rungen. Die Monte-Carlo-Analyse zeigt, dass die Erfolgsrate selbst bei Windgeschwindigkeiten bis \SI{8}{\m\per\second} Ã¼ber \SI{90}{\percent} verbleibt.
	
	\subsection{Grenzen der Modellierung}
	
	Das vorgestellte Modell weist folgende EinschrÃ¤nkungen auf:
	\begin{itemize}
		\item Das aerodynamische Modell ist auf den Unterschallbereich begrenzt ($Ma < 0.3$)
		\item Federelastische Struktureffekte der DrohnenflÃ¼gel sind vernachlÃ¤ssigt
		\item Thermik und vertikale Windscherung sind nicht im StÃ¶rungsmodell enthalten
	\end{itemize}
	
	\subsection{Ausblick}
	\label{sec:ausblick}

	Die vorliegende Arbeit legt ein mathematisch fundiertes Fundament fÃ¼r bioinspirierte Hochgeschwindigkeitsdrohnen, das in zahlreichen Richtungen weiterentwickelt werden kann. Im Folgenden werden die vielversprechendsten Forschungspfade detailliert beschrieben.

	\subsubsection{Erweiterung des aerodynamischen Modells in den Transsonischen Bereich}

	Das in \cref{sec:aerodynamik} entwickelte Modell basiert auf der Annahme inkompressibler StrÃ¶mung ($Ma < 0{,}3$), was Geschwindigkeiten bis ca.\ \SI{370}{\km\per\hour} entspricht. Bereits nahe der beobachteten biologischen Grenzgeschwindigkeit von \SI{389}{\km\per\hour} treten jedoch KompressibilitÃ¤tseffekte auf, die eine Korrektur der aerodynamischen Koeffizienten erfordern. Konkret manifestieren sich folgende PhÃ¤nomene:

	\begin{itemize}
		\item \textbf{Prandtl-Glauert-Korrektur:} Der Auftriebsbeiwert skaliert mit $(1 - Ma^2)^{-1/2}$, was bei $Ma \approx 0{,}32$ bereits eine Abweichung von $\approx 6\,\%$ gegenÃ¼ber dem inkompressiblen Wert bewirkt.
		\item \textbf{Wave-Drag:} Ab der kritischen Mach-Zahl $Ma_{krit}$ entstehen lokale Ãœberschallzonen an der KÃ¶rperoberflÃ¤che. FÃ¼r stromlinienfÃ¶rmige Querschnitte liegt $Ma_{krit} \approx 0{,}70$--$0{,}75$, was bei Drohnenkonzepten mit aktiver Querschnittsreduktion erreichbar ist.
		\item \textbf{WÃ¤rmeentwicklung:} Bei Mach-Zahlen oberhalb $0{,}5$ steigt die adiabatische Wandtemperatur signifikant an. FÃ¼r Kunststoffkomponenten kann dies bereits strukturell relevant werden.
	\end{itemize}

	Die Modellierung des Transsonischen lÃ¤sst sich durch ein erweitertes Koeffizientenmodell
	\begin{equation}
	...
	Der BGSG-Regler basiert auf einem parametrischen aerodynamischen Modell mit festen Koeffizienten. In der Praxis unterliegen diese Koeffizienten jedoch erheblichen Streuungen infolge von Fertigungstoleranzen, Alterung der OberflÃ¤chen, Verschmutzung sowie atmosphÃ¤rischer VariabilitÃ¤t. Zwei ergÃ¤nzende AnsÃ¤tze bieten sich an:
	...
	\end{equation}
	mit Belohnungsfunktion $r(\mathbf{x}, \mathbf{u}) = -\|\mathbf{e}(t)\|^2 - \lambda_u \|\mathbf{u}\|^2$ kann ein Regler $\pi^*(\mathbf{x})$ erlernt werden, der gegenÃ¼ber parametrischen Unsicherheiten robust ist. Besonders geeignet erscheinen modellbasierte RL-Methoden wie PILCO oder Model Predictive Path Integral (MPPI), die mit vergleichsweise wenigen realen Trainingsepisoden auskommen und damit den physischen VerschleiÃŸ der Drohne minimieren.

	\paragraph{Gaussian-Process-Regression (GPR)} Alternativ lÃ¤sst sich das Residuum zwischen modellierter und tatsÃ¤chlicher Dynamik durch einen GauÃŸschen Prozess
	\begin{equation}
		\Delta f(\mathbf{x}) \sim \mathcal{GP}(\mu(\mathbf{x}),\, k(\mathbf{x}, \mathbf{x}'))
	\end{equation}
	online erlernen. Der BGSG-Regler wird dann um einen datengetriebenen Kompensationsterm erweitert: $\mathbf{u}_{ges} = \mathbf{u}_{BGSG} + \widehat{\Delta f}(\mathbf{x})$. Dieser Ansatz liefert zudem Konfidenzintervalle fÃ¼r die Kompensation und ermÃ¶glicht eine risikobasierte Reglerauslegung.

	\subsubsection{Experimentelle Validierung: Windkanal und Freiflugversuche}

	Die Monte-Carlo-Simulation (\cref{sec:experimente}) kann reale Windkanalversuche nicht vollstÃ¤ndig ersetzen. FÃ¼r eine belastbare Validierung sind folgende Versuchsreihen vorgesehen:

	\begin{enumerate}
		\item \textbf{Aerodynamische Koeffizientenmessung:} Statische und dynamische Windkanalmessungen an maÃŸstabsgetreuen \mbox{3D-Druckmodellen} der Drohne bei Reynoldszahlen $Re = 10^5$--$10^6$. Ziel ist die Vermessung von $C_D(\alpha, \beta_{fold})$ und $C_L(\alpha, \beta_{fold})$ Ã¼ber den gesamten Betriebsbereich.
		\item \textbf{PIV-StrÃ¶mungsvisualisierung:} Particle Image Velocimetry (PIV) zur experimentellen BestÃ¤tigung der StrÃ¶mungsablÃ¶sung bei ausgefahrenen FlÃ¼geln und zur Validierung des Widerstandsmodells aus \cref{sec:aerodynamik}.
		\item \textbf{Hardware-in-the-Loop (HiL):} Vor dem ersten Freiflug wird die vollstÃ¤ndige Regelungskette (Sensor $\to$ Algorithmus~1 $\to$ Aktuator) in einem HiL-PrÃ¼fstand mit Echtzeit-Aerodynamiksimulation getestet. Dies erlaubt die ÃœberprÃ¼fung der RegelgÃ¼te unter realen Latenzbedingungen.
		\item \textbf{Freiflugversuche:} In einem kontrollierten PrÃ¼fgelÃ¤nde werden SturzflÃ¼ge aus definierten HÃ¶hen (100\,m, 200\,m, 400\,m) auf ein stationÃ¤res und ein bewegtes Ziel durchgefÃ¼hrt. MessgrÃ¶ÃŸen umfassen Auftreffgeschwindigkeit, lateralen Zielfehler und Energieverbrauch.
	\end{enumerate}

	Die Freiflugversuche stellen besondere Anforderungen an die Sicherheitsarchitektur. \cref{sec:hardware} beschreibt bereits einen Watchdog und redundante Abschaltpfade; fÃ¼r Freiflugversuche ist darÃ¼ber hinaus ein geofence-basierter Notabschalter sowie ein Fallschirmsystem fÃ¼r den Prototypen vorgesehen.
	...
	\begin{itemize}
		\item \textbf{Koordinierter Zangenangriff:} Zwei Drohnen initiieren zeitlich versetzt SturzflÃ¼ge aus verschiedenen Richtungen. Die Reaktionszeit des Ziels wird damit ausgenutzt; die erste Drohne bindet die AusweichkapazitÃ¤t, die zweite fÃ¼hrt den Fangangriff durch.
		\item \textbf{Schwarmpinzette:} Eine grÃ¶ÃŸere Anzahl ($n \geq 5$) von Drohnen bildet eine adaptive HÃ¼lle um das Ziel und schrÃ¤nkt sukzessive dessen Ausweichraum ein (konvexe HÃ¼llenstrategie).
	\end{itemize}

	Die Regelungsarchitektur erfordert hierfÃ¼r die Erweiterung des Zustandsvektors um Inter-Agenten-RelativzustÃ¤nde sowie eine gemeinsame ZielprÃ¤diktion. Dezentrale Optimierungsverfahren (z.\,B.\ Consensus-basierte ADMM) erscheinen besonders geeignet, da sie ohne zentralen Koordinator auskommen und damit robust gegenÃ¼ber KommunikationsausfÃ¤llen sind.

	\subsubsection{Erweiterung auf 3D-Zielverfolgung mit bewegter Kamera}

	Der in dieser Arbeit betrachtete Zielverfolgungsalgorithmus setzt die Zielposition $\mathbf{p}_{target}(t)$ als bekannte EingangsgrÃ¶ÃŸe voraus. In der Praxis muss diese aus Sensordaten geschÃ¤tzt werden. FÃ¼r Hochgeschwindigkeitsanwendungen bieten sich folgende Sensorkonzepte an:

	\begin{description}
		\item[Gimbalkamera + Kalman-Filter:] Eine kardanisch gelagerte Kamera liefert azimut- und elevationskorrigierte Bilddaten. Ein Extended Kalman Filter (EKF) fusioniert diese mit IMU-Daten zu einer 3D-LagerschÃ¤tzung des Ziels.
		\item[Event-Kamera (neuromorphe Sensorik):] Ereignisbasierte Kameras messen asynchron HelligkeitsÃ¤nderungen mit Mikrosekunden-Latenz. Sie sind fÃ¼r die schnellen Winkelbewegungen in Phase $\mathcal{P}_5$ besonders geeignet, da konventionelle Frame-Kameras bei $> 200 \, \mathrm{fps}$ unzureichende Lichtausbeute aufweisen.
		\item[Radar + optische Kamera:] Radar liefert Range-Doppler-Informationen bei schlechter Sicht; die Fusion mit optischen Daten erhÃ¶ht die TrennfÃ¤higkeit gegenÃ¼ber Clutter.
	\end{description}

	Die Integration eines solchen Wahrnehmungsmoduls wÃ¼rde den BioFalcon-Tracking-Algorithmus (\cref{alg:tracking}) zu einem vollstÃ¤ndig autonomen Erfassungs- und Verfolgungssystem vervollstÃ¤ndigen.

	\subsubsection{Morphing-FlÃ¼gelstrukturen und bioinspirierte Materialien}

	Der Wanderfalke nutzt nicht nur eine starre FlÃ¼gelgeometrie, sondern verÃ¤ndert die FlÃ¼gelform kontinuierlich und anisotrop. Technisch lÃ¤sst sich dies durch so genannte \textit{Morphing-Strukturen} nachbilden:

	\begin{itemize}
		\item \textbf{FormgedÃ¤chtnislegierungen (SMA):} Nickeltitan-Aktoren kÃ¶nnen durch elektrische ErwÃ¤rmung reversibel ihre Form Ã¤ndern und bieten damit eine direkte Analogie zur Muskelkontraktion. Die erreichbaren StellkrÃ¤fte and Stellzeiten ($\sim \SI{100}{\ms}$) sind fÃ¼r Phase $\mathcal{P}_2$ ausreichend.
		\item \textbf{Pneumatische Balgaktoren:} Leichte, integrierbare BÃ¤lge kÃ¶nnen lokale FlÃ¼gelwÃ¶lbungen erzeugen und die Auftriebsverteilung wÃ¤hrend des Pullout-ManÃ¶vers $({\mathcal{P}_4})$ optimieren.
		\item \textbf{Kohlefaser-Verbundstrukturen mit gezielter Anisotropie:} Durch gezielte Ausrichtung der FaserverstÃ¤rkung lÃ¤sst sich eine wash-out-Torsion implementieren, die bei hohen Anstellwinkeln automatisch die Spitzenbelastung reduziert --- analog zum durch Muskelspannung kontrollierten VogelflÃ¼gel.
	\end{itemize}

	Die Modellierung dieser Effekte erfordert eine Kopplung des aerodynamischen Modells mit einem Finite-Elemente-Modell der FlÃ¼gelstruktur (aeroelastische Simulation).

	\subsubsection{Rechtliche und ethische Rahmenbedingungen}

	Der Betrieb autonomer Hochgeschwindigkeitsdrohnen im Luftraum unterliegt strengen regulatorischen Anforderungen. In der EuropÃ¤ischen Union ist die EASA-Verordnung (EU) 2019/947 maÃŸgeblich: Drohnen mit kinetischer Energie $> \SI{80}{\J}$ im Sturzflug sind der Kategorie \textit{Specific} zuzuordnen und erfordern eine Betriebsgenehmigung (SORA-Verfahren). Die im Rahmen dieser Arbeit ausgelegte Referenzdrohne (\SI{1.2}{\kg}, $v_{max} \approx \SI{316}{\m\per\second}$) Ã¼berschreitet diese Schwelle erheblich und ist daher ausschlieÃŸlich in Sperrzonen oder militÃ¤risch freigegebenen LuftrÃ¤umen betreibbar.

	DarÃ¼ber hinaus werfen vollstÃ¤ndig autonome Angriffssysteme grundlegende ethische Fragen auf, die Ã¼ber den technischen Rahmen dieser Arbeit hinausgehen. Das \textit{Campaign to Stop Killer Robots} sowie zahlreiche nationale Deklarationen fordern einen ``meaningful human control''-Standard, der auch fÃ¼r zivile Anwendungen wie Wildtierschutz oder Kontradrohnen-Einsatz zu beachten ist. ZukÃ¼nftige Arbeiten sollten daher stets einen integrierten Operator-in-the-Loop-Modus vorsehen, der eine manuelle Ãœbernahme der Steuerung in jeder Phase des Fluges ermÃ¶glicht.

	\subsubsection{Langzeitziele: Biologischer Digitaler Zwilling}

	Das langfristige wissenschaftliche Ziel der BioSAS-Forschungslinie ist die Schaffung eines vollstÃ¤ndigen \textit{digitalen Zwillings} des Wanderfalken-Sturzfluges: ein Simulationsmodell, das biologische Messdaten (Telemetrie-GPS, Hochgeschwindigkeitsvideometrie, integrierte Beschleunigungssensoren am lebenden Tier) direkt assimiliert und daraus optimale Regelungsstrategien fÃ¼r technische Systeme ableitet. Der hier entwickelte Formalismus --- 6-DOF-Modell, Pontryagin-OptimalitÃ¤tsbeweis, BGSG-Regler --- stellt dabei die regelungstechnische Kernkomponente dar, die durch biologische Datenfusion, maschinelles Lernen und Echtzeit-Embedded-Implementierung zu einem umfassenden kybernetischen System vervollstÃ¤ndigt werden soll.
	...
	\begin{thebibliography}{99}
		...
		\bibitem{aero1}
		Ponitz, B., Schmitz, A., Fischer, D., Bleckmann, H., \& BrÃ¼cker, C. (2014). Diving-flight aerodynamics of a peregrine falcon (\textit{Falco peregrinus}). \textit{PLOS ONE}, 9(2), e86506.
		...
		\bibitem{aero2}
		Henningsson, P., Johansson, L.C., \& HedenstrÃ¶m, A. (2010). How swift is a swift? Undulating posture improves transient flight performance. \textit{PLOS ONE}, 5(8), e11895.
		...
		\bibitem{prandtl1919}
		Prandtl, L. (1919). TragflÃ¼geltheorie. I. Mitteilung. \textit{Nachrichten von der Gesellschaft der Wissenschaften zu GÃ¶ttingen, Mathematisch-Physikalische Klasse}, 1919, 451--477.
		...
		HedenstrÃ¶m, A., \& RosÃ©n, M. (2003). Body frontal area in passerine birds. \textit{Journal of Avian Biology}, 34(2), 159--162.
		...
		\bibitem{astrom2008}
		Ã…strÃ¶m, K.J., \& Murray, R.M. (2008). \textit{Feedback Systems: An Introduction for Scientists and Engineers}. Princeton University Press, Princeton.

		\bibitem{fossen2011}
		Fossen, T.I. (2011). \textit{Handbook of Marine Craft Hydrodynamics and Motion Control}. Wiley, Chichester.

		\bibitem{brockett1983}
		Brockett, R.W. (1983). Asymptotic stability and feedback stabilization. In: \textit{Differential Geometric Control Theory}, pp.~181--191. BirkhÃ¤user, Boston.
