...
moderner Softwareentwicklung. In einer Branche, in der Softwaresysteme Ã¼ber
Jahrzehnte gepflegt, erweitert und auf neue Plattformen portiert werden mÃ¼ssen,
entscheidet die innere Ordnung eines Projekts maÃŸgeblich Ã¼ber Entwicklungskosten,
TeamproduktivitÃ¤t und die langfristige ÃœberlebensfÃ¤higkeit einer Codebasis.

Insbesondere bei objektorientierter Programmierung mit tiefen Vererbungshierarchien
stellt sich die Frage, wie der Quellcode so organisiert werden kann, dass er fÃ¼r
...
geht die Notwendigkeit, Klassenhierarchien -- die conceptuellen RÃ¼ckgrate
objektorientierter Systeme -- auch in der physischen Dateistruktur des Projekts
sichtbar zu machen.

FÃ¼r die Modellierung und Dokumentation von Softwarearchitekturen hat sich die
...
einem eigenen Python-Modul. Diese Konvention erhÃ¶ht die
\textit{Analysierbarkeit} und \textit{Wartbarkeit} des Codes erheblich, da ein
Software-Engineer beim Lesen eines bestimmten Moduls sofort weiÃŸ, auf welcher
Abstraktionsebene er sich befindet.

ErgÃ¤nzend wird das Prinzip eingefÃ¼hrt, das Werkzeug \textbf{Pyreverse}
...
% ================================================
\section{Vererbungshierarchie und Modulstruktur}
% ================================================

\subsection{Das UML-Klassendiagramm als Ausgangspunkt}

Das UML-Klassendiagramm bildet die konzeptuelle Grundlage fÃ¼r die vorgeschlagene
...
\begin{table}[h]
	...
	\begin{tabular}{>{\bfseries}c p{4cm} p{6cm}}
		...
		2 & IntermediÃ¤re Klassen (Spezialisierung) & \texttt{intermediate\_classes.py} \\
		3 & Konkrete Klassen (DU-Klassen, direkt verwendbar) & \texttt{concrete\_classes.py} \\
		\bottomrule
	\end{tabular}
\end{table}

Ein Software-Engineer, der \texttt{intermediate\_classes.py} Ã¶ffnet, weiÃŸ unmittelbar:
\begin{itemize}
...
\subsection{Vorgehen zur Umgestaltung}
Das Vorgehen zur Umgestaltung: Erstelle ein neues Python Modul fÃ¼r jede Ebene der Vererbungshierarchie. Dazu wird nur Code zum Softwaresystem hinzugefÃ¼gt und kein Code gelÃ¶scht. Die neuen Python Module enthalten dann alle neuen Klassen, die es aber im Projekt schon geben muss. Sonst wÃ¼rden sie nicht erstellt werden. Jetzt gilt es nur noch, die alten Python Klassen der Vererbungshierarchie, fÃ¼r die neue erstellt worden sind, zu entfernen. Diese einfache Umgestaltung erhÃ¤lt das ganze Verhalten des Systems und bildet gleichzeitig die Vererbungshierarchie horizontal ab.
...
\subsection{Modul Ebene 2: IntermediÃ¤re Klassen}
...
die Analysierbarkeit einer Codebasis auf fundamentale und vielfÃ¤ltige Weise.
Analysierbarkeit bedeutet dabei mehr als nur das Verstehen einzelner Codezeilen --
sie umfasst die FÃ¤higkeit, AbhÃ¤ngigkeiten zu erkennen, das Gesamtbild zu erfassen
und gezielt in der richtigen Schicht des Systems zu navigieren.

\paragraph{Sofortige Kontextorientierung.}
Ein Entwickler, der ein Modul Ã¶ffnet, entnimmt dem Dateinamen und dem Kopfkommentar
...
begrenzte KapazitÃ¤t fÃ¼r die gleichzeitige Verarbeitung von Informationen besitzen
\cite{sweller1988}. Wenn der Kontext bereits durch den Modulnamen geliefert wird,
steht die gesamte mentale KapazitÃ¤t fÃ¼r die inhaltliche Analyse des Codes zur
VerfÃ¼gung.

\paragraph{Klare AbhÃ¤ngigkeitsrichtung.}
Da Module hÃ¶herer Ebenen stets von Modulen niedrigerer Ebenen importieren (und
nicht umgekehrt), ergibt sich ein azyklischer AbhÃ¤ngigkeitsgraph -- ein
...
automatisiert geprÃ¼ft werden. ZirkulÃ¤re Importe -- eine hÃ¤ufige Ursache von
...
verstehen, sondern auch sofort einschÃ¤tzen, welche Ebenen von einer Ã„nderung
betroffen sein kÃ¶nnten.
...
Diese Zuordnung ist deterministisch und projektÃ¼bergreifend konsistent. Entwickler,
die von einem Projekt in ein anderes wechseln, das dieselbe Konvention verwendet,
mÃ¼ssen die Orientierungsphase nicht neu durchlaufen. Es entsteht ein wiederverwendbares
...
\texttt{pylint}~\cite{pylint2024} oder \texttt{flake8} kÃ¶nnen bei klar separierten
Modulen prÃ¤zisere und weniger rauschende Analysen liefern. Da jede Ebene nur
Typen und Klassen der eigenen oder darunter liegenden Ebene referenziert, werden
Typfehler und AbhÃ¤ngigkeitsverletzungen frÃ¼her und an der richtigen Stelle gemeldet.

\paragraph{Vereinfachte Code-Reviews.}
Bei Pull Requests oder Merge Requests ist auf einen Blick erkennbar, welche
Hierarchieebene(n) von einer Ã„nderung betroffen sind. Reviewer kÃ¶nnen sich gezielt
auf die verÃ¤nderte Ebene konzentrieren, ohne das gesamte Klassenmodell im Kopf
halten zu mÃ¼ssen. Dies reduziert den Review-Aufwand und erhÃ¶ht gleichzeitig die
QualitÃ¤t der Reviews, da Reviewer weniger abgelenkt und mental belastet sind
\cite{bird2011}.

\paragraph{Reproduzierbare UML-Dokumentation.}
Da jedes Modul exakt einer Hierarchieebene entspricht, lÃ¤sst sich aus der
...
Dokumentationspflege entÃ¼berflÃ¼ssigt (siehe Kapitel~\ref{sec:pyreverse}).

\subsection{Wartbarkeit}

Wartbarkeit ist nach der ISO/IEC-25010-Norm~\cite{iso25010} eine der Kerneigenschaften
von SoftwarequalitÃ¤t. Sie umfasst Modifizierbarkeit, Testbarkeit, Analysierbarkeit,
ModularitÃ¤t und Wiederverwendbarkeit. Die horizontale Ebenenabbildung adressiert
alle diese Teilaspekte direkt und nachweislich.

\paragraph{Lokalisierte Ã„nderungen und minimaler Ã„nderungsaufwand.}
Ã„nderungen an Basisklassen betreffen ausschlieÃŸlich \texttt{base\_classes.py}.
Der Entwickler weiÃŸ, dass jede Modifikation dort Auswirkungen auf alle darÃ¼ber
liegenden Ebenen haben kann -- eine klar kommunizierte und beherrschbare Situation.
Im Gegensatz dazu fÃ¼hrt eine unstrukturierte Codebasis, in der alle Klassen
einer DomÃ¤ne in einer einzigen Datei gesammelt sind, dazu, dass selbst kleine
Ã„nderungen schwer lokalisierbare Nebeneffekte erzeugen kÃ¶nnen.
...
-- nÃ¤mlich die Klassen \textit{einer} Vererbungsebene zu definieren und zu
kapseln.

\paragraph{UnabhÃ¤ngige Testbarkeit und Test-Isolation.}
Jedes Modul kann unabhÃ¤ngig und in isolierter Umgebung getestet werden. Unit-Tests
fÃ¼r Ebene 1 mÃ¼ssen keine konkreten Klassen aus Ebene 3 kennen und sind daher
robust gegenÃ¼ber Ã„nderungen in hÃ¶heren Ebenen. Dies entspricht dem Prinzip
...
DarÃ¼ber hinaus ermÃ¶glicht die klare Trennung das gezielte Einsetzen von
\textit{Test Doubles} (Mocks, Stubs, Fakes) auf Ebenengrenze: Tests fÃ¼r Ebene 2
kÃ¶nnen die Ebene-1-Klassen durch einfache Mocks ersetzen, ohne komplexe
AbhÃ¤ngigkeitsketten aufzulÃ¶sen. Dies erhÃ¶ht die AusfÃ¼hrungsgeschwindigkeit der
Testsuite erheblich und senkt den Testpflegeaufwand~\cite{meszaros2007}.

\paragraph{Erleichtertes Onboarding neuer Entwickler.}
Neue Teammitglieder kÃ¶nnen die Codebasis \textit{schichtweise} erschlieÃŸen: Sie
...
das VerstÃ¤ndnis einer neuen Codebasis im Durchschnitt 30 bis 50\,\% der
...
Der Software-Engineer hÃ¤lt im ArbeitsgedÃ¤chtnis stets nur den Kontext einer
einzigen Hierarchieebene. Sweller~\cite{sweller1988} zeigt, dass die begrenzte
KapazitÃ¤t des ArbeitsgedÃ¤chtnisses (\textit{Working Memory}) eine fundamentale
EinschrÃ¤nkung menschlicher ProblemlÃ¶sungsfÃ¤higkeit darstellt. Ohne die
Ebenenkonvention mÃ¼ssten Entwickler beim Lesen einer einzigen groÃŸen Datei
gleichzeitig Basisklassen, Zwischenklassen und konkrete Klassen mental verwalten
-- ein typischer AuslÃ¶ser fÃ¼r Fehler und MissverstÃ¤ndnisse.

\paragraph{ErhÃ¶hte Wiederverwendbarkeit.}
Da Basisklassen und intermediÃ¤re Klassen in separaten Modulen isoliert sind,
kÃ¶nnen sie deutlich leichter in anderen Projekten wiederverwendet werden. Ein
...
Klassenbaum des Ursprungsprojekts zu importieren. Diese Eigenschaft fÃ¶rdert
die Entstehung robuster, kompositionsfÃ¤higer Bibliotheken.

\paragraph{Langfristige Evolvierbarkeit.}
Softwaresysteme sind selten statisch -- sie wachsen, verÃ¤ndern sich und
werden an neue Anforderungen angepasst. Die Ebenenabbildung unterstÃ¼tzt diese
Evolvierbarkeit~\cite{lehman1997}, indem neue Vererbungsebenen einfach durch
HinzufÃ¼gen neuer Module eingefÃ¼hrt werden kÃ¶nnen, ohne bestehende Module zu
modifizieren. Dies entspricht dem \textit{Open/Closed Principle}
(OCP)~\cite{martin2017}: Module sind offen fÃ¼r Erweiterung (durch neue Ebenen),
aber geschlossen fÃ¼r Modifikation.

% ================================================
\section{Kostenreduktion in Softwareprojekten}
% ================================================

\subsection{Wirtschaftliche Bedeutung der inneren CodequalitÃ¤t}

Die Kosten von Softwareprojekten werden in der Praxis hÃ¤ufig unterschÃ¤tzt.
...
Ã¼ber den gesamten Lebenszyklus des Systems.

\subsection{Reduktion von Fehlerbehebungskosten}

Fehler, die frÃ¼h im Entwicklungsprozess erkannt werden, sind um GrÃ¶ÃŸenordnungen
gÃ¼nstiger zu beheben als solche, die erst in der Produktion auftreten. Das
...
in der Designphase gefundenen Fehlers. Die Ebenenabbildung fÃ¶rdert frÃ¼hes
Fehlererkennen auf mehrere Weisen:

\begin{itemize}[leftmargin=*]
	\item Durch eindeutige ModulzustÃ¤ndigkeit sind Fehlerquellen schnell lokalisierbar.
	\item Statische Analysetools liefern prÃ¤zisere Meldungen, da der Scope je Modul
	kleiner ist.
	\item Unittests kÃ¶nnen gezielt auf einzelne Ebenen zugeschnitten werden, was die
	Fehlerabdeckung erhÃ¶ht.
\end{itemize}
...
identifizieren das Fehlen klarer Strukturkonventionen als einen der hÃ¤ufigsten
...
Quantitativ lÃ¤sst sich der Effekt abschÃ¤tzen: Wenn das Onboarding eines
erfahrenen Entwicklers im Durchschnitt vier Wochen dauert und durch eine
konsequente Strukturkonvention um eine Woche verkÃ¼rzt werden kann, entspricht
dies bei einem Jahresgehalt von 80.000~Euro einer Einsparung von etwa
\textbf{1.500 Euro pro neuem Teammitglied}. In einem Team, das jÃ¤hrlich fÃ¼nf
...
stark davon abhÃ¤ngt, wie gut ein Reviewer die betroffene Codestelle
kontextualisieren kann. Ein Reviewer, der weiÃŸ, dass eine Ã„nderung ausschlieÃŸlich
Ebene-2-Module betrifft, kann seinen Review auf intermediÃ¤re Klassen
konzentrieren, ohne die gesamte Klassenhierarchie im Kopf halten zu mÃ¼ssen.
SchÃ¤tzungsweise kann diese Fokussierung den Review-Aufwand je Ã„nderung um
15--25\,\% reduzieren.

\subsection{Reduktion von Regressionskosten}

Regressionen -- also Fehler, die durch Ã„nderungen an bestehendem Code in
bisher funktionierenden Bereichen eingefÃ¼hrt werden -- sind eine der
kostspieligsten Fehlerklassen~\cite{pressman2014}. Die Ebenenabbildung
reduziert Regressionsrisiken strukturell:

\begin{itemize}[leftmargin=*]
	\item Ã„nderungen in Ebene 3 kÃ¶nnen Ebene 1 und 2 nicht direkt beeinflussen
	(da keine RÃ¼ckwÃ¤rtsimporte existieren).
	\item Ebene-1-Ã„nderungen sind durch die explizite Isolierung als
	...
	systematische RegressionsprÃ¼fung je Ebene ermÃ¶glicht.
\end{itemize}
...
LÃ¶sungen implementieren. Die Ebenenabbildung wirkt dieser Tendenz entgegen,
indem sie eine \textit{strukturelle Invariante} einfÃ¼hrt, die auch unter
...
Eine der grÃ¶ÃŸten Gefahren in der Softwareentwicklung ist die Divergenz zwischen
...
Entwickler zu falschen Annahmen und kÃ¶nnen schwerwiegende Designfehler
verursachen~\cite{kruchten1995}.

\textit{Die Wahrheit steht immer im Code. UML-Diagramme sind keine eigenstÃ¤ndigen
Artefakte, die gepflegt werden mÃ¼ssen -- sie sind \textbf{abgeleitete Sichten auf den Code}. Jedes Diagramm, das nicht automatisch aus dem aktuellen
Quellcode erzeugt wird, ist ein potenzielles Sicherheitsrisiko fÃ¼r die
ArchitekturintegritÃ¤t des Projekts.}

Dieses Prinzip lÃ¤sst sich mit \textbf{Pyreverse} konsequent umsetzen. Pyreverse
ist ein Werkzeug, das automatisch UML-Klassen- und Paketdiagramme aus
Python-Quellcode generiert~\cite{pyreverse2024}. Es ist standardmÃ¤ÃŸig Bestandteil
von Pylint~\cite{pylint2024} und kann ohne zusÃ¤tzliche Installation genutzt werden.
...
FÃ¼r die Ausgabe als \texttt{.png}, \texttt{.pdf} oder \texttt{.svg} wird
Graphviz benÃ¶tigt~\cite{graphviz2024}:
...
Paketdiagramm fÃ¼r ein gegebenes Python-Paket:
...
FÃ¼r Projekte, die noch strenger vorgehen mÃ¶chten, kann Pyreverse als
Pre-commit Hook eingebunden werden, sodass bei jedem Commit automatisch
geprÃ¼ft wird, ob sich die UML-Diagramme verÃ¤ndert haben:
...
vollstÃ¤ndiges, stets aktuelles Bild der Systemarchitektur -- ohne manuellen
...
Das vorgestellte Prinzip ist bewusst einfach gehalten und ergÃ¤nzt bestehende
Architekturkonzepte, ohne mit ihnen zu konkurrieren:

\begin{itemize}[leftmargin=*]
	\item \textbf{Schichtenarchitektur (Layered Architecture):} Dort werden
	Schichten nach \textit{technischer Funktion} (PrÃ¤sentation, Logik,
	Datenzugriff) unterschieden~\cite{fowler2002}. Die Ebenenabbildung
	hier richtet sich nach der \textit{Vererbungstiefe} im DomÃ¤nenmodell.
	Beide Prinzipien sind orthogonal und kÃ¶nnen kombiniert werden.
	
	\item \textbf{Package by Feature:} Strukturiert Code nach fachlichen
	Features. Die Ebenenabbildung ist komplementÃ¤r und kann innerhalb
	eines Feature-Pakets angewandt werden.
	
	\item \textbf{Hexagonale Architektur / Ports and Adapters:} Fokussiert auf
	AbhÃ¤ngigkeitsumkehr an Systemgrenzen~\cite{cockburn2005}. Die
	Ebenenabbildung betrifft die interne Strukturierung der DomÃ¤nenklassen.
...
Das Prinzip entfaltet seinen grÃ¶ÃŸten Nutzen, wenn:
\begin{itemize}[leftmargin=*]
...
\begin{itemize}[leftmargin=*]
	...
	gering; eine einzelne Datei genÃ¼gt.
	...
	\item Das Prinzip ersetzt \textbf{keine vollstÃ¤ndige Architekturdokumentation},
	sondern ist eine ergÃ¤nzende Konvention auf Modulebene.
	\item \textbf{Pyreverse} stÃ¶ÃŸt bei sehr groÃŸen Projekten (>500 Klassen) an
	visuelle Grenzen -- hier empfiehlt sich eine Aufteilung in
	Teilpakete-Diagramme.
\end{itemize}

% ================================================
\section{Ausblick: Konsequenzen fÃ¼r alle Bereiche der Softwareentwicklung}
\label{sec:ausblick}
% ================================================
...
Konsequenzen fÃ¼r nahezu jeden Bereich moderner Softwareentwicklung. Im Folgenden
werden diese Konsequenzen systematisch und ausfÃ¼hrlich beleuchtet.
...
die SchÃ¤tzung von Story Points und die Aufteilung von Aufgaben auf Teammitglieder
ein zentraler Bestandteil des Prozesses. Die Ebenenabbildung liefert hierfÃ¼r
eine natÃ¼rliche GranularitÃ¤t: Aufgaben kÃ¶nnen gezielt einer bestimmten
...
\textit{Trunk-Based Development}, werden Ã„nderungen hÃ¤ufig und in kleinen
Einheiten in den Hauptzweig integriert. Die Ebenenstruktur fÃ¶rdert diese Praxis,
da Ã„nderungen je Ebene atomar und weitgehend unabhÃ¤ngig voneinander sind. Feature
Branches bleiben kurzlebig, da die Wahrscheinlichkeit von Cross-Ebenen-Konflikten
gering ist.

\paragraph{Definition of Done.}
Teams, die das Prinzip einsetzen, kÃ¶nnen ihre \textit{Definition of Done} um
einen klaren Punkt erweitern: \enquote{UML-Diagramme mittels Pyreverse aktualisiert
	und committet.} Damit wird die ArchitekturkonformitÃ¤t zu einem expliziten
QualitÃ¤tsmerkmal jedes Inkrements -- nicht zu einem gelegentlichen Audit.
...
Entwurfsphase. Entwickler kÃ¶nnen Klassen nicht mehr \textit{irgendwo}
hinzufÃ¼gen -- sie mÃ¼ssen bewusst entscheiden, ob es sich um eine Basis-,
intermediÃ¤re oder konkrete Klasse handelt. Diese Disziplin fÃ¼hrt zu saubereren
Designs und verhindert das schleichende Entstehen von \textit{God Classes}
\cite{riel1996} -- Klassen, die zu viel Verantwortung Ã¼bernehmen.

\paragraph{FÃ¶rderung von SOLID-Prinzipien.}
Das Strukturprinzip fÃ¶rdert mehrere der SOLID-Prinzipien~\cite{martin2017}
...
Ã¼ber neue Ebenenmodule gestÃ¤rkt, und das \textit{Liskov Substitution Principle}
wird durch die explizite Trennung der Ebenen natÃ¼rlich unterstÃ¼tzt.

\paragraph{Schnittstellen- und API-Design.}
Ebene-1-Module definieren die stabilen Schnittstellen des Systems. Da diese
Module von allen hÃ¶heren Ebenen abhÃ¤ngen, werden SchnittstellenÃ¤nderungen als
hochimpaktiv erkannt und erfordern bewusste Entscheidungen. Dies fÃ¶rdert ein
stabiles, rÃ¼ckwÃ¤rtskompatibles API-Design und erleichtert die EinfÃ¼hrung von
\textit{Semantic Versioning}~\cite{semver2024}.

\subsection{Auswirkungen auf Testing und QualitÃ¤tssicherung}

\paragraph{Testpyramide und Testschichten.}
Das Konzept der \textit{Testpyramide}~\cite{fowler2012} empfiehlt, mÃ¶glichst
...
Pyramide: Ebene-1-Tests sind reine Unit-Tests ohne externe AbhÃ¤ngigkeiten,
Ebene-2-Tests kÃ¶nnen Ebene-1-Klassen mocken, und Ebene-3-Tests kÃ¶nnen die
vollstÃ¤ndige Klassenhierarchie integrieren. Jede Ebene hat ihre natÃ¼rliche
...
ermÃ¶glichen eine ebenenweise Analyse der Testabdeckung und MutationsstÃ¤rke.
Teams kÃ¶nnen explizite Abdeckungsziele je Ebene definieren (z.\,B. 100\,\%
fÃ¼r Ebene 1, da dort die stabilsten Abstraktionen liegen) und diese in der
CI/CD-Pipeline durchsetzen.

\paragraph{Property-Based Testing.}
Frameworks wie \texttt{Hypothesis}~\cite{hypothesis2024} fÃ¼r
...
definiert und durch alle Unterklassen hindurch getestet werden kÃ¶nnen. Dies
erhÃ¶ht die Robustheit des Tests enorm, da alle konkreten Implementierungen
automatisch auf Einhaltung der Basisinvarianten geprÃ¼ft werden.
...
auf null -- und erhÃ¶ht gleichzeitig die ZuverlÃ¤ssigkeit der Dokumentation.
...
Methode, um Architekturentscheidungen zu dokumentieren. Das EinfÃ¼hren der
Ebenenabbildungskonvention ist selbst eine solche Entscheidung und sollte als ADR
im Repository festgehalten werden. Dies schafft Nachvollziehbarkeit fÃ¼r zukÃ¼nftige
Entwickler und verhindert, dass die Konvention versehentlich aufgegeben wird.

\paragraph{Onboarding-Dokumentation.}
Die Ebenenstruktur ermÃ¶glicht eine systematische und skalierbare
Onboarding-Dokumentation: Anstatt die gesamte Codebasis zu beschreiben, kann
die Dokumentation schichtweise aufgebaut werden. Neue Entwickler erhalten zunÃ¤chst
eine EinfÃ¼hrung in die Basisklassen (Ebene 1), dann in die intermediÃ¤ren Klassen,
und schlieÃŸlich in die konkreten Klassen. Dieses progressive Lernmodell
entspricht modernen Erkenntnissen der Lernpsychologie~\cite{vygotsky1978}.

\subsection{Auswirkungen auf DevOps und CI/CD}

\paragraph{Automatisierte QualitÃ¤tstore.}
In modernen DevOps-Pipelines werden QualitÃ¤tstests als \textit{Quality Gates}
formuliert: Builds, die bestimmte QualitÃ¤tsschwellen nicht erfÃ¼llen, werden
automatisch abgelehnt~\cite{humble2010}. Die Ebenenabbildung ermÃ¶glicht granulare
...
\begin{itemize}[leftmargin=*]
	...
	\item Pyreverse muss erfolgreich abschlieÃŸen (keine unklassifizierbaren AbhÃ¤ngigkeiten)
\end{itemize}

\paragraph{Release-Automation und Changelogs.}
Da jede Ebene ein eigenes Modul ist, kÃ¶nnen automatisierte Changelogs
...
\textit{"\texttt{feat(level-2)}: Neue Spezialisierung fÃ¼r SÃ¤ugetiere"} ist
fÃ¼r alle Stakeholder sofort verstÃ¤ndlich.
...
kann die Ebenenstruktur auf Servicegrenzen Ã¼bertragen werden: Basisklassen-Bibliotheken
werden als eigenstÃ¤ndige Container-Images oder Python-Pakete verÃ¶ffentlicht und
versioniert. IntermediÃ¤re und konkrete Klassen kÃ¶nnen diese Pakete als AbhÃ¤ngigkeiten
einbinden. Dies fÃ¶rdert die Wiederverwendbarkeit Ã¼ber Servicegrenzen hinweg.
...
Umgekehrt beeinflusst die Codestruktur, wie Teams organisiert werden kÃ¶nnen.
Die Ebenenabbildung ermÃ¶glicht spezialisierte Teams (oder Rollen innerhalb
eines Teams): Ein \textit{Core-Team} pflegt die stabilen Basisklassen (Ebene 1),
wÃ¤hrend Feature-Teams die konkreten Implementierungen (Ebene 3) entwickeln --
mit klarer Ownership und minimalen Koordinationskosten.

\paragraph{Open-Source-Kollaboration.}
In Open-Source-Projekten ist eine klare Modulstruktur entscheidend fÃ¼r den
Erfolg externer BeitrÃ¤ge~\cite{raymond1999}. Externe Contributer, die nur
Ebene-3-Klassen (konkrete Implementierungen) beitragen mÃ¶chten, mÃ¼ssen die
Basisklassen nur lesen, nicht verstehen. Die EinstiegshÃ¼rde sinkt, die
QualitÃ¤t externer BeitrÃ¤ge steigt.
...
UML-Hierarchieebenen -- unterstÃ¼tzt durch den OMG-Standard~\cite{omg2017} --
Ã¼berbrÃ¼ckt sprachliche und kulturelle Barrieren. Ein Entwickler in Indien,
der weiÃŸ, dass er an Ebene-2-Klassen arbeitet, versteht die Rolle seiner
Arbeit im Gesamtsystem ohne ausfÃ¼hrliche schriftliche ErklÃ¤rungen.

\subsection{Auswirkungen auf KI-gestÃ¼tzte Softwareentwicklung}

\paragraph{LLM-gestÃ¼tzte Codegenerierung.}
Die zunehmende Nutzung von Large Language Models (LLMs) wie GitHub Copilot oder
Claude~\cite{chen2021} in der Softwareentwicklung erÃ¶ffnet neue MÃ¶glichkeiten --
...
Codegenerierung architekturkonform durchzufÃ¼hren. Ein Prompt wie
\enquote{FÃ¼ge eine neue Klasse auf Ebene 2 ein, die von BaseAnimal erbt} ist
fÃ¼r ein LLM wesentlich prÃ¤ziser auswertbar als \enquote{FÃ¼ge eine neue Klasse
	in das Projekt ein}.

\paragraph{Automatisierte ArchitekturprÃ¼fung.}
In Zukunft kÃ¶nnten KI-basierte Werkzeuge die EbenenkonformitÃ¤t einer Codebasis
automatisch prÃ¼fen und Verletzungen -- wie eine konkrete Klasse in einem
Basismodul -- als Architektursmells markieren~\cite{tamburri2019}. Die horizontale
Ebenenabbildung liefert die formale Grundlage fÃ¼r solche PrÃ¼fungen.

\paragraph{Automatische Dokumentationsgenerierung.}
Kombiniert mit modernen LLMs kann Pyreverse als Grundlage fÃ¼r die automatische
Generierung narrativer Architekturbeschreibungen dienen: Das LLM erhÃ¤lt das
Pyreverse-Klassendiagramm als Eingabe und generiert eine natÃ¼rlichsprachliche
Beschreibung der Architektur -- die dann ebenfalls zum Release automatisch
aktualisiert wird. Dies realisiert eine vollstÃ¤ndig automatisierte,
...
Ã¶konomisch tragfÃ¤hig bleibt. Die horizontale Ebenenabbildung ist ein Beitrag
...
\begin{itemize}[leftmargin=*]
	...
	strukturelle Klarheit auch bei wachsender CodegrÃ¶ÃŸe erhalten bleibt.
	\item Sie senkt den \textbf{Energieverbrauch} der Entwicklung: Weniger Zeit
	fÃ¼r Orientierung, Review und Fehlerbehebung bedeutet weniger
	Entwicklerstunden pro Feature -- und damit einen kleineren
	Ã¶kologischen FuÃŸabdruck des Entwicklungsprozesses.
	\item Sie fÃ¶rdert \textbf{Wissenstransfer} zwischen Generationen von
	Entwicklern: Systeme, die nach klaren Konventionen strukturiert sind,
	kÃ¶nnen auch nach einem vollstÃ¤ndigen Teamwechsel weiterentwickelt werden.
\end{itemize}
...
einfache, aber tiefgreifend wirkungsvolle Konvention. Sie Ã¼bersetzt die grafische
...
Ã¶ffnet, weiÃŸ sofort, auf welcher Ebene der Abstraktion er sich befindet.

Die Vorteile reichen weit Ã¼ber technische Details hinaus: sofortige
Kontextorientierung, klare AbhÃ¤ngigkeitsrichtungen, lokalisierte Ã„nderungen,
...
entfalten sich aus einer einzigen, leicht einzufÃ¼hrenden Konvention.

ErgÃ¤nzt durch den konsequenten Einsatz von Pyreverse~\cite{pyreverse2024} zu
jedem Release entsteht ein vollstÃ¤ndiger Kreislauf: Der Code definiert die
...
Insbesondere in einer Ã„ra, in der KI-gestÃ¼tzte Entwicklungswerkzeuge zunehmend
Einzug halten~\cite{chen2021}, liefert eine klar strukturierte Codebasis mit
konsistenter UML-Dokumentation den prÃ¤zisen Kontext, den sowohl menschliche
als auch maschinelle Entwickler benÃ¶tigen, um korrekt und architekturkonform
zu arbeiten.

\vspace{1em}
\noindent Die EinfÃ¼hrung des Prinzips erfordert keine Ã„nderung der Sprache,
des Frameworks oder des Build-Systems -- es genÃ¼gt, eine gemeinsame Konvention
...
\begin{thebibliography}{99}
	...
	Online verfÃ¼gbar: \url{https://alistair.cockburn.us/hexagonal-architecture/}, 2005.
	...
	Martin Fowler:
	\enquote{TestPyramid}.
	Online verfÃ¼gbar: \url{https://martinfowler.com/bliki/TestPyramid.html}, 2012.
	...
	Online verfÃ¼gbar: \url{https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions},
	2011.
