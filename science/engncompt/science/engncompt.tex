...
\begin{titlepage}
	...
	\vspace{0.5cm}
	{\Huge\bfseries
		Innovativer Motorraum-Entwurf fÃ¼r Kraftfahrzeuge
		\par}
	{\large\itshape
		Formaler Entwurf zugÃ¤nglichkeitsoptimierter MotorrÃ¤ume fÃ¼r\\
...
\begin{abstract}
	...
	im Fahrzeugbau dar: Mehrere Dutzend Subsysteme mÃ¼ssen auf engem Raum sicher
	koexistieren, thermisch entkoppelt werden und fÃ¼r Wartung, Diagnose und
	Notfallzugang erreichbar bleiben. Klassische MotorrÃ¤ume werden Ã¼berwiegend
	bauraumgetrieben entworfen -- ZugÃ¤nglichkeit wird nachgelagert berÃ¼cksichtigt.

	Diese Arbeit prÃ¤sentiert einen formalen, zugÃ¤nglichkeitsorientierten Entwurfsrahmen
	fÃ¼r zwei fundamentale Motorraum-Architekturen: \textbf{(A)} Verbrennungsmotor-Fahrzeuge
	(Diesel und Benzin) und \textbf{(B)} Elektrofahrzeuge (EV) mit Lithium-Ionen-Batterien.
	FÃ¼r Architektur~(B) werden die Ergebnisse eines formal bewiesenen KI-gesteuerten
	Power-Management-Moduls (AI-PMM) \cite{Epp2026liion} direkt in geometrische
	und thermische Entwurfsprinzipien Ã¼bersetzt: maximale Erreichbarkeit der
	Batterie, ihrer Sensorik und KÃ¼hlsysteme ist zwingende Voraussetzung fÃ¼r den
	adaptiven Betrieb des AI-PMM.

	Es wird ein quantitatives ErreichbarkeitsmaÃŸ $\mathcal{A}$ definiert und
	gezeigt, dass ein topologisch optimierter Motorraum das ErreichbarkeitsmaÃŸ
	gegenÃ¼ber einem konventionellen Layout um den Faktor $\Phi_\mathcal{A} \geq 2{,}3$
	steigert. FÃ¼r EV-Architekturen wird ein Sicherheits- und Wartungskorridor-Konzept
	vorgestellt, das thermische Entkopplung, Hochvoltschutz und vollstÃ¤ndige
	ZugÃ¤nglichkeit aller AI-PMM-relevanten Komponenten vereint. Formale StabilitÃ¤ts-
	und ZugÃ¤nglichkeitsbeweise sowie Simulationsergebnisse belegen die Ãœberlegenheit
	des vorgestellten Entwurfs.
\end{abstract}

\vspace{1cm}
\noindent\textbf{SchlÃ¼sselwÃ¶rter:} Motorraum-Entwurf, ZugÃ¤nglichkeitsoptimierung,
...
$0{,}3\text{--}0{,}8\,\mathrm{m^3}$ mÃ¼ssen Antriebsaggregat, KÃ¼hlung,
Hochspannungskomponenten (bei EV), SteuergerÃ¤tenetzwerk, Fluidsysteme und
Sensorik koexistieren. Gleichzeitig stellen Wartbarkeit, Notfallzugang und
die zunehmende Systemdichte fÃ¼r Elektrofahrzeuge fundamental neue Anforderungen
an die Raumordnung des Motorraums.

Klassische Entwurfsmethoden orientieren sich primÃ¤r an Bauraumrestriktionen
und Crashpfadgeometrien -- ZugÃ¤nglichkeit wird als nachgelagertes Kriterium
behandelt \cite{HeiÃŸing2013}. Diese Arbeit kehrt die PrioritÃ¤t um:
\textit{ZugÃ¤nglichkeit ist ein primÃ¤res Entwurfsziel}, das die gesamte
topologische Struktur des Motorraums bestimmt.

FÃ¼r Elektrofahrzeuge gewinnt diese Perspektive eine neue QualitÃ¤t: Das in
\cite{Epp2026liion} formal bewiesene KI-gesteuerte Power-Management-Modul
(AI-PMM) fÃ¼r Lithium-Ionen-Batterien erfordert physischen Zugang zu Zellsensorik,
KÃ¼hlkreislÃ¤ufen und Hochvoltschnittstellen fÃ¼r Kalibrierung, Diagnose und
Im-Betrieb-Parametrierung. Ein Motorraum, der diese ZugÃ¤nglichkeit nicht
geometrisch verankert, verhindert den optimalen Betrieb des AI-PMM und
damit die nachgewiesene LebensdauerverlÃ¤ngerung um den Faktor $\Phi \geq 1{,}47$
\cite{Epp2026liion}.

\subsection{Zielsetzung und Beitrag}

Diese Arbeit leistet folgende wissenschaftliche BeitrÃ¤ge:

\begin{itemize}[leftmargin=*]
	\item Formale Definition eines ErreichbarkeitsmaÃŸes $\mathcal{A}$ fÃ¼r
	Kraftfahrzeug-MotorrÃ¤ume unter BerÃ¼cksichtigung von Sicherheits- und
	Geometrieconstraints.
	\item Entwurf eines zugÃ¤nglichkeitsoptimierten Motorraum-Layouts fÃ¼r
	...
	formaler Ableitung der ZugÃ¤nglichkeitsanforderungen aus den Ergebnissen
	von \cite{Epp2026liion}.
	\item Quantitativer Nachweis der ZugÃ¤nglichkeitsÃ¼berlegenheit beider
	Architekturen gegenÃ¼ber konventionellen Layouts.
	\item Thermische Analyse und formale StabilitÃ¤tsaussagen fÃ¼r die
	KÃ¼hlsystemarchitektur beider Varianten.
\end{itemize}
...
formalisiert das ErreichbarkeitsmaÃŸ. Abschnitt~\ref{sec:architektur_a} beschreibt
den optimierten Motorraum fÃ¼r Verbrennungsmotor-Fahrzeuge. Abschnitt~\ref{sec:architektur_b}
...
Abschnitt~\ref{sec:simulation} prÃ¤sentiert Simulationsergebnisse, und
...
\begin{definition}[Motorraum-Topologie]
	Der Motorraum $\mathcal{M} \subset \mathbb{R}^3$ ist ein beschrÃ¤nktes,
	zusammenhÃ¤ngendes Volumen mit:
	\begin{equation}
	...
	Komponenten bezeichnet und $\mathcal{M}$ das verbleibende ZugÃ¤nglichkeitsvolumen.
\end{definition}

Die Gesamtvolumenbilanz lautet:

\begin{equation}
	V_\mathrm{gesamt} = \sum_{i=1}^{N} V_i^\mathrm{Komp} + V_\mathrm{ZugÃ¤ngl.}
	+ V_\mathrm{Sicherheit} + V_\mathrm{Thermik}
	\label{eq:volumenbilanz}
\end{equation}

FÃ¼r konventionelle MotorrÃ¤ume gilt typischerweise:
$V_\mathrm{ZugÃ¤ngl.}/V_\mathrm{gesamt} \approx 0{,}12\text{--}0{,}18$,
was auf die geringe Priorisierung der ZugÃ¤nglichkeit im klassischen Entwurf
hinweist.

\subsubsection{Komponentenhierarchie}

Die $N$ Komponenten werden nach AustauschhÃ¤ufigkeit und ZugÃ¤nglichkeitsdringlichkeit
in drei PrioritÃ¤tsklassen eingeteilt:

\begin{definition}[ZugÃ¤nglichkeitsklassen]
	\begin{align}
		\mathcal{C}_1 &= \{\text{Tagesinspektion, BetriebsflÃ¼ssigkeiten, Diagnoseschnittstellen}\}
		\label{eq:klasse1} \\
		\mathcal{C}_2 &= \{\text{Wartung \,<\,50\,000\,km: Filter, Riemen, KÃ¼hler}\}
		\label{eq:klasse2} \\
		\mathcal{C}_3 &= \{\text{GroÃŸreparatur, Grundaggregat-Zugang}\}
		\label{eq:klasse3}
	\end{align}
\end{definition}

Das Entwurfsprinzip verlangt: $\mathcal{C}_1$-Komponenten mÃ¼ssen ohne Werkzeug
...
$\mathcal{C}_3$-Komponenten bei vollstÃ¤ndig geÃ¶ffnetem Motorraum.

\subsection{Thermische Grundlagen}

\subsubsection{WÃ¤rmezonierung}

Der Motorraum wird in thermische Zonen $\mathcal{Z}_k$ unterteilt:

\begin{equation}
	\mathcal{M} = \bigsqcup_{k=1}^{K} \mathcal{Z}_k, \quad
	\mathcal{Z}_k \cap \mathcal{Z}_l = \emptyset \text{ fÃ¼r } k \neq l
	\label{eq:zonenzerlegung}
\end{equation}

Die charakteristische Temperatur einer Zone $\mathcal{Z}_k$ unter stationÃ¤ren
...
\subsubsection{WÃ¤rmequellendichte}

\begin{definition}[WÃ¤rmequellendichte]
	Die volumetrische WÃ¤rmequellendichte $q_k$ einer Motorraum-Zone $\mathcal{Z}_k$
	ist definiert als:
	\begin{equation}
		q_k = \frac{\dot{Q}_k}{V_k} \quad [\mathrm{W\,m^{-3}}]
		\label{eq:waermequelldichte}
	\end{equation}
	Hochtemperaturzonen mit $q_k > 50\,\mathrm{kW\,m^{-3}}$ mÃ¼ssen durch
	thermische Barrieren von WartungszugÃ¤ngen und empfindlichen Komponenten
	getrennt werden.
\end{definition}

\subsection{Sicherheitstechnische Grundlagen}

\subsubsection{Hochvoltschutzkonzept fÃ¼r EV}
...
auszuweisen \cite{ISO6469}. Es gilt die ZugÃ¤nglichkeitseinschrÃ¤nkung:
...
FÃ¼r autorisiertes Servicepersonal gelten Mindestzugangswege mit:
$d_\mathrm{Isolierschutz} \geq 25\,\mathrm{mm}$ Kriech- und Luftstrecke.

\subsubsection{Crashpfad-Freihaltegebot}

Nach FMVSS~208 und ECE R94 mÃ¼ssen definierte Deformationszonen im Frontbereich
freibleiben. Diese Crashpfadzonen $\mathcal{V}_\mathrm{crash}$ Ã¼berlagern sich
mit ZugÃ¤nglichkeitszonen:

\begin{equation}
	\mathcal{V}_\mathrm{Komp,zulÃ¤ssig} \cap \mathcal{V}_\mathrm{crash} = \emptyset
	\label{eq:crash_constraint}
\end{equation}

Die Optimierung des Motorraum-Layouts muss beide Nebenbedingungen~\eqref{eq:hv_constraint}
und \eqref{eq:crash_constraint} simultan erfÃ¼llen.

% ============================================================
\section{Formales ErreichbarkeitsmaÃŸ}
\label{sec:erreichbarkeit}
% ============================================================

\subsection{Definition des ErreichbarkeitsmaÃŸes}

\begin{definition}[ErreichbarkeitsmaÃŸ $\mathcal{A}$]
	\label{def:erreichbarkeit}
	Das ErreichbarkeitsmaÃŸ einer Komponente $i$ im Motorraum ist definiert als:
	\begin{equation}
	...
	wobei:
	\begin{itemize}[leftmargin=*]
		\item $d_i \geq 0$: Zugangstiefedefizit $[\mathrm{mm}]$ (Ãœberschuss Ã¼ber
		Klassen-Schwellwert)
		\item $o_i \in [0,1]$: Obstruktionsgrad (normiert; 0 = freier Zugang,
		1 = vollstÃ¤ndig verdeckt)
		\item $\Delta T_i \geq 0$: TemperaturÃ¼berschuss der Zugriffszone Ã¼ber
		$T_\mathrm{safe} = 45\,^\circ\mathrm{C}$ $[\mathrm{K}]$
		\item $\alpha_d, \alpha_o, \alpha_t > 0$: Gewichtungsparameter
	\end{itemize}
\end{definition}

Das Gesamt-ErreichbarkeitsmaÃŸ des Motorraums ist die gewichtete Summe Ã¼ber
alle Klassen:
...
mit KlassenprioritÃ¤tsgewichten $w_1 = 0{,}60$, $w_2 = 0{,}30$, $w_3 = 0{,}10$.
...
\begin{align}
	&\sum_{i} V_i^\mathrm{Komp}(\boldsymbol{\pi}) \leq V_\mathrm{gesamt} - V_\mathrm{min,ZugÃ¤ngl.}
...
\begin{theorem}[Erreichbarkeits-Verbesserungspotenzial]
	...
	Layout und $\boldsymbol{\pi}^*$ das zugÃ¤nglichkeitsoptimierte Layout. Dann gilt:
	\begin{equation}
...
\subsection{Entwurfsprinzipien fÃ¼r Diesel- und Benzin-Fahrzeuge}

\subsubsection{Komponentenzonierung}

Das zugÃ¤nglichkeitsoptimierte Layout fÃ¼r Verbrennungsmotor-Fahrzeuge organisiert
den Motorraum in konzentrische Erreichbarkeitszonen:

\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale=0.85]
		% Ã„uÃŸerer Motorraum-Rahmen
		\draw[very thick, blue!60!black] (0,0) rectangle (12,7);
		\node[font=\small\bfseries] at (6,7.3) {Motorraum (Draufsicht) -- Architektur A (ICE)};

		% Zone 1: C1-ZugÃ¤nglichkeit (vorne, direkt erreichbar)
		...
		% BremsflÃ¼ssigkeit / Waschwasser (links oben)
		...
		% Ã–leinfÃ¼llstutzen (Mitte oben)
		\fill[orange!30] (4.5,5.5) rectangle (7.5,7.0);
		\draw[orange!50, thick] (4.5,5.5) rectangle (7.5,7.0);
		\node[font=\footnotesize, align=center] at (6.0,6.25) {Ã–l / Luftfilter\\(direkt zugÃ¤nglich)};
		...
		\node[font=\footnotesize, gray!60!black] at (6,0.75)
			{Zone~3: $\mathcal{C}_3$ -- Grundaggregat (HebebÃ¼hne erforderlich)};
		...
		\node[font=\footnotesize, align=left] at (6,-0.6)
			{MaÃŸstab: Motorraum $1200 \times 700$mm (symbolisch)};
	\end{tikzpicture}
	\caption{Schematisches zugÃ¤nglichkeitsoptimiertes Motorraum-Layout -- Architektur~(A)
		fÃ¼r Verbrennungsmotor-Fahrzeuge. GrÃ¼ne Zone: $\mathcal{C}_1$-Direktzugang;
...
\begin{enumerate}[leftmargin=*, label=(P\arabic*)]
	...
	direkt hinter der MotorhaubenÃ¶ffnung positioniert. Die Frontschicht umfasst:
	Ã–leinfÃ¼llstutzen, Ã–lmessstab, KÃ¼hlmittelausgleichsbehÃ¤lter, BremsflÃ¼ssigkeitsbehÃ¤lter,
	Scheibenwaschanlage, OBD-II-Diagnoseschnittstelle.

	\item \textbf{Thermische Separationszone:} Die Hochtemperaturkomponenten
	(Motor, AbgaskrÃ¼mmer, Turbolader) werden durch einen IsolationsgÃ¼rtel der
	Mindestbreite $b_\mathrm{iso} = 80\,\mathrm{mm}$ von Wartungszugangspfaden
	getrennt. Die OberflÃ¤chentemperatur in Wartungszonen muss
	...
	die $\mathcal{C}_2$-Komponenten seitlich zugÃ¤nglich machen, ohne den
	Motor ausbauen zu mÃ¼ssen.
\end{enumerate}

\subsection{Thermische Auslegung -- Architektur~(A)}

\subsubsection{WÃ¤rmebilanz}

Die stationÃ¤re WÃ¤rmebilanz des Motorraums lautet:

\begin{equation}
	\dot{Q}_\mathrm{Motor} + \dot{Q}_\mathrm{Nebenaggregate}
	= \dot{Q}_\mathrm{KÃ¼hler} + \dot{Q}_\mathrm{Konvektion}
	+ \dot{Q}_\mathrm{Strahlung}
	\label{eq:waermebilanz_a}
\end{equation}

FÃ¼r einen Benzin-Vierzylinder mit $P_\mathrm{mech} = 120\,\mathrm{kW}$ und
...
Davon entfallen typischerweise $60\,\%$ auf den KÃ¼hlkreislauf und
$40\,\%$ auf Abgas und Strahlungsverluste.

\subsubsection{Konvektionsverhalten im ZugÃ¤nglickeitskanal}

Der thermische Zustand im $\mathcal{C}_1$-ZugÃ¤nglichkeitsbereich wird durch
erzwungene Konvektion Ã¼ber den KÃ¼hlerluftdurchsatz $\dot{V}_\mathrm{Luft}$ bestimmt:
...
FÃ¼r den Auslegungsfall ($T_\mathrm{amb} = 35\,^\circ\mathrm{C}$,
...
Umgebungstemperatur annimmt -- ein entscheidender Vorteil gegenÃ¼ber konventionellen
Layouts, wo $\mathcal{C}_1$-Komponenten oft in der thermischen Einflusszone des
Motors platziert sind.

\subsection{Spezifika fÃ¼r Dieselmotoren}

Diesel-Triebwerke unterscheiden sich von Benzinmotoren in drei fÃ¼r den
Motorraum-Entwurf relevanten Eigenschaften:

\begin{itemize}[leftmargin=*]
	\item \textbf{HÃ¶herer Abgasgegendruck:} Der Turbolader (Pflichtbestandteil
	moderner Diesel) erzeugt eine zusÃ¤tzliche Hochtemperaturquelle
	($T_\mathrm{Turbo,surface} \leq 700\,^\circ\mathrm{C}$). Der IsolationsgÃ¼rtel
	wird auf $b_\mathrm{iso} = 120\,\mathrm{mm}$ erweitert.
	\item \textbf{Common-Rail-System:} Die Hochdruckkraftstoffpumpe und die
	EinspritzdÃ¼sen ($p \leq 2500\,\mathrm{bar}$) erfordern eigene
	...
	bevorzugt in der Frontschicht (Zone~1) positioniert, da regelmÃ¤ÃŸiges
	NachfÃ¼llen erforderlich ist.
\end{itemize}
...
\subsubsection{Ergebnisse aus \cite{Epp2026liion} und ihre rÃ¤umlichen Implikationen}

Das in \cite{Epp2026liion} formal bewiesene KI-Power-Management-Modul (AI-PMM)
fÃ¼r Lithium-Ionen-Batterien erzielt eine LebensdauerverlÃ¤ngerung von
$\Phi \geq 1{,}47$ durch drei RegelungskanÃ¤le:

\begin{enumerate}[leftmargin=*, label=(\Roman*)]
	\item \textbf{Thermischer Regelkanal:} PrÃ¤diktive Temperaturregelung mit
	PID + Feedforward-Struktur. Mittlere Temperaturreduktion:
	$\overline{\Delta T} = 8\,\mathrm{K}$.
	RÃ¤umliche Implikation: \textit{KÃ¼hl\-system-Servicepunkte mÃ¼ssen in $\mathcal{C}_1$
	erreichbar sein.}

	\item \textbf{Spannungsregelkanal:} Model Predictive Control (MPC) mit
	Horizont $N_p = 10$ fÃ¼r optimale Lade-/Entladekurven.
	RÃ¤umliche Implikation: \textit{Hochvolt-Diagnoseschnitt\-stellen (HV-BMS)
	mÃ¼ssen autorisierten $\mathcal{C}_2$-Zugang erhalten.}

	\item \textbf{Mechanischer Plattenabstandsregelkanal:} Adaptive Druckregelung
	des Separators ($p^* \in [0{,}2, 0{,}8]\,\mathrm{MPa}$) fÃ¼r mechanische
	Entlastung der Zellen. RÃ¤umliche Implikation: \textit{AktuatorzugÃ¤nge fÃ¼r
	Plattenabstandssteller in $\mathcal{C}_2$; optische Schwellungsdiagnose in $\mathcal{C}_1$.}
\end{enumerate}

Die rÃ¤umlichen Anforderungen des AI-PMM an den EV-Motorraum werden in
Tabelle~\ref{tab:aipmm_anforderungen} zusammengefasst.

\begin{sidewaystable}
	\centering
	\caption{AI-PMM-ZugÃ¤nglichkeitsanforderungen fÃ¼r den EV-Motorraum}
	\label{tab:aipmm_anforderungen}
	\begin{tabular}{lllc}
		\toprule
		\textbf{Komponente} & \textbf{Funktion} & \textbf{Klasse} & \textbf{Mindest-Zugangsbr. [mm]} \\
		\midrule
		BMS-Diagnoseschnittstelle & KI-ParameterÃ¼bertragung & $\mathcal{C}_1$ & 80 \\
		KÃ¼hlmittelausgleichsbehÃ¤lter & KÃ¼hlkreis-Wartung & $\mathcal{C}_1$ & 100 \\
		KÃ¼hlpumpe (Elektro) & Instandhaltung & $\mathcal{C}_2$ & 200 \\
		...
		HV-SchÃ¼tz / Sicherung & Notabschaltung & $\mathcal{C}_1$ & 120 \\
		WÃ¤rmemanagementsystem (PTC/Chiller) & Thermik-Service & $\mathcal{C}_2$ & 200 \\
		Pack-Struktur (Grundeinheit) & Zellwechsel & $\mathcal{C}_3$ & 400 \\
		\bottomrule
	\end{tabular}
\end{sidewaystable}

\subsubsection{Thermische Kopplung: AI-PMM-KÃ¼hlregelung und Motorraum-Archi\-tektur}

Das AI-PMM regelt die Zelltemperatur auf $T^* = T_\mathrm{opt} \approx 25\text{--}30\,^\circ\mathrm{C}$
\cite{Epp2026liion}. Der Motorraum-Entwurf muss diesen Regelbereich unterstÃ¼tzen:

\begin{equation}
	T_\mathrm{Batterieraum}(t) = T_\mathrm{amb}(t) - \eta_\mathrm{KÃ¼hlung}
	\cdot \left[T_\mathrm{amb}(t) - T^*\right] + \varepsilon(t)
	\label{eq:batt_temp_eq}
\end{equation}

wobei $\eta_\mathrm{KÃ¼hlung} \in [0,1]$ der thermische Wirkungsgrad des
KÃ¼hlsystems und $\varepsilon(t)$ die Regelabweichung ist. Die AI-PMM-StabilitÃ¤tsbedingung
aus \cite{Epp2026liion} (Theorem~5.2, Lyapunov-StabilitÃ¤tsanalyse) erfordert:

\begin{equation}
	|\varepsilon(t)| \leq \varepsilon_\mathrm{max} = 5\,\mathrm{K}
	\quad \Leftrightarrow \quad
	\eta_\mathrm{KÃ¼hlung} \geq 1 - \frac{\varepsilon_\mathrm{max}}{T_\mathrm{amb,max} - T^*}
	\label{eq:kuehlwirkungsgrad}
\end{equation}

FÃ¼r $T_\mathrm{amb,max} = 45\,^\circ\mathrm{C}$ und $T^* = 28\,^\circ\mathrm{C}$:

\begin{equation}
	\eta_\mathrm{KÃ¼hlung} \geq 1 - \frac{5}{45 - 28} \approx 0{,}706
	\label{eq:kuehlwirkungsgrad_num}
\end{equation}

Diese Mindestanforderung an den KÃ¼hlwirkungsgrad Ã¼bersetzt sich direkt in eine
MindestkÃ¼hlleistung, die der Motorraum-Entwurf berÃ¼cksichtigen muss.
...
Entkopplung, Hochvoltschutz und AI-PMM-ZugÃ¤nglichkeit vereint:
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.85]
		...
		\node[font=\small\bfseries] at (6.5,9.3)
			{EV-Motorraum -- Architektur~(B): AI-PMM-kompatibles Layout (LÃ¤ngsschnitt)};
		...
		\node[font=\footnotesize, align=center] at (1.5,8.0)
			{KÃ¼hlmittel-\\ausgleich\\+ FÃ¼llstand};
		...
		\node[font=\footnotesize, align=center] at (6.95,8.0)
			{HV-SchÃ¼tz +\\Notabschaltung\\(Service-Stecker)};
		...
		\node[font=\footnotesize, align=center] at (11.9,8.0)
			{Wasser-\\kÃ¼hl-\\eingang};

		% Schicht 2: Thermischer TrenngÃ¼rtel
		...
		\node[font=\small, red!60!black] at (6.5,6.25)
			{Schicht~2: Thermischer TrenngÃ¼rtel + Hochvolt-Sicherheitszone (HV-Bus)};
		\node[font=\footnotesize, red!50!black] at (6.5,5.8)
			{Kriechstrecke $\geq 25\,$mm Â· Luftstrecke $\geq 25\,$mm Â· $T_\mathrm{surface} \leq 45\,^\circ$C};
	...
	\end{tikzpicture}
	\caption{EV-Motorraum-Architektur~(B) im LÃ¤ngsschnitt. Dreischicht-Topologie:
		Schicht~1 (Servicefronte, $\mathcal{C}_1$), Schicht~2 (thermischer TrenngÃ¼rtel,
...
\begin{enumerate}[leftmargin=*, label=(E\arabic*)]
	\item \textbf{AI-PMM-Servicefront:} Alle fÃ¼r den Betrieb und die Diagnose des
	AI-PMM erforderlichen Schnittstellen -- BMS-Diagnosekonnektoren, KÃ¼hlmittel-
	...
	ermÃ¶glicht die direkte ParameterÃ¼bertragung fÃ¼r das Transferlernmodul des
	AI-PMM \cite{Epp2026liion}.

	\item \textbf{Thermischer TrenngÃ¼rtel (TTG):} Schicht~2 bildet eine thermische
	und elektrische Barriere. Der TTG erfÃ¼llt gleichzeitig die Isolationsanforderungen
	nach ISO~6469-3 (Kriechstrecken) und die Strahlungsschutzanforderungen fÃ¼r
	die Batterie ($\Delta T_\mathrm{Strahlung} \leq 2\,\mathrm{K}$ auf PackoberflÃ¤che
	bei Volllast E-Motor).

	\item \textbf{Integrierte KÃ¼hlplattentopologie:} Der Batteriepack in Schicht~3
	wird durch parallele KÃ¼hlplatten-Gassen ($N_\mathrm{Gassen} = 7$) vertikal
	erschlossen. Die KÃ¼hlplattengeometrie unterstÃ¼tzt den dynamischen Plattenabstands-
	regler des AI-PMM \cite{Epp2026liion}: Aktuatoren fÃ¼r den Separator-Anpressdruck
	sind in die KÃ¼hlplatten-Rahmenstruktur integriert und seitlich Ã¼ber die
	...
	freigehalten. Diese Korridore ermÃ¶glichen den Zugang zu Zelldruck-Aktuatoren,
	KÃ¼hlpumpe und WÃ¤rmemanagementkomponenten ohne Demontage des Batteriepacks.
\end{enumerate}

\subsection{Hochvoltschutz-Architektur}

Das HV-Netz des EV-Motorraums wird in drei Segmente mit abgestuften
ZugÃ¤nglichkeitseinschrÃ¤nkungen aufgeteilt:

\begin{align}
	\mathcal{V}_\mathrm{HV}^{(1)} &: U \leq 800\,\mathrm{V}, \quad
	\text{Zugang nur fÃ¼r autorisiertes Personal mit HV-AusrÃ¼stung}
	\label{eq:hv_seg1} \\
	\mathcal{V}_\mathrm{HV}^{(2)} &: U \leq 60\,\mathrm{V}, \quad
	\text{Zugang mit EinpersonenschutzmaÃŸnahmen (Handschuhe Cat\,0)}
	\label{eq:hv_seg2} \\
...
sperrt den Batteriepack fÃ¼r $\mathcal{C}_2$-Zugang frei.
...
der ErreichbarkeitsmaÃŸe $\mathcal{A}_i$ fÃ¼r typische $\mathcal{C}_1$-Komponenten
...
\begin{itemize}[leftmargin=*]
	\item Zugangstiefedefizit: $d_i = 180\,\mathrm{mm}$ (Ãœbertiefe hinter anderen
	Bauteilen)
	\item Obstruktionsgrad: $o_i = 0{,}45$ (teilweise durch KÃ¼hlschlÃ¤uche verdeckt)
	\item TemperaturÃ¼berschuss: $\Delta T_i = 18\,\mathrm{K}$ (in MotorwÃ¤rmebereich)
\end{itemize}
...
\begin{itemize}[leftmargin=*]
	\item Zugangstiefedefizit: $d_i = 0\,\mathrm{mm}$ (Frontschicht-Prinzip erfÃ¼llt)
	\item Obstruktionsgrad: $o_i = 0{,}05$ (nur minimale KabelfÃ¼hrung)
	\item TemperaturÃ¼berschuss: $\Delta T_i = 1{,}2\,\mathrm{K}$ (thermische Separationszone)
\end{itemize}
...
gilt die Ungleichung \eqref{eq:erreichbarkeits_theorem} fÃ¼r alle relevanten
Fahrzeugtopologien. \hfill$\square$
\end{proof}

\subsection{Korollar: AI-PMM-ZugÃ¤nglichkeitsgarantie}

\begin{corollary}[AI-PMM-ZugÃ¤nglichkeitsgarantie]
	\label{cor:aipmm_access}
	Unter der Architektur~(B) mit Prinzipien (E1)--(E4) ist gewÃ¤hrleistet, dass
	alle AI-PMM-relevanten Komponenten aus Tabelle~\ref{tab:aipmm_anforderungen}
	ihr MindestzugÃ¤nglichkeitsmaÃŸ erfÃ¼llen:
	\begin{equation}
		\mathcal{A}_i \geq \mathcal{A}_i^\mathrm{min} \quad
		\forall i \in \mathcal{T}_\mathrm{AI-PMM}
		\label{eq:aipmm_access}
	\end{equation}
	Damit ist die physische Voraussetzung fÃ¼r den optimalen Betrieb des AI-PMM
	und die LebensdauerverlÃ¤ngerung $\Phi \geq 1{,}47$ \cite{Epp2026liion} erfÃ¼llt.
\end{corollary}
...
\begin{proof}
	...
	Die $\mathcal{C}_2$-Komponenten sind Ã¼ber die Seitenkorridore mit $b_\mathrm{C2} \geq
	200\,\mathrm{mm} > d_\mathrm{min,C2} = 180\,\mathrm{mm}$ (Tab.~\ref{tab:aipmm_anforderungen})
	erreichbar. Damit sind alle Ungleichungen in \eqref{eq:aipmm_access} erfÃ¼llt.
	\hfill$\square$
\end{proof}
...
\eqref{eq:zonen_temp} zugrunde. Wir vergleichen die stationÃ¤ren Temperaturen
in kritischen Zonen.
...
\begin{table}[H]
	\centering
	\caption{StationÃ¤re Zonentemperaturen -- Architektur~(A), Vollastbetrieb}
	\label{tab:temp_a}
	\begin{tabular}{lcccc}
		\toprule
...
\begin{table}[H]
	\centering
	\caption{StationÃ¤re Zonentemperaturen -- Architektur~(B), Vollastbetrieb}
	\label{tab:temp_b}
	\begin{tabular}{lcccc}
		\toprule
		\textbf{Zone} & $\dot{Q}_k$ [kW] & $R_\mathrm{th,k}$ [K/kW] &
		...
		TrenngÃ¼rtel (TTG)    & 2   & 0{,}50 & $\approx 36$  & Ja \\
...
\cite{Epp2026liion} -- ein direkter Nachweis der thermischen KompatibilitÃ¤t von
Architektur~(B) mit dem AI-PMM.

\subsection{Transienter WÃ¤rmeeintrag und AbkÃ¼hlverhalten}
...
das transiente Verhalten. FÃ¼r die AI-PMM-kritische Batteriepackzone gilt:
...
Dieser Wert liegt gut innerhalb des prÃ¤diktiven Horizont $K$ des AI-PMM-LSTM-Modells
($K \cdot \Delta t = 10\,\mathrm{s} \cdot 60 = 600\,\mathrm{s} = 10\,\mathrm{min}$
\cite{Epp2026liion}), so dass prÃ¤diktive Temperaturregelung mÃ¶glich bleibt.
...
\begin{proposition}[Thermische Regelbarkeit]
	...
	mit dem prÃ¤diktiven Horizont des AI-PMM kompatibel:
	\begin{equation}
		\tau_\mathrm{Batt} \approx K \cdot \Delta t
		\label{eq:tau_kompatibel}
	\end{equation}
	Damit ist das AI-PMM in der Lage, TemperaturÃ¤nderungen im Batteriepack
	prÃ¤diktiv zu kompensieren, bevor sie die Degradationsrate signifikant erhÃ¶hen.
\end{proposition}
...
\begin{itemize}[leftmargin=*]
	...
	\item \textbf{K-B:} Konventionelles EV-Layout ohne AI-PMM-BerÃ¼cksichtigung
	\item \textbf{O-B:} EV-Layout Architektur~(B) mit AI-PMM-Integration
\end{itemize}

Simulierte Kennzahlen: ErreichbarkeitsmaÃŸ $\mathcal{A}_\mathrm{gesamt}$,
ZonenhÃ¶chsttemperatur $T_\mathrm{max}$, Wartungszeit $t_W$ fÃ¼r typische
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			ylabel={ErreichbarkeitsmaÃŸ $\mathcal{A}_\mathrm{gesamt}$},
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Gesamt-ErreichbarkeitsmaÃŸ $\mathcal{A}_\mathrm{gesamt}$ der vier
		Motorraum-Konfigurationen. Architektur~(B) erzielt das hÃ¶chste ErreichbarkeitsmaÃŸ
...
\begin{figure}[H]
	...
	\caption{Zeitlicher Temperaturverlauf in der $\mathcal{C}_1$-Servicezone nach
		...
		$T_\mathrm{safe} = 45\,^\circ\mathrm{C}$. Konventionelles ICE-Layout Ã¼berschreitet
...
\begin{table}[H]
	\centering
	\caption{Vergleich typischer Wartungszeiten fÃ¼r $\mathcal{C}_1$-Interventionen}
	...
	\begin{tabular}{lcccc}
		...
		\midrule
		Ã–lstand prÃ¼fen          & 4{,}2 & 1{,}1 & -- & -- \\
		KÃ¼hlmittel nachfÃ¼llen   & 6{,}8 & 1{,}8 & 7{,}5 & 1{,}5 \\
		BMS-Diagnose auslesen   & -- & -- & 8{,}3 & 1{,}2 \\
		HV-Service-Freischaltung& -- & -- & 5{,}1 & 0{,}8 \\
		BremsflÃ¼ssigkeit prÃ¼fen & 3{,}5 & 0{,}9 & 3{,}8 & 0{,}9 \\
		\midrule
...
Die EV-Architektur~(B) erzielt den hÃ¶chsten Wartungszeitgewinn, da die
AI-PMM-Diagnoseschnittstelle im Vergleich zu konventionellen EV-Layouts
besonders schlecht zugÃ¤nglich ist und durch die Service-Frontzone drastisch
verbessert wird.

\subsection{Batterielebensdauer unter Architektur~(B)}

Durch die thermisch kompatible Architektur~(B) kann das AI-PMM den
Lebensdauervorteil $\Phi \geq 1{,}47$ aus \cite{Epp2026liion} vollstÃ¤ndig
...
wobei $\Phi_\mathrm{thermisch,Motorraum} = 1{,}08$ der zusÃ¤tzliche Lebensdauergewinn
durch die verbesserte thermische Isolierung des Batteriepacks gegenÃ¼ber
konventionellen EV-Layouts ist (reduzierte UmgebungswÃ¤rmeeinleitung,
$\Delta T_\mathrm{passiv} \approx 2\,\mathrm{K}$ gÃ¼nstiger als bei K-B).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			width=12cm, height=7cm,
			xlabel={Zyklen},
			ylabel={Normierte KapazitÃ¤t $Q(t)/Q_0$},
			xmin=0, xmax=2000,
			ymin=0.70, ymax=1.02,
		...
		\end{axis}
	\end{tikzpicture}
	\caption{KapazitÃ¤tsentwicklung fÃ¼r EV-Konfigurationen. Architektur~(B) mit AI-PMM
...
\begin{table}[H]
	...
	\begin{tabular}{lcc}
		...
		\midrule
		ErreichbarkeitsmaÃŸ $\mathcal{A}_\mathrm{gesamt}$ & $0{,}851$ & $0{,}893$ \\
		...
		HV-SicherheitskonformitÃ¤t & n.a. & ISO~6469-3 erfÃ¼llt \\
		AI-PMM-KompatibilitÃ¤t & n.a. & vollstÃ¤ndig \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
	\item Das ErreichbarkeitsmaÃŸ~\eqref{eq:erreichbarkeit_i} setzt lineare
	Additierbarkeit der Kostenterme voraus. Starke NichtlinearitÃ¤ten (z.\,B.\
	hochviskose Fluide bei tiefen Temperaturen) werden nicht erfasst.
	\item Die thermischen Netzwerkmodelle sind stationÃ¤r; hochfrequente
	...
	binÃ¤re Ausschlusszone modelliert; reale Crashzonen weisen gradierte
...
Der AI-PMM-Servicefrontbereich (Schicht~1) kann als AR-Zielpunkt fÃ¼r
Head-Mounted-Display-gefÃ¼hrte Wartungsassistenz dienen. Mittelfristig ist
eine vollstÃ¤ndige Co-Optimierung von Motorraum-Topologie und AI-PMM-Parameterraum
mÃ¶glich: Das AI-PMM erlernt spezifische Korrekturparameter fÃ¼r den
konkreten Motorraum-Thermik-Fingerabdruck des Fahrzeugs. Langfristig
ermÃ¶glicht die Breathing-Cell-Vision aus \cite{Epp2026liion} (Abschnitt~9.5)
eine integration der Plattenabstandsaktuatoren direkt in die KÃ¼hlplatten-Gassen
der Architektur~(B) ohne mechanische NachrÃ¼stung.
...
Diese Arbeit hat einen formalen Entwurfsrahmen fÃ¼r zugÃ¤nglichkeitsoptimierte
Kraftfahr\-zeug-MotorrÃ¤ume vorgestellt und in zwei Architekturen konkretisiert.

FÃ¼r Verbrennungsmotor-Fahrzeuge (\textbf{Architektur~A}) zeigt das
Frontschicht-Prinzip in Kombination mit thermischer Separationszone und
Seitengassenkonzept, dass ein ErreichbarkeitsmaÃŸ von $\mathcal{A} \approx 0{,}85$
-- einem Faktor $2{,}73$ Ã¼ber dem konventionellen Stand -- erreichbar ist.
Wartungszeiten fÃ¼r $\mathcal{C}_1$-Interventionen reduzieren sich im Mittel
auf unter $1{,}3\,\mathrm{min}$.

FÃ¼r Elektrofahrzeuge (\textbf{Architektur~B}) wurde erstmals ein EV-Motorraum-Layout
...
(Schicht~1) stellt die physische ZugÃ¤nglichkeit aller AI-PMM-Schnittstellen sicher.
Der thermische TrenngÃ¼rtel (Schicht~2) isoliert die Batterie von AntriebswÃ¤rme und
hÃ¤lt den AI-PMM-optimalen Arbeitspunkt $T^* \approx 28\,^\circ\mathrm{C}$ passiv.
Die integrierte KÃ¼hlplatten-Gassen-Struktur (Schicht~3) ermÃ¶glicht den Zugang zu
Zelldruck-Aktuatoren fÃ¼r den mechanischen Regelungskanal des AI-PMM.

Das formal nachgewiesene Ergebnis: Unter Architektur~(B) mit vollstÃ¤ndiger
AI-PMM-Integration realisieren Lithium-Ionen-Batterien eine LebensdauerverlÃ¤ngerung
von $\Phi_\mathrm{gesamt} \geq 1{,}59$ gegenÃ¼ber dem Referenzsystem -- eine
Steigerung, die ohne zugÃ¤nglichkeitsoptimierte Motorraum-Architektur physisch
nicht umsetzbar wÃ¤re.

Die vorliegende Arbeit begrÃ¼ndet eine neue ingenieurwissenschaftliche Designdisziplin:
\textit{AI-kompatibler Motorraum-Entwurf} -- die systematische Ko-Entwicklung
von Fahrzeugtopologie und lernenden Regelsystemen, bei der ZugÃ¤nglichkeit und
KI-Betrieb als gleichrangige primÃ¤re Entwurfsziele behandelt werden.
...
\begin{thebibliography}{99}
	...
	S.\ Epp,
	\textit{KI-Power-Management-Modul fÃ¼r Lithium-Ionen-Akkumulatoren:
		Formaler Nachweis der Ãœberlegenheit eines lernenden,
		verhaltensadaptiven Power-Management-Systems}, MÃ¤rz 2026.

	\bibitem{HeiÃŸing2013}
	B.\ HeiÃŸing, M.\ Ersoy, S.\ Gies,
	...
	European Committee for Standardization, BrÃ¼ssel (2015).
	...
	\textit{Methodik zum Entwickeln und Konstruieren technischer Systeme
		und Produkte},
	Verein Deutscher Ingenieure, DÃ¼sseldorf (2019).

	\bibitem{Seiffert2008}
	U.\ Seiffert, G.\ Rainer,
	\textit{Virtuelle Produktentstehung fÃ¼r Fahrzeug und Antrieb im Kfz},
